return {
	id = "AIGUANQIA11",
	mode = 2,
	fadein = 1.5,
	once = True,
	scripts = {
		{
			actor = 10400030,
			nameColor = "#a9f548",
			bgName = "bg_story_ailevel",
			side = 2,
			dir = 1,
			actorName = "Kizuna AI",
			say = "Everyone~! Look, look!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301050,
			side = 2,
			bgName = "bg_story_ailevel",
			nameColor = "#a9f548",
			dir = 1,
			say = "Ooh, you've recovered another outfit!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 201210,
			side = 2,
			bgName = "bg_story_ailevel",
			nameColor = "#a9f548",
			dir = 1,
			say = "Cuuuute~! It's all frilly and princess-like~! AI, it suits you really well!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301110,
			side = 2,
			bgName = "bg_story_ailevel",
			nameColor = "#a9f548",
			dir = 1,
			say = "I see, I see! So this is someone who looks good in everything! I also agree that this is super cute!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_ailevel",
			nameColor = "#a9f548",
			dir = 1,
			actor = 10400030,
			actorName = "Kizuna AI",
			say = "Ehehe~ Thanks, everyone~!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 312010,
			side = 2,
			bgName = "bg_story_ailevel",
			nameColor = "#a9f548",
			dir = 1,
			say = "There's big trouble, nya! This isn't the time to be fawning over clothes, nya!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 101170,
			side = 2,
			bgName = "bg_story_ailevel",
			nameColor = "#a9f548",
			dir = 1,
			say = "Akashi? What's the fuss all of a sudden?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 312010,
			side = 2,
			bgName = "bg_story_ailevel",
			nameColor = "#a9f548",
			dir = 1,
			say = "A new Virtual Mirror Sea has appeared, nya!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 302010,
			side = 2,
			bgName = "bg_story_ailevel",
			nameColor = "#a9f548",
			dir = 1,
			say = "If we leave it alone, the virus will rapidly multiply until it is nearly impossible to contain! Please hurry before we're out of time!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 312010,
			side = 2,
			bgName = "bg_story_ailevel",
			nameColor = "#a9f548",
			dir = 1,
			say = "This Virtual Mirror Sea is a bit different than the ones you've encountered previously, so take this equipment just in case, nya.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 10400030,
			nameColor = "#a9f548",
			bgName = "bg_story_ailevel",
			side = 2,
			dir = 1,
			actorName = "Kizuna AI",
			say = "Thanks, Akashi-chan! Let's get over there proto!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_story_ailevel",
			say = "Virtual Mirror Sea - 4 kilometers above sea level",
			flashN = {
				color = {
					1,
					1,
					1
				},
				alpha = {
					{
						0,
						1,
						0.5
					},
					{
						1,
						0,
						0.5,
						0.5
					}
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 10400030,
			nameColor = "#a9f548",
			bgName = "bg_story_ailevel",
			side = 2,
			dir = 1,
			actorName = "Kizuna AI",
			say = ".........Huh?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_ailevel",
			nameColor = "#a9f548",
			dir = 1,
			actor = 10400030,
			actorName = "Kizuna AI",
			say = "GYAAAAAAAAAAAAAAAAAAAAAHHH! WHY DID WE GET WARPED UP HEREEEEEEEE!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "move",
					y = -2250,
					delay = 0.5,
					dur = 0.5,
					x = 0
				}
			}
		},
		{
			actor = 201210,
			side = 2,
			bgName = "bg_story_ailevel",
			nameColor = "#a9f548",
			dir = 1,
			say = "We're faaaaaaaaaalling------!!!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "move",
					y = -2250,
					delay = 0,
					dur = 1,
					x = 0
				}
			}
		},
		{
			actor = 401230,
			side = 2,
			bgName = "bg_story_ailevel",
			nameColor = "#a9f548",
			dir = 1,
			say = "Woooooooaaaaahhhh!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "move",
					y = -2250,
					delay = 0,
					dur = 1,
					x = 0
				}
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_story_ailevel",
			nameColor = "#a9f548",
			dir = 1,
			actor = 10400030,
			actorName = "Kizuna AI",
			say = "Laffey-channnnnnn~ saaaaaave meeeeeeeeee!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "move",
					y = -2250,
					delay = 0,
					dur = 1,
					x = 0
				}
			}
		},
		{
			actor = 101170,
			side = 2,
			bgName = "bg_story_ailevel",
			nameColor = "#a9f548",
			dir = 1,
			say = "(Unconscious)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "move",
					y = -2250,
					delay = 0,
					dur = 1,
					x = 0
				}
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_story_ailevel",
			nameColor = "#a9f548",
			dir = 1,
			actor = 10400030,
			actorName = "Kizuna AI",
			say = "... She passed out------?!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 401230,
			side = 2,
			bgName = "bg_story_ailevel",
			nameColor = "#a9f548",
			dir = 1,
			say = "*gasp* That's right... now is the perfect time to use that equipment that Akashi gave us!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301050,
			side = 2,
			bgName = "bg_story_ailevel",
			nameColor = "#a9f548",
			dir = 1,
			say = "Come to think of it, we: have something like that... Akashi's prototype parachute, deploying! ...Hopefully!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			say = "………",
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301120,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "Pheww... we somehow managed to land safely onto the water... Thanks, Akashi!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			nameColor = "#a9f548",
			actor = 10400030,
			dir = 1,
			actorName = "Kizuna AI",
			say = "I thought we were:ne for... Thanks, Akashi-chan~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 101170,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "I'm glad the one Akashi gave me was self-deploying... Thanks, Akashi...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 201210,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "Huh? But, uhh, why are we surrounded by some weird fog? It seems to be creeping closer...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 401230,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "Somehow... I have a bad feeling about this...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301110,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "I see, I see! This must be like one of those games where you take damage if you stand inside the cloud!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			nameColor = "#a9f548",
			actor = 10400030,
			dir = 1,
			actorName = "Kizuna AI",
			say = "*shivers* Eh...? Why is this giving me a sense of deja vu?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 22.5,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 3
				}
			}
		},
		{
			expression = 6,
			side = 2,
			nameColor = "#a9f548",
			actor = 10400030,
			dir = 1,
			actorName = "Kizuna AI",
			say = "My super intelligent AI senses are telling me that we won't last anywhere near a fortnight here unless we get to the center of this zone!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301120,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "Let's proceed carefully. Make sure you:n't get caught inside the fog!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
