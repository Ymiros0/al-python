return {
	id = "ANJINBUYECHENG3",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			asideType = 3,
			blackBg = True,
			say = "Prison - Underground Garage",
			sequence = {
				{
					"Prison - Underground Garage",
					1
				},
				{
					"Exit K-03",
					2
				},
				{
					"CAM03 LIVE",
					3
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_543",
			say = "The elevator:or slowly opens.",
			bgm = "story-antarctica-serious",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_543",
			say = "I follow phantom thief Miss Terious into an underground garage beneath the prison.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101452,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_543",
			live2d = True,
			side = 2,
			actorName = "Phantom Thief Miss Terious",
			say = "Now, get ready for the final step of my escape magic~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101452,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_543",
			live2d = True,
			side = 2,
			actorName = "Phantom Thief Miss Terious",
			say = "As soon as we can find a car, it's bye-bye prison!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101452,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_543",
			live2d = True,
			side = 2,
			actorName = "Phantom Thief Miss Terious",
			say = "Ahh, this was so easy that it was almost boring~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101452,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_543",
			live2d = True,
			side = 2,
			actorName = "Phantom Thief Miss Terious",
			say = "Here I was hoping for some more excitement!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_543",
			factiontag = "Commander",
			nameColor = "#A9F548FF",
			say = "Allen? What are you talking about?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101452,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_543",
			live2d = True,
			side = 2,
			actorName = "Phantom Thief Miss Terious",
			say = "Oh, it's no big deal. I just expected better security given the layout of this place.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101452,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_543",
			live2d = True,
			side = 2,
			actorName = "Phantom Thief Miss Terious",
			say = "If I were them, I'd put an ambush here and let the two escapees fall right into the palm of my hand.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "???",
			nameColor = "#FF9B93",
			blackBg = True,
			say = "Is that so?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "???",
			nameColor = "#FF9B93",
			blackBg = True,
			say = "I thought you'd show us some more interesting tricks. How disappointing.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "???",
			nameColor = "#FF9B93",
			blackBg = True,
			say = "Allen M. Sumner... Or should I say Miss Terious?!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_543",
			factiontag = "Phantom Thief Miss Terious",
			dir = 1,
			actor = 101452,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Ack! Inspector Monarch... You again!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 12,
			side = 2,
			bgName = "star_level_bg_543",
			factiontag = "Phantom Thief Miss Terious",
			dir = 1,
			actor = 101452,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Look how shrewd you're being. You want to disrupt things this late in the game?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_543",
			factiontag = "City Inspector",
			dir = 1,
			actor = 299023,
			nameColor = "#FF9B93",
			say = "I: my best to be a spectator, but sometimes, circumstances dictate that I must act.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_543",
			say = "Monarch – the city's police inspector – turns her eyes toward me.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_543",
			factiontag = "City Inspector",
			dir = 1,
			actor = 299023,
			nameColor = "#FF9B93",
			say = "My dear friend, my old partner...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_543",
			factiontag = "City Inspector",
			dir = 1,
			actor = 299023,
			nameColor = "#FF9B93",
			say = "Hahaha... What a sad state of affairs.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "Excuse me?",
					flag = 1
				}
			}
		},
		{
			actor = 299023,
			side = 2,
			bgName = "star_level_bg_543",
			factiontag = "City Inspector",
			dir = 1,
			nameColor = "#FF9B93",
			say = "...Ahem. Sorry, but just go with it for now.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_543",
			factiontag = "City Inspector",
			dir = 1,
			actor = 299023,
			nameColor = "#FF9B93",
			say = "We can discuss old times later. For now, there's something more important that must be:ne.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_543",
			factiontag = "City Inspector",
			dir = 1,
			actor = 299023,
			nameColor = "#FF9B93",
			say = "Allen M. Sumner, you've pilfered a far greater treasure than I anticipated!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_543",
			say = "Monarch raises a hand and snaps.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_543",
			say = "When she:es, the garage's emergency exit is kicked:wn.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_543",
			factiontag = "Security Assault Team Member",
			dir = 1,
			actor = 701111,
			nameColor = "#FF9B93",
			say = "Please:n't move! No more resisting!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_543",
			factiontag = "City Inspector",
			dir = 1,
			actor = 299023,
			nameColor = "#FF9B93",
			say = "\"A friend in need is a friend indeed.\" You taught me this.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_543",
			factiontag = "Phantom Thief Miss Terious",
			dir = 1,
			actor = 101452,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "(whispering) Wow, she's really got it out for you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "Excuse me? (again)",
					flag = 1
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_543",
			factiontag = "Phantom Thief Miss Terious",
			dir = 1,
			actor = 101452,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Okay,:. On to plan B!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 8,
			side = 2,
			bgName = "star_level_bg_543",
			factiontag = "Phantom Thief Miss Terious",
			dir = 1,
			actor = 101452,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Now, Artist! Do your thing!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_543",
			say = "Sudden gunfire interrupted the tense confrontation.",
			soundeffect = "event./ui/kaiqiang",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			icon = {
				scale = 2,
				image = "Props/story_chongfengqiang",
				pos = {
					0,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_543",
			say = "The source of it is Svirepy's rather antique machine gun, and it has more than enough firepower to make Monarch and Ognevoy take cover.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 701121,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_543",
			hidePaintObj = True,
			actorName = "Artist",
			side = 2,
			say = "Hahahaha! Fight me, assholes!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_543",
			say = "Thanks to her merciless fire, the tables turn back in favor of the phantom thief and her companion.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 8,
			side = 2,
			bgName = "star_level_bg_543",
			factiontag = "Phantom Thief Miss Terious",
			dir = 1,
			actor = 101452,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Alright! Suppressive fire has arrived!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_543",
			factiontag = "Phantom Thief Miss Terious",
			dir = 1,
			actor = 101452,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Now go! Take the Commander with you!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_543",
			hidePaintObj = True,
			actor = 701121,
			actorName = "Artist",
			nameColor = "#A9F548FF",
			say = "Gotcha!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 701121,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_543",
			hidePaintObj = True,
			side = 2,
			actorName = "Artist",
			say = "Symbol of rebellion, Comrade Commander! Come with me!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_543",
			say = "A manhole cover at my feet opens up, and Svirepy jumps right inside.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_543",
			say = "It would be easy to follow her, but I have reasons not to, as well.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "Allen, aren't you going to run?",
					flag = 1
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_543",
			factiontag = "Phantom Thief Miss Terious",
			dir = 1,
			actor = 101452,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "*sigh*... You never change,: you? Heheh.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_543",
			factiontag = "Phantom Thief Miss Terious",
			dir = 1,
			actor = 101452,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Don't worry about me. You just go ahead and flee.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_543",
			factiontag = "Phantom Thief Miss Terious",
			dir = 1,
			actor = 101452,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "...Don't just sit there! That little squirt is setting up her heavy weapon as we speak! If you:n't wanna die,: get in that manhole!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_543",
			say = "I look over to the pillar and see that Ognevoy is, indeed, assembling that same ridiculous weapon she'd fired at the helicopter.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_543",
			factiontag = "Commander",
			nameColor = "#A9F548FF",
			say = "(So I have no choice...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "(Go into the manhole.)",
					flag = 1
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_543",
			say = "I thought I'd made up my mind, but before jumping in, I turn to look at Monarch and Ognevoy again.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_543",
			say = "My eyes meet those of my former friend, the police inspector.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_543",
			factiontag = "City Inspector",
			dir = 1,
			actor = 299023,
			nameColor = "#FF9B93",
			say = "Dear friend!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_543",
			factiontag = "City Inspector",
			dir = 1,
			actor = 299023,
			nameColor = "#FF9B93",
			say = "It's not too late to: the right thing!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 701111,
			side = 2,
			bgName = "star_level_bg_543",
			factiontag = "Security Assault Team Member",
			dir = 1,
			nameColor = "#FF9B93",
			say = "Full poweeeer!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_543",
			say = "Somebody closes the manhole cover, and before long, I hear nothing but an explosion.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_543",
			say = "In the dark silence, that phantom's voice comes to me again.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_543",
			factiontag = "Illusion",
			dir = 1,
			actor = 9702070,
			nameColor = "#A9F548FF",
			say = "It's not over yet. Commander, it's not over.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			effects = {
				{
					active = True,
					name = "juqing_mengjing"
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_543",
			factiontag = "Illusion",
			dir = 1,
			actor = 9702070,
			nameColor = "#A9F548FF",
			say = "Go to the Manjuu Tower.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			actor = 9702070,
			factiontag = "Illusion",
			dir = 1,
			blackBg = True,
			nameColor = "#A9F548FF",
			say = "Go to the Manjuu Tower...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			blackBg = True,
			say = ".........",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			blackBg = True,
			say = "......",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
