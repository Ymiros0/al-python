return {
	id = "ANJINBUYECHENG5",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_145",
			say = "Sleepless City - Block 13, Abandoned Arcade",
			bgm = "story-antarctica-serious",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_145",
			say = "We hide out in several places for nearly an hour until we finally arrive at a safe house.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_145",
			say = "It's an abandoned arcade. Svirepy, the Artist, seems to know a lot about this place.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_145",
			say = "Apparently looking for a change of pace, she continues to fiddle with a barely-operating crane game.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_145",
			say = "...She keeps trying, but she fails to win anything.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_145",
			say = "Realizing the tokens in her pocket have run out, she turns around to face me.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 701121,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_145",
			expression = 3,
			side = 2,
			actorName = "Artist",
			say = "Tch. Too much going on today to focus.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 701121,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_145",
			expression = 1,
			side = 2,
			actorName = "Artist",
			say = "I was gonna ask you if you wanted to play, but too bad.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_145",
			say = "She shrugs and leans against a shelf covered in graffiti.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_145",
			factiontag = "Commander",
			nameColor = "#A9F548FF",
			say = "(Is she... trying to chat with me, or?)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "Svire... Ahem. I:n't think I've gotten your name, stranger.",
					flag = 1
				}
			}
		},
		{
			actor = 701121,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_145",
			expression = 1,
			side = 2,
			actorName = "Artist",
			say = "Hmph. Names:n't mean much in this city.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 701121,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_145",
			expression = 4,
			side = 2,
			actorName = "Artist",
			say = "...D-don't look at me like that. Fine, it's Svirepy.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 701121,
			side = 2,
			bgName = "star_level_bg_145",
			factiontag = "Artist",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Ahem. I'd prefer if you used my pseudonym, though.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_145",
			factiontag = "Artist",
			dir = 1,
			actor = 701121,
			nameColor = "#A9F548FF",
			say = "I'm the Artist. I use this city as a canvas, painting over it with the rule of art.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_145",
			factiontag = "Commander",
			nameColor = "#A9F548FF",
			say = "Hmm... So that graffiti there is one of your rules?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 701121,
			side = 2,
			bgName = "star_level_bg_145",
			factiontag = "Artist",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Hahaha, close enough! Let's make it simpler – I'M the rules here.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_145",
			factiontag = "Artist",
			dir = 1,
			actor = 701121,
			nameColor = "#A9F548FF",
			say = "Rules only have meaning when you enforce them. This is my territory, so I could just break into this game and take the prizes if I wanted.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 701121,
			side = 2,
			bgName = "star_level_bg_145",
			factiontag = "Artist",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "But that breaks a rule, and that's lame as hell.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "So your philosophy is that we should all follow the rules?",
					flag = 1
				},
				{
					content = "But aren't you painting over the rules?",
					flag = 2
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_145",
			factiontag = "Artist",
			dir = 1,
			optionFlag = 1,
			actor = 701121,
			nameColor = "#A9F548FF",
			say = "Humans make the rules, and humans enforce them.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 701121,
			side = 2,
			bgName = "star_level_bg_145",
			factiontag = "Artist",
			dir = 1,
			optionFlag = 1,
			nameColor = "#A9F548FF",
			say = "I'm no exception!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 701121,
			side = 2,
			bgName = "star_level_bg_145",
			factiontag = "Artist",
			dir = 1,
			optionFlag = 2,
			nameColor = "#A9F548FF",
			say = "Ahahaha! C'mon, I gotta leave some canvas for other artists.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 701121,
			side = 2,
			bgName = "star_level_bg_145",
			factiontag = "Artist",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Most of all, I was born and raised in this city. I love it like I love myself.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_145",
			factiontag = "Artist",
			dir = 1,
			actor = 701121,
			nameColor = "#A9F548FF",
			say = "I just can't stand the scum that pollute it and act like they're better than us.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_145",
			factiontag = "Artist",
			dir = 1,
			actor = 701121,
			nameColor = "#A9F548FF",
			say = "...Mm?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_145",
			factiontag = "Artist",
			dir = 1,
			actor = 701121,
			nameColor = "#A9F548FF",
			say = "Someone's eavesdropping on us.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "???",
			bgName = "star_level_bg_145",
			nameColor = "#FEF15E",
			say = "Aww, you got me...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_145",
			factiontag = "Artist",
			dir = 1,
			actor = 701121,
			nameColor = "#A9F548FF",
			say = "Quit hiding, already. Show yourself!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_145",
			say = "While Svirepy talks about her ideals, she suddenly becomes alert and yells at the arcade:or.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_145",
			say = "When she:es, the dusty:or opens with a creak.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_145",
			say = "From it appears a girl wearing detective clothes and holding bread in her mouth.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 707011,
			nameColor = "#FEF15E",
			bgName = "star_level_bg_145",
			expression = 5,
			side = 2,
			actorName = "Private Detective",
			say = "G-good evening~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 707011,
			side = 2,
			bgName = "star_level_bg_145",
			factiontag = "Private Detective",
			dir = 1,
			nameColor = "#FEF15E",
			say = "Don't be so tense~ As you can see, I'm just a private detective...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_145",
			factiontag = "Private Detective",
			dir = 1,
			actor = 707011,
			nameColor = "#FEF15E",
			say = "U-umm... I TOTALLY agree with you, Svirepy!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 707011,
			side = 2,
			bgName = "star_level_bg_145",
			factiontag = "Private Detective",
			dir = 1,
			nameColor = "#FEF15E",
			say = "The rules of this city need to be changed from the bottom up!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_145",
			factiontag = "Private Detective",
			dir = 1,
			actor = 707011,
			nameColor = "#FEF15E",
			say = "...I've finally found you, legend of the Sleepless City.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "You mean me?",
					flag = 1
				}
			}
		},
		{
			actor = 707011,
			side = 2,
			bgName = "star_level_bg_145",
			factiontag = "Private Detective",
			dir = 1,
			nameColor = "#FEF15E",
			say = "Don't be surprised. You're more famous in this city than you think.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_145",
			factiontag = "Private Detective",
			dir = 1,
			actor = 707011,
			nameColor = "#FEF15E",
			say = "Famous Commander of the security force, famous:-gooder of:wntown... You're all over the history of this city. *munch*",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_145",
			say = "The oddly familiar private detective Volga takes a bite of her bread before turning serious.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 707011,
			side = 2,
			bgName = "star_level_bg_145",
			factiontag = "Private Detective",
			dir = 1,
			nameColor = "#FEF15E",
			say = "...I come with an invitation.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_145",
			factiontag = "Private Detective",
			dir = 1,
			actor = 707011,
			nameColor = "#FEF15E",
			say = "My client is willing to take you to the Manjuu Tower – as long as you're willing to meet with her.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "She won't take no for an answer, will she?",
					flag = 1
				}
			}
		},
		{
			actor = 707011,
			side = 2,
			bgName = "star_level_bg_145",
			factiontag = "Private Detective",
			dir = 1,
			nameColor = "#FEF15E",
			say = "I-I wonder... Can I take that as a yes, though?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_145",
			factiontag = "Private Detective",
			dir = 1,
			actor = 707011,
			nameColor = "#FEF15E",
			say = "Hehehe, you're just like my client said~ She knew you wouldn't refuse.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 707011,
			side = 2,
			bgName = "star_level_bg_145",
			factiontag = "Private Detective",
			dir = 1,
			nameColor = "#FEF15E",
			say = "This means I lost my bet, though... It's your victory, subordinate of the Queen.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 701101,
			nameColor = "#FEF15E",
			bgName = "star_level_bg_145",
			expression = 2,
			side = 2,
			actorName = "Contact",
			say = "The Queen never loses, Detective.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_145",
			say = "A voice, full of firm resolve, was accompanied by the appearance of another girl from the:or.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 701101,
			nameColor = "#FEF15E",
			bgName = "star_level_bg_145",
			expression = 4,
			side = 2,
			actorName = "Contact",
			say = "Do not:ubt the Queen's decisions.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_145",
			factiontag = "Private Detective",
			dir = 1,
			actor = 707011,
			nameColor = "#FEF15E",
			say = "Oh, I-I'm sorry...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_145",
			factiontag = "Private Detective",
			dir = 1,
			actor = 707011,
			nameColor = "#FEF15E",
			say = "Anyway, I should go... Commander, you just work with the Contact from now on.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 707011,
			side = 2,
			bgName = "star_level_bg_145",
			factiontag = "Private Detective",
			dir = 1,
			nameColor = "#FEF15E",
			say = "Oh, and I'll get rid of any trace that we met here~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 701101,
			nameColor = "#FEF15E",
			bgName = "star_level_bg_145",
			expression = 1,
			side = 2,
			actorName = "Contact",
			say = "Right. Come with me.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
