return {
	id = "BIANHUAKAIANXIANGLAI4",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_530",
			hidePaintObj = True,
			say = "Somewhere in the Iris Orthodoxy",
			bgm = "theme-vichy-church",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_530",
			hidePaintObj = True,
			say = "In a small town, divorced from the hustle and bustle of the big city...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_530",
			hidePaintObj = True,
			say = "A quiet, peaceful place teeming with nature...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 202081,
			side = 2,
			bgName = "star_level_bg_530",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "No unusual radio waves, no secret underground rooms, and no signs of suspicious folk.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_530",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 202081,
			say = "Likewise here.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 202081,
			side = 2,
			bgName = "star_level_bg_530",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Tch... It's such a pain to work without clues. My only option is to comb the entire place.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 202081,
			side = 2,
			bgName = "star_level_bg_530",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...Still, I can't help but notice how friendly Master has gotten with Cardinal Richelieu's entourage of late.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_530",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 202081,
			say = "Did she get a hold of your weaknesses? Or did you let a secret slip?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 202081,
			side = 2,
			bgName = "star_level_bg_530",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Either way, I need to find the cause immediately.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_530",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 202081,
			say = "Clemenceau is a dangerous woman. If you're not careful, you may end up her prey – possibly in more ways than one.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 202081,
			side = 2,
			bgName = "star_level_bg_530",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...Hm?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = 907010,
			side = 2,
			bgName = "star_level_bg_530",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actorName = "Joffre",
			say = "If it isn't Sheffield of the Royal Navy. Fancy meeting you here.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 202081,
			side = 2,
			bgName = "star_level_bg_530",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Joffre... Angel of the Tribunal.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_530",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 907010,
			say = "As I understand, you're responsible for guarding the Royal Navy's representatives.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_530",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 907010,
			say = "What could possibly bring you out to this town?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_530",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 202081,
			say = "Well, the Tribunal's venue security is perfect. It left me without any work to:.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 202081,
			side = 2,
			bgName = "star_level_bg_530",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "So I thought I'd sightsee and make full use of my per-diem allowance.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_530",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 907010,
			say = "Well, thank you for your appraisal of our security.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_530",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 907010,
			say = "However... Abandoning your post is hardly noble.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_530",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 907010,
			say = "Of course, the Tribunal and I:n't mean to spoil our guests' fun. Especially not when they're clearly so interested in our attractions.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_530",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 907010,
			say = "Out of deference to the blessed Iris for uniting us on this day, I'll show you my favorite attraction.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_530",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 907010,
			say = "Come, come. Right this way.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
