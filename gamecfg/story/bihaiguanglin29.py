return {
	fadeOut = 1.5,
	mode = 2,
	id = "BIHAIGUANGLIN29",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_daofeng_6",
			actorName = "{namecode.18}",
			dir = 1,
			bgmDelay = 1,
			say = "This looks like the pedestal of rejuvenation! We'll start off with this one!",
			bgm = "battle-longgong",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "{namecode.18}",
			bgName = "bg_daofeng_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "What the... It's a lot harder to see once you get close to it...!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "{namecode.18}",
			bgName = "bg_daofeng_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "Okay,: as Eugen showed you... Under there... This goes here...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Mysterious Voice",
			bgName = "bg_daofeng_6",
			nameColor = "#ffff4d",
			dir = 1,
			say = "\"Answer me this...\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "{namecode.18}",
			bgName = "bg_daofeng_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "Whuah?! Wh-who said that?!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Mysterious Voice",
			bgName = "bg_daofeng_6",
			nameColor = "#ffff4d",
			dir = 1,
			say = "\"What motivated you to come this far?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "{namecode.18}",
			bgName = "bg_daofeng_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "Well... my friends! They're fighting for dear life right now, and I'm:ing this so I can save them!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Mysterious Voice",
			bgName = "bg_daofeng_6",
			nameColor = "#ffff4d",
			dir = 1,
			say = "\"The treasure of the verdant gales is only for the brave. Yes, you may proceed.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "{namecode.18}",
			bgName = "bg_daofeng_6",
			nameColor = "#a9f548",
			dir = 1,
			blackBg = True,
			say = "I-is that all I had to:...? All right, thank you!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
