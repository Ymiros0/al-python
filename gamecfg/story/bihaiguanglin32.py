return {
	fadeOut = 1.5,
	mode = 2,
	id = "BIHAIGUANGLIN32",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			side = 2,
			nameColor = "#ffff4d",
			bgName = "bg_daofeng_6",
			actorName = "Mysterious Voice",
			dir = 1,
			bgmDelay = 1,
			say = "\"Answer me this final question...\"",
			bgm = "battle-longgong",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Mysterious Voice",
			bgName = "bg_daofeng_6",
			nameColor = "#ffff4d",
			dir = 1,
			say = "\"...\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Mysterious Voice",
			bgName = "bg_daofeng_6",
			nameColor = "#ffff4d",
			dir = 1,
			say = "\"Supposing the war comes to an end, what comes next for the world's shipgirls?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "{namecode.18}",
			bgName = "bg_daofeng_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "Huh? That depends on what you mean...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "{namecode.18}",
			bgName = "bg_daofeng_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "I can only speak for myself, but I'd like to travel all around the world... Or maybe start a:jo and teach the Shimakaze fighting style!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Mysterious Voice",
			bgName = "bg_daofeng_6",
			nameColor = "#ffff4d",
			dir = 1,
			say = "\"The question is not about you specifically, it is about what will become of all shipgirls.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "{namecode.18}",
			bgName = "bg_daofeng_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "Hmm... Well, I:n't think we exist solely to fight.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "{namecode.18}",
			bgName = "bg_daofeng_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "When we're not fighting, we all have fun hobbies to pursue. If there wasn't any more fighting for us to:, couldn't we just enjoy a life of peace?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "{namecode.18}",
			bgName = "bg_daofeng_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "As for \"what will become of us,\" I'm afraid I:n't know...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Mysterious Voice",
			bgName = "bg_daofeng_6",
			nameColor = "#ffff4d",
			dir = 1,
			say = "\"That is... a pleasant idea. Yes.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Mysterious Voice",
			bgName = "bg_daofeng_6",
			nameColor = "#ffff4d",
			dir = 1,
			say = "\"I have no more questions for you. The rest of this all is Eugen's problem now.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "{namecode.18}",
			bgName = "bg_daofeng_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "What was that? It's \"Eugen's problem now\"? What: you mean?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Mysterious Voice",
			bgName = "bg_daofeng_6",
			nameColor = "#ffff4d",
			dir = 1,
			say = "\"Err, I meant... Listen closely, O brave one!\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "{namecode.18}",
			bgName = "bg_daofeng_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "I-I'm all ears!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Mysterious Voice",
			bgName = "bg_daofeng_6",
			nameColor = "#ffff4d",
			dir = 1,
			say = "\"You have acquired every treasure the Dragon Palace has to give.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Mysterious Voice",
			bgName = "bg_daofeng_6",
			nameColor = "#ffff4d",
			dir = 1,
			blackBg = True,
			say = "\"Now go. Return to your friends and use the treasures' power to save them!\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
