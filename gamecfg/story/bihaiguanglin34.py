return {
	fadeOut = 1.5,
	mode = 2,
	id = "BIHAIGUANGLIN34",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			nameColor = "#a9f548",
			side = 2,
			bgName = "bg_fuxiangxian_1",
			say = "Somewhere in the Sakura Empire...",
			dir = 1,
			bgmDelay = 1,
			bgm = "battle-xinnong-image",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_fuxiangxian_1",
			actor = 305110,
			dir = 1,
			nameColor = "#a9f548",
			say = "Shimakaze, you've come back from the Siren facility?!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_fuxiangxian_1",
			actor = 307080,
			dir = 1,
			nameColor = "#a9f548",
			say = "Good... I can see that you've brought it back, with the help of the Iron Blood...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_fuxiangxian_1",
			actor = 305110,
			dir = 1,
			nameColor = "#a9f548",
			say = "I never thought I'd live to see this... They brought the entire Dragon Palace back?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_fuxiangxian_1",
			actor = 305050,
			dir = 1,
			nameColor = "#a9f548",
			say = "\"Hakuryuu\"... I have heard this name uttered before.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_fuxiangxian_1",
			actor = 305110,
			dir = 1,
			nameColor = "#a9f548",
			say = "Was that the name of a blueprint ship? Maybe something the higher-ups haven't told us yet?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_fuxiangxian_1",
			actor = 305050,
			dir = 1,
			nameColor = "#a9f548",
			say = "When I was sealed inside the Sacred Tree, I would have dreams, similar to Shinano.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_fuxiangxian_1",
			actor = 305050,
			dir = 1,
			nameColor = "#a9f548",
			say = "But one such dream was very different... I:n't remember what kind of dream it was, but I know I heard that name.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_fuxiangxian_1",
			actor = 305050,
			dir = 1,
			nameColor = "#a9f548",
			say = "Not just that name... But others as well, most of them unfamiliar.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_fuxiangxian_1",
			actor = 307080,
			dir = 1,
			nameColor = "#a9f548",
			say = "Even when the fragments are in place, even when you can see them reflected within the mirror... Only emptiness awaits your hand when you reach out. 'tis a feeling that this one knows well...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_fuxiangxian_1",
			actor = 305110,
			dir = 1,
			nameColor = "#a9f548",
			say = "I can't say I understand what's going on, but...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_fuxiangxian_1",
			actor = 305110,
			dir = 1,
			nameColor = "#a9f548",
			say = "We can say with confidence that Hakuryuu is a member of the Sakura Empire, right?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_fuxiangxian_1",
			actor = 305050,
			dir = 1,
			nameColor = "#a9f548",
			say = "This, I am sure of.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_fuxiangxian_1",
			actor = 305110,
			dir = 1,
			nameColor = "#a9f548",
			say = "It's just been one major incident after another since Watatsumi was swapped out...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_fuxiangxian_1",
			actor = 307080,
			dir = 1,
			nameColor = "#a9f548",
			say = "With Watatsumi gone, no more rituals can take place, and my dreams can no longer guide the Sakura Empire...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_fuxiangxian_1",
			actor = 307080,
			dir = 1,
			nameColor = "#a9f548",
			say = "But alas, even this one, who used to mourn being lost in the currents of fate, can surely move forward...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_fuxiangxian_1",
			actor = 305050,
			dir = 1,
			nameColor = "#a9f548",
			say = "Fear not. I am sure there is something we can: about the Watatsumi's fragments.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_fuxiangxian_1",
			actor = 305110,
			dir = 1,
			nameColor = "#a9f548",
			say = "...Shinano, it is good that you have faith, but you:n't need to push yourself.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301810,
			side = 2,
			bgName = "bg_fuxiangxian_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "Lady Shinano, the Iron Blood fleet is getting ready to depart.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301810,
			side = 2,
			bgName = "bg_fuxiangxian_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "The others are waiting, so... if you're still planning on going...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307080,
			side = 2,
			bgName = "bg_fuxiangxian_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "Thank you for letting me know...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_fuxiangxian_1",
			actor = 305110,
			dir = 1,
			nameColor = "#a9f548",
			say = "We will deal with the aftermath of the Dragon Palace incident, so you can continue with your trip to the Iron Blood.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_fuxiangxian_1",
			actor = 305050,
			dir = 1,
			nameColor = "#a9f548",
			say = "Indeed. I wish you the best of luck.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_fuxiangxian_1",
			actor = 307080,
			dir = 1,
			blackBg = True,
			nameColor = "#a9f548",
			say = "Thank you for everything you've:ne...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
