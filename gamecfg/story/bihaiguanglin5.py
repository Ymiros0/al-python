return {
	fadeOut = 1.5,
	mode = 2,
	id = "BIHAIGUANGLIN5",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_luoxuan_5",
			nameColor = "#ffa500",
			dir = 1,
			bgmDelay = 1,
			expression = 1,
			bgm = "deepblue-image",
			actor = 900315,
			actorName = "???",
			hidePaintObj = True,
			say = "Um, Shinano, I have to ask you something first. What: you think of the commander?",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_luoxuan_5",
			paintingNoise = True,
			dir = 1,
			actor = 307080,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Commander... is the only one who understands what I've experienced...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_luoxuan_5",
			paintingNoise = True,
			dir = 1,
			actorName = "???",
			actor = 900315,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "Then, hold tightly onto the fragments of this dream when you awaken. And never forget what happened here.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_luoxuan_5",
			paintingNoise = True,
			dir = 1,
			actor = 307080,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "I see... This dream, too, is about to come to an end...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_luoxuan_5",
			paintingNoise = True,
			dir = 1,
			actorName = "???",
			actor = 900315,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "Yes... I am about to rewrite the very core of this Mirror Sea.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_luoxuan_5",
			actorName = "???",
			dir = 1,
			actor = 900315,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "I: not understand the principle behind your power, but... the aftershocks from what I am about to: will likely erase the fleeting changes you've been able to effect in this world.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_luoxuan_5",
			paintingNoise = True,
			dir = 1,
			actorName = "???",
			actor = 900315,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "That... is when your dream will come to an end.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_luoxuan_5",
			paintingNoise = True,
			dir = 1,
			actor = 307080,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Very well... Our meeting has been brief, but I will remember it forever... I wish you the best of luck...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_luoxuan_5",
			paintingNoise = True,
			dir = 1,
			actorName = "???",
			actor = 900315,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "(whispers) ...I'm sorry.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_luoxuan_5",
			paintingNoise = True,
			dir = 1,
			actorName = "???",
			actor = 900315,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "Do not stray from your path. Become the light in the darkness to guide your friends.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_luoxuan_5",
			paintingNoise = True,
			dir = 1,
			actorName = "???",
			actor = 900315,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "Three, two, one...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			say = "KABOOOOM!",
			side = 2,
			bgName = "bg_luoxuan_5",
			hidePaintObj = True,
			dir = 1,
			blackBg = True,
			soundeffect = "event./battle/boom2",
			flashN = {
				color = {
					1,
					1,
					1,
					1
				},
				alpha = {
					{
						0,
						1,
						0.2,
						0
					},
					{
						1,
						0,
						0.2,
						0.2
					},
					{
						0,
						1,
						0.2,
						0.4
					},
					{
						1,
						0,
						0.2,
						0.6
					}
				}
			},
			dialogShake = {
				speed = 0.09,
				x = 8.5,
				number = 2
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			nameColor = "#a9f548",
			side = 2,
			bgName = "bg_story_nepu2",
			say = "Sakura Empire Fleet",
			dir = 1,
			bgmDelay = 2,
			bgm = "battle-pacific",
			flashout = {
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 307080,
			dir = 1,
			nameColor = "#a9f548",
			say = "......",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301810,
			side = 2,
			bgName = "bg_story_nepu2",
			nameColor = "#a9f548",
			dir = 1,
			say = "Did you sleep well, Shinano?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 307080,
			dir = 1,
			nameColor = "#a9f548",
			say = "Ah... A fragment of bliss that tempts me to indulge in it for the rest of time...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 307080,
			dir = 1,
			nameColor = "#a9f548",
			say = "Kasumi... Do you know... the commander...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301810,
			side = 2,
			bgName = "bg_story_nepu2",
			nameColor = "#a9f548",
			dir = 1,
			say = "Mhm. Kasumi knows the commander.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305140,
			side = 2,
			bgName = "bg_story_nepu2",
			nameColor = "#a9f548",
			dir = 1,
			say = "Do you wish to speak to the commander, Lady Shinano?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 307080,
			dir = 1,
			nameColor = "#a9f548",
			say = "Not quite... I simply... wish to learn more about that person...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 307080,
			dir = 1,
			nameColor = "#a9f548",
			say = "But first... How:es Shimakaze's fleet fare...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 303060,
			dir = 1,
			nameColor = "#a9f548",
			say = "I was just informed they discovered a great castle atop an uninhabited island.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 303060,
			side = 2,
			bgName = "bg_story_nepu2",
			nameColor = "#a9f548",
			dir = 1,
			say = "Yamakaze described it as looking just like the legendary Dragon Palace.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 303060,
			dir = 1,
			nameColor = "#a9f548",
			say = "I ordered them to stay put until further notice so I could ask you how to proceed. What are your orders, Lady Shinano?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 307080,
			dir = 1,
			nameColor = "#a9f548",
			say = "The Dragon Palace... Never have I seen such a thing, not even in my dreams...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 303060,
			side = 2,
			bgName = "bg_story_nepu2",
			nameColor = "#a9f548",
			dir = 1,
			say = "We have reason to believe it's some sort of secret laboratory, as there's no record of an island ever existing there.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 303060,
			dir = 1,
			nameColor = "#a9f548",
			say = "But, seeing as they found it simply sitting out in the open...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305140,
			side = 2,
			bgName = "bg_story_nepu2",
			nameColor = "#a9f548",
			dir = 1,
			say = "...It's very likely to be a trap laid by the Sirens.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307080,
			side = 2,
			bgName = "bg_story_nepu2",
			nameColor = "#a9f548",
			dir = 1,
			say = "If the Sirens are aware of us...: 'twould be folly to ignore this island...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 303060,
			dir = 1,
			nameColor = "#a9f548",
			say = "Should I order them to bombard the facility,:?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 307080,
			dir = 1,
			nameColor = "#a9f548",
			say = "We mustn't destroy this place if it is one of the Sakura Empire's secret facilities... And if it is a Siren facility, I::ubt if our cannons can even destroy it...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 307080,
			dir = 1,
			nameColor = "#a9f548",
			say = "But at the very least... we must determine if the Sirens are behind this...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 307080,
			dir = 1,
			nameColor = "#a9f548",
			say = "Chikuma, Suruga, my request for you... is to lend aid to the Shimakaze fleet...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305140,
			side = 2,
			bgName = "bg_story_nepu2",
			nameColor = "#a9f548",
			dir = 1,
			say = "But Lady Shinano! We can't leave your side! It's too dangerous for you!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 307080,
			dir = 1,
			nameColor = "#a9f548",
			say = "Worry not for my safety... I have already seen what fate has in store...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 305140,
			dir = 1,
			nameColor = "#a9f548",
			say = "If so... Then very well! Chikuma and I shall join up with the vanguard fleet!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 305140,
			dir = 1,
			blackBg = True,
			nameColor = "#a9f548",
			say = "(Ugh... I've got SUCH a bad feeling about this...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
