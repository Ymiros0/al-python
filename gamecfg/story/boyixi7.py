return {
	fadeOut = 1.5,
	mode = 2,
	id = "BOYIXI7",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Me and the Boise\n\n<size=45>7.The Sweetness of Tiramisu</size>",
					1
				}
			}
		},
		{
			say = "Boise asked to have dinner with me, and I met her at a little bar by the Eagle Union:rms.",
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			bgmDelay = 2,
			bgm = "story-1",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			say = "I felt like she wasn't the kind of person who'd: something as proactive as asking me out for dinner though...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "{namecode.197}",
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "C-Commander...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			say = "My eyes followed the sound of her voice back to its source–",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			say = "Boise stood there dressed in an alluring bunnygirl outfit, hiding her mouth with her hair.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 102291,
			say = "Haah... Haah...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 102291,
			say = "I took off the mask... just like you asked me to...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 102291,
			say = "Um, umm... would you like something to drink...",
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			say = "For whatever reason, the two of us were the only people at the bar tonight.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			say = "My mind was racing to come up with reasons as to why she was dressed like this...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 102291,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I made a promise with St. Louis... If you asked to see my face, I'd change into this outfit and:... ask you out on a date...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 102291,
			say = "And if I did that, she'd help me pick out a gift that you like...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			say = "Is that why she put herself through all this...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 102291,
			say = "C-Commander, um... Now that you see me, what: you think...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			say = "Seeing her without her mask, and in this outfit... made her seem like a completely different person.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 102291,
			say = "D-deep breaths... Haah... Haah... Okay, I feel a lot better...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 102291,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Commander... Umm, thanks for always treating me so well...",
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 102291,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I know I've caused you a lot of trouble, but I'm glad you've always been so patient and understanding with me...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 102291,
			say = "If you like what you see,:, um... I'll: everything I can to support you, so, um...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			say = "Despite the fact that her face was beet-red from embarrassment, she mustered the courage to put her feelings forward.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 102291,
			say = "Oh, um, I think the food is here...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_145",
			dir = 1,
			blackBg = True,
			actor = 102291,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Um, tonight was my way of saying... I've become stronger, and I want to continue being with you... is that okay?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
