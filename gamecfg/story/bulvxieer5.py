return {
	fadeOut = 1.5,
	mode = 2,
	id = "BULVXIEER5",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"All-Love Assault!\n\n<size=45>5. Romance Snowmance</size>",
					1
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_153",
			say = "Northern Parliament - Ski Resort",
			bgm = "story-richang-2",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			effects = {
				{
					active = True,
					name = "bg_story_outdoor_snow"
				}
			}
		},
		{
			actor = 403021,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_153",
			side = 2,
			dir = 1,
			actorName = "Blücher",
			say = "Heheh☆ Look at me, Commander!",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_153",
			actorName = "Blücher",
			dir = 1,
			hidePaintEquip = True,
			actor = 403021,
			nameColor = "#A9F548FF",
			say = "This place is AWESOME! Just like Avrora said!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_153",
			say = "Blücher and I are at a mountain ski resort in the Northern Parliament, open only to shipgirls.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_153",
			say = "As a pleasantly cold breeze brushes against my cheek, I think to myself what a perfect vacation destination this is, particularly for skiing enthusiasts.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			fontsize = 45,
			side = 2,
			bgName = "star_level_bg_153",
			actorName = "Blücher",
			dir = 1,
			hidePaintEquip = True,
			actor = 403021,
			nameColor = "#A9F548FF",
			say = "This is the best date ever! I love you, Commandeeer!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_153",
			actorName = "Blücher",
			dir = 1,
			hidePaintEquip = True,
			actor = 403021,
			nameColor = "#A9F548FF",
			say = "...What? There's no echo! Isn't it supposed to echo when you shout on a mountain?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_153",
			say = "\"Maybe all the snow is dampening the noise.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_153",
			actorName = "Blücher",
			dir = 1,
			hidePaintEquip = True,
			actor = 403021,
			nameColor = "#A9F548FF",
			say = "Makes sense, actually. You're so smart!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_153",
			say = "Blücher,:nning her winter gear, zips right past me on her snowboard, a cloud of snow trailing behind her.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_153",
			actorName = "Blücher",
			dir = 1,
			hidePaintEquip = True,
			actor = 403021,
			nameColor = "#A9F548FF",
			say = "Heheh☆ Think you can catch up with me, Commander?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_153",
			actorName = "Blücher",
			dir = 1,
			hidePaintEquip = True,
			actor = 403021,
			nameColor = "#A9F548FF",
			say = "...You:n't wanna snowboard? Then how abooout... you catch me in your arms, hmm?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_153",
			say = "She makes a u-turn and uses her speed to reach a slope just above me.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_153",
			say = "...I know where this is going.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403021,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_153",
			side = 2,
			dir = 1,
			actorName = "Blücher",
			say = "Commandeeer! Turn around and give me a big hug!",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_153",
			say = "She jumps off the slope, arms extended, right towards me.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "(Catch her with grace)",
					flag = 1
				},
				{
					content = "(Catch her while trying not to panic)",
					flag = 2
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_153",
			optionFlag = 1,
			say = "Predicting where she's going to land, I run over and extend my arms.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_153",
			optionFlag = 1,
			say = "She soars through the air squarely into my arms and I manage to catch her.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			actorName = "Blücher",
			side = 2,
			bgName = "star_level_bg_153",
			dir = 1,
			optionFlag = 1,
			hidePaintEquip = True,
			actor = 403021,
			nameColor = "#A9F548FF",
			say = "Heheh☆ Ten out of ten catch, Commander~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_153",
			actorName = "Blücher",
			dir = 1,
			optionFlag = 1,
			hidePaintEquip = True,
			actor = 403021,
			nameColor = "#A9F548FF",
			say = "That was a heart-thumping moment in more ways than one~♡",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_153",
			optionFlag = 2,
			say = "Fearing for her well-being, I bumble my way forward and try to catch her.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_153",
			optionFlag = 2,
			say = "However, I act without thinking and lose my balance. I fall on my face in the snow, and so:es she.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_153",
			optionFlag = 2,
			say = "Luckily, neither of us is hurt thanks to the deep snow.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_153",
			actorName = "Blücher",
			dir = 1,
			optionFlag = 2,
			hidePaintEquip = True,
			actor = 403021,
			nameColor = "#A9F548FF",
			say = "Eep! That's cold... I got snow under my clothes...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_153",
			actorName = "Blücher",
			dir = 1,
			optionFlag = 2,
			hidePaintEquip = True,
			actor = 403021,
			nameColor = "#A9F548FF",
			say = "...You know what would warm me up? A nice, loving hug~ Heheh♡",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403021,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_153",
			side = 2,
			dir = 1,
			actorName = "Blücher",
			say = "I can't call it a day until we've snowboarded at least once together~ C'mon, let's go!",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_153",
			say = "The passion she radiates could melt all the snow in the world.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
