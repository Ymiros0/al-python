return {
	fadeOut = 1.5,
	mode = 2,
	id = "BULVXIEER7",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"All-Love Assault!\n\n<size=45>7. Another Day, Another Date</size>",
					1
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_task_2",
			say = "Port - Office",
			bgm = "story-richang-1",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_task_2",
			dir = 1,
			hidePaintEquip = True,
			actor = 403020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Heheh☆ Good morning, Commander!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "\"Good morning.\"",
					flag = 1
				},
				{
					content = "\"Uh, good day?\"",
					flag = 2
				},
				{
					content = "\"Good evening.\"",
					flag = 3
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_task_2",
			dir = 1,
			optionFlag = 2,
			hidePaintEquip = True,
			actor = 403020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "What? It's not even close to afternoon yet.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_task_2",
			dir = 1,
			optionFlag = 2,
			hidePaintEquip = True,
			actor = 403020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Besides, it's always morning until you've had breakfast!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_task_2",
			dir = 1,
			optionFlag = 3,
			hidePaintEquip = True,
			actor = 403020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Evening? Uh-oh, I think something's wrong with you. I'd better call Vestal right away...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_task_2",
			optionFlag = 3,
			say = "\"I was just joking to see how you'd react. Please put the phone:wn.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_task_2",
			dir = 1,
			optionFlag = 3,
			hidePaintEquip = True,
			actor = 403020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Alrighty:~ In that case, good evening to you too♪",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_task_2",
			dir = 1,
			hidePaintEquip = True,
			actor = 403020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Hey, did you dream of me last night? The look on your face says you did~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_task_2",
			dir = 1,
			hidePaintEquip = True,
			actor = 403020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "On that note, I have a small present for you~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_task_2",
			say = "\"A present? What's the occasion?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It's, uhh... You know! To celebrate our 100-day wedding anniversary!",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_task_2",
			say = "\"How nice.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_task_2",
			dir = 1,
			hidePaintEquip = True,
			actor = 403020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "That, and to commemorate all the sweet and love-packed memories we've made!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_task_2",
			dir = 1,
			hidePaintEquip = True,
			actor = 403020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "I'm not expecting anything, but I sure would love something in return~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_task_2",
			dir = 1,
			hidePaintEquip = True,
			actor = 403020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Like a warm hug coupled with a loving kiss, for instance~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_task_2",
			say = "She's either thinking out loud, or hinting at what she's going to: next.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_task_2",
			dir = 1,
			hidePaintEquip = True,
			actor = 403020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Commander! Eyes on me!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_task_2",
			dir = 1,
			hidePaintEquip = True,
			actor = 403020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "And:n't think about shutting them! I want your full, undivided attention~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_task_2",
			say = "I turn to face her and realize she's standing right next to me. Her puckered lips graze my cheek.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_story_task_2",
			dir = 1,
			hidePaintEquip = True,
			actor = 403020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "*smooch*!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_task_2",
			say = "The warmth of a gentle kiss spreads through me.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_task_2",
			dir = 1,
			hidePaintEquip = True,
			actor = 403020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Heheh☆ Gotcha! I'll make: with that for now~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Anyway, it's time we get to work!",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_task_2",
			dir = 1,
			hidePaintEquip = True,
			actor = 403020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "...Hm? What's up, Commander? Waiting for something?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_task_2",
			say = "\"You left me a bit stunned by changing gears just like that.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_task_2",
			dir = 1,
			hidePaintEquip = True,
			actor = 403020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Well, what are we gonna: if not work? Unless... you wanna go on a date?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_task_2",
			say = "\"I'm feeling a little light-headed. I can't exactly focus right now.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_task_2",
			dir = 1,
			hidePaintEquip = True,
			actor = 403020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Oh no! You sound sick! We can't let you work like this!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_story_task_2",
			dir = 1,
			hidePaintEquip = True,
			actor = 403020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Okay, forget about work, how about I take you to the infirmary?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_task_2",
			say = "\"It:esn't seem like we have any other option.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_task_2",
			dir = 1,
			hidePaintEquip = True,
			actor = 403020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Heheh☆ We're going on an infirmary date~ Come on, let's go♡",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
