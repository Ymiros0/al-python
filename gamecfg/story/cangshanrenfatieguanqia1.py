return {
	fadeOut = 1.5,
	mode = 2,
	id = "CANGSHANRENFATIEGUANQIA1",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			mode = 1,
			stopbgm = True,
			asideType = 1,
			sequence = {
				{
					"In a place unknown, shrouded in darkness...",
					2
				}
			}
		},
		{
			mode = 1,
			stopbgm = True,
			asideType = 1,
			sequence = {
				{
					"In a place that makes no sound, wrapped in silence...",
					2
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_508",
			dir = 1,
			bgm = "bsm-2",
			actor = 11000050,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Mgh... Mmgh...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Oh, you're finally awake. You were out for a long while, sleepyhead.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "I was starting to worry you'd given up the ghost. Seems I won't need to arrange a funeral.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000050,
			say = "Where... am I...?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Heheh. You're in my secret room, and I invited you here.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 11000050,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...Who are you?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "What a boring question. Here I thought you'd ask something more... pertinent.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Like, \"Am I dead?!\" for a start.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 11000050,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I'm afraid I know my state of being quite well. What I need to know at this moment is where I am, and also...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000050,
			say = "Just who and what are you?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Heheheh. So calm and composed. But you're bluffing, aren't you? I: like a bluff.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "But your elevated heart rate isn't:ing a good job of selling it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Anyway, you're asking who I am? Frankly, I'd appreciate it if you wouldn't concern yourself with that.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Explaining everything would be a huge hassle, you see.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "You:n't like being kept in the dark, I'm sure, but I'd like you to forget about that and assist with my research for a while.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "I'll let you go home once I'm happy with the results. That is, assuming you play nice. Otherwise, maybe I won't.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 11000050,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "......",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "You:n't trust me? Yes, that's only natural...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "However, whether you trust me or not:esn't change your situation one bit.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Let me guess. You think you've pieced the answer together now, no?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Then let me apologize and leave you with this – take your time and enjoy yourself.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 11000050,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "......",
			bgm = "sk-az-pv1",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = False,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = False,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000050,
			say = "What in the world is going on?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000050,
			say = "I wonder where the girls are... I hope they're safe.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Heheheh. These values are more interesting than I projected. This will be fun.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			bgName = "bg_burningsea_1",
			mode = 1,
			asideType = 4,
			spacing = 30,
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			},
			rectOffset = {
				200,
				200,
				400,
				400
			},
			sequence = {
				{
					"<size=34>Shinobis – they were shadows who lived long ago during the Warring States era.</size>",
					1
				},
				{
					"<size=34>They hid among crowds, they snooped, provoked, deceived, and assassinated.</size>",
					2
				},
				{
					"<size=34>Where there was dirty work, there were shinobis to undertake it.</size>",
					3
				},
				{
					"<size=34>As time went on, they disappeared from the public consciousness, becoming little more than legends.</size>",
					4
				},
				{
					"<size=34>But, though the people had forgotten them, the nature of man was still what it was.</size>",
					5
				},
				{
					"<size=34>Those who lived in the dark underworld were still needed.</size>",
					6
				},
				{
					"<size=34>Shinobis have continued to exist to this day.</size>",
					7
				},
				{
					"<size=34>And here, in a faraway land, a battle of theirs was beginning to unfold...</size>",
					8
				}
			}
		},
		{
			mode = 1,
			blackBg = True,
			effects = {
				{
					active = True,
					name = "cangshanrenfatie"
				}
			},
			sequence = {
				{
					"",
					2
				}
			}
		}
	}
}
