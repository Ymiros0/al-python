return {
	fadeOut = 1.5,
	mode = 2,
	id = "CANGSHANRENFATIEGUANQIA10",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			say = "Mirror Sea - Somewhere else",
			bgm = "bsm-2",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			say = "There, in a place no one knows, the captive princess stood all by herself.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 11000050,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "......",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000050,
			say = "...How long has it been since:?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Would you like to know?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000050,
			say = "*gasp*! How long have you been here?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Three seconds or so. I'm giving you the cold shoulder here, mind you.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "I only thought you, our esteemed guest, may have wanted some quiet time to yourself.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "I've been consciously leaving you alone as a courtesy. Although, you probably:n't believe me when I say that.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000050,
			say = "Do you... WANT me to believe you?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Hmm... If I'm to be frank, I:n't care either way if you believe me or not.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "What I DO care about is seeing how your attitude changes depending on whether you: or:n't. It's interesting.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "If you'd like, shall we have a tea party? It'll be an opportunity for you to speak with me at length.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000050,
			say = "I'll pass, thank you. The idea of letting you get even a crumb of information out of me makes my skin crawl.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "How harsh. At the same time, you're trying to get a reaction out of me to gauge information from it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "If I had to describe your attempt with one word, it would be \"adorable.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 11000050,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "......",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Heheh. Don't scowl like that... I have so much to tell you. Don't you want to know how your friends are:ing?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000050,
			say = "Ugh...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Heheheheh! I adore it when your weakness peeks through the cracks like this.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "So desperately trying to keep your emotions in the bottle. That's also adorable.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Unfortunately, all I have to share is that your friends are:ing all they can to get the situation under control.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Oh, but they did make some trusty allies, so there's that.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "I'm sure they'll overcome all the challenges and come and rescue you.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "It's a shame the ultimate challenge they have to face is me. How cruel the world can be.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "In my case, I'll be happy as long as they kick and scream nicely...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000050,
			say = "That's what makes you happy?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Oops, I might have let something slip there. Be nice and forget you heard that.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "What matters is this – everyone is safe and sound. You have nothing to worry about for now.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Also, I'm lucky someone from your world held the key to the gateway. It saved me a lot of time and effort.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000050,
			say = "What? Key? Gateway? Oh... You exploited my powers to cross world boundaries. That's what you're saying, isn't it?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Ah, you're sharper than you let on. I need to watch my mouth in the future. Heheh...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Look at that. It seems things have changed whilst you and I have been chatting.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "It's looking like your friends will end up right where I want them, even without any advice on your part.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "So, stay here and take it easy for a while.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "If you need anything,:n't be afraid to let me know. I'll get it for you as soon as possible.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "I: expect you to lend an ear to my chitchat in return, however.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000050,
			say = "......",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Alright, see you later.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 11000050,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "......",
			bgm = "sk-az-pv1",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = False,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = False,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000050,
			say = "My gut feeling was right. My powers were the catalyst behind this incident...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000050,
			say = "I need to escape this place immediately and link up with the girls. My conscience won't let me rest until I apologize.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 11000050,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "The problem is, all my ninjutsu seems to have no effect here. I need to think of another way...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000050,
			say = "No, it's not that it's ineffective, per se – it feels more like my energy is being sapped. Brute-forcing it isn't going to work.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000050,
			say = "If my powers warrant being actively suppressed, that must mean they are the trump card to put a stop to all this.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000050,
			say = "If that's the case, I need to wait for the right time. The time to strike. That's when I'll act swiftly...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Heheh. She's taken the bait.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "With people like her, you just need to drip-feed a little information and they'll fill in the gaps in your plan. Easily exploitable, in that way.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Oh, I look forward to seeing what experiments can be:ne using her powers.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
