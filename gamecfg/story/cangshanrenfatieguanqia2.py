return {
	fadeOut = 1.5,
	mode = 2,
	id = "CANGSHANRENFATIEGUANQIA2",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			mode = 1,
			stopbgm = True,
			asideType = 3,
			blackBg = True,
			sequence = {
				{
					"At sea - Night",
					1
				},
				{
					"Akatsuki - On a regular patrol",
					2
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Ah, what a calm and peaceful day. Nothing out of the ordinary, and nothing to be afraid of...",
			bgm = "level",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It's so quiet. TOO quiet... I think I'd prefer it if the Sirens came to attack...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "No, no, no! What am I saying? I'd much rather be a little scared than be chased around all day!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Maybe if I pinch myself, the pain will distract me from... Actually, I:n't want to hurt. Oh, I know! Some stargazing may lift my spirit!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "That reminds me, either Ikazuchi or Inazuma said I should wish upon a falling star not to be scared...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "If I: just that, hopefully I won't have to be afraid on my patrols anymore.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Yeah! I'll keep my eyes on the sky and ignore the murky water under my feet!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...That said, I'm not really seeing any falling stars tonight.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Most days, I'll usually notice a few whether I'm paying attention or not.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			say = "Just:, there was a twinkle in the sky.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Oh! There's one! Okay, now to make my wish... Please let me not be scared, please let me not be scared...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			say = "The thing she thought was a falling star did not vanish. It seemed to be going toward her.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "What the...? Am I imagining it, or is it coming this way? It almost looks like it's getting bigger...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			say = "It drew closer and closer to where Akatsuki was...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "AAAHHHH!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I:n't think that was a falling star – it might've been a new Siren superweapon! And it crashed into that islet over there...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = False,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = False,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Thank goodness it didn't hit me. *sigh*, I suppose now I must investigate before I can report what happened...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I really:n't want to, but they'll get mad at me if I:n't... I wish Hibiki were here so I could ask her to: it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			mode = 1,
			stopbgm = True,
			asideType = 3,
			blackBg = True,
			sequence = {
				{
					"At sea - Morning",
					1
				},
				{
					"Akatsuki - Forcibly scouting the area",
					2
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Hmm... I'm fairly sure this is the impact zone. I can't see anything because of all this fog, though...",
			bgm = "story-6",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "What even are these waters? This shouldn't be that far off from my regular patrol route...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "And yet, I:n't remember seeing any island like this before.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "What if this is a Siren trap, and this is a Mirror Sea?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...Huh? Is that a person? Oh no! They're passed out on the ground! I think they got caught in the impact!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Ma'am? Are you okay?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 11000010,
			side = 2,
			bgName = "bg_battle_night",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Uuughh...",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Oh! You're alive! Listen to the ninja's voice! Are you in any kind of pain?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I should probably radio the port and get some repair ships to help... But if this is a Mirror Sea, the lines are:wn!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_battle_night",
			actor = 11000010,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "*gasp*! Urgh... Where am I?",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Phew! Looks like you're okay! As for where you are, uhh...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_battle_night",
			actor = 11000010,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Hey! Homura! Fubuki! Are you two okay? Are you still alive? Noooo! They're dead!",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I...:n't think so...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_battle_night",
			dir = 1,
			bgm = "sk-az-story",
			actor = 11000030,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "You need to take a chill pill, Asuka! You really think we'd bite the dust just like that?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_battle_night",
			actor = 11000050,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "My apologies for all this ruckus. We didn't scare you, I hope?",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Oh, no, I'm fine. May I ask who you people are?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_battle_night",
			actor = 11000010,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "We're all shino– Sorry, I meant to say, \"normal high schoolers.\"",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "High schoolers? Yes, I suppose you look the part.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 11000050,
			side = 2,
			bgName = "bg_battle_night",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Asuka, I: not believe this land shares our laws... An instinctual feeling tells me so.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_battle_night",
			actor = 11000050,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "That being the case, we should come forth with our identities and ask for cooperation.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_battle_night",
			actor = 11000050,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Besides, this young lady's attire suggests that she plays the same role as us.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_battle_night",
			actor = 11000010,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Huh? You mean she's a shinobi?",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Did someone just say \"shinobi\"?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_battle_night",
			actor = 11000010,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Oh, yeah, I did! We're shinobis, and we support the world from the shadows, fighting bad guys day and night. Do you: that, too?",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Well, I, uh... Y-yes. I follow the path of the ninja...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_battle_night",
			actor = 11000010,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Wow, you really are a fellow shinobi! I'm glad we're in good company! I thought we'd be all alone in this scary new place.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_battle_night",
			actor = 11000010,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "My name's Asuka, by the way! I'm a second-year student at Hanzō Academy.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_battle_night",
			actor = 11000010,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "The tomboy over here is Homura, and this pretty lady is Fubuki. Now tell us about who you are.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Ah, right... Greetings. I am Akatsuki, from the Sakura Empire.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000030,
			say = "Sakura Empire Academy? Doesn't ring any bells... It's top secret, isn't it? Makes sense. That's how shinobis roll.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Umm, well... That's not exactly correct...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_battle_night",
			actor = 11000010,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "And what ninjutsu: you specialize in? Oh! Wait! Let me guess...",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_battle_night",
			actor = 11000010,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Hmmm. Judging by your gear and the fact we're in the middle of the ocean... I'll bet you sneak around in the water! Do you work with the navy?",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Err, no, I'm in a Destroyer Division, specifically the–",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_battle_night",
			actor = 11000010,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Destruction ninjutsu?! Whoooa! That sounds awesome! And really powerful!",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "No, that means I fight primarily using torpedoes...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000030,
			say = "Torpedoes? As in, electric rays? So you've studied beastmastery. This sea-fighting academy of yours is impressive. I could use a tip or two...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Umm, I... It's...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_battle_night",
			actor = 11000050,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "You're barraging her with too many questions to keep up.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_battle_night",
			actor = 11000050,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "While I'm certain we all have many more questions, for now, let us look for a safe place to relax.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = 900232,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actorName = "Observer",
			say = "Heheh. Found you.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "What?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_battle_night",
			hidePaintObj = True,
			say = "That very moment...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_1104",
			hidePaintEquip = True,
			dir = 1,
			bgm = "bsm-2",
			actor = 11000010,
			nameColor = "#A9F548FF",
			say = "I can't see a thing! What's happening?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = False,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = False,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "star_level_bg_1104",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Huh? Where did Fubuki go?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "star_level_bg_1104",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "What just happened?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
