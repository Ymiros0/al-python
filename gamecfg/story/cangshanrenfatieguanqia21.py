return {
	fadeOut = 1.5,
	mode = 2,
	id = "CANGSHANRENFATIEGUANQIA21",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_kagura_2",
			soundeffect = "event./battle/boom2",
			say = "KABOOOM!",
			bgm = "sk-az-battle-boss",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashN = {
				color = {
					1,
					1,
					1,
					1
				},
				alpha = {
					{
						0,
						1,
						0.2,
						0
					},
					{
						1,
						0,
						0.2,
						0.2
					},
					{
						0,
						1,
						0.2,
						0.4
					},
					{
						1,
						0,
						0.2,
						0.6
					}
				}
			},
			dialogShake = {
				speed = 0.09,
				x = 8.5,
				number = 2
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_kagura_2",
			actor = 11000010,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Oww! That was such a cheap attack!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_kagura_2",
			actor = 11000030,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Stop whining and fight back! It's not gonna go easy on you just 'cause you called it cheap!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_kagura_2",
			actor = 11000030,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Yūyaki! Can you keep that thing in check?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_kagura_2",
			actor = 11000070,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "For now, but I can't keep this up forever! Think of something to take it out, dammit!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_kagura_2",
			actor = 103240,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "The hell is this? Its attack pattern's the same, but different stuff happens every time! What're we supposed to:?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_kagura_2",
			actor = 402080,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It's purposely throwing us off our rhythm so we can't predict its moves. It's... a bit like how a ninja attacks.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_kagura_2",
			actor = 103240,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "She called it \"battle ninja armor,\" yeah? It's got the schmoves of a ninja and the strength of Tester herself!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_kagura_2",
			actor = 103240,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "We've gotta think of something, quick, or we are so boned!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "bg_kagura_2",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "If it fights like a ninja, I shall hit tit for tat with the new techniques I've learned! High-explosive clone shell, strike my foe!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_kagura_2",
			actor = 102260,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Then I'll whip out the new gear I made with the shinobis' help – the infinite caltrop gun! Fire!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_kagura_2",
			actor = 11000030,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Wow. I taught her the clone shuriken technique, and she's using it in a whole new way... Reno might just be a natural genius.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_kagura_2",
			actor = 11000010,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "That caltrop launcher could be extremely useful! If you scaled it:wn a little, maybe we could use them as well!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_kagura_2",
			actor = 103240,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Focus, people! Don't get distracted by her gadgets!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
