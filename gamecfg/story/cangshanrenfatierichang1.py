return {
	fadeOut = 1.5,
	mode = 2,
	id = "CANGSHANRENFATIERICHANG1",
	once = True,
	fadeType = 2,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"The Ninja Scrolls. At Your Leisure\n\n<size=45>Fun & Play Holiday</size>",
					1
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			say = "Port - Harbor",
			bgm = "sk-az-story",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000010,
			say = "Man, I envy you girls.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 102260,
			say = "Envy us? How so? Is this about the food again?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 102260,
			say = "Because you really gushed about the sashimi the Commander treated you to yesterday.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000010,
			say = "What? Oh, yeah, that was amazing. So fresh, so piquant, so... Wait, that's not what I was thinking about!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000010,
			say = "True, I: envy the food you girls get here, but it was something else...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000010,
			say = "Right! The sea! Opening my window and hearing the waves! The smell of the ocean when I go outside to grab a bite!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000010,
			say = "Leaving the:rm and feeling the cool breeze! Walking just a short distance and seeing the beautiful coast!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000010,
			say = "And finally, a body of water being just a short walk away when I wanna swim! I envy you because it's all so great.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 102260,
			say = "Well, sure, there's fun to be had at sea, but the sea basically means exercises and expeditions for us.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 102260,
			say = "We also get in a more somber mindset once we put our riggings on.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 102260,
			say = "While it's nice being so close to the sea, it's more convenient for going on missions than it is for enjoying yourself.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000010,
			say = "Oh, really? Huh. But I've seen girls wearing both swimsuits AND riggings when they disembark.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000010,
			say = "Were they going on some sort of special mission?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 102260,
			say = "You mean the submarines, right? They fight under the water, so they wear swimsuits optimized for diving.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 102260,
			say = "Now that you mention it, though, I could also fight in a swimsuit if I wanted!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000010,
			say = "Really?! Well, I can't... I tried a swimsuit the other day and couldn't equip my rigging for some reason.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 102260,
			say = "Maybe you need a special swimsuit designed for rigging use.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000010,
			say = "Or one made with special fabric...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 312010,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "What are you girls chatting about, nya?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 102260,
			say = "Akashi? Where'd the HECK did you come from?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000010,
			say = "You appeared out of nowhere, almost like a ninja!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 312010,
			say = "Heh. My methods are a trade secret, nya.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 312010,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Anyway, I'm dropping by to deliver Asuka's new swimsuit, nya. The Commander requested one, and it's finally:ne, nya. Just sign here, nya.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000010,
			say = "Sign where? The:tted line?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 312010,
			say = "Yep, nya. My work here is:ne, and I've got places to be, so... toodles, nya!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000010,
			say = "Gone just as quickly as she came... Is this swimsuit what I think it is?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 102260,
			say = "Wow, perfect timing on the Commander's part! Now you can go into battle wearing a swimsuit!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000010,
			say = "Heck yeah! That sounds like it'd be a bucketload of fun! I so wanna try that.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 102260,
			say = "You:? Then go for it. I'll call some people and we'll organize a mock battle!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000010,
			say = "Just for little ol' me? Aww, thank you so much!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305131,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Ahahah! Wasn't expectin' this when I woke up today. It's cool, though – just so happens my schedule's empty!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 303191,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Look at us, all wearing swimsuits. Heehee. What a magnificent sight.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000011,
			say = "Whoooa! You look so good! Like you came straight out of a TV show!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000011,
			say = "Seriously! I'm not exaggerating when I say you look like models! You're gorgeous, you're sex bombs, you're... so much prettier than me!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105172,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Don't say that. You're a real bombshell yourself!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000011,
			say = "Y-you think so? Heehee. You're just saying that!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301293,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I would argue all you shinobis are slim to perfection and have curves that could kill, though.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301891,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Take Fubuki, for one. She's drop-dead sexy.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000011,
			say = "That's True! She is one pretty lady...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000011,
			say = "Oh, Yumi, too! How she sparkles like falling snowflakes... She's got such a mystical vibe!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 102262,
			say = "Guess who's ready! Heehee! How: I look?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000011,
			say = "Cute as all heck!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 102262,
			say = "Thanks! So, let's all: some stretches,: get this mock battle started, shall we?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 102262,
			say = "The Commander got us these swimsuits, so let's put on a cute battle as thanks!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Everyone",
			bgName = "star_level_bg_526",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Okay!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		}
	}
}
