return {
	fadeOut = 1.5,
	mode = 2,
	id = "CANGSHANRENFATIERICHANG7",
	once = True,
	fadeType = 2,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"The Ninja Scrolls. At Your Leisure\n\n<size=45>Start the Training - Part 2</size>",
					1
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_unnamearea_1",
			hidePaintObj = True,
			say = "Training Area - Somewhere else",
			bgm = "sk-az-battle",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 608010,
			side = 2,
			bgName = "bg_unnamearea_1",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "So... What's the point in forcing us to come here, exactly?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301210,
			side = 2,
			bgName = "bg_unnamearea_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "That's a lot of mass-produced training ships...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_unnamearea_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000060,
			say = "A-and their guns are trained on US!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 402040,
			side = 2,
			bgName = "bg_unnamearea_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Is this... what I think it is?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105170,
			side = 2,
			bgName = "bg_unnamearea_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Yup! Since you can't handle groups of people, I got you some non-people opponents!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_unnamearea_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 105170,
			say = "Take your time getting used to practicing with these and you'll be ready to try the real deal with the rest of us!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_unnamearea_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 105170,
			say = "(Heheh. I'm a genius.)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 608010,
			side = 2,
			bgName = "bg_unnamearea_1",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "St-starting mission... I'll go underwater...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105170,
			side = 2,
			bgName = "bg_unnamearea_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Also, there's swarms of mass-produced subs waiting under the surface. There's nowhere to run!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_unnamearea_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000060,
			say = "N-nowhere to run? Umm...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105170,
			side = 2,
			bgName = "bg_unnamearea_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Don't worry! You've got me for backup!~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_unnamearea_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 105170,
			say = "I'll handle the small stuff so you can focus on the bigger picture. Plus, we'll get buddy-buddy faster if we fight side by side.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105170,
			side = 2,
			bgName = "bg_unnamearea_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "If you:n't wanna get KO'd, you've either gotta evade or KO the enemy first. Either way,:n't just stand there! Go, go, go!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301210,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "*pant*... *pant*... *pant*...",
			bgm = "sk-az-story",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000060,
			say = "G-god... That was insane...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 105170,
			say = "Oopsie... I might've gone too far in a few places.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000060,
			say = "WAY too far...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 402040,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I can't move a muscle... Fishy, help me...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 608010,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Heheh... Heheheh... Heheheheh...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = 319010,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actorName = "{namecode.188}",
			say = "Murasaki! Are you okay?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 11000010,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "We're here to rescu– Whoa. Check out the numbers on their report sheet!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I can't believe my eyes! Look how many foes they defeated!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 11000030,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Here I thought they'd need a hand. Looks like we came for nothing.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105170,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Oh, if you girls haven't had enough, I'll call in some more ships.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000060,
			say = "*whimper*...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 319010,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "All this fighting has left them exhausted. I think this is a good time to call it for today.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301210,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Are you sure? But you just got here to help us out.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 11000030,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "You saying you wanna fight some more? 'Cause I'm up for it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			side = 2,
			actor = 608010,
			say = "Nuh-uh, no way... I'm:ne...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 11000010,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Alright,: that's enough training for today! Let's all go swimming now!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Did you not swim, like, several kilometers just a minute ago?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000010,
			say = "Yeah, but that's different! It's not the same as swimming with these girls!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000010,
			say = "No time to waste! I'll see you there!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301100,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I'm:wn for a swim!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301090,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I-I will join you,:!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105170,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Well... That worked out surprisingly well in the end.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 105170,
			say = "In the heat of battle, all you think about is what your teammates will:, when to:dge, and what enemy to sink...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105170,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "There's no time to waste thinking about your own little troubles. Isn't that nice?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000060,
			say = "Yes, but the thing is, my friends have told me that before...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000060,
			say = "I went into the battle already knowing about that...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 105170,
			say = "Wait, you did? So in the end, you didn't need me to: anything for you?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000060,
			say = "No. You can be a bit of a hothead sometimes... Does anyone tell you that?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000060,
			say = "Still, even if this wasn't necessary, it showed that you care for us... Thank you for that.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 11000060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "……………………",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000060,
			say = "I need to work on becoming more confident...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = 11000010,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actorName = "Asuka",
			say = "White sands and blue waves – I'm coming for you!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 11000060,
			say = "...But to reach her level of confidence is going to take years.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
