return {
	fadeOut = 1.5,
	mode = 2,
	id = "CHANGYEJIANGLINZHIQIAN7",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			side = 2,
			bgName = "star_level_bg_1104",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			soundeffect = "event./ui/fengling",
			say = "A dream. A dream of one possible future. Its dreamer – Shinano.",
			seDelay = 1,
			flashin = {
				delay = 1,
				dur = 1,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_xinnong2_2",
			hidePaintObj = True,
			bgm = "bgm-waterwave",
			say = "A wind chime rang. Then, the sound of overlapping waves.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_xinnong2_2",
			hidePaintObj = True,
			say = "The sky was dark as soot. Beneath it, a long column of torii dyed a vivid red.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_xinnong2_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 307080,
			say = "The line between illusion and reality, the interstices between dreams...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307080,
			side = 2,
			bgName = "bg_xinnong2_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "A place seldom seen, as the dream usually presents itself immediately...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_xinnong2_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 307080,
			say = "Nevertheless... This one has been here before, a few times...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_xinnong2_2",
			hidePaintObj = True,
			say = "The torii split the world into two halves. From opposite sides, the winds brought crashing waves into each other, in defiance of the laws of nature.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_xinnong2_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 307080,
			say = "The realm of illusion can be calm at some times, chaotic at others...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307080,
			side = 2,
			bgName = "bg_xinnong2_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Tonight... It appears particularly at unrest...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_xinnong2_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 307080,
			say = "An ill omen, that is... My dream shan't be a pleasant one...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			stopbgm = True,
			bgName = "star_level_bg_501",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			oldPhoto = True,
			say = "In her dream, Shinano saw a scene from the \"past.\"",
			bgm = "theme-starsea-core",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_501",
			hidePaintObj = True,
			oldPhoto = True,
			say = "There was a lone woman hidden behind a pile of books, speaking with delight in her voice.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_501",
			oldPhoto = True,
			dir = 1,
			actor = 699010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "...Really? You've tracked:wn the Crown's location?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 699010,
			oldPhoto = True,
			bgName = "star_level_bg_501",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			side = 2,
			say = "...Magnificent! Truly, truly magnificent!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_501",
			oldPhoto = True,
			dir = 1,
			actor = 699010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "I knew I could count on you. Many follow my orders, but none truly help me like you:.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 699010,
			oldPhoto = True,
			bgName = "star_level_bg_501",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			side = 2,
			say = "Now that we know where it is, victory is already within my grasp.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 699010,
			oldPhoto = True,
			bgName = "star_level_bg_501",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			side = 2,
			say = "...The Royal Navy and Iron Blood? What about them?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 699010,
			oldPhoto = True,
			bgName = "star_level_bg_501",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			side = 2,
			say = "You're worried they might throw a wrench in my plan?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_501",
			oldPhoto = True,
			dir = 1,
			actor = 699010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Hmm... Yes, it's clear that their transgressions have gone unpunished for too long.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 699010,
			oldPhoto = True,
			bgName = "star_level_bg_501",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			side = 2,
			say = "But, Bismarck and the little queen are nothing to worry about. Whatever they've been preparing, I'm already five steps ahead of them.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_501",
			oldPhoto = True,
			dir = 1,
			actor = 699010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Still, I must admit that you have a point. One can never be too cautious.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 699010,
			oldPhoto = True,
			bgName = "star_level_bg_501",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			side = 2,
			say = "I'll take your advice to heart and figure something out.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_501",
			oldPhoto = True,
			dir = 1,
			actor = 699010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "\"Thus the whole earth was of one language, one speech. And they said, let us build a city and a tower, whose top may reach unto heaven.\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 699010,
			oldPhoto = True,
			bgName = "star_level_bg_501",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			side = 2,
			say = "\"Seeing that the people were now one, with nothing restrained from them,:ce did the Lord confound the people with different languages and scatter them upon the face of all the earth.\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_501",
			oldPhoto = True,
			dir = 1,
			actor = 699010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Hahaha... I will not forget the favor you've:ne for me.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 699010,
			oldPhoto = True,
			bgName = "star_level_bg_501",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			side = 2,
			say = "I swear that I will grant you your wish – once my own ambitions are realized.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_501",
			oldPhoto = True,
			dir = 1,
			actor = 307080,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "(The woman is speaking with someone...)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_501",
			oldPhoto = True,
			dir = 1,
			actor = 307080,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "(The room's interior is in the Sardegnian style, of that I am sure...)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307080,
			oldPhoto = True,
			bgName = "star_level_bg_501",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			side = 2,
			say = "(That voice... It must be Marco Polo's...)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307080,
			oldPhoto = True,
			bgName = "star_level_bg_501",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			side = 2,
			say = "(But who she is speaking to, I cannot fathom a guess...)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_501",
			oldPhoto = True,
			dir = 1,
			actor = 307080,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "(A simple dream. Only Marco Polo, conversing with another person...)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307080,
			oldPhoto = True,
			bgName = "star_level_bg_501",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			side = 2,
			say = "(Though gloomy to a degree, this dream isn't chaotic like the ill omen foreboded...)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_501",
			oldPhoto = True,
			dir = 1,
			actor = 307080,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "(Then perhaps... there is more to this dream than meets the eye?)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			dialogueBgAlpha = 0.6,
			side = 2,
			bgName = "bg_firedust_5",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "A black sun, quietly burning in the dusky sky.",
			soundeffect = "event./ui/fengling",
			effects = {
				{
					active = True,
					name = "yuanzhou_juqing01",
					interlayer = 1001
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_firedust_5",
			hidePaintObj = True,
			dialogueBgAlpha = 0.6,
			say = "Brightly it shone, like a star, but offered no warmth – like an impossibly bright black hole.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_firedust_5",
			dialogueBgAlpha = 0.6,
			dir = 1,
			actor = 307080,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Never before have I dreamed of anything like this...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_firedust_5",
			dialogueBgAlpha = 0.6,
			dir = 1,
			actor = 307080,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "It is a frightening sight to behold...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_firedust_5",
			dialogueBgAlpha = 0.6,
			dir = 1,
			actor = 307080,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "May this be... the source of all the chaos?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_firedust_5",
			hidePaintObj = True,
			dialogueBgAlpha = 0.6,
			say = "\"The time of judgment hath come.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_firedust_5",
			dialogueBgAlpha = 0.6,
			dir = 1,
			actor = 307080,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "...Judgment?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_firedust_5",
			dialogueBgAlpha = 0.6,
			dir = 1,
			actor = 307080,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Who is to be judged, and for what?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_firedust_5",
			hidePaintObj = True,
			dialogueBgAlpha = 0.6,
			say = "\"What your eyes behold shall descend upon the land.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_firedust_5",
			dialogueBgAlpha = 0.6,
			dir = 1,
			actor = 307080,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "It... Shall descend upon the land...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			oldPhoto = True,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "\"The time of judgment hath come.\"",
			effects = {
				{
					active = False,
					name = "yuanzhou_juqing01"
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			oldPhoto = True,
			say = "\"The Divine shall descend upon the land.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			stopbgm = True,
			mode = 1,
			asideType = 3,
			soundeffect = "event./ui/fengling",
			sequence = {
				{
					"Time before the World Expo's closing...",
					1
				},
				{
					"10 hours",
					2
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		}
	}
}
