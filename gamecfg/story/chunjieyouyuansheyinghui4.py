return {
	fadeOut = 1.5,
	mode = 2,
	id = "CHUNJIEYOUYUANSHEYINGHUI4",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"The Great Lunar New Year Photoshoot\n\n<size=45>4. Wandering Beyond the Boundaries</size>",
					1
				}
			}
		},
		{
			say = "Port - Dragon Empery Estate",
			side = 2,
			bgName = "star_level_bg_152",
			dir = 1,
			bgm = "main-chunjie3",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_152",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 101491,
			say = "Miss Chen Hai, is th-th-this spot okay?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_152",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 101491,
			say = "I, umm... I think I'm p-p-perfectly relaxed now!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 506010,
			side = 2,
			bgName = "star_level_bg_152",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "(She says that, but she looks about as relaxed as a coiled spring...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_152",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 506010,
			say = "(If anything, telling her to relax is only going to end up being counterproductive. In that case...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_152",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 506010,
			say = "Miss Bristol, I heard you have a knack for adventuring. Do you think you'd be able to picture some place you'd like to explore?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 101491,
			side = 2,
			bgName = "star_level_bg_152",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Picture... an adventure?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_152",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 506010,
			say = "Anything will:. The sky's the limit.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_152",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 502070,
			say = "Hmm... That's an interesting proposal. Bristol, have you ever heard the tale of Carefree Wandering?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_152",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 506010,
			say = "Ah, yes. Now, Bristol, please close your eyes and listen to the story I'm about to tell you.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_152",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 101491,
			say = "Um, okay! I'll give it a try!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_152",
			say = "Bristol closed her eyes, and Hai Tien began to tell her an ancient tale from the Dragon Empery.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_152",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 502070,
			say = "\"In the northern deeps lived the Kun, its length several thousand li immeasurable.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_152",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 506010,
			say = "Once upon a time in the waters of the North, there lived a fish of mythical proportions called the Kun.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_152",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 506010,
			say = "It was said to have been thousands of li in length, a veritable living island.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_152",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 506010,
			say = "You land upon the Kun's back as it drifts across the ocean, seemingly without purpose.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_152",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 101491,
			say = "Woahh... I can already picture how epic that is...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 8,
			side = 2,
			bgName = "star_level_bg_152",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 502070,
			say = "\"Into a Peng it transforms, its length several thousand li immeasurable. When it takes flight, its wings are clouds that blanket the skies.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_152",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 506010,
			say = "Suddenly, the island beneath your feet begins to tremble. When you look around you, the fish has turned into a massive bird that: takes off to the skies.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_152",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 506010,
			say = "The water that splashes from its wings when it takes flight rains:wn from the skies, its huge wings like clouds that descend over the land.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_152",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 506010,
			say = "You cling to its body desperately as to not be thrown from its back––",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_152",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 101491,
			say = "Woaaah!! I'm gonna fall!!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_152",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 506010,
			say = "You: not know how long it has been, but the bird finally stops its ascension and begins to glide serenely across the sky.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_152",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 506010,
			say = "You finally let go and begin to relax. Small birds chirp and gather at your sides, clamoring to play with you.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_152",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 506010,
			say = "You reach into your pocket for a straw, and start to blow colorful little bubbles for them.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 101491,
			side = 2,
			bgName = "star_level_bg_152",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Fwooh...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 502080,
			side = 2,
			bgName = "star_level_bg_152",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "––Now that's a photo-worthy expression! Thank you very much!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_152",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 506010,
			say = "Yes, excellently:ne. Bristol, you can now open your eyes.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 101491,
			side = 2,
			bgName = "star_level_bg_152",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Wait... I can see something in the clouds...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_152",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 502080,
			say = "Oh? What might you be seeing?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 101491,
			side = 2,
			bgName = "star_level_bg_152",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "There's some kind of rainbow-colored cone coming out of the clouds... It seems to be rising up about ten feet, with a diameter about that wide as well...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 502070,
			side = 2,
			bgName = "star_level_bg_152",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "A rainbow-colored cone... Wait, hold on a sec!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_152",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 101491,
			say = "I... I can't look away from it!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 101491,
			side = 2,
			bgName = "star_level_bg_152",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "The cone seems to be covered with strange, uneven scales of some sort...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_152",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 101491,
			say = "At the top of the cone, there's four retractable organs... W-woah, that looks like some kind of evil creature!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_152",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 101491,
			say = "Let's go, giant Kun! Let's go fight it together!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 502070,
			side = 2,
			bgName = "star_level_bg_152",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "You can come back to reality now, Miss Bristol. Our time's almost up~",
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "star_level_bg_152",
			dir = 1,
			blackBg = True,
			say = "And thus, another photoshoot was completed successfully.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
