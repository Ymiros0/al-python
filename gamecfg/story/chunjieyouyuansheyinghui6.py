return {
	fadeOut = 1.5,
	mode = 2,
	id = "CHUNJIEYOUYUANSHEYINGHUI6",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"The Great Lunar New Year Photoshoot\n\n<size=45>6. At Home?</size>",
					1
				}
			}
		},
		{
			say = "Port - Dragon Empery Estate",
			side = 2,
			bgName = "star_level_bg_128",
			dir = 1,
			bgmDelay = 2,
			bgm = "main-chunjie3",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 202301,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Miss Chen Hai, how:es this look?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 506010,
			say = "Oh my, Miss Charybdis... What a daring look you're sporting.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 202301,
			say = "Hm? Did you not request that I wear something capturing the spirit of being \"at home?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 202301,
			say = "To me, that means something that conveys a sense of peace and comfort, something that soothes your spirits the moment you walk through the:or no matter how tired you might be. Isn't that right?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 506010,
			say = "You're right. Did you have a particular pose in mind?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 202301,
			say = "I did, actually. How about getting ready to clean the Com– I mean, the Master's ears?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 202301,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "After a long day of work, the Master returns home exhausted, but finds sanctuary in my lap~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 506010,
			say = "Mm, that's quite interesting. Combining our culture with the Royal Maids' spirit of service.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 506010,
			say = "If you:n't mind, I'd like to take the picture now.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 202301,
			say = "Not at all. Thank you very much.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 506010,
			say = "We're all:ne with the photoshoot. Thank you so much for coming, Miss Charybdis.",
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 312010,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Good job, nya~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 202301,
			say = "You're very welcome. The Royal Maids are here to help after all~♪",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 202301,
			say = "By the way, Akashi, what brings you here?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 312010,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "It's nothing major, nya. Akashi simply came by to check the site of another potential store branch.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 202301,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I see... By the way, can I take these props back with me? I think I might actually want to try servicing the Master a little bit~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 506010,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "That's fine by me.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_128",
			dir = 1,
			blackBg = True,
			actor = 202301,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Thank you very much! If you:n't need anything else from me, I shall take my leave~",:
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
