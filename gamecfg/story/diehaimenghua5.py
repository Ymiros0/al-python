return {
	fadeOut = 1.5,
	mode = 2,
	id = "DIEHAIMENGHUA5",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_xinnong2_3",
			actorName = "{namecode.182}",
			dir = 1,
			bgmDelay = 1,
			say = "(Did I... fall asleep...?) \"Others... approach...\"",
			bgm = "story-4",
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "{namecode.80}",
			bgName = "bg_xinnong2_3",
			nameColor = "#a9f548",
			dir = 1,
			say = "Lady Shinano, is your perception so keen even when you're asleep? Indeed, there's another Sakura Empire fleet in front of us.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "{namecode.27}",
			bgName = "bg_xinnong2_3",
			nameColor = "#a9f548",
			dir = 1,
			say = "Let me confirm! It seems to be the munition ship, Kashino, being escorted back to the main island by Chitose and Chiyoda's group.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "{namecode.182}",
			bgName = "bg_xinnong2_3",
			nameColor = "#a9f548",
			dir = 1,
			say = "\"Munition ship... Kashino...\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "{namecode.80}",
			bgName = "bg_xinnong2_3",
			nameColor = "#a9f548",
			dir = 1,
			say = "Hmm... I:n't remember hearing anything about a transport operation. It's unusual that a munition ship would be sailing with a sortieing fleet.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "{namecode.60}",
			bgName = "bg_xinnong2_3",
			nameColor = "#a9f548",
			dir = 1,
			say = "I wonder what she's carrying with her!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "{namecode.60}",
			bgName = "bg_xinnong2_3",
			nameColor = "#a9f548",
			dir = 1,
			say = "Kashino's been in charge of carrying stuff for Yamato-class guns, right? I know there weren't any Yamato-class ships planned after Lady Shinano, but: you think that...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "{namecode.27}",
			bgName = "bg_xinnong2_3",
			nameColor = "#a9f548",
			dir = 1,
			say = "Curiosity kills the cat, Kumano. Weren't you specifically prohibited from talking about this stuff?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "{namecode.50}",
			bgName = "bg_xinnong2_3",
			nameColor = "#a9f548",
			dir = 1,
			say = "Suzutsuki is right. It seems that Zuikaku and Shoukaku learned something that put them at odds with Akagi. I remember how thick the tension was between them at the last festival...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "{namecode.60}",
			bgName = "bg_xinnong2_3",
			nameColor = "#a9f548",
			dir = 1,
			say = "Spooky, spooky... Guess I really should leave this stuff alone~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "{namecode.50}",
			bgName = "bg_xinnong2_3",
			nameColor = "#a9f548",
			dir = 1,
			say = "But, at least we're headed in the same direction. If we work together, we'll all be safer along the way.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "{namecode.50}",
			bgName = "bg_xinnong2_3",
			nameColor = "#a9f548",
			dir = 1,
			say = "Will you order them to join us, Lady Shinano?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_xinnong2_1",
			actor = 307080,
			dir = 1,
			nameColor = "#a9f548",
			say = "(Does such a weakly individual as myself truly deserve to be surrounded by so many retainers...?)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307080,
			side = 2,
			bgName = "bg_xinnong2_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "\"...They have a much more important duty to accomplish than escorting one such as myself.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_xinnong2_1",
			actor = 307080,
			dir = 1,
			nameColor = "#a9f548",
			say = "\"It would be best to not force the convergence of different commands.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 303180,
			side = 2,
			bgName = "bg_xinnong2_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "Yeah, exactly! ...Hey, Suzutsuki, the heck is she talking about...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 301840,
			side = 2,
			bgName = "bg_xinnong2_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "I think she's trying to say that because our missions are different, there's no need for them to accompany us.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_xinnong2_1",
			actor = 303180,
			dir = 1,
			nameColor = "#a9f548",
			say = "So, we're gonna pass each other by:?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301840,
			side = 2,
			bgName = "bg_xinnong2_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "Kii, we're receiving an incoming communication from the other fleet. Will you respond?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305120,
			side = 2,
			bgName = "bg_xinnong2_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "Sure, put me through. Even if we're not working together, we at least shouldn't forget common courtesy on the high seas as fellow members of the Sakura Empire.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_xinnong2_1",
			actor = 306080,
			dir = 1,
			nameColor = "#a9f548",
			say = "This is Chitose. We are currently carrying out a transport mission to the main island. It's a pleasure to see you.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = False,
			side = 2,
			bgName = "bg_xinnong2_1",
			actor = 305120,
			dir = 1,
			nameColor = "#a9f548",
			say = "This is Kii, also currently on the back to the main island. We appreciate your hard work in escorting our transport ships.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_xinnong2_1",
			actor = 301820,
			dir = 1,
			nameColor = "#a9f548",
			say = "There's an aircraft carrier among their formation... Eh?! Is that Lady Shinano?!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_xinnong2_1",
			actor = 306090,
			dir = 1,
			nameColor = "#a9f548",
			say = "Woahh! How lucky, are we witnessing Lady Shinano's maiden voyage?!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_xinnong2_1",
			paintingNoise = False,
			dir = 1,
			actor = 307080,
			nameColor = "#a9f548",
			say = "\"Long has it been since we last met... It is good to see you all...\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_xinnong2_1",
			actor = 306090,
			dir = 1,
			nameColor = "#a9f548",
			say = "Since we last...? Have we met before, Lady Shinano?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			paintingNoise = False,
			side = 2,
			bgName = "bg_xinnong2_1",
			actor = 307080,
			dir = 1,
			nameColor = "#a9f548",
			say = "\"Nay, those events... were not of this world... This would be our first meeting.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_xinnong2_1",
			actor = 319010,
			dir = 1,
			nameColor = "#a9f548",
			say = "Hehe, this would be my first time seeing you awake! Pleased to meet you, Lady Shinano. I am Kashino, munition ship, currently carrying out a transport mission.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_xinnong2_1",
			actor = 319010,
			dir = 1,
			nameColor = "#a9f548",
			say = "By the way, I heard you just awakened not long ago? Do you feel alright? Is there anything you're having trouble with?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "bg_xinnong2_1",
			paintingNoise = False,
			dir = 1,
			actor = 307080,
			nameColor = "#a9f548",
			say = "\"Just now... This one felt a flicker of strength from within... Kashino, thank you for your contribution to this one's rebirth.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_xinnong2_1",
			actor = 319010,
			dir = 1,
			nameColor = "#a9f548",
			say = "Hehe~ It's all part of my job. No need to thank me.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_xinnong2_1",
			actor = 306080,
			dir = 1,
			nameColor = "#a9f548",
			say = "In any case, Lady Shinano, we won't delay you any further. It is an honor to meet you, and I hope that the rest of your journey is smooth.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_xinnong2_1",
			paintingNoise = False,
			dir = 1,
			blackBg = True,
			actor = 307080,
			nameColor = "#a9f548",
			say = "Mm... May the gods watch over you and ferry you safely to your destination.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
