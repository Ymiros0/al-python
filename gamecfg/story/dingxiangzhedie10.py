return {
	fadeOut = 1.5,
	mode = 2,
	id = "DINGXIANGZHEDIE10",
	once = True,
	fadeType = 2,
	scripts = {
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_zhedie_3",
			stopbgm = True,
			say = "In the midst of the exercise...",
			bgmDelay = 2,
			bgm = "theme-aostelab",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_zhedie_3",
			say = "Meager as our forces are, I still split them up into three groups.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_zhedie_3",
			say = "I assign Northampton to Hornet, have Hammann and Yorktown pair up, and keep Langley with me by the command vessel.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_zhedie_3",
			paintingNoise = True,
			dir = 1,
			actor = 107120,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Tons of enemy aircraft, incoming! Looks like they're from the carrier strike team! Memphis is finally putting her cards on the table!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_zhedie_3",
			paintingNoise = True,
			dir = 1,
			actor = 103260,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Langley, give me a sitrep, ASAP!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107270,
			side = 2,
			bgName = "bg_zhedie_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I'm working on it! Just two more seconds...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_zhedie_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107270,
			say = "Sent! The carrier strike team has several AA-focused corvettes as escorts! Be careful!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107270,
			side = 2,
			bgName = "bg_zhedie_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I'm still working on designating their command ships. Hold off your assault until I'm finished!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_zhedie_3",
			paintingNoise = True,
			dir = 1,
			actor = 107120,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Roger!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107270,
			side = 2,
			bgName = "bg_zhedie_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Commander, Hornet's group is hanging in there, for now.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_zhedie_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107270,
			say = "Unfortunately, things aren't looking very good. The enemy's strength is simply dumbfounding.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_zhedie_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107270,
			say = "In addition to aircraft and drones, they've also deployed a lot of submersibles and mini-submarines.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107270,
			side = 2,
			bgName = "bg_zhedie_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "While we: have the advantage in one-on-one fights, their swarms of expendable weaponry are starting to overwhelm us.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_zhedie_3",
			say = "Anzeel gave me a report the night before which outlined the Type II rigging's construction and the functions it improved upon.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_zhedie_3",
			say = "Chief among them are a general combat effectiveness increase, jamming countermeasures, and built-in electronic warfare capabilities.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_zhedie_3",
			say = "Special mentions should also be given to the enhanced communications and information processing systems.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_zhedie_3",
			say = "These allow shipgirls on the field to directly share info with each other via data links, and draw much more potential out of a shipgirl's hull.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_zhedie_3",
			say = "This technology is the result of a production process that consumes additional Wisdom Cubes...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_zhedie_3",
			say = "And research that, according to Anzeel, I played a significant role in.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_zhedie_3",
			say = "\"We have the upper hand when it comes to information sharing. We have to use it to our advantage.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107270,
			side = 2,
			bgName = "bg_zhedie_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "You mean the data link? I guess so... The increased data processing power is one of the major selling points of the new rigging type.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_zhedie_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107270,
			say = "This makes it possible to analyze data instantaneously and relay the results to shipgirls on the field in almost real-time.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107270,
			side = 2,
			bgName = "bg_zhedie_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It allows us to act as one, based on the orders you issue us.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_zhedie_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107270,
			say = "Your terminal should be relaying information received from our end as well, right? It's an astronomical improvement over the back-and-forth radioing of the past.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_zhedie_3",
			say = "\"So, aircraft can be much better coordinated when they all have access to the same information, right?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107270,
			side = 2,
			bgName = "bg_zhedie_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Correct! That's one of the features you helped develop.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_zhedie_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107270,
			say = "You can also hand over control of certain reconnaissance equipment, something I took advantage of at the beginning of the exercise to continue gathering data while letting the others focus on the fight.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_zhedie_3",
			say = "\"Does the same apply to other carrier-based aircraft? Isn't it possible for carriers to share each other's air wings?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107270,
			side = 2,
			bgName = "bg_zhedie_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Um, let me see... Oh, you can! I can't believe I overlooked that.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_zhedie_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107270,
			say = "I'm glad you're here, Commander. You have a better grasp of the situation than we: individually.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_zhedie_3",
			say = "The truth is, however... I:n't know any more about this new rigging than they:. Maybe even less. They say I helped develop it, but I have no recollection of that at all.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_zhedie_3",
			say = "\"Yorktown, can you hear me?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_zhedie_3",
			paintingNoise = True,
			dir = 1,
			actor = 107100,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Yes, Commander?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_zhedie_3",
			say = "\"How's the situation on your end? Have you been able to cover the anti-submarine warfare?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_zhedie_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107100,
			say = "We've managed to deal with the underwater threat for now...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_zhedie_3",
			paintingNoise = True,
			dir = 1,
			actor = 107100,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "But, the air situation is still not looking too good. While I'm fighting for some airspace control, Hammann has to worry about both anti-ship and anti-submarine combat.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_zhedie_3",
			paintingNoise = True,
			dir = 1,
			actor = 107100,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Sorry, but I won't be able to help the other two groups right now.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_zhedie_3",
			say = "\"Don't worry. In fact, I'm here to lend you a hand.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_zhedie_3",
			say = "\"Your recon planes aren't much use in a:gfight. Let Langley take control of them so she can pinpoint the location of the enemy command ships.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_zhedie_3",
			paintingNoise = True,
			dir = 1,
			actor = 107100,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Give control to Langley? Okay, let me just... There, they're hers now.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_zhedie_3",
			paintingNoise = True,
			dir = 1,
			actor = 101500,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Look out, Yorktown! We've got more mini-subs incoming!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_zhedie_3",
			paintingNoise = True,
			dir = 1,
			actor = 107100,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Shoot, what terrifying numbers... Memphis really is going all-out this time.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_zhedie_3",
			say = "\"Hang in there a bit longer. We'll have our chance to turn things around soon enough.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_zhedie_3",
			paintingNoise = True,
			dir = 1,
			actor = 107100,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Understood! I'll fight with all I've got!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_zhedie_3",
			say = "\"Langley, use all the recon aircraft at your disposal to mark those command ships.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_zhedie_3",
			say = "\"As soon as you: that, I'll give the order to have everyone focus fire on them.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_zhedie_3",
			say = "\"Then, while chaos spreads among the mass-produced ships, we'll deal a devastating blow to Memphis' main fleet and seize victory.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107270,
			side = 2,
			bgName = "bg_zhedie_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Roger!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
