return {
	fadeOut = 1.5,
	mode = 2,
	id = "DINGXIANGZHEDIE14",
	once = True,
	fadeType = 2,
	scripts = {
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_aircraft_future",
			stopbgm = True,
			say = "[Reality Lens Replicated Space] [Location. Chartered plane WFH 'n' Chill Mk. 1] [Cognitive merging data unknown]",
			bgmDelay = 2,
			bgm = "theme-dailyfuture",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_aircraft_future",
			say = "As soon as we get our luggage sorted, Anzeel plops:wn on a plush couch inside her jet's cabin and gazes out the window.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Ooh, that's nice... I tell ya, there's no place like home.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I did a full day's work. Finally, time to kick back and relax...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_aircraft_future",
			say = "\"Speaking of home, I've been wondering about this plane's name.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Hm? You got a problem with the name I gave it?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_aircraft_future",
			say = "\"...Never mind, actually.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "What's with that look on your face? C'mon, grab a coffee and take it easy.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_aircraft_future",
			say = "I accept the cup of hot coffee Anzeel hands me. She goes right back to flipping through some:cuments.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_aircraft_future",
			say = "Not exactly my idea of \"taking it easy.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_aircraft_future",
			say = "Or maybe, reading something is just a way for her to clear her head.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_aircraft_future",
			say = "I've probably been overthinking this too much. I'm not going to get more information about this world if I:n't ask her.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_aircraft_future",
			say = "Last night was a bit too hectic to even make small talk, but this is a good opportunity.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "What are you staring at me for?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Oh, I get it. You're part of the tea faction, aren't you?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_aircraft_future",
			say = "Where should I even begin...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "\"So about that movie, 'The Final Countdown'...\"",
					flag = 1
				}
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "The one where the fleet travels through time?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Pretty crazy how Bonnie got the lead role in that movie, huh?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I already knew though, since I got to sit in on their internal screenings.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Y'know, I would've at least liked to be one of the extras...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "But it just didn't mesh with my schedule, so I couldn't.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Oh, right. You wanted to ask me about something, huh? Maybe time travel?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I'm the kind of person who'd try to change the past, no:ubt about it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Sure, it might cause the preordained future to unravel, but that's not a bad thing, is it?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "If anything,:esn't that mean there's a chance to create a better future?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "In the end, isn't that what humans are always trying to:, with or without time travel?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "For the sake of creating a better future, I think changing the past is perfectly reasonable. Maybe even morally correct.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "But, Aoste probably wouldn't agree with me on that one.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_aircraft_future",
			say = "\"Speaking of Aoste, what has he been up to recently?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "That guy... What:esn't he research? His team has gotten so big these days that he has his hands in every cookie jar at once.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "After all, he's now our prized Renaissance man, master-of-all-trades.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I think he's been focusing on AI research these days though.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I think you've already met his favorite. Does \"Zero\" ring a bell?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "She's part of Project ANTI-X. They seem to be next-generation humanoid weapons.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_aircraft_future",
			say = "...Zero?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_aircraft_future",
			say = "Suddenly, Yorktown comes up to us, holding a tray of pastries.",
			soundeffect = "event./ui/knockdoor1",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Oh, Yorktown. Need something?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107100,
			say = "Doctor, Commander. Would you like some sweets?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107100,
			say = "The new instant mechanical confectioner is working very well.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Sweets, huh? Wait, come again? An instant mechanical confectioner?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			},
			effects = {
				{
					active = True,
					name = "speed"
				}
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I said I'd pay for souvenirs, not whatever that is!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			},
			effects = {
				{
					active = False,
					name = "speed"
				}
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Spill the beans, Yorktown! Who did the deed?! You? Hornet? Or maybe the Commander?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107100,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Hornet bought it. Erm, she said it seemed like fun to make sweets in a chartered plane...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "And she got you hooked, huh?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I get it. What's:ne is:ne, and if it tastes good, maybe it's not all bad...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_aircraft_future",
			say = "Despite her grumbling, she gets up and makes her way to the mechanical confectioner.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_aircraft_future",
			say = "\"The facilities and furniture in this charter plane are too nice to be just a private plane, aren't they?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107100,
			say = "The military provided this plane for the:ctor.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107100,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "As I recall, some corporate alliance from the Union used cutting-edge Energy Cube technology to make it. It can fly smoothly in any weather.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107100,
			say = "Heehee. This is my first time riding, too, Commander.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Commander, this mechanical confectioner is awesome!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_aircraft_future",
			say = "It looks like Yorktown's going to tell me something, but Anzeel interrupts with her shouting from the hallway.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It's seriously so cool! You gotta try it out! I mean, it can make anything!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_aircraft_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107100,
			say = "Shall we, Commander?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
