return {
	fadeOut = 1.5,
	mode = 2,
	id = "DINGXIANGZHEDIE22",
	once = True,
	fadeType = 2,
	scripts = {
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_zhedie_7",
			stopbgm = True,
			say = "With the Devil's support, we finish a series of bombing, recon, and electronic warfare tests.",
			bgmDelay = 2,
			bgm = "battle-hightech",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_zhedie_7",
			say = "Finally, it is time for us to face each other in a head-on test.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107120,
			say = "Man, I thought we'd be here all day, but those drones made things a piece of cake!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107120,
			say = "Despite your name, you're a real angel when it comes to buffing us up!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107120,
			side = 2,
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Lemme tell you, if I ever find myself on the battlefield, I'd want you behind my back!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900335,
			nameColor = "#A9F548FF",
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			actorName = "The Devil XV",
			side = 2,
			say = "Sure thing. I:n't mind having meatshields like you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107100,
			say = "I just hope... it never comes to that. War never accomplishes anything in the end.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107120,
			say = "Haha, relax! It's just a bit of friendly banter!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107100,
			say = "I understand you're just joking, but that is also our duty as shipgirls.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107100,
			say = "If war truly was to break out, I would proudly carry out that duty.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_zhedie_7",
			say = "Yorktown's resolve seems to overlap with the memory of the Yorktown I know.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_zhedie_7",
			say = "She sounds exactly like her. Even inside this simulation, I'm sure she must be the same person.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_zhedie_7",
			say = "\"Devil, are you ready to begin?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900335,
			nameColor = "#A9F548FF",
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			actorName = "The Devil XV",
			side = 2,
			say = "Ready whenever you are.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900335,
			nameColor = "#A9F548FF",
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			actorName = "The Devil XV",
			side = 2,
			say = "Now, let's see how you fare in an actual battle against me.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900335,
			nameColor = "#A9F548FF",
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			actorName = "The Devil XV",
			side = 2,
			say = "Let me remind you that I'm not a frontline fighter. Supporting from the backlines is what I: best.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900335,
			nameColor = "#A9F548FF",
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			actorName = "The Devil XV",
			side = 2,
			say = "Having that said, my vessel is no slouch in a direct engagement.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900335,
			nameColor = "#A9F548FF",
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			actorName = "The Devil XV",
			side = 2,
			say = "In addition to my arsenal of support drones and defensive weapons, I obviously have the mobility to get out of dangerous situations.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900335,
			nameColor = "#A9F548FF",
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			actorName = "The Devil XV",
			side = 2,
			say = "Speaking of those drones, they will be used to strengthen the mass-produced ships to make your job harder.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900335,
			nameColor = "#A9F548FF",
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			actorName = "The Devil XV",
			side = 2,
			say = "While I: not hit as hard as my peers on my own, you will find it difficult to approach me.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900335,
			nameColor = "#A9F548FF",
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			actorName = "The Devil XV",
			side = 2,
			say = "My win condition is simple – I will isolate you, and pick you off one by one.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107100,
			say = "You oversee and coordinate the state of the battle:... Much like the Commander...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107100,
			side = 2,
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "But the difference is, you are already isolated to begin with. Once we take out your mass-produced ships, there's nothing to protect you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 101500,
			say = "That's right! I bet I could take you out single-handedly!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 900335,
			nameColor = "#A9F548FF",
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			actorName = "The Devil XV",
			side = 2,
			say = "Oh yeah? We'll see about that. Remember what I said about mass-produced weapons at the start?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900335,
			nameColor = "#A9F548FF",
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			actorName = "The Devil XV",
			side = 2,
			say = "Well... Just take a look around you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107270,
			side = 2,
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "What?! Yorktown, look at the radar... We're completely surrounded by unidentifiable signals!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107100,
			say = "Are these... also your drones? But, they look humanoid, just like your vessel...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900335,
			nameColor = "#A9F548FF",
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			actorName = "The Devil XV",
			side = 2,
			say = "They're technically both at once. Unlike me, however, they are not controlled by a mainframe.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107100,
			say = "How did you manage to produce that many vessels in such a short period of time?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900335,
			nameColor = "#A9F548FF",
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			actorName = "The Devil XV",
			side = 2,
			say = "I didn't, actually. My current vessel:es not have the ability to mass-produce Enforcers, so I hid them beforehand.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900335,
			nameColor = "#A9F548FF",
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			actorName = "The Devil XV",
			side = 2,
			say = "As for why you didn't notice their presence... It's because I hacked into all your electronics before we even began.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107120,
			say = "When did you... Oh, I bet it was when you transferred all that data to us!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 103260,
			say = "No wonder her communicator acted up... Here I thought she really had:wnloaded too many movies.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900335,
			nameColor = "#A9F548FF",
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			actorName = "The Devil XV",
			side = 2,
			say = "Hmph, make sure you take this lesson in electronic warfare to heart. Whoever controls information also controls the flow of battle.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900335,
			nameColor = "#A9F548FF",
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			actorName = "The Devil XV",
			side = 2,
			say = "Now, allow me to introduce my little helpers.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900335,
			nameColor = "#A9F548FF",
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			actorName = "The Devil XV",
			side = 2,
			say = "Enforcer XV. Temptation. Aircraft carrier. Specialty. using aircraft to harass targets and support allies.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900335,
			nameColor = "#A9F548FF",
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			actorName = "The Devil XV",
			side = 2,
			say = "Enforcer XV. Restriction. Battleship. Specialty. blowing targets to bits in head-on engagements.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900335,
			nameColor = "#A9F548FF",
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			actorName = "The Devil XV",
			side = 2,
			say = "This is the Antiochus force you'll be fighting. It's our integrated combat systems versus you and your Commander's wits.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900335,
			nameColor = "#A9F548FF",
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			actorName = "The Devil XV",
			side = 2,
			say = "Just so you know – this is probably not going to be a very fair fight.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107270,
			side = 2,
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Let's see... Enforcers, Enforcers... Found it. Commander, those Antiochus models boast absurd combat effectiveness.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107270,
			say = "They will make short work of us extremely quickly if we:n't take this 100% seriously.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_zhedie_7",
			say = "Against any other Arbiter, our meager forces would likely make any prospect of victory a pipe dream.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_zhedie_7",
			say = "But I've fought plenty of Enforcers before. If direct combat is this Arbiter's weakness...: there might still be a chance.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "\"There are no challenges we can't overcome.\"",
					flag = 1
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107100,
			say = "Well said, Commander. Let's give it our all.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900335,
			nameColor = "#A9F548FF",
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			actorName = "The Devil XV",
			side = 2,
			say = "So, any final questions before we start?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107120,
			side = 2,
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I have one. Two, actually.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 900335,
			nameColor = "#A9F548FF",
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			actorName = "The Devil XV",
			side = 2,
			say = "As long as it won't affect the experiment results, shoot.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107120,
			say = "Question one. your support drones can't buff us by mistake, can they?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900335,
			nameColor = "#A9F548FF",
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			actorName = "The Devil XV",
			side = 2,
			say = "Nope. They're calibrated to only empower Antiochus units.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107120,
			say = "Figures. Question two. are we using live ammunition or blanks?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900335,
			nameColor = "#A9F548FF",
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			actorName = "The Devil XV",
			side = 2,
			say = "Do you WANT to use live ammunition?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107120,
			say = "Oh, absolutely not. Dr. Aoste would cry if I kicked your asses AND left you full of holes.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900335,
			nameColor = "#A9F548FF",
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			actorName = "The Devil XV",
			side = 2,
			say = "You'll be the one crying at the end of this, KAN-SEN.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107120,
			say = "Big girls:n't cry, and I'm bigger than you in more ways than one. I'll show you why I'm worthy of inheriting the title of Grey Ghost!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 900335,
			nameColor = "#A9F548FF",
			bgName = "bg_zhedie_7",
			hidePaintObj = True,
			dir = 1,
			actorName = "The Devil XV",
			side = 2,
			say = "Hahaha! Those are fighting words! This will be interesting!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
