return {
	fadeOut = 1.5,
	mode = 2,
	id = "DINGXIANGZHEDIE3",
	once = True,
	fadeType = 2,
	scripts = {
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_highschool_future",
			stopbgm = True,
			say = "Anzeel leads us through the streets leisurely.",
			bgmDelay = 2,
			bgm = "theme-schoolfuture",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_highschool_future",
			say = "Though there's hardly a soul outside, the walk:es not feel lonely thanks to the outspoken and hungry researcher.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_highschool_future",
			say = "She tells stories of when she attended this university, and talks about her friendship with a colleague named Aoste.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			nameColor = "#A9F548FF",
			bgName = "bg_highschool_future",
			hidePaintObj = True,
			dir = 1,
			actorName = "Anzeel",
			side = 2,
			say = "...And that's the long and short of it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			nameColor = "#A9F548FF",
			bgName = "bg_highschool_future",
			hidePaintObj = True,
			dir = 1,
			actorName = "Anzeel",
			side = 2,
			say = "He set up a script that automatically signed me up for classes, and I just barely managed to snag enough credits to graduate.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			nameColor = "#A9F548FF",
			bgName = "bg_highschool_future",
			hidePaintObj = True,
			dir = 1,
			actorName = "Anzeel",
			side = 2,
			say = "I tell you, he always thought one step ahead of me. Raising a cat went exactly like he said it would, too.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			nameColor = "#A9F548FF",
			bgName = "bg_highschool_future",
			hidePaintObj = True,
			dir = 1,
			actorName = "Anzeel",
			side = 2,
			say = "By the time I graduated, I never managed to teach it to: my housework.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_highschool_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107120,
			say = "...Wait, what was that about a cat and housework?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			nameColor = "#A9F548FF",
			bgName = "bg_highschool_future",
			hidePaintObj = True,
			dir = 1,
			actorName = "Anzeel",
			side = 2,
			say = "I just told you. Weren't you listening?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_highschool_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 103260,
			say = "I'm preeetty sure ordinary cats can't be taught to clean and stuff.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			nameColor = "#A9F548FF",
			bgName = "bg_highschool_future",
			hidePaintObj = True,
			dir = 1,
			actorName = "Anzeel",
			side = 2,
			say = "No... I guess you're right. It didn't have an ordinary upbringing. I must've taught it wrong.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			nameColor = "#A9F548FF",
			bgName = "bg_highschool_future",
			hidePaintObj = True,
			dir = 1,
			actorName = "Anzeel",
			side = 2,
			say = "It did learn to send and receive emails, though!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 107270,
			side = 2,
			bgName = "bg_highschool_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Could you elaborate on that, please?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			nameColor = "#A9F548FF",
			bgName = "bg_highschool_future",
			hidePaintObj = True,
			dir = 1,
			actorName = "Anzeel",
			side = 2,
			say = "Sure. So, one day I got this email from Aoste that said, \"Nevermind, I guess I won't pry,\" and as it turns out...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107100,
			side = 2,
			bgName = "bg_highschool_future",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Doctor, Professor, I believe the entrance to Granate is over there.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			nameColor = "#A9F548FF",
			bgName = "bg_highschool_future",
			hidePaintObj = True,
			dir = 1,
			actorName = "Anzeel",
			side = 2,
			say = "Huh? Let's see...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			nameColor = "#A9F548FF",
			bgName = "bg_highschool_future",
			hidePaintObj = True,
			dir = 1,
			actorName = "Anzeel",
			side = 2,
			say = "Yeah, this is it! I'm glad it's still here.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 900332,
			nameColor = "#A9F548FF",
			bgName = "bg_highschool_future",
			hidePaintObj = True,
			dir = 1,
			actorName = "Anzeel",
			side = 2,
			say = "Looks like they've modernized quite a bit. Guess they've been:ing well for themselves since I last visited.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			nameColor = "#A9F548FF",
			bgName = "bg_highschool_future",
			hidePaintObj = True,
			dir = 1,
			actorName = "Anzeel",
			side = 2,
			say = "Anyhoo, let's talk inside. You can't come by this campus and NOT pay a visit to Granate!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			say = "We head inside, and Anzeel leads us into a private room.",
			bgmDelay = 2,
			bgm = "theme-dailyfuture",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 900332,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			actorName = "Anzeel",
			side = 2,
			say = "I'm amazed they still accept my old membership card! Now this is why shops that've been around for a while are the best~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			say = "The cafe's interior is a quaint mix of traditional and modern, harmonizing perfectly to create a cozy atmosphere.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			say = "This private room in particular toes a fine line between austere and luxurious, providing a unique, art-like aesthetic.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			say = "Having that said...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "\"Doesn't it seem a bit too high-class?\"",
					flag = 1
				}
			}
		},
		{
			actor = 900332,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			actorName = "Anzeel",
			side = 2,
			say = "Hmm? Well, that's exactly what it's supposed to be. Or so I think, anyway...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			actorName = "Anzeel",
			side = 2,
			say = "By the way, umm...:es anyone know where the menu is?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			say = "Anzeel fumbles around the back of her chair, but finds nothing.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Waiter Robot",
			bgName = "star_level_bg_160",
			nameColor = "#A9F548FF",
			say = "GOOD EVENING. DO YOU NEED SOMETHING?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			actorName = "Anzeel",
			side = 2,
			say = "Whuah?! Where'd this thing come from?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			side = 2,
			actorName = "Waiter Robot",
			bgName = "star_level_bg_160",
			nameColor = "#A9F548FF",
			say = "WELCOME TO GRANATE. I WILL BE YOUR WAITER THIS EVENING. PLEASE HAVE A LOOK AT OUR MENU.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 103260,
			say = "Well, it's dressed to look the part...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			actorName = "Anzeel",
			side = 2,
			say = "Sheesh... Modernizing the shop is one thing, but this is ridiculous.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 900332,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			actorName = "Anzeel",
			side = 2,
			say = "I think I've seen you guys before. Aren't you, like, the fourth generation of E-Cube-powered service bots?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Waiter Robot",
			bgName = "star_level_bg_160",
			nameColor = "#A9F548FF",
			say = "THANK YOU FOR YOUR COMPLIMENT. YOUR PRAISE MOTIVATES US TO CONTINUE IMPROVING OUR SERVICE.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			actorName = "Anzeel",
			side = 2,
			say = "That wasn't a compliment in any... Meh, whatever.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			actorName = "Anzeel",
			side = 2,
			say = "At least they've kept their uniforms the same.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			actorName = "Anzeel",
			side = 2,
			say = "To hold on to tradition, or to embrace modernity... Looks like the owner has thought about both.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			actorName = "Anzeel",
			side = 2,
			say = "Man... Coming here again sure brings back some fond memories.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			actorName = "Anzeel",
			side = 2,
			say = "So I'll have the usual for old times' sake. Seafood paella and a cappuccino, please.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Waiter Robot",
			bgName = "star_level_bg_160",
			nameColor = "#A9F548FF",
			say = "COMING RIGHT UP. WOULD YOU LIKE ANYTHING ELSE?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107120,
			say = "Huh, looks like paella is the house specialty.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107100,
			say = "Isn't that... a pretty hearty meal? This being a café, I expected smaller items on the menu.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			actorName = "Anzeel",
			side = 2,
			say = "See, that's what's unique about this place. They:, of course, offer a more conventional menu as well.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 101500,
			say = "What a weird café...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			actorName = "Anzeel",
			side = 2,
			say = "Call it weird if you want, but I would argue that their paella is the secret reason this café has stuck around so long.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107270,
			say = "Now I can see why you used to be a regular here, Doctor...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			actorName = "Anzeel",
			side = 2,
			say = "Hang on, are you saying I've got weird tastes?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 900332,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			actorName = "Anzeel",
			side = 2,
			say = "Because, let me remind you, it was Aoste who introduced this place to me!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			actorName = "Anzeel",
			side = 2,
			say = "Trust me, he didn't seem like the type of guy to frequent fancy-schmancy places like this.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			actorName = "Anzeel",
			side = 2,
			say = "But it only took one visit, and I was hooked.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			actorName = "Anzeel",
			side = 2,
			say = "In any case, order whatever you want. Everything on their menu is good.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107120,
			say = "Alrighty:, I like the sound of that! Then, hook me up with a Hawaiian pizza and some large fries!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107120,
			say = "What: you want, Hammann? A hamburger combo?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101500,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Do you think I'm a baby? No! I'll have these!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 107270,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "A slice of Black Forest gateau and a cup of black coffee?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 101500,
			say = "You heard me right! Hmph!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107270,
			say = "Maybe I'll have the hamburger combo,:.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 103260,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I think... I'll try the chorizo-cheese paella.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107100,
			say = "Me, I'll have some loaded pub fries, and a cup of herbal tea.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107100,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "What are you going to order, Professor?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			say = "\"I think I'll have the seafood paella as well.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			actorName = "Anzeel",
			side = 2,
			say = "Good taste, my assistant! That should be our final order.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Waiter Robot",
			bgName = "star_level_bg_160",
			nameColor = "#A9F548FF",
			say = "THAT'S TWO SEAFOOD PAELLAS, ONE CHORIZO-CHEESE PAELLA, ONE HAWAIIAN PIZZA, ONE HAMBURGER COMBO...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Waiter Robot",
			bgName = "star_level_bg_160",
			nameColor = "#A9F548FF",
			say = "ONE LARGE FRIES, ONE LOADED PUB FRIES, ONE BLACK FOREST GATEAU, ONE HERBAL TEA, ONE BLACK COFFEE, AND ONE CAPPUCCINO.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Waiter Robot",
			bgName = "star_level_bg_160",
			nameColor = "#A9F548FF",
			say = "IS THAT CORRECT?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			actorName = "Anzeel",
			side = 2,
			say = "Yep, that'll be all!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
