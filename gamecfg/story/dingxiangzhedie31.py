return {
	fadeOut = 1.5,
	mode = 2,
	id = "DINGXIANGZHEDIE31",
	once = True,
	fadeType = 2,
	scripts = {
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_roma_12",
			stopbgm = True,
			say = "World Expo Venue - Iron Blood pavilion",
			bgmDelay = 2,
			bgm = "story-roma-outside",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403030,
			side = 2,
			bgName = "bg_roma_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Hmhm~♪ Hmhm~♪",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 401020,
			say = "You're in high spirits, Eugen.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 403030,
			say = "Of course I am! Hehe,: you know what this is?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 401020,
			side = 2,
			bgName = "bg_roma_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Would this be... information on the blueprint shipgirls of other fleets?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 403030,
			say = "That's right! You can't imagine the effort it took to put all of this together.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403030,
			side = 2,
			bgName = "bg_roma_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "All that remains is the Dragon Empery... It's such a shame that Harbin didn't bother attending the World Expo.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401020,
			side = 2,
			bgName = "bg_roma_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "One would imagine that they didn't allow her to come, lest a certain snoop gets too close to her.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 403030,
			say = "Don't be so mean~ Especially after I brought our cutting-edge Prinz Rupprecht here.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 401020,
			say = "You're clearly trying to show her off. Hah, quite unlike the Empery.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403030,
			side = 2,
			bgName = "bg_roma_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Now, now~ So, how is Ulrich:ing?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401020,
			side = 2,
			bgName = "bg_roma_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "So far, so good.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 401020,
			say = "I bring news, as well. The Empery's delegation will depart tomorrow.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 403030,
			say = "Tomorrow?! What's the rush?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 403030,
			say = "The Commander isn't even here yet... Did they catch wind of something?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 403030,
			say = "Z2,: the Empery girls have plans tonight? Are they meeting with another fleet, or anything?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401020,
			side = 2,
			bgName = "bg_roma_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I wouldn't know. But if they plan to depart tomorrow, I see no reason for them to make plans for the night before?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 403030,
			say = "Good! Let's make plans with them. Dinner, a party, whatever it takes—just get them into our pavilion.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
