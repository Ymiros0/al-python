return {
	fadeOut = 1.5,
	mode = 2,
	id = "DINGXIANGZHEDIE4",
	once = True,
	fadeType = 2,
	scripts = {
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_160",
			stopbgm = True,
			say = "We are waiting for our orders. Hammann seems to have found a strange ice cream machine with a variety of zany flavors, which has attracted the attention of the others.",
			bgmDelay = 2,
			bgm = "theme-dailyfuture",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			say = "Everyone except Anzeel, that is, who remains in place as if deep in contemplation.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			say = "Has she noticed my strange behavior?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "\"You're not going to take a look at the ice cream machine?\"",
					flag = 1
				}
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Did you know that it didn't always use to be a self-serve? You used to have to wait for people to bring out your ice cream.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "But, that's one thing I:n't miss about this place. Those flavors aren't something you come back for.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Like, pepperoni and jalapeno. Who came up with that anyway?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			say = "\"I:n't know about you, but that sounds like it could work.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Are you serious? It's your funeral.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "At any rate, only kids would get fired up over an ice cream machine. Do I look like a kid to you?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...Or, should I ask, how:es the current \"you\" perceive me?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			say = "Sure enough, she has noticed.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "You've been acting odd this whole time. Something about you just:esn't sit right with me.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Maybe you're just having a bad day. Maybe I'M just having a bad day. Or maybe...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "You've been replaced by aliens.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			say = "\"Aliens? Calling that far-fetched would be an understatement.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It's a non-zero possibility, so it could theoretically happen. Heck, it'd take an alien to enjoy pepperoni and jalapeno ice cream.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "So... Let's have a little pop quiz just to see if you're an alien imposter or not~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			say = "\"...Are you serious, or are you just messing with me?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Do I look like I'm messing with you? C'mon, just take a seat right over there~!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 0,
					delay = 0,
					dur = 0.4,
					x = 30,
					number = 2
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			say = "I:n't have much of a choice.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			say = "All of a sudden, I feel the hairs on my back standing on end, as if an electric current just ran:wn my spine.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			say = "The words coming out of my mouth seem to bypass my brain.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			say = "It's as if I subconsciously have always been used to these kinds of interactions with Anzeel.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "First question. Who am I?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			say = "\"You're Dr. Anzeel. You discovered Wisdom Cubes and pioneered their application, and now lead the Oceanic Federation's Wisdom Cube Research Team...\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Uhh... I said I wasn't messing with you, but you also:n't need to recite my whole CV...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			say = "The words seem to flow right out of my mind, as if I had known this information my whole life.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Next question. What am I currently researching?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			say = "\"Practical applications of Wisdom Cubes, and the effect of nurture on KAN-SEN.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "What's the reason I came back to my alma mater?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			say = "\"To test the effect of Type II hull-fusion rigging on KAN-SEN combat capabilities.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Right... Who's the general I talked about earlier?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			say = "\"The vice-admiral of the Eagle Union Navy, head of Division 13 and supervisor of Cube weaponization research.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Finally, who is Dr. Aoste?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			say = "\"Your colleague at the university, a leading expert in the fields of Energy Cubes and artificial intelligence, as well as head of the 'Antiochus' project.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Hahaha! You actually answered everything!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			say = "\"Am I in the clear now?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Hmm... Actually, I have one more question, but my gut tells me it's not a good idea to ask it right now.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...Forget I said that. You're in the clear.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I can safely say that you have not been abducted by aliens. We all have those rough days, huh?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			say = "...How in the world did I manage to answer those questions?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			say = "The Oceanic Federation, Division 13... I have seen some of those terms before, during my adventures in the Northern Parliament as well as at the Sea of Stars.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			say = "But, I couldn't even guess as to how they were related, nor did I know how I tied together all those threads of information into something coherent.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			say = "It left me with a bizarre feeling I couldn't quite put my finger on.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			say = "Almost as if some memories locked away deep inside me had been awakened at just the right time.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			say = "But how is any of this happening...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107120,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Guess who's back with ice cream!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 107100,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "We got something for each of you as well!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 103260,
			say = "(She's really gonna: it?)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 101500,
			say = "(If Yorktown said it's fine, no harm to it, right?)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107270,
			say = "(Oh boy... I suppose it'll be fun to see how they react.)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 101500,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Starting with the one I chose~!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "The colors on those cones... Ugh, those are nasty flavors.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107120,
			say = "Allow me to tell you your choices!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107120,
			say = "The one with the yellow cone is yogurt beef flavor.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107120,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "And the red cone, that's pepperoni flavor!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Pepperoni? Wow, that's the Professor's favorite!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			},
			effects = {
				{
					active = True,
					name = "speed"
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 101500,
			say = "I-it is? Okay, it's yours!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_160",
			say = "Anzeel forces the pepperoni-flavored ice cream on me before I have a chance to respond.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			effects = {
				{
					active = False,
					name = "speed"
				}
			}
		},
		{
			actor = 900332,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Come on, chow:wn! Don't let perfectly good food go to waste!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
