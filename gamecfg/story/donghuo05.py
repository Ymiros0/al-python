return {
	fadeOut = 1.5,
	mode = 2,
	id = "DONGHUO05",
	once = True,
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			actor = 0,
			side = 2,
			nameColor = "#ff0000",
			actorName = "？？？",
			dir = 1,
			blackBg = True,
			say = "The special observation target, \"Code G,\" hasn't been captured yet... And now, an Eagle Union task force has come to crash the party.",
			withoutPainting = True,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 0,
			side = 1,
			withoutPainting = True,
			actorName = "？？？",
			dir = 1,
			blackBg = True,
			nameColor = "#ff0000",
			say = "It seems that the signal secretly sent out by Code G has attracted some unintended guests...",
			paintingFadeOut = {
				time = 0.5,
				side = 0
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 0,
			side = 0,
			nameColor = "#ff0000",
			actorName = "？？？",
			dir = 1,
			blackBg = True,
			say = "In that case, I'll move my Executors as well. They're rather outdated and:n't have much going for them in the personality department, but they should be more than a match for them.",
			withoutPainting = True,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 0,
			side = 1,
			nameColor = "#ff0000",
			actorName = "？？？",
			dir = 1,
			blackBg = True,
			say = "\"Should be?\" A concept as amorphous as that should not exist within our program.",
			withoutPainting = True,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 0,
			side = 1,
			nameColor = "#ff0000",
			actorName = "？？？",
			dir = 1,
			blackBg = True,
			say = "Don't underestimate them. For now, locating Code G is our top priority.",
			withoutPainting = True,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 0,
			side = 1,
			nameColor = "#ff0000",
			actorName = "？？？",
			dir = 1,
			blackBg = True,
			say = "As for those Eagle Union cowgirls... they can be used for our new combat test... two birds with one stone, as they like to say.",
			withoutPainting = True,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 0,
			side = 0,
			nameColor = "#ff0000",
			actorName = "？？？",
			dir = 1,
			blackBg = True,
			say = "Understood. Diverting system resources accordingly.",
			withoutPainting = True,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 0,
			side = 1,
			nameColor = "#ff0000",
			actorName = "？？？",
			dir = 1,
			blackBg = True,
			say = "Code G... How should I lure you out this time? Heehee...",
			withoutPainting = True,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
