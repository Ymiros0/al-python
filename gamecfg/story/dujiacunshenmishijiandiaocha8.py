return {
	fadeOut = 1.5,
	mode = 2,
	id = "DUJIACUNSHENMISHIJIANDIAOCHA8",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			paintingNoise = True,
			side = 2,
			bgName = "star_level_bg_518",
			actorName = "In-Story Narration",
			stopbgm = True,
			bgm = "cw-story",
			actor = 307080,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "It was a fervid summer day... The skies were clear, and the sun hung high in the sky...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			actor = 101490,
			actorName = "Mysterious Voice 1",
			nameColor = "#A9F548FF",
			say = "Hey! They got a new narrator!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			actor = 301890,
			actorName = "Mysterious Voice 2",
			nameColor = "#A9F548FF",
			say = "And new actors, too. The Commander's the lead now, and it's our turn to watch the play.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			actor = 501020,
			actorName = "Mysterious Voice 3",
			nameColor = "#A9F548FF",
			say = "We should be quiet:! Like audience members are supposed to!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			actor = 307080,
			actorName = "In-Story Narration",
			nameColor = "#A9F548FF",
			say = "Ahem... As this one was saying...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_518",
			paintingNoise = True,
			actorName = "In-Story Narration",
			actor = 307080,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "An army marched the path through the woodlands. Their general – the Commander – was resting inside a sedan chair, carried by the troops...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Wait, a sedan chair? Is this story set during the Warring States period?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_518",
			paintingNoise = True,
			actorName = "In-Story Narration",
			actor = 307080,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Indeed... The Commander's sedan chair was large and highly ornate, filled with curiosities and antiques – more than one could count...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "It IS pretty spacious in here. It could: without all the boxes and bottles, though...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "And this uniform... it's very different to my usual one. I look like some sort of Edo-period magistrate.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Safe to assume, I must be playing an extremely wealthy person.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			actor = 301290,
			actorName = "Fleet-Footed Retainer",
			nameColor = "#A9F548FF",
			say = "I-I bring you a report, Milord! Our battle on the front is unfolding smoothly!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301290,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			side = 2,
			actorName = "Fleet-Footed Retainer",
			say = "Milord's vanguard has already seized two castles with all the ease of a walk through the grasslands! Victory shall surely be yours soon!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301900,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			side = 2,
			actorName = "Boastful Retainer",
			say = "Hahahah! Of course! Our great army's victory is inevitable, particularly with Milord at the helm!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			actor = 301900,
			actorName = "Boastful Retainer",
			nameColor = "#A9F548FF",
			say = "Our foes would be wise to surrender, and instead they struggle like imbeciles! They must be taught a lesson!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301820,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			side = 2,
			actorName = "Graceful Retainer",
			say = "I-I think you should calm yourself... It's hot today, and you risk a heatstroke if your anger overboils.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301290,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			side = 2,
			actorName = "Fleet-Footed Retainer",
			say = "Milord! A large group of villagers have gathered on the road ahead of us!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			actor = 301290,
			actorName = "Fleet-Footed Retainer",
			nameColor = "#A9F548FF",
			say = "They knew of Milord's coming and are offering alcohol and rations to thank Milord's army!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			actor = 301900,
			actorName = "Boastful Retainer",
			nameColor = "#A9F548FF",
			say = "So the commoners embrace the tides of change. Milord, shall we halt and savor a moment's respite?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			actor = 301820,
			actorName = "Graceful Retainer",
			nameColor = "#A9F548FF",
			say = "I support this idea. It's sweltering, and the villagers are extending their goodwill. Let us stop and rest.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			actor = 301820,
			actorName = "Graceful Retainer",
			nameColor = "#A9F548FF",
			say = "As for where... Yes, I think the flat plain by the side of the mountain shall:. It's not the most spacious, but it's adequate to set up camp.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Hmm...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "Aye! Rest we shall!",
					flag = 1
				},
				{
					content = "Hold your horses!",
					flag = 2
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_518",
			paintingNoise = True,
			actorName = "In-Story Narration",
			optionFlag = 1,
			actor = 307080,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "The army set up camp by the mountain, gorged themselves on the offerings of the villagers, and enjoyed their break...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			optionFlag = 2,
			say = "Can we really stop and nonchalantly put:wn camp by the roadside? Aren't we marching for war?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			optionFlag = 2,
			say = "(The location is suspect. It's a long way away from our other units, and the enemy may ambush us from over the mountain...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_518",
			paintingNoise = True,
			actorName = "In-Story Narration",
			optionFlag = 2,
			actor = 307080,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "The army set up camp by the mountain, gorged themselves on the offerings of the villagers, and enjoyed their break...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			optionFlag = 2,
			say = "What? I thought I as the general would decide!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			optionFlag = 2,
			say = "(I suppose not... Something tells me I:n't get a say in this story.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_111",
			hidePaintObj = True,
			say = "Moments later, the scene changes to a massive encampment.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_111",
			hidePaintObj = True,
			say = "A feast of delectable dishes is lined up before me. I look around me and see swords, books, treasures... and even exotic animals around the camp.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_111",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "The flyer wasn't lying about taking you from one magic place to the next.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_111",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Why would a marching army be hauling these things around, though?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107030,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_111",
			hidePaintObj = True,
			side = 2,
			actorName = "Attractive Dancer",
			say = "Oh, my General, why are you all alone in here? Everyone outside is enjoying their drinks~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305130,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_111",
			hidePaintObj = True,
			side = 2,
			actorName = "Passionate Dancer",
			say = "Perhaps it's because... you really wanted to see us? Heehee♪",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305130,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_111",
			hidePaintObj = True,
			side = 2,
			actorName = "Passionate Dancer",
			say = "There's no one else here but you, me, and her... You can: aaanything you want with us♥",:
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_111",
			hidePaintObj = True,
			actor = 107030,
			actorName = "Attractive Dancer",
			nameColor = "#A9F548FF",
			say = "Good point! The only question is – WHAT would you:, hmm? Heehee♥",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_111",
			hidePaintObj = True,
			actor = 107030,
			actorName = "Attractive Dancer",
			nameColor = "#A9F548FF",
			say = "...Excuse me, why won't you look at me?! My script says I'm the attractive dancer!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107030,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_111",
			hidePaintObj = True,
			side = 2,
			actorName = "Attractive Dancer",
			say = "Come on, say it~ Aren't I attractive?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_111",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Um... Yeah.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_508",
			bgm = "musashi-2",
			actorName = "In-Story Narration",
			paintingNoise = True,
			soundeffect = "event./ui/dalei",
			actor = 307080,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "The lovely mood was suddenly disrupted by the rumble of lightning...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			},
			flashN = {
				color = {
					1,
					1,
					1,
					1
				},
				alpha = {
					{
						0,
						1,
						0.5,
						0
					},
					{
						1,
						0,
						0.5,
						0.2
					}
				}
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_508",
			paintingNoise = True,
			actorName = "In-Story Narration",
			actor = 307080,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "In mere moments, a rainstorm had swept over the camp...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "That sure came out of nowhere.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "I feel like I've forgotten something really important...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "Umbrella! I need to find an umbrella!",
					flag = 1
				},
				{
					content = "Where are we now?",
					flag = 2
				}
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			optionFlag = 1,
			say = "...Well, I:, but more importantly – where even am I?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301290,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			side = 2,
			actorName = "Fleet-Footed Retainer",
			say = "We are in Okehazama, Milord!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			actor = 301290,
			actorName = "Fleet-Footed Retainer",
			nameColor = "#A9F548FF",
			say = "It's shocking how quickly rain came upon us, when the skies were clear just moments ago... An unfortunate change of weather.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "I see. So we're in Okehazama...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "Let us play kemari!",
					flag = 1
				},
				{
					content = "Wait a second...",
					flag = 2
				}
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			optionFlag = 2,
			say = "It's been a really hot day, rain and thunder came suddenly...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			optionFlag = 2,
			say = "And I'm playing a general in the Warring States period... Oh no!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			optionFlag = 2,
			say = "We must prepare for battle, posthaste!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_518",
			paintingNoise = True,
			actorName = "In-Story Narration",
			actor = 307080,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Meanwhile, under the cover of the thunderstorm, a cavalry unit had gathered in an attacking position atop the mountain...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			},
			effects = {
				{
					active = True,
					name = "juqing_xiayu_da"
				}
			}
		},
		{
			actor = 105031,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_518",
			side = 2,
			actorName = "Valiant Officer",
			say = "We are in position. Awaiting orders to commence the attack.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 204034,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_518",
			side = 2,
			actorName = "Dignified Officer",
			say = "Heehee. Imagine a sudden rainstorm whipping up to our benefit. The heavens are on our side.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107091,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_518",
			side = 2,
			actorName = "Diligent Officer",
			say = "Indeed. Milord's unyielding determination to attack has resulted in the perfect opportunity for us.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 299033,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_518",
			side = 2,
			actorName = "Adorable Lord",
			say = "Heehee~ Keep the praise coming~♪",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 206039,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_518",
			side = 2,
			actorName = "Timid Officer",
			say = "D-do we really need to fight?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 702042,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_518",
			side = 2,
			actorName = "Principled Officer",
			say = "Yes. There is no room for personal feelings on the battlefield.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 205064,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_518",
			side = 2,
			actorName = "Self-Confident Officer",
			say = "Milord, give us the order, and the cavalry will charge. We shall return to you victorious.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499090,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_518",
			side = 2,
			actorName = "Impatient Officer",
			say = "I am on the brink of keeling over from boredom. Let's begin, shall we?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Hmm...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			},
			options = {
				{
					content = "Cavalry? But some aren't even riding horses!",
					flag = 1
				},
				{
					content = "Really? They all count as cavalry?",
					flag = 2
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_518",
			paintingNoise = True,
			actorName = "In-Story Narration",
			optionFlag = 1,
			actor = 307080,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Apologies, but we lacked the resources for a complete cavalry unit...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "star_level_bg_518",
			actorName = "In-Story Narration",
			optionFlag = 2,
			actor = 307080,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Some say that a soldier atop any vehicle of war may be considered cavalry...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_518",
			paintingNoise = True,
			actorName = "In-Story Narration",
			actor = 307080,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Ahem. Unfortunately, we did not have sufficient time to provide uniform equipment for the cavalry unit...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			actor = 307080,
			actorName = "In-Story Narration",
			nameColor = "#A9F548FF",
			say = "Regardless, they all exhibit a burning fighting spirit under their lord's leadership...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			actor = 299033,
			actorName = "Adorable Lord",
			nameColor = "#A9F548FF",
			say = "My Owner's waiting for us:wn there~♪",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			actor = 299033,
			actorName = "Adorable Lord",
			nameColor = "#A9F548FF",
			say = "Troops, hear me! Go get 'em!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			actor = 299033,
			actorName = "Adorable Lord",
			nameColor = "#A9F548FF",
			say = "Your target is my Owner, and my Owner alone!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "My troops camped nearby won't be able to make it here in time due to the rainstorm...",
			effects = {
				{
					active = False,
					name = "juqing_xiayu_da"
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "This is bad. Very bad. My camp is isolated and exposed...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			say = "I can already hear rallying cries calling out from outside my tent.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			say = "Then, the opposing lord lifts the flap and comes running into my tent!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "So this is the end...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			actor = 299033,
			actorName = "Adorable Lord",
			nameColor = "#A9F548FF",
			say = "Owneeeeer! There's no escaping your fate!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			actor = 299033,
			actorName = "Adorable Lord",
			nameColor = "#A9F548FF",
			say = "Come heeeere!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			say = "She lunges at me like a cat, arms outstretched... without a weapon!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			actorName = "In-Story Narration",
			blackBg = True,
			bgm = "cw-story",
			actor = 307080,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "The battle was over, and peace was restored to the lands...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 4,
			side = 2,
			paintingNoise = True,
			actorName = "In-Story Narration",
			blackBg = True,
			actor = 307080,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "And so, the curtains fall on this act...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			factiontag = "Here With My Owner!",
			dir = 1,
			blackBg = True,
			actor = 299030,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Owneeer! *squeeze* *cuddle* *sniff*",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 102160,
			side = 2,
			nameColor = "#A9F548FF",
			factiontag = "Perpetually Overworked",
			dir = 1,
			blackBg = True,
			hidePaintObj = True,
			say = "It's over now. You can stop hugging the Commander to death...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			factiontag = "Here With My Owner!",
			dir = 1,
			blackBg = True,
			actor = 299030,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "I'm not letting go of my Owner even if it's over! ...Hey, when did you get here, Memphis?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 102160,
			side = 2,
			nameColor = "#A9F548FF",
			factiontag = "Perpetually Overworked",
			dir = 1,
			blackBg = True,
			hidePaintObj = True,
			say = "I've been here this whole time!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			blackBg = True,
			say = "Well, I suppose that's the end...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			blackBg = True,
			say = "Or...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
