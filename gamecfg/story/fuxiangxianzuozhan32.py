return {
	fadeOut = 1.5,
	mode = 2,
	id = "FUXIANGXIANZUOZHAN32",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			say = "Floating Fortress E2 - A few days later",
			side = 2,
			bgName = "bg_fuxiangxian_6",
			dir = 1,
			bgm = "bsm-5",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Prinz Heinrich",
			bgName = "bg_fuxiangxian_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "Alright, and that's it! That concludes today's tactics meeting! Thanks for having us, Strasser!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Peter Strasser",
			bgName = "bg_fuxiangxian_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "You: know it's only been 15 minutes since I've started to talk,:n't you? I took time out of my precious schedule to go over this information with you. I won't let you escape until I'm:ne going over every... single... point.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Prinz Heinrich",
			bgName = "bg_fuxiangxian_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "But it feels like it's been 15 hours already... Why am I here, just to suffer...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_fuxiangxian_6",
			actorName = "Prinz Heinrich",
			dir = 1,
			say = "Eugen, please come save me, before it's too late...",
			dialogShake = {
				speed = 0.09,
				x = 10,
				number = 2
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Peter Strasser",
			bgName = "bg_fuxiangxian_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "Eugen's in the classified facility. Besides, Eugen wouldn't be dumb enough to ask me to let you go anyway.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Prinz Heinrich",
			bgName = "bg_fuxiangxian_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "Et tu, Eugen?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Peter Strasser",
			bgName = "bg_fuxiangxian_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "I have to say though, I'm thoroughly disappointed with your attitude. Just when I was about to make you a focal part of the Iron Blood's main fleet, considering you potential.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Prinz Heinrich",
			bgName = "bg_fuxiangxian_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "Is it wrong to just want to fight? Why sweat the small details anyway!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Peter Strasser",
			bgName = "bg_fuxiangxian_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "As representatives of the Iron Blood, calling the enemies \"octopus\" and \"cockroach\" in front of our close allies stains our name.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Peter Strasser",
			bgName = "bg_fuxiangxian_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "Though generous as I am, know that I have no problem taking time out of my busy schedule to... educate you. I'll have you know that my stern lectures are just as \"informative\" as Z23's.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Prinz Heinrich",
			bgName = "bg_fuxiangxian_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "Noooo! Never! Anything but that!! Reading boring books makes me all sleepy and stuff...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Prinz Heinrich",
			bgName = "bg_fuxiangxian_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "At least give me something cool and action-packed!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Peter Strasser",
			bgName = "bg_fuxiangxian_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "I see. This must indeed be too easy for someone of your caliber. How about something on Graf Zeppelin's world view? It's a bit philosophical in nature, but...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Prinz Heinrich",
			bgName = "bg_fuxiangxian_6",
			nameColor = "#a9f548",
			dir = 1,
			blackBg = True,
			say = "I:n't even know what you're talking about anymore!!!!!!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
