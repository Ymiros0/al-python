return {
	fadeOut = 1.5,
	mode = 2,
	id = "FUXIANGXIANZUOZHAN33",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			say = "Taranto Harbor",
			side = 2,
			bgName = "bg_guild_red",
			dir = 1,
			bgmDelay = 2,
			bgm = "story-italy",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_guild_red",
			actor = 601020,
			dir = 1,
			nameColor = "#a9f548",
			say = "Over. That's the end of my report of my observations during my stay with the Iron Blood, Lady Littorio.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 8,
			side = 2,
			bgName = "bg_guild_red",
			paintingNoise = False,
			dir = 1,
			actor = 605020,
			nameColor = "#a9f548",
			say = "Good work. You've been a great help.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_guild_red",
			actor = 601020,
			dir = 1,
			nameColor = "#a9f548",
			say = "Lady Littorio, concerning the demonstration of the Iron Blood's new weapons and technology. How would you like for me to respond to their request for me to stay a bit longer to see them?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = False,
			side = 2,
			bgName = "bg_guild_red",
			actor = 605020,
			dir = 1,
			nameColor = "#a9f548",
			say = "If they're going to that length to show us what they've accomplished, it'd be rude to turn them:wn. Sorry, but would you mind staying there a bit longer?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_guild_red",
			actor = 601020,
			dir = 1,
			nameColor = "#a9f548",
			say = "Yes ma'am! For the glory of the Sardegna Empire, I will undertake any mission!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "bg_guild_red",
			paintingNoise = False,
			dir = 1,
			actor = 605020,
			nameColor = "#a9f548",
			say = "(The facility that was built in the Mirror Sea was attacked by the Sirens... Hmm)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_guild_red",
			actor = 605020,
			dir = 1,
			nameColor = "#a9f548",
			say = "(I'm really curious to hear about the incident in person, but... I'm rather in shock over how overwhelming the Iron Blood's strength actually is.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_guild_red",
			actor = 605020,
			dir = 1,
			nameColor = "#a9f548",
			say = "(Mmmn... As the guiding voice of the Sardegna Empire, what should I: about this situation...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_guild_red",
			actor = 605050,
			dir = 1,
			nameColor = "#a9f548",
			say = "Littorio, the delegation has arrived. Which one of you will head out to greet them?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 605020,
			side = 2,
			bgName = "bg_guild_red",
			nameColor = "#a9f548",
			dir = 1,
			say = "I'll head out. Make sure to let Veneto know as well.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_guild_red",
			actor = 605050,
			dir = 1,
			nameColor = "#a9f548",
			say = "Understood.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 8,
			side = 2,
			bgName = "bg_guild_red",
			actor = 605020,
			dir = 1,
			nameColor = "#a9f548",
			say = "(Well, whatever. I can think about the more troublesome issues later.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 605020,
			side = 2,
			bgName = "bg_guild_red",
			nameColor = "#a9f548",
			dir = 1,
			blackBg = True,
			say = "(After all, today's a promising day. I'll have to greet the guest from far away properly.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
