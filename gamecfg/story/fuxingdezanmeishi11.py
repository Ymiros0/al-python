return {
	id = "FUXINGDEZANMEISHI11",
	mode = 2,
	once = True,
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			expression = 1,
			side = 2,
			actor = 607010,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Ugh... It's no good! There seems to be some kind of \"wall\" stopping my aircraft...",
			bgm = "story-italy",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			actor = 607010,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "My planes are just coming to a halt... They're not getting damaged, but there:esn't seem to be any way past the wall!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			actor = 607010,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I'm able to get a birds' eye view of the Labyrinth, but there's some kind of haze coming from the passages that makes it impossible to see where we are.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 601040,
			nameColor = "#a9f548",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "Miss Veneto... This Labyrinth must be a trap set by the Sirens!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 6,
			side = 2,
			actor = 601040,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "According to Carabiniere, Mirror Seas are supposed to be Siren research facilities, but I:n't think there's such a rational explanation for this labyrinth...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			actor = 605010,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "(That's a valid point... I can't see what the purpose would be of erecting such a large labyrinth.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			actor = 605010,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "(But there's no knowing what machinations the Sirens are scheming up within their research facilities... We've already heard about some that defy all reason.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			actor = 605010,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "(There's no telling what waits for us in there, but I:n't think there's any other way to get to the bottom of this and rescue Littorio and Gioberti.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			actor = 605010,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "It's alright, Maestrale. Trap or not, there is nothing that we cannot overcome.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 605010,
			nameColor = "#a9f548",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "O Eternal Glory of Sardegna, guide our path—",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
