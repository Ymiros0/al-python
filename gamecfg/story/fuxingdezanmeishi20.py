return {
	fadeOut = 1.5,
	mode = 2,
	id = "FUXINGDEZANMEISHI20",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			expression = 3,
			side = 2,
			bgName = "bg_italyv2_3",
			dir = 1,
			bgmDelay = 1,
			bgm = "story-italy",
			actor = 605010,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "What did Friedrich and I discuss at the last faction summit? Why: you ask, Littorio? I thought you weren't interested in politics.",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_italyv2_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 605020,
			say = "Make no mistake, I am not. But me being clueless about political developments would not stand in the event you were incapacitated, my dear Eternal Flagship.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_italyv2_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 605010,
			say = "I'm sure we would be just fine with you taking over the fleet~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_italyv2_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 605010,
			say = "In any case, we mostly talked about the Mirror Sea that Carabiniere went to. Apparently, a battle against the Sirens broke out there.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_italyv2_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 605010,
			say = "They were able to emerge victorious thanks to the efforts of their new carriers, Peter Strasser, as well as our Carabiniere.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_italyv2_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 605020,
			say = "There must be a reason behind that as well. Veneto, you also:n't believe that their involvement with the Sirens ends at merely \"using captured technology,\": you?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 605020,
			side = 2,
			bgName = "bg_italyv2_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I've heard that the Northern Parliament uses captured technology as well, though nothing to the extent of what the Iron Blood has at their disposal.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_italyv2_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 605010,
			say = "While I see your point, what good:es it: for us to distrust them?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_italyv2_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 605010,
			say = "Now that I think about it, you were opposed to letting Friedrich visit Sardegna, weren't you?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 8,
			side = 2,
			bgName = "bg_italyv2_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 605020,
			say = "The better question is, why weren't you? You practically rolled out the red carpet for her!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_italyv2_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 605010,
			say = "Huh, was that a poor decision on my part?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_italyv2_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 605020,
			say = "Now the realization dawns on you? Your ignorance is sometimes as stunning as your beauty, \"signorina.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_italyv2_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 605010,
			say = "Oh my, I never knew you had... such a sharp tongue. How fascinating.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_italyv2_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 605020,
			say = "But yes, I knew that she was interested in prying into the inner workings of our Senate.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 605010,
			side = 2,
			bgName = "bg_italyv2_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Oh? Have you spoken to Friedrich as well?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_italyv2_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 605020,
			say = "That I have. She asked me how I felt about our leaders, and if I was interested in further spreading Sardegna's splendor.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 8,
			side = 2,
			bgName = "bg_italyv2_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 605020,
			say = "I was more than happy to talk about the latter, but the former? I had no desire to invite that kind of trouble, so I said nothing.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_italyv2_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 605010,
			say = "It's good that you were able to have a conversation with her. And even better that you were able to carefully watch your words.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "bg_italyv2_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 605020,
			say = "I think it's fair to say that I have some experience with that. My signorina, the two of you can be quite similar in certain regards~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_italyv2_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 605010,
			say = "Hehe~ I'm glad you're sufficiently perceptive. In any case, the splendor of Sardegna is something that Sardegnians should spread, using Sardegnian methods.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_italyv2_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 605010,
			say = "The routes in the Mediterranean that connect countless peoples and cultures have been an important part of human history. Only through keeping it safe for all... can the world truly understand what it means to be Sardegnian.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 8,
			side = 2,
			bgName = "bg_italyv2_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 605020,
			say = "Yes, we shall spread peace and prosperity in a way that has never been seen in the past, first throughout the Mediterranean,: all of Europa.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_italyv2_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 605020,
			say = "These noble dreams and aspirations are a constant reminder that we are more than weapons of war.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "bg_italyv2_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 605020,
			say = "More than ever, we understand the importance of shattering this Siren Labyrinth and retrieving the stolen artwork.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 8,
			side = 2,
			bgName = "bg_italyv2_3",
			dir = 1,
			blackBg = True,
			actor = 605020,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "No matter how many obstacles lie in our path, we will never lose sight of our goal!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
