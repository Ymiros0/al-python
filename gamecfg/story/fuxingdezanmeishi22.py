return {
	id = "FUXINGDEZANMEISHI22",
	mode = 2,
	once = True,
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			actor = 602010,
			nameColor = "#a9f548",
			side = 2,
			bgm = "blueocean-image",
			dir = 1,
			say = "That's the end of them.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 605020,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "All their efforts amount to is a temporary hindrance at best in the face of Sardegna's greatest fleet.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			nameColor = "#a9f548",
			side = 2,
			actor = 607010,
			dir = 1,
			say = "Allow me to handle the skies. I shall support you while scouting ahead to the best of my abilities.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 8,
			nameColor = "#a9f548",
			side = 2,
			actor = 605020,
			dir = 1,
			say = "Of course, just:n't overextend yourself now.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 6,
			nameColor = "#a9f548",
			side = 2,
			actor = 602010,
			dir = 1,
			say = "Yawn. I can't be bothered to keep track of how many enemies we've defeated by now.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 602010,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "(As trivial as they are to fight, they still eat away at our ammo reserves. Furthermore, more of them seem to appear every time the labyrinth changes...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 601060,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "Over here, folks! This-a way!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			nameColor = "#a9f548",
			side = 2,
			actor = 601050,
			dir = 1,
			say = "You heard her! Last one there's a rotten egg, Maestrale!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 601040,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "W-wait! Don't go running off! *sigh*...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 8,
			nameColor = "#a9f548",
			side = 2,
			actor = 605020,
			dir = 1,
			say = "We've made significant progress through the labyrinth now, all thanks to Aquila's eyes in the skies.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 7,
			nameColor = "#a9f548",
			side = 2,
			actor = 605020,
			dir = 1,
			say = "But, though we've traveled a great distance, we have nothing to show for it... It might be time I took a good look around to see what's here.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			actor = 605010,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Be careful, Littorio. The Sirens' deployments:n't bode well for us.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 8,
			nameColor = "#a9f548",
			side = 2,
			actor = 605020,
			dir = 1,
			say = "I'm well aware. Rest assured, I'm not about to let myself get overconfident just from a few victories against measly Pawns.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			actor = 605010,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Good. If Gioberti and the Mirror Sea control device are at the center of the labyrinth,: their defenses should start ramping up significantly pretty soon.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 8,
			nameColor = "#a9f548",
			side = 2,
			actor = 605020,
			dir = 1,
			say = "Even if they:, have no worries. Our forces are strong, and should we need to retreat, the exit is right behind our backs.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 7,
			side = 2,
			actor = 605020,
			nameColor = "#a9f548",
			dir = 1,
			say = "All you need:, my dear Eternal Flagship, is continue calmly issuing orders like normal.",
			effects = {
				{
					active = True,
					name = "speed"
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "Narration",
			nameColor = "#a9f548",
			side = 2,
			dir = 1,
			say = "The Sardegnian fleet's venture into the labyrinth continued...",
			dialogShake = {
				speed = 0.09,
				x = 10,
				number = 2
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
