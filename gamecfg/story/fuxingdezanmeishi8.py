return {
	fadeOut = 1.5,
	mode = 2,
	id = "FUXINGDEZANMEISHI8",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			side = 2,
			bgName = "bg_italyv2_1",
			dir = 1,
			bgmDelay = 1,
			bgm = "story-italy",
			actor = 601040,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Lady Veneto, we are approaching the waters surrounding the island of Crete.",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 605010,
			side = 2,
			bgName = "bg_italyv2_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Thank you. That means we'll be entering the Aegean Sea soon...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 607010,
			side = 2,
			bgName = "bg_italyv2_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I heard that Littorio's fleet engaged a Siren enemy in combat here not too long ago.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 601050,
			side = 2,
			bgName = "bg_italyv2_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "All the ships along this route have already been evacuated.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 605010,
			side = 2,
			bgName = "bg_italyv2_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Yes, Littorio is always looking out for our people.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_italyv2_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 605010,
			say = "She may be prideful and glory-seeking, but she understands the importance of protecting these shipping routes.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 605010,
			side = 2,
			bgName = "bg_italyv2_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Once we've handled the Mirror Sea situation, I'd like to take this opportunity to warn the Senate about further Siren activity.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_italyv2_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 607010,
			say = "Since private vessels would also be affected, wouldn't that make it harder to reach any kind of agreement with the Senate?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 605010,
			side = 2,
			bgName = "bg_italyv2_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "That's where the reclaimed art comes into play. Hehe~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_italyv2_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 605010,
			say = "Libeccio, there's no:ubt that we are in the right place. Littorio's fleet should be just ahead of us.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_italyv2_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 601050,
			say = "Yes! This is where I last received a communication from her!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_italyv2_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 601050,
			say = "I've been trying to contact the other members of the fleet, but nobody has responded...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_italyv2_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 605010,
			say = "Sudden equipment failure, abrupt weather changes, and disappearing ships... I hope civilian ships haven't been caught up in all this.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_italyv2_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 601040,
			say = "Based on Littorio's broadcast, it seems that all the other ships have already evacuated the area.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_italyv2_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 605010,
			say = "And: she rushed right in, huh?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 607010,
			side = 2,
			bgName = "bg_italyv2_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Hehe~ Lady Veneto, aren't you forgetting about someone? I believe I can be of assistance.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 605010,
			side = 2,
			bgName = "bg_italyv2_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Right, we: have an aircraft carrier with us now. I'm not quite used to...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_italyv2_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 605010,
			say = "Ahem... Aquila, can you use your recon planes to check out the coordinates listed here?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_italyv2_1",
			dir = 1,
			soundeffect = "event./battle/plane",
			actor = 607010,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "As you wish~♪",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_italyv2_1",
			dir = 1,
			blackBg = True,
			actor = 605010,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Let's move forward carefully while monitoring the status of our communications equipment.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
