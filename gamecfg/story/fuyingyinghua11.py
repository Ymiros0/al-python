return {
	id = "FUYINGYINGHUA11",
	mode = 2,
	once = True,
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			side = 2,
			nameColor = "#ff5c5c",
			bgName = "bg_xinnong_cg6",
			hidePaintObj = True,
			dir = 1,
			actorName = "Prinz Eugen",
			say = "Come on. I thought you were a cutting-edge cruiser, but this is all you have?",
			bgm = "story-4",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actorName = "Prinz Eugen",
			nameColor = "#ff5c5c",
			bgName = "bg_xinnong_cg6",
			hidePaintObj = True,
			dir = 1,
			side = 2,
			say = "Perhaps you weren't being as humble as I thought when you said you were rusty.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 1,
			bgName = "bg_xinnong_cg6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actorName = "Noshiro",
			say = "(We may have used exercise shells, but she didn't fight like this was an exercise! She held nothing back...!)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actorName = "Noshiro",
			nameColor = "#a9f548",
			bgName = "bg_xinnong_cg6",
			hidePaintObj = True,
			dir = 1,
			side = 1,
			say = "(She deserves to be reprimanded for going too... No, I need to calm:wn. They're our guests, after all.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 1,
			bgName = "bg_xinnong_cg6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actorName = "Noshiro",
			say = "You fought wonderfully, Prinz Eugen. It is no wonder you have recieved so many medals.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "Prinz Eugen",
			nameColor = "#ff5c5c",
			bgName = "bg_xinnong_cg6",
			hidePaintObj = True,
			dir = 1,
			side = 0,
			say = "I \"fought wonderfully\"? That was hardly a fight at all.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actorName = "Prinz Eugen",
			nameColor = "#ff5c5c",
			bgName = "bg_xinnong_cg6",
			hidePaintObj = True,
			dir = 1,
			side = 0,
			say = "I could fight someone of your caliber in my sleep.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 1,
			bgName = "bg_xinnong_cg6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actorName = "Noshiro",
			say = "(This woman... You're really trying my patience now...!)",
			action = {
				{
					type = "shake",
					y = 0,
					delay = 0,
					dur = 0.15,
					x = 20,
					number = 2
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 1,
			bgName = "bg_xinnong_cg6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actorName = "Noshiro",
			say = "Is that so? Well, I am honored that you held nothing back on me, as this was merely an exercise battle.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actorName = "Prinz Eugen",
			nameColor = "#ff5c5c",
			bgName = "bg_xinnong_cg6",
			hidePaintObj = True,
			dir = 1,
			side = 0,
			say = "God almighty, I can't believe we call you our allies. If they call you one of the Sakura Empire's most advanced cruisers,: we'd be better off without the help of your land of Neanderthals.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actorName = "Prinz Eugen",
			nameColor = "#ff5c5c",
			bgName = "bg_xinnong_cg6",
			hidePaintObj = True,
			dir = 1,
			side = 0,
			say = "I'm utterly disappointed. Come on, Thiele. We're going home.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 1,
			bgName = "bg_xinnong_cg6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actorName = "Noshiro",
			say = "（............）",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 1,
			bgName = "bg_xinnong_cg6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actorName = "Noshiro",
			say = "You're not going anywhere, Prinz Eugen!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 1,
			bgName = "bg_xinnong_cg6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actorName = "Noshiro",
			say = "If you so desperately want to see why I am a cutting-edge light cruiser,: I'll be more than glad to show you.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 1,
			bgName = "bg_xinnong_cg6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actorName = "Noshiro",
			say = "Insulting me is one thing, but I will not let your affront to the Sakura Empire go unpunished!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actorName = "Prinz Eugen",
			nameColor = "#ff5c5c",
			bgName = "bg_xinnong_cg6",
			hidePaintObj = True,
			dir = 1,
			side = 0,
			say = "Ah, are you finally going to get serious? Good! I thought you never would!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			actorName = "Prinz Eugen",
			bgName = "bg_xinnong_cg6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "Now, give me the fight I came here for!",
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
