return {
	id = "GONGMINGDEPASSION6",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_115",
			hidePaintObj = True,
			say = "Port - Royal Navy Dorm",
			bgm = "votefes-start",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_115",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 207140,
			say = "Really? You would all like to join team red?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_115",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 701050,
			say = "I was just asked to help out. I:n't care much about getting a wish granted.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_115",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 202280,
			say = "And I will: anything not to disappoint my Master! I want to get up on stage once more and shine like a star!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 0,
			side = 2,
			bgName = "star_level_bg_115",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 108050,
			say = "Heh, you all sound mighty fired up!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_115",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 108050,
			say = "I'm just surprised Astrum is getting the chance to perform on stage again.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_115",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 103250,
			say = "Ain't that the truth. I'll even be able to perform more like an idol since I've grown since the last time!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_115",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 207110,
			say = "Heehee. Thank you all for agreeing to help.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 207140,
			side = 2,
			bgName = "star_level_bg_115",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Illustrious, you didn't round up all this talent just to extend a helping hand to me, did you?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_115",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 207110,
			say = "Well, I did, but there's more to it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_115",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 207110,
			say = "In all honesty, I have a wish I'd like to come True.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_115",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 207140,
			say = "Oh really?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_111",
			hidePaintObj = True,
			say = "Port - Sakura Empire Dorm",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_111",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 304080,
			say = "It seems that the red team has already secured the talented Astrum unit for themselves...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_111",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 304080,
			say = "While we did successfully recruit the Dragon Sisters, we still must make haste and recruit additional talent.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307100,
			side = 2,
			bgName = "star_level_bg_111",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Are you seriously worried that we're lagging behind?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_111",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 307100,
			say = "Don't be. Victory is all but guaranteed with me on your team.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_111",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 304080,
			say = "Oh? Have you secured us an advantage?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_111",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 307100,
			say = "Of course I have. I convinced the entirety of Polaris to team up with us.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_111",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 304080,
			say = "This may be rude of me to ask, but you didn't \"convince\" them by force, did you?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307100,
			side = 2,
			bgName = "star_level_bg_111",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It should go without saying that I didn't. If you:n't believe me, just ask Gascogne.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_111",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 905030,
			say = "Akagi has not made any threats toward me.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_111",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 905030,
			say = "I have simply gained new insight into the meaning of the word \"idol,\" and I joined the blue team to demonstrate it to my Master.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_111",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 304080,
			say = "I... see... I apologize, Akagi. I shouldn't be so suspicious of you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_111",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 307100,
			say = "Apology accepted. To bring us back on topic, I also spoke to another unit.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_111",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 307100,
			say = "I'm not very keen on allying with old rivals of mine... but I won't shy away from it if that's what it takes to win.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_111",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 307100,
			say = "Verheerender, you're late. Why is that?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_111",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 403080,
			say = "I'm really sorry. I was rehearsing,: I thought about the ways I was going to crush my enemies and I lost track of time.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_111",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 307110,
			say = "Tsk... I'm only:ing this for my Commander. Otherwise I wouldn't be caught dead helping you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_111",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 307110,
			say = "If and when I win, I will spend my wish on my Commander, and:... Hehehehe...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_111",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 307100,
			say = "Oh, you're saying that to my face and think I'll let it slip? The only love that will appease the Commander is MY love!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_111",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 403080,
			say = "Heehee. Looks like I've made new enemies today.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_111",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 304080,
			say = "*sigh*...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_111",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 304080,
			say = "Please get a room, you three.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
