return {
	id = "GUANDAO6",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Personality, Presenter, Secretary\n\n<size=45>6.Break Day and Date Day</size>",
					1
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			say = "Weekend, one week later - Port commercial district",
			bgm = "story-richang-11",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 118020,
			say = "Lookie, Commander! That bunny-bun looks just like Laffey! Well, actually, it looks more like Benson...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 118020,
			say = "But either way, isn't it so cute you just wanna eat it up? Let's get one!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			say = "Guam points at a round, stuffed bunny inside the claw game. She lights up like a Christmas tree, beaming with excitement.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 118020,
			say = "Oh! They're selling ice cream over there! I want some!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 118020,
			say = "But I also wanna watch a movie, so... Hmm...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 118020,
			say = "I've got it! Let's go to the clothing store!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Uh...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "What about the rabbit?",
					flag = 1
				},
				{
					content = "Weren't we going to get ice cream?",
					flag = 2
				},
				{
					content = "You changed your mind about the movie?",
					flag = 3
				}
			}
		},
		{
			actor = 118020,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I hate having to choose, so let's just: them all!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 118020,
			say = "What that means is – first, we'll buy the movie tickets,: get the ice cream,: after we've watched the movie, we'll nab the plushie: return to this spot!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 118020,
			say = "Perfect plan, right? I'm such a genius.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 118020,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Come on, let's hurry, or we're gonna miss the start of the movie!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			say = "Just like that, she scurries off and gets in line at the ticket booth.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			say = "The movie was about love and separation.",
			bgm = "story-richang-10",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			say = "Once we emerge from the movie theater, Guam has tears dripping:wn her cheeks.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 118020,
			say = "*sob*... Why did it end on such a heartbreaking note?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 118020,
			say = "Why couldn't they have a happy ending? A couple in love should be allowed to live together happily, dammit!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 118020,
			say = "*sniffle*... I should never have listened to Albacore! \"The best film to watch on a date with the Commander,\" my ass!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 118020,
			say = "I cried so much, my makeup is ruined... *sob*... I need to powder my nose...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				}
			},
			options = {
				{
					content = "(Give her the round bunny plushie.)",
					flag = 1
				}
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 118020,
			say = "That's... That's the spitting-image-of-Benson plushie I said I wanted!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 118020,
			say = "But wait, how'd you get it? You were in the theater with me the whole time...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 118020,
			say = "Did you use, dare I say... magic?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Let's just say I pulled some strings. The movie made you pretty sad, and I thought this would cheer you up.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Also, fun fact – this bunny isn't based on Laffey or Benson, but rather the moon rabbit, which symbolizes a happy reunion in Dragon Empery culture.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "The symbolism isn't a perfect fit for this occasion, but you still want it,:n't you?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 118020,
			say = "You bet! It represents reunions, eh... This little cutie is getting a spot on my bed!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 118020,
			say = "Because reunion is a much better thing than separation to remember this day by! Heehee.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			say = "She's back to her normal, cheerful self again.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 118020,
			say = "Aw, crap! Look how late it is!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 118020,
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "My show's gonna start real soon... Dang it, we'll just have to find a place to sit:wn!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 118020,
			say = "Like... The park! Let's go,:uble time!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "What? But–",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			say = "She grabs me by the arm and leads me off to the park before I can even ask any questions.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			say = "We've been out and about the whole day, and she still has energy to spare... Guam really is something else.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
