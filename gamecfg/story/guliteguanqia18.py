return {
	id = "GULITEGUANQIA18",
	mode = 2,
	once = True,
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			dir = 1,
			side = 2,
			bgm = "ssss-az-battle-boss",
			say = "Nearby Waters",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 202111,
			nameColor = "#a9f548",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "Are you sure we're going the right way...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			actor = 10800070,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Yeah. This is the place it always goes to hide.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900233,
			nameColor = "#a9f548",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			actorName = "Purity",
			say = "That's the same reading that your favorite teacher here got!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 202111,
			nameColor = "#a9f548",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "...That Siren ability to control mass-produced ships really comes in handy, huh...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 10800070,
			nameColor = "#a9f548",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "That it:es, but it:esn't seem to always work. There's much I still: not understand about this power.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			actor = 900318,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Luckily enough, there's an entire buffet of Siren ships in front of us for you to test your powers on~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			actor = 900318,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "We just have to get closer first to see exactly how many there are. This should be a great opportunity to vent a bit~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Communicator",
			nameColor = "#a9f548",
			hidePaintObj = True,
			dir = 1,
			say = "(Alarm blaring) .........!!",
			soundeffect = "event./ui/alarm",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			actor = 301812,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "There's something massive approaching us... What is that...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_ssss_2",
			say = "From beyond the horizon, the sea was covered by a steel shadow.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_ssss_3",
			say = "Not only was it as menacing as a dragon ready to strike, but various parts of its body were equipped with heavy artillery, endowing it with enough firepower to defeat any enemy while still maintaining a dignified appearance.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_ssss_4",
			say = "There could be no:ubt – this was the kaiju.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 10800050,
			nameColor = "#a9f548",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "No way... it's even bigger than it was before?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900318,
			nameColor = "#a9f548",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "Quite the formidable foe... but I wouldn't have it any other way~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			nameColor = "#a9f548",
			hidePaintObj = True,
			dir = 1,
			actorName = "Purity",
			actor = 900233,
			say = "Wait, hold on! We need to get out of here! We can't fight that thing!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 2,
			side = 2,
			actor = 900318,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Oh my, is the mighty Purifier going to run with her tail tucked between her legs?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			nameColor = "#a9f548",
			hidePaintObj = True,
			dir = 1,
			actorName = "Purity",
			actor = 900233,
			say = "Fear has nothing to: with it! That thing... isn't normal!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			nameColor = "#a9f548",
			hidePaintObj = True,
			dir = 1,
			actorName = "Purity",
			actor = 900233,
			say = "We can't defeat it! As a matter of fact, we'll be totally crushed! We need to get out of here!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 2,
			side = 2,
			actor = 900318,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "...You say that, but that thing's already headed straight towards us. Since we have the numbers advantage, why:n't we send some planes at it to see how tough it really is?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307041,
			nameColor = "#a9f548",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "I like the sound of that. Souryuu, let's get ready to send our planes out to get a feel for how this thing fights!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 10800060,
			side = 2,
			nameColor = "#a9f548",
			hidePaintObj = True,
			dir = 1,
			say = "Senpaiiii!! I can help too!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			dir = 1,
			side = 2,
			soundeffect = "event./battle/boom2",
			say = "KABOOM––!!",
			flashN = {
				color = {
					1,
					1,
					1,
					1
				},
				alpha = {
					{
						0,
						1,
						0.2,
						0
					},
					{
						1,
						0,
						0.2,
						0.2
					},
					{
						0,
						1,
						0.2,
						0.4
					},
					{
						1,
						0,
						0.2,
						0.6
					}
				}
			},
			dialogShake = {
				speed = 0.09,
				x = 8.5,
				number = 2
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			say = "The shipgirls deployed their aircraft to engage the massive kaiju.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			say = "...However, neither gun nor bomb could leave even a scratch on the kaiju's body.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			say = "The attacks seemed to bounce harmlessly of its body, as if protected by some kind of invisible shield.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_ssss_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Chise Asukagawa",
			say = "Whaaaat? It didn't work?!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_ssss_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Mujina",
			say = "It's different from before. It seems to have gained some kind of defensive mechanism.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_ssss_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Mujina",
			say = "It's not just bigger. It's also gotten stronger.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "Rikka Takarada",
			side = 2,
			bgName = "bg_ssss_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I:n't like the way it's looking at us... Uhh, isn't this pretty bad?",
			soundeffect = "event./battle/boom2",
			flashN = {
				color = {
					1,
					1,
					1,
					1
				},
				alpha = {
					{
						0,
						1,
						0.2,
						0
					},
					{
						1,
						0,
						0.2,
						0.2
					},
					{
						0,
						1,
						0.2,
						0.4
					},
					{
						1,
						0,
						0.2,
						0.6
					}
				}
			},
			dialogShake = {
				speed = 0.09,
				x = 8.5,
				number = 2
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_ssss_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Roon",
			say = "Ugh! I'll admit, that thing DOES pack a punch!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			paintingNoise = True,
			nameColor = "#a9f548",
			dir = 1,
			actor = 900233,
			actorName = "Purity",
			hidePaintObj = True,
			say = "Didn't I already tell you before? Let's hurry up and get out of here, dunces!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 2,
			side = 2,
			actor = 900318,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I'm not:ne yet! Raaaaargh!!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			actor = 102163,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Roon! Get back here...!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			actor = 102163,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "She's not listening! Crap, we're getting separated by the kaiju's attacks...!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 202111,
			nameColor = "#a9f548",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "What would the Commander: in this situation? Ugh, but the flagship should be the one to make that call...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			actor = 10800050,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Is that really important right now?!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 202111,
			nameColor = "#a9f548",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "Y-you're right... but Roon's really scary, and the Commander isn't here... What would Bel: if she was here..?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			actor = 306060,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Don't worry about her, Edinburgh! Just: what you think is right!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			say = "The massive enemy began charging its massive beam, and the party could only gulp as they prepared to face the incoming crisis.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 10800050,
			nameColor = "#a9f548",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "I'll take care of things over here, so make sure you go and get the other shipgirls!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			actor = 900318,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Tsk... Those defenses are really keeping me from having my fun...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 202111,
			nameColor = "#a9f548",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "That's enough! Roon, everyone, regroup and retreat at once!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			actor = 10800060,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Edinburgh!!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 202111,
			nameColor = "#a9f548",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "Hurry, let's go!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
