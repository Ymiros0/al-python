return {
	fadeOut = 1.5,
	mode = 2,
	id = "GULITERICHANG4",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Dynamic Days\n\n<size=45>4 Band Together</size>",
					1
				}
			}
		},
		{
			say = "It was a tranquil afternoon. Chise was out on a casual stroll.",
			side = 2,
			bgName = "star_level_bg_103",
			dir = 1,
			bgmDelay = 2,
			bgm = "ssss-az-story",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 10800060,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Oh man, Dunkerque sure makes some amazing pastries! Eating 'em really fills you with joy.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_103",
			say = "Just:wn the street, Urakaze and Isokaze were standing next to some instruments, handing out flyers. Chise laid eyes on them.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 10800060,
			say = "You got a band thing going on or somethin'? Not too weird for this port though, I guess.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 10800060,
			say = "Dang, that's some high-end equipment... Cool stuff you got!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 301592,
			say = "Thanks. We are the Sengokus. We strive to unify the port under one band... Ours, of course.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 301602,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Mhm! We're looking for new members. Interested, Chise?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 10800060,
			say = "Eh... I dunno if I'm good enough.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 301592,
			say = "Even at a glance, I can tell you have potential. Come on, we could use someone of your caliber.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 10800060,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Aww, shucks, you flatter me...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 10800060,
			say = "Oh, all right! If you're fine with havin' me,: I'll give it my best shot!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 301592,
			say = "Splendid! See, Isokaze? I told you we'd eventually get a new member, given enough perseverance!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301602,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Wahoo! Now help yourself to any instrument you want, Chise!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301592,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "By the way, there's a voucher for a fancy patisserie on the back of your flyer.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 10800060,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Whoa, I didn't notice! What a \"sweet\" sign-up bonus.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_103",
			say = "Urakaze and Isokaze kept on handing out flyers while Chise familiarized herself with her instrument. The two Sengokus had now become three.",
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 10800070,
			say = "You're forming a band? How charming.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301602,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Yep, and we're looking for more members. Would you like to join, Mujina?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 10800070,
			say = "I: appreciate the offer, but I'm afraid I can't play an instrument.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301592,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "It's never too late to start learning. Who knows? There may be untapped talent within you.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 10800070,
			say = "Thanks, but no. I'll come and watch, but...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_103",
			say = "Mujina meant to decline, but experienced a sudden change of heart when Urakaze and Isokaze surrounded her with instruments.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 10800070,
			say = "Well... On second thought...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 10800060,
			say = "Hella persuasive, aren't they? It can't hurt to give it a shot at least. C'mon, let's hear it!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 0,
			bgName = "star_level_bg_103",
			actor = 301592,
			dir = 1,
			actorName = "{namecode.142}&{namecode.143}",
			hideOther = True,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "*looks of anticipation*",
			subActors = {
				{
					actor = 301602,
					pos = {
						x = 1185
					}
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 10800070,
			say = "Fine... I'll see how this one plays...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_103",
			say = "Mujina picked up a drumstick and gave the drums a soft hit.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 10800070,
			say = "Hmm... Not bad.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_103",
			say = "She hit it a second time, this time with a bit more force.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 10800070,
			say = "Such devastatingly powerful sound... I could get into this, actually.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 10800060,
			say = "Oh snap! I:n't think she's being sarcastic with us.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 301592,
			say = "You're free to try all the instruments if you want, Mujina.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301602,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "You're free to join the band too! Just so you know!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 301592,
			say = "She knows, Isokaze. Don't pester her.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_103",
			say = "Chise kept on familiarizing herself, and Mujina tested other instruments while Urakaze and Isokaze watched. They became engrossed in the world of music and forgot all about the flyers.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "star_level_bg_103",
			dir = 1,
			blackBg = True,
			say = "The four members of the Sengokus performed for hours and hours, the sun even setting before they were:ne.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
