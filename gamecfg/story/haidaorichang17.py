return {
	fadeOut = 1.5,
	mode = 2,
	id = "HAIDAORICHANG17",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Uncharted Summer\n\n<size=45>Titillating Sunbathing Time</size>",
					1
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_106",
			say = "A golden beach under the brilliant sun. White waves caress the shore, their sound like music to my ears.",
			bgmDelay = 2,
			bgm = "story-richang-4",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_106",
			say = "But, that sound is all but drowned out by my racing heart.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105172,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = "touch1",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Hi, honey~ Seeing as I've got your attention, wanna sunbathe with me?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105172,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Ahh... Chilling on a sunny beach while sipping a nice tropical drink. This is what summer's all about!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105172,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Here, honey – enjoy~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105172,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = "mission_complete",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Wait, lemme have a sip first! ...Yeah, you're gonna love this. Try it!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105172,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			withoutActorName = True,
			nameColor = "#A9F548FF",
			dir = 1,
			say = "With some hesitation, I accept the glass she's already had a sip from, and drink it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105172,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "What's up, honey? You're blushing. Sun too much for you?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105172,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Ooor, is it the fact we had an indirect kiss? That's no big deal, if you ask me~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105172,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = "mission",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Oh, I know. In exchange for the drink, can you apply sunscreen on me?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105172,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I love lying in the sunshine as much as the next girl, but I can: without getting sunburned.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105172,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...Whatcha looking at, hmm? My eyes are up here, honey.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105172,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Checking out my swimsuit? So you DO love it♪",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "\"What can I say? It looks great on you.\"",
					flag = 1
				},
				{
					content = "\"It's a bit... TOO sexy for my liking.\"",
					flag = 2
				}
			}
		},
		{
			actor = 105172,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			optionFlag = 1,
			nameColor = "#A9F548FF",
			say = "Of course it:es. You're the one who picked it out♪",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105172,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			optionFlag = 2,
			nameColor = "#A9F548FF",
			say = "So you say, but you chose it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105172,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			optionFlag = 2,
			nameColor = "#A9F548FF",
			say = "Hahaha! You're blushing again~ You've got \"New Jersey has stolen my heart\" written all over your face♪",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105172,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = "touch2",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...Whuah! That's cold!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105172,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Getting back at me using the glass, huh? Fair enough. Now apply the sunscreen, will you?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105172,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Ooh, that's the ticket. Nice and slippery...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105172,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "So, whatcha wanna: next, honey?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105172,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = "expedition",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "No? Hahaha! I know exactly what you're thinking.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105172,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "If it's too embarrassing to say out loud::n't. But:n't play stupid either.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105172,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Oh, honey~ You're so cute when you blush♪",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105172,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Now get comfy next to me and let's sunbathe♥",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
