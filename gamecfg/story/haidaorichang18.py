return {
	fadeOut = 1.5,
	mode = 2,
	id = "HAIDAORICHANG18",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Uncharted Summer\n\n<size=45>The Dark Dragon's Invitation</size>",
					1
				}
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_106",
			actorName = "???",
			say = "You. What: you think you're:ing?",
			bgmDelay = 2,
			bgm = "story-richang-5",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_106",
			say = "While I'm walking along the coast of the marina, Regensburg abruptly calls out to me.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 402101,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = "main3",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Don't act like you:n't recognize me just because I'm wearing new attire.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 402101,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I can read you like an open book. Obviously, you're floored by my swimsuit.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 402101,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...I'm wrong, you say?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 402101,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Tsk. Fine, deny it all you want, but you cannot hide the desires that urge your body onwards.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 402101,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = "main1",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Stop putting on airs. Tell me how you really– Eep! H-hands off my tail, you:g!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 402101,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Hmph. Don't give me that smug look. Misreading you is bad enough without you rubbing salt in the wound...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 402101,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Ahem... That's enough beating around the bush.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 402101,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = "mission",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Since you're here, why:n't we: something relaxing?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 402101,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = "mission",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "You're supposed to be on vacation, yet all you think about is developing the island. Forget about all that for a while, will you?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 402101,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "If it's yacht races you fancy, you can spectate them from here. And if it's hands-on action you want, I'll fill the entry form for you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 402101,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Or, if you simply want to lie:wn and relax, that's fine too. The sand on this beach is lovely.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 402101,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...Come again? You want to go someplace quiet to be alone with me?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 402101,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Okay:... If it's what you want, I suppose I can: that.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 402101,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Just to be clear, I'm agreeing to this out of my own will – not out of obedience to you...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 402101,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = "touch",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "So,:n't you dare say you've subjugated the dragon!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 402101,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			withoutActorName = True,
			nameColor = "#A9F548FF",
			dir = 1,
			say = "She covers my mouth with her hand, like she's too embarrassed to hear anything from me.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 402101,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Look, I know. Don't talk – let your actions speak for you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 402101,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Now come on. Take me wherever you want to go!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
