return {
	fadeOut = 1.5,
	mode = 2,
	id = "HAIDAORICHANG26",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Uncharted Summer\n\n<size=45>Progress Update</size>",
					1
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			say = "Over in the camp's conference room, a small discussion is taking place.",
			bgmDelay = 2,
			bgm = "main-SeaAndSun",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			say = "The topic – a progress update on the island's state of development.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107060,
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Well, we've established a cargo route between us and the port.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107060,
			say = "Work on the island's main infrastructure is:ne too. And the pier can be considered complete.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 312010,
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "The hot spring is finished too, nya. Drop in whenever you want, nya.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Heehee... I'll be waiting for you there, Commander♡",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 705050,
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "On the mineral and lumber extraction front, things are going quite well.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 705050,
			say = "They're currently configured to operate for two hours a day. We also plan to develop them into scenic industrial spots eventually.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 502010,
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "With assistance from Hanazuki, Ning Hai and Ping Hai have got their flower garden set up.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 502010,
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I'm sure they'd love it if you came to have a look, Commander.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			say = "\"Flower garden? Last time I spoke to them, they said they wanted to establish a VEGETABLE garden.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			say = "\"Wonder what changed their minds... I'll look into it later.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It sounds like our work on the island is:ne, so–",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 699010,
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Not so fast! You've forgotten one very, very important thing!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			},
			effects = {
				{
					active = True,
					name = "speed"
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 699010,
			say = "You're some of the brightest minds I know, so how could you not notice?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			effects = {
				{
					active = False,
					name = "speed"
				}
			}
		},
		{
			actor = 405010,
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "There's an important facility that we've all forgotten about? That can't be possible.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 699010,
			say = "But it is! And I shall tell you what it is... another time!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 699010,
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Worry not, I will personally see to its construction. Just wait and see.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 699010,
			say = "Hehehehehehe♪",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 605010,
			say = "I:n't like that she's making this needlessly mysterious...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 405010,
			say = "(Whatever she's planning, I hope it isn't a nasty last-minute surprise.)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 8,
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 405010,
			say = "...Iron Blood will assist with the construction. We're free at the moment.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Ahem. As I was going to say!",
			fontsize = 60,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It sounds like our work on the island is ALMOST:ne!",
			fontsize = 60,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Which I think calls for a grand celebration!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			say = "......",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "\"Great idea!\"",
					flag = 1
				},
				{
					content = "\"But Marco said to wait and see.\"",
					flag = 2
				}
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			dir = 1,
			optionFlag = 1,
			nameColor = "#A9F548FF",
			say = "I knew you'd agree, servant! Nothing says \"job well:ne\" quite like a banquet.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			dir = 1,
			optionFlag = 2,
			nameColor = "#A9F548FF",
			say = "Be quiet, servant! Let me finish.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "We'll host it on the beach near the camp.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I've already been there. It has the best scenery anywhere on the island.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "As for the food, the Royal Maids will provide it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 805010,
			say = "Your Majesty, since this is a banquet for everyone who's helped develop the island,:n't you think everyone should be involved in making the food as well?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "You have a point... Apologies. The Royal Maids will take a more supporting role instead so we can all bring our best dishes to the table.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 105170,
			say = "Sounds sweet! Now you've got me excited!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 605010,
			say = "If it's not too much to ask, I'd like Sardegna to be in charge of preparing any and all pizza dishes.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 502010,
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Heehee. It seems I'll be quite busy soon.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			say = "Suddenly, I remember that I haven't seen a trace of Royal Fortune since the yacht race.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			say = "In fact, nobody has seen her for the past few days.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			say = "I hope she'll turn up soon so she can partake in the banquet.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_164",
			hidePaintObj = True,
			say = "After all, she deserves recognition for leading us to this island in the first place.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
