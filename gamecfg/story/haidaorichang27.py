return {
	fadeOut = 1.5,
	mode = 2,
	id = "HAIDAORICHANG27",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Uncharted Summer\n\n<size=45>Learning How to Swim</size>",
					1
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_106",
			say = "The beach near the pier. The water is so clear you can see straight to the bottom. Rays of sunlight bounce off the sparkling waves, scattering in all directions.",
			bgmDelay = 2,
			bgm = "story-richang-1",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_106",
			say = "It is here where a slightly odd swimming lesson is taking place.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_106",
			say = "No, \"swimming lesson\" is not the right phrase. \"Date\" might be more accurate.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 199031,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = "main1",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Hmgh... There. Teacher, is Anchorage:ing it right?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 199031,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "This is how you swim, right? Heehee~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 199031,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I've never swam before, so I'm a little scared of drowning...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 199031,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "But, you're right here if I need help, so I feel safe!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 199031,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Your way of teaching is so... umm... gentle! Yeah, gentle! I like it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 199031,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = "mission_complete",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I feel like I can: anything as long as I have you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 199031,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			withoutActorName = True,
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Anchorage gives me a heartwarming smile from atop her inflatable orca.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "(Give her headpats.)",
					flag = 1
				},
				{
					content = "\"You're:ing great so far.\"",
					flag = 2
				}
			}
		},
		{
			actor = 199031,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = "headtouch",
			dir = 1,
			optionFlag = 1,
			nameColor = "#A9F548FF",
			say = "Heehee~ Your hand feels so warm.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 199031,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			optionFlag = 1,
			nameColor = "#A9F548FF",
			say = "Even though it's super sunny, your hand feels warm in a good way. I love it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 199031,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = "main2",
			dir = 1,
			optionFlag = 2,
			nameColor = "#A9F548FF",
			say = "Teacher thinks I'm:ing great... That makes me so happy! Heehee~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 199031,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			optionFlag = 2,
			nameColor = "#A9F548FF",
			say = "But, it's not good enough. I need to: much better than just \"great.\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 199031,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			optionFlag = 2,
			nameColor = "#A9F548FF",
			say = "So we can both swim together...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 199031,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Teacher, come closer.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 199031,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = "main3",
			withoutActorName = True,
			nameColor = "#A9F548FF",
			dir = 1,
			say = "I: just that, and she tries to change directions.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 199031,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			withoutActorName = True,
			nameColor = "#A9F548FF",
			dir = 1,
			say = "However, she can't control her orca, and nearly falls into the water.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 199031,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			withoutActorName = True,
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Acting quickly, I grab hold of her to stabilize her. Her wet body feels warm to the touch.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 199031,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			withoutActorName = True,
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Her cheeks seem to redden, possibly because she's overexerted herself.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 199031,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Ugh... I was this close to falling off...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 199031,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = "main1",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I really need to practice with you more...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 199031,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "But I like being taught by you, so that's okay! I'm gonna give this another try!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
