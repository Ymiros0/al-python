return {
	fadeOut = 1.5,
	mode = 2,
	id = "HAIDAORICHANG28",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Uncharted Summer\n\n<size=45>Poolside Beauty</size>",
					1
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_164",
			say = "I decide to pay a visit to the island's indoor pool.",
			bgmDelay = 2,
			bgm = "bar-soft",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_164",
			say = "Bismarck is there. She comes up after a quick swim and sits by the poolside. She is drenched – and looking gorgeous.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 405051,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_164",
			spine = True,
			dir = 1,
			side = 2,
			actorName = "{namecode.435}",
			say = "Whew... My hair keeps getting caught on my body. I should've tied it up before going in.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_164",
			spine = True,
			dir = 1,
			actorName = "{namecode.435}",
			actor = 405051,
			nameColor = "#A9F548FF",
			say = "Fancy seeing you here, Commander. The water is perfect – neither too hot nor cold.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 405051,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_164",
			spine = True,
			dir = 1,
			side = 2,
			actorName = "{namecode.435}",
			say = "A splendid pool is among the last things I thought we could build on this remote island.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_164",
			spine = True,
			dir = 1,
			actorName = "{namecode.435}",
			actor = 405051,
			nameColor = "#A9F548FF",
			say = "It's a great place to swim, but leaves much desired as a date spot... But never mind that. What: you think about the pool?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_164",
			spine = True,
			withoutActorName = True,
			nameColor = "#A9F548FF",
			actor = 405051,
			actorName = "{namecode.435}",
			say = "Frankly, the main reason I'm here is because she invited me to swim with her.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_164",
			spine = True,
			withoutActorName = True,
			nameColor = "#A9F548FF",
			actor = 405051,
			actorName = "{namecode.435}",
			say = "Not just to burn calories, of course, but also to enjoy some time together. Go on a date, in other words.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_164",
			spine = True,
			withoutActorName = True,
			nameColor = "#A9F548FF",
			actor = 405051,
			actorName = "{namecode.435}",
			say = "\"I noticed that we're alone in here.\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_164",
			spine = True,
			dir = 1,
			actorName = "{namecode.435}",
			actor = 405051,
			nameColor = "#A9F548FF",
			say = "Yes. That's because I reserved it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 405051,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_164",
			spine = True,
			dir = 1,
			side = 2,
			actorName = "{namecode.435}",
			say = "We all want to be left undisturbed sometimes. Especially now, when... You know what I mean,:n't you?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 405051,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_164",
			spine = True,
			dir = 1,
			side = 2,
			actorName = "{namecode.435}",
			say = "Incidentally, I chose this swimsuit myself. I thought you might like it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_164",
			spine = True,
			dir = 1,
			actorName = "{namecode.435}",
			actor = 405051,
			nameColor = "#A9F548FF",
			say = "It strikes a good balance between form and function.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 405051,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_164",
			spine = True,
			dir = 1,
			side = 2,
			actorName = "{namecode.435}",
			say = "In truth, I did ask Eugen for suggestions, but all the swimsuits she picked out were...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_164",
			spine = True,
			dir = 1,
			actorName = "{namecode.435}",
			actor = 405051,
			nameColor = "#A9F548FF",
			say = "The kinds that draw undesirable attention. Let's just say that...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_164",
			spine = True,
			withoutActorName = True,
			nameColor = "#A9F548FF",
			actor = 405051,
			actorName = "{namecode.435}",
			say = "Bismarck is not usually one to mince her words. Through squinted eyes, I notice that she's blushing ever so faintly.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 405051,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_164",
			spine = True,
			dir = 1,
			side = 2,
			actorName = "{namecode.435}",
			say = "Ahem... I see you've:ne your warmups. Aren't you going in the pool?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_164",
			spine = True,
			dir = 1,
			actorName = "{namecode.435}",
			actor = 405051,
			nameColor = "#A9F548FF",
			say = "...Together? Someone's impatient... I just got up a minute ago.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 405051,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_164",
			spine = True,
			dir = 1,
			side = 2,
			actorName = "{namecode.435}",
			say = "Oh, never mind. We're alone here – there's no point fussing over appearances.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 405051,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_164",
			spine = True,
			dir = 1,
			side = 2,
			actorName = "{namecode.435}",
			say = "Go on in. I'll join you in a second.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_164",
			spine = True,
			dir = 1,
			actorName = "{namecode.435}",
			actor = 405051,
			nameColor = "#A9F548FF",
			say = "Care for a challenge? Let's see who's the best swimmer. I'm not going to go easy on you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
