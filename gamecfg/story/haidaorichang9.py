return {
	fadeOut = 1.5,
	mode = 2,
	id = "HAIDAORICHANG9",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Uncharted Summer\n\n<size=45>Beachside Daydream</size>",
					1
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_106",
			say = "A beautiful woman stands alone on the beach, bathed in the sun's golden glow.",
			bgmDelay = 2,
			bgm = "theme-warmwinter-daily",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_106",
			say = "I daydream of sitting:wn to appreciate the scenery... until her voice calls out to me.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307075,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = "main2",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Ah, this weather is perfect – perfect for a date with you, my Commander~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307075,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			withoutActorName = True,
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Taihou wraps her arm around mine and takes me on a stroll through the empty beach.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307075,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I'm so happy to have you all to myself today~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307075,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = "main1",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Oh... You can't take your eyes off my swimsuit – off me, hmm?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307075,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "By all means, stare as much as you wish. In fact, DO stare. I love it when you:♥",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307075,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It seems this swimsuit is a winner, because it has already captured your heart♥",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307075,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "And yet, now your gaze is darting from place to place... Are you looking for someone else?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307075,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I'm afraid you won't find another soul out here. The only people on this beach are you and me♥",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307075,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Do you know why?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307075,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = "main3",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Because I've reserved the whole beach♥",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307075,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			withoutActorName = True,
			nameColor = "#A9F548FF",
			dir = 1,
			say = "She leaned in close and teasingly whispered that right into my ear.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307075,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "And: you know what that means?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307075,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "That there's no way to run from my love♥",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307075,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = "touch",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Heehee~ Seeing your anxious face makes me want to tease you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307075,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Yes, it won't: with you passively staring at me. That won't deepen our bond at all.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307075,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "That being the case...♥",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307075,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I'll make the first move myself.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307075,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			withoutActorName = True,
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Before I can attempt to wriggle my way loose, she grabs hold of me.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307075,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = "touch2",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "You needn't: anything, my Commander. Allow me to take the lead.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307075,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Your desires, your body, your spirit... I'll take care of them all♥",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307075,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Now, close your eyes...♥",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
