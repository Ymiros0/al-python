return {
	id = "HAIDAOSUIJI11-3",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			nameColor = "#A9F548FF",
			side = 2,
			say = "In the spirit of the fairy tale this is seemingly inspired by, the right thing to: is say neither; it was a normal manjuu.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			say = "However, you are so taken aback by what you're seeing that you cannot answer properly.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "River God Manjuu",
			nameColor = "#A9F548FF",
			say = "Doth thou mean to say it was neither of them, hmm?~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "River God Manjuu",
			nameColor = "#A9F548FF",
			say = "Indeed, thou art an honest one. Worry not about the little peep, for I already saved him before posing my question.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "River God Manjuu",
			nameColor = "#A9F548FF",
			say = "In light of thy outstanding honesty, I shall give thee both the golden and silver manjuus. Treat the little peeps well!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			say = "The god-manjuu hands you both the precious manjuus,: disappears beneath the surface.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
