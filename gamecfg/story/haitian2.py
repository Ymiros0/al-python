return {
	id = "HAITIAN2",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Set Sail! An Inspiration-Seeking Journey\n\n<size=45>2 Camping Preparations</size>",
					1
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			say = "Hai Tien was more than glad to accept my invitation for a camping trip, and we started making preparations for our next day off.",
			bgm = "story-richang-6",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			say = "After wrapping up the remainder of the work, the two of us head out to the port's shopping district for our day off.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "Commander, I've consulted all my other peers and compiled a list of camping supplies. Let's follow this list when deciding what to buy.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Sounds good to me!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 305130,
			say = "Woah, if it isn't Commander and Hai Tien! Are you guys out here to stock up for a camping trip?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305130,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I'm quite familiar with that stuff, y'know, so feel free to gimme a holler!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 9,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "If memory serves me properly,:n't you run a produce store, Miss Owari? I'm not sure you'd be able to help us with camping supplies...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 305130,
			say = "Hmm~ Well, yeah, but fruits and veggies aren't the only things we sell. We've got a whole bunch of other useful stuff too.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305130,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "After all,:in' business around the port is no walk in the park. You've gotta branch out and expand your market share if you:n't wanna strike out.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 312010,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Nya? Did I hear someone say they were shopping, nya?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			say = "As if we had spoken forbidden words of power, a familiar green cat suddenly popped out right in front of us.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 312010,
			say = "Let's take a look at that shopping list you've got there, Hai Tien... Hrmm, hrmm... I see, I see, nya.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 312010,
			say = "Looking for camping supplies, nya? You'll find everything you're looking for, and more, in Akashi's store, nya~♪",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 305130,
			say = "Wait, hold yer horses! I was the one who found these customers first! This is practically daylight robbery!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 312010,
			say = "Nya? In the end, it's the customer who decides where to: business, and with whom.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 312010,
			say = "That being the case, why:n't we both bring our wares and let Commander and Hai Tien decide, nya?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305130,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "You're on! Practically took the words right outta my mouth!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			say = "Some time later...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 305130,
			say = "*pant*... *pant*... Feel's like my tongue's going numb just explainin' all the different products...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 305130,
			say = "Why:es Akashi look like she's not havin' a hard time at all?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 312010,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Mwahaha... Well, well~ it comes with the turf, nya~!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 312010,
			say = "Drumroll please... and tadah! An 'oral spray that keeps your mouth hydrated no matter how much you talk', nya!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 312010,
			say = "An absolute essential for anyone who wants to: business! And, it can be yours for a cheap, cheap price of 9,999 coins, nya!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305130,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Woah, that sounds crazy useful! Gimme two canisters!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(...What's crazy is how she's had the tables turned on her completely. And how:es anyone fall for that shady sales pitch anyway?)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 312010,
			say = "Thanks for your patronyage!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 312010,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Now:, you two, have you decided on what you want, nya? Since it's your first time camping, you might want to make some extra preparations by buying everything. Just in case, nya~♪",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305130,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Exactly! Supplies aside, you'll also want to make sure you have a first aid kit and some non-perishable food on hand~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Err... Why:n't you give us some time to browse on our own first?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 8,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "Miss Owari certainly brought up a good point. While there seem to be a lot of things on the list, unforeseen circumstances can always pop out...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 10,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "I'd like this, and this. And all of these over here!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305130,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_171",
			side = 1,
			hideOther = True,
			actorName = "Akashi＆Owari",
			say = "- Thank you, thank, nya! You are most generous, Hai Tien! - Thanks a ton for supporting my biz! Come back again next time~♪",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			subActors = {
				{
					actor = 312010,
					pos = {
						x = -1125
					}
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			say = "I wasn't able to stop Hai Tien from going on her shopping frenzy, nor did I want to rain on her parade either. In the end, we bought every last thing that Akashi and Owari brought.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
