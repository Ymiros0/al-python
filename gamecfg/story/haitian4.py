return {
	id = "HAITIAN4",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Set Sail! An Inspiration-Seeking Journey\n\n<size=45>4 A Short Story</size>",
					1
				}
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_520",
			dir = 1,
			bgm = "story-richang-5",
			actor = 502070,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Hum hum~♪",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			say = "Hai Tien walks next to me with a sprightly spring in her step, wearing the semblance of a flower crown that I had crafted on her head.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			say = "As I follow her, I also wear the ring of grass she had made for me on my left hand.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			say = "We continue to pass through a tiny trail in the forest, and eventually arrive at a pleasant clearing that seems perfect for camping.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 10,
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "Commander, shall we pitch the tent? I'd be glad to help out.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Sure, good idea.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			say = "Though this is the first time camping for both of us, we still manage to get the tent set up without any difficulty.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(Hmm... The sun might get pretty intense soon. Maybe I should throw up the tarp?)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 9,
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "Commander, is there anything else I can help with?",:
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Uhh, I think I'll be fine. Here, I just finished unfolding the chairs. Take a breather and see if you can come up with any inspiration.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "I can put up the tarp by myself.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "Well:, I'll leave it to you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			say = "As I go to set up the tarp, I suddenly hear Hai Tien's voice.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "No, not like that! The emotion here is completely off! There needs to be more intimacy in your expression!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 10,
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "Yes, yes, that's it! That's exactly how you should be looking at your crush!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(Hmm? That's weird... I:n't think there's anyone else here, so who is she talking to...?)",:
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			say = "Curiosity gets the better of me, and I stop what I'm:ing to head back to where Hai Tien is–",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			say = "At some point, a whole bunch of squirrels had lined up in front of her.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			say = "And Hai Tien is... enthusiastically teaching them to act.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			say = "When I listen in a bit more, I notice that the content of her \"playbook\" seems to be from the novel she was working on before.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Squirrel",
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Squeak, squeak!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Squirrel",
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Squeak! Squeak squeak squeak!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "You're:ing great~ That's exactly the emotion you want! Next, the storm scene is about to start!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "Hold your lover tight, and let her feel your heartbeat–",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "......",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			say = "There's a certain magic to her fairytale-like story that puts a wide smile on my face.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			say = "The question is, how is Hai Tien able to teach the lines to the squirrels? Does she know how to communicate with them...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
