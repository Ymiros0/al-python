return {
	id = "HAITIAN5",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Set Sail! An Inspiration-Seeking Journey\n\n<size=45>5 An Unseen Scenery</size>",
					1
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			say = "I finish setting up the tarp and: come back to watch Hai Tien for a little longer before the squirrels scurry off.",
			bgm = "story-richang-5",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Your story's pretty captivating, Hai Tien.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 9,
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "Oh, umm... I got a bit carried away in the moment. I forgot that you'd be able to hear me.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "By the way, are you able to talk to squirrels?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 502070,
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Huh? I definitely can't...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "What? The way they were moving and responding to you made it seem like you were able to communicate with them...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "I think they were just responding to me being nice to them...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 502070,
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "The reason it looked like we were communicating is probably because of some preconception on your part, Commander~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "Since you've been working so hard, why:n't you take a seat and rest for a while? We can take another walk around the area later.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Actually, I'm good to go still.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			say = "I put:wn the work tools and start taking Hai Tien around our campsite.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 9,
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "Hmm? Do you see that little trail over there?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			say = "I look in the direction she's pointing.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_520",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Looks promising. Want to head over and take a look?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			say = "The path leads to a grassy clearing, and there's even a little lake in the distance.",
			bgm = "story-richang-10",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 502070,
			side = 2,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "\"Winding paths lead to secluded beauty\" ...I'm not sure this saying quite captures the essence of this place, but...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "This scenery is quite different from what we've seen so far in the woods.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "That's True.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "Heehee. That's an interesting idea. You certainly have a way with words, Commander.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "\"Willow threads sway in the wind, the grass' garments trimmed by the rain\"...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "I never expected to see such beautiful scenery here.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "Commander. I've got it. I think I just had an epiphany.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			say = "The girl next to me gently gets up on her tiptoes, and rests her head against my shoulder.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
