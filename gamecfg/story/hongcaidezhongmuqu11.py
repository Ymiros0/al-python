return {
	fadeOut = 1.5,
	mode = 2,
	id = "HONGCAIDEZHONGMUQU11",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			say = "Outside the Singularity - NA Ocean",
			side = 2,
			bgName = "bg_story_bsmlevel",
			dir = 1,
			bgmDelay = 1,
			bgm = "theme-highseasfleet",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_bsmlevel",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 401020,
			say = "Eugen, you've tampered with the radar, haven't you?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403030,
			side = 2,
			bgName = "bg_story_bsmlevel",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Not this time. What's wrong with it?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_bsmlevel",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 401020,
			say = "\"Not this time\"... I see...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403030,
			side = 2,
			bgName = "bg_story_bsmlevel",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Really, I haven't. You're wondering about the Eagle Union and Northern Parliament fleets, aren't you?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403030,
			side = 2,
			bgName = "bg_story_bsmlevel",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I'll tell you right now, they're both actually here. Naval HQ greenlit this operation.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_bsmlevel",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 401020,
			say = "Have they? That explains it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 401020,
			side = 2,
			bgName = "bg_story_bsmlevel",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I see some of our forces are gearing up for battle. Are we not here simply to stall for time?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403030,
			side = 2,
			bgName = "bg_story_bsmlevel",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Not quite. When push comes to shove, you need to be ready to fight back.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_bsmlevel",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 401020,
			say = "Then allow me to transfer the recon data we've gathered. It should arrive in time.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403030,
			side = 2,
			bgName = "bg_story_bsmlevel",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Don't. You have to save it for after at least a skirmish has broken out.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403030,
			side = 2,
			bgName = "bg_story_bsmlevel",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "They've assembled a massive fleet, you know. We need to show them we mean business too.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_bsmlevel",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 401020,
			say = "I:n't like it when you use the phrase \"mean business\"... Should I start writing my will?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403030,
			side = 2,
			bgName = "bg_story_bsmlevel",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Not today. I promise to keep you safe, Z2.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_bsmlevel",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 401020,
			say = "Please extend that promise to my little Lebe. In fact, to our entire fleet, if you would.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403030,
			side = 2,
			bgName = "bg_story_bsmlevel",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I'm already:ing my best to keep the Azur Lane at bay through radar jamming.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_bsmlevel",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 401020,
			say = "They'll see through the jamming as soon as they get close. What's more, I dare say you're going overkill with it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403030,
			side = 2,
			bgName = "bg_story_bsmlevel",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Sure, they'll see through it, but: it's too late. Besides, overkill is the way to go when stalling for time.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403030,
			side = 2,
			bgName = "bg_story_bsmlevel",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "While they scramble to work around the jamming... Bam, that's when we strike. Then we have a shot at winning.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_bsmlevel",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 401020,
			say = "So that's the plan... I cannot deny it is sound, at least in theory.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_bsmlevel",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 401020,
			say = "You're sortieing too, aren't you? What is your target? Surely not the Commander's personal fleet, I hope.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403030,
			side = 2,
			bgName = "bg_story_bsmlevel",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Please, why would I be? The Commander's our guest.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403030,
			side = 2,
			bgName = "bg_story_bsmlevel",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "(Our next operation will run into difficulties if we commit too much to this one. But if we:n't commit enough, we can expect Naval HQ to keep a watchful eye on us...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403030,
			side = 2,
			bgName = "bg_story_bsmlevel",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Tell you what – we'll go for the Northern Parliament's fleet.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_bsmlevel",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 401020,
			say = "Very well:. I assume we have no other choice anyway.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403030,
			side = 2,
			bgName = "bg_story_bsmlevel",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "You assume correctly.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_bsmlevel",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 401020,
			say = "As I tend to:... If we must fight, so be it, but please try not to aggravate them too much.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403030,
			side = 2,
			bgName = "bg_story_bsmlevel",
			hidePaintObj = True,
			dir = 1,
			blackBg = True,
			nameColor = "#a9f548",
			say = "Don't worry, I can't even if I wanted to. It's my job to keep adjusting the frequency on this little jammer.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
