return {
	id = "HONGCAIDEZHONGMUQU13",
	mode = 2,
	once = True,
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			bgm = "theme-highseasfleet-reborn",
			side = 2,
			bgName = "bg_endingsong_5",
			say = "KABOOOM!",
			dir = 1,
			bgmDelay = 1,
			soundeffect = "event./battle/boom2",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			flashN = {
				color = {
					1,
					1,
					1,
					1
				},
				alpha = {
					{
						0,
						1,
						0.2,
						0
					},
					{
						1,
						0,
						0.2,
						0.2
					},
					{
						0,
						1,
						0.2,
						0.4
					},
					{
						1,
						0,
						0.2,
						0.6
					}
				}
			},
			dialogShake = {
				speed = 0.09,
				x = 8.5,
				number = 2
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 404040,
			say = "Direct hit! Damn, I'm good.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 404040,
			say = "Fighting with a rigging is pretty fun, actually.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 405040,
			say = "All the power of a warship compressed into a weapon on the scale of a human...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405040,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "With a rigging, a single soldier can accomplish what once took an entire crew to:.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 405040,
			say = "It's no wonder that shipgirls made warships obsolete.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 404040,
			say = "Plus, we're way smaller targets, so they're gonna miss most shots.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405040,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Yes, but that:esn't mean you should make like a sitting duck.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 404040,
			say = "I'm cool with being a sitting duck if it can't hit me regardless.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 404030,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "......",
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 403110,
			say = "What's the matter, Seydlitz?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 404030,
			say = "It's just... As I was calibrating my main guns, it dawned on me that it will be a long while before I master my rigging.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 403110,
			say = "Want some help with that? Heavy cruisers and battlecruisers must have at least a few attributes in common.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 404030,
			say = "Yes, please! First of all, if you could look at this...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			say = "KABOOOM!",
			side = 2,
			bgName = "bg_endingsong_5",
			dir = 1,
			soundeffect = "event./battle/boom2",
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			},
			flashN = {
				color = {
					1,
					1,
					1,
					1
				},
				alpha = {
					{
						0,
						1,
						0.2,
						0
					},
					{
						1,
						0,
						0.2,
						0.2
					},
					{
						0,
						1,
						0.2,
						0.4
					},
					{
						1,
						0,
						0.2,
						0.6
					}
				}
			},
			dialogShake = {
				speed = 0.09,
				x = 8.5,
				number = 2
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 402080,
			say = "I... I hit the target?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 402080,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I sank it, too?! How in the world did my peashooter cannons manage that...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 402080,
			say = "Oh, it must be because of my wish that I wouldn't land any successful hits. Thank goodness...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 404030,
			say = "It wasn't a fluke, Elbing. You've simply become stronger.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 404030,
			say = "Put your faith in yourself and your rigging, and you'll realize your full potential.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 404030,
			say = "I believe in you. These insignificant Sirens are no match for you!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 403110,
			say = "Enemy planes on the horizon! Ready your anti-air weapons!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 404040,
			say = "Planes, eh? Time to put this baby's AA capabilities to the test!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 405040,
			say = "Ditto. The more acquainted we become with our arsenals, the better.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 402080,
			say = "I-I think I should just fall back...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 404030,
			say = "There is no need to, Elbing! Get behind me!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			say = "KABOOOM!",
			side = 2,
			bgName = "bg_endingsong_5",
			dir = 1,
			soundeffect = "event./battle/boom2",
			flashN = {
				color = {
					1,
					1,
					1,
					1
				},
				alpha = {
					{
						0,
						1,
						0.2,
						0
					},
					{
						1,
						0,
						0.2,
						0.2
					},
					{
						0,
						1,
						0.2,
						0.4
					},
					{
						1,
						0,
						0.2,
						0.6
					}
				}
			},
			dialogShake = {
				speed = 0.09,
				x = 8.5,
				number = 2
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_5",
			say = "The aircraft dropped their payloads over Seydlitz, but only scored near misses. Her allies trained their AA guns on the planes and promptly shot them:wn.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 404030,
			say = "Enemy aircraft neutralized! Excellent. Now I also have a good idea of what our armor can withstand.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 402070,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Why weather the enemy's attacks when one can evade them?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_5",
			say = "The Swan of the East gracefully moved out of the way of incoming machine-gun fire,: retaliated with attacks of her own.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 404040,
			say = "Wow, look at you. You've already become a pro at this.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 402070,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "\"Emden's strength is not to be scoffed at, Lützow.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 402070,
			say = "We are a long way away from perfection, but an average battle poses no issue to us.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 404040,
			say = "...You have it easy just 'cause there's two of you.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 402070,
			say = "If you have something you wish to say, speak up and look us in the eye.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 402070,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			blackBg = True,
			nameColor = "#a9f548",
			say = "\"And if you've the time for that, use it on fighting instead. Emden will not tolerate any laziness.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
