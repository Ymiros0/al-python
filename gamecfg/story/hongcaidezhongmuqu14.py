return {
	fadeOut = 1.5,
	mode = 2,
	id = "HONGCAIDEZHONGMUQU14",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			say = "A group of shipgirls watched from afar as the reborn warships practiced with their riggings.",
			side = 2,
			bgName = "bg_endingsong_5",
			dir = 1,
			bgmDelay = 1,
			bgm = "battle-againstfate",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Looks like they've gotten the hang of it. Taking out the small fry Sirens around here should be a breeze for them.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Once that's:ne, I reckon we can start surveying the area beyond the anchorage.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Then there's only one thing left to: – find out what that Siren superweapon was.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307010,
			say = "(Friedrich has:ne it again...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "(The power balance between the four big factions is at risk of collapsing entirely if the Iron Blood's forces keep growing at this rate.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "...Akagi?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Is there something on your mind?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307010,
			say = "No, nothing in particular.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Say, Zuikaku, have we ever needed to learn the very basics of our riggings?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Now that you mention it... No. We: study the specifics of new planes and gear and the like, but that's different.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I long thought it impossible to directly compare Sakura Empire and Iron Blood riggings due to their design differences. It seems I was wrong.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Wrong? How so? Is there something unusual about theirs?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "They look in line with all other Iron Blood riggings I've seen before...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Think about it. Mikasa predates even those girls, but she never had to learn how to use her rigging.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "This is only speculation, but I see this as a strong indication that those girls were never meant to equip these riggings.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 499020,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Heheh. How perceptive you are.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 499020,
			say = "The notion that the Cube creates rigging based on a particular ship's hull is not entirely True. While this:es occur in many instances, it is a guideline rather than a hard rule.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 499020,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "We learned this through our research. All I've:ne here is put it to good use.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 499020,
			say = "Let me remind you, Akagi, that our rondo is still underway.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 499020,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Please be patient and see it to its conclusion. I assure you, you will not be disappointed.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "...Very well. I will: as you say and wait for the finale.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307010,
			say = "(That's assuming Azur Lane:esn't show up and end her performance prematurely, anyway.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307010,
			say = "(The clock is ticking on the Singularity. Friedrich is wasting time on these frivolities...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "......",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "......",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "......",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Why are you staring at me?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "You and I are both spectators here. I cannot speculate on what Friedrich's grand plan is.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307010,
			say = "(Then there's Elizabeth... It'd be naive to think she's granting us access to Scapa Flow just out of the kindness of her heart.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "(There must be something of greater importance to this place than what we've seen so far.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			blackBg = True,
			nameColor = "#a9f548",
			say = "(I can only hope the Commander knows what that is... But if not...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
