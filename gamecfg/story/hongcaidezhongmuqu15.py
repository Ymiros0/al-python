return {
	fadeOut = 1.5,
	mode = 2,
	id = "HONGCAIDEZHONGMUQU15",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			say = "KABOOOM!",
			side = 2,
			bgName = "bg_endingsong_5",
			dir = 1,
			bgmDelay = 1,
			bgm = "theme-highseasfleet-reborn",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405040,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Direct hit. We've broken up the Sirens' fleet.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403110,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Fascinating. It seems charging in is a valid strategy when you're a shipgirl.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 404040,
			say = "Warships can: that too! Though they'll get lit up like a Christmas tree.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 405040,
			say = "Speaking of which, we've only fought warship-sized Sirens thus far. Zuikaku mentioned \"humanoid Sirens\" in passing earlier.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 405040,
			say = "I imagine they're in a league of their own. Running and gunning like this will only work on small groups of big enemies.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 404030,
			say = "Should it come:wn to a point-blank fight with a human-sized adversary, my sword will come in handy!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 404030,
			say = "Even if all my shells are spent, I can keep fighting the Sirens!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 404040,
			say = "Knock yourself out. Me, I'd just call it quits at that point.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 404040,
			say = "On another note, is anyone else curious how inter-faction relations have developed?",:
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 404040,
			say = "Putting aside the obvious stuff anyone can infer, anyway.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 404040,
			say = "You reckon things are getting tense because of this operation they've got going?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405040,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "It's safe to say there are at least two major camps. the Azur Lane and the Crimson Axis.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 405040,
			say = "The Azur Lane seems to be under Naval HQ's joint command, whereas the Iron Blood is the de-facto representative of the Crimson Axis.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 404040,
			say = "Sooo... Which side is fighting the Sirens? They can't still be fighting among themselves, surely.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403110,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Both of them:. They have the same objective, but differ in how they aim to achieve it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 402070,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "That is human nature – to splinter off into groups that fight among each other.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 402070,
			say = "\"However, they can achieve incredible things on the rare occasions they come together as one.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 402070,
			say = "\"Taken as individuals, they are frail, powerless, and always yearning for stability.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 402070,
			say = "That's what makes them so sweet. Heehee...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 404040,
			say = "Weird definition of \"sweet,\" but you: you...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 404040,
			say = "Speaking of coming together, anyone else find it interesting how representatives from the Iron Blood, Sakura Empire, and Royal Navy are all in one place to: something other than fight?",:
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 404030,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "It is unusual, yes. Perhaps they've managed to purge the Sirens from their world... or they now face a threat even bigger than them?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 404030,
			say = "Either way, they deserve respect for all the battles they've fought leading up to today.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 404030,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Remember what Friedrich said – a shipgirl's duty is to wage war on the Sirens, not her own kind.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 404030,
			say = "We are soldiers, which is both our greatest strength and greatest flaw.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 404030,
			say = "Our forte is fighting, not unifying all of humanity. Only one person can reforge the Azur Lane of old...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 402070,
			say = "The one known as the Commander.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 404030,
			say = "Indeed. All shipgirls, regardless of affiliation, are welcome under the Commandant's banner.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 402070,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "\"To think a feeble human would be the key to a peaceful future.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_5",
			say = "The Iron Blood shipgirls became more and more proficient at tracking and sinking Sirens as they accustomed to their new riggings.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_5",
			say = "The former warships that were hunted:wn at Scapa Flow had now become the hunters.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 402070,
			side = 2,
			bgName = "bg_endingsong_5",
			hidePaintObj = True,
			dir = 1,
			blackBg = True,
			nameColor = "#a9f548",
			say = "I wish to see this Commander – my darling human – for myself. Heehee...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
