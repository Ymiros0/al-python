return {
	fadeOut = 1.5,
	mode = 2,
	id = "HONGCAIDEZHONGMUQU19",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			say = "Scapa Flow - Harbor",
			side = 2,
			bgName = "bg_endingsong_4",
			dir = 1,
			bgmDelay = 1,
			bgm = "theme-highseasfleet",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 302130,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "It's terribly quiet... And the Iron Blood's ships are nowhere to be seen.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 499020,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Ägir and August von Parseval will remain behind as well:. If the Azur Lane comes, you know what to:.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			effects = {
				{
					active = True,
					name = "memoryFog"
				}
			}
		},
		{
			actor = 302130,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Friedrich said they'd be here. The fact they aren't makes me think they were diverted to the frontlines.",
			effects = {
				{
					active = False,
					name = "memoryFog"
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 302130,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I was under the impression this wasn't our fight. Perhaps soon it will be, though...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 305020,
			say = "Well, Akagi did give us orders to back you up if the Azur Lane shows up!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 301160,
			say = "You should've brought Yukikaze the Great along! Then this stroke of bad luck would've never happened!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307050,
			say = "Have you still not heard anything from Zuikaku?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307020,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "She's with Akagi. No harm will come to her as long as the Azur Lane:esn't reach us.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307050,
			say = "Given how the situation is developing, that's not very reassuring... If the Iron Blood deployed their forces to the frontlines, things must be quite dire.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307020,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I see your point. Though there's still a fair distance between us and the fighting, my recon planes are already picking up smoke clouds.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307020,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "They're headed straight for us, that much is clear. What I can't say is whether or not they know we are here.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307050,
			say = "Maybe they hope to ascertain the Royal Navy's intentions? None of their faction leaders know about their involvement in this operation.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307050,
			say = "...Either way, I'll: as Akagi said and head there.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307050,
			say = "Let's just hope the Sakura Empire won't need to join the fight today.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307020,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Got it. I'll let Akagi know you–",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_4",
			say = "All of a sudden, the placid Singularity roared. A beam of light burst out from its gateway.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_4",
			say = "The energy spike made the water's surface boil. Amid the steam, a figure clad in a red kimono descended.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307020,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Akagi?! Are you all right? How are things beyond the gate?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I'm fine, as is Zuikaku. We just got:ne witnessing something very, very interesting.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307010,
			say = "Though now's not the time to talk about it. That comes later.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I see you and Jintsuu are the only ones here... The Azur Lane has made a move, hasn't it?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307020,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Correct. They're headed for us right now.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307020,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "The Iron Blood is employing stalling tactics, but it's only a matter of time before they'll reach us.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307020,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Shoukaku just said she's on the way. If we're lucky, we can escape before the Azur Lane arrives.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Escape? Don't be ridiculous. Everything is going exactly according to plan.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307010,
			say = "You've:ne well, guarding the gate, but now it's time for us to join the fight.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307010,
			say = "I know you think I sound crazy. We're outnumbered and seemingly have our backs against the wall...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "To which I say – it's a good thing we made some new friends inside the Singularity.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_4",
			say = "Akagi gestured towards the gateway to dispel Kaga's confusion.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_4",
			say = "Zuikaku was first to return,: Friedrich,: Queen Elizabeth... Everyone who had entered was back.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_4",
			say = "But: came the party of Iron Blood ships given new life. As the last one exited, the gate faded into specks of nothingness.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307020,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Are they... META ships?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307010,
			say = "No. They are ordinary shipgirls like us.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			blackBg = True,
			nameColor = "#a9f548",
			say = "Friedrich needed more cards in her hand to accomplish the True goal of this operation. Now she has them.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
