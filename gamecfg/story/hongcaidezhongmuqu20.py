return {
	fadeOut = 1.5,
	mode = 2,
	id = "HONGCAIDEZHONGMUQU20",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			say = "Northern Parliament Control Center - Commanding Vessel",
			side = 2,
			bgName = "bg_endingsong_10",
			dir = 1,
			bgmDelay = 1,
			bgm = "theme-highseasfleet",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_endingsong_10",
			hidePaintObj = True,
			dir = 1,
			actorName = "Georgia",
			say = "Commander, I've come to update you on the situation.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_endingsong_10",
			hidePaintObj = True,
			dir = 1,
			actorName = "Georgia",
			say = "Soyuz's fleet is in the midst of encircling Scapa Flow, as planned.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_endingsong_10",
			hidePaintObj = True,
			dir = 1,
			actorName = "Georgia",
			say = "New Jersey's fleet is also on the move, but progress is still slow owing to Iron Blood's jamming and stalling tactics.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_endingsong_10",
			hidePaintObj = True,
			dir = 1,
			actorName = "Georgia",
			say = "Speaking of the Iron Blood, we've caught wind of the fact they're fielding blueprint ships.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_endingsong_10",
			hidePaintObj = True,
			dir = 1,
			actorName = "Georgia",
			say = "Most of them are under Ulrich's command. She seems to be making a beeline for this very fleet.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_endingsong_10",
			hidePaintObj = True,
			dir = 1,
			actorName = "Georgia",
			say = "Our forces are spread out because of all the jamming, so stopping Ulrich's fleet in its tracks will be difficult.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_endingsong_10",
			hidePaintObj = True,
			dir = 1,
			actorName = "Georgia",
			say = "But, if they: reach us, we can deal with them just fine. You're with us, after all.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_10",
			say = "\"Sounds like whatever they:, it won't be enough to turn the battle in their favor.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_10",
			say = "\"Just earlier, I heard Iron Blood planes have been spotted as well. I'll notify Enterprise's reserve fleet to be on their guard.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_endingsong_10",
			hidePaintObj = True,
			dir = 1,
			actorName = "Georgia",
			say = "The fact they're fielding blueprint ships now has me a bit wary, though...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_10",
			say = "\"You're right to be. Every faction voiced concerns over blueprint ships as this operation was being drafted, too. Soyuz insisted there wouldn't be any.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_10",
			say = "\"One of her informants had said so. Allegedly. Looking back at it now, she might just have been bluffing for the sake of getting the operation approved...\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			options = {
				{
					content = "\"Have you heard anything from the Royal Navy?\"",
					flag = 1
				}
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_endingsong_10",
			hidePaintObj = True,
			dir = 1,
			actorName = "Georgia",
			say = "You mean regarding Scapa Flow? They're still dancing around the subject.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_endingsong_10",
			hidePaintObj = True,
			dir = 1,
			actorName = "Georgia",
			say = "One minute they say they can't get in touch with anyone at the base, the next they say it's largely irrelevant to the operation at hand.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_endingsong_10",
			hidePaintObj = True,
			dir = 1,
			actorName = "Georgia",
			say = "Curiously, Her Majesty has also been telling us not to worry about it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_endingsong_10",
			hidePaintObj = True,
			dir = 1,
			actorName = "Georgia",
			say = "All signs point toward an Iron Blood invasion, but: why is the Royal Navy being so uncooperative? Scapa Flow is still an important base to them.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_endingsong_10",
			hidePaintObj = True,
			dir = 1,
			actorName = "Georgia",
			say = "If they wanted to hide something from us, it'd be the ideal location for it, too.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_10",
			say = "\"An invasion:es seem like the likeliest answer. If this operation is intended as a retaliatory attack against the Iron Blood, why are we headed for a Royal Navy anchorage?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_10",
			say = "\"And for that matter, why would the Iron Blood attack Scapa Flow to begin with?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_10",
			say = "\"Friedrich and Elizabeth will both have some explaining to: the next time I see them.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_endingsong_10",
			hidePaintObj = True,
			dir = 1,
			actorName = "Georgia",
			say = "Guess we'll have to find out for ourselves...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_endingsong_10",
			hidePaintObj = True,
			dir = 1,
			actorName = "Georgia",
			say = "Sorry for changing topics, but would you let me deal with Ulrich's fleet, Commander?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_endingsong_10",
			hidePaintObj = True,
			dir = 1,
			actorName = "Georgia",
			say = "Your fleet is more than enough to handle hers, but that would take time, and we've been stalled long enough as it is.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_endingsong_10",
			hidePaintObj = True,
			dir = 1,
			actorName = "Georgia",
			say = "I just need a few others to come with me. We'll head out and intercept Ulrich's blueprint brigade to ensure you have a clear path to Scapa Flow.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_endingsong_10",
			hidePaintObj = True,
			dir = 1,
			actorName = "Georgia",
			say = "We've identified the majority of the blips on the map by now, and if Friedrich's anywhere, she should be in the harbor.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			options = {
				{
					content = "\"All right. Permission granted to sortie.\"",
					flag = 1
				},
				{
					content = "\"Do it, and treat yourself to an Oxy-cola.\"",
					flag = 2
				},
				{
					content = "\"One must wonder what Ulrich's goal is with all this...\"",
					flag = 3
				}
			}
		},
		{
			blackBg = True,
			side = 2,
			bgName = "bg_endingsong_10",
			nameColor = "#a9f548",
			dir = 1,
			optionFlag = 1,
			actorName = "Georgia",
			hidePaintObj = True,
			say = "Thanks. I'll head out right now:. Best of luck to you!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			blackBg = True,
			side = 2,
			bgName = "bg_endingsong_10",
			nameColor = "#a9f548",
			dir = 1,
			optionFlag = 2,
			actorName = "Georgia",
			hidePaintObj = True,
			say = "Thanks. I'll head out right now:. Best of luck to you!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Georgia",
			bgName = "bg_endingsong_10",
			hidePaintObj = True,
			dir = 1,
			optionFlag = 3,
			nameColor = "#a9f548",
			say = "Yeah. I:ubt you're their real objective, because that'd be way too simple...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			blackBg = True,
			side = 2,
			bgName = "bg_endingsong_10",
			nameColor = "#a9f548",
			dir = 1,
			optionFlag = 3,
			actorName = "Georgia",
			hidePaintObj = True,
			say = "Hahahah! I'll treat myself to a whole case of 'em after we get home.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
