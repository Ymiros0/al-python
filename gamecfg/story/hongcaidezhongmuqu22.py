return {
	fadeOut = 1.5,
	mode = 2,
	id = "HONGCAIDEZHONGMUQU22",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			say = "Flagship Briefing Room - Near Scapa Flow",
			side = 2,
			bgName = "bg_endingsong_11",
			dir = 1,
			bgmDelay = 1,
			bgm = "theme-highseasfleet",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_endingsong_11",
			hidePaintObj = True,
			dir = 1,
			actorName = "Eldridge",
			say = "Many messages... Come in...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_endingsong_11",
			hidePaintObj = True,
			dir = 1,
			actorName = "Eldridge",
			say = "Should Eldridge... read?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			options = {
				{
					content = "\"Sure. Read them out loud for me.\"",
					flag = 1
				},
				{
					content = "\"No need to. I'll: it.\"",
					flag = 2
				}
			}
		},
		{
			side = 2,
			actorName = "Eldridge",
			bgName = "bg_endingsong_11",
			hidePaintObj = True,
			dir = 1,
			optionFlag = 1,
			nameColor = "#a9f548",
			say = "First message...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Eldridge",
			bgName = "bg_endingsong_11",
			hidePaintObj = True,
			dir = 1,
			optionFlag = 1,
			nameColor = "#a9f548",
			say = "Commander... Iron Blood forces...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Eldridge",
			bgName = "bg_endingsong_11",
			hidePaintObj = True,
			dir = 1,
			optionFlag = 1,
			nameColor = "#a9f548",
			say = "Repositioned...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Eldridge",
			bgName = "bg_endingsong_11",
			hidePaintObj = True,
			dir = 1,
			optionFlag = 1,
			nameColor = "#a9f548",
			say = "Ulrich and her fleet... retreated. To Scapa Flow...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Eldridge",
			bgName = "bg_endingsong_11",
			hidePaintObj = True,
			dir = 1,
			optionFlag = 1,
			nameColor = "#a9f548",
			say = "Failed to... deal damage...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Eldridge",
			bgName = "bg_endingsong_11",
			hidePaintObj = True,
			dir = 1,
			optionFlag = 1,
			nameColor = "#a9f548",
			say = "Blueprint ships... strong... Put up... solid fight...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Eldridge",
			bgName = "bg_endingsong_11",
			hidePaintObj = True,
			dir = 1,
			optionFlag = 1,
			nameColor = "#a9f548",
			say = "Heading back to...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Eldridge",
			bgName = "bg_endingsong_11",
			hidePaintObj = True,
			dir = 1,
			optionFlag = 1,
			nameColor = "#a9f548",
			say = "Yes. Heading back to resupply... and... reorganize...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Eldridge",
			bgName = "bg_endingsong_11",
			hidePaintObj = True,
			dir = 1,
			optionFlag = 1,
			nameColor = "#a9f548",
			say = "After that... move forward again...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Eldridge",
			bgName = "bg_endingsong_11",
			hidePaintObj = True,
			dir = 1,
			optionFlag = 1,
			nameColor = "#a9f548",
			say = "Resupply... mass-produced ships too...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Eldridge",
			bgName = "bg_endingsong_11",
			hidePaintObj = True,
			dir = 1,
			optionFlag = 1,
			nameColor = "#a9f548",
			say = "See you... at Scapa Flow... From New Jersey...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_11",
			say = "\"Um, thanks, Eldridge. I'll read the rest for myself, if you:n't mind.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_11",
			say = "A short while later...",
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_11",
			say = "Based on the reports from all our fleets, it seemed the Iron Blood's strategy had changed.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_11",
			say = "They had given up on their electronic interference and stalling tactics and instead rallied all their forces at Scapa Flow.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_11",
			say = "Had our attacks put them on the back foot? Were they:ne trying to diverge our attention away from whatever they were hiding? Nobody knew.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_11",
			say = "Kronshtadt's fleet had apparently engaged Odin, and they survived the battle without any major casualties.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_11",
			say = "Georgia's fleet fought Ulrich's fleet and, incredibly, won. The former was now on its way back to regroup with us, while the latter broke off to retreat.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_11",
			say = "Lastly, New Jersey's fleet defeated Tirpitz's fleet, but sustained minor damage after a battle with Ulrich's fleet as it was retreating.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_11",
			say = "All in all, though the Iron Blood had fought admirably, they were still losing the battle.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_11",
			say = "I still had no idea what Friedrich's goal was, but it seemed to me like her plan was going to be a bust.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_11",
			say = "I kicked back in my seat, drew a sigh, and reviewed our fleets' statuses once more.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_11",
			say = "Saratoga's reserve fleet had linked up with ours. I now commanded a very respectable fleet for the final battle.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_endingsong_11",
			dir = 1,
			blackBg = True,
			say = "I had all the right cards in my hand. All I had to: was lay them on the table.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
