return {
	fadeOut = 1.5,
	mode = 2,
	id = "HONGCAIDEZHONGMUQU27",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			bgm = "theme-frederick",
			side = 2,
			bgName = "bg_endingsong_12",
			dir = 1,
			say = "Equipped with heavy armor, layers of defensive barriers, and powerful regeneration abilities, Compiler is one of the most durable high-ranking Sirens.",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_12",
			say = "However, that only means anything against a normal-sized fleet.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_12",
			say = "Faced with a relentless barrage, the combined forces were able to break through her barriers much faster than she was able to regenerate them.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 402070,
			side = 2,
			bgName = "bg_endingsong_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "\"Weren't your defenses supposed to be impenetrable? What happened to that? Ahahaha!\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_endingsong_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 404040,
			say = "Now, to pry apart that armor of yours~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 404030,
			side = 2,
			bgName = "bg_endingsong_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "I'll admit, she's incredibly resilient, but there's no way she'll be able to repair all this damage in time.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403110,
			side = 2,
			bgName = "bg_endingsong_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "But we still have to watch out for her pesky attacks... Especially those damn tentacles!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 402080,
			side = 2,
			bgName = "bg_endingsong_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Those tentacles are going to mess me up and: unspeakable things to me... I already know it...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405040,
			side = 2,
			bgName = "bg_endingsong_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Even a high-spec Siren like her won't be able to: a thing against the combined might of all us shipgirls.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 102160,
			side = 2,
			bgName = "bg_endingsong_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Those new Iron Blood ships seem like they're:ing way more damage to her barriers than we are!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_12",
			say = "\"You're right. Those barriers that completely negate attacks from other ships are getting ripped apart by Emden's attacks.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_12",
			say = "\"Wait... Is Compiler unable to adapt to their attacks?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_endingsong_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			actor = 900230,
			say = "Warning. Barrier module damaged. Energy charger overheating. Unable to sustain repairs.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			side = 2,
			bgName = "bg_endingsong_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "Warning. Armor destroyed. Damage to core module imminent.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 199020,
			side = 2,
			bgName = "bg_endingsong_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "This is the end of the line for you, Compiler. You and your fleet are going straight back to the abyss you came from!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_endingsong_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 404040,
			say = "Are we going to score a total victory here?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 900230,
			side = 2,
			bgName = "bg_endingsong_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "No... This is just the beginning.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			side = 2,
			bgName = "bg_endingsong_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "Deviation from experimental values has risen to a dangerous threshold.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			side = 2,
			bgName = "bg_endingsong_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "The reset sequence must be activated.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			side = 2,
			bgName = "bg_endingsong_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "The reset sequence must be activated...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 499020,
			side = 2,
			bgName = "bg_endingsong_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "You won't be able to: that.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_endingsong_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 499020,
			say = "For you, your body is just another meaningless vessel. No matter how many times you fail, no matter how many times you're destroyed, you will be reborn again and again.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 499020,
			side = 2,
			bgName = "bg_endingsong_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "By using the reset sequence, you might be able to destroy and recreate this Mirror Sea...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 499020,
			say = "But what if this place also houses your program's mainframe?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_endingsong_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 499020,
			say = "What would happen to your data?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_endingsong_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			actor = 900230,
			say = "This place is...?! How did you...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_endingsong_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			actor = 900230,
			say = "How?! Why?! Without the Sirens, you––",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			say = "KABOOOM!",
			side = 2,
			bgName = "bg_endingsong_12",
			dir = 1,
			soundeffect = "event./battle/boom2",
			flashN = {
				color = {
					1,
					1,
					1,
					1
				},
				alpha = {
					{
						0,
						1,
						0.2,
						0
					},
					{
						1,
						0,
						0.2,
						0.2
					},
					{
						0,
						1,
						0.2,
						0.4
					},
					{
						1,
						0,
						0.2,
						0.6
					}
				}
			},
			dialogShake = {
				speed = 0.09,
				x = 8.5,
				number = 2
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_12",
			say = "Countless shells rained:wn on Compiler, the vessel falling silent after her consciousness is transferred away.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_12",
			say = ".........",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			options = {
				{
					content = "\"Why didn't you let her finish?\"",
					flag = 1
				},
				{
					content = "\"What did you mean when you said 'mainframe'?\"",
					flag = 2
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_endingsong_12",
			dir = 1,
			optionFlag = 1,
			actor = 499020,
			nameColor = "#ffff4d",
			hidePaintObj = True,
			say = "Because I:n't care what she had to say.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 499020,
			side = 2,
			bgName = "bg_endingsong_12",
			hidePaintObj = True,
			dir = 1,
			optionFlag = 2,
			nameColor = "#ffff4d",
			say = "In layman's terms, the origin of the Sirens. The data center that allows them to operate in this world.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 499020,
			side = 2,
			bgName = "bg_endingsong_12",
			hidePaintObj = True,
			dir = 1,
			optionFlag = 2,
			nameColor = "#ffff4d",
			say = "If we are able to destroy it, the Sirens will lose their anchor to our world and our timeline.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 499020,
			say = "Ägir, August, you know what to:. Blow the gateway wide open.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_endingsong_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 499060,
			say = "I shall see to it. Friedrich, evacuate everyone away from the gate.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_endingsong_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 499050,
			say = "If you get caught in its path, it'll cleave you in two.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_12",
			say = "\"You're going to 'blow it wide open'?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 499020,
			side = 2,
			bgName = "bg_endingsong_12",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Quite literally. We will stretch the very fabric of the Mirror Sea and bring Compiler's mainframe to us.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_endingsong_12",
			dir = 1,
			blackBg = True,
			actor = 499020,
			nameColor = "#ffff4d",
			hidePaintObj = True,
			say = "Now... Your attention, please. It is time to play the final movement.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
