return {
	fadeOut = 1.5,
	mode = 2,
	id = "HONGCAIDEZHONGMUQU3",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			say = "Just on the horizon, the first vestiges of the anchorage became visible to the Sakura fleet.",
			side = 2,
			bgName = "bg_story_nepu2",
			dir = 1,
			bgmDelay = 1,
			bgm = "battle-againstfate",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Akagi, we're about to arrive at Scapa Flow!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "It seems like the Royal Navy hasn't even mobilized any forces in this area yet, let alone launched a counterattack.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Does that mean they were caught off-guard by a surprise attack?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307020,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Perhaps... But look closely, Zuikaku.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307020,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "There aren't any signs of fighting here, and it:esn't seem like the Iron Blood has taken the facilities for their own use either.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307020,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Did they drive out the Royal Navy and: leave things alone... or was the Iron Blood never interested in occupying Scapa Flow in the first place?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 302130,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Isn't the Scapa Flow anchorage critical to the defense of the Royal Navy's homeland? It wouldn't make any sense for them to just give it up without a fight.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 302130,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "It's possible that the Iron Blood needed this area to conduct some sort of experiment, but that still:esn't explain the absence of Royal Navy ships.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307010,
			say = "Hard to say. Just be glad we:n't have to fight our way to the spectator seats.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Let's go. I'm sure Friedrich will soon provide answers to our questions.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			say = "Scapa Flow Anchorage",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Wow... Even though this place is enclosed by islands, there are so many waterways leading out...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Seeing this in person really makes you appreciate how powerful Mother Nature can be.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Akagi, what's that ship over there?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "It looks mass-produced, but a really old-fashioned model.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			say = "As the fleet entered the harbor, the shadow of a large moored warship loomed in front of them.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307010,
			say = "This is... a Queen Elizabeth-class?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "It:esn't look anything like the mass-produced ships we've seen before though...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307010,
			say = "Because it's not. This is a warship that predates the mass-produced \"ships\" of our era.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "We are \"ships\" born from Wisdom Cubes, but these vessels are constructs assembled from steel, protecting the seas long before our time.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307010,
			say = "Before the advent of Wisdom Cubes, humanity built these vessels, and they became the blade they wielded.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307010,
			say = "Nowadays, you'll only find transport ships and rare models of mass-produced ships made the old-fashioned way.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307010,
			say = "Unlike modern mass-production which can churn out a large number of ships in a short amount of time, each one of these warships had unique designs and a long construction process.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307010,
			say = "That is why when we, the shipgirls, appeared, these older vessels were quickly phased out of production.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Even the mass-produced ships are little more than mementos of a bygone era, reserve forces that assist us in our operations.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307010,
			say = "But some things are for the best. The era when these warships were the only line of defense against the Sirens was a tragic one.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307010,
			say = "(Rather than being scrapped like so many others, at least you were able to spend the rest of your lives in the relative peace of this harbor...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307010,
			say = "(I wonder what would've happened to our \"past forms\"...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307010,
			say = "(...If they could have been preserved into an era of peace...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "But judging by this warship's presence here, it seems that even the Iron Blood cannot touch the past.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I wonder if we have any warships like this back in the Sakura Empire...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307010,
			say = "Who knows? It's something I stopped caring about long ago.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 302130,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Akagi, an Iron Blood ship is coming our way.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			say = "Several Iron Blood ships were moored along the coast of the harbor.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 499050,
			say = "Heheh. You've come, Akagi of the Sakura Empire.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 499060,
			say = "Good day, esteemed guests. You've been personally invited by Friedrich der Große.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "(Ägir and August von Parseval... the Iron Blood's latest blueprint ships...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 499050,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I can see the curiosity in your eyes, but let's save that for later.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			say = "Guided by the Iron Blood ships, Akagi and her fleet made their way deeper into the harbor.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			dir = 1,
			blackBg = True,
			actor = 499060,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Right this way. Friedrich has been waiting for you.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
