return {
	id = "HONGCAIDEZHONGMUQU30-1",
	mode = 2,
	once = True,
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			actor = 403110,
			side = 2,
			nameColor = "#ffff4d",
			hidePaintObj = True,
			dir = 1,
			say = "We're almost within firing range of the core tower, but these Compiler vessels won't let us get a clear shot...",
			bgm = "theme-arbitrator-tower",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			actor = 404040,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "There's just no end to them! I've BEEN sick and tired of this!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 404030,
			nameColor = "#ffff4d",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "To make things worse, their combat specs keep going up the closer we get to the tower!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 499020,
			nameColor = "#ffff4d",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "The Sirens are getting desperate. But we've defeated plenty of these vessels before, and so we shall: to the rest.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			actor = 499020,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "As I mentioned before, the \"consciousness\" of high-ranking Sirens can be transferred between any number of bodies, so they:n't have a True body per se.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 499020,
			nameColor = "#ffff4d",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "What we face now are scarcely different from Pawns, albeit stronger.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			actor = 499020,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Oh my... I seem to have struck a nerve with someone.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			nameColor = "#ff5c5c",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "...That you've come this far has already exceeded my calculations.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			actor = 402070,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "\"Exceeded your calculations, huh? Has anything ever fallen within your calculations?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			nameColor = "#ff5c5c",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "Further calibrations will be conducted. The outcome shall be self-evident. Nothing will change.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 402070,
			nameColor = "#ffff4d",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "We keep exceeding your calculations because you constantly look:wn on us.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			actor = 402070,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "You were the one who attacked us as Scapa Flow, weren't you? How the tides have turned.",
			effects = {
				{
					active = True,
					name = "speed"
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			actor = 900230,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "Your bravado is foolish and meaningless.",
			effects = {
				{
					active = False,
					name = "speed"
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			nameColor = "#ff5c5c",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "Without the Sirens' intervention, the Iron Blood fleet would've been scuttled at Scapa Flow.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			actor = 402070,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "\"...What?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			say = "Compiler did not respond, instead using the vessel to spread a thick sea fog around the Iron Blood ships.",
			effects = {
				{
					active = True,
					name = "miwu_01",
					center = True
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			say = "A number of shipgirls began to appear in the fog, like ghosts returning from the grave with untold stories.",
			effects = {
				{
					active = True,
					name = "miwu_01",
					center = True
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			actor = 404030,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "They ordered us to take our Iron Blood flag:wn ten days ago and haven't given us permission to fly it again ever since.",
			effects = {
				{
					active = True,
					name = "miwu_01",
					center = True
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			actor = 404030,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "And now, they've even unbolted our cannons. We:n't have the ability to fight anymore.",
			effects = {
				{
					active = True,
					name = "miwu_01",
					center = True
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 404030,
			side = 2,
			nameColor = "#ffff4d",
			hidePaintObj = True,
			dir = 1,
			say = "Are we supposed to just sit here until the talks reach a conclusion?",
			effects = {
				{
					active = True,
					name = "miwu_01",
					center = True
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			actor = 404030,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "That person is... me? But I:n't remember ever saying anything like that...",
			effects = {
				{
					active = True,
					name = "miwu_01",
					center = True
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			say = "The sea fog grew thicker, and a new scene began to play out.",
			effects = {
				{
					active = True,
					name = "miwu_01",
					center = True
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 404030,
			side = 2,
			nameColor = "#ffff4d",
			hidePaintObj = True,
			dir = 1,
			say = "We've been stuck here for months, and the peace talks still haven't panned out yet...",
			effects = {
				{
					active = True,
					name = "miwu_01",
					center = True
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			actor = 404030,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "I can't just sit by and watch any longer! The Royal Navy fleet sailed out of Scapa Flow this morning for training, so this is our chance.",
			effects = {
				{
					active = True,
					name = "miwu_01",
					center = True
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			actor = 404030,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "We move out today. We will honor our vows to the Iron Blood!",
			effects = {
				{
					active = True,
					name = "miwu_01",
					center = True
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			actor = 404030,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "It's a message from Emden... \"Carry out Paragraph 11.\"",
			effects = {
				{
					active = True,
					name = "miwu_01",
					center = True
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 404030,
			side = 2,
			nameColor = "#ffff4d",
			hidePaintObj = True,
			dir = 1,
			say = ".........Understood.",
			effects = {
				{
					active = True,
					name = "miwu_01",
					center = True
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			actor = 404030,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "I have no desire to become a hero. Our duty is but to: and die.",
			effects = {
				{
					active = True,
					name = "miwu_01",
					center = True
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			actor = 404030,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Goodbye, my comrades. For the Iron Blood.",
			effects = {
				{
					active = True,
					name = "miwu_01",
					center = True
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			say = "One after another, the shades of the shipgirls sank into the sea quietly and disappeared.",
			effects = {
				{
					active = True,
					name = "miwu_01",
					center = True
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			actor = 405040,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "We fought against the Royal Navy. We were defeated, and: scuttled...",
			effects = {
				{
					active = False,
					name = "miwu_01"
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			actor = 405040,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Is this the truth... or a vicious illusion to poison our minds?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 6,
			side = 2,
			actor = 403110,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "I have no recollection of:ing or saying any of those things.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 402080,
			nameColor = "#ffff4d",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "There's no way that happened, but...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			actor = 402070,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Emden is still trembling... What is the meaning of what you've shown us?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 900230,
			nameColor = "#ff5c5c",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "It is your truth.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			actor = 402070,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "I see now... Quite a convincing ruse you put on just now, Compiler.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			actor = 402070,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "But your \"records\" are neither our past nor our future.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			actor = 402070,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Your cognitive jamming has no effect on Emden! Heheh... Hahaha~♥",
			effects = {
				{
					active = True,
					name = "speed"
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
