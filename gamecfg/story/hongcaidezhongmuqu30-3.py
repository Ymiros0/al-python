return {
	id = "HONGCAIDEZHONGMUQU30-3",
	mode = 2,
	once = True,
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			expression = 2,
			side = 2,
			actor = 102160,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "We've got planes coming in from behind us! It must be the Sakura fleet's air support!",
			bgm = "theme-arbitrator-tower",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			say = "\"Did Akagi finally make up her mind? Great news - that's going to increase our odds of winning.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			nameColor = "#ff5c5c",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "Fools. I will show you your truths as well.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			say = "Once again, Compiler's vessel blanketed the area with a dense sea fog.",
			effects = {
				{
					active = True,
					name = "miwu_01",
					center = True
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			nameColor = "#ffff4d",
			hidePaintObj = True,
			dir = 1,
			say = "Ugh...!",
			effects = {
				{
					active = True,
					name = "miwu_01",
					center = True
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			actor = 306090,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Zuikaku, you're badly injured!",
			effects = {
				{
					active = True,
					name = "miwu_01",
					center = True
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 307060,
			side = 2,
			nameColor = "#ffff4d",
			hidePaintObj = True,
			dir = 1,
			say = "Don't worry about me... Get out of here as quickly as you can!",
			effects = {
				{
					active = True,
					name = "miwu_01",
					center = True
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			actor = 306080,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "The operation has failed, and we are the decoys. Where are we supposed to run...?",
			effects = {
				{
					active = True,
					name = "miwu_01",
					center = True
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			nameColor = "#ffff4d",
			hidePaintObj = True,
			dir = 1,
			say = "Beats me. Just, anywhere but here...",
			effects = {
				{
					active = True,
					name = "miwu_01",
					center = True
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			nameColor = "#ffff4d",
			hidePaintObj = True,
			dir = 1,
			say = "Haha... I was so gung-ho about making a heroic last stand... But I couldn't: anything at all...",
			effects = {
				{
					active = True,
					name = "miwu_01",
					center = True
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			actor = 306090,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "There are more enemy planes coming at us! Chitose, Zuikaku,: you have any planes left?",
			effects = {
				{
					active = True,
					name = "miwu_01",
					center = True
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			nameColor = "#ffff4d",
			hidePaintObj = True,
			dir = 1,
			say = "Of course not. We never had a chance to begin with... We all knew that, didn't we?",
			effects = {
				{
					active = True,
					name = "miwu_01",
					center = True
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			actor = 306090,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "I'm scared, Chitose... I can't feel my body anymore...",
			effects = {
				{
					active = True,
					name = "miwu_01",
					center = True
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			actor = 306080,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "It's okay, Chiyoda. I'll... always be with you...",
			effects = {
				{
					active = True,
					name = "miwu_01",
					center = True
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			nameColor = "#ffff4d",
			hidePaintObj = True,
			dir = 1,
			say = "Shoukaku... I'll be able to see you again, soon...",
			effects = {
				{
					active = True,
					name = "miwu_01",
					center = True
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			say = "The shadows of the Sakura Empire's carriers rumbled and burst into flames before vanishing beneath the waves.",
			effects = {
				{
					active = True,
					name = "miwu_01",
					center = True
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 302130,
			side = 2,
			nameColor = "#ffff4d",
			hidePaintObj = True,
			dir = 1,
			say = "Keep your wits about you, Zuikaku. She's trying to get into your head.",
			effects = {
				{
					active = False,
					name = "miwu_01"
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			nameColor = "#ffff4d",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "...I know. I've heard about a similar thing happening to the Eagle Union fleet inside the Bermuda Triangle.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			nameColor = "#ffff4d",
			hidePaintObj = True,
			dir = 1,
			say = "I'll be alright, Jintsuu. I know those illusions aren't really us.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 307060,
			nameColor = "#ffff4d",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "Right now, we're all allies... And we're even stronger when we fight together!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			nameColor = "#ffff4d",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "So something like this will never happen – and I'll fight to make sure of it!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			nameColor = "#ffff4d",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "Don't be deceived by her illusions, everyone! ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			nameColor = "#ff5c5c",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "What I have shown you is the truth. What you have experienced is False. History:es not lie.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			actor = 900230,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "You know this to be True, Akagi.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			nameColor = "#ff5c5c",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "After all, your reality is different from Zuikaku's.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			nameColor = "#ffff4d",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "Hahaha... Ahahahaha!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			actor = 307010,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "How disappointing, Siren. Our relationship goes no deeper than mutually exploiting one another.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			nameColor = "#ffff4d",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "You're mistaken if you think you can freely manipulate me to your convenience.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			nameColor = "#ffff4d",
			hidePaintObj = True,
			dir = 1,
			say = "You have my permission to go all out. Let's show these Sirens the pride of the Sakura Empire!",
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307020,
			side = 2,
			nameColor = "#ffff4d",
			hidePaintObj = True,
			dir = 1,
			say = "Heh. Well said.",
			effects = {
				{
					active = True,
					name = "speed"
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			nameColor = "#ffff4d",
			hidePaintObj = True,
			dir = 1,
			say = "That's the Akagi I know!",
			effects = {
				{
					active = False,
					name = "speed"
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
