return {
	fadeOut = 1.5,
	mode = 2,
	id = "HONGCAIDEZHONGMUQU31",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			say = "Scapa Flow - Harbor",
			side = 2,
			bgName = "bg_endingsong_15",
			dir = 1,
			bgmDelay = 1,
			bgm = "theme-frederick",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 9704020,
			side = 2,
			bgName = "bg_endingsong_15",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "We should be able to make it back to the resupply base with all these resources.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_15",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 9704020,
			say = "Have to say, this is one crazy idea, coming from you.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_15",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 9704020,
			say = "You're sure you want to step:wn as the leader? Even if you work best behind the scenes, the fact remains...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 499020,
			side = 2,
			bgName = "bg_endingsong_15",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "If you're worried about the Iron Blood of this world,:n't be. Bismarck survived her METAmorphosis – she'll lead the Iron Blood well.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_endingsong_15",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 499020,
			say = "Besides, since I can't see my child every day, I need to get back to devising plans, lest I get restless.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_15",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 9704020,
			say = "If you're fine with it,: fair enough, but what about these girls?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_endingsong_15",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 404040,
			say = "I'm good with whatever, honestly. All I care about is having a good time.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_endingsong_15",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 404040,
			say = "And if I'm not having fun, I'll just bail back home when I feel like it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_15",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 404040,
			say = "...Wait, how DO we get home? Nobody's told me that part yet.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_endingsong_15",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 9704020,
			say = "We'll find a way. Friedrich's not the kind of woman who makes one-way trips.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_endingsong_15",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 404040,
			say = "Let's have Seydlitz fill in the blanks:. She's all about:ing the right thing.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 404030,
			side = 2,
			bgName = "bg_endingsong_15",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "I'm not sure that falls within the:main of \"doing the right thing,\" but whatever the case, I'll: everything in my power to serve the Iron Blood!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 402080,
			side = 2,
			bgName = "bg_endingsong_15",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "I bet we're never coming back... We'll sink at the hands of the Ashes if the Sirens:n't get us first...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_endingsong_15",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 403110,
			say = "After this job's over, I just want to settle:wn as someone's right hand.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405040,
			side = 2,
			bgName = "bg_endingsong_15",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "There's no \"settling:wn\" now that we've been reborn as shipgirls. Fighting is what we:. Also, whose right hand?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_endingsong_15",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 403110,
			say = "It:esn't really matter. Who knows, it could end up being Emden.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403110,
			side = 2,
			bgName = "bg_endingsong_15",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Although, my dream would be to serve the Commander.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405040,
			side = 2,
			bgName = "bg_endingsong_15",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "I'm confident your dream will come True someday.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_endingsong_15",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 9704020,
			say = "All right, I've plugged in the coordinates. Energy tanks are full. It's time to go.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 499020,
			side = 2,
			bgName = "bg_endingsong_15",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Great. Please lead the way, Scharnhorst.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_15",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 499020,
			say = "I'm dying to see the Ashes' base and hear their orchestra. What instruments will they play, I wonder?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			mode = 1,
			flashout = {
				dur = 0.1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.1,
				dur = 0.1,
				bgName = "bg_white",
				alpha = {
					1,
					0
				}
			}
		}
	}
}
