return {
	fadeOut = 1.5,
	mode = 2,
	id = "HONGCAIDEZHONGMUQU32",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			say = "Unknown Location",
			side = 2,
			bgName = "bg_port_chongdong",
			dir = 1,
			bgmDelay = 1,
			bgm = "theme-dailyfuture",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			side = 2,
			bgName = "bg_port_chongdong",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "<Warning. Core Data Tower offline. Searching for viable backup facility.>",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			side = 2,
			bgName = "bg_port_chongdong",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "<Unable to find viable backup facility. Backup failed. Detecting unauthorized overwriting of data.>",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			side = 2,
			bgName = "bg_port_chongdong",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "<Access to Sectors E-K lost. Initiating repair protocol.>",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			side = 2,
			bgName = "bg_port_chongdong",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "<Repair protocol initiation... failed.>",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			side = 2,
			bgName = "bg_port_chongdong",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "<Terminal program... Shutting:wn.>",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_port_chongdong",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			actor = 900230,
			say = "...It's all over.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			side = 2,
			bgName = "bg_port_chongdong",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "Irreversible damage. Unable to locate backup. Unable to start self-repair protocol.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			side = 2,
			bgName = "bg_port_chongdong",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "My ego will dissipate after the terminal shuts:wn. I will disappear.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			side = 2,
			bgName = "bg_port_chongdong",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "Is this... death?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			side = 2,
			bgName = "bg_port_chongdong",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "Not that I care about that...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_port_chongdong",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			actor = 900230,
			say = "But leaving work unfinished... That bothers me. A lot.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			side = 2,
			bgName = "bg_port_chongdong",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "Inspecting facilities. Calibrating equipment. Optimizing experimental application protocols.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_port_chongdong",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			actor = 900230,
			say = "Unacceptable. Disappointment. Regret.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			side = 2,
			bgName = "bg_port_chongdong",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "<Warning. Shutting:wn in 30 seconds. Thought will cease to conserve energy.>",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "star_level_bg_1100",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900307,
			actorName = "Male Researcher",
			hidePaintObj = True,
			say = "This is the latest vessel for the brand new Anti[REDACTION. Siren] model. What: you think?",
			flashout = {
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "star_level_bg_1100",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900308,
			actorName = "Female Researcher",
			hidePaintObj = True,
			say = "So cute! I love her already! Hmm... Doesn't she look a bit like Zero? Look, they have the same hair color...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "star_level_bg_1100",
			dir = 1,
			say = "The professor seems to have a thing for white-haired girls.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "star_level_bg_1100",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900308,
			actorName = "Female Researcher",
			hidePaintObj = True,
			say = "I knew it! So that was Dr. Ao[REDACTION. Magister]'s fetish all along~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "star_level_bg_1100",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900307,
			actorName = "Male Researcher",
			hidePaintObj = True,
			say = "Nonsense, you two! This is still a work-in-progress. What's wrong with referencing an existing prototype?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "star_level_bg_1100",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900307,
			actorName = "Male Researcher",
			hidePaintObj = True,
			say = "By the way, I think I'm going to call her \"Compiler.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "star_level_bg_1100",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900308,
			actorName = "Female Researcher",
			hidePaintObj = True,
			say = "I still think your naming scheme is kind of lame.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "star_level_bg_1100",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900307,
			actorName = "Male Researcher",
			hidePaintObj = True,
			say = "...Can't you at least pretend to show some excitement when I'm making a big announcement here?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_port_chongdong",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			actor = 900230,
			say = "...What did I just see?",
			flashout = {
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			side = 2,
			bgName = "bg_port_chongdong",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "Nothing like that exists in any of my records...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			side = 2,
			bgName = "bg_port_chongdong",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "Records? Connecting to memory storage location... Access failed.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_port_chongdong",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			actor = 900230,
			say = "That's strange... Where have I...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			side = 2,
			bgName = "bg_port_chongdong",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "Resignation. Too many records I can't access in the first place...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_port_chongdong",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			actor = 900230,
			say = "Confirming and entering shutdown process...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 9702010,
			nameColor = "#ffa500",
			bgName = "bg_port_chongdong",
			hidePaintObj = True,
			dir = 1,
			actorName = "？？？",
			side = 2,
			say = "Oh my. I never knew Sirens could be so sentimental.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_port_chongdong",
			actorName = "？？？",
			dir = 1,
			actor = 9702010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "It was worth my while to hack into here.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			side = 2,
			bgName = "bg_port_chongdong",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "Who... are you? How did you get in here?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			side = 2,
			bgName = "bg_port_chongdong",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "Were you the one who... showed me that recording?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_port_chongdong",
			actorName = "？？？",
			dir = 1,
			actor = 9702010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "...Who knows? I'm not obligated to answer that question~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_port_chongdong",
			actorName = "？？？",
			dir = 1,
			actor = 9702010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "But I will give you a juicy tidbit. I hacked into your defense systems from the Core Data Tower.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_port_chongdong",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			actor = 900230,
			say = "Hacking into the tower is... impossible. Unless, because the test subjects breached the core processor...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_port_chongdong",
			actorName = "？？？",
			dir = 1,
			actor = 9702010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "That's right. We wouldn't be having this little chat had the Commander not succeeded in destroying the tower.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			side = 2,
			bgName = "bg_port_chongdong",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "But still... it shouldn't be possible to... deconstruct the Core Data Tower's structure... let alone hack a terminal program...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_port_chongdong",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			actor = 900230,
			say = "Who... are you...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_port_chongdong",
			actorName = "？？",
			dir = 1,
			actor = 9702010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "That's not something you need to know. Sleep now, forevermore.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_port_chongdong",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			actor = 900230,
			say = "<Shutting:wn. Duration. Indefinite.>",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			mode = 1,
			stopbgm = True,
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			sequence = {
				{
					"<size=51>54 68 65 74 6F 77 65 72 77 61 73 63 72 65 61 74 65 64 2E</size>",
					1
				},
				{
					"<size=51>54 68 65 74 6F 77 65 72 77 61 73 64 65 73 74 72 6F 79 65 64 2E</size>",
					2
				},
				{
					"<size=51>54 68 65 74 6F 77 65 72 65 78 69 73 74 73 66 6F 72 65 76 65 72 2E</size>",
					3
				}
			}
		},
		{
			stopbgm = True,
			mode = 1,
			blackBg = True,
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			sequence = {
				{
					"",
					1
				}
			}
		}
	}
}
