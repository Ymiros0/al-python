return {
	fadeOut = 1.5,
	mode = 2,
	id = "HONGCAIDEZHONGMUQU34",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			say = "Capital of Sardegna",
			side = 2,
			bgName = "bg_midgard_9",
			dir = 1,
			bgmDelay = 1,
			bgm = "story-italy",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 699010,
			say = "Bwahaha... Ahahahaha~♪",
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 699010,
			say = "Get a load of this... The Iron Blood declared war against the Sirens! Bwahaha!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 699010,
			say = "And now Friedrich der Große stepped:wn and Bismarck returned from out of nowhere?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 699010,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Ahh, this is too funny... Gotta calm:wn...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 699010,
			say = "I couldn't write a better script if I wanted to!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 699010,
			say = "I hear the cries of the world in turmoil... a world that cries out to be RULED!",
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 699010,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "And when I obtain the Crown of the Holy See, I shall: rule with the authority of God!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 699010,
			say = "Even the Sirens will bow:wn before Sardegna's majestic light!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 699010,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "At last, at long last!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_midgard_9",
			dir = 1,
			blackBg = True,
			actor = 699010,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "The era of Sardegna... Nay, of Marco Polo, has finally arrived!",
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
