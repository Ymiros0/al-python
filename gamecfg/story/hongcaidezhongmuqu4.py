return {
	fadeOut = 1.5,
	mode = 2,
	id = "HONGCAIDEZHONGMUQU4",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			say = "Scapa Flow Anchorage",
			side = 2,
			bgName = "bg_endingsong_3",
			dir = 1,
			bgmDelay = 1,
			bgm = "battle-againstfate",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 499020,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "It is a pleasure to see you again, my allies from the Sakura Empire.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Certainly, it feels like an eternity has passed since our banquet in Sardegna.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307010,
			say = "Though the curtains have yet to rise, I am quite impressed at this... stage that you've set.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "How were you able to take Scapa Flow without a fight?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 499020,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "A \"performance\" like this is best enjoyed by a select few, without the interruption of a boorish audience. Let's just say I employed a few little tricks.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307010,
			say = "...Are you not worried about the Azur Lane bringing:wn the entire theater?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307010,
			say = "I heard their staging area includes this area as well. It would be a pity if they barged in on your performance.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "You face the combined might of the Azur Lane, and also stand against the Commander.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307010,
			say = "The jamming can buy you some time, but they will see through those tricks soon enough.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 499020,
			say = "You're right... Indeed, my fleet alone is unable to defeat the Azur Lane.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 499020,
			say = "If I was to call upon the Sakura Empire's aid, would you join us...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307010,
			say = "Hehehe... I'm glad you acknowledge the capability of the Sakura Empire...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "But I must decline your offer. We did not come here to take part in your play.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 499020,
			say = "Of course. And at no point did I expect you to agree.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			say = "And just like you, I have no intention of participating in this play either.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 499020,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Incidentally, are you familiar with the incident that took place here at Scapa Flow?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "If I remember correctly, that was the Siren attack on Scapa Flow during the last great war, right?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "The Sirens ambushed the Iron Blood fleet, and the Azur Lane's leadership made the decision to not assist them...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "The Iron Blood fleet that fled into the harbor was completely annihilated... and that incident became one of the chief factors that led to the Iron Blood withdrawing from the Azur Lane.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 499020,
			say = "Yes, a fairly standard answer. That's what everyone has agreed on.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 499020,
			say = "Don't you think something:esn't quite add up though? Even though the Sirens were already the common enemy of humanity at that point, why would the Iron Blood main fleet flee into a Royal Navy harbor?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 499020,
			say = "In other words, if the Royal Navy took in the Iron Blood,: why is it that only the Iron Blood fleet got wiped out?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "..........",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307010,
			say = "I've heard another account of the events, but I'm not sure how reliable it is.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 499020,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Let's hear it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307010,
			say = "That there was actually no surprise attack – but rather, something went wrong when the Iron Blood was trying to capture Sirens for their own purposes.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "The Royal Navy was also caught in the crossfire, and Scapa Flow's facilities also suffered severe damage as a result.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "So you're saying, the Iron Blood invited the Sirens into the harbor...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Who knows what the truth really is? Maybe Friedrich has some insight. After all, we weren't invited here simply to chat, correct?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 499020,
			say = "It is said that history is written by the victors. But to write is to compose a narrative.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 499020,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "But we are not writers – we are warriors. What is a record in comparison to the scars of war?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 499020,
			say = "For us, the truth of this battle is––",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#ffff4d",
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			actorName = "???",
			say = "Enough of your senseless bloviating, Friedrich!",
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#ffff4d",
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			actorName = "???",
			say = "We could have hired a jester if we wanted to listen to someone talk in circles all day!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			say = "A small girl suddenly burst into the room in the middle of Friedrich's monologue.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307020,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Queen Elizabeth...?!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "That's right! You are in the presence of the Queen!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 499020,
			say = "What an impetuous child. Hehe.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Wait, this isn't making any sense to me... Why is the leader of the Royal Navy here?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307010,
			say = "My, it seems like you got your hands on much more than just an antiquated warship.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I see, so this is the secret behind the Iron Blood's little \"transaction.\" Control of Scapa Flow, in exchange for the little queen's safety.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307010,
			say = "Color me surprised by the extraordinary lengths to which you'd go, Friedrich. You'd even hold one of the major faction's leaders hostage.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "As if! If anything, it's the other way around! Friedrich is currently under the supervision of the Royal Navy!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Besides, if I was the hostage here, I wouldn't be moving around freely.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 499020,
			say = "Indeed... What kind of hostage would have the luxury to barge in during the kidnapper's speech?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Good grief, just how much: you love listening to yourself talk?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Belfast and the others are almost:ne with their preparation. The Singularity is getting hard to maintain, so I'll need to have them head over soon.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "You wish to know what really happened at Scapa Flow? Then I shall show you the truth!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 499020,
			say = "Very well. The Azur Lane is making progress faster than expected, so we must make haste.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 499020,
			say = "I look forward to hearing the piece you will play for us, little queen.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Hmph. And hear it you shall!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			say = "Queen Elizabeth pulled out a communicator and pressed the button, issuing her instructions to the recipients on the other end.",
			side = 2,
			bgName = "bg_endingsong_4",
			dir = 1,
			bgmDelay = 1,
			bgm = "theme-highseasfleet",
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_4",
			say = "In that moment, the still waters of Scapa Flow suddenly became turbulent and the surface of the sea began to cave in.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_4",
			say = "From the crevice rose an empty sun, a gateway pulsating with otherworldly energy.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 499020,
			say = "Allow me to welcome you once more, esteemed guests from the Sakura Empire.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_endingsong_4",
			dir = 1,
			blackBg = True,
			actor = 499020,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Harken to the coming of the final movement!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
