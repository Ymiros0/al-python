return {
	fadeOut = 1.5,
	mode = 2,
	id = "HONGCAIDEZHONGMUQU5",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			say = "Under the instructions of Queen Elizabeth, a new Singularity rose above the waters of Scapa Flow.",
			side = 2,
			bgName = "bg_endingsong_4",
			dir = 1,
			bgmDelay = 1,
			bgm = "theme-highseasfleet",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_4",
			say = "A steady stream of energy seeped from the ring of light surrounding the gate, evaporating the surrounding seawater into a fine mist.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_4",
			say = "In contrast to its intimidating form, a rainbow glistened within the shining mist, like a road stretching beyond the gateway.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307010,
			say = "My... This is rather unexpected...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307010,
			say = "Is this based on the Iron Blood's Singularity research? No... this is much more advanced than anything I've seen before...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Judging by the looks on the little queen's face, the Royal Navy seems to have played a part in this.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 499020,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I'll leave it to your imagination. What I will say is, none of this conflicts with my child's wishes.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I see... I'll also assume that you didn't put on this show just to ask us to close the Singularity.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 499020,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Correct. Beyond the Singularity lies the truth of what happened at Scapa Flow, as well as the answers you seek.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Wait, hold on, this is a bit much to take in all at once... What's the deal with this Singularity, and are you saying we have to go THROUGH it?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 499020,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "It is another branch, a world parallel to ours.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 499020,
			say = "But this gateway:es not lead directly to another world, but rather a Mirror Sea of sorts that projects the records of the Natural Calculation System.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307020,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "So if I'm understanding this correctly, it's different from the realm where you retrieved those META ships?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307020,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "The only information we have comes from the reports you've given us. How: we know that it's safe to pass through this Singularity?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307020,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "What proof can you offer that beyond this:or lies the things that you promised?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307010,
			say = "...I want to see the truth, Kaga.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307020,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Then I shall remain here until you return.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Suit yourself, Kaga.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 499020,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Ägir and August von Parseval will remain behind as well:. If the Azur Lane comes, you know what to:.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 499060,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Understood...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 499050,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Hehe. Don't underestimate me, Friedrich der Große. Just who: you think I am?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307010,
			say = "Jintsuu, contact Yamashiro's group. Provide them the instructions on how to get here.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "...Will you be coming with us as well, little queen?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Of course! And stop it with the little, and the queen! Just Elizabeth will:!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "We've already:ne some preliminary scouting, so follow my lead!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_4",
			say = "With those words, Queen Elizabeth was the first to disappear through the gate.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Man, I'm still not so sure about this...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307010,
			say = "It's fine. She may seem moody and fickle at times, but she is the leader of the Royal Navy for a reason.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "The fact that she went in first should be solid enough proof that it's safe.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 307010,
			say = "We can trust her for now. Zuikaku, await my instructions once you're inside.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_endingsong_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Got it, Akagi.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_4",
			say = "Akagi and Zuikaku disappeared into the Singularity as well, with Friedrich der Große following right behind them.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_endingsong_4",
			dir = 1,
			blackBg = True,
			actor = 499020,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "At long last, our rondo finally begins.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
