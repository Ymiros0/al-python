return {
	fadeOut = 1.5,
	mode = 2,
	id = "HUADENGLIUCAIZHENGJIAYIN1",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Myriad Colors of the New Year\n\n<size=45>Golden Dragon Among Auspicious Clouds</size>",
					1
				}
			}
		},
		{
			side = 2,
			bgName = "star_level_bg_102",
			dir = 1,
			bgmDelay = 2,
			bgm = "main-chunjie3",
			actor = 499052,
			nameColor = "#A9F548FF",
			live2d = "expedition",
			say = "Happy Lunar New Year, Commander.",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499052,
			side = 2,
			bgName = "star_level_bg_102",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Welcome to the abode of the golden dragon among auspicious clouds.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499052,
			side = 2,
			bgName = "star_level_bg_102",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I've heard bits and pieces of the Empery's dragon folklore. Apparently their dragons are very different to Iron Blood's.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499052,
			side = 2,
			bgName = "star_level_bg_102",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Incidentally, their dragons inspired the design of this outfit, too.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499052,
			side = 2,
			bgName = "star_level_bg_102",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "What: you think? Looks quite stunning,:esn't it?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499052,
			side = 2,
			bgName = "star_level_bg_102",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...What's with that look, huh? Are you suggesting that it:esn't fit the spirit of the holiday?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "\"Of course not. It:es.\"",
					flag = 1
				},
				{
					content = "\"I'd say it's a bit risqué.\"",
					flag = 2
				}
			}
		},
		{
			actor = 499052,
			side = 2,
			bgName = "star_level_bg_102",
			live2d = "headtouch",
			dir = 1,
			optionFlag = 1,
			nameColor = "#A9F548FF",
			say = "Heehee. That's what I thought. You have good taste.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499052,
			side = 2,
			bgName = "star_level_bg_102",
			live2d = True,
			dir = 1,
			optionFlag = 1,
			nameColor = "#A9F548FF",
			say = "But what matters most is that I'M happy with how it looks.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499052,
			side = 2,
			bgName = "star_level_bg_102",
			live2d = True,
			dir = 1,
			optionFlag = 2,
			nameColor = "#A9F548FF",
			say = "I:n't care for your critique! I chose this outfit very deliberately, I'll have you know.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499052,
			side = 2,
			bgName = "star_level_bg_102",
			live2d = "headtouch",
			dir = 1,
			optionFlag = 2,
			nameColor = "#A9F548FF",
			say = "So what if this isn't a holiday for celebrating virility? I know that, obviously!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			actor = 499052,
			side = 2,
			bgName = "star_level_bg_102",
			live2d = True,
			dir = 1,
			optionFlag = 2,
			nameColor = "#A9F548FF",
			say = "Hmph. Whatever. You simply have no taste in fashion.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499052,
			side = 2,
			bgName = "star_level_bg_102",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...I will say, I'm pleasantly surprised by how quiet it is here. Everywhere else seems to be swept up in merrymaking.",:
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499052,
			side = 2,
			bgName = "star_level_bg_102",
			live2d = "touch",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Seeing as you've mostly finished the festival preparations, why:n't you have a seat next to me?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499052,
			side = 2,
			bgName = "star_level_bg_102",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Even lords of the sea yearn for moments of respite.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499052,
			side = 2,
			bgName = "star_level_bg_102",
			live2d = True,
			withoutActorName = True,
			nameColor = "#A9F548FF",
			dir = 1,
			say = "I take her up on her suggestion and sit:wn next to her.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499052,
			side = 2,
			bgName = "star_level_bg_102",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Heehee. You're curious about the aroma, are you?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499052,
			side = 2,
			bgName = "star_level_bg_102",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It's from an incense I made. I learned how to just today.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499052,
			side = 2,
			bgName = "star_level_bg_102",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "That's not the only exciting skill I've picked up. In fact, it's one of many.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499052,
			side = 2,
			bgName = "star_level_bg_102",
			live2d = True,
			withoutActorName = True,
			nameColor = "#A9F548FF",
			dir = 1,
			say = "\"Such as...?\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499052,
			side = 2,
			bgName = "star_level_bg_102",
			live2d = "main1",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Oh, you know! Calligraphy! Writing with this fluffy brush is more satisfying than I expected.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499052,
			side = 2,
			bgName = "star_level_bg_102",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "My only issue is... I'm unsure how to hold the brush. Is it like this? Or like... this?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "\"I'll show you.\"",
					flag = 1
				},
				{
					content = "Simply observe her.",
					flag = 2
				}
			}
		},
		{
			actor = 499052,
			side = 2,
			bgName = "star_level_bg_102",
			live2d = True,
			dir = 1,
			optionFlag = 1,
			nameColor = "#A9F548FF",
			say = "Hah! Of course YOU would know the proper technique.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499052,
			side = 2,
			bgName = "star_level_bg_102",
			live2d = True,
			dir = 1,
			optionFlag = 1,
			nameColor = "#A9F548FF",
			say = "Then I shall give you a demonstration of what I've learned, and I permit you to verbally correct me.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499052,
			side = 2,
			bgName = "star_level_bg_102",
			live2d = True,
			dir = 1,
			optionFlag = 2,
			nameColor = "#A9F548FF",
			say = "Bah, I can't seem to get it right...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499052,
			side = 2,
			bgName = "star_level_bg_102",
			live2d = True,
			dir = 1,
			optionFlag = 2,
			nameColor = "#A9F548FF",
			say = "I refuse to believe you:n't know the proper technique! Stop gawking and tell me!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499052,
			side = 2,
			bgName = "star_level_bg_102",
			live2d = True,
			withoutActorName = True,
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Ägir haplessly runs the brush over the paper. I can feel something bad about to happen...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499052,
			side = 2,
			bgName = "star_level_bg_102",
			live2d = "touch2",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Aahh! Dammit, I got ink on myself!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			actor = 499052,
			side = 2,
			bgName = "star_level_bg_102",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...Do I need help? No, I can: it on my own, thank you very much!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499052,
			side = 2,
			bgName = "star_level_bg_102",
			live2d = True,
			withoutActorName = True,
			nameColor = "#A9F548FF",
			dir = 1,
			say = "A bit of floundering later...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499052,
			side = 2,
			bgName = "star_level_bg_102",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Ugh... This is needlessly complicated...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499052,
			side = 2,
			bgName = "star_level_bg_102",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Don't you grin at me! I've figured it out now!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499052,
			side = 2,
			bgName = "star_level_bg_102",
			live2d = "touch",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...There! Finally.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499052,
			side = 2,
			bgName = "star_level_bg_102",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Forget about the new skills thing! We'll simply lie back, relax, and enjoy the quiet!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
