return {
	fadeOut = 1.5,
	mode = 2,
	id = "HUADENGLIUCAIZHENGJIAYIN4",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Myriad Colors of the New Year\n\n<size=45>The Gate Dragon's Advent</size>",
					1
				}
			}
		},
		{
			side = 2,
			bgName = "star_level_bg_127",
			dir = 1,
			bgmDelay = 2,
			bgm = "main-chunjie3",
			actor = 499071,
			nameColor = "#A9F548FF",
			live2d = "main2",
			say = "Ugh... Where has that idiot gone? Just as the festival enters its busiest hour, too...",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499071,
			side = 2,
			bgName = "star_level_bg_127",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Oh, there you are! Over here, Commander!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499071,
			side = 2,
			bgName = "star_level_bg_127",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Heh! Here, take this!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499071,
			side = 2,
			bgName = "star_level_bg_127",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...What is it? A red envelope, duh. I think that's what they call it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499071,
			side = 2,
			bgName = "star_level_bg_127",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "While there's no money inside, it's still a gift from me, so you owe me your loyalty all the way into the next year!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "\"Is that really how the tradition works?\"",
					flag = 1
				}
			}
		},
		{
			actor = 499071,
			side = 2,
			bgName = "star_level_bg_127",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Obviously! That's why they give them to their subordinates.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499071,
			side = 2,
			bgName = "star_level_bg_127",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "And you're below me, so I expect you to humbly accept it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "Stare in silence.",
					flag = 1
				},
				{
					content = "\"Well, here's a gift from me – a small treat.\"",
					flag = 2
				}
			}
		},
		{
			actor = 499071,
			side = 2,
			bgName = "star_level_bg_127",
			live2d = "home",
			dir = 1,
			optionFlag = 1,
			nameColor = "#A9F548FF",
			say = "Hmph. I'll take your silence to mean, \"Of course, Milady.\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499071,
			side = 2,
			bgName = "star_level_bg_127",
			live2d = True,
			dir = 1,
			optionFlag = 1,
			nameColor = "#A9F548FF",
			say = "Perfect! Now give me a full tour of the port's Spring Festival makeover! It's your duty, you know♪",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499071,
			side = 2,
			bgName = "star_level_bg_127",
			live2d = "mission_complete",
			dir = 1,
			optionFlag = 2,
			nameColor = "#A9F548FF",
			say = "Hm? What's that?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			actor = 499071,
			side = 2,
			bgName = "star_level_bg_127",
			live2d = True,
			dir = 1,
			optionFlag = 2,
			nameColor = "#A9F548FF",
			say = "Nom... It's a \"tanghulu\"? It's sour yet sweet, and surprisingly tasty.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499071,
			side = 2,
			bgName = "star_level_bg_127",
			live2d = True,
			dir = 1,
			optionFlag = 2,
			nameColor = "#A9F548FF",
			say = "Not bad. In return, you get the exclusive privilege of holding my hand while giving me a tour of all the festivities!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499071,
			side = 2,
			bgName = "star_level_bg_127",
			live2d = True,
			withoutActorName = True,
			nameColor = "#A9F548FF",
			dir = 1,
			say = "\"Why: you need a guide? You weren't lost up until a moment ago, were you?\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499071,
			side = 2,
			bgName = "star_level_bg_127",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Huh? Nuh-uh! I was the polar OPPOSITE of lost!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			actor = 499071,
			side = 2,
			bgName = "star_level_bg_127",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "O-obviously! Now quit wasting time and lead the way!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499071,
			side = 2,
			bgName = "star_level_bg_127",
			live2d = "expedition",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Huh. The Empery sure knows how to throw a bustling festival.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 499071,
			side = 2,
			bgName = "star_level_bg_127",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "But why: they call it the Spring Festival? It's still the middle of winter.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499071,
			side = 2,
			bgName = "star_level_bg_127",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Meh, not like it matters much. Let's go see that dancing flying lion Ägir told me about.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499071,
			side = 2,
			bgName = "star_level_bg_127",
			live2d = True,
			withoutActorName = True,
			nameColor = "#A9F548FF",
			dir = 1,
			say = "\"What dancing flying lion?\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499071,
			side = 2,
			bgName = "star_level_bg_127",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "What, you seriously:n't know?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499071,
			side = 2,
			bgName = "star_level_bg_127",
			live2d = "main1",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I'm talking about that beast that flies from place to place and scares the daylights out of little kids.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499071,
			side = 2,
			bgName = "star_level_bg_127",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...It has a name? Okay, well...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499071,
			side = 2,
			bgName = "star_level_bg_127",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "So what if it's called a nian or whatever? If I SAY it's a lion: it's a LION!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499071,
			side = 2,
			bgName = "star_level_bg_127",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I will hear no more arguing from that big mouth of yours! Let's go see it!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499071,
			side = 2,
			bgName = "star_level_bg_127",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "This time, I'll lead the way! And you'll follow for the rest of the year, like the loyal servant you are♪",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
