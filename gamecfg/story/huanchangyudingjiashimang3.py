return {
	id = "HUANCHANGYUDINGJIASHIMANG3",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			say = "Huan Ch'ang's Place - Living Room",
			bgm = "china-party",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 504010,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Have a seat.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			side = 2,
			actor = 504010,
			say = "My divinations told me unexpected visitors were coming, but I sure didn't think it would you and an entourage of Empery girls, Commander.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 504010,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "What brings you here? Do you need my help?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501070,
			say = "So you knew we were coming! That's our bagua fortune-teller for you!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 9,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501070,
			say = "And yeah, that's exactly why we're here!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 12,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501060,
			say = "See, we're actually in a real pickle here...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501060,
			say = "We invited another faction to dine with us tonight, but Lung Wu is away and I'm the only one who can cook the food.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 9,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501060,
			say = "And I'm not even half the chef she is!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 14,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501060,
			say = "But! I can make up for it if I can just get some good ingredients!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 11,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501060,
			say = "Which is what we need help with! Huan Ch'ang, can you please divine where to find ingredients that'll make delicious food?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 504010,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "In order to: that, I need to know the cause and consequences of the events that led you to this point.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			side = 2,
			actor = 504010,
			say = "You'll need to give me more details.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			side = 2,
			actor = 504010,
			say = "For a start, why did you suddenly invite them to dinner if Lu Wung wasn't there?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 13,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501070,
			say = "...It was because I ran my mouth and made a promise I couldn't keep. Sorry.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 10,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501070,
			say = "Please:n't be mad at Lady Yuen...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 14,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501060,
			say = "L-look, what's:ne is:ne!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 17,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501060,
			say = "We can't let:wn our guests, or we'll sully both Lung Wu's name and the rep of our cuisine!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 504010,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Chi An, are they telling the truth?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 16,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501070,
			say = "Um... Umm...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 502090,
			say = "It's okay, little ones. I know you:n't want to be rude, but you:n't need to walk on eggshells for my sake.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 502090,
			say = "In truth, this was all my fault. I was in the kitchen, and–",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 504010,
			say = "You insisted on cooking, caused an accident, and just by pure bad luck, the Sardegnians happened to be visiting at the same time. Is that right?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 502090,
			say = "Huh? Wait, did you divine all that?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 502090,
			say = "No, it:esn't matter. Yes, you're exactly right. I caused an explosion in the kitchen.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			side = 2,
			actor = 504010,
			say = "......",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 502090,
			say = "It's fine. You can laugh at me.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			side = 2,
			actor = 504010,
			say = "I can?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 502090,
			say = "Yes.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			side = 2,
			actor = 504010,
			say = "......",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 504010,
			say = "Heh... Heheh...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 2,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			side = 2,
			actor = 504010,
			say = "You actually blew up the kitchen. Wow. I knew you were a bad cook, but this is something else.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 502090,
			say = "Hmph. It's not THAT funny.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 8,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 502090,
			say = "Just because I'm from the Empery:esn't mean I was born with great cooking skills.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 502090,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "That's a stereotype, you know.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 502090,
			say = "What I lack in talent, I'm trying to make up for with hard work and practice. I'll get good at it eventually.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 504010,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Of course. Keep at it. Now, how: you want me to help?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 502090,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It's simple. We'd like you to divine where to find the ingredients Hu Pen needs.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			side = 2,
			actor = 504010,
			say = "Just to be sure – you're not going to \"help\" in the kitchen, are you?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 502090,
			say = "Heehee, no. I know my limits.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			side = 2,
			actor = 504010,
			say = "One last thing,:. Commander, as the one who knocked on my:or, what is your opinion on this?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 504010,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "The heavens discern not between matters great and small, good or evil.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			side = 2,
			actor = 504010,
			say = "I'd like to hear a reason for me to call upon the bagua.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(Is she asking me to give the final push?)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(Alright:...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "We already have the right time and right place. What we need is the right person.",
					flag = 1
				},
				{
					content = "We'll never know what the heavens decide if we:n't at least try.",
					flag = 2
				},
				{
					content = "I need your help, my lovely Huan Ch'ang!",
					flag = 3
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			optionFlag = 1,
			actor = 504010,
			say = "Such sophistry...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			optionFlag = 1,
			actor = 504010,
			say = "But, given the current situation, I suppose it's not beyond the realm of possibility...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			optionFlag = 1,
			actor = 504010,
			say = "If you insist...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			optionFlag = 2,
			actor = 504010,
			say = "So you're leaving it to me?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 504010,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			optionFlag = 2,
			say = "But, given the current situation, I suppose it's not beyond the realm of possibility...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			optionFlag = 2,
			actor = 504010,
			say = "If you insist...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 9,
			side = 0,
			bgName = "star_level_bg_157",
			actorName = "Hu Pen & Fei Yuen",
			dir = 1,
			optionFlag = 3,
			actor = 501070,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "...Wow.",
			subActors = {
				{
					expression = 10,
					actor = 501060,
					hidePaintObj = True,
					pos = {
						x = 1125
					}
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			optionFlag = 3,
			actor = 504010,
			say = "Hey!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			optionFlag = 3,
			actor = 504010,
			say = "Wh-what's with you? You:n't say that in front of people...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			optionFlag = 3,
			actor = 504010,
			say = "Ahem... Alright... I'll start divining now...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 0,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 504010,
			say = "......",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 5,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			side = 2,
			actor = 504010,
			say = "Of paths fiftyfold, only forty-nine may be divined. The hidden truth is...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			side = 2,
			actor = 504010,
			say = "...I've got it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			side = 2,
			actor = 504010,
			say = "You can catch good fish at the port's freshwater lake.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			say = "For some reason, she shoots me a glance when she says that.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(Hm? Is she talking about the same freshwater lake where we went fishing the other day?)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(Does she want to fish some more?)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 11,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501060,
			say = "Really?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501060,
			say = "Hooray! Let's get going,:!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 1,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			side = 2,
			actor = 504010,
			say = "I'll come with you.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 504010,
			side = 2,
			bgName = "star_level_bg_157",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Since the bagua pointed us there, now I'm curious to see what ingredients we'll get.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
