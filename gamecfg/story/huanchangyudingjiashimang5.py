return {
	id = "HUANCHANGYUDINGJIASHIMANG5",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			say = "Port - Dragon Empery Dorm's Kitchen",
			bgm = "main-chunjie5-1",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			say = "Lung Wu heard about the incident at the:rm and hurried back to the port. By the time she was back, the sky had grown dark.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			say = "Meanwhile, the Empery's kitchen is operating at full speed in preparation for the dinner.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501050,
			say = "...What's going on?! Someone explain!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501060,
			say = "Lung Wu! You're back!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501060,
			say = "We're cooking up a feast for our guests, that's what!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501060,
			say = "Ah, the fish soup is:ne! Come here and give it a taste!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501050,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Uh... Thanks?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 13,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501050,
			say = "No, forget about the soup! Why are you cooking up a feast?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501050,
			say = "And who are the guests?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = 502090,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			actorName = "Chi An",
			nameColor = "#A9F548FF",
			say = "Hey,:es anyone know what I'm supposed to: with this?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 10,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501060,
			say = "Whoa, whoa! You need to dish that up in a very specific way!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501060,
			say = "Leave it be! I'll show you in a second!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 16,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501050,
			say = "What's Chi An:ing in the kitchen?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501060,
			say = "Don't worry, she's forbidden from touching the pans! Just eat some soup and relax! I'll explain everything later!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 14,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501050,
			say = "Not so fast! You need to tell me what's–",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 17,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501050,
			say = "Aaand she's gone...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "You:n't need to worry about them.",
					flag = 1
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501050,
			say = "Oh, Commander. Hello.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501050,
			say = "Wait, why are YOU in the kitchen? Let me guess...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "No, I'm not cooking anything today.",
					flag = 1
				},
				{
					content = "They did all the work, not me.",
					flag = 2
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_128",
			dir = 1,
			optionFlag = 1,
			actor = 501050,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "You're telling me THEY cooked all this?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 10,
			side = 2,
			bgName = "star_level_bg_128",
			dir = 1,
			optionFlag = 1,
			actor = 501050,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Not bad. Not bad at all.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_128",
			dir = 1,
			optionFlag = 2,
			actor = 501050,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Really? Wow. I'm impressed.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501050,
			say = "So, who are the guests? And who invited them?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Does it really matter?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 13,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501050,
			say = "Yes, it:es!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 10,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501050,
			say = "That said... I can't deny that this soup is pretty good.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501050,
			say = "Maybe I should trust them a little more...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501050,
			say = "This is a trial, and it's theirs alone to overcome. I'll let–",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = 501060,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			actorName = "Hu Pen",
			nameColor = "#A9F548FF",
			say = "No, no, no! You're:ing it all wrong, Chi An!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = 502090,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			actorName = "Chi An",
			nameColor = "#A9F548FF",
			say = "What? I am? Okay... How about this?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = 501060,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			actorName = "Hu Pen",
			nameColor = "#A9F548FF",
			say = "Nope! Still wrong!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 16,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501050,
			say = "...I will NOT let Chi An cook!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 14,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501050,
			say = "She should not be allowed inside the kitchen!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 17,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501050,
			say = "*sigh*... And here I hoped they could handle it on their own, at least this once...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			say = "Lung Wu covers her face with her hands and mutters despondently. Despite this, I can see a smile on her face.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "You want to: it yourself?",
					flag = 1
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501050,
			say = "Yes. Even if it means leaving out their help!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501050,
			say = "Also, I'll make you one special dish as thanks for helping them as much as you did.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501050,
			say = "Any theme you want. Just name it, and I'll try to create it!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "How about a dish based on happiness?",
					flag = 1
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501050,
			say = "...You really know how to pick a theme.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 10,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501050,
			say = "Then I'll make an additional dish as a bonus, based on the idea of a family reunion.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501050,
			say = "Now, those dishes aren't going to make themselves! I'd better get started!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			say = "Lung Wu:ns an apron and plunges into the busy kitchen.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			say = "With her at the helm, this dinner will no:ubt go without a hitch.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_128",
			hidePaintObj = True,
			say = "A happy ending for everyone.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			hidePaintObj = True,
			blackBg = True,
			say = "The Sardegnians and everyone else they invited all relished in the feast the Empery cooked up for them.",:
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			hidePaintObj = True,
			blackBg = True,
			say = "But, as a result, the stereotype that everyone from the Empery is great at cooking solidified even further.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = 502090,
			side = 2,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			dir = 1,
			blackBg = True,
			actorName = "Chi An",
			say = "It shouldn't be that way!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			hidePaintObj = True,
			blackBg = True,
			say = "The End",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
