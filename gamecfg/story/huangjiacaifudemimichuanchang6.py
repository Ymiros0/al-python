return {
	fadeOut = 1.5,
	mode = 2,
	id = "HUANGJIACAIFUDEMIMICHUANCHANG6",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Royal Fortune's Secret Shipyard\n\n<size=45>6 Cannons Are Might</size>",
					1
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_battleshipyard_5",
			hidePaintObj = True,
			say = "Royal Fortune's Secret Shipyard - Gun deck finished",
			bgm = "theme-seaandsun-image",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9600010,
			side = 2,
			bgName = "bg_battleshipyard_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "At last, we've reached this exciting moment!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 9600010,
			side = 2,
			bgName = "bg_battleshipyard_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Cannons! The symbols of a sailing frigate's might!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 9600010,
			side = 2,
			bgName = "bg_battleshipyard_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "The section where the cannons are installed – called the \"gun deck\" – comes in two different arrangements.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9600010,
			side = 2,
			bgName = "bg_battleshipyard_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It's easy to tell them apart. It's about whether their cannons are arranged continuously along the sides, or if they're spaced out.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9600010,
			side = 2,
			bgName = "bg_battleshipyard_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "The number of cannons also determines the ship's rate – its ranking, essentially.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9600010,
			side = 2,
			bgName = "bg_battleshipyard_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "For example, compared to a fourth-rate ship, a first-rate ship typically has two more gun decks with continuous cannon arrangements.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9600010,
			side = 2,
			bgName = "bg_battleshipyard_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "A fourth-rate ship, meanwhile, has around 50 cannons less than a first-rate.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9600010,
			side = 2,
			bgName = "bg_battleshipyard_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "A nation commanding even one first-rate ship is proof of its navy's might!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_battleshipyard_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 9600010,
			say = "Although, first-rates:n't mean much to pirates, of course. Hahah!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_battleshipyard_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 9600010,
			say = "On the open sea, speed is everything. Having way too many cannons will only slow you:wn.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_battleshipyard_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 9600010,
			say = "And that is why any respectable fleet needs smaller ships as well.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9600010,
			side = 2,
			bgName = "bg_battleshipyard_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "A wise admiral builds their fleet according to the conditions and who they're fighting against.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9600010,
			side = 2,
			bgName = "bg_battleshipyard_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "That's a fact that still holds up after many centuries.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
