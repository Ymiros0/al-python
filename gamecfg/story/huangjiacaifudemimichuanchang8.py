return {
	fadeOut = 1.5,
	mode = 2,
	id = "HUANGJIACAIFUDEMIMICHUANCHANG8",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Royal Fortune's Secret Shipyard\n\n<size=45>8 Hoist Sails!</size>",
					1
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_battleshipyard_7",
			hidePaintObj = True,
			say = "Royal Fortune's Secret Shipyard - Sails rigging finished",
			bgm = "theme-seaandsun-image",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_battleshipyard_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 9600010,
			say = "Everything's in order to set off, but there's one last thing I wanna tell you about.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9600010,
			side = 2,
			bgName = "bg_battleshipyard_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "You guessed it – it's about a ship's sails.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = 9600010,
			side = 2,
			bgName = "bg_battleshipyard_7",
			say = "From bow to stern, you have the jib, the foresail, the mainsail, and lastly, the mizzensail.",
			dir = 1,
			actor = 9600010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			withoutPainting = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = 9600010,
			side = 2,
			bgName = "bg_battleshipyard_7",
			say = "When all these sails are raised at the same time, it's known as a \"full sail.\"",
			dir = 1,
			actor = 9600010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			withoutPainting = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = 9600010,
			side = 2,
			bgName = "bg_battleshipyard_7",
			say = "And that is when the ship can cruise at its top speed.",
			dir = 1,
			actor = 9600010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			withoutPainting = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = 9600010,
			side = 2,
			bgName = "bg_battleshipyard_7",
			say = "But, ships usually lower some of their sails to adapt to the sea's often turbulent whims, a technique known as \"reefing.\"",
			dir = 1,
			actor = 9600010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			withoutPainting = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = 9600010,
			side = 2,
			bgName = "bg_battleshipyard_7",
			say = "When the wind is weak, they raise sails to catch more of it, and when the wind is too strong, they: the opposite.",
			dir = 1,
			actor = 9600010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			withoutPainting = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = 9600010,
			side = 2,
			bgName = "bg_battleshipyard_7",
			say = "Doing this allows a ship to sail at a comparatively stable speed.",
			dir = 1,
			actor = 9600010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			withoutPainting = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9600010,
			side = 2,
			bgName = "bg_battleshipyard_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Knowing what to: in a storm is also required knowledge in order to sail.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_battleshipyard_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 9600010,
			say = "When a storm blows, you raise only your topsail and keep all other sails lowered.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9600010,
			side = 2,
			bgName = "bg_battleshipyard_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "That way, your bow will stay raised, and you won't go off-course 'cause of any strong winds.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_battleshipyard_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 9600010,
			say = "You could argue that mastering the art of raising and lowering your sails is THE most important part of sailing.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9600010,
			side = 2,
			bgName = "bg_battleshipyard_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Then again, I reckon you folks already know that, no?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9600010,
			side = 2,
			bgName = "bg_battleshipyard_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Now... Hoist sails and weigh anchor!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_battleshipyard_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 9600010,
			say = "Let our great journey begin!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		}
	}
}
