return {
	id = "JIARIXINTIAODAYOULUN20",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			say = "I've come to the ammo storage, as the schedule dictates.",
			bgm = "story-richang-11",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 202081,
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "As the most powerful naval organization in history, our each and every battle is equally important.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			say = "The lights switch on with a click, and Sheffield steps forth from between the shadows and the light.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 202081,
			say = "Even a minor mistake can prove fatal. Therefore, we must always be alert and aware of everything on the battlefield.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			say = "Next, Sheffield and Nevada also come forth.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			say = "That makes it one maid-slash-undercover-agent, one cowgirl, and one plainclothed hacker.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			say = "Of the three, only Nevada and Sheffield are carrying weapons. This motley crew really:esn't really scream, \"powerful naval organization.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 102161,
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Ahem. I know what you're thinking. You shouldn't judge a book by its cover.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105019,
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Hahah! Damn right. Just 'cause I'm dressed for the Wild West:esn't mean I can't fight!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 202081,
			say = "Master, I hope you aren't harbouring:ubts about our ability as soldiers.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "Of course not.",
					flag = 1
				},
				{
					content = "Maybe I:?",
					flag = 2
				}
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			optionFlag = 1,
			say = "You've more than proven your ability just by undertaking this mission.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			optionFlag = 2,
			say = "But I'll put them on hold. You'll demonstrate your ability to me another time.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "By the way,: you have any gear I can use? Relying solely on others for protection isn't ideal.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(I:n't know about Memphis and Nevada, but Sheffield really likes these drills. Let's see if she'll indulge me.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 202081,
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I: not. However, if you would like to have some, you are free to try to steal it from the enemy.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105019,
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Good idea! Now you got me fired up.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			say = "While we've been talking, Memphis has been tapping away at the tablet in her hands.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 102161,
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "The signal has been isolated. Area ahead, all clear.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 202081,
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Unit 141, commencing escort mission. Master, we will now pass through an active fire zone. Please:n't stray too far from us.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "Got it. Or should I say, roger, pardner!",
					flag = 1
				}
			}
		},
		{
			actor = 105019,
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Hah! You're gettin' it, young'un!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			say = "It was smooth sailing to the next safehouse after Memphis blocked the signal from Soyuz. Or so I thought...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 718011,
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "You are surrounded! Lay:wn your arms and surrender!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 202081,
			say = "Tsk. They've already sniffed us out...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			bgName = "bg_niceship_cg1",
			mode = 1,
			sequence = {
				{
					"",
					0
				}
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			side = 2,
			actorName = "Nevada",
			bgName = "bg_niceship_cg1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Memphis, can't you: somethin' with them fancy-schmancy hacking skills?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Memphis",
			bgName = "bg_niceship_cg1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "They:n't have any equipment hooked up to the 'net. There's nothing TO hack.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Memphis",
			bgName = "bg_niceship_cg1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "All I can: is shut:wn their comms channel. That'll at least slow them:wn.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_niceship_cg1",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(For Kronshtadt to: that, it can only mean one of two things...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_niceship_cg1",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(One – they're bluffing and waiting for us to make a move before they launch an all-out attack.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_niceship_cg1",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(Or two – they really: have us surrounded and they want to arrest all of us in one one fell swoop.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_niceship_cg1",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(Whatever the case, I know one thing for certain – they still:n't know exactly where we are.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_niceship_cg1",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(If we can exploit that and find out where exactly THEY are, we can turn the tables on them with an ambush...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_niceship_cg1",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Memphis, can you seize control of the ammo depot's security cameras?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Memphis",
			bgName = "bg_niceship_cg1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I can, but TB will catch on to me in no time. At most we'll get 30 seconds.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_niceship_cg1",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "That's all I need!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			say = "Using the cameras, I manage to pinpoint Kronshtadt's location. I give Sheffield and Nevada the signal to move.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 202081,
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Understood.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105019,
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Comin' right up!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			say = "Nevada provides covering fire while Sheffield runs faster than the eye can see, getting right behind Kronshtadt.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Now! Do it!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			say = "CLACK CLACK!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			say = "I hear the distinct sound of heels hitting the ground amidst the gunfire.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 705021,
			say = "Didn't you hear, Comrade Commander? You're surrounded.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "......",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "(Pull out your secret weapon.)",
					flag = 1
				}
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(It's time for my little friend!)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			say = "I take out the stuffed Arctic hare I bought before the drill and throw it at Soyuz.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 705021,
			say = "Oh!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 705021,
			say = "Oh my god! It's so adorable!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 202081,
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "......",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 102161,
			say = "THAT'S what you meant by \"secret weapon\"?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105019,
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Well, butter my butt and call me a biscuit...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 202081,
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "If it works, it works. Let's get out of here while we have the chance.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			say = "And so, the security drill ended with a surprising twist.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			say = "While in truth it didn't have much to: with security, everyone enjoyed it. That's what matters to me.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			say = "Time for a short break,: I'll go somewhere else on the ship.",:
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
