return {
	id = "JIARIXINTIAODAYOULUN21",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_135",
			hidePaintObj = True,
			say = "It's time for the masquerade ball. I've come to the nightclub, wearing my usual attire.",
			bgm = "story-niceship-soft",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_135",
			hidePaintObj = True,
			say = "To make the place look the part for a ball, they've adjusted the layout, replaced the ornaments, and even rolled out a red carpet.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_135",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 900437,
			say = "Good evening, Master.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_135",
			hidePaintObj = True,
			say = "Scylla lifts the rims of her evening gown and gives me a gracious bow.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_135",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 900437,
			say = "You chose an... interesting outfit. I can see the thought behind it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_135",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 900437,
			say = "Anyway, allow me to show you inside.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			say = "Shortly after we step inside, Hermione calls for Scylla's help, so she has to leave my side.",
			bgm = "story-oldcastle-carnival",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			say = "I decide to hide away in a corner so I can get a good look at everyone's costumes.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(Since we didn't decide on a theme, some are wearing actual costumes while others are wearing more casual party dresses.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			say = "After a while, my phone suddenly rings. It's Kursk.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_156",
			paintingNoise = True,
			dir = 1,
			actor = 900440,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Commander, are you at the ball?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Yeah. Since you're asking about it, I'm assuming you're here, too.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_156",
			paintingNoise = True,
			dir = 1,
			actor = 900440,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "I'm not. Although, I am wearing an Empery dress in the spirit of the event.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_156",
			paintingNoise = True,
			dir = 1,
			actor = 900440,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "I can't stand crowded parties, simple as that. Besides, an experienced hunter prefers to create their own environment so they can approach their prey sneakily.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_156",
			paintingNoise = True,
			dir = 1,
			actor = 900440,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "I'm sure you know what I mean. Bye now.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			say = "She hangs up before I can get another word in.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(Sounds like she's telling me to come find her later.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			say = "Just as I fall deep in thought, I suddenly hear a familiar laugh.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9600061,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "\"Ahahaha! I've finally found you, Master! I've been waiting so, sooo long for you to return...\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 9600061,
			say = "Well, what: you think? My costume looks pretty smart,:esn't it?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			say = "Adventure Galley comes and speaks to me, casually placing her giant cleaver on the ground and smiling.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9600031,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "There you are, my darling! I've been looking aaall over for you~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			say = "Not long after, many more Tempestas gather – Golden Hind, São Martinho, and even Royal Fortune.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 9600011,
			say = "Sooo... why are you wearing your regular uniform to a masquerade?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 9600021,
			say = "It's almost like you aren't wearing a costume at all.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 9600031,
			say = "I beg to differ. Wearing your regular outfit is like dressing up as yourself. That's what the Commander is:ing~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9600011,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "So it's, like, the same, but different? You're saying the Commander IS wearing a costume?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 9600031,
			say = "Exactly. And we'll have to get real close to get a good look at it~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9600041,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			side = 2,
			actorName = "Mary Celeste",
			say = "Don't mind if I:.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			say = "With all the surprise of the Grim Reaper, Mary Celeste appears and puts her prop scythe behind my neck.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			say = "She: pulls it toward her, dragging me closer.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 9600031,
			say = "Oh my! Mary made the first move... Well, now it's MY turn to get closer~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 9600031,
			say = "Oh yes, I'm going to wrap you up like a gift and make you squishy with love~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			say = "I had a dance with all the members of Tempesta, save for Whydah who stuck to a corner of the club and played video games.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(Phew... All that dancing was exhausting. I need a break.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			say = "But before I can even find a place to sit:wn, something else happens.",:
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900438,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Hoho! This is where we demonstrate our powers! Old man, let them behold! \"This spectacle will leave them speechless!\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			say = "A small whirlwind whips up around Natori, lifting her feet off the ground.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 901061,
			say = "The wine glasses, they're floating away! Oh no... One fell and shattered... I messed up yet again...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(Oh boy... I need to stop Natori before this gets any worse.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			say = "I manage to bring Natori:wn to earth again. Edinburgh and Hermione: arrive to clean up the fragments on the floor and rebuild the wine glass tower.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			say = "While they: that, I join Épée in her little hidey-hole and sit:wn next to her.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 901061,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It's all my fault, Commander... I can't: anything right and I only ruin things for people... *sob*...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "It's not your fault at all. You did all you could to save the glasses. Only one broke, and many more would have if not for you.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 901061,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Y-you mean it? It's not my fault? You really think I did all I could?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Absolutely. You're a good girl who's always trying her best.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 901061,
			say = "Th-thank you... I'll keep trying my best so I can always be with you!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			say = "I comfort the sobbing Épée, putting a wrap on everything that commotion caused.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(I feel rested now. Time to get up and check out other people's costumes.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 406032,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Heehee. Not wearing a costume to a costume party is naughty, you know, Commander.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			say = "As I'm ogling the desserts at the table, Jade dressed in a witch's outfit approaches from behind and pokes me in the back.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 406032,
			say = "But, if you'll dance with a good girl – i.e., me –: I might just turn a blind eye.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "Alright. Let's dance.",
					flag = 1
				},
				{
					content = "Technically, this is a costume. I'm dressed as a naval commander.",
					flag = 2
				}
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_156",
			dir = 1,
			optionFlag = 1,
			actor = 406032,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "That's what I wanted to hear. Heehee.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_156",
			dir = 1,
			optionFlag = 2,
			actor = 406032,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Come on, who: you think you're fooling with that? You'll never be like me if you always make excuses.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301292,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Hippity hoppity! There you are, Commander! Please dance with me!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 406032,
			say = "Ugh. Looks like Shimakaze brought a whole crowd of eager dancers...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 402081,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Pl-please dance with me, too... Er, or:n't. You probably already have way better dance partners in mind...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 203131,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Roleplay mode engaged. \"Urgh. Step aside, you mopey babies. I saw the Commander first.\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403111,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "My, my. Doesn't look like I can just stand by and await orders. Be quick, Commander – decide how this is going to go♥",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307071,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I know you'll want to dance with me more than anyone else. Right, my Commander?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "......",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "Form a queue, ladies. I'll dance with you all.",
					flag = 1
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			say = "As I conclude my final dance, the masquerade ball, too, reaches its end.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			say = "The gentle melody gradually fades into silence, and the spotlights go out one after the next until it's all dark.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			say = "I leave the nightclub and watch as the girls retire to their cabins.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			say = "All the beautiful things that transpired tonight will linger in my mind for a long time.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
