return {
	id = "JIARIXINTIAODAYOULUN23",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			say = "The arcade is booming with video game sounds coming from home consoles, to portables, to arcade cabinets.",
			bgm = "story-niceship-soft",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			say = "This can mean only one thing – it's game night.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			say = "Of course, video games aren't the only thing the arcade has. There is analog games and entertainment, too.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			say = "Table tennis, pool, card games, you name it. It's all here, and it's easy to find someone who'll play with you tonight.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 901051,
			say = "Oh, you're here, Commander! If you're looking for the gamers, they're sparring with each other to warm up. Why:n't you play a card game with me meanwhile?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 901051,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I won't cheat with any Sacraments. Pinky promise.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "(Take her up on her offer.)",
					flag = 1
				}
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			optionFlag = 1,
			say = "Sure!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			say = "Fleuret wins faster than my mind can properly process.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 0,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 901051,
			say = "Wooow, I didn't think you'd lose THAT fast. Wanna try again?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "(Cut your losses.)",
					flag = 1
				},
				{
					content = "(Take her on another time.)",
					flag = 2
				}
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			optionFlag = 1,
			say = "Thanks, but no. I'm not cut out for these games.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			optionFlag = 2,
			say = "Another time. I need to get in the right mindset first.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 901051,
			say = "Alrighty:. I'll be right here if you wanna play again!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			say = "I turn my back on Fleuret and walk over to the arcade cabinet section.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 106014,
			say = "Commaaander! I saw that. You got completely whooped by Fleuret!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 106014,
			say = "How about you enter our tournament? It's a chance to repair your tarnished pride.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 702020,
			say = "Ahahaha! You barely got started before you lost! Sorry to say, but you:n't stand a shadow of a chance of beating us.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301050,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Yes. I will win at any cost.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Before I decide anything, can you at least tell me what genre of game you're playing?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 303100,
			say = "*yaaawn*... A fighting game, obviously? That's, like, the go-to tournament genre.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 306060,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "If Milord will enter, I shall pull out all the stops!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "All the cabinets here are one-on-one games. How about we play each other in teams?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 306060,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I approve! I propose you, I, and Pamiat' Merkuria make up one team.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 303100,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "So... *yaaawn*... that means me, Long Island, and Ayanami are on the other team. What prize:es the winner get?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 106014,
			say = "Hmm... How abooout... the winning team gets to be the first to play Shadow of the Silvertree? I know we're all hyped about it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 702020,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Heh, sounds good to me. Let's run with that.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 106014,
			say = "Also, I want to put a personal wager between you and me on this match, Commander.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Let me guess – it's work-related, right?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 106014,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Bingo! If I win, will you give me the next few days off?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "What's in it for me if I win?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 106014,
			say = "Ugh... If you win, I'll actually go to work in person and NOT slack on the job.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Sounds good. Bring it on.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 106014,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Then we have a deal! Don't expect me to hold anything back!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			say = "Thus, our fierce tournament with much on the line began. Both teams nominate one player each per match.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			say = "Shortly after the first round, I managed to knock out Haguro, one of their best players.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			say = "However, Long Island and Ayanami made a swift comeback, defeating both Pamiat' and Ryuujou.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			say = "Just like that, our one-point lead became a one-point disadvantage.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(I can't let Long Island win. That's what matters.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 106014,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Round 4, let's go! I've got a hadouken with your name on it, Commander!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(Boy, she's A LOT better at fighting games than I thought... I need an opener to punish her.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			say = "She combos my character to the edge of the screen, and she lowers her guard, thinking her victory is guaranteed.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "1 HP is all you need!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 106014,
			say = "Nooo! I was THIS close! THIS close!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 301050,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It's okay. I will avenge you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "You've got to be kidding me...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			say = "I mashed my heart out against Long Island and my fingers cramped up. To the surprise of nobody, Ayanami beat me.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			say = "That's okay, though. While Ayanami beat me, I at least beat Long Island.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 106014,
			say = "Maaan... You beat me fair and square, so I'll: some real work for once...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "I WILL make sure you're True to your word.",
					flag = 1
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 106014,
			say = "Waaahhh! This ghost got totally busted! Goodbye, video games – goodbye, slack time...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(I'm starting to feel bad for the poor girl... You know what? I won't be so strict.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Fine. No days off, but you can keep \"working\" like you have until now.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 106014,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "You mean... hardly working at all? Yippee! I love you, Commander!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 106014,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Let's celebrate with some more gaming!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			say = "With the conclusion of our tournament, calm returns to the once rowdy corner of the arcade.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			say = "The gamer girls aren't ready to retire yet, so they decide to stay there.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			say = "I, on the other hand, walk out of the arcade to the backdrop of Kursk teaching her pool technique to Belorussiya and Hunter.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			say = "Before I can fully exit, however, Fleuret walks up and asks me to walk her to her cabin.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(Once I've:ne that, I'll visit some other part of the ship.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
