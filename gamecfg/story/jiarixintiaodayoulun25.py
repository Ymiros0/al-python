return {
	id = "JIARIXINTIAODAYOULUN25",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			say = "I decide to pay a visit to the ship's gym.",
			bgm = "story-niceship-soft",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(Huh, so it's divided into two sections. It makes sense that they'd include space for track events.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 403036,
			say = "Oh, Commander. Fancy seeing you here.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "I'm just passing by. Don't mind me.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 403036,
			say = "Too late for that. Hey, are you busy right now?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Not really. Why?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 403036,
			say = "Heehee. Well, since you're here, let's: something fun.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 403036,
			say = "Just wait here and I'll be right back.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			say = "Eugen turns on her heel and goes to the indoor part of the gym. Marseillaise is there, and Eugen whispers something to her.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			say = "Marseillaise: gets up and starts talking to everyone else inside the gym.",:
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			say = "A short while later, a small crowd has gathered – Eugen, Marseillaise, Z46, Brünhilde, Kersaint, and Suffren.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 902021,
			say = "...I am the Fighting Angel. Defeat is not an option for me.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 404051,
			say = "I answer your call, Commander, and I'll bring you victory.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Sorry, what?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 403036,
			say = "Did you forget what I said? We're going to: something fun.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 403036,
			say = "The girls and I talked, and we settled on a three-versus-three game of beach volleyball. I wanted you to join us... but you can't have everything.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 902021,
			say = "If you were playing, the team you're not on might be hesitant to go all out.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "So, what am I supposed to:? Spectate?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 902021,
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Not quite. We'd like you to keep score for us.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 803011,
			say = "Yeah. You:n't have a horse in this race, so you'll tally points fair and square! That's why I'm gonna give it 100%!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 401462,
			say = "For fun we play, for prize we fight. Like leopards we'll move, seize victory proud and bright.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 901041,
			say = "If I j-just pray for victory, maybe, just maybe...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 403036,
			say = "Hear that? They're all fired up. We're just waiting for your answer, Commander.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(There's no way this is a casual match, considering the motivation on display here...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Let me guess... you promised something to the winners, didn't you?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 403036,
			say = "Dang, the jig is up. I only promised the winning team would get to have dinner with you, that's all.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 403036,
			say = "You're not going to turn them:wn now that we're all here, are you?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			say = "Eugen's expression screams, \"I have you exactly where I want you.\" Admittedly, she:es look brilliant in the sunlight.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			say = "With a nod, the curtains rise on a fervent game of beach volleyball.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			say = "Marseillaise leaps up into the air, hitting the ball with all her might.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			say = "With a whoosh, it flies straight toward the floor, but Z46 intercepts the ball just before it lands.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			say = "Brünhilde makes a brilliant feint, followed by Eugen jumping up high, smacking the ball with full force.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			say = "It's on course toward Kersaint, who panics and trips, thus giving a point to Iron Blood's team.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 404051,
			say = "That's one point to us! Keep at it and we'll win!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 401462,
			say = "The ball shall fly like a star through the sky.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 901041,
			say = "I'm sorry... It's all my fault...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 803011,
			say = "Don't be like that! The game has only just begun!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 902021,
			say = "The Commander is watching. We must give this our all!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			say = "The motivation flowing through the players burns even brighter.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(This is getting intense. I really wonder who'll win in the end.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			say = "Marseillaise and Suffren fought tooth and nail and finally ended the match with a draw. That means I'll have dinner with both teams.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_601",
			hidePaintObj = True,
			say = "There's still a while to go until:. I'll take the time to check out something else meanwhile.",:
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
