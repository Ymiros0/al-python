return {
	id = "JIARIXINTIAODAYOULUN27",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			say = "I'm walking near the ship's classroom, and I can hear a familiar voice pleading for mercy.",
			bgm = "story-niceship-soft",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 501020,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Pleeease, An Shan! I've memorized it all, I promise! Let me leeeave...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(Fu Shun and An Shan? I should go in and have a look.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			say = "In the classroom I find not just the two aforementioned, but also Hai Tien, Fei Yuen, and Anchorage.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 501010,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Oh, great timing, Commander! Have you come to observe the lecture?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 199030,
			say = "Teacher is gonna study with Anchorage! Hooray! Heehee...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501070,
			say = "Commander, look, look! Miss Hai Tien and Miss An Shan are teaching me and Anchorage how to paint!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			say = "Fei Yuen jogs up to me, holding a picture of what looks like one big haphazard scribble.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "Yuen, drawing is like writing – you need a steady hand and a steady mind.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 9,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "I understand being excited to show your work to the Commander, but... well...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501070,
			say = "Aww... I just wanted the Commander to see my art...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "(Hold your tongue.)",
					flag = 1
				},
				{
					content = "(Praise her.)",
					flag = 2
				}
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			optionFlag = 1,
			say = "......",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_147",
			dir = 1,
			optionFlag = 1,
			actor = 502070,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Let's not bother the Commander more, okay? You just need to practice and improve.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			optionFlag = 2,
			say = "Your drawing is very, uhh... Keep practicing and it'll no:ubt spark a new art movement!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_147",
			dir = 1,
			optionFlag = 2,
			actor = 501070,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "You mean it?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_147",
			dir = 1,
			optionFlag = 2,
			actor = 502070,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Don't get too excited, Yuen. You have much learning to:.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501070,
			say = "I know, I know.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			say = "In contrast to Yuen's eagerness for praise, Anchorage is solemnly taking in the tips An Shan has for her.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 199030,
			say = "How: you draw... this part? I'm confuzzled...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501010,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Like this. Watch closely.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			say = "Fu Shun, meanwhile, has been quiet as a mouse. From a blind spot in An Shan's line of sight, she winks at me as hard as she can.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			say = "When she sees me look her way, she holds up her notebook in the air for me to read.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "\"I want to go adventuring! Get me out of here, Commander!\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501010,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Fuuu Shuuun!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501020,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "WHY'D YOU READ IT OUT LOUD?! Nooo,:n't make me stand in the corner!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501070,
			say = "You're going on an adventure? Then Lady Yuen is coming, too!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 199030,
			say = "Teacher! Take Anchorage on an ad-ven-chur!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Now, now, you've still got classes. Your adventure will have to wait until schooltime is over.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "An Shan and Hai Tien are working really hard to teach you. Don't let their efforts be in vain.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 501070,
			say = "Hrmh... You're right. Fine, Lady Yuen will stay put!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 8,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 199030,
			say = "Anchorage will listen to you, Teacher! When class is over, it's ad-ven-chur time!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501010,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Thanks for calming them:wn, Commander. Fu Shun, come here!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501020,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I-it's over... I'm sorry for nearly instigating a rebellion...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(I didn't like:ing this to her, but education is important. She'll just have to hang in there.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(I'll probably distract from their lessons if I stick around any longer. I'd better get going.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			say = "I leave the classroom the same way as I entered it – to the sound of Fu Shun's chagrin.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
