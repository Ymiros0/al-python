return {
	fadeOut = 1.5,
	mode = 2,
	id = "JIARIXINTIAODAYOULUN41",
	placeholder = {
		"playername"
	},
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Pleasure, Leisure, and Treasure\n\n<size=45>41 An Afternoon on the Lido Deck</size>",
					1
				}
			}
		},
		{
			hideRecordIco = True,
			side = 2,
			bgName = "star_level_bg_133",
			live2d = "login",
			withoutActorName = True,
			bgm = "story-niceship-soft",
			actor = 102134,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "I rush over to the pool, where St. Louis is relaxing on a swim ring.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_133",
			live2d = True,
			hideRecordIco = True,
			actor = 102134,
			actorName = "{playername}",
			hidePaintObj = True,
			say = "Sorry I'm late. Some sudden work came up.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 102134,
			side = 2,
			bgName = "star_level_bg_133",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Don't be so worried about it, Commander.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 102134,
			side = 2,
			bgName = "star_level_bg_133",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Heheh, I can have plenty of fun on my own... but I am happier to have you around.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_133",
			live2d = True,
			hideRecordIco = True,
			actor = 102134,
			actorName = "{playername}",
			hidePaintObj = True,
			say = "Sorry for making you wait.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 102134,
			side = 2,
			bgName = "star_level_bg_133",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Stop apologizing, silly. I'm not mad at you, or anything.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 102134,
			side = 2,
			bgName = "star_level_bg_133",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "If you really want to make it up to me, though...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 102134,
			side = 2,
			bgName = "star_level_bg_133",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "How about you help me put on a fresh layer of sunscreen?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_133",
			live2d = True,
			hideRecordIco = True,
			actor = 102134,
			actorName = "{playername}",
			hidePaintObj = True,
			say = "A fresh layer?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 102134,
			side = 2,
			bgName = "star_level_bg_133",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Sunscreen loses effectiveness after a couple hours. I think mine's just about to wear off.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 102134,
			side = 2,
			bgName = "star_level_bg_133",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "I:n't wanna get sunburned. Won't you please, Commander?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 102134,
			side = 2,
			bgName = "star_level_bg_133",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Don't miss a single spot, okay?♪",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "(Silently get to work.)",
					flag = 1
				},
				{
					content = "Leave it to me!",
					flag = 2
				}
			}
		},
		{
			live2d = True,
			side = 2,
			bgName = "star_level_bg_133",
			withoutActorName = True,
			hideRecordIco = True,
			actor = 102134,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "I accept the sunscreen and start applying it as directed.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 102134,
			side = 2,
			bgName = "star_level_bg_133",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = "main2",
			say = "Heehee... Yeah, right there.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 102134,
			side = 2,
			bgName = "star_level_bg_133",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Gentle and careful fingers, please.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_133",
			live2d = True,
			hideRecordIco = True,
			actor = 102134,
			actorName = "{playername}",
			hidePaintObj = True,
			say = "...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 102134,
			side = 2,
			bgName = "star_level_bg_133",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Heehee, you're adorable when you're so focused, Commander.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 102134,
			side = 2,
			bgName = "star_level_bg_133",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Mm... Ah, can you just move my swimsuit aside there?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 102134,
			side = 2,
			bgName = "star_level_bg_133",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "It's hard to reach myself, so you'll have to: it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 102134,
			side = 2,
			bgName = "star_level_bg_133",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Yeah, right there. A little more, please.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 102134,
			side = 2,
			bgName = "star_level_bg_133",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Heehee. I won't complain if you go further.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			live2d = True,
			side = 2,
			bgName = "star_level_bg_133",
			withoutActorName = True,
			hideRecordIco = True,
			actor = 102134,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "With St. Louis's patient guidance, I manage to get her covered in sunscreen again.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 102134,
			side = 2,
			bgName = "star_level_bg_133",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = "main1",
			say = "Phew... Quick sip of a nice, cold drink... Perfect. Now I can really bask.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 102134,
			side = 2,
			bgName = "star_level_bg_133",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Want a sip, Commander? As a reward for helping me out♪",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 102134,
			side = 2,
			bgName = "star_level_bg_133",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "It'll cool you right:wn.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_133",
			live2d = True,
			hideRecordIco = True,
			actor = 102134,
			actorName = "{playername}",
			hidePaintObj = True,
			say = "But the straw...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 102134,
			side = 2,
			bgName = "star_level_bg_133",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "You're bothered that there's only one? Heehee, well,: you expect me to get two when I'm drinking it myself?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 102134,
			side = 2,
			bgName = "star_level_bg_133",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Come on. Keep waiting, and the ice will melt. Drink up!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			live2d = True,
			side = 2,
			bgName = "star_level_bg_133",
			withoutActorName = True,
			hideRecordIco = True,
			actor = 102134,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "The cold drink flows through the straw and into my throat. Heat seems to drain out of me all at once.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			live2d = True,
			side = 2,
			bgName = "star_level_bg_133",
			withoutActorName = True,
			hideRecordIco = True,
			actor = 102134,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "I glance at St. Louis. She's changed into a comfy position to lie on her floatie.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 102134,
			side = 2,
			bgName = "star_level_bg_133",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Now that you've finished putting my sunscreen on,: you wanna sunbathe with me?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 102134,
			side = 2,
			bgName = "star_level_bg_133",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Nice rays, a sea breeze... It's just right.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 102134,
			side = 2,
			bgName = "star_level_bg_133",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = "mission",
			say = "Your swim ring... Hmm, going to get it seems like unnecessary effort. Just come join me on mine.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 102134,
			side = 2,
			bgName = "star_level_bg_133",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "It might be a little cramped... but if we snuggle tight, we'll fit just fine.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 102134,
			side = 2,
			bgName = "star_level_bg_133",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Besides... I think it might be more fun that way.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			live2d = True,
			side = 2,
			bgName = "star_level_bg_133",
			withoutActorName = True,
			hideRecordIco = True,
			actor = 102134,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "I: as she says and try getting on the floatie.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			live2d = True,
			side = 2,
			bgName = "star_level_bg_133",
			withoutActorName = True,
			hideRecordIco = True,
			actor = 102134,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "She smiles coquettishly at me.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 102134,
			side = 2,
			bgName = "star_level_bg_133",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Heehee, now... Let's have fun together...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 102134,
			side = 2,
			bgName = "star_level_bg_133",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Commander♥",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
