return {
	fadeOut = 1.5,
	mode = 2,
	id = "JICHANG13",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			expression = 3,
			side = 2,
			bgName = "bg_jichang_1",
			dir = 1,
			bgmDelay = 1,
			bgm = "idol-inorinouta-inst",
			actor = 901120,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "*pant*... *pant*... I think... this is it for me... Oof!",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 207110,
			side = 2,
			bgName = "bg_jichang_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "So it would seem.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_jichang_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 202280,
			say = "Splendid work, everyone! We made a spectacular comeback!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_jichang_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 108050,
			say = "Yeah, somehow... Le Malin put up a hell of a fight. I thought with how lazy she normally is, she'd be a pushover!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_jichang_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 108050,
			say = "Guess the Muse System really can work miracles, huh?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 202280,
			side = 2,
			bgName = "bg_jichang_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "It was almost as though she'd been possessed... until she fell over, at least.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 207110,
			side = 2,
			bgName = "bg_jichang_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "This was no case of possession, Dido. Le Malin was simply performing with all her body and soul. The Muse System was amplified by her passion.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_jichang_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 103250,
			say = "If passion equals power,: I guess that means we were nearly out-passioned by her. That's not exactly encouraging.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 701050,
			side = 2,
			bgName = "bg_jichang_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "...…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_jichang_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 701050,
			say = "(Could that be why I was underperforming? Have I been trying too hard and lacked heartfelt passion as a result?)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_jichang_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 701050,
			say = "(They're all:ing it for our dear comrade, or for some other personal reason, while I just...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_jichang_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 108050,
			say = "Maybe, but what if some of the teams just got more powerful riggings than others? What if we drew the short straw?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "？？？",
			bgName = "bg_jichang_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "Muse System:es not function as proposed. Posited theory has no logical basis.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "？？？",
			bgName = "bg_jichang_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "Assessment. the Muse System is a military instrument that converts emotion into weaponized energy. As \"passion\" is an unquantifiable property, its utilization by the Muse System is impossible.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_jichang_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 103250,
			say = "Huh? Wait a minute, aren't you...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			actor = 307110,
			nameColor = "#a9f548",
			dir = 1,
			blackBg = True,
			say = "(The Muse System is fueled by one's emotions, is it?)",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307110,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			blackBg = True,
			say = "(If that is True,: this grants me unparalleled power! Nobody, and I mean nobody has more passionate feelings for my Commander than I:!)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			actor = 307110,
			nameColor = "#a9f548",
			dir = 1,
			blackBg = True,
			say = "(Yes, I am truly in a league of my own! Oh, my Commander, come my time to perform, I shall prove it to you!)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
