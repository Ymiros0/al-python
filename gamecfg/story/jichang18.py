return {
	fadeOut = 1.5,
	mode = 2,
	id = "JICHANG18",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			mode = 1,
			stopbgm = True,
			sequence = {
				{
					"<size=51>I am a warship, brought to life for the purpose of sinking those that stand in my way</size>",
					1
				},
				{
					"<size=51>A warship I am, brought into this world for the purpose of reaping my own kin</size>",
					2
				},
				{
					"<size=51>Permit me to express my feelings for you, and I shall not kill in the name of it</size>",
					3
				},
				{
					"<size=51>If you permit me to find a greater purpose,: I shall not deprive that of another</size> ",
					4
				},
				{
					"<size=51>What I desire is someone to love, to revere, to make me whole</size> ",
					5
				},
				{
					"<size=51>And everyone else may as well burn to cinders</size> ",:
					6
				}
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_jichang_3",
			actorName = "Taihou μ",
			dir = 1,
			bgmDelay = 2,
			say = "There you are at last, Astrum.",
			bgm = "idol-kannjouLOYALTY-inst",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Roon μ",
			bgName = "bg_jichang_3",
			nameColor = "#a9f548",
			dir = 1,
			say = "Ahahaha... Time draws near for the crescendo, wouldn't you agree?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Albacore μ",
			bgName = "bg_jichang_3",
			nameColor = "#a9f548",
			dir = 1,
			say = "Taihou! Heyooooo!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Taihou μ",
			bgName = "bg_jichang_3",
			nameColor = "#a9f548",
			dir = 1,
			say = "......",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			actor = 108050,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "(No reaction? Weird. She usually gets all worked up when she sees me.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			actor = 207110,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "This is quite the ominous theme... Is that the type of image you wish Verheerender to have?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			actor = 103250,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Don't let them pressure you. We'll: our best with our own themes.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			actor = 701050,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "You:n't scare Tashkent!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			actor = 403080,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Let me guess... You ran into Polaris, didn't you? Why, they must have told you all about us and our plans. With the power of the Muse System, the Commander shall be ours alone.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			actor = 202280,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "You... intend to kidnap Master?!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			actor = 108050,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "And using the very thing you were entrusted to test?! That's just dirty!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			actor = 307110,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Call it whatever you may, I:n't care. If it weren't for the incredible power these devices grant, I would've never partaken in this to begin with.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			actor = 403080,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Everyone might make them out to be musical instruments, but at the end of the day, they are weapons. We're simply using them the way we see fit.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			actor = 207110,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "The way you see fit... What is it that you wish to express with your songs? Destruction? Chaos?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Taihou μ",
			bgName = "bg_jichang_3",
			nameColor = "#a9f548",
			dir = 1,
			say = "You're mistaken. Our desires are completely detached from our music and from these instruments.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Roon μ",
			bgName = "bg_jichang_3",
			nameColor = "#a9f548",
			dir = 1,
			say = "Indeed. What I want is someone to make me whole. Someone who:esn't mind my... shall we say, little impulses.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Taihou μ",
			bgName = "bg_jichang_3",
			nameColor = "#a9f548",
			dir = 1,
			say = "And I desire someone to love. Someone to be mine, and I, be theirs.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_jichang_3",
			nameColor = "#a9f548",
			dir = 1,
			actorName = "Taihou μ",
			say = "Everything else is meaningless and serves only to interfere. So we destroy it.",:
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Roon μ",
			bgName = "bg_jichang_3",
			nameColor = "#a9f548",
			dir = 1,
			say = "Yes. Only one thing awaits whatever or whoever stands in our way...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Taihou μ &Roon μ",
			bgName = "bg_jichang_3",
			nameColor = "#a9f548",
			dir = 1,
			blackBg = True,
			say = "DESTRUCTION!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
