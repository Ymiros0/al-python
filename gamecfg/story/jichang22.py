return {
	fadeOut = 1.5,
	mode = 2,
	id = "JICHANG22",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			bgm = "bsm-2",
			side = 2,
			dir = 1,
			blackBg = True,
			say = "Location unknown - Time unknown",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900232,
			actorShadow = True,
			side = 2,
			nameColor = "#ff5c5c",
			dir = 1,
			blackBg = True,
			say = "Data compilation complete. And so, another legacy has been:cumented.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900233,
			actorShadow = True,
			side = 2,
			nameColor = "#ff5c5c",
			dir = 1,
			blackBg = True,
			say = "Only took us like a million years. Sure, it's a task from the depths of the repository, but:es it really take that much computing power?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900232,
			side = 2,
			actorShadow = True,
			nameColor = "#ff5c5c",
			dir = 1,
			blackBg = True,
			say = "There's more to art than the final product, you know. Reproducibility is an important element as well.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900232,
			side = 2,
			actorShadow = True,
			nameColor = "#ff5c5c",
			dir = 1,
			blackBg = True,
			say = "Or are you saying that there's some error within our database?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			actorShadow = True,
			side = 2,
			actor = 900233,
			dir = 1,
			blackBg = True,
			nameColor = "#ff5c5c",
			say = "Ugh, I forgot how pointless it is to argue with you. Whatever, forget I said anything.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900232,
			side = 2,
			actorShadow = True,
			nameColor = "#ff5c5c",
			dir = 1,
			blackBg = True,
			say = "Oh, you're dropping the topic? Shame. I was hoping to hear about your little vacation.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			actorShadow = True,
			side = 2,
			actor = 900233,
			dir = 1,
			blackBg = True,
			nameColor = "#ff5c5c",
			say = "My what now?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900232,
			side = 2,
			actorShadow = True,
			nameColor = "#ff5c5c",
			dir = 1,
			blackBg = True,
			say = "Conductor saw you. Said you were really enjoying yourself. You thought we wouldn't find out?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			actorShadow = True,
			side = 2,
			actor = 900233,
			dir = 1,
			blackBg = True,
			nameColor = "#ff5c5c",
			say = "...…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900232,
			side = 2,
			actorShadow = True,
			nameColor = "#ff5c5c",
			dir = 1,
			blackBg = True,
			say = "Would you look at the time, I need to get back to work. Don't worry, I'll keep your little misbehavior a secret. I've nothing to gain from reporting it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			actorShadow = True,
			side = 2,
			actor = 900233,
			dir = 1,
			blackBg = True,
			nameColor = "#ff5c5c",
			say = "...Uh-huh. Got it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
