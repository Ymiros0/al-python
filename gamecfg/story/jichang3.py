return {
	id = "JICHANG3",
	mode = 2,
	once = True,
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			side = 2,
			nameColor = "#a9f548",
			bgm = "idol-BlueSpirit-inst",
			dir = 1,
			say = "Port - Exercise Area",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 202280,
			nameColor = "#a9f548",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "(So this is the Muse rigging... Equipment that utilizes the power of music and feelings... A precious item Master gave to me.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			actor = 202280,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "(I want to work hard so I can be useful to Master, but... will I be able to be a proper idol...? Oh no...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			actor = 202280,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "(If I were to fail, Master would no:ubt abandon me...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 701050,
			nameColor = "#a9f548",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "What's wrong, Dido? You look like you've been fretting over something for quite some time now.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			actor = 202280,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I-it's nothing, really.…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			actor = 202280,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "It's just that, it's both amazing and terrifying that a mere maid like me is going to be an idol...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 701050,
			nameColor = "#a9f548",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "Is that so. The maid corps must have it hard, having to worry about things like that.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 202280,
			nameColor = "#a9f548",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "...Perhaps I'm simply overthinking it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 701050,
			nameColor = "#a9f548",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "Whatever it is, if you're going to take part in it anyway, you just have to go with the flow. It's best not to worry too much about it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			actor = 701050,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Though we're taking part in idol activities, in the end, it's just to test the Muse rigging. It might be better to just think of this like any other mission...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			actor = 103250,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "You're not the only one without any idea of what you're:ing. I took a look at a video of Lexington's group's performance earlier to help me prepare, and it made me feel a bit uneasy...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			actor = 103250,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "You know, wondering if it was possible for me to be as cute as they were, and all.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			actor = 103250,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "That said, this is a chance that was given to me. If I'm going to participate anyway, I'll give it my best.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 207110,
			nameColor = "#a9f548",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "Yes, that's right. As idols, we've received a wonderful chance to spread love and peace to the world through song.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			actor = 207110,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Even Unicorn was looking forward to this, hehe.~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 202280,
			nameColor = "#a9f548",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "Is Unicorn participating as well?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 207110,
			nameColor = "#a9f548",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "Yes. She seemed nervous about being alone, so I asked Vampire to help her out for a bit.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 701050,
			nameColor = "#a9f548",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "...So how many people from the different factions will be participating? The Northern Parliament wasn't on last time, so I'm not really sure what the situation is.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 108050,
			nameColor = "#a9f548",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "Are you the only one from the Northern Parliament?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 701050,
			nameColor = "#a9f548",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "Yup. Also, the Iris Libre told me that they'd like for me to work extra hard on their behalf, and that they'd be participating individually.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			actor = 108050,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Grr...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 701050,
			nameColor = "#a9f548",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "Aren't you curious? About the other factions, and the teams, and such.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 103250,
			nameColor = "#a9f548",
			side = 2,
			hidePaintObj = True,
			dir = 1,
			say = "Of course I'm curious, but this isn't the time to be worrying about that stuff.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			actor = 103250,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "All right, I'm all ready, let's get started. Muse System exercise, begin!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
