return {
	id = "JICHANG7",
	mode = 2,
	once = True,
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			actor = 108050,
			side = 2,
			nameColor = "#a9f548",
			hidePaintObj = True,
			dir = 1,
			say = "Whewie... I never expected the choreography I came up with to be this tiring...",
			bgm = "idol-BlueSpirit-inst",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 202280,
			side = 2,
			nameColor = "#a9f548",
			hidePaintObj = True,
			dir = 1,
			say = "It might be a good idea to revise it a bit...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			actor = 108050,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Grr...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 103250,
			side = 2,
			nameColor = "#a9f548",
			hidePaintObj = True,
			dir = 1,
			say = "It's okay, it's a given that we'll run into problems as we practice.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 103250,
			side = 2,
			nameColor = "#a9f548",
			hidePaintObj = True,
			dir = 1,
			say = "The Polaris girls went through the same ordeals. It's not something we can avoid.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 207110,
			side = 2,
			nameColor = "#a9f548",
			hidePaintObj = True,
			dir = 1,
			say = "That's right. Also, we haven't run into any major issues yet. That must mean we're at least on the right track.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 701050,
			side = 2,
			nameColor = "#a9f548",
			hidePaintObj = True,
			dir = 1,
			say = "That's quite True. If we're able to continue with our lessons in this manner, even our current issues won't be problematic for us in the future.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			actor = 701050,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Even the performance on stage shouldn't be an issue if we keep it up.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 103250,
			side = 2,
			nameColor = "#a9f548",
			hidePaintObj = True,
			dir = 1,
			say = "If we perform better on stage than we: during practice, that'd mean we're overselling our equipment's performance. Make sure you:n't forget the objective of our operation.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			actor = 108050,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "There's still something I:n't get about our equipment, though.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 202280,
			side = 2,
			nameColor = "#a9f548",
			hidePaintObj = True,
			dir = 1,
			say = "Something about our equipment...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			actor = 108050,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Cleveland's Muse System had a mechanism where the more excited the crowd got, the more power it could output.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 108050,
			side = 2,
			nameColor = "#a9f548",
			hidePaintObj = True,
			dir = 1,
			say = "Think about it. There are plenty of people without the Muse System participating, right?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			actor = 108050,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "So, I just:n't get why they'd deliberately split up people with the Muse System into different teams.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			actor = 207110,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I wonder... why that's the case.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 701050,
			side = 2,
			nameColor = "#a9f548",
			hidePaintObj = True,
			dir = 1,
			say = "So you suspect the Muse System's power output depends on who wields it?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			actor = 701050,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Last time, there was only one unit, Polaris. But this time, there are multiple units. That's a bit strange, I think.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 701050,
			side = 2,
			nameColor = "#a9f548",
			hidePaintObj = True,
			dir = 1,
			say = "For instance, if the Sakura Empire and Iron Blood duo achieve a higher performance with the Muse System, that would mean that they're stronger than us.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			actor = 103250,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "In other words, people will think there's a power imbalance between the factions... But I wonder if we're reading too deeply into this.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 108050,
			side = 2,
			nameColor = "#a9f548",
			hidePaintObj = True,
			dir = 1,
			say = "We'll never find out unless we participate in the competition~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			actor = 202280,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "All we can: now is focus on the lesson at hand.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 103250,
			side = 2,
			nameColor = "#a9f548",
			hidePaintObj = True,
			dir = 1,
			say = "That's right. Come on everyone, once more!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
