return {
	fadeOut = 1.5,
	mode = 2,
	id = "JICHANG8",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			expression = 2,
			side = 2,
			bgName = "bg_jichang_1",
			dir = 1,
			bgmDelay = 1,
			bgm = "song06",
			actor = 207110,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Become a star, and sing a song of love and peace~",
			flashin = {
				delay = 1,
				dur = 1,
				alpha = {
					1,
					0
				}
			},
			flashN = {
				color = {
					1,
					1,
					1
				},
				alpha = {
					{
						0,
						1,
						0.5
					},
					{
						1,
						0,
						0.5,
						0.2
					}
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 103250,
			side = 2,
			bgName = "bg_jichang_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "...All right! Break time, folks!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 108050,
			side = 2,
			bgName = "bg_jichang_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Yaay~ *highfive*",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_jichang_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 701050,
			say = "Yay. *highfive*",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_jichang_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 202280,
			say = "Miss Illustrious, you did great...!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_jichang_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 103250,
			say = "Really, fantastic job. Thanks to you, everyone maintained their positions well.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_jichang_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 207110,
			say = "You give me too much credit. It's thanks to your great leadership that we were able to come this far.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 103250,
			side = 2,
			bgName = "bg_jichang_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "It wasn't just me, it was thanks to everyone's cooperation.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 108050,
			side = 2,
			bgName = "bg_jichang_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "This light should be aimed there... Come on, let's get going! I'll be giving the directions to the manjuus.",
			flashout = {
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 0.5,
				alpha = {
					1,
					0
				}
			},
			effects = {
				{
					active = True,
					name = "memoryFog"
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_jichang_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 108050,
			say = "Aaand this prop. It matches me just fine, but as I thought, it really won't match Illustrious and the others at all~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_jichang_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 202280,
			say = "This precious commodity that Miss Victorious entrusted to me... I'll use it to make the ultimate costume!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_jichang_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 202280,
			say = "Oh, Sirius, you:n't need to help out with that. Please, help me with something else!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 701050,
			side = 2,
			bgName = "bg_jichang_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I see, so the current fad is idols with animal ears...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_jichang_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 701050,
			say = "Grozny, you've gotten hold of some great intel...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 103250,
			side = 2,
			bgName = "bg_jichang_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "All right, there's only a bit more left for today. Let's finish up and head back to the:rms.",
			effects = {
				{
					active = False,
					name = "memoryFog"
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_jichang_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 103250,
			say = "And tomorrow... let's give it our all!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Everyone",
			bgName = "bg_jichang_1",
			nameColor = "#a9f548",
			dir = 1,
			blackBg = True,
			say = "Yeah!",
			dialogShake = {
				speed = 0.09,
				x = 10,
				number = 2
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
