return {
	fadeOut = 1.5,
	mode = 2,
	id = "JINGHAIJINGLEI1",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Tranquil Sea, Distant Thunder\n\n<size=45>1 The Visitor Amidst Clouds</size>",
					1
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			say = "Somewhere in the Sakura Islands...",
			bgm = "theme-IJNdailymeeting",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			say = "A girl with a fox mask on her head was painstakingly pushing a wheelbarrow up a mountain path.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Phew... Still a long way to go, huh...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "At least the road is well-maintained and not too hard to walk on...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I wonder what people think about this famous tourist spot being closed for so long. Doesn't it raise more eyebrows than if we had just kept it open?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I can understand barring uninvolved people from entering, but why: all the people involved have to stay quiet as well?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "And why: I have to use a wheelbarrow? I'm sure the gods of the forest would forgive me if I drove up...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I'm at my wit's end here! This stuff's ridiculously heavy to lug around in a cart!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "What's next, are they going to ask me to bring up the disassembled naval gun components by cart too?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...Ooh, I really shouldn't jinx myself like that. It probably isn't even out of the realm of possibility, haha...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "*sigh*... I'll just: as Akagi said and leave the nonessential supplies by the base of the mountain so they can be picked up later.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Everything else is coming with me up this long trail...",:
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It was raining here not too long ago, but as long as I watch my step and make sure not to slip–––",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...Hwaah! Darn me and my big, fat mouth!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			dir = 1,
			say = "As someone well-versed in tripping over her own feet, the girl quickly regained her balance and stabilized her wheelbarrow before it could tip over.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Holy moly, that was a close one... I would've been in so much trouble if I broke the stuff inside...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...Focus, Yamashiro, focus! You've got to arrive before sunset!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			say = "As the mountain road leveled out, the girl came upon a dignified, old-fashioned wooden building.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Here we are... Hellooo? Is anybody hooome?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301370,
			side = 2,
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Shiranui, we've got a visitor!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301180,
			side = 2,
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "If it isn't Yamashiro. I bid you welcome.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Oh hi! It's nice to see the two of you!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301180,
			side = 2,
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "We:n't often get visitors around these parts.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "That's not surprising. You're the ones who closed off the road, right?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301180,
			side = 2,
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I'm afraid you're mistaken. We're only following orders, same as you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301180,
			side = 2,
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "On another note, what's in that wheelbarrow you brought?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "A present from Akagi, I suppose?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "She told me to leave the nonessential supplies at the mountain's base and bring the important stuff straight to you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "What should I: with it? Leave it over here?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301180,
			side = 2,
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Yes, over there will be just fine.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301370,
			side = 2,
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Thanks, Yamashiro. Could you send Akagi a message for us?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301370,
			side = 2,
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Just tell her we got everything.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Will:! I'll let her know as soon as I can!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301180,
			side = 2,
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Much obliged. It must've been a painstaking journey getting here.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It's fiiine, I'm used to it! Now, I'd better be on my way. I've got other business to deal with!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301180,
			side = 2,
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Of course. Do mind your footing on the way back, as it rained earlier today.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Yeah, hahaha... I noticed that.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301370,
			side = 2,
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Bye-bye, Yamashiro! Stay safe!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_xinnong2_98",
			hidePaintObj = True,
			dir = 1,
			blackBg = True,
			nameColor = "#A9F548FF",
			say = "Will:! See you!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
