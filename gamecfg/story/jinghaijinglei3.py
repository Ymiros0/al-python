return {
	fadeOut = 1.5,
	mode = 2,
	id = "JINGHAIJINGLEI3",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Tranquil Sea, Distant Thunder\n\n<size=45>3 Gathering of Dark Clouds</size>",
					1
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			say = "Sakura Islands - The Great Library",
			bgm = "story-richang-7",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Sheesh, this library really has it all, from books you'll find anywhere to ancient tomes...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Since there was nothing unusual in the confidential files, I guess my only option is to look through older:cuments.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I'll at least know if someone's tampered with those. You can alter digital data without leaving a trace, but not so with physical paper and ink.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			say = "Zuikaku trodded through great bookshelves forming long rows. Ancient works were lined up high, but her spirits were low.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Mikasa was totally right. Nearly every record covering the Sanctuary is inconsistent with the next.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Same with any books touching on the Sacred Sakura and the Diadem of Light...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "There are records dating further back, but they're either the only copy known to exist, or the things written in them:n't make sense.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It's almost like somebody went and fabricated a bunch of stories...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...As if they wanted to tamper with our legends.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "If so, how long have they been:ing it?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			say = "She continued rummaging through papers. Then, she suddenly heard the voice of a young girl.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301330,
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Z-Zuikaku, are you looking for a book?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Kisaragi? I guess you could say that. You looking for one, too?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301330,
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "No... I-I just came to give you a message.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301330,
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "{namecode.96}————",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301330,
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "\"A storm is coming. You should head home soon.\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I see... Thanks. I'll get going as soon as I find the book I need.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301330,
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Okay... I'm gonna leave:.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301330,
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Don't stay too long, okay?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			say = "Zuikaku resumed reading. When she looked over her shoulder just a moment later, she saw Kisaragi had disappeared into the forest of literature.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "How'd she find her way here? That's kind of... Nah, I'm probably overthinking it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "She's right about the storm thing. Looks like it's gonna rain. I'll buy an umbrella on my way home.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...Gah, I'm getting distracted! Need to find something ASAP and tell Mikasa about it...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			say = "Zuikaku put:wn the last in a series of:cuments and gazed out the window at darkly overcast skies.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Oh crap, I lost track of time.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Guess that's enough reading for one day. I'll come back and read more tomorrow.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It's really pouring:wn out there... I'll definitely want an umbrella before I head back.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			say = "She put on her coat and began heading for the store. That's when a girl with an umbrella appeared from nowhere.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302070,
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "You may use mine if you'd like.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Oh! Thanks, Yura! Whatcha here for? Looking for something to read?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 302070,
			say = "Not exactly. I've come to relay orders to you from Musashi.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Come again? Musashi?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 302070,
			say = "Yes. She wants you to travel to an Iron Blood base at once to serve as the Sakura Empire's representative there.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Talk about sudden... Where even is this base? And what am I supposed to say to them?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 302070,
			say = "Sadly, I:n't know. However, I've been told you'll be sent further details after you depart.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 302070,
			say = "There's a car waiting outside for you. I suggest you get in, go to the port, and set sail as soon as you can.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Cruising in the rain isn't exactly my– Wait, what? I have to go NOW?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 302070,
			say = "Yes. Those are Musashi's direct orders.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Oookay:... I'll head out right away.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "(What's going on here? Why send me away now, Musashi?)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "(I'll have to tell Shoukaku to take over the investigation for me... I hope she can find something while I'm gone.)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "star_level_bg_116",
			hidePaintObj = True,
			dir = 1,
			blackBg = True,
			nameColor = "#A9F548FF",
			say = "(Here's praying some of my good luck will come your way, Sis!)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
