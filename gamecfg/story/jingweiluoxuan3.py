return {
	fadeOut = 1.5,
	mode = 2,
	id = "JINGWEILUOXUAN3",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			say = "PH Harbor - Temporary Office, Hours Later",
			side = 2,
			bgName = "bg_luoxuan_2",
			hidePaintObj = True,
			dir = 1,
			bgmDelay = 1,
			bgm = "story-2",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_luoxuan_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 103140,
			say = "Hey there, Puddin'! Your new office is looking hella spiffy~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_luoxuan_2",
			hidePaintObj = True,
			dir = 1,
			say = "After sending New Jersey and Ticonderoga off on their patrol mission, I started to head back to the office to finish organizing the:cuments I brought with me.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_luoxuan_2",
			hidePaintObj = True,
			dir = 1,
			say = "Just when I was about to sit back:wn, I noticed someone on the sofa in front of me.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_luoxuan_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 103140,
			say = "Oh, did Ticonderoga already hook you up with the goods?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_luoxuan_2",
			hidePaintObj = True,
			dir = 1,
			say = "This new girl was waving around the:cuments I had painstakingly sorted as if they were cheap magazines.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_luoxuan_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 103140,
			say = "Jeez, that girl sure moves fast, huh... What the heck's with this handwriting? Look at these hearts and curlicues!",
			effects = {
				{
					active = True,
					name = "speed"
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_luoxuan_2",
			hidePaintObj = True,
			dir = 1,
			say = "\"I'm not sure what the problem is... Doesn't everyone have their little quirks when it comes to handwriting?\"",
			effects = {
				{
					active = False,
					name = "speed"
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 103140,
			side = 2,
			bgName = "bg_luoxuan_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "That's only barely scratching the surface, I tell you. If you know how to interpret these things, there's a whole bunch of hints from the way these papers are ordered to how the pages are folded.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_luoxuan_2",
			hidePaintObj = True,
			dir = 1,
			say = "\"Sounds impressive, but overly complicated.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_luoxuan_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 103140,
			say = "Haha, you're way too dense, Puddin'~ But that's part of what makes you so delicious~♪",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_luoxuan_2",
			hidePaintObj = True,
			dir = 1,
			say = "\"By the way, when you say 'pudding,' are you referring... to me?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_luoxuan_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 103140,
			say = "A girl's gotta make her mark, yeah? Tell ya what, hang out with me for a while, and I'll teach you how to become popular!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			side = 2,
			bgName = "bg_luoxuan_2",
			hidePaintObj = True,
			dir = 1,
			say = "This girl was completely beyond my grasp...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_luoxuan_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 103140,
			say = "Ahaha! I can't believe you'd fall for that! All these:cuments were typed up to begin with!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_luoxuan_2",
			hidePaintObj = True,
			dir = 1,
			say = "So this was the San Francisco I had heard so much about... a fun-loving girl who tries to live it up every waking moment of her life.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_luoxuan_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 103140,
			say = "Anyway, if these:cuments are straight from the Naval HQ, is the Crimson Axis somehow involved?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_luoxuan_2",
			hidePaintObj = True,
			dir = 1,
			say = "\"The Crimson Axis... Nominally, an organization that serves as our geopolitical enemies...\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_luoxuan_2",
			hidePaintObj = True,
			dir = 1,
			say = "\"Regardless of the feelings and sentiments of the various shipgirls, the politics between our two major organizations have always been opaque and inscrutable.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_luoxuan_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 103140,
			say = "Do you think it's because the Sakura Empire's fleet also appeared during the last Reenactment?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_luoxuan_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 102050,
			say = "Commander, I've received a message from NY City.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_luoxuan_2",
			hidePaintObj = True,
			dir = 1,
			say = "\"Naval HQ, huh? Put them through.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_luoxuan_2",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Sister Sara is soooooo worried about you, Commander! Is everything okay?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_luoxuan_2",
			hidePaintObj = True,
			dir = 1,
			say = "The excessively lively voice of Saratoga, the fleet idol, bubbled over from the other side of the communicator.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_luoxuan_2",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Actually, there's a little something that I need your advice on~~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_luoxuan_2",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "So, this is actually pretty far from where you are, but I've gotten information that suggests the Sakura Empire is amassing a large fleet.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_luoxuan_2",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "We:n't know who's leading it, or what their purpose is.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_luoxuan_2",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "But the fact that they're:ing it now of all times has the Naval HQ on edge.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_luoxuan_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 103140,
			say = "Woah, speak of the devil...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_luoxuan_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 103140,
			say = "Weren't we supposed to be working together in staving off the Sirens, though?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_luoxuan_2",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Yeah! That's why I'd like to remind you to avoid hostilities with them if possible...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_luoxuan_2",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Just saying, be careful out there! You never know what kind of craziness might happen if you get thrown into another Reenactment!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_luoxuan_2",
			hidePaintObj = True,
			dir = 1,
			say = "\"Understood. I'll keep that in mind.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_luoxuan_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 107030,
			say = "Anyway, that's all from Sister Sara! Be careful out there, Commander~ Mwah!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_luoxuan_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 103140,
			say = "I have a bad feeling about this...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			side = 2,
			bgName = "bg_luoxuan_2",
			hidePaintObj = True,
			dir = 1,
			say = "\"Me too. Too much is happening at once...\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_luoxuan_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 103140,
			say = "That's not what I'm talking about... Why's that girl– Well, whatever, I'm not gonna let it keep me up at night!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_luoxuan_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 103140,
			say = "Catch ya later, Puddin'~ Keep me posted if anything new happens!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			options = {
				{
					content = "\"Wait, San Francisco, why are you taking the:cuments with you?\"",
					flag = 1
				},
				{
					content = "Watch San Francisco leave.",
					flag = 2
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_luoxuan_2",
			dir = 1,
			optionFlag = 1,
			actor = 103140,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Forgot to mention it, but I'm also in charge of looking after your health while you're here, Commander. No overtime while I'm here, got it?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 103140,
			side = 2,
			bgName = "bg_luoxuan_2",
			hidePaintObj = True,
			dir = 1,
			optionFlag = 1,
			nameColor = "#a9f548",
			say = "If you want these back so badly, you'll have to follow me back to the bedroom~♪ Ahaha!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_luoxuan_2",
			dir = 1,
			optionFlag = 2,
			actor = 103140,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Oh yeah, forgot to mention it, but I'm also in charge of looking after your health while you're here, Commander. There won't be any overtime under my watch!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			stopbgm = True,
			side = 2,
			bgName = "bg_luoxuan_1",
			hidePaintObj = True,
			dir = 1,
			say = "When I woke up the next day...",
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_luoxuan_1",
			hidePaintObj = True,
			dir = 1,
			blackBg = True,
			say = "I learned that we had lost contact with the AF base.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
