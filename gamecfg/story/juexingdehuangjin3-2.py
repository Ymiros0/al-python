return {
	id = "JUEXINGDEHUANGJIN3-2",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			nameColor = "#A9F548FF",
			side = 2,
			say = "A fierce battle unfolded. The Mechadrake suddenly brandished its iron claws and charged forward.",
			hidePaintObj = True,
			bgm = "story-battle-16bit-sfc",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			say = "It happened so fast, the Prototype Super Drill MKII couldn't:dge, and was cleaved in two with just one attack.",
			hidePaintObj = True,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			actor = 100011,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "What, purin?! An iron claw sweep, purin?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403130,
			nameColor = "#FF9B93",
			actorName = "Iron Claw Demon",
			hidePaintObj = True,
			side = 2,
			expression = 2,
			say = "Heheh! It's over for you!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403130,
			nameColor = "#FF9B93",
			actorName = "Iron Claw Demon",
			hidePaintObj = True,
			side = 2,
			say = "I'm not surprised. This is the best your antiquated Gilded Age mech could:.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			say = "The Iron Claw Demon smirked, drunk on a sense of triumph.",
			hidePaintObj = True,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			say = "But:...",
			hidePaintObj = True,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			say = "Wasting not even a second to mourn the loss of her mech, the Bulin charged back into battle with a new mech.",
			hidePaintObj = True,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 100011,
			side = 2,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			dir = 1,
			say = "Pierce the heavens once more, my Prototype Super Drill MKII!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403130,
			nameColor = "#FF9B93",
			actorName = "Iron Claw Demon",
			hidePaintObj = True,
			side = 2,
			say = "What?! You have another?!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			actor = 100011,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Hahaha! \"Another,\" purin? What a naive fool you are, purin!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			actor = 100011,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Obviously there wasn't just ONE Gilded Age mech, purin!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			say = "Indeed, the mech was of a mass-produced variety. The Iron Claw Demon was not expecting this.",
			hidePaintObj = True,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			say = "She couldn't react in time to its golden laser, which reduced her Mechadrake into a pile of scrap.",
			hidePaintObj = True,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			actor = 100011,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "NOW it's over, purin!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			actor = 100011,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Victory is mine, purin!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403130,
			nameColor = "#FF9B93",
			actorName = "Iron Claw Demon",
			hidePaintObj = True,
			side = 2,
			say = "Are you sure about that?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403130,
			nameColor = "#FF9B93",
			actorName = "Iron Claw Demon",
			hidePaintObj = True,
			side = 2,
			expression = 2,
			say = "Then how come the results screen hasn't popped up yet?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 8,
			side = 2,
			actor = 100011,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Purin?! No, it can't be...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			nameColor = "#FF9B93",
			hidePaintObj = True,
			actorName = "Iron Claw Demon",
			actor = 403130,
			say = "Come back to me, my second half – Iron Claw Mechadrake!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			effects = {
				{
					active = True,
					name = "speed"
				}
			}
		},
		{
			side = 2,
			actorName = "Iron Claw Mechadrake",
			nameColor = "#FFC960",
			hidePaintObj = True,
			say = "ROOOOOOAAAAARRRR!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
