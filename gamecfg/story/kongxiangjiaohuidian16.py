return {
	fadeOut = 1.5,
	mode = 2,
	id = "KONGXIANGJIAOHUIDIAN16",
	once = True,
	fadeType = 2,
	scripts = {
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_bsmre_cg9",
			hidePaintObj = True,
			stopbgm = True,
			bgmDelay = 2,
			say = "Mirror Sea - α phase, tower observation deck",
			bgm = "deepblue-image",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			actorName = "Zuikaku",
			bgName = "bg_bsmre_cg9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Whoooa! The view from up here is amazing!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			actorName = "Zuikaku",
			bgName = "bg_bsmre_cg9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Is it this good up on the original Tower of Midgard as well?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Ulrich von Hutten",
			bgName = "bg_bsmre_cg9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Do you mean the Floating Fortress's control tower?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Ulrich von Hutten",
			bgName = "bg_bsmre_cg9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "The view's not too bad, but we normally:n't let unauthorized personnel onto the deck.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			actorName = "Zuikaku",
			bgName = "bg_bsmre_cg9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Dang. It's a great place to: recon from...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			side = 1,
			actorName = "Zuikaku",
			bgName = "bg_bsmre_cg9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "I mean, heck, you can make out every detail of that massive city up here.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			actorName = "Zuikaku",
			bgName = "bg_bsmre_cg9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Speaking of, now that you've had a closer look,:es anything jump out to you?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Ulrich von Hutten",
			bgName = "bg_bsmre_cg9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Sort of.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Ulrich von Hutten",
			bgName = "bg_bsmre_cg9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "It:esn't really match the architectural style of the Sirens.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			actorName = "Zuikaku",
			bgName = "bg_bsmre_cg9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Yeah. Now that you mention it, it almost looks like something... humans would build.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Ulrich von Hutten",
			bgName = "bg_bsmre_cg9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "It:esn't mean much when they've copied us time and time again, but that's not important. Take a look at that huge tower in the middle.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			actorName = "Zuikaku",
			bgName = "bg_bsmre_cg9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "In the dead center? Hmm, yeah, that one:esn't look anything like the other buildings surrounding it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			side = 1,
			actorName = "Zuikaku",
			bgName = "bg_bsmre_cg9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "If you ask me, it looks more like...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Ulrich von Hutten",
			bgName = "bg_bsmre_cg9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "The tower we saw in Compiler's mainframe when we took the fight to her.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			actorName = "Zuikaku",
			bgName = "bg_bsmre_cg9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "I thought the same thing! Does that mean... this Mirror Sea might be another Siren's mainframe?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Ulrich von Hutten",
			bgName = "bg_bsmre_cg9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "It's a possibility.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			actorName = "Zuikaku",
			bgName = "bg_bsmre_cg9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "I'm thinking... Gneisenau said that The Hermit is behind the attack on the Fortress.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			actorName = "Zuikaku",
			bgName = "bg_bsmre_cg9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "There's no way this tower is supposed to be her mainframe, right? Ahaha...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Ulrich von Hutten",
			bgName = "bg_bsmre_cg9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Your guess is as good as mine. None of us have ever seen an Arbiter's mainframe with our own eyes.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Ulrich von Hutten",
			bgName = "bg_bsmre_cg9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Not to mention, it would be immensely illogical.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Ulrich von Hutten",
			bgName = "bg_bsmre_cg9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Their mainframe allows them to essentially come back to life endlessly.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Ulrich von Hutten",
			bgName = "bg_bsmre_cg9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "I can't think of any reason they'd willingly expose themselves like this.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Gneisenau META",
			bgName = "bg_bsmre_cg9",
			hidePaintObj = True,
			nameColor = "#ffa500",
			say = "Unless, they're trying to bait us into attacking it?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Ulrich von Hutten",
			bgName = "bg_bsmre_cg9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Highly unlikely. There's no way in hell we'd ever have the firepower to assault an Arbiter mainframe head-on.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Ulrich von Hutten",
			bgName = "bg_bsmre_cg9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "If she wanted to get rid of us, she could easily overwhelm us with her vessels.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Gneisenau META",
			bgName = "bg_bsmre_cg9",
			hidePaintObj = True,
			nameColor = "#ffa500",
			say = "No, you're right. Why waste time on setting an elaborate trap when she could get rid of us in a fraction of the time?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Gneisenau META",
			bgName = "bg_bsmre_cg9",
			hidePaintObj = True,
			nameColor = "#ffa500",
			say = "Perhaps her goal is to force us inside the city,:.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Gneisenau META",
			bgName = "bg_bsmre_cg9",
			hidePaintObj = True,
			nameColor = "#ffa500",
			say = "I could see her thinking that we'd feel pressured to look for a place to flee the red sea once it starts closing in on us.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Gneisenau META",
			bgName = "bg_bsmre_cg9",
			hidePaintObj = True,
			nameColor = "#ffa500",
			say = "This city in the sky would be the obvious option in such a scenario.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Gneisenau META",
			bgName = "bg_bsmre_cg9",
			hidePaintObj = True,
			nameColor = "#ffa500",
			say = "Especially if she provided the means for us to reach it...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Ulrich von Hutten",
			bgName = "bg_bsmre_cg9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "She wants to corner us for some reason, is what you're saying?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Ulrich von Hutten",
			bgName = "bg_bsmre_cg9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Still seems easier to just kill us and be:ne with it. One thing's for sure – we're in no condition to take on an Arbiter or a mainframe right now.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Ulrich von Hutten",
			bgName = "bg_bsmre_cg9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "I say we head back. Let's ask Bismarck what we should: next.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
