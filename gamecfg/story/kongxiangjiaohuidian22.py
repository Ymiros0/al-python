return {
	fadeOut = 1.5,
	mode = 2,
	id = "KONGXIANGJIAOHUIDIAN22",
	once = True,
	fadeType = 2,
	scripts = {
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_bsmre_6",
			stopbgm = True,
			say = "Mirror Sea - ω phase",
			bgmDelay = 2,
			bgm = "theme-bismark-reborn",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9708010,
			nameColor = "#ffa500",
			bgName = "bg_bsmre_6",
			hidePaintObj = True,
			dir = 1,
			actorName = "U-556 META?",
			side = 2,
			say = "Lord Bismarck, wouldn't it be smart to patrol the area?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_bsmre_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 405050,
			say = "No need. I'll have my hands full with touching up Regensburg's rigging.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 405050,
			side = 2,
			bgName = "bg_bsmre_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "In the meantime, I want to discuss the Mirror Sea's structure with someone. Will you stay?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_bsmre_6",
			actorName = "U-556 META?",
			dir = 1,
			actor = 9708010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "Of course! I'd: anything if it helps you out!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 405050,
			side = 2,
			bgName = "bg_bsmre_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Heheh. Alright,: I'd like you to tell me what you think this place is.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_bsmre_6",
			actorName = "U-556 META?",
			dir = 1,
			actor = 9708010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "What it is? Uhhhh... Hmmmm...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_bsmre_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 405050,
			say = "Don't think too hard. Just tell me the first thing that comes to mind.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9708010,
			nameColor = "#ffa500",
			bgName = "bg_bsmre_6",
			hidePaintObj = True,
			dir = 1,
			actorName = "U-556 META?",
			side = 2,
			say = "In that case... A loop? Maybe an hourglass?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_bsmre_6",
			actorName = "U-556 META?",
			dir = 1,
			actor = 9708010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "Yeah, I think it's like an hourglass!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 405050,
			side = 2,
			bgName = "bg_bsmre_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "An hourglass?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_bsmre_6",
			actorName = "U-556 META?",
			dir = 1,
			actor = 9708010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "A really, reeeally big one.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_bsmre_6",
			actorName = "U-556 META?",
			dir = 1,
			actor = 9708010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "Like, you know how at first, everything was red other than the area around the Tower?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9708010,
			nameColor = "#ffa500",
			bgName = "bg_bsmre_6",
			hidePaintObj = True,
			dir = 1,
			actorName = "U-556 META?",
			side = 2,
			say = "And every time we used the gate, we ended up in the same place, but the red sea was getting closer...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_bsmre_6",
			actorName = "U-556 META?",
			dir = 1,
			actor = 9708010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "Then, when we used it again after seeing the upside-down city, the red and blue were reversed!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_bsmre_6",
			actorName = "U-556 META?",
			dir = 1,
			actor = 9708010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "So I think the Tower is the middle of the hourglass, and the gate is the passage of time.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_bsmre_6",
			actorName = "U-556 META?",
			dir = 1,
			actor = 9708010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "The sea starts out blue,: gradually becomes red...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 9708010,
			nameColor = "#ffa500",
			bgName = "bg_bsmre_6",
			hidePaintObj = True,
			dir = 1,
			actorName = "U-556 META?",
			side = 2,
			say = "And when it's all turned red, the hourglass is flipped, turning red sea back into blue sea...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_bsmre_6",
			actorName = "U-556 META?",
			dir = 1,
			actor = 9708010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "I've got it! If the red sea is sand,: us passing through the gate is what lets it fall through!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_bsmre_6",
			actorName = "U-556 META?",
			dir = 1,
			actor = 9708010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "I mean, look at the Tower. Its light beam has changed from blue to red. I think that indicates what's on the other side of the hourglass!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9708010,
			nameColor = "#ffa500",
			bgName = "bg_bsmre_6",
			hidePaintObj = True,
			dir = 1,
			actorName = "U-556 META?",
			side = 2,
			say = "And maybe the reason we saw the upside-down city is because we were standing on top of the sand!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_bsmre_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 405050,
			say = "This Mirror Sea is an hourglass, and the red sea represents sand falling:wn...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 405050,
			side = 2,
			bgName = "bg_bsmre_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Using the gate only transports us through time, not space, during which sand falls:wn, expanding the red sea.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_bsmre_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 405050,
			say = "Hang on, that reminds me...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_bsmre_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Yeah, we were looking for Ulrich when we first arrived, and there was no red sea here at all.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			effects = {
				{
					active = True,
					name = "memoryFog"
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_bsmre_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 405030,
			say = "......",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_bsmre_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Oh, uh, you were sleepwalking at the time, so it's natural you wouldn't remember it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_bsmre_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "While I can't be 100% sure, I think Jade was the first to observe the red sea.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_bsmre_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 405030,
			say = "(All I did remember from my episode is going through the gate and heading straight for the control center... No, wait, there's one more thing.)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_bsmre_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 405030,
			say = "Those giant cubes. I:n't remember them being there.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_bsmre_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Huh? That's weird. Gneisenau and I definitely saw them when we first got here.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_bsmre_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 402100,
			say = "If what Ulrich said is True, and the cubes aren't based on some kind of timetable, we must've:ne something to cause them to appear.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_bsmre_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 405050,
			say = "(Ulrich said that when she first arrived, there was no sign of the red sea, or those floating cubes.)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			effects = {
				{
					active = False,
					name = "memoryFog"
				}
			}
		},
		{
			actor = 405050,
			side = 2,
			bgName = "bg_bsmre_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "(After Ulrich activated the tower, Zuikaku and Gneisenau came through and saw the floating cubes, but not the red sea.)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_bsmre_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 405050,
			say = "(By the time I arrived, the red sea was already present, and with each activation of the tower, the red area grew more and more.)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 405050,
			side = 2,
			bgName = "bg_bsmre_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "(When we saw the upside-down city, that must've been when all the \"sand\" had passed through one phase.)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_bsmre_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 405050,
			say = "(Then, there's the corrosion effect of the red sea, the METAmorphosis phenomenon, and the unknown enemies that appear from the red waters...)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_bsmre_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 405050,
			say = "(But if we assume that this Mirror Sea houses a Siren Arbiter's mainframe, why would those things be here? Unless...)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_bsmre_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 405050,
			say = "...Oh my god!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			actor = 9708010,
			nameColor = "#ffa500",
			bgName = "bg_bsmre_6",
			hidePaintObj = True,
			dir = 1,
			actorName = "U-556 META?",
			side = 2,
			say = "What's wrong, Lord Bismarck? You look pale!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_bsmre_6",
			actorName = "U-556 META?",
			dir = 1,
			actor = 9708010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "I think my guess might be wrong after all...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 405050,
			side = 2,
			bgName = "bg_bsmre_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Ugh... Sorry, I didn't mean to make you worry.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_bsmre_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 405050,
			say = "I'm working with a number of assumptions here, but it's best if we act quickly.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 405050,
			side = 2,
			bgName = "bg_bsmre_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Come with me, U-556. We're going back to the control center.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			stopbgm = True,
			mode = 1,
			bgName = "bg_bsmre_cg10",
			bgmDelay = 1,
			bgm = "story-midgard",
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			},
			sequence = {
				{
					"",
					0
				}
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_bsmre_cg10",
			actorName = "U-556 META?",
			dir = 1,
			actor = 9708010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "Wh-what is that?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_bsmre_cg10",
			say = "In place of the previous vision of Friedrich, the:me in the central control room displayed a new, rather ill-boding image.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_bsmre_cg10",
			actorName = "U-556 META?",
			dir = 1,
			actor = 9708010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "All of Iron Blood has been defeated?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_bsmre_cg10",
			say = "Heaps of wreckages floating in a burning sea. Among them were the girls of Iron Blood, including Bismarck's sister, Tirpitz.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 405050,
			side = 2,
			bgName = "bg_bsmre_cg10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Calm:wn. It's not real.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_bsmre_cg10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 405050,
			say = "(My corrosion sensor is responding, though. This place really:es connect to the throat of the hourglass.)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Communicator",
			bgName = "bg_bsmre_cg10",
			nameColor = "#A9F548FF",
			say = "BEEP BEEP BEEP!",
			soundeffect = "event./battle/button",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_bsmre_cg10",
			paintingNoise = True,
			dir = 1,
			actor = 9704010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "Bismarck, we've finished surveying the nearby waters. There seems to be nothing of note here.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_bsmre_cg10",
			paintingNoise = True,
			dir = 1,
			actor = 9704010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "The other META's trail also seems to have vanished, I'm afraid.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_bsmre_cg10",
			actorName = "Geryon",
			nameColor = "#ffa500",
			say = "......",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_bsmre_cg10",
			paintingNoise = True,
			dir = 1,
			actor = 405030,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Don't draw conclusions just yet, Gneisenau. We're bound to find a lead on her if we keep looking.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_bsmre_cg10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 405050,
			say = "I admire your drive, but pursuing a wild goose chase will only squander our finite resources.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 405050,
			side = 2,
			bgName = "bg_bsmre_cg10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "If the META is here, we'll run into her sooner or later. If she's already left, we'll gain nothing by searching for her.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_bsmre_cg10",
			paintingNoise = True,
			dir = 1,
			actor = 405030,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "...Alright. We'll head back to the Tower.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 405050,
			side = 2,
			bgName = "bg_bsmre_cg10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Meet me at the gate. I have a theory I'd like to prove.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9708010,
			nameColor = "#ffa500",
			bgName = "bg_bsmre_cg10",
			hidePaintObj = True,
			dir = 1,
			actorName = "U-556 META?",
			side = 2,
			say = "Are you sure about this, Lord Bismarck?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			},
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_bsmre_cg10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 405050,
			say = "It'll be fine. The corruption in the control center is:rmant for now.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 405050,
			side = 2,
			bgName = "bg_bsmre_cg10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Now, as for my theory... Use your communicator to relay a message to everyone.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_bsmre_cg10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 405050,
			say = "\"I strongly believe this Mirror Sea is structured like an hourglass.\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 405050,
			side = 2,
			bgName = "bg_bsmre_cg10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "\"We are going to test if the gate advances the passage of time and makes the red sea, symbolizing sand, expand or not.\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_bsmre_cg10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 405050,
			say = "\"If the red sea:es not expand, or if it thins out, that means we're on the top side of the hourglass.\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9708010,
			nameColor = "#ffa500",
			bgName = "bg_bsmre_cg10",
			hidePaintObj = True,
			dir = 1,
			actorName = "U-556 META?",
			side = 2,
			say = "Ah! So once all the sand is gone, we can safely investigate the city at the hourglass' throat!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 405050,
			side = 2,
			bgName = "bg_bsmre_cg10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I've not made my mind on that just yet. We'd need to find a way to get there, too.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_bsmre_cg10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 405050,
			say = "As I was saying... \"Conversely, if the red sea:es expand, that means we are on the bottom side.\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 405050,
			side = 2,
			bgName = "bg_bsmre_cg10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "\"We must find a way out of this place as soon as possible, before the corruption bears:wn on us.\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
