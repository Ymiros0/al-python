return {
	fadeOut = 1.5,
	mode = 2,
	id = "KONGXIANGJIAOHUIDIAN30",
	once = True,
	fadeType = 2,
	scripts = {
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_bsmre_10",
			stopbgm = True,
			say = "Following Bismarck's guidance, the girls all touched the control center's:me and were transported through the datastream.",
			bgmDelay = 2,
			bgm = "battle-thedevilXV-control",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_bsmre_10",
			say = "They carefully traversed it to its terminus and arrived beneath the inverted city, only to discover that it looked nothing like it did before.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 406030,
			say = "Oh no! What's wrong with this place? It looks totally corrupted!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 401990,
			say = "Yeah, what's going on? Everything here is dyed red!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 402100,
			say = "Are we too late after all?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 405030,
			say = "If The Devil has seized full control, there's no point in going after The Hermit's vessel.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 405030,
			say = "In that case, our only hope of ever escaping is to take on The Devil herself...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9712010,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a020f0",
			say = "I fear that may be the case.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a020f0",
			actor = 9712010,
			say = "If only I could reach Helena and ask for help...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "There's no going back to the control center now. It's: or die!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "My planes have spotted The Hermit's vessel! She's not far from here!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Although... is it just me, or:es she not look right?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_bsmre_10",
			say = "Zuikaku put on display the picture her aircraft took.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_bsmre_10",
			say = "The Arbiter vessel was glowing red and standing idly in the sea as though it was waiting on somebody.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900355,
			nameColor = "#ff5c5c",
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			actorName = "Arbiter. The Hermit IX?",
			side = 2,
			say = "......",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_bsmre_10",
			say = "The robotic yet intense drive its kin displayed the last time they fought one was completely gone.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 405050,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Just to confirm, Sirens: not have Wisdom Cube-powered hulls, and thus cannot be affected by METAmorphosis. Is that correct?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a020f0",
			actor = 9712010,
			say = "Yes. In theory, Sirens should immune to the effects of corrosion.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a020f0",
			actor = 9712010,
			say = "While they: utilize Cube technology, they only use it as a power source. It's not integral to their design.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9712010,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a020f0",
			say = "It may be possible to damage this power source, but that would have no bearing on the Siren's properties, let alone cause METAmorphosis.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 405050,
			say = "Both you and the Commander said that The Devil isn't like other Arbiters.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 405050,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "So if this isn't the effects of corruption, what is it:?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9712010,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a020f0",
			say = "Well, there's another theory about that... It basically holds that The Devil's erratic qualities are perfectly normal.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a020f0",
			actor = 9712010,
			say = "Just as their naming pattern implies, the Arbiters may have both upright and reversed forms.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9712010,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a020f0",
			say = "Under this theory, she is simply exhibiting the reversed characteristics of her Major Arcana.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a020f0",
			actor = 9712010,
			say = "Of course, there's no proof one way or the other, and the theory has holes.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a020f0",
			actor = 9712010,
			say = "(She's caught between the ideas of being a person and being a weapon, much like us.)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a020f0",
			actor = 9712010,
			say = "In any case, this vessel is a wholly different beast to the last one you fought.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9712010,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a020f0",
			say = "The Devil's support makes it many, many times more powerful than before.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a020f0",
			actor = 9712010,
			say = "The difference is so drastic, fighting this vessel would be like taking on two Arbiters at once.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a020f0",
			actor = 9712010,
			say = "(Worse yet, none of us here have fought a juiced-up Arbiter before...)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a020f0",
			actor = 9712010,
			say = "(If only I could reach Helena, or if Arizona was here with us...)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9712010,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a020f0",
			hasPaintbg = True,
			say = "(Or better yet, if I was only a more capable fighter...)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a020f0",
			actor = 9712010,
			say = "(Enty... Give me courage...)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "(Vestal looks like she's about to break:wn...) It's alright, Vestal! Let's think this through first!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "What if The Devil hasn't actually seized full control over this place yet, and is actually bluffing?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "If she's already seized full control, she wouldn't have any reason to care about us!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "She's just trying to stall for time! We can win if we take her on here and now!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9712010,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a020f0",
			say = "(Zuikaku...)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a020f0",
			actor = 9712010,
			say = "I... I think you're right.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a020f0",
			actor = 9712010,
			say = "The Devil is an incredibly cunning Arbiter. I certainly wouldn't put it past her to resort to deception...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a020f0",
			actor = 9712010,
			say = "I believe we can win. Let's take on this vessel!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 405050,
			say = "That's what I like to hear.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9708010,
			nameColor = "#ffa500",
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			actorName = "U-556 META?",
			side = 2,
			say = "Now it's our turn to go on the offensive!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 405050,
			say = "Yes. I've had enough of dancing in the Sirens' palms.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 405050,
			side = 2,
			bgName = "bg_bsmre_10",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Everyone! Commence the attack!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
