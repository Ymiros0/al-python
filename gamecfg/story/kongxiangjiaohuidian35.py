return {
	fadeOut = 1.5,
	mode = 2,
	id = "KONGXIANGJIAOHUIDIAN35",
	once = True,
	fadeType = 2,
	scripts = {
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_bsmre_14",
			stopbgm = True,
			say = "Somewhere, in an unknown location...",
			bgmDelay = 2,
			bgm = "theme-thetowerXVI",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_bsmre_14",
			say = "A young girl dressed in black stood in front of a control device, making changes of some sort.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_bsmre_14",
			say = "Then, she heard footsteps from behind her. Someone entered the room.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_bsmre_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 900356,
			say = "Devil, got a moment?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_bsmre_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 900356,
			say = "Didn't catch you at a bad time, did I?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900335,
			side = 2,
			bgName = "bg_bsmre_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Oh, it's you. Surprised to see you here.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_bsmre_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 900356,
			say = "I'M the one who's surprised.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_bsmre_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 900356,
			say = "Where have you been? I couldn't trace you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_bsmre_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 900356,
			say = "What are you:ing inside Hermit?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900335,
			side = 2,
			bgName = "bg_bsmre_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Ugh, give me a break...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900335,
			side = 2,
			bgName = "bg_bsmre_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Look, we have a lot in common,:n't we?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900335,
			side = 2,
			bgName = "bg_bsmre_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "So tell you what. I'll give up peacefully. Happy?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_bsmre_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 900356,
			say = "I'll let you off the hook this time, but you still have to repay me for breaking my lens.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900335,
			side = 2,
			bgName = "bg_bsmre_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Oh, for... That's your fault for letting rats inside.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_bsmre_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 900356,
			say = "They're chinchillas.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900335,
			side = 2,
			bgName = "bg_bsmre_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Same difference.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900335,
			side = 2,
			bgName = "bg_bsmre_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I've got important stuff to:, so let's just pretend we didn't see each other. Deal?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_bsmre_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 900356,
			say = "That depends on your answer to this next question.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_bsmre_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 900356,
			say = "Who's that behind you?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
