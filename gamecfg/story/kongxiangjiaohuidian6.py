return {
	fadeOut = 1.5,
	mode = 2,
	id = "KONGXIANGJIAOHUIDIAN6",
	once = True,
	fadeType = 2,
	scripts = {
		{
			actorName = "U-556",
			side = 2,
			bgName = "bg_bsmre_cg3",
			nameColor = "#A9F548FF",
			bgmDelay = 2,
			bgm = "story-midgard",
			stopbgm = True,
			hidePaintObj = True,
			say = "Lord Bismarck! Something's rising to the surface!",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_bsmre_cg3",
			hidePaintObj = True,
			say = "KABOOOM!",
			soundeffect = "event./battle/boom2",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashN = {
				color = {
					1,
					1,
					1,
					1
				},
				alpha = {
					{
						0,
						1,
						0.2,
						0
					},
					{
						1,
						0,
						0.2,
						0.2
					},
					{
						0,
						1,
						0.2,
						0.4
					},
					{
						1,
						0,
						0.2,
						0.6
					}
				}
			},
			dialogShake = {
				speed = 0.09,
				x = 8.5,
				number = 2
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_bsmre_cg3",
			hidePaintObj = True,
			say = "Before U-556 even had a chance to finish her warning, a fierce artillery barrage hurtled towards them from the horizon.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_bsmre_cg3",
			hidePaintObj = True,
			say = "The waters shook violently, but the shells all missed. Either the girls were out of range, or they weren't the intended target.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Regensburg",
			bgName = "bg_bsmre_cg3",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "What incredible power... We would've been:ne for if we got caught in that.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Regensburg",
			bgName = "bg_bsmre_cg3",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Jade! What's the status?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Jade",
			bgName = "bg_bsmre_cg3",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "I see... about a:zen mass-produced Siren ships. Where in the world did they come from?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "U-556",
			bgName = "bg_bsmre_cg3",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "There's a ton of enemies surfacing from beneath the waters!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			side = 2,
			actorName = "U-556",
			bgName = "bg_bsmre_cg3",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "They've even got submarines too! Be careful, everyone!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Jade",
			bgName = "bg_bsmre_cg3",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Seems we've walked into an ambush... Enemies are starting to close in on us from all directions!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Jade",
			bgName = "bg_bsmre_cg3",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "They're no pushovers either... Those are Type-IV Sirens!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Otto von Alvensleben",
			bgName = "bg_bsmre_cg3",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "The same model Ulrich fought when she made contact with the METAs, right?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			side = 2,
			actorName = "Otto von Alvensleben",
			bgName = "bg_bsmre_cg3",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Not only are they extremely dangerous, but they can also continue posing a serious threat even without a command unit!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Otto von Alvensleben",
			bgName = "bg_bsmre_cg3",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "There's no way we can handle those numbers all by ourselves!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Otto von Alvensleben",
			bgName = "bg_bsmre_cg3",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "This is bad. REALLY bad!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_bsmre_cg3",
			hidePaintObj = True,
			say = "KABOOOM!",
			soundeffect = "event./battle/boom2",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashN = {
				color = {
					1,
					1,
					1,
					1
				},
				alpha = {
					{
						0,
						1,
						0.2,
						0
					},
					{
						1,
						0,
						0.2,
						0.2
					},
					{
						0,
						1,
						0.2,
						0.4
					},
					{
						1,
						0,
						0.2,
						0.6
					}
				}
			},
			dialogShake = {
				speed = 0.09,
				x = 8.5,
				number = 2
			},
			effects = {
				{
					active = True,
					name = "speed"
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_bsmre_cg3",
			hidePaintObj = True,
			actorName = "Regensburg",
			say = "A second barrage is coming! Regina, cover me!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			effects = {
				{
					active = False,
					name = "speed"
				}
			}
		},
		{
			stopbgm = True,
			mode = 1,
			bgName = "bg_bsmre_cg4",
			bgm = "theme-bismark-reborn",
			flashin = {
				delay = 0.5,
				dur = 0.5,
				alpha = {
					1,
					0
				}
			},
			sequence = {
				{
					"",
					0
				}
			}
		},
		{
			side = 2,
			actorName = "Otto von Alvensleben",
			bgName = "bg_bsmre_cg4",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "What the...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Jade",
			bgName = "bg_bsmre_cg4",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Bismarck...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Regensburg",
			bgName = "bg_bsmre_cg4",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "What's going on?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_bsmre_cg4",
			hidePaintObj = True,
			say = "Explosions went off all around the girls like dazzling fireworks, but the devastating impact never reached them. Before them loomed a massive shadow that they had never seen before.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Bismarck Zwei",
			bgName = "bg_bsmre_cg4",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Geryon, become an iron temple! Do not allow harm to befall our friends!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Geryon",
			bgName = "bg_bsmre_cg4",
			hidePaintObj = True,
			nameColor = "#ffa500",
			say = "ROOOAAAR!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			},
			effects = {
				{
					active = True,
					name = "speed"
				}
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_bsmre_cg4",
			hidePaintObj = True,
			say = "The massive metallic beast spread its wings, the bombardment from the Sirens finding no purchase as shell after shell fizzled harmlessly.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			effects = {
				{
					active = False,
					name = "speed"
				}
			}
		},
		{
			side = 2,
			actorName = "U-556",
			bgName = "bg_bsmre_cg4",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Yeah! Show 'em how it's:ne, Geryon!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Regensburg",
			bgName = "bg_bsmre_cg4",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "\"Geryon\"? Is that what your new rigging is called?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Bismarck Zwei",
			bgName = "bg_bsmre_cg4",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Why so surprised? I'm far from the first to name my rigging.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Regensburg",
			bgName = "bg_bsmre_cg4",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Don't get me wrong, I like the name. But how in the world did you–",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Bismarck Zwei",
			bgName = "bg_bsmre_cg4",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "We'll talk about it later. A third barrage is coming.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_bsmre_cg4",
			hidePaintObj = True,
			say = "KABOOOM!",
			soundeffect = "event./battle/boom2",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashN = {
				color = {
					1,
					1,
					1,
					1
				},
				alpha = {
					{
						0,
						1,
						0.2,
						0
					},
					{
						1,
						0,
						0.2,
						0.2
					},
					{
						0,
						1,
						0.2,
						0.4
					},
					{
						1,
						0,
						0.2,
						0.6
					}
				}
			},
			dialogShake = {
				speed = 0.09,
				x = 8.5,
				number = 2
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_bsmre_cg4",
			hidePaintObj = True,
			say = "Bismarck once more pivoted into a defensive stance, protecting her allies while they took evasive maneuvers.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_bsmre_cg4",
			hidePaintObj = True,
			say = "Explosions once again filled the sky, but even the few shells that appeared on target did nothing more than reflect off Bismarck's rigging.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_bsmre_cg4",
			hidePaintObj = True,
			say = "What would have otherwise been certain destruction was turned into little more than a fireworks show beneath the massive iron wings.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_bsmre_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 406030,
			say = "Wooooah... What is that magnificent rigging?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_bsmre_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 406030,
			say = "I've never seen anything like it before...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_bsmre_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 401990,
			say = "I... I'm totally speechless...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			},
			effects = {
				{
					active = True,
					name = "speed"
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_bsmre_2",
			actor = 405050,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Ahem... Let's, uh, first get back into formation.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			effects = {
				{
					active = False,
					name = "speed"
				}
			}
		},
		{
			actor = 405050,
			side = 2,
			bgName = "bg_bsmre_2",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "(Impressive. So this was the final piece of the puzzle– a reinforced hull that harnesses conceptual energy...)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_bsmre_2",
			actor = 405050,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "(It has many limitations right now, but I can envision a future where everyone has access to this power.)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 408040,
			side = 2,
			bgName = "bg_bsmre_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "We finally get to witness Lord Bismarck's TRUE power!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_bsmre_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 408040,
			say = "The power of the Type Zwei!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_bsmre_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 401990,
			say = "You knew she had it all along?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_bsmre_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 408040,
			say = "Well, duh!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_bsmre_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 408040,
			say = "She wanted me to keep it a secret though, and what kind of knight would I be if I couldn't keep a promise?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 408040,
			side = 2,
			bgName = "bg_bsmre_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "And now that you all know, you also have to keep it a secret for now!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_bsmre_2",
			actor = 405050,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Yes. It's experimental and has hardly been tested, so I'd appreciate it if you'd keep quiet about it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_bsmre_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 402100,
			say = "So this is the absolute power of a superior being... I've seen the light...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_bsmre_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 408040,
			say = "Lord Bismarck, let's launch our counterattack!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_bsmre_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 408040,
			say = "Whatever happens, I'll fight with you until the very–",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_bsmre_2",
			hidePaintObj = True,
			say = "Just as U-556 started to cheerily approach Bismarck, she suddenly took a step back.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_bsmre_2",
			hidePaintObj = True,
			say = "A moment later, she staggered, letting out a scream as if her whole body had been wracked by an electric shock.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_bsmre_2",
			actor = 405050,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "U-556? What's wrong?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_bsmre_2",
			hidePaintObj = True,
			say = "A black mist crept up from the water beneath U-556 and started to coil around her body rapidly.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_bsmre_2",
			hidePaintObj = True,
			say = "KABOOOM!",
			soundeffect = "event./battle/boom2",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashN = {
				color = {
					1,
					1,
					1,
					1
				},
				alpha = {
					{
						0,
						1,
						0.2,
						0
					},
					{
						1,
						0,
						0.2,
						0.2
					},
					{
						0,
						1,
						0.2,
						0.4
					},
					{
						1,
						0,
						0.2,
						0.6
					}
				}
			},
			dialogShake = {
				speed = 0.09,
				x = 8.5,
				number = 2
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_bsmre_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 401990,
			say = "A FOURTH volley?! Why haven't they given up already?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			},
			effects = {
				{
					active = True,
					name = "speed"
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_bsmre_2",
			actor = 405050,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Damnit... Geryon! To me!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Geryon",
			bgName = "bg_bsmre_2",
			hidePaintObj = True,
			nameColor = "#ffa500",
			say = "ROOOAAAR!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			bgName = "bg_bsmre_cg5",
			mode = 1,
			flashin = {
				delay = 0.5,
				dur = 0.5,
				alpha = {
					1,
					0
				}
			},
			sequence = {
				{
					"",
					0
				}
			},
			effects = {
				{
					active = False,
					name = "speed"
				}
			}
		},
		{
			side = 2,
			actorName = "U-556?",
			bgName = "bg_bsmre_cg5",
			hidePaintObj = True,
			nameColor = "#ffa500",
			say = "Lord Bismarck...:n't worry... about me...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			side = 2,
			actorName = "U-556?",
			bgName = "bg_bsmre_cg5",
			hidePaintObj = True,
			nameColor = "#ffa500",
			say = "Protect... the others...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_bsmre_cg5",
			hidePaintObj = True,
			say = "Before the black mist could entirely engulf her, she mustered the last of her strength to extend her hand.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_bsmre_cg5",
			hidePaintObj = True,
			say = "Towards the one she vowed to protect – the one she admired so – the one she had already saved in the past, and was now trying to save once more.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_bsmre_cg5",
			hidePaintObj = True,
			say = "But, she could not reach. Her hand fell limp and disappeared beneath the waves.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
