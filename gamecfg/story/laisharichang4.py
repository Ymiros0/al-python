return {
	fadeOut = 1.5,
	mode = 2,
	id = "LAISHARICHANG4",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"The Alchemist and the Archipelago of Secrets - Peaceful Port Days\n\n<size=45>4 A Royal Tea Party</size>",
					1
				}
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_107",
			bgm = "story-1",
			say = "Port - Royal Navy:rmitory",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 207030,
			say = "We are honoured to have you at our tea party today, Miss Patricia.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 10900030,
			say = "Please, I'm just honored to have been invited.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 10900030,
			say = "All the more so considering you're said to be a very high-class lady, Miss Illustrious.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 207030,
			say = "Hehehe. You needn't be so modest. We are all friends here at port.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 202120,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Would you like some tea, Miss Patricia?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 10900030,
			say = "Yes, please. Thank you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_107",
			say = "Patricia takes a sip from her cup of tea.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 10900030,
			say = "Oh!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 10900030,
			say = "(It's hard to find tea this sublime even in the Royal Capital... These ladies know their stuff.)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 10900030,
			say = "(I can also tell it was intricately brewed based on this rich fragrance... That maid is definitely no ordinary housekeeper.)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 207030,
			say = "What: you think? I hope it is to your taste.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 10900030,
			say = "It's delicious. I feel incredibly privileged to get to drink something this wonderful!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 207030,
			say = "Hehe. It relieves me to know that you like it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 10900030,
			say = "By the way, Miss Illustrious, you are free to call me \"Patty\" instead.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 207030,
			say = "In that case, you're also free to simply call me \"Illustrious.\" On another subject,: you feel like you've grown accustomed to life here at port?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 10900030,
			say = "Yes, I:. I've settled in quite well, thanks to the Commander and so many others.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 207030,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "That is good to hear. If you ever need guidance, you may come talk to me whenever you like.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 202120,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I've brought biscuits. Would you like some, Miss Patricia?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 10900030,
			say = "Why, yes, I would. Thank you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_107",
			say = "The elegant and courtly tea party continues. Then, the still mood is disrupted by shouting.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 207050,
			say = "Illustrious! We've returned from our expedition! You're not going to believe what we saw... out...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			},
			effects = {
				{
					active = True,
					name = "speed"
				}
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 207050,
			say = "Oh...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			effects = {
				{
					active = False,
					name = "speed"
				}
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 207120,
			say = "Ahem. Good day to you, Miss Patricia. We appreciate that you could be our guest at this tea party.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 10900030,
			say = "Huh? Um... Thank you for having me.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 207030,
			say = "I presume you've already met Formidable. She is one of my sisters, as is this other lady. Her name is Indomitable.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 207030,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I have one more sister, Victorious, who unfortunately can't join us today as she is on a mission. I'd have loved to introduce her to you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 10900030,
			say = "Ah, I see... It's a pleasure to meet you, Miss Indomitable. I'm Patricia, one of the new residents here at port. If I may ask, what was Formidable about to say?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 207120,
			say = "N-nothing important, I'm sure! If you'll excuse me and her, we'll go and get changed now. Please: carry on with your conversation.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 10900030,
			say = "Okay...?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 10900030,
			say = "Apropos of nothing, Miss Ryza told me that Sardegna plans to open a hot spring. Have you ladies heard about that?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 207030,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "That we have. We've already made plans for after the hot spring has been excavated.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 207050,
			say = "There's also been talks of erecting a building where one can relax after they've finished bathing.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 207120,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Yes. Her Majesty has announced that the Royal Navy will supply the food and beverages as well.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 10900030,
			say = "Interesting. That being the case, I'd like to ask for a favor...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 10900030,
			say = "Would you let me assist in the hot spring's development? I want to help people out as much as I can.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 207030,
			say = "Of course you can! As a matter of fact, we've yet to reach an agreement with Sardegna regarding the decorations around the hot spring.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 207030,
			say = "I'd like you to handle that detail, if you feel up to the task.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 10900030,
			say = "Yes, I'd love to!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_107",
			say = "And so, the tea party became enriched with an air of enthusiasm.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
