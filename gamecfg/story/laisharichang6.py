return {
	fadeOut = 1.5,
	mode = 2,
	id = "LAISHARICHANG6",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"The Alchemist and the Archipelago of Secrets - Peaceful Port Days\n\n<size=45>6 Flowers</size>",
					1
				}
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_103",
			bgm = "story-1",
			say = "Port - Outskirts",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 10900050,
			say = "I've never seen a plant like this before... Nor this one...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 10900050,
			say = "I suppose it's to be expected that different worlds have different vegetation...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301820,
			say = "Um, hello. Are you looking for some flowers?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 10900050,
			say = "Yes, in fact. Who are you?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 8,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301820,
			say = "Oh, I should've introduced myself first! I'm Hanazuki from the Sakura Empire. If I remember correctly, you're... Miss Serri, right?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 10900050,
			say = "Yes. Nice to meet you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 10900050,
			say = "Actually... Earlier while I was looking for seeds, I remember hearing about a girl around port who's especially fond of flowers. I:n't suppose that's you they were talking about?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301820,
			say = "I'm humbled that you've heard of me.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301820,
			say = "Picking flowers is the reason I came out here, in fact. I'm working on a floral arrangement, but I'm struggling to decide on which species to use since there are so many...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 8,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301820,
			say = "Seeing as you're here as well, are flowers your hobby by any chance, Miss Serri?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 10900050,
			say = "Not exactly, but you're pretty close. I'm specifically looking for a flower with potent purifying properties.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 10900050,
			say = "Do you know any flowers like that, Hanazuki?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301820,
			say = "Purifying properties? Umm,: you mean something akin to an air freshener?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 8,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301820,
			say = "Then I would suggest this one, a species of aloe. You can leave it in a vase to freshen up the air in a room.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 10900050,
			say = "So this is what you call \"aloe\"... Interesting.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_103",
			say = "Serri inquisitively touches the flower,: softly shakes her head.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 10900050,
			say = "You're right, it probably would purify the air. However, that's not the kind of purifying I'm after.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301820,
			say = "Oh, I see... Well, you might find what you need not far from here.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 10900050,
			say = "I really:ubt I will. Ryza and I have only found a single flower with the right properties after we searched countless places.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301820,
			say = "That's... unfortunate.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 10900050,
			say = "We went through a lot in order to find that flower... Now, however, I look back fondly on our adventures.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 10900050,
			say = "Oh, that reminds me. You said you can't decide on a flower to use, right? How about this one?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 10900050,
			say = "While it hasn't fully bloomed yet, touching it fills me with a kind of warmth. I think it represents warm feelings of love and affection.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 8,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301820,
			say = "Ah, that flower is called a \"carnation.\" You know, I think that's quite a nice suggestion. I'll use this one.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301820,
			say = "Incidentally, how: you know about one of its symbolisms? Do carnations exist in your world, too?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 10900050,
			say = "It's an ability I have as a member of the Green Plume clan. We can figure out most things about plants by just looking at them.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 10900050,
			say = "...It's easier to show than to explain. Do you have some plant seeds on you?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301820,
			say = "I:, in fact. Here you go.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_103",
			say = "Hanazuki takes out a couple of seeds and hands them to Serri. She makes guess after guess at what they grow into and, miraculously, is correct every time.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301820,
			say = "Wow, that's incredible! I wish I had an ability like that...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 10900050,
			say = "I'd say you're already immensely knowledgeable about flowers even without my ability.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 8,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301820,
			say = "Aww, thank you. If I learn anything about a plant like what you're looking for, I'll be sure to tell you as soon as possible.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 10900050,
			say = "Thanks. I appreciate it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301840,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Hey, there you are, Hanazuki! I've been looking for you!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301820,
			say = "Suzutsuki? What's the matter?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301840,
			say = "Akashi and a whole bunch of people are building a new hot spring inn. They need someone who knows flowers, and I thought who'd be better for the job than you, right?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301840,
			say = "So, if you've got some time, could you give us a hand?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301820,
			say = "Of course. I'll pick some flowers right now and bring them with me!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 8,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301820,
			say = "Oh, and Miss Serri, would you mind accompanying me? I think I could: an even better job if I could ask for your wisdom.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 10900050,
			say = "Sure. Ryza told me about the project, and I want to help you all in any way I can.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_103",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301840,
			say = "Perfect! Follow me,:!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		}
	}
}
