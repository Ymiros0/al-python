return {
	fadeOut = 1.5,
	mode = 2,
	id = "LAIZIYUANWEIDETIANSHI2",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Angel of the Iris\n\n<size=45>2 The Knight-Princess and Her Sisters</size>",
					1
				}
			}
		},
		{
			side = 2,
			bgName = "bg_iris_dusk",
			dir = 1,
			bgmDelay = 2,
			bgm = "battle-pacific",
			actor = 907010,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "The party venue is just up ahead. Judging from how extravagant this place looks, there must be some really important people showing up.",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_iris_dusk",
			say = "Joffre stepped into a park near the building the party was being held in.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_iris_dusk",
			say = "As she walked through the greenery, she heard a set of voices coming from around a corner ahead of her.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			nameColor = "#a9f548",
			bgName = "bg_iris_dusk",
			hidePaintObj = True,
			dir = 1,
			actorName = "???",
			say = "This is not the place for that. Can't you at least hang in until we get to the waiting room?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_iris_dusk",
			hidePaintObj = True,
			dir = 1,
			actorName = "???",
			say = "No, I can't move a muscle... I'm too exhausted...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_iris_dusk",
			hidePaintObj = True,
			dir = 1,
			actorName = "???",
			say = "I need you to rub me:wn here... We're the only ones around, so nobody has to know.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_iris_dusk",
			hidePaintObj = True,
			dir = 1,
			actorName = "???",
			say = "Come on, please. Do it for your big sister.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			nameColor = "#a9f548",
			bgName = "bg_iris_dusk",
			hidePaintObj = True,
			dir = 1,
			actorName = "???",
			say = "Well... If you insist,: fine. Just relax.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_iris_dusk",
			hidePaintObj = True,
			dir = 1,
			actorName = "???",
			say = "Mmmm~ Oh, that feels so good~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 907010,
			side = 2,
			bgName = "bg_iris_dusk",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "What... the...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_iris_dusk",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 907010,
			say = "......",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_iris_dusk",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 907010,
			say = "What in the world are they:ing out in this park?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_iris_dusk",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 907010,
			say = "A little peek can't hurt, right...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_iris_dusk",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 901110,
			say = "Ohhh... Oww... It hurts... It hurts so good...!",
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_iris_dusk",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 901110,
			say = "Up here, please. Here, too... Yeah, rub 'em good~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 901130,
			side = 2,
			bgName = "bg_iris_dusk",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Your arms? As you say...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_iris_dusk",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 901130,
			say = "You know, I think this is excessive for a break. You should consider improving your physique and endurance.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_iris_dusk",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 901110,
			say = "Ehh...? I'm just keeping an eye on my energy expenditure though. Wouldn't want to run out of juice when you're on the battlefield, right?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 901130,
			side = 2,
			bgName = "bg_iris_dusk",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Oh, I see! That makes perfect sense!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_iris_dusk",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 901130,
			say = "Le Triomphant, let's both help her conserve her energy!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 801010,
			side = 2,
			bgName = "bg_iris_dusk",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Sorry, but no. Spoiling her any more than we already: would set a bad precedent.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 801010,
			side = 2,
			bgName = "bg_iris_dusk",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Could we please get going now? We must report to Lady Clemenceau as soon as possible.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_iris_dusk",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 901110,
			say = "I know, but I'm seriously too tired to take one more step...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_iris_dusk",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 901110,
			say = "Look, I'll: the hard part of the report if you just let me enjoy my massage for another five minutes.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 801010,
			side = 2,
			bgName = "bg_iris_dusk",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Well, I suppose I can't blame you for being exhausted... You worked harder than anyone else during this mission, after all.",:
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 801010,
			side = 2,
			bgName = "bg_iris_dusk",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Your calves look quite tense. Allow me to massage them.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_iris_dusk",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 901110,
			say = "Ah... I'm truly blessed to have sisters as great as you two...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 907010,
			side = 2,
			bgName = "bg_iris_dusk",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "So that's all it is... The Knight-Princess and her sisters have just come back from a mission.",
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_iris_dusk",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 907010,
			say = "Hard work:es not excuse sloth, for sloth is a deadly sin. If others heard of this, it could very well cancel out the merit she just earned.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_iris_dusk",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 907010,
			say = "However, seeing as nobody else is around...",:
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 907010,
			side = 2,
			bgName = "bg_iris_dusk",
			hidePaintObj = True,
			dir = 1,
			blackBg = True,
			nameColor = "#a9f548",
			say = "I think it'll be fine to turn a blind eye this time.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
