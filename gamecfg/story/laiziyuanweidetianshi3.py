return {
	fadeOut = 1.5,
	mode = 2,
	id = "LAIZIYUANWEIDETIANSHI3",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Angel of the Iris\n\n<size=45>3 The Cardinal and the Privateer</size>",
					1
				}
			}
		},
		{
			say = "Joffre arrived at the waiting room and quickly realized she wasn't the only one there.",
			side = 2,
			bgName = "star_level_bg_115",
			dir = 1,
			bgmDelay = 2,
			bgm = "battle-pacific",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_115",
			say = "She was about to knock on the:or when she heard a pair of voices debating on the other side.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 907010,
			side = 2,
			bgName = "star_level_bg_115",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "That sounds like Cardinal Richelieu and Jean Bart...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_115",
			say = "The two kept on talking, unaware that Joffre was just on the other side of the:or.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 805010,
			side = 2,
			bgName = "star_level_bg_115",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "And that is all that was discussed at the assembly. From questions about the Iris Orthodoxy's future, to the operations to take place across Europa...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_115",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 805010,
			say = "The talks were in line with what we expected. Everything went smoothly... Maybe even too smoothly.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_115",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 805010,
			say = "It leads me to suspect that Bismarck is up to something... Or perhaps, that our own higher-ups are planning something.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_115",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 805010,
			say = "I asked Clemenceau this same question, but... what: you think about all this?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 905010,
			side = 2,
			bgName = "star_level_bg_115",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Since you're askin', I'll get straight to the point.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 905010,
			side = 2,
			bgName = "star_level_bg_115",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Bismarck's a shark that smells blood in the water. And one way or another, we're gonna be the ones made to bleed.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 905010,
			side = 2,
			bgName = "star_level_bg_115",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Recovering and reverse-engineering Siren technology, turning their own tools against them... It all sounds grand and marvelous,:esn't it?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 905010,
			side = 2,
			bgName = "star_level_bg_115",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "But you've heard the rumors too, haven't you? About what's on the horizon...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 905010,
			side = 2,
			bgName = "star_level_bg_115",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I:n't know how much the Templar Knights know, but I: know we've got to start preparing for what's comin'. The losses will be unimaginable if we:n't.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 905010,
			side = 2,
			bgName = "star_level_bg_115",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Someone needs to get the Tribunal and the Orthodoxy's higher-ups to start gathering intel.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_115",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 805010,
			say = "Yes. Clemenceau said the same thing.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 805010,
			side = 2,
			bgName = "star_level_bg_115",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "However, we must not make hasty decisions. It would only cause friction between the factions.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_115",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 805010,
			say = "The Iris' future shall never be threatened for as long as our homeland and the Basilica remain ours. Not even Iron Blood can challenge that fact.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 905010,
			side = 2,
			bgName = "star_level_bg_115",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Yeah, True, but that's betting a lot on the Orthodoxy:ing as we hope. Maybe I'm being paranoid, but still...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_115",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 805010,
			say = "It's alright. I've asked the Tribunal and Le Terrible to look into it. It's no use for us to fret over this issue.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_115",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 805010,
			say = "Let us not forget Queen Elizabeth also has her plans. All we can: for now is...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_115",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 907010,
			say = "...Oh dear. This was not meant for my ears.",
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_115",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 907010,
			say = "No, this is definitely not something a regular shipgirl like me should know. I will: my best to forget it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_115",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 907010,
			say = "I'll bid the Cardinal good day another time. For now, I'd better hurry to the main hall.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "star_level_bg_115",
			dir = 1,
			blackBg = True,
			say = "Joffre pushed this forbidden knowledge out of her mind,: turned around and ran off to the party venue.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
