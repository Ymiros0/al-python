return {
	id = "LINGHANGYUANYANGCHENGJIHUA18",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_project_tb_room2",
			hidePaintObj = True,
			say = "TB's school entrance examination is coming up soon.",
			bgm = "qe-ova-1",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_project_tb_room2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(I should ask how her studies are going. Just to be sure she's:ing alright.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_project_tb_room2",
			hidePaintObj = True,
			say = "I walk over to her room and she lets me inside.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "TB",
			bgName = "bg_project_tb_cg13",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Um...",
			voice = "event./educate/tb/educate-tb-story-2-17",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			side = 2,
			actorName = "TB",
			bgName = "bg_project_tb_cg13",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I have no idea what stuff I'm supposed to go over.",
			voice = "event./educate/tb/educate-tb-story-2-18",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "TB",
			bgName = "bg_project_tb_cg13",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I've read tons of books, but I forget everything in them.",
			voice = "event./educate/tb/educate-tb-story-2-19",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "TB",
			bgName = "bg_project_tb_cg13",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "My teachers say I'm great at some subjects, but that's not enough because my other subjects are too far behind.",
			voice = "event./educate/tb/educate-tb-story-2-20",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_project_tb_cg13",
			hidePaintObj = True,
			say = "Seeing the grades on the tests on her desk, a chuckle escapes my lips.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "TB",
			bgName = "bg_project_tb_cg13",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It's NOT funny.",
			voice = "event./educate/tb/educate-tb-story-2-21",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_project_tb_cg13",
			hidePaintObj = True,
			say = "She pouts.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_project_tb_cg13",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "I'm just amazed that your grades are behind. How: I put it...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_project_tb_cg13",
			hidePaintObj = True,
			say = "I had assumed TB could: anything and everything. I never thought she'd get bad grades in certain subjects.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_project_tb_cg13",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Sorry, you're right. I shouldn't laugh.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_project_tb_cg13",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Let's review those subjects you're not so good at, shall we?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
