return {
	id = "LINGHANGYUANYANGCHENGJIHUA2",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_540",
			hidePaintObj = True,
			say = "I step into Akashi's workshop.",
			bgm = "story-richang-3",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 312010,
			side = 2,
			bgName = "star_level_bg_540",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Ah, Commander, nya. What can I: for you, nya?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_540",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "It's a lot to explain...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "(Tell her the gist of it.)",
					flag = 1
				}
			}
		},
		{
			actor = 312010,
			side = 2,
			bgName = "star_level_bg_540",
			hidePaintObj = True,
			dir = 1,
			optionFlag = 1,
			nameColor = "#A9F548FF",
			say = "Hmm. I see how it is, nya.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_540",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 312010,
			say = "You want to ease TB's woes about personality, nya.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_540",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 312010,
			say = "The thing is, an AI can't grow up if they:n't have a physical body to grow IN, nya. TB is not gonna develop a personality as it stands now, nya.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 312010,
			side = 2,
			bgName = "star_level_bg_540",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "But if making a physical body is off the table, making a virtual city for her to live in could: the trick, nya.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 312010,
			side = 2,
			bgName = "star_level_bg_540",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Then she'll experience what it's like to grow up and develop her own personality, little by little, nya.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "Can you make something like that?",
					flag = 1
				}
			}
		},
		{
			actor = 312010,
			side = 2,
			bgName = "star_level_bg_540",
			hidePaintObj = True,
			dir = 1,
			optionFlag = 1,
			nameColor = "#A9F548FF",
			say = "I can try, nya.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_540",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 312010,
			say = "However...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 312010,
			side = 2,
			bgName = "star_level_bg_540",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I want a teensy little thing in exchange, nya.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_540",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "And what would that be?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_540",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 312010,
			say = "9,999,999,999,999 coins, nya!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_540",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Nine TRILLION?! Are you kidding me?!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_540",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 312010,
			say = "Nya?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 312010,
			side = 2,
			bgName = "star_level_bg_540",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "That's my price after a generous discount, nya.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_540",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Dare I ask what the original price was?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_540",
			hidePaintObj = True,
			say = "I took the bait and asked the foolish question. Akashi cracks a grin,: whispers a number into my ear.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_540",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "......",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_540",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = ".........",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_540",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Fine, I'll take it! You have a deal!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 312010,
			side = 2,
			bgName = "star_level_bg_540",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "A snappy answer, just the way I like it, nya!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_540",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 312010,
			say = "Always a pleasure:ing business with you, nya.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 312010,
			side = 2,
			bgName = "star_level_bg_540",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Come back in a month and I'll have it ready for you, nya.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_540",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "A whole month? There's no way you need that much time.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_540",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 312010,
			say = "It's actually a lot less than I'd like to have, nya. This is complicated, intricate work we're talking about, nya.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_540",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "That's a shame. I suppose I'll take my business to da Vinci instead,:.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_540",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 312010,
			say = "I-I was just kidding, nya! Give me 20 days, no, 10 days is all I need, nya!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 312010,
			side = 2,
			bgName = "star_level_bg_540",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I'll come knocking with good news before you know it, nya!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
