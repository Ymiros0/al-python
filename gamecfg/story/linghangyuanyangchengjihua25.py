return {
	defaultTb = 1300,
	mode = 2,
	fadeOut = 1.5,
	id = "LINGHANGYUANYANGCHENGJIHUA25",
	scripts = {
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_151",
			hidePaintObj = True,
			say = "Winter has arrived in the virtual town.",
			bgm = "qe-ova-9",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 9,
			side = 2,
			bgName = "star_level_bg_151",
			tbActor = True,
			dir = 1,
			voice = "event./educate/tb/educate-tb-33-laugh5",
			actor = 1300,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Look! It's snowing!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 9,
			side = 2,
			bgName = "star_level_bg_151",
			tbActor = True,
			dir = 1,
			voice = "event./educate/tb/educate-tb-33-laugh2",
			actor = 1300,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Can you believe it's snowing, right on Christmas? If that ain't a good omen, I dunno what is!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_151",
			hidePaintObj = True,
			say = "TB is glued to the window. Her eyes in the reflection glitter like stars.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 9,
			side = 2,
			bgName = "star_level_bg_151",
			tbActor = True,
			dir = 1,
			voice = "event./educate/tb/educate-tb-33-talking3",
			actor = 1300,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "But we haven't put up the Christmas decorations yet... Time is running short! Let's go out and buy them, now!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_151",
			hidePaintObj = True,
			say = "I:n't even have time to respond before she grabs me by the hand and drags me outside.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_151",
			hidePaintObj = True,
			say = "When we return home, I barely have the strength left to even shuffle over to the couch and sink into it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_151",
			hidePaintObj = True,
			say = "TB, meanwhile, is zipping and zooming around the house, putting up decorations here and there. It's like that whole outing didn't drain a drop of her energy.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_151",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Four hours... Four whole hours spent running around without a minute to rest...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_151",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "And you're not tired in the slightest...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_151",
			tbActor = True,
			dir = 1,
			voice = "event./educate/tb/educate-tb-33-answer4",
			actor = 1300,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Do I LOOK tired? I told you you need to exercise more! This is what happens when you skip leg day!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 9,
			side = 2,
			bgName = "star_level_bg_151",
			tbActor = True,
			dir = 1,
			voice = "event./educate/tb/educate-tb-33-answer8",
			actor = 1300,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "I'll make the place look nice and festive. You just stay there and rest, okay?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_151",
			hidePaintObj = True,
			say = "With speed and moxie, she once more makes off like the wind into another room to decorate.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_151",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "I: desperately need to rest, but I'll:ze off if I sit here for too long...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_151",
			tbActor = True,
			dir = 1,
			voice = "event./educate/tb/educate-tb-33-talking1",
			actor = 1300,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Wake up. Hey, wake up.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 10,
			side = 2,
			bgName = "star_level_bg_151",
			tbActor = True,
			dir = 1,
			voice = "event./educate/tb/educate-tb-33-talking2",
			actor = 1300,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "I said wake up!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_151",
			hidePaintObj = True,
			say = "Still half-asleep and out of it, I hear TB calling out to me.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "TB",
			bgName = "bg_project_tb_cg19",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Tadah! TB Claus is here in style!",
			voice = "event./educate/tb/educate-tb-story-33-4",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			side = 2,
			actorName = "TB",
			bgName = "bg_project_tb_cg19",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Heehee. What: you think? Don't I look Christmassy as anything?",
			voice = "event./educate/tb/educate-tb-story-33-5",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_project_tb_cg19",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(She's all but begging for compliments.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "TB",
			bgName = "bg_project_tb_cg19",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "This is for you!",
			voice = "event./educate/tb/educate-tb-story-33-6",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_project_tb_cg19",
			hidePaintObj = True,
			say = "She presents a small box, lovingly wrapped and tied up in a nice bow.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "TB",
			bgName = "bg_project_tb_cg19",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "The decorations and stuff basically cost my whole allowance, so this was all I could afford.",
			voice = "event./educate/tb/educate-tb-story-33-7",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "TB",
			bgName = "bg_project_tb_cg19",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "But I spent what was left very wisely! It's not the most lavish gift, but that:esn't mean there's not a lot of thought behind it!",
			voice = "event./educate/tb/educate-tb-story-33-8",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_project_tb_cg19",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Of course. I appreciate any gift I get from you, TB Claus.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "TB",
			bgName = "bg_project_tb_cg19",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Heheh! That's what I want to hear!",
			voice = "event./educate/tb/educate-tb-story-33-9",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "TB",
			bgName = "bg_project_tb_cg19",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Merry Christmas!",
			voice = "event./educate/tb/educate-tb-story-33-10",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_project_tb_cg19",
			hidePaintObj = True,
			say = "It's a lively Christmas, and TB and I celebrate it together.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
