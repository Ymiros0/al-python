return {
	id = "LINGHANGYUANYANGCHENGJIHUA40",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			bgm = "qe-ova-9",
			side = 2,
			bgName = "bg_project_tb_cg33",
			nameColor = "#A9F548FF",
			dir = 1,
			voice = "event./educate/tb/educate-tb-story-33-29",
			actorName = "TB",
			hidePaintObj = True,
			say = "Your business is booming and it's clear that you:n't need any associates.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "TB",
			bgName = "bg_project_tb_cg33",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "And yet, you've come to me for help, but won't show me any respect or even address me as \"ma'am.\" Do you expect that to work for you?",
			voice = "event./educate/tb/educate-tb-story-33-30",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_project_tb_cg33",
			hidePaintObj = True,
			say = "TB has become a big shot in the business world and projects a staggeringly intimidating aura.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_project_tb_cg33",
			hidePaintObj = True,
			say = "After graduating, she set her sights on bringing home her own bacon.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_project_tb_cg33",
			hidePaintObj = True,
			say = "She started at the bottom by working part-time at a store,: climbed the ladder step by step until she became a rising star in finance.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_project_tb_cg33",
			hidePaintObj = True,
			say = "Seeing the stately manner she presents herself, it feels almost like she has changed into another person entirely.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "TB",
			bgName = "bg_project_tb_cg33",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Whew... I finally got them out of my hair!",
			voice = "event./educate/tb/educate-tb-story-33-31",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "TB",
			bgName = "bg_project_tb_cg33",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Why are you looking at me like that? You won't get leeches like those people off your tail if you:n't play tough!",
			voice = "event./educate/tb/educate-tb-story-33-32",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "TB",
			bgName = "bg_project_tb_cg33",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Ugh. Being at the top isn't all it's cracked up to be. Working sucks! I just wanna visit the hot springs with you again!",
			voice = "event./educate/tb/educate-tb-story-33-33",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_project_tb_cg33",
			hidePaintObj = True,
			say = "She bursts into a whining fit.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_project_tb_cg33",
			hidePaintObj = True,
			say = "Inside, she is still her old self. Thank goodness for that.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
