return {
	fadeOut = 1.5,
	mode = 2,
	id = "LINGSHIGUANGTING1",
	once = True,
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			say = "*knock knock*",
			side = 2,
			bgName = "bg_endingsong_3",
			dir = 1,
			bgmDelay = 1,
			bgm = "theme-partydress",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			say = "*knock knock knock*",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205020,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Strange... Her Majesty should be in her room with Belfast around this hour...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205020,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Pardon, Your Majesty. I'm coming in.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			say = "Warspite opened the:or to Queen Elizabeth's lavish office.",
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			say = "However, the queen was not there, and neither was the maid who accompanies her almost at all times.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			say = "She searched all over Scapa Flow, but could not find a trace of Her Majesty. The concerned Warspite called for a meeting.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_endingsong_3",
			dir = 1,
			bgmDelay = 1,
			bgm = "xinnong-3",
			actor = 205020,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Yesterday, the ceremony to celebrate Scapa Flow's reconstruction was cancelled. Today, something even worse has happened...",
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205020,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Her Majesty has gone missing, along with Belfast, Sheffield, and several of our blueprint ships.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205020,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Now, this has happened before. During the last major operation at Scapa Flow, she took the Royal Maids with her to survey a Singularity.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205020,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "This time is different, however. She's left no written message explaining her absence. Assuming she has already left these islands, she could be anywhere right now.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205020,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Ladies of the Royal Knights,: you have any idea where she may have gone?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205050,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Afraid not. As you said, Her Majesty took the Royal Maids with her during her expedition into that Singularity. The Royal Knights entrust them with matters of secrecy by a matter of precedent.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205050,
			say = "Vanguard, you're part of Her Majesty's palace guard. Did she tell you anything before she left?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205130,
			say = "She did not. Being the head of the patrol fleets here, I can however say that she did not sail out of Scapa Flow.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205130,
			say = "Yesterday's air raid put us on high alert, so I deployed additional patrols around the area.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205130,
			say = "Even if she snuck out very early in the morning, there is not a chance she could've left undetected.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205130,
			say = "Especially not considering she made off with an entourage of maids and blueprint ships.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205050,
			say = "The fact she took the blueprint ships with her is puzzling. The Royal Maids should be plenty for a covert operation.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205020,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "So, if nobody saw her leave Scapa Flow, yet she is nowhere to be seen, that leaves only one possibility... The Singularity.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205050,
			say = "You're saying she went through the gate to that Mirror Sea we've discovered?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205050,
			say = "\"Camelot,\" was it? Why would Her Majesty go there?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205020,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "After the battle, we continued investigating the Mirror Sea and found a teleportation device that seems to lead to a number of other places.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205020,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "The name Camelot applies to not only the Mirror Sea, but also to the teleportation device and all facilities connected to it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205020,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "And the way to it is through the gate we have here at Scapa Flow.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205050,
			say = "In essence, our gate here at Scapa Flow is the bridge to a hub world of sorts?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205020,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Correct. I suspect the reason Her Majesty left,:, is to search for a cure for Hood's illness in Siren territory.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205050,
			say = "In that case, I have a theory as to why she took those blueprint ships with her.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205050,
			say = "If we assume that the Black Cube corrupts the foundation of our hulls, the defense mechanisms of regular shipgirls like us may be insufficient to deal with these effects.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205050,
			say = "On the other hand, the hulls of blueprint ships are imprinted in a completely different manner that may allow them to fare better.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205050,
			say = "Do you think Her Majesty planned this expedition in advance, and that's why she invited them to the ceremony?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205020,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I wouldn't go that far. Her Majesty couldn't predict Hood would become bedridden from her illness.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205050,
			say = "True. If Hood had a say in the matter, she would never let others risk their well-being for her sake. Yet, we cannot simply stand by and watch.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205020,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Of course not. But rushing on an expedition into a Mirror Sea with no support is just reckless.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205050,
			say = "I agree. This is quite unlike how she conducted herself during the last operation.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205020,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "That one was much better organized. Her Majesty had the Iron Blood's cooperation as well as information about the META, giving the Royal Maids a chance to conduct a preliminary survey.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205020,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Their thorough preparation was the only reason the Naval HQ was deceived into thinking that Friedrich had captured Her Majesty.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205050,
			say = "Indeed. This time, she has no such advantages, and we have no idea where Camelot leads to.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205050,
			say = "She must have a good reason to have:ne all this...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205020,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Last evening, I informed her it would take very long to survey Camelot. I hope that had nothing to: with it...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205020,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Let us make:ubly sure before acting. Vanguard, have you heard of unusual activity in any of Scapa Flow's facilities?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205130,
			say = "I have not. The guards I assigned to this building claim Her Majesty retired here for the night without incident. Naval HQ's representatives have:ne nothing out of the ordinary, either.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205050,
			say = "It seems Camelot is our only bet,:. We've no other leads to go on.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205130,
			say = "Let us not be too hasty, King George.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205130,
			say = "If we find Her Majesty in Camelot, all is fine. However, if she has used the teleporter to go to a different Mirror Sea,: what we're facing is a major emergency.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205050,
			say = "Precisely, which is why I'm so concerned about Her Majesty's wellbeing.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205130,
			say = "You misunderstand. I am saying we must not act without thinking first.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205020,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Vanguard is right. We need to plan ahead for the possibility that Her Majesty cannot be located within Camelot.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205130,
			say = "To add to that, Naval HQ would become deeply suspicious if both you and King George went missing in addition to Her Majesty.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205130,
			say = "So allow me to make a proposition – you two shall stay here, and I shall lead a small search party into Camelot.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205130,
			say = "My fleetmates and I make up a formal Mirror Sea expedition fleet. We've been briefed on them and should be able to respond flexibly to whatever we encounter.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205130,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Upon my honour, we will find and bring Her Majesty back safely!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205130,
			say = "In the meanwhile, I ask that you provide us with all the support you can, and that you keep the gate to Camelot guarded and maintained.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205050,
			say = "Hmm... You make a good case. I'm in favour of entrusting the task to a select few specialists.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205050,
			say = "Sending in all the search parties we can muster wouldn't be more efficient. It would only result in chaos and many casualties.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205020,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Indeed it would. Vanguard, you face a monumental task, but I put my faith in you.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_task",
			dir = 1,
			bgmDelay = 2,
			bgm = "blueocean-image",
			actor = 205050,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Seems we have our Mirror Sea expedition team. Vanguard, Indomitable, Jervis, Janus, Revenge, and Bellona.",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205020,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I should feel confident that Vanguard is leading this fleet, but I still can't shake my sense of unease.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205020,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I fear that she may lose herself in the fighting and lose sight of our goal...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205050,
			say = "You've nothing to worry about. Yes, she rarely gets to fight and becomes a little overexcited as a result. But...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205050,
			say = "In times of crisis, she knows what must be:ne and she:es it. She wouldn't put herself over what's important.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205050,
			say = "Now, changing topics... We need to come up with an excuse to explain Hood's and Her Majesty's absence.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205020,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I'll deploy a reserve fleet outside Camelot, just in case we need it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205020,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "The fact remains that it's Siren territory, even if we have explored Camelot before. Better to be safe than sorry.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			mode = 1,
			stopbgm = True,
			sequence = {
				{
					"It wasn't long ago the events at Scapa Flow unfolded.",
					1
				},
				{
					"The Iron Blood rebelled against the Sirens, igniting a battle that would finally bring an end to one of their leaders.",
					2
				},
				{
					"This sent shockwaves across not just the Azur Lane and the Crimson Axis, but the entire world.",
					3
				},
				{
					"While they all processed the news, the Royal Navy embarked on a secret expedition into their Singularity.",
					4
				},
				{
					"Now they set out once more, in search of a cure for the Black Cube's corruption.",
					5
				},
				{
					"Vanguard leads the foray into the unknown, in pursuit of the missing queen.",
					6
				},
				{
					"What awaits them inside the fabled land of Camelot?",
					7
				},
				{
					"The Holy Grail? Or something even greater?",
					8
				}
			}
		},
		{
			stopbgm = True,
			mode = 1,
			blackBg = True,
			effects = {
				{
					active = True,
					name = "LINGSHIGUANGTING"
				}
			},
			sequence = {
				{
					"",
					2
				}
			}
		}
	}
}
