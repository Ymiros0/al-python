return {
	fadeOut = 1.5,
	mode = 2,
	id = "LINGSHIGUANGTING12",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			expression = 3,
			side = 2,
			bgName = "bg_camelot_7",
			dir = 1,
			bgmDelay = 1,
			bgm = "theme-royalknights-battle",
			actor = 205130,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Oi! It's Purifier!",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 900233,
			side = 2,
			bgName = "bg_camelot_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Yeah, it's me, what's up.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_camelot_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 900233,
			say = "Don't answer that – what's up is, on top of you being late, you smashed up like six thousand ships when we were SUPPOSED to: this on the quiet!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_camelot_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 900233,
			say = "I suffered the headache of plotting a safe route for you, and you pay me back by putting an entire patrol fleet outta commission!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_camelot_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 900233,
			say = "What, did you think nobody would realize if you just sank everything in sight? Assholes!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_camelot_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205130,
			say = "What in god's name is she on about?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_camelot_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 900233,
			say = "*sigh*... What's:ne is:ne, yada yada. Let's get this show on the road.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_camelot_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 900233,
			say = "Put on your booties, stow away your planes, and form a line. We're going on a field trip, children.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_camelot_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 900233,
			say = "You! The destroyer! ...Are you even a destroyer? Hella stacked for one.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_camelot_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 900233,
			say = "Hang on, this ain't the lineup I heard about. All right, roll call! Raise your hand when you hear your name!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_camelot_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 900233,
			say = "Renown! Repulse! Vanguard! Jervis!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_camelot_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 900233,
			say = "...Some of you called in sick, huh. Oh well, whatever.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_camelot_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 900233,
			say = "Listen up – we're going straight to Point Berth. No detours, no bathroom breaks, nothing.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_camelot_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 900233,
			say = "You're under Antiochus' command while you're here. Don't start anything if you:n't wanna sully the good Doctor's name any further.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_camelot_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 900233,
			say = "C'mon, it's time to go see Elizabeth. Y'all ready?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 207120,
			side = 2,
			bgName = "bg_camelot_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Umm... Was she talking to us this whole time?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_camelot_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205130,
			say = "No clue. For all I know, she just spent the last minute monologuing.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_camelot_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205130,
			say = "All I caught was \"Elizabeth.\" Maybe she can point us in the general direction of Her Majesty...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_camelot_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205130,
			say = "All right, ladies. Let us follow this Siren and see what she has in store for us.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_camelot_7",
			dir = 1,
			blackBg = True,
			actor = 900233,
			nameColor = "#ffff4d",
			hidePaintObj = True,
			say = "There ya go. Follow the teacher, children. AND DON'T! SHOOT! ANYTHING! I swear to god if you:!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
