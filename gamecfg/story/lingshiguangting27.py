return {
	fadeOut = 1.5,
	mode = 2,
	id = "LINGSHIGUANGTING27",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			say = "Mirror Sea - Forest of Forgetfulness",
			side = 2,
			bgName = "bg_camelot_14",
			dir = 1,
			bgmDelay = 1,
			bgm = "theme-camelot",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900327,
			side = 2,
			bgName = "bg_camelot_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "...Looks like the visions are over. We're back, sis.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_camelot_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 900326,
			say = "What a relief. We're back to reality.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_camelot_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 900326,
			say = "Do you think Vanguard's team has come back? Based on the fact their memories were among those we witnessed.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_camelot_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 900327,
			say = "Can't say. All I know is the situation's dire. We need to find and warn them about this.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_camelot_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 900326,
			say = "You're right. We must head for their gate posthaste.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900327,
			side = 2,
			bgName = "bg_camelot_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "...Look! Someone just came through it!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_camelot_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 900326,
			say = "Oh! It must be Vanguard and company!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900327,
			side = 2,
			bgName = "bg_camelot_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "No, not quite... Their numbers and looks:n't match.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_camelot_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 900326,
			say = "Numbers? But if it isn't them, who could it–",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_camelot_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 900327,
			say = "It's Warspite?! Plus Sheffield and some others!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_camelot_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 900327,
			say = "Phew. If I had to guess, I think they're reinforcements from Vanguard's branch.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900326,
			side = 2,
			bgName = "bg_camelot_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "We're in luck,:! We'd better go introduce our–",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900327,
			side = 2,
			bgName = "bg_camelot_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "Whoa, whoa, sis! Wait!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_camelot_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 900326,
			say = "What? What's the matter?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900327,
			side = 2,
			bgName = "bg_camelot_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "Did you forget we're META? And Ashes members, to boot? They'll shoot at us if we're not careful!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_camelot_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 900326,
			say = "Oh dear. You're right. But: what should we...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_camelot_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 900326,
			say = "Wait, I know! Use the broom again! Pretend you're sweeping the ground!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_camelot_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 900326,
			say = "Once they've established you're not a threat, I'll come forth and explain everything!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_camelot_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 900327,
			say = "It's a silly plan... but I think it's going to work!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_camelot_14",
			dir = 1,
			blackBg = True,
			actor = 900327,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "Let's be quick,: find Vanguard's team and help them!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
