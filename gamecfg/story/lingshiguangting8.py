return {
	fadeOut = 1.5,
	mode = 2,
	id = "LINGSHIGUANGTING8",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			nameColor = "#a9f548",
			side = 2,
			bgName = "bg_camelot_6",
			dir = 1,
			bgmDelay = 1,
			bgm = "theme-richard",
			actor = 900322,
			actorName = "???",
			hidePaintObj = True,
			say = "Hmm? Are you all alone out here?",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_camelot_6",
			say = "From the veil of black smoke suddenly emerged a white-haired girl clad in some type of naval uniform, facing Monarch.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 299020,
			side = 2,
			bgName = "bg_camelot_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "(Did I imagine it, or did her appearance change for just a flash?!)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900322,
			nameColor = "#ffff4d",
			bgName = "bg_camelot_6",
			hidePaintObj = True,
			dir = 1,
			actorName = "???",
			side = 2,
			say = "Figures my disguise:esn't work on you. You've got a sharp eye.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 299020,
			side = 2,
			bgName = "bg_camelot_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Who are you? An elite Siren?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900322,
			nameColor = "#ffff4d",
			bgName = "bg_camelot_6",
			hidePaintObj = True,
			dir = 1,
			actorName = "???",
			side = 2,
			say = "Hey now, you're on my turf, so at least tell me your own name first. It's only polite, right?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 299020,
			side = 2,
			bgName = "bg_camelot_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I am the Royal Navy blueprint ship Monarch. Now state your name.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_camelot_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Sure. I'm Bon Homme Richard, and this is my home for now. What are you:ing here?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 299020,
			side = 2,
			bgName = "bg_camelot_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "(She:esn't appear to be a Siren, but she may be a Pawn. I'll want to keep my distance, just to–)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_camelot_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Be safe? No need to, I'm not a threat. We're both KAN-SEN, you and I!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 299020,
			side = 2,
			bgName = "bg_camelot_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "(She read my mind?! Whoever this girl is, she is no ordinary shipgirl. Perhaps she's META?)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 299020,
			side = 2,
			bgName = "bg_camelot_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "There's no use beating around the bush,:. I'm searching for a black Wisdom Cube, or rather, the remedy to the effects of one.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_camelot_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Oh! Guess what, you're in luck! You're talking to the world expert on METAmorphosis right now!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_camelot_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "METAmorphosis is a restructuring of the hull, whether or not induced by a Black Cube. It's a one-way trip with no turning back.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_camelot_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "I could explain all day, but as they say – show,:n't tell. So let me show you what it feels like.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_camelot_6",
			say = "The girl broke out into a sinister grin that only grew wider as she spoke more.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 299020,
			side = 2,
			bgName = "bg_camelot_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "You're right. Actions speak more than words, and hands-on experience teaches best.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 299020,
			side = 2,
			bgName = "bg_camelot_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "But I'll pass on your twisted offer. I've no interest in falling victim to METAmorphosis! Now get out of my way!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_camelot_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Hahaha! No, I get it. It's not like YOU could experience it just by wishing for it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_camelot_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Your hull is very special, for better or worse. You need a little help to jumpstart the process.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_camelot_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "See, you have hardly any essence compared to ordinary shipgirls. Which basically means, you're a tool to be used.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 299020,
			side = 2,
			bgName = "bg_camelot_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "You have some nerve. I'm not a tool, and I'm not being used.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_camelot_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Yes you are, and we both know it. Let's be real here.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_camelot_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "You know what your special status as a blueprint ship entails. You are an outcast, a defective monster kept on a tight leash for when your firepower is needed... Only to be discarded afterwards.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 299020,
			side = 2,
			bgName = "bg_camelot_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Lies! I'm here right now entirely of my own volition!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_camelot_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Because you're a tool! You chase \"glory,\" but what IS glory? Medals? Money? Oh, I know – it's in pursuit of the accomplishments your hull lacks in comparison to your peers!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_camelot_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "But:n't you worry, lady. I shall give all that you ever want, and more.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_camelot_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Succumb. All you need to reshape reality is enough power. Come, and I shall give you just that!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_camelot_6",
			say = "The more the girl spoke, the more of her unassuming demeanor peeled off to reveal the insanity beneath. The sea around her churned, birthing a vortex that grew increasingly violent.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_camelot_6",
			say = "Richard's lunacy invoked an instinctual fear in Monarch. She knew she had to get out of here – now.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_camelot_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#BDBDBD",
			say = "Don't leave already. Stay here and have fun with me.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 299020,
			side = 2,
			bgName = "bg_camelot_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Urgh? What?! Her voice is...?!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 299020,
			side = 2,
			bgName = "bg_camelot_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "GET OUT OF MY HEAD!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			say = "KABOOOM!",
			side = 2,
			bgName = "bg_camelot_6",
			dir = 1,
			soundeffect = "event./battle/boom2",
			flashN = {
				color = {
					1,
					1,
					1,
					1
				},
				alpha = {
					{
						0,
						1,
						0.2,
						0
					},
					{
						1,
						0,
						0.2,
						0.2
					},
					{
						0,
						1,
						0.2,
						0.4
					},
					{
						1,
						0,
						0.2,
						0.6
					}
				}
			},
			dialogShake = {
				speed = 0.09,
				x = 8.5,
				number = 2
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_camelot_6",
			say = "Monarch's voice boomed, and so too did her main cannons. She scored a crippling direct hit on Richard... or so she thought.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_camelot_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#BDBDBD",
			say = "Nice shot there, lady. Just imagine what a devastating weapon you could be if you became META...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_camelot_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#BDBDBD",
			say = "Hahaha... Ahahahah! AHAHAHAHAHA! Light me up! Reduce me to a pile of ash!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_camelot_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#BDBDBD",
			say = "Release all your pent-up loneliness, bitterness, stress, and anger! Take it all out on me! Don't think, just SHOOT ME!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			say = "KABOOOM!",
			side = 2,
			bgName = "bg_camelot_6",
			dir = 1,
			soundeffect = "event./battle/boom2",
			flashN = {
				color = {
					1,
					1,
					1,
					1
				},
				alpha = {
					{
						0,
						1,
						0.2,
						0
					},
					{
						1,
						0,
						0.2,
						0.2
					},
					{
						0,
						1,
						0.2,
						0.4
					},
					{
						1,
						0,
						0.2,
						0.6
					}
				}
			},
			dialogShake = {
				speed = 0.09,
				x = 8.5,
				number = 2
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_camelot_6",
			say = "A moment's hesitation held back Monarch's trigger finger before she discharged the next shot, this time hitting the water to the left of Richard.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_camelot_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#BDBDBD",
			say = "That one missed. You're holding yourself back, lady. Or maybe you just:n't know where to shoot a person?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 299020,
			side = 2,
			bgName = "bg_camelot_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Shut your gob!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_camelot_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#BDBDBD",
			say = "It's no big deal. I'll teach you the ABC of it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_camelot_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#BDBDBD",
			say = "This is the head, this is the heart. If you want someone dead, those are where you start. Easy to remember, right?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			say = "KABOOOM!",
			side = 2,
			bgName = "bg_camelot_6",
			dir = 1,
			soundeffect = "event./battle/boom2",
			flashN = {
				color = {
					1,
					1,
					1,
					1
				},
				alpha = {
					{
						0,
						1,
						0.2,
						0
					},
					{
						1,
						0,
						0.2,
						0.2
					},
					{
						0,
						1,
						0.2,
						0.4
					},
					{
						1,
						0,
						0.2,
						0.6
					}
				}
			},
			dialogShake = {
				speed = 0.09,
				x = 8.5,
				number = 2
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_camelot_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#BDBDBD",
			say = "There you go! Now we're cooking!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_camelot_6",
			hidePaintObj = True,
			dir = 1,
			blackBg = True,
			nameColor = "#BDBDBD",
			say = "Still, I can sense you're clinging on to a shred of sanity... Fine, have it your way! Let's throw some friends into the mix!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
