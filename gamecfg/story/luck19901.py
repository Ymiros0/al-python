return {
	fadeOut = 1.5,
	mode = 2,
	id = "LUCK19901",
	once = True,
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			actor = 199010,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "Oh yeah, it's all coming together! Really feeling like I could take on the world right now! Thanks for everything, Commander!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			nameColor = "#a9f548",
			side = 2,
			actor = 199010,
			dir = 1,
			say = "Y'know, I've been thinkin' it's time to get everyone together and throw the most hype party ever!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			nameColor = "#a9f548",
			side = 2,
			actor = 199010,
			dir = 1,
			say = "Buuut... Feels like we're not 100%:ne, y'know? Like we've still got some potential left in me to wring out.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			nameColor = "#a9f548",
			side = 2,
			actor = 199010,
			dir = 1,
			say = "So let's put that party to end all parties on hold for now and just have ourselves a medium-sized one instead.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 199010,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "I'll: the prep, and you go give that Fate Simulation thing a look, Commander!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
