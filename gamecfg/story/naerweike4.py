return {
	fadeOut = 1.5,
	mode = 2,
	id = "NAERWEIKE4",
	once = True,
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			actor = 201160,
			nameColor = "#a9f548",
			side = 0,
			bgm = "battle-boss-1",
			dir = 1,
			say = "It's a good job we've got this heavy snow. Lets us slip right by the enemy's patrol destroyers.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 201200,
			side = 0,
			nameColor = "#a9f548",
			dir = 1,
			say = "Yes. Lucky.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			nameColor = "#a9f548",
			side = 0,
			actor = 201160,
			dir = 1,
			say = "Right, But... with guards this close, it seems Iron Blood really has taken Narvik... Damn.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			nameColor = "#a9f548",
			side = 0,
			actor = 201200,
			dir = 1,
			say = "Relax. Soften them up with a coastal bombardment and we should be good.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			nameColor = "#a9f548",
			side = 0,
			actor = 201160,
			dir = 1,
			say = "You know, Hunter, you always say I'm easily riled, but every now and: you say some pretty excitable things yourself.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 201200,
			side = 0,
			nameColor = "#a9f548",
			dir = 1,
			say = "Patience and perseverance are critical to the hunt.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 201160,
			side = 0,
			nameColor = "#a9f548",
			dir = 1,
			say = "I-Is that right...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			nameColor = "#a9f548",
			side = 0,
			actor = 201160,
			dir = 1,
			say = "Well:, we've successfully evaded the enemy patrols and made it into the fjord. Now that we have the initiative, let's gather some intelligence.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 401190,
			nameColor = "#ff5c5c",
			side = 1,
			actorName = "？？？",
			dir = 1,
			say = "Hey, Z2, is that you? Weren't me and Z18 getting resupplied next?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			nameColor = "#a9f548",
			side = 0,
			actor = 201200,
			dir = 1,
			say = "?!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 201160,
			side = 0,
			nameColor = "#a9f548",
			dir = 1,
			say = "We're out of options. It's time to strike!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
