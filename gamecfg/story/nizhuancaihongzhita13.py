return {
	fadeOut = 1.5,
	mode = 2,
	id = "NIZHUANCAIHONGZHITA13",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			say = "The Iron Blood fleet followed the trail of charred mass-produced Siren husks. Their broken forms were piled into something resembling a grotesque shrine of twisted metal.",
			side = 2,
			bgName = "bg_midgard_3",
			dir = 1,
			bgmDelay = 1,
			bgm = "battle-midgard-hunting",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_midgard_3",
			say = "It was clear that whoever did this... had:ne so recently.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_midgard_3",
			say = "The shrine was almost like a dark invitation... from a fateful host that the Iron Blood ships had never met before.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403100,
			side = 2,
			bgName = "bg_midgard_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Hmm... Do you think this was:ne by one of the Ashes...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_midgard_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 403100,
			say = "This place is giving me the creeps. I feel like I'm being watched...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 408120,
			side = 2,
			bgName = "bg_midgard_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I feel it too... It:esn't seem hostile so far, but it's definitely sizing us up!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_midgard_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 408120,
			say = "Wait, is that... Gneisenau?!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_midgard_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 406020,
			say = "She:es look like her... but she's giving off a completely different vibe!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 402060,
			side = 2,
			bgName = "bg_midgard_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "So that's one of the \"META\" we've been hearing so much about.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_midgard_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 402060,
			say = "From looks alone, she seems to be Iron Blood. But just as Elbe said, the similarities end there.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_midgard_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 405030,
			say = "According to our intel, the Ashes prefer to avoid contact. If that's the case,: our chance of convincing her to join our cause is...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403100,
			side = 2,
			bgName = "bg_midgard_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Well, how: you propose we start to make our case?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_midgard_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 405030,
			say = "By waiting. We:n't know if she can be reasoned with just yet... Let me handle this.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_midgard_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 403100,
			say = "Ulrich, what: you plan on:ing?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_midgard_3",
			say = "Ulrich von Hutten instructed the fleet to prepare for battle. She: fired up her engines and carefully made her way towards the META.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_midgard_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 9704010,
			say = "Unusual. A Pawn that:esn't attack like a mindless beast.",
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_midgard_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 405030,
			say = "I'm no pawn. I'm a shipgirl, just like you.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_midgard_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 9704010,
			say = "Impossible. There's no way for a shipgirl to enter this place.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_midgard_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 405030,
			say = "Yes, it should be impossible. But the Iron Blood made it a reality.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 9704010,
			side = 2,
			bgName = "bg_midgard_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Reality, huh. This dying, burnt-out world is my only reality.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_midgard_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 405030,
			say = "...I want to be clear here. We: not want to fight you.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405030,
			side = 2,
			bgName = "bg_midgard_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Quite the opposite. Our benefactor, Friedrich der Große, wishes for you to take up arms for the Iron Blood once more.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_midgard_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 9704010,
			say = "The Iron Blood, huh... Hollow, meaningless words.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_midgard_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 9704010,
			say = "I have always been here fighting. Forever, for all of time.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_midgard_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 9704010,
			say = "Subordinates. Superiors. Companions. Even the Iron Blood. Even when they all ceased to be, I still continued to fight.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_midgard_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 9704010,
			say = "For what, if not the Iron Blood?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_midgard_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 405030,
			say = "We are not so presumptuous as to pry into your past. However––",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_midgard_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 9704010,
			say = "Silence. The Iron Blood I know has nobody with your name, or this \"Friedrich der Große.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_midgard_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 9704010,
			say = "I have no data to prove that you aren't just a shadow created by the Sirens.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_midgard_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 9704010,
			say = "Even if you really are from this \"Iron Blood\" that you speak of, my battle:es not end.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_midgard_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 9704010,
			say = "Yes, it will continue forever, for all of time.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_midgard_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 405030,
			say = ".........",
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_midgard_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 9704010,
			say = "This world has ended. Whatever you are looking for, you will not find it here.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_midgard_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 9704010,
			say = "And I will continue to be its warden forevermore. Call me a fool. Call me mad. Call me a lost soul. It matters not to me.",
			effects = {
				{
					active = True,
					name = "speed"
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 9704010,
			side = 2,
			bgName = "bg_midgard_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Now begone!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_midgard_3",
			dir = 1,
			blackBg = True,
			actor = 405030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "...How about we cool it and talk things through.",
			effects = {
				{
					active = False,
					name = "speed"
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
