return {
	fadeOut = 1.5,
	mode = 2,
	id = "NIZHUANCAIHONGZHITA28",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			side = 2,
			bgName = "bg_midgard_6",
			dir = 1,
			bgmDelay = 1,
			bgm = "theme-midgard-hunting",
			actor = 406020,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "There aren't any Sirens on our tail anymore! We lost them!",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405030,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "What about the wreckage in front of us?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 406020,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Don't worry, I won't mess up this time!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 408120,
			say = "There aren't any Lurker-type Sirens ahead of us, either!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405030,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "(Hmph... I've got a bad feeling about this. Things are going way too well.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405030,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "(If there was an Arbiter there, why didn't they just finish Scharnhorst off? And, why aren't they chasing after us?)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405030,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "An Arbiter should be more than a match for a force of our size, so why...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900319,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "Keep... up your guard... The sea is still...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 402060,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "You're hurt real bad! Don't talk, and more importantly, no sudden movements!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900319,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "I'm alright... I've been through worse... I can fight once you treat me... Ugh!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 402060,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Fight?! In this condition...?!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900319,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "Yeah. The main engine still works... and my main gun too... One of them, anyway... Heh, hehehe...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900319,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "Why did you guys... even come here? Didn't you expect... for this to be a Siren trap...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405030,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Well, of course we knew it was a trap. But Gneisenau insisted that we come save you.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900319,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "Gneisenau...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 9704010,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "You aren't acting like yourself. The Scharnhorst I knew would have been able to put up a good show, even in this state.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900319,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "You're... one to talk...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900319,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "Look how beat up you are...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 408120,
			say = "That's because... she insisted on charging into the enemy headfirst to try to get to you before they did.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 9704010,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Even if you're not the Scharnhorst from my world, you're still Scharnhorst... Just as there must have been a Gneisenau in your world.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 9704010,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "If I let you sink here... I wouldn't be able to look the other Gneisenau in the eyes.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900319,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "Gneisenau... Heh. I'll... remember those words...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405030,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "If you can't return on your own, come to our base until you're all healed up. We won't try to keep you there if you:n't want to stay.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405030,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I:n't know if we're able to repair Ashes ships, but... we're willing to try. ... And you might be interested in our leader.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900319,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "It's not like... I'm in any position... to refuse...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405030,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "There is one thing I want from you in exchange though. Tell me what just happened, and any information you have on Arbiter VI.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900319,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "It's... the first time I encountered that Arbiter.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900319,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "Enforcers aren't a big deal, so I underestimated them, but... they're a pretty dangerous bunch...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 406020,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "The Enforcers... aren't a big deal?!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900319,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "We are pretty used to fighting them... So I didn't think they'd pose much of a threat to Gneisenau either...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 406020,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Oh, uh, I see! That:esn't make me feel bad at all...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900319,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "The Enforcers' combat ability and style depends on their corresponding Arbiter... and the compatibility between them...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900319,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "Basically, they'll either take you out by outnumbering you, or with sheer force on their own...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405030,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Which one happened to you?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900319,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "Both... I hate to say it, but I still haven't figured out a way to deal with them.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900319,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "In the first wave of attack, they overwhelmed me with sheer numbers... and: after, the Arbiter completely overpowered me on her own...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405030,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I see. I didn't think any number of Enforcers alone would've been able to take you:wn.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900319,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "It's as you say... In the end, Enforcers are just Enforcers... Even you guys should be able to handle them just fine...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900319,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "But, the Arbiter Vessel... No, Arbiter VI's True form...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900319,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "A huge armored structure appeared in the sky... and that's the last thing I saw before losing consciousness.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405030,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "So you were taken out in an instant. If Gneisenau hadn't come looking for you, the Sirens would've \"salvaged\" you...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405030,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "...Just like what they're:ing with the rest of the wreckage.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900319,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "That may well be a part of the Arbiter's powers... Taking control of the Singularity to create Mirror Seas...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900319,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "Once she finishes... No, she might already be:ne...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900319,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "She will be able to deconstruct and reconstitute herself at will, using the resources from the endless seas of debris to regenerate herself.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900319,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "And her Enforcers have likewise become more and more durable... and until we deal with them, we have no hope of getting to the Arbiter.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900319,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "That is the Arbiters' power... The weapon they use to hunt:wn the Ashes.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 408120,
			say = "So those Sirens were retrieving resources for the Arbiter...?!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403100,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "This entire singularity... is the Arbiter's:main...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900319,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "Yes... This was once a world branch that they turned into a testing site... They won't show us any mercy.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 9704010,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Hold on, so the reason I was made to fight here was...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900319,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "Yes. They wanted to test your power... Or perhaps simply \"erase\" you from existence...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 9704010,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "......",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405030,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Gneisenau, I'll leave Scharnhorst in your hands. The rest of you, stay vigilant. The enemy may strike at any moment.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 9704010,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "...Understood.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405030,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "The wounded should stay in the back and offer fire support. I'll clear the way.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 9704010,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "I would've been able to give a better showing if I was in better condition...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 9704010,
			side = 2,
			bgName = "bg_midgard_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Sis– I mean, Scharnhorst... Stay with us.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_midgard_6",
			dir = 1,
			blackBg = True,
			say = "Only a few hours remain until the Singularity maintenance limit––",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
