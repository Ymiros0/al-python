return {
	fadeOut = 1.5,
	mode = 2,
	id = "NIZHUANCAIHONGZHITA37",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			say = "??? - ???",
			side = 2,
			bgName = "bg_cccpv2_11",
			dir = 1,
			bgmDelay = 1,
			bgm = "bsm-2",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			side = 2,
			bgName = "bg_cccpv2_11",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "Observer, the analysis of the Mirror Sea is complete.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			side = 2,
			bgName = "bg_cccpv2_11",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "The experiment time was consistent with our calculations. The bait has been retrieved, and no abnormal phenomena were detected.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			side = 2,
			bgName = "bg_cccpv2_11",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "Allow me to report on the task I've been entrusted with.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			side = 2,
			bgName = "bg_cccpv2_11",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "After analyzing and verifying all observational data, we've detected activities from exactly three Arbiters.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_cccpv2_11",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			actor = 900230,
			say = "...But I must ask again, are we seriously investigating the Arbiters?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_cccpv2_11",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "This is our testing site. Everything that appears in it is a variable.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_cccpv2_11",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			actor = 900230,
			say = "Well, it's not like that's any of my business.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			side = 2,
			bgName = "bg_cccpv2_11",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "Let me check one of the permissions in my function profile though.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			side = 2,
			bgName = "bg_cccpv2_11",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "Branches are forbidden from interacting with each other. The Iron Blood's creation of a Singularity should be a major violation.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			side = 2,
			bgName = "bg_cccpv2_11",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "Should we start the destruction protocol of this branch?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_cccpv2_11",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "Even had the Iron Blood not:ne so, the Ashes... and the KAN-SEN that had undergone metamorphosis, would have been drawn to this world branch. There are many ways to open a Singularity.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_cccpv2_11",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "Destruction of this branch is forbidden. It is still too early, for now.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_cccpv2_11",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			actor = 900230,
			say = "I didn't give them the technology for creating Singularities. So it must have been you, Observer.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_cccpv2_11",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "Can't you tell from watching the Arbiters? This branch is very adept at learning.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			side = 2,
			bgName = "bg_cccpv2_11",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_cccpv2_11",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			actor = 900230,
			say = "When was the last time you received instructions from Zero?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_cccpv2_11",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "The last time I received instructions from Zero, Compiler.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			side = 2,
			bgName = "bg_cccpv2_11",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_cccpv2_11",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "Let me ask you something as well.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_cccpv2_11",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "What happens when a branch, a world that was the subject of a natural deductive experiment, is destroyed?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			side = 2,
			bgName = "bg_cccpv2_11",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "I've never seen any materials related to it. This branch's material should all be transmitted through your network mainframe, Observer. Shouldn't you already. know that?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_cccpv2_11",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "Yes. All of the data is transmitted to Zero. What I want to know is, what will happen to me.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_cccpv2_11",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "Basically, what will happen to the Siren observations, experiments, systems, battle... everything related to the terminal when a branch is discarded...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_cccpv2_11",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "Does my soul return to the eternal sea, or will I become something else, like Theseus's ship? Will I still be me, or will I become something else?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_cccpv2_11",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			actor = 900230,
			say = "The only one with a connection to the main frame is you, Observer. What you:n't know, there's no way for a Compiler like me to know.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_cccpv2_11",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			actor = 900230,
			say = "Besides, it's annoying to have to find out.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			side = 2,
			bgName = "bg_cccpv2_11",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "I will go back to allocating resources to the development of new weapons. Working is pain.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_cccpv2_11",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			actor = 900230,
			say = "...Next time, if you need someone to run your errands, get Purifier instead. She hasn't been on the experimental field in quite some time.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900230,
			side = 2,
			bgName = "bg_cccpv2_11",
			hidePaintObj = True,
			dir = 1,
			blackBg = True,
			nameColor = "#ff5c5c",
			say = "Appreciate it. Bye.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
