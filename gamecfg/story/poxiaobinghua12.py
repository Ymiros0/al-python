return {
	fadeOut = 1.5,
	mode = 2,
	id = "POXIAOBINGHUA12",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			bgm = "bgm-cccp2",
			side = 2,
			bgName = "bg_cccpv2_2",
			dir = 1,
			say = "The fleet made its way to a massive iceberg, easily dispatching the enemies in the way. It is said that this iceberg is the only facility here with an internal structure, and also houses the “Gate.”",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 8,
			side = 2,
			bgName = "bg_cccpv2_3",
			actor = 702040,
			dir = 1,
			nameColor = "#a9f548",
			say = "This place is where we first recovered some truly revolutionary information. Isn’t it spectacular, Comrade?",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "bg_cccpv2_3",
			actor = 702040,
			dir = 1,
			nameColor = "#a9f548",
			say = "The first time, we had to fight our way here. Countless comrades became martyrs in these very waters before we finally managed to clear out the swarming Siren forces.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 702040,
			side = 2,
			bgName = "bg_cccpv2_3",
			nameColor = "#a9f548",
			dir = 1,
			say = "But considering what we got out of it... their sacrifices were not in vain.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_cccpv2_3",
			actor = 705040,
			dir = 1,
			nameColor = "#a9f548",
			say = "It can be said that what we discovered here was a turning point for fate. The intelligence we recovered from here was the reason why the Northern Parliament,: on the verge of collapse, managed to hold its tattered line of defense and: claw its way back into becoming one of the major world powers today.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_cccpv2_3",
			actor = 705040,
			dir = 1,
			nameColor = "#a9f548",
			say = "But that’s not all. We also discovered a sliver of the Sirens’ secrets...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_cccpv2_3",
			say = "The Sirens’ secrets...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_cccpv2_3",
			actor = 705040,
			dir = 1,
			nameColor = "#a9f548",
			say = "What I’m about to tell you has never been made public. After so much information has been leaked, we’ve completely lost trust in the ability of the Naval HQ to maintain confidentiality, but we: think you need to know this information.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_cccpv2_3",
			say = "I assume I’m also to keep this a secret from the Naval HQ?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_cccpv2_3",
			actor = 705040,
			dir = 1,
			nameColor = "#a9f548",
			say = "That’s not our intent, no. However, I: hope that after listening to everything, you will utilize your best judgment in deciding what to disclose.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 705040,
			side = 2,
			bgName = "bg_cccpv2_3",
			nameColor = "#a9f548",
			dir = 1,
			say = "All the information you learn in the Arcana Sanctum today should be treated in that manner.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_cccpv2_3",
			say = "I can agree to those terms.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_cccpv2_3",
			actor = 705040,
			dir = 1,
			nameColor = "#a9f548",
			say = "We’re almost convinced that the Sirens were not spontaneously “born” from these oceans, in any sense of the word, but rather came from a distant place.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_cccpv2_3",
			actor = 705040,
			dir = 1,
			nameColor = "#a9f548",
			say = "As for their True purpose, it is not something as simple as trying to eradicate humanity.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_cccpv2_3",
			actor = 705040,
			dir = 1,
			nameColor = "#a9f548",
			say = "Since the very beginning, most of their actions have been aimed at driving wedges between our major factions and amplifying those conflicts, plunging us into a state of endless war.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_cccpv2_3",
			actor = 705040,
			dir = 1,
			nameColor = "#a9f548",
			say = "Looking back at everything that’s happened, the Iron Blood had the deepest ties to the Sirens. Their splintering from the Azur Lane, sudden betrayal, and formation of the Crimson Axis, all reek with the Sirens’ lingering influence.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_cccpv2_3",
			actor = 705040,
			dir = 1,
			nameColor = "#a9f548",
			say = "And perhaps, us too in the past, no... Even right now, we might have walked straight into a trap set by the Sirens.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_cccpv2_3",
			say = "It’s not just you. Both the Eagle Union and Royal Navy have been somewhat aware of this.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_cccpv2_3",
			actor = 705040,
			dir = 1,
			nameColor = "#a9f548",
			say = "Somewhat aware, perhaps. But they still continued to fight, whatever the reasons may have been. In the end, everything still happened as the Sirens desired. This is the True nature of their power.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_cccpv2_3",
			actor = 705040,
			dir = 1,
			nameColor = "#a9f548",
			say = "As for where the Sirens came from and what they’re actually after... Unfortunately, we still haven’t found the answers. But we’re sure that the answers can be found behind the Gate.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_cccpv2_3",
			actor = 705040,
			dir = 1,
			blackBg = True,
			nameColor = "#a9f548",
			say = "We’ll reach it on the other side of this great hall. Let’s keep moving.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
