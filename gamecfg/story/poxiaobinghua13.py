return {
	fadeOut = 1.5,
	mode = 2,
	id = "POXIAOBINGHUA13",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			bgm = "bgm-cccp3",
			side = 2,
			bgName = "bg_cccpv2_3",
			dir = 1,
			say = "So, the reason you need me is because you haven’t been able to open the Gate.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 702040,
			side = 2,
			bgName = "bg_cccpv2_3",
			nameColor = "#a9f548",
			dir = 1,
			say = "Correct. We believe that your natural attunement to Wisdom Cubes will aid us in opening this dusty Gate.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_cccpv2_3",
			actor = 702040,
			dir = 1,
			nameColor = "#a9f548",
			say = "We’ve scoured the entire Mirror Sea, but have not found any device that can control the facilities here. We can’t even control the Mirror Sea or access its maintenance device.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_cccpv2_3",
			actor = 702040,
			dir = 1,
			nameColor = "#a9f548",
			say = "The most suspicious thing is this console up in front.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_cccpv2_3",
			say = "Ahead of us in the middle of the great hall is a metallic console that looks like some kind of control device.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 703010,
			side = 2,
			bgName = "bg_cccpv2_3",
			nameColor = "#a9f548",
			dir = 1,
			say = "There’s a groove in it where you’re supposed to insert a Wisdom Cube, right?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_cccpv2_3",
			say = "Hmm, seems like it... It:es look like some special device that requires a Wisdom Cube to activate.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_cccpv2_3",
			say = "However, all the Siren devices and facilities we’ve encountered so far have been powered by a special energy storage device. Why would this thing here require a Wisdom Cube to activate?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 705040,
			side = 2,
			bgName = "bg_cccpv2_3",
			nameColor = "#a9f548",
			dir = 1,
			say = "We may never know until we see what’s on the other side of this:or.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_cccpv2_3",
			say = "An infinite loop, huh...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_cccpv2_3",
			actor = 703010,
			dir = 1,
			nameColor = "#a9f548",
			say = "We’ve conducted a series of experiments to try to activate the console, but everything we’ve tried has resulted in failure. It won’t respond at all.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_cccpv2_3",
			say = "So you believe that this device will only respond to special individuals?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "bg_cccpv2_3",
			actor = 702040,
			dir = 1,
			nameColor = "#a9f548",
			say = "Mhm. Comrade, the tests you took when you first became a commander showed that you have the highest attunement with Wisdom Cubes that this world has ever seen.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 702040,
			side = 2,
			bgName = "bg_cccpv2_3",
			nameColor = "#a9f548",
			dir = 1,
			say = "All the science we have today cannot explain why you are able to produce benign changes to Wisdom Cubes, but even you should be aware of this phenomenon.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_cccpv2_3",
			say = "Mhm. I’ve always wanted to know... Why me?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_cccpv2_3",
			say = "But where is this “Gate” that you keep talking about? We’ve reached the end of the great hall, but I:n’t see any:ors here...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_cccpv2_3",
			actor = 705040,
			dir = 1,
			nameColor = "#a9f548",
			say = "Ah... Sorry, sorry. I forgot to tell you one little detail about the Gate~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_cccpv2_3",
			actor = 705040,
			dir = 1,
			nameColor = "#a9f548",
			say = "The wall of ice in front of you, right at the end of this hall, is what we call the “Gate.” We’re confident that if you’re able to activate this console, this will open up into another passage.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_cccpv2_3",
			say = "Just from its size and texture, there’s no way something like this could have formed naturally. But I can’t see any kinds of mechanisms inside... It seems like just a sheet of ice.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_cccpv2_3",
			actor = 703010,
			dir = 1,
			nameColor = "#a9f548",
			say = "And that’s where you’d be wrong, Comrade. There’s much, much more to it than that.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_cccpv2_3",
			actor = 703010,
			dir = 1,
			nameColor = "#a9f548",
			say = "Gromky, let’s: a little demonstration for the commander. Shoot at that corner over there.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 701080,
			side = 2,
			bgName = "bg_cccpv2_3",
			nameColor = "#a9f548",
			dir = 1,
			say = "Aye aye! Everyone, please keep a safe distance! Commencing fire!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			say = "KABOOM————————!!",
			side = 2,
			bgName = "bg_cccpv2_3",
			dir = 1,
			soundeffect = "event./battle/boom2",
			flashN = {
				color = {
					1,
					1,
					1
				},
				alpha = {
					{
						0,
						1,
						0.2
					},
					{
						1,
						0,
						0.2,
						0.2
					},
					{
						0,
						1,
						0.2,
						0.4
					},
					{
						1,
						0,
						0.2,
						0.6
					}
				}
			},
			dialogShake = {
				speed = 0.09,
				x = 8.5,
				number = 2
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_cccpv2_3",
			say = "The shell discharged from Gromky’s gun slammed into the wall of ice and erupted in a cloud of smoke and steam... But when it cleared, there wasn’t a scratch on the wall...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_cccpv2_3",
			actor = 703010,
			dir = 1,
			nameColor = "#a9f548",
			say = "As you can see, we can’t damage the wall even with this amount of force.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_cccpv2_3",
			actor = 703010,
			dir = 1,
			nameColor = "#a9f548",
			say = "Of course, we’ve tried many other methods. Using larger-caliber guns, having the entire fleet fire salvos, using explosives, using heavier explosives...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_cccpv2_3",
			actor = 703010,
			dir = 1,
			nameColor = "#a9f548",
			say = "But nothing has worked so far. All the firepower we’ve been able to muster to date, and still not a scratch on this wall.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_cccpv2_3",
			say = "I guess you really: have to work from the console... Hm? Wait, this insignia here... it’s not what the Sirens use to represent themselves... I’ve never seen this before.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_cccpv2_3",
			actor = 702060,
			dir = 1,
			nameColor = "#a9f548",
			say = "Huh? That’s weird... I:n’t think we would’ve glossed over something like that...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_cccpv2_3",
			actor = 703010,
			dir = 1,
			nameColor = "#a9f548",
			say = "What...?! Impossible! I’m certain that symbol was never there before.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_cccpv2_3",
			actor = 705040,
			dir = 1,
			nameColor = "#a9f548",
			say = "In other words, the console changed in a way we’ve never seen before?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 705040,
			side = 2,
			bgName = "bg_cccpv2_3",
			nameColor = "#a9f548",
			dir = 1,
			say = "Sure enough, mysterious things start to happen as soon as the commander sets foot in here... Here, use this Wisdom Cube.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 705040,
			side = 2,
			bgName = "bg_cccpv2_3",
			nameColor = "#a9f548",
			dir = 1,
			say = "Don’t keep us waiting. Go ahead and give it a try.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_cccpv2_3",
			say = "There’s nothing else on the console that can be operated. Might as well start with the Wisdom Cube first——",:
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			options = {
				{
					content = "Place it",
					flag = 1
				}
			}
		},
		{
			mode = 4,
			bgs = {
				{
					"bg_cccpv2_3",
					0.5
				},
				{
					"bg_cccpv2_4",
					0.5
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_cccpv2_4",
			actor = 701070,
			dir = 1,
			nameColor = "#a9f548",
			say = "...Woah, the console’s actually reacting!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_cccpv2_4",
			actor = 701080,
			dir = 1,
			nameColor = "#a9f548",
			say = "Woaahhhh... It’s glowing!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_cccpv2_4",
			say = "As soon as I pressed the Wisdom Cube into the groove, the device began to vibrate and hum. The Wisdom Cube seemed to dissolve, replaced by a ball of light that expanded above the console.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_cccpv2_4",
			actor = 703010,
			dir = 1,
			nameColor = "#a9f548",
			say = "The structure of the Wisdom Cube is reflected in the motes of light... as if they’re trying to deconstruct the cube...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 702040,
			side = 2,
			bgName = "bg_cccpv2_4",
			nameColor = "#a9f548",
			dir = 1,
			say = "...Belorussiya, looks like our hypothesis might be correct.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 705040,
			side = 2,
			bgName = "bg_cccpv2_4",
			nameColor = "#a9f548",
			dir = 1,
			blackBg = True,
			say = "Yes! The Gate has also began to change shape! Now, show me what lies behind this wall of ice!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
