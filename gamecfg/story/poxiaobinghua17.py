return {
	fadeOut = 1.5,
	mode = 2,
	id = "POXIAOBINGHUA17",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			actor = 701080,
			side = 2,
			bgName = "bg_cccpv2_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "Hum~ dee dum~♪",
			bgm = "deepblue-image",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_cccpv2_6",
			say = "Gromky seemed to be enjoying this a lot.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_cccpv2_6",
			actor = 701070,
			dir = 1,
			nameColor = "#a9f548",
			say = ".........",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_cccpv2_6",
			say = "Gremyashchy, on the other hand, looked no different than before, even beneath the splendors of the city of light.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_cccpv2_6",
			actor = 701070,
			dir = 1,
			nameColor = "#a9f548",
			say = "Why are you staring at me again? ...Do you need something?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 1,
			bgName = "bg_cccpv2_6",
			actor = 701080,
			dir = 1,
			nameColor = "#a9f548",
			say = "Gremyashchy, it’s not good to be so grumpy around Comrade Commander! You’ll be able to get along better with others if you just put a smile on your face~!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			paintingFadeOut = {
				time = 0.5,
				side = 0
			}
		},
		{
			expression = 3,
			side = 0,
			bgName = "bg_cccpv2_6",
			actor = 701070,
			dir = 1,
			nameColor = "#a9f548",
			say = "I’m not grumpy. This is just the face I was born with. Besides, I’m here as an escort ship, not a therapist. Mm, that’s all.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 1,
			bgName = "bg_cccpv2_6",
			actor = 701080,
			dir = 1,
			nameColor = "#a9f548",
			say = "Aww, why: you always have to be like this? Fine:! Comrade Commander, if there’s anything you:n’t understand, I’ll be your Gremyashchy translator~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 8,
			side = 0,
			bgName = "bg_cccpv2_6",
			actor = 701070,
			dir = 1,
			nameColor = "#a9f548",
			say = "Hmph... Well, that’ll be all.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 701080,
			side = 2,
			bgName = "bg_cccpv2_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "Look, look, she’s blushing!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 701080,
			side = 2,
			bgName = "bg_cccpv2_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "Gremyashchy might act all cold, but she’s actually happy that she gets to work with you!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_cccpv2_6",
			say = "You two really understand each other, huh.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 701080,
			side = 2,
			bgName = "bg_cccpv2_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "Heheh, we sure:! We used to be in the same fleet, but I was assigned to defend the Sanctum later on. It’s so nice to be back together!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_cccpv2_6",
			actor = 701080,
			dir = 1,
			nameColor = "#a9f548",
			say = "Naturally, it’s as if we were never apart! Right, Gremyashchy~?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 7,
			side = 0,
			bgName = "bg_cccpv2_6",
			actor = 701070,
			dir = 1,
			nameColor = "#a9f548",
			say = "Yeah, yeah... So, can you let go of me already...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			paintingFadeOut = {
				time = 0.5,
				side = 1
			}
		},
		{
			actor = 701080,
			side = 1,
			bgName = "bg_cccpv2_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "No can:! I’ve missed you a tooooooon, so you owe me a bazillion hugs! You should come join in too, Comrade Commander~!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 1,
			side = 0,
			bgName = "bg_cccpv2_6",
			actor = 701070,
			dir = 1,
			nameColor = "#a9f548",
			say = "......",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_cccpv2_6",
			say = "The hugging session was quickly cut short by the piercing sensation of Gremyashchy’s icy gaze.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 702040,
			side = 2,
			bgName = "bg_cccpv2_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "Oh, you’ve returned? We’ve finally made some progress. There seems to be a passageway that goes deeper into this research facility, from this point here on the iceberg with the control room.",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_cccpv2_6",
			actor = 702060,
			dir = 1,
			nameColor = "#a9f548",
			say = "Hm... Weird stuff happened when we got across the Gate. Do you think we’ll be able to leave the way we came through?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 8,
			side = 2,
			bgName = "bg_cccpv2_6",
			actor = 702040,
			dir = 1,
			blackBg = True,
			nameColor = "#a9f548",
			say = "We’ll never know until we try.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
