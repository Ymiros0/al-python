return {
	fadeOut = 1.5,
	mode = 2,
	id = "POXIAOBINGHUA19",
	once = True,
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			actor = 701080,
			nameColor = "#a9f548",
			side = 2,
			bgm = "deepblue-image",
			dir = 1,
			say = "The whole atmosphere around us has changed!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			nameColor = "#a9f548",
			side = 2,
			actor = 703010,
			dir = 1,
			say = "I’m sure we’re at the same place, but nothing else about this facility matches what we saw before. I:n’t think this is the same console from before.",:
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 705040,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "Doesn’t this feel familiar? This seems more like one of the Northern Parliament’s command centers rather than what we saw before.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			nameColor = "#a9f548",
			side = 2,
			actor = 702040,
			dir = 1,
			say = "That’s also the impression I’m getting. Belorussiya, I:n’t think I’ve seen that insignia anywhere before. Ring any bells?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			nameColor = "#a9f548",
			side = 2,
			actor = 705040,
			dir = 1,
			say = "I can’t say I recognize it either. This place really is full of mysteries.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 701080,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "Look at all those:ors ahead of us! Why:n’t we go explore them all?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 702060,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "I:n’t think anyone’s home anyway~ Surely nobody will mind if we take a little peek inside~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 7,
			nameColor = "#a9f548",
			side = 2,
			actor = 702060,
			dir = 1,
			say = "Besides, it’s not like we’re:ing anything else right now...",:
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 705040,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "Everything here must have been hidden away for a reason. I, for one, am in favor of getting whatever valuable intelligence we can.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			nameColor = "#a9f548",
			side = 2,
			actor = 705040,
			dir = 1,
			say = "There must be something around here that gives us more information about this Mirror Sea, and how to control it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			nameColor = "#a9f548",
			side = 2,
			actor = 705040,
			dir = 1,
			say = "...Having said that though, we:n’t have the time to look through every single room.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			say = "How about starting from the big picture and working our way:wn?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 702040,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "I agree. If this is some kind of command center,: there must also be a conference room, a records room, and some information about what this place was used for.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 701080,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "Ah... Look at that room up ahead! Don’t you think that fits the bill?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_cccpv2_8",
			say = "After entering the curious room, we were immediately greeted by a huge display screen on the opposing wall.",
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Sovetskaya Belorussiya",
			bgName = "bg_cccpv2_8",
			nameColor = "#a9f548",
			dir = 1,
			say = "Hmm... Why would something like this be here?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Tallinn",
			bgName = "bg_cccpv2_8",
			nameColor = "#a9f548",
			dir = 1,
			say = "This is a chart that seems to track the course of a battle of some kind... It seems to be of... an island defense?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Kirov",
			bgName = "bg_cccpv2_8",
			nameColor = "#a9f548",
			dir = 1,
			say = "Hmm... the red star in the middle must represent an important target, and a defensive line has been erected all around it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Sovetskaya Belorussiya",
			bgName = "bg_cccpv2_8",
			nameColor = "#a9f548",
			dir = 1,
			say = "Trying to defend such a small island in this manner seems like a fool’s errand.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Kirov",
			bgName = "bg_cccpv2_8",
			nameColor = "#a9f548",
			dir = 1,
			say = "Indeed. You can buckle:wn all you want, but anyone trying to defend this island will be at a huge disadvantage due to the lack of maneuverability.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Kirov",
			bgName = "bg_cccpv2_8",
			nameColor = "#a9f548",
			dir = 1,
			say = "You might be able to use some of the smaller surrounding islands as a buffer, but even so... Assuming each mark represents the same amount of forces, the defenders are outnumbered several times over.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Murmansk",
			bgName = "bg_cccpv2_8",
			nameColor = "#a9f548",
			dir = 1,
			say = "“The purpose of this operation is the absolute defense of the island’s facilities, and to prevent the enemy from establishing a foothold in the region.”",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Murmansk",
			bgName = "bg_cccpv2_8",
			nameColor = "#a9f548",
			dir = 1,
			say = "This place must’ve had some greater importance we’re not seeing. Otherwise, it wouldn’t make sense to defend this place so desperately at such an overwhelming disadvantage.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Murmansk",
			bgName = "bg_cccpv2_8",
			nameColor = "#a9f548",
			dir = 1,
			say = "“Use the fleet's speed to fight back. Even if it’s impossible to overcome the difference in numbers, we can still buy more time...”",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Murmansk",
			bgName = "bg_cccpv2_8",
			nameColor = "#a9f548",
			dir = 1,
			say = "“There is still a glimmer of hope in this operation. We were expecting to be crushed, but luckily, the enemy’s forces seem to be getting diverted elsewhere.”",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Murmansk",
			bgName = "bg_cccpv2_8",
			nameColor = "#a9f548",
			dir = 1,
			say = "“We will hold the line until our last breath.”",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Murmansk",
			bgName = "bg_cccpv2_8",
			nameColor = "#a9f548",
			dir = 1,
			say = "“We’ve:ne all that we could. Your dedication and sacrifice shall not be forgotten.”",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Murmansk",
			bgName = "bg_cccpv2_8",
			nameColor = "#a9f548",
			dir = 1,
			say = "...That sounds pretty tragic...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Kirov",
			bgName = "bg_cccpv2_8",
			nameColor = "#a9f548",
			dir = 1,
			say = "Thank you for your dramatic display of oratorical flair, Murmansk. We can already see all that information on the screen.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Murmansk",
			bgName = "bg_cccpv2_8",
			nameColor = "#a9f548",
			dir = 1,
			say = "What’s wrong with embellishing things a little bit~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Murmansk",
			bgName = "bg_cccpv2_8",
			nameColor = "#a9f548",
			dir = 1,
			say = "...Huh? Gremyashchy, what’s the matter?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Gremyashchy",
			bgName = "bg_cccpv2_8",
			nameColor = "#a9f548",
			dir = 1,
			say = "Wait...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Gremyashchy",
			bgName = "bg_cccpv2_8",
			nameColor = "#a9f548",
			dir = 1,
			say = "We’re not alone... Something is watching us. Do you feel that as well?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Gromky",
			bgName = "bg_cccpv2_8",
			nameColor = "#a9f548",
			dir = 1,
			say = "Eh? I:n’t feel anything...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Murmansk",
			bgName = "bg_cccpv2_8",
			nameColor = "#a9f548",
			dir = 1,
			say = "Me neither... Jeez, Gremyashchy, stop saying spooky stuff like that.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_cccpv2_8",
			say = "I will admit... I’ve had an uneasy feeling ever since I stepped into this room...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Murmansk",
			bgName = "bg_cccpv2_8",
			nameColor = "#a9f548",
			dir = 1,
			say = "Eh? You feel it too, Comrade?!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Murmansk",
			bgName = "bg_cccpv2_8",
			nameColor = "#a9f548",
			dir = 1,
			say = "It's weird, isn't it? Nobody's been here for ages, but the monitor is still on, like everyone just up and disappeared.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Murmansk",
			bgName = "bg_cccpv2_8",
			nameColor = "#a9f548",
			dir = 1,
			say = "More than weird, it's kind of creepy, even... Like we've entered a haunted house.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Kirov",
			bgName = "bg_cccpv2_8",
			nameColor = "#a9f548",
			dir = 1,
			say = "Ahaha, how unscientific of you, comrade Murmansk! Besides, how could the Revolution falter before some ghosts?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Kirov",
			bgName = "bg_cccpv2_8",
			nameColor = "#a9f548",
			dir = 1,
			say = "That said, since several of you share this suspicion, I suppose we should proceed under the assumption that we are indeed being watched.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Sovetskaya Belorussiya",
			bgName = "bg_cccpv2_8",
			nameColor = "#a9f548",
			dir = 1,
			blackBg = True,
			say = "Stay close, everyone. Nobody is to wander off on their own, lest we risk getting lost, or worse yet, walk into a trap.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
