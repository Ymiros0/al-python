return {
	fadeOut = 1.5,
	mode = 2,
	id = "POXIAOBINGHUA30",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			say = "",
			side = 2,
			bgName = "bg_guild_blue",
			dir = 1,
			bgmDelay = 2,
			bgm = "bgm-cccp3",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900218,
			side = 2,
			bgName = "bg_guild_blue",
			nameColor = "#a9f548",
			dir = 1,
			say = "Through our expedition into the Arcana Sanctum, we've all seen firsthand how anomalous an existence the Commander is.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_guild_blue",
			actor = 900218,
			dir = 1,
			nameColor = "#a9f548",
			say = "The Commander's attunement to Wisdom Cubes not only affects shipgirls, but also devices powered by Wisdom Cubes.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_guild_blue",
			actor = 900218,
			dir = 1,
			nameColor = "#a9f548",
			say = "Lasting changes have already been observed in the Sanctum, something we could not bring about despite all our efforts.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 705040,
			side = 2,
			bgName = "bg_guild_blue",
			nameColor = "#a9f548",
			dir = 1,
			say = "Do you think that the Commander is the \"seed of change\" mentioned in the:cuments?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_guild_blue",
			actor = 900218,
			dir = 1,
			nameColor = "#a9f548",
			say = "I cannot say for sure. We may arrive at a clearer understanding after analyzing all the data we've collected.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_guild_blue",
			actor = 900218,
			dir = 1,
			nameColor = "#a9f548",
			say = "Whether or not the Commander will become this seed... is not something that we can decide.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_guild_blue",
			actor = 900218,
			dir = 1,
			nameColor = "#a9f548",
			say = "Now that there are no outsiders here, I shall speak frankly.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_guild_blue",
			actor = 900218,
			dir = 1,
			nameColor = "#a9f548",
			say = "As long as the endless war between the Northern Parliament and the Sirens persists, there can be no future for our people.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_guild_blue",
			actor = 900218,
			dir = 1,
			nameColor = "#a9f548",
			say = "Even with the advantages we have currently, the Sirens' infinite adaptability will eventually overwhelm us.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_guild_blue",
			actor = 900218,
			dir = 1,
			nameColor = "#a9f548",
			say = "We've used everything to try to secure our future. The aid of the Azur Lane. The technology of the Crimson Axis. Even the secrets of the Sirens and the Ashes.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_guild_blue",
			actor = 900218,
			dir = 1,
			nameColor = "#a9f548",
			say = "But now we know, the Commander has gained the \"Key\" from the Arcana Sanctum.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_guild_blue",
			actor = 900218,
			dir = 1,
			nameColor = "#a9f548",
			say = "We must now turn this key together and grasp the possibilities that open up before us.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_guild_blue",
			actor = 705050,
			dir = 1,
			nameColor = "#a9f548",
			say = "If that is the case, why didn't we pull some strings to keep the Commander here a bit longer?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_guild_blue",
			actor = 900218,
			dir = 1,
			nameColor = "#a9f548",
			say = "The Royal Navy and the Eagle Union have not yet grasped the Commander's True worth. Were they to: so... I fear we may not ever meet again.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_guild_blue",
			actor = 900218,
			dir = 1,
			nameColor = "#a9f548",
			say = "We are viewed with distrust from the outside, but the Commander has given us a way forward.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_guild_blue",
			actor = 705040,
			dir = 1,
			nameColor = "#a9f548",
			say = "...You're right. We can't squander this opportunity that the Commander earned for us.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_guild_blue",
			actor = 900218,
			dir = 1,
			nameColor = "#a9f548",
			say = "For the sake of the Northern Parliament, nay, for humanity itself, we must seek every advantage.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_guild_blue",
			actor = 900218,
			dir = 1,
			nameColor = "#a9f548",
			say = "To these ends, the Commander's assistance is indispensable, and we must squeeze everything we can from this \"Key.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_guild_blue",
			actor = 900218,
			dir = 1,
			nameColor = "#a9f548",
			say = "But that is something reserved for the future. For now, we must be practical and: the things we ought to:. ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_guild_blue",
			actor = 900218,
			dir = 1,
			blackBg = True,
			nameColor = "#a9f548",
			say = "Study these materials well. Do not waste the light of dawn that the Commander has given us—",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
