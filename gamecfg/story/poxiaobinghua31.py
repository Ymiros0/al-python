return {
	fadeOut = 1.5,
	mode = 2,
	id = "POXIAOBINGHUA31",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			expression = 1,
			side = 2,
			actor = 900136,
			nameColor = "#ff5c5c",
			dir = 1,
			blackBg = True,
			say = "And the conclusion...?",
			bgm = "bsm-2",
			flashin = {
				black = True,
				dur = 1,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900191,
			side = 2,
			nameColor = "#ff5c5c",
			dir = 1,
			blackBg = True,
			say = "The one known as the \"Commander\" is in fact unique.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900191,
			side = 2,
			nameColor = "#ff5c5c",
			dir = 1,
			blackBg = True,
			say = "Three times, the one known as the \"Commander\" vanished from this world, albeit for a few microseconds.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900191,
			side = 2,
			nameColor = "#ff5c5c",
			dir = 1,
			blackBg = True,
			say = "Disappeared,: reappeared, with no fluctuations in energy. As if the world was rejecting its existence,: re-accepting it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			actor = 900136,
			nameColor = "#ff5c5c",
			dir = 1,
			blackBg = True,
			say = "This timeline did not diverge:, but rather is being pruned...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900191,
			side = 2,
			nameColor = "#ff5c5c",
			dir = 1,
			blackBg = True,
			say = "If this phenomenon is real,: there is much for us to consider.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			actor = 900136,
			nameColor = "#ff5c5c",
			dir = 1,
			blackBg = True,
			say = "Indeed.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900191,
			side = 2,
			nameColor = "#ff5c5c",
			dir = 1,
			blackBg = True,
			say = "Your other intuition proved correct as well. The Observer's Reenactment was compromised.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900191,
			side = 2,
			nameColor = "#ff5c5c",
			dir = 1,
			blackBg = True,
			say = "The work of the Arbiters cannot be detected by even the most unique of Observers... Even the one in this timeline.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			actor = 900136,
			nameColor = "#ff5c5c",
			dir = 1,
			blackBg = True,
			say = "Continue to monitor the experiment, and stay concealed. You are forbidden from interfering in it. Another Arbiter will be sent to deal with Code G.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			actor = 900136,
			nameColor = "#ff5c5c",
			dir = 1,
			blackBg = True,
			say = "A large number of variables that cannot be empirically determined have begun to converge upon a single timeline. We must handle this situation with extreme caution.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			actor = 900136,
			nameColor = "#ff5c5c",
			dir = 1,
			blackBg = True,
			say = ".........",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900136,
			side = 2,
			nameColor = "#ff5c5c",
			dir = 1,
			blackBg = True,
			say = "What exactly did you see... \"Commander?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
