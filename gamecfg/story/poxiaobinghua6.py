return {
	id = "POXIAOBINGHUA6",
	mode = 2,
	once = True,
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			paintingNoise = True,
			side = 2,
			actor = 701040,
			nameColor = "#a9f548",
			dir = 1,
			say = "This is our regularly-scheduled transmission. Dear comrade, we’ve already reached our destination, and are now holding our positions.",
			bgm = "bgm-cccp2",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			nameColor = "#a9f548",
			side = 2,
			actor = 701040,
			dir = 1,
			say = "I haven’t seen any higher-tier Sirens like Omitter yet, but I’m sure they’ll be forced to come out as long as we keep fighting.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			nameColor = "#a9f548",
			side = 2,
			actor = 701040,
			dir = 1,
			say = "How are things on your end, dear comrade?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			say = "The Sirens here haven’t gotten any stronger. I’d say your gamble was a success. However, the Sirens around the Sanctum haven’t pulled back either, and we’re currently engaging them.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			nameColor = "#a9f548",
			side = 2,
			actor = 701040,
			dir = 1,
			say = "I see. Well, I believe in you. Teach them a harsh lesson for me. Over.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			nameColor = "#a9f548",
			side = 2,
			actor = 705040,
			dir = 1,
			say = "Seems like the assault on the Winter’s Crown is going smoothly. Unfortunately, we’re facing more resistance here than expected...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			nameColor = "#a9f548",
			side = 2,
			actor = 702040,
			dir = 1,
			say = "The more times we clash with Omitter, the more incongruous her behavior seems. The way she acts says little about the way she commands.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			nameColor = "#a9f548",
			side = 2,
			actor = 701070,
			dir = 1,
			say = "I concur. I have witnessed their cunning ambushes time and time again along the Arctic routes.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			nameColor = "#a9f548",
			side = 2,
			actor = 701070,
			dir = 1,
			say = "It is hard to picture such ruthless and cunning attacks coming from that clown. Mm, that’s all.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			nameColor = "#a9f548",
			side = 2,
			actor = 702060,
			dir = 1,
			say = "I: have a theory though... Commander, the Sirens are supposed to be some kind of program, right...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			say = "Mhm. Though there’s much we:n’t understand about the Sirens, based upon the Naval HQ’s classifications, Sirens are more structurally similar to machines than they are to organic lifeforms.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			say = "Normally, lower-level Siren combat units are simply mindless tools of destruction, whereas higher-tier humanoid Sirens exhibit both knowledge and intelligence, and have absolute command over the lower-level units.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			nameColor = "#a9f548",
			side = 2,
			actor = 702060,
			dir = 1,
			say = "Almost like a hive of bees commanded by a queen...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			say = "“Right, that’s the impression I get.”",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 705040,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "In theory, you can eliminate the treat of a Siren fleet by taking out their command structure. But in reality, the Sirens often have multiple command units, and each command unit can use special flagships to remotely relay their orders.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			nameColor = "#a9f548",
			side = 2,
			actor = 705040,
			dir = 1,
			say = "Theory rarely stacks up to the reality of the battlefield.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			nameColor = "#a9f548",
			side = 2,
			actor = 702060,
			dir = 1,
			say = "Given that fact... there’s obviously a better way for the Sirens to communicate that would not only be more efficient, but also be more machine-like. Why would they need to take on human personalities?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 705040,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "Hmm... I can’t say I’ve considered this before...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			nameColor = "#a9f548",
			side = 2,
			actor = 701070,
			dir = 1,
			say = "Perhaps they’re trying to exploit our empathy to create an advantage on the battlefield...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			nameColor = "#a9f548",
			side = 2,
			actor = 702040,
			dir = 1,
			say = "What’s your take on this, Comrade?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			say = "“Hm......”",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			options = {
				{
					content = "“Maybe it’s a disguise used to approach us...?”",
					flag = 1
				},
				{
					content = "“Are they trying to cast themselves in our image...?”",
					flag = 2
				}
			}
		},
		{
			expression = 5,
			side = 2,
			actor = 705040,
			nameColor = "#a9f548",
			dir = 1,
			optionFlag = 1,
			say = "Mm... Obviously, lower-level Sirens are unambiguously menacing in their appearance. As for their leaders, they not only are rich with emotions, but also take on the appearance of young girls.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			actor = 705040,
			nameColor = "#a9f548",
			dir = 1,
			optionFlag = 1,
			say = "Given the Sirens’ pragmatism and precision, it’s understandable that they would seek to reach out to us in a form we can relate to.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			actor = 705040,
			nameColor = "#a9f548",
			dir = 1,
			optionFlag = 1,
			say = "Oh, that reminds me... Kirov, has your latest work been related to this kind of stuff?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			actor = 702040,
			nameColor = "#a9f548",
			dir = 1,
			optionFlag = 1,
			say = "Uhh, I guess? As we continue to encounter these Siren commanders, a growing number of Siren-worshipping cults have begun to appear across the world, their members perhaps bewitched by their alluring forms. A most disturbing trend.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			optionFlag = 1,
			say = "“Siren worshippers, huh? I’ve also heard about this...”",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			actor = 702040,
			nameColor = "#a9f548",
			dir = 1,
			optionFlag = 1,
			say = "Comrade, we can’t allow this trend to continue! The Northern Parliament has been monitoring this development closely, but the Naval NQ needs to recognize the seriousness of this situation and act accordingly.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			optionFlag = 1,
			say = "“I’ll make sure they get the memo.”",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			actor = 702060,
			nameColor = "#a9f548",
			dir = 1,
			optionFlag = 2,
			say = "The bodies of young girls, and massive ship riggings... These Siren commanders are similar to us in many ways.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 705040,
			nameColor = "#a9f548",
			side = 2,
			dir = 1,
			optionFlag = 2,
			say = "Do you think the Sirens might be a product of humankind?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			optionFlag = 2,
			say = "Though we:n’t have that kind of technology, I’d say it’s quite likely that the Sirens were constructed. In any case, whoever created the Sirens... must know us very well.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			actor = 705040,
			nameColor = "#a9f548",
			dir = 1,
			optionFlag = 2,
			say = "You make a very interesting point...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = -30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				}
			}
		},
		{
			dir = 1,
			side = 2,
			optionFlag = 2,
			say = "And the purpose of designing them in this manner... must surely be some kind of disguise to let them get closer to us.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			nameColor = "#a9f548",
			side = 2,
			actor = 702040,
			dir = 1,
			say = "They sound like us. They look like us. The Sirens exploit these similarities to subconsciously influence our thinking and interfere with our judgment, even without us being aware of it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			nameColor = "#a9f548",
			side = 2,
			actor = 702040,
			dir = 1,
			say = "This battle is proof of that. We misjudged the battle situation based upon an assessment of Omitter’s outward personality.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			nameColor = "#a9f548",
			side = 2,
			actor = 702060,
			dir = 1,
			say = "Wearing emotions as a disguise to give us the illusion of having common ground, of being able to communicate... when in fact, underneath those expressions, they are cold-blooded killing machines. How terrifying...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 701070,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "Like demons that adorn themselves in the trappings of men... but humans are their victims.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			nameColor = "#a9f548",
			side = 2,
			actor = 702060,
			dir = 1,
			say = "Indeed... Just like what the name they’ve been given symbolizes...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			nameColor = "#a9f548",
			side = 2,
			actor = 702060,
			dir = 1,
			say = "Speaking of which... did humans name them “Sirens,” or is that what they call themselves?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			nameColor = "#a9f548",
			side = 2,
			actor = 701070,
			dir = 1,
			say = "When I entered service, the Sirens were already known as Sirens. Mm, I:n’t know anything else beyond that.",:
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			nameColor = "#a9f548",
			side = 2,
			actor = 702040,
			dir = 1,
			say = "I:n’t think I remember either... But most likely, whoever discovered them named them “Sirens,” as mysterious enemies that emerged from the ocean.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			nameColor = "#a9f548",
			side = 2,
			actor = 702060,
			dir = 1,
			say = "Just going off of destructive potential, wouldn’t it have made more sense to call them “Leviathans” or something? ...Why Siren?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 705040,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "You have a point... But they’ve just been called that for so long, I’m not sure anyone actually has an explanation for the origin of that name.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			say = "(The Ashes keep referring to something as “Antiochus”... Could that have something to: with the Sirens’ True identities...?)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 705040,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "Murmansk, your questions have been unusually on the mark today. Have you recently changed your hobby from observing small animals to perusing tomes of philosophy?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			nameColor = "#a9f548",
			side = 2,
			actor = 702060,
			dir = 1,
			say = "Whether you’re observing nature or reading books, a thirst for knowledge goes a long way~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			nameColor = "#a9f548",
			side = 2,
			actor = 702060,
			dir = 1,
			say = "Ehehe... Did I just say something totally quotable? To be honest though, I suddenly thought of all these things after meeting Comrade Commander today...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			nameColor = "#a9f548",
			side = 2,
			actor = 705040,
			dir = 1,
			say = "I see... I can only hope that the answers we seek lie behind the:ors of the Arcana Sanctum.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			say = "(I can only pray... that the solution to our problems also lies behind those:ors...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
