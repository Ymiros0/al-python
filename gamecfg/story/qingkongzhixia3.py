return {
	id = "QINGKONGZHIXIA3",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			say = "Sakura Empire - Musashi's Residence",
			bgm = "map-longgong",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 305100,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Akagi, I've heard the Prime Sakura incident has been resolved.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305100,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "The Great Sage regained her hull as Unzen and purged the Shadow of Corruption within the Sakura. Then, a phantom of Amagi appeared.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 305100,
			say = "Not only did we avoid the worst-case scenario, but also you could argue it was a blessing in disguise.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_508",
			paintingNoise = True,
			dir = 1,
			actor = 307010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "The entire ordeal placed great strain on Lady Nagato, and she will not be able to intervene in our plans for some time.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_508",
			paintingNoise = True,
			dir = 1,
			actor = 307010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "After Unzen solidified into her own entity, she has been severed from the concept of the Great Sage, and can no longer tap into Watatsumi's conceptual framework.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_508",
			paintingNoise = True,
			dir = 1,
			actor = 307010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "In other words, with Shinano away in Europa, there is nobody controlling the Watatsumi framework, making it ripe for the picking.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 305100,
			say = "...So you're committed to this.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 305100,
			say = "Unzen and Shinano were both born with a strong connection to Watatsumi, and therefore have an innate power...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 305100,
			say = "What you're trying to:, Akagi, is to forcefully change Watatsumi's nature through your acquired power. That is simply too dangerous.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 305100,
			say = "If you insist that there is no other way,: at least let us, the Yamato class,: it. The spirit of the Sacred Sakura breathes through us.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_508",
			paintingNoise = True,
			dir = 1,
			actor = 307010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "No, Musashi. This is something only I can:.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_508",
			paintingNoise = True,
			dir = 1,
			actor = 307010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Buried beneath the great tree's shade, Amagi's soul has at least been able to remain in this world due to the connection between the Sacred Sakura and Watatsumi.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_508",
			paintingNoise = True,
			dir = 1,
			actor = 307010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "While it couldn't fix the faults in her hull, it did work as a desperate measure to keep her in this world. You could even call it a move made in anticipation of the current situation.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_508",
			paintingNoise = True,
			dir = 1,
			actor = 307010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "This is all to say, it has to be me. Only I can: it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_508",
			paintingNoise = True,
			dir = 1,
			actor = 307010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Tell me, what is really on your mind, Musashi? Are you just worried you won't be able to birth your superweapon without Watatsumi?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 305100,
			say = "......",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_508",
			paintingNoise = True,
			dir = 1,
			actor = 307010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Don't play dumb with me. Nothing happens without me hearing about it, not even in a Mirror Sea, and especially not in the Sanctuary.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 307010,
			say = "But be that way if you want. It's a good thing if the decisive superweapon you've pinned your hopes on DOES get made.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 307010,
			say = "Just remember this one important thing – your weapon is going to need a core.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305100,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...I cannot rely on the Great Sage's power anymore now that the Watatsumi's concept has changed. I am obviously not going to use it on my allies.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 307010,
			say = "Of course. If you say so.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_508",
			paintingNoise = True,
			dir = 1,
			actor = 307010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "That leaves you with few options. If, in the end, all my efforts come to nothing, I myself shall become the core you need.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 305100,
			say = "That is the last thing I want to happen.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_508",
			paintingNoise = True,
			dir = 1,
			actor = 307010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Things will happen whether you like it or not. Even you, in all your wisdom, can't predict what will become of all this.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_508",
			paintingNoise = True,
			dir = 1,
			actor = 307010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Once the pieces start moving, there is no stopping them. Whatever happens, happens.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_508",
			paintingNoise = True,
			dir = 1,
			actor = 307010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "...That's everything I wanted to say.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_508",
			paintingNoise = True,
			dir = 1,
			actor = 307010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "By now, Amagi's prior soul should have drawn plenty of sustenance from the Sacred Sakura.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_508",
			paintingNoise = True,
			dir = 1,
			actor = 307010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "It's time to take my plan to the next stage.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 305100,
			say = "...Very well. I hope it goes as you hope.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_508",
			dir = 1,
			bgm = "theme-musashi-inside",
			actor = 9702050,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Interesting. So Akagi's faith in the path she chose is unyielding.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 305100,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Her choice can be considered the best of the countless ones we've evaluated over the years.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305100,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Do you have any misgivings with it, Jintsuu?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9702050,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "With the plan itself? No, none whatsoever.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 9702050,
			say = "My only question is if this \"Amagi,\" whose hull you wish to restore to full health, is truly Amagi as you know her.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 8,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 9702050,
			say = "Bear in mind that what's resting by the Sacred Sakura is only a fragment of her.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 8,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 9702050,
			say = "The seal on the Shadow of Corruption was broken, and the Watatsumi's essence has undergone change. The fragment of Amagi could not have gone unaffected through all that.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 9702050,
			say = "Now, I: understand that the Watatsumi is connected to the Sacred Sakura and that it is analogous to a Wisdom Cube in that it's a manifestation of your collective wishes...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 9702050,
			say = "That:es not, however, clarify if what you wish for is Amagi herself, or the concept of Amagi.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 305100,
			say = "...Regardless, Akagi is not going to stop.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 8,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 9702050,
			say = "Indeed, she won't. In the worst case, you'll at least get your core.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_508",
			actor = 305100,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Jintsuu, stop. You are not to speak of that.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 9702050,
			say = "Forgive me. I won't bring it up again.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 8,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 9702050,
			say = "...Are you moving forward with your superweapon plan?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305100,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "That is the idea.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_508",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 9702050,
			say = "Very well. Then what comes next will be of great importance.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			say = "Sakura Empire - Mikasa's Residence",
			bgm = "nagato-map",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			say = "*ka-dunk ka-dunk*",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			say = "Carts hauling their luggage rattled by outside the building.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 305050,
			say = "...Kawakaze. What is the meaning of this?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301490,
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It would seem these are Lady Mutsu's carts.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301490,
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I believe she has come to visit you, Lady Nagato.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 305050,
			say = "Mutsu is visiting? How did she know my whereabouts?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 305050,
			say = "Hmph. Elder Mikasa surely told her.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = 305060,
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actorName = "Mutsu",
			say = "Yuppers! I'd have had to look for ages if she hadn't told me!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305050,
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Mutsu, for heavens' sake! I told you and my other associates not to visit! I said I would return in a few days once I feel better!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 305050,
			say = "Elder Mikasa being meddlesome is one thing, but I didn't expect you to come barging in!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305060,
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Come on, you know it's not right to keep secrets!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305060,
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "You're not just:wn with a simple cold! You're bedridden because you're constantly overworking yourself!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			actor = 305060,
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "You need to take six months off to rest!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305060,
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "No, scratch that, a whole year!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305060,
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "In fact, you should go to the Sacred Sakura to get the best rest possible. Then you'll heal faster, and...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = 301370,
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actorName = "Minazuki",
			say = "Um... Lady Nagato? Is everything okay?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305050,
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...Minazuki?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305050,
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Did Mutsu bring you along?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305060,
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Nah. Well, kind of, but not really.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305060,
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I met her on the way here. She said she was going to meet you, so we came together.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305050,
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "*sigh*... Mutsu, go and wait in the parlor. Minazuki and I have something to discuss.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305060,
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "You're just looking for an excuse to dismiss me, aren't you?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305050,
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "No! You are right, and I will consider an extended rest later. For now, though, won't you give us a moment?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305060,
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Okay! Tell me when you're:ne!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301370,
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "\"Well met, Lady Nagato. I applaud how you handled the Sacred Divine.\"",
			bgm = "map-longgong",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 305050,
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I did only my duty as the Guardian Fox, Yamato. How fares Unzen?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301370,
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "\"She has lost all connection to the Great Sage's concept. She is Unzen the shipgirl once more.\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301370,
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "\"On the one hand, she has been freed of her duty as keeper of the Watatsumi's seal and may now wander. On the other, it has become impossible for her to connect to the Watatsumi.\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 305050,
			say = "I feared as much... Then what comes next?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301370,
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "\"Fear not. I have already summoned her to join me.\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 305050,
			say = "So she will be staying with you? Good. This puts the mind at ease.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301370,
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "\"Heaven's Door will be opened. I beseech you to join us, if possible, Lady Nagato.\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 305050,
			say = "I cannot, sadly. There is still something here I must:. I will not flee this time.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 305050,
			say = "You said Heaven's Door will open... Is the situation truly so dire?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305050,
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "What have you seen, Yamato?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301370,
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "\"...Lady Nagato.\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301370,
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "\"Recall Zuikaku posthaste, and keep Shimanto close by your side.\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301370,
			side = 2,
			bgName = "bg_unnamearea_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "\"A fire that will scorch the world is coming.\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
