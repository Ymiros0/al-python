return {
	id = "QINGKONGZHIXIA6",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			say = "Northern Parliament - Forward Operating Base",
			bgm = "battle-deepecho",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 705050,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Siste– Pardon. Comrade Soyuz, welcome to our frontline base.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 705050,
			say = "If you'll excuse me for skipping the pleasantries, but how is the situation with the Iris Orthodoxy?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			side = 2,
			actor = 900218,
			say = "While it's still in a delicate balance, Avrora and Pamiat' can handle Europa.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900218,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Our most pertinent objective lies here, in the Polar Sea.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 705050,
			say = "Indeed. It was good that you happened to be near the Mediterranean when that near-disaster happened.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			side = 2,
			actor = 900218,
			say = "I didn't \"happen\" to be there. All of us were stationed there.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 705050,
			say = "...What: you mean?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			side = 2,
			actor = 900218,
			say = "Nothing. Forget I mentioned it. More importantly, how is the mission progressing?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			side = 2,
			actor = 900218,
			say = "If I remember right, Kursk reported that an unidentified META had appeared in remote waters at the same time as the Mediterranean crisis happened, yes?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 705050,
			say = "Yes, ma'am. A fleet was dispatched to conduct reconnaissance.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 705050,
			say = "However, the META did not appear again until we had eliminated the Sirens' forces and recaptured the base.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900218,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "That is very strange...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			side = 2,
			actor = 900218,
			say = "If the META's goal was to eliminate Sirens, you'd think she would go on the offensive while our command chain was paralyzed due to what was happening in the Mediterranean.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			side = 2,
			actor = 900218,
			say = "And yet, she didn't: that. Then what if she had already achieved her goal? Or she went away for some other reason?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900218,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "She couldn't have come and left simply on a whim.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			side = 2,
			actor = 900218,
			say = "Please send me all the samples and reports. I will form a special task force and conduct a second investigation.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 705050,
			say = "Yes, ma'am. One more thing – unfortunately, we had no choice but to terminate our search for the mainframe yesterday.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			side = 2,
			actor = 900218,
			say = "May I ask why?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 705050,
			say = "It was due to the sudden failure of the Light of Dawn's beacon, which we used to detect the location of the mainframe.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			side = 2,
			actor = 900218,
			say = "What caused it?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 705050,
			say = "We:n't know yet, but we are looking into it. It will likely take a few days before we determine the cause.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 705050,
			say = "What we: know is that Kirov's fleet sighted a non-Siren unidentified flying object at the same time as the Light of Dawn malfunctioned.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 705050,
			say = "The reports claim the object had the appearance of an eyeball with a tail made of blue fire.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 705050,
			say = "Additionally, it was somehow completely undetectable on radar, and planes launched by the fleet were unable to pursue it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 705050,
			say = "Belorussiya speculates that it was responsible for the Light of Dawn's malfunction.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			side = 2,
			actor = 900218,
			say = "Do we know if the object was hostile?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 705050,
			say = "That is uncertain. It did not counterattack in response to their anti-air fire, it only evaded the flak.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900218,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Perhaps it was a new model the Sirens sent to both disrupt our search for the mainframe and to scout out our forces and firepower...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900218,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "It's a possibility, at least.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			side = 2,
			actor = 900218,
			say = "(We did recapture our base, destroy the Sirens' FOB, and eliminate one of Omitter's vessels. Our operation can be concluded as a success.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			side = 2,
			actor = 900218,
			say = "(This is the perfect chance to report to Naval HQ that we're:ne.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900218,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "...Thank you for the report. I'll inform Naval HQ that the operation is finished.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			side = 2,
			actor = 900218,
			say = "Abandon all areas captured from Omitter, excluding K-1 and K-4, and withdraw all the forces within those areas.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 705050,
			say = "What?! Even around the Crown and the polar stronghold?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			expression = 3,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			side = 2,
			actor = 900218,
			say = "Yes. There is no point in holding these areas while Omitter's mainframe remains intact.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900218,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "She will just continue whittling us:wn with her endless reinforcements, like how Compiler defended the NA Ocean before her mainframe was destroyed.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900218,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Now that the Europa situation has changed, the Sirens are surely going to make a big move.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			side = 2,
			actor = 900218,
			say = "The near future is uncertain, so it's time to go on the defensive and wait for a better opportunity.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 705050,
			say = "...As you say.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
