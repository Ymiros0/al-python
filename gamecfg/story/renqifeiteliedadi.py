return {
	fadeOut = 1.5,
	mode = 2,
	id = "RENQIFEITELIEDADI",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Hall of Fame\n\n<size=45>Date-Night Concert</size>",
					1
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_522",
			bgm = "bsm-5",
			say = "I went to attend a wonderful concert at Friedrich der Große's invitation.",
			hidePaintEquip = True,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_522",
			say = "Though the concert is now over, my heart is still reverberating to the music's lingering afterglow, making it difficult for me to compose myself.",
			hidePaintEquip = True,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_522",
			actor = 499020,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "So, how did you find that performance?",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_522",
			actor = 499020,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...Is what I planned on asking you originally. Though, you look a bit too intoxicated to give me any sort of concrete answer~",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_522",
			actor = 499020,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "That just means there's no need to pursue the question any further, haha.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_522",
			nameColor = "#A9F548FF",
			say = "The world built by music:es that to people. Thank you for bringing me along.",
			hidePaintEquip = True,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_522",
			nameColor = "#A9F548FF",
			say = "But, I am rather surprised that you sat with me in the audience. I thought... you'd be on stage, conducting or something.",
			hidePaintEquip = True,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 499020,
			side = 2,
			bgName = "star_level_bg_522",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Normally, yes, I would be the one to be putting on the performance.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_522",
			actor = 499020,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "But this time, I thought it would be nice to sit by your side, enjoying an intimate evening together...",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_522",
			actor = 499020,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Which is why I decided to be an ordinary listener. And that was my plan all along.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_522",
			actor = 499020,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Having that said, despite the performance being thoroughly enjoyable, there is one thing I regret...",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "...Regret?",
					flag = 1
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_522",
			actor = 499020,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Yes, just one thing.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_522",
			actor = 499020,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I regret that I am not the one to put on the performance, but rather other people.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_522",
			actor = 499020,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Because seeing you so thoroughly enraptured by other people's music, rather than my own... is making me a little jealous.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_522",
			nameColor = "#A9F548FF",
			say = "It's not such a bad thing being on this side of the stage every now and:, right?",
			hidePaintEquip = True,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_522",
			nameColor = "#A9F548FF",
			say = "I mean, the fact that I'm getting to enjoy a concert with THE Friedrich der Große is already an honor in its own right.",
			hidePaintEquip = True,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_522",
			actor = 499020,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I know you're just trying to flatter me, but... Thank you for saying that, my child.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_522",
			actor = 499020,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Haha. In the end though, you're right. I'm just glad that we could enjoy this performance without anything going awry.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_522",
			actor = 499020,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "In fact, there were a few times where I wanted to go up on that stage and grab the baton for myself... just so you would be making that intoxicated expression just for me.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_522",
			actor = 499020,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Luckily, you were holding onto my arm that whole time, which was the only reason I was able to restrain myself.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_522",
			say = "As Friedrich mentions it, only:: I notice the red markings on her fair skin.",
			hidePaintEquip = True,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "I'm so sorry... I had no idea I was clutching you so tightly without realizing it...",
					flag = 1
				},
				{
					content = "My bad... I guess my inner desire to stop you from leaving me was just too strong.",
					flag = 2
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_522",
			dir = 1,
			optionFlag = 1,
			hidePaintEquip = True,
			actor = 499020,
			nameColor = "#A9F548FF",
			say = "There's no need to apologize, my child. It's quite understandable––",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_522",
			dir = 1,
			optionFlag = 2,
			hidePaintEquip = True,
			actor = 499020,
			nameColor = "#A9F548FF",
			say = "Haha. I'm glad to hear you say that, but I already know––",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_522",
			actor = 499020,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "You were:ing that because you were sucked in so deeply by the music that it affected your emotions on a subconscious level.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_522",
			actor = 499020,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "The way you clung to me, like an innocent, fragile baby in need of protection – tugged at my heartstrings even more than the music itself.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_522",
			actor = 499020,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "If anything, I was overjoyed to see you need me so desperately.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_522",
			actor = 499020,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Hmm? It's embarrassing when I put it that way?",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_522",
			actor = 499020,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Haha,:n't worry. What happened today is just between the two of us. The way you looked will be deeply imprinted in my memory, but mine alone.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_522",
			actor = 499020,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "So, please continue to rely on me as much as you wish. We:n't get to spend time together like this all the time, so you can act a bit more spoiled, you know?",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_522",
			say = "After my eyes met with Friedrich's gentle gaze, I decided to give her the gift that I was planning to save until we got back home––",
			hidePaintEquip = True,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_522",
			bgm = "bsm-4",
			dir = 1,
			hidePaintEquip = True,
			actor = 499020,
			nameColor = "#A9F548FF",
			say = "Wh-what's this?! A conductor's baton... with our names engraved on it?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_522",
			actor = 499020,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "You had something like this custom-made, just for this occasion...",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_522",
			actor = 499020,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Haha... With this, I feel like I'll be able to make music more beautifully than ever before.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_522",
			nameColor = "#A9F548FF",
			say = "I look forward to hearing it.",
			hidePaintEquip = True,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_522",
			actor = 499020,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "And I look forward to playing it for you... My child,: you want to make a promise with me?",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_522",
			actor = 499020,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "That promise is...",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_522",
			actor = 499020,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Next time, I'll put on a performance for you, with the baton you just gifted me.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_522",
			actor = 499020,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "And during that performance... I will offer to you only the most splendid music, worthy of you becoming enraptured and engrossed within it.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_522",
			say = "She extends her hand and wriggles her pinky in front of me.",
			hidePaintEquip = True,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_522",
			say = "I also reach out and hook my pinky around hers. And with that, our promise is set.",
			hidePaintEquip = True,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_522",
			say = "At that moment, the music's afterglow suddenly swirls to life within my head, intertwining with the illusion of Friedrich standing up on the stage.",
			hidePaintEquip = True,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_522",
			say = "And I cannot help but to fall into the world of music once more––",
			hidePaintEquip = True,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_522",
			actor = 499020,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Haha. By the looks of things, we might have to postpone our future plans a little.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_522",
			actor = 499020,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Please enjoy this wonderful moment to your heart's content, my dear child.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
