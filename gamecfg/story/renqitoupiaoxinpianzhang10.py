return {
	fadeOut = 1.5,
	mode = 2,
	id = "RENQITOUPIAOXINPIANZHANG10",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			actor = 201210,
			side = 2,
			bgName = "star_level_bg_166",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I'm back, and with parfaits for everyone!",
			bgm = "votefes-start",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 301050,
			side = 2,
			bgName = "star_level_bg_166",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Thank you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401230,
			side = 2,
			bgName = "star_level_bg_166",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "The group B vote is almost upon us. It came around quickly, didn't it?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101170,
			side = 2,
			bgName = "star_level_bg_166",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It sure feels like it...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 201210,
			side = 2,
			bgName = "star_level_bg_166",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Time flies when you're having fun, that's why!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 201210,
			side = 2,
			bgName = "star_level_bg_166",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "By the way,: you know how many people from the group poll will advance to the finals?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401230,
			side = 2,
			bgName = "star_level_bg_166",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Let me: the math... The rules state that the top five people from every group make it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401230,
			side = 2,
			bgName = "star_level_bg_166",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Those in sixth place to fifteenth place, meanwhile, move on to the repechage round.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401230,
			side = 2,
			bgName = "star_level_bg_166",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "There are four group polls all in all, so those plus the fifteen who win the repechage makes it a total of...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401230,
			side = 2,
			bgName = "star_level_bg_166",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Thirty-five people in the finals.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 201210,
			side = 2,
			bgName = "star_level_bg_166",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Wow, that's a lot. The finals are gonna be so exciting!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401230,
			side = 2,
			bgName = "star_level_bg_166",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Mhm. I can hardly wait to the finalist lineup.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		}
	}
}
