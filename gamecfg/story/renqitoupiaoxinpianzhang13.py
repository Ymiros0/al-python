return {
	fadeOut = 1.5,
	mode = 2,
	id = "RENQITOUPIAOXINPIANZHANG13",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			actor = 201210,
			side = 2,
			bgName = "star_level_bg_527",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It looks like there are not just two, but three special votes.",
			bgm = "votefes-start",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 201210,
			side = 2,
			bgName = "star_level_bg_527",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "The Sirens were up first,: METAs. Who could be next?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101170,
			side = 2,
			bgName = "star_level_bg_527",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "There's your answer...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 201210,
			side = 2,
			bgName = "star_level_bg_527",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Oh! It's Little Bel!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301050,
			side = 2,
			bgName = "star_level_bg_527",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It's not just her. All the little ones are here.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401230,
			side = 2,
			bgName = "star_level_bg_527",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "They're so adorable it should be illegal to put them all in one place. I'm not surprised they made a polling round just for them.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 201210,
			side = 2,
			bgName = "star_level_bg_527",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "For once, TB:esn't need to simulate a promotional speech for a candidate. They promote themselves just by existing.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301050,
			side = 2,
			bgName = "star_level_bg_527",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Ugh. Thanks for reminding me of that horrific flurry of error messages she coughed up.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 201210,
			side = 2,
			bgName = "star_level_bg_527",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Oops... Anyway, they're getting up on stage! Come on, let's go watch!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
