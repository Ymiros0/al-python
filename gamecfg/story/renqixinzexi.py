return {
	fadeOut = 1.5,
	mode = 2,
	id = "RENQIXINZEXI",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Hall of Fame\n\n<size=45>A Perfect Schedule</size>",
					1
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_171",
			dir = 1,
			bgm = "story-richang-1",
			actor = 105170,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Good moooorning, honey~!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			say = "A gentle tap on the shoulder coupled with a distinctively spirited greeting makes it quite easy to figure out who it is without even turning to look.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105170,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It's way before the time we agreed on, you know? You didn't have to come this early!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 105170,
			say = "Oh...!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 105170,
			say = "I get it now. You must've been looking forward to today's date so badly that you came super early. Isn't that right, honey?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105170,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Mmhmm! After all, you're dating the super-cute, super-smart, super-powerful, the Black Dragonest, the lovely Miss New Jersey whom you've been so charmed by recently, so it's completely understandable!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "...the Black Dragonest?",
					flag = 1
				}
			}
		},
		{
			actor = 105170,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Ah, umm...! Well, y'know, I was just rattling off a lot of random stuff, b-but! I'm sure one of those things matters to you, right?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 105170,
			say = "L-let's set all that aside for now, since I was just joking anyway! In any case–",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "Actually, I WAS looking forward to it!",
					flag = 1
				},
				{
					content = "Actually, I was really, really looking forward to it!",
					flag = 2
				}
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 105170,
			say = "R-really? Oh, umm...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			actor = 105170,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Geez, honey, I didn't expect you to be that forward... You're gonna make me blush if you suddenly spring that on me!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 105170,
			say = "Ahem~ In any case, I've put together the perfect arrangement for today's date plans!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105170,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Honey, all you need to: is relax and follow my lead, and I'll make sure you have the happiest, most fulfilling day ever!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 105170,
			say = "Our first stop... is the movie theater!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "You want to go to the movies first thing in the morning?",
					flag = 1
				},
				{
					content = "Sounds like a plan!",
					flag = 2
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_171",
			dir = 1,
			optionFlag = 1,
			actor = 105170,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Because there's a really good movie showing at that time! I'm sure you're gonna love it too, honey!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_171",
			dir = 1,
			optionFlag = 2,
			actor = 105170,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Yay~ Looks like you're all fired up already, honey~! That's what I love to see!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105170,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Oh right, almost forgot...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 105170,
			say = "C'mon, take my hand! Heehee~ It won't feel like a date if we:n't lock fingers~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_171",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 105170,
			say = "All righty, time to set off... for the movie theater!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			say = "Just as New Jersey said, the movie theater was only our first stop. After that, we went to the shopping district, the aquarium...",
			bgm = "story-richang-5",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			say = "She said all I needed to: is follow her lead, but I never expected her to plan out a full day's worth of places and events in an orderly manner, without making a single miscalculation.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			say = "Eventually, the sun began to set–",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 105170,
			say = "Honey, this is the last stop on today's ride~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105170,
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "A family-owned burger joint that has Ticonderoga's seal of approval!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 105170,
			say = "The burgers they serve up are not only delicious and filling, but even come with random prizes!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105170,
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "You must be hungry after having fun all day, right? Then, let's hurry up and–",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			say = "Suddenly, New Jersey's words and footsteps fell silent as we approached the front of the closed:or.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105170,
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Due to certain circumstances, we will be closed for the day and will resume normal operating hours tomorrow~'...?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 105170,
			say = "...H-how could this be?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 105170,
			say = "Crap... I've really messed it up this time...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			say = "New Jersey had been totally confident and in her element all day, but now she's hanging her head in disappointment.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105170,
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I'm sorry, honey, I should've called in advance and been more thorough in my planning...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "There's only so much you can realistically:. These things: happen from time to time.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Don't worry. We can always try our luck somewhere else.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 105170,
			say = "Heehee. You're so nice, honey... Then, I shouldn't be sulking either!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105170,
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Oh... That's right! There was one other place I scoped out as a backup plan!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_523",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 105170,
			say = "C'mon, honey~ It's a Royal Isles restaurant that should be open right now. Let's go grab ourselves a good seat!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			say = "After dinner, New Jersey suddenly took out a notebook from her lap and started writing something:wn into it.",
			bgm = "story-richang-8",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 105170,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Gotta learn my lesson after this restaurant incident...",
			fontsize = 24,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(Wait, did New Jersey prepare an entire notebook for our date...?)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			say = "I can't help but be curious.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			say = "In the gaps between New Jersey's fingertips, I can vaguely see cutouts and maps attached to the notebook.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			say = "There are various circles and lines drawn on the maps, and they were even labeled with words that appeared to be \"critical offensive\" or \"all-out advance\"...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			say = "For whatever reason, the only words that enter my head are \"this looks like a full-blown battleplan\".",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			say = "In addition, there are a variety of sticky notes in the margins... They might have been added by Ticonderoga?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			say = "Wait... Seeing how seriously New Jersey is summarizing her experiences, maybe she was the one who wrote them as well?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 105170,
			say = "Hooooney! What are you staring at a girl's diary for?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 105170,
			say = "Every girl's gotta keep her secrets, y'know? I wouldn't let you see, even if you asked super nicely!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105170,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...But tell me the truth, you didn't see anything inside, did you?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "I saw a tactical battle map.",
					flag = 1
				},
				{
					content = "I saw some sticky notes.",
					flag = 2
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_160",
			dir = 1,
			optionFlag = 1,
			actor = 105170,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Heehee~ Close enough, but I'd call it more of a tactical date map~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_160",
			dir = 1,
			optionFlag = 2,
			actor = 105170,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Heehee~ Those are the collaborative efforts between me and the rest of the crew~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 105170,
			say = "Phew... Well, no harm no foul if that's all you saw. The plan's still in motion as long as you didn't see the itinerary~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 105170,
			say = "...Actually, I had a little surprise prepared for you during dinner.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105170,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Even though we didn't get to go to the burger joint, it should still be fine:ing it here.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 105170,
			say = "Honey, close your eyes... and:– open your mouth!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "New Jersey",
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			dir = 1,
			blackBg = True,
			say = "Just like that~ No peeking, okay?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			hidePaintObj = True,
			blackBg = True,
			say = "I hear the rustling of fabric, and: a faint fragrance reaches the tip of my nose.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			hidePaintObj = True,
			blackBg = True,
			say = "Something soft brushes against my lips, and the juicy sweetness of fruit blooms between my teeth.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "New Jersey",
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			dir = 1,
			blackBg = True,
			say = "Heehee. You can open your eyes now, honey~!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_160",
			dir = 1,
			bgm = "story-richang-6",
			actor = 105170,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Special surprise delivery complete~ The candy I just gave you was a specialty shipped straight from the Eagle Union. My favorite kind, actually. You can't get these from Akashi's place, you know?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 105170,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "So, what'd you think?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 105170,
			say = "Heehee. As long as you like it~ Also, this candy has special ingredients to relieve fatigue and lift your spirits!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105170,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Now, you'll be ready for the FINAL final leg of our date– our evening activities!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Wait... evening activities?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 105170,
			say = "That's right! A date can only be considered comprehensive if it fills up the entire day, so I've got stuff planned for tonight as well!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 105170,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It's almost time to get going, honey! Let's go ride the Ferris wheel!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 105170,
			say = "The scenery at the top is breathtaking!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			say = "Without giving me a chance to respond, or perhaps, already knowing how I'd respond, New Jersey stood up, took my hand, and started to run out of the restaurant.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_160",
			hidePaintObj = True,
			say = "...Fortunately, I already paid the bill in advance.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
