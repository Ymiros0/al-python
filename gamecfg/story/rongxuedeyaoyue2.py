return {
	fadeOut = 1.5,
	mode = 2,
	id = "RONGXUEDEYAOYUE2",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Invitation of Melting Snow\n\n<size=45>Melting in the Heat</size>",
					1
				}
			}
		},
		{
			side = 2,
			bgName = "star_level_bg_167",
			dir = 1,
			bgmDelay = 1.5,
			bgm = "theme-warmwinter-daily",
			actor = 701025,
			nameColor = "#A9F548FF",
			live2d = "headtouch",
			say = "I've never felt this hot in my whole life...",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 701025,
			side = 2,
			bgName = "star_level_bg_167",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Ugghh... I think I'm going to croak...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 701025,
			side = 2,
			bgName = "star_level_bg_167",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Comrade Commander, get me water... Cold water... Hurry...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 701025,
			side = 2,
			bgName = "star_level_bg_167",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "*gulp gulp*...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 701025,
			side = 2,
			bgName = "star_level_bg_167",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...Pfahh! Ahh, that's much better. I'm back from the brink.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 701025,
			side = 2,
			bgName = "star_level_bg_167",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It won't be long before it feels hot again, though. This back-and-forth is gonna make me melt...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 701025,
			side = 2,
			bgName = "star_level_bg_167",
			live2d = "main3",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...Why are you here anyway, Comrade Commander?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "\"Making sure you:n't melt like ice cream.\"",
					flag = 1
				},
				{
					content = "\"I want to enjoy the sauna with you.\"",
					flag = 2
				}
			}
		},
		{
			actor = 701025,
			side = 2,
			bgName = "star_level_bg_167",
			live2d = True,
			dir = 1,
			optionFlag = 1,
			nameColor = "#A9F548FF",
			say = "I'll be fine. The melting thing is just hyperbole, you know.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 701025,
			side = 2,
			bgName = "star_level_bg_167",
			live2d = True,
			dir = 1,
			optionFlag = 1,
			nameColor = "#A9F548FF",
			say = "Heat just isn't my strong suit. I'm used to the cold.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 701025,
			side = 2,
			bgName = "star_level_bg_167",
			live2d = "mission",
			dir = 1,
			optionFlag = 1,
			nameColor = "#A9F548FF",
			say = "If you really need to keep an eye on me, at least sit:wn. Here, I'll make some space.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 701025,
			side = 2,
			bgName = "star_level_bg_167",
			live2d = True,
			dir = 1,
			optionFlag = 2,
			nameColor = "#A9F548FF",
			say = "Oh, okay. I'll make some space so you can sit next to me.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 701025,
			side = 2,
			bgName = "star_level_bg_167",
			live2d = True,
			dir = 1,
			optionFlag = 2,
			nameColor = "#A9F548FF",
			say = "They say saunas are good for your circulation. A lot of my Northern Parliament friends use them.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 701025,
			side = 2,
			bgName = "star_level_bg_167",
			live2d = "touch",
			dir = 1,
			optionFlag = 2,
			nameColor = "#A9F548FF",
			say = "But in my case, they:n't: anything. Other than making me feel like I'll melt, anyway...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 701025,
			side = 2,
			bgName = "star_level_bg_167",
			live2d = True,
			dir = 1,
			optionFlag = 2,
			nameColor = "#A9F548FF",
			say = "If I were a snowman, I'd already be a puddle on the ground by now... Aahh...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 701025,
			side = 2,
			bgName = "star_level_bg_167",
			live2d = True,
			withoutActorName = True,
			nameColor = "#A9F548FF",
			dir = 1,
			say = "I sit:wn beside her and proceed to explain how saunas are meant to be used.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 701025,
			side = 2,
			bgName = "star_level_bg_167",
			live2d = "expedition",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...So after warming up a bit, you go out to take a cold bath,: have a break, rinse and repeat?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 701025,
			side = 2,
			bgName = "star_level_bg_167",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "That explains a lot... Yeah. Let me have an ice-cold oxy-cola to clear my head. *gulp gulp*...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 701025,
			side = 2,
			bgName = "star_level_bg_167",
			live2d = "main1",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...Food isn't allowed in the sauna? No drinks either?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 701025,
			side = 2,
			bgName = "star_level_bg_167",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "But I need to rehydrate! I won't leave this sauna alive without something to wet my whistle!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 701025,
			side = 2,
			bgName = "star_level_bg_167",
			live2d = "mission_complete",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Pl-please, one more glass with ice in it!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 701025,
			side = 2,
			bgName = "star_level_bg_167",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I promise, just this one glass,: I'll go out and rest!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 701025,
			side = 2,
			bgName = "star_level_bg_167",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...Pfahh! That hits the spot. I'm starting to enjoy this...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
