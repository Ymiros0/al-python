return {
	fadeOut = 1.5,
	mode = 2,
	id = "RONGXUEDEYAOYUE5",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Invitation of Melting Snow\n\n<size=45>Thawing in the Snow</size>",
					1
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_167",
			say = "In a misty open-air hot spring, there's a girl sitting on the blue-tinted stones, watching the snow around her melt.",
			bgmDelay = 1.5,
			bgm = "theme-warmwinter-daily",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 702071,
			side = 2,
			bgName = "star_level_bg_167",
			spine = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "There you are. Glad you kept your word, Comrade Commander.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "\"I'm not late, am I?\"",
					flag = 1
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_167",
			spine = True,
			dir = 1,
			actor = 702071,
			nameColor = "#A9F548FF",
			say = "Nope, you're on time. I finished changing just a minute ago.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_167",
			spine = True,
			dir = 1,
			actor = 702071,
			nameColor = "#A9F548FF",
			say = "What: you think of this place? I felt it'd be good to have a hot spring date, since you're always working so hard and deserve some relief.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_167",
			spine = True,
			dir = 1,
			actor = 702071,
			nameColor = "#A9F548FF",
			say = "The water is the perfect temperature, and the scenery is pretty gorgeous as well.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_167",
			spine = True,
			dir = 1,
			actor = 702071,
			nameColor = "#A9F548FF",
			say = "It's definitely one of my favorite spots. You can watch the snow fall and melt for hours while you rest up.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_167",
			spine = True,
			dir = 1,
			actor = 702071,
			nameColor = "#A9F548FF",
			say = "Ahem... What are you waiting for? Come on, let's go in the water.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 702071,
			side = 2,
			bgName = "star_level_bg_167",
			spine = True,
			withoutActorName = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Voroshilov and I have a great time in the hot spring.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 702071,
			side = 2,
			bgName = "star_level_bg_167",
			spine = True,
			withoutActorName = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "While the water warms my body, her touch warms my heart. I almost feel like I'm going to overheat.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_167",
			spine = True,
			dir = 1,
			actor = 702071,
			nameColor = "#A9F548FF",
			say = "Are you okay? Are you in over your head?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 702071,
			side = 2,
			bgName = "star_level_bg_167",
			spine = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Maybe the water is just a touch too hot? It would explain why you're blushing.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 702071,
			side = 2,
			bgName = "star_level_bg_167",
			spine = True,
			withoutActorName = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I:n't get the chance to explain myself before she starts clinging to me with a look of concern.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "\"The temperature's a bit high...\"",
					flag = 1
				},
				{
					content = "\"Not really. The temperature's fine.\"",
					flag = 2
				},
				{
					content = "\"That's just because I feel so refreshed.\"",
					flag = 3
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_167",
			spine = True,
			dir = 1,
			optionFlag = 1,
			actor = 702071,
			nameColor = "#A9F548FF",
			say = "Oh... Yeah, I see what you mean. I'm starting to sweat.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_167",
			spine = True,
			dir = 1,
			optionFlag = 1,
			actor = 702071,
			nameColor = "#A9F548FF",
			say = "That's why they call it a HOT spring, I suppose...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_167",
			spine = True,
			dir = 1,
			optionFlag = 1,
			actor = 702071,
			nameColor = "#A9F548FF",
			say = "(Moments alone with Comrade Commander:n't come every day... I need to seize this opportunity!)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_167",
			spine = True,
			withoutActorName = True,
			optionFlag = 2,
			actor = 702071,
			nameColor = "#A9F548FF",
			say = "It's not the water that's making me feel so hot. It's the fact that I'm rubbing shoulders with her.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_167",
			spine = True,
			dir = 1,
			optionFlag = 2,
			actor = 702071,
			nameColor = "#A9F548FF",
			say = "Then you've probably been soaking for too long...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_167",
			spine = True,
			dir = 1,
			optionFlag = 3,
			actor = 702071,
			nameColor = "#A9F548FF",
			say = "O-oh. That's all? You clearly handle the heat a lot better than I:...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_167",
			spine = True,
			dir = 1,
			optionFlag = 3,
			actor = 702071,
			nameColor = "#A9F548FF",
			say = "That's good, though. I'm glad to hear it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_167",
			spine = True,
			dir = 1,
			actor = 702071,
			nameColor = "#A9F548FF",
			say = "Whew... By the way, there's more stuff to: after this bath. We can go on a stroll through the forest or the city and take in the snowy vistas.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_167",
			spine = True,
			dir = 1,
			actor = 702071,
			nameColor = "#A9F548FF",
			say = "Anything you want, as long as I can be with you. I want you all to myself, at least for today.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 702071,
			side = 2,
			bgName = "star_level_bg_167",
			spine = True,
			withoutActorName = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "She holds my hand. It feels even warmer than the water.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_167",
			spine = True,
			dir = 1,
			actor = 702071,
			nameColor = "#A9F548FF",
			say = "This is only the start of our date. The best has yet to come.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
