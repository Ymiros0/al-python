return {
	fadeOut = 1.5,
	mode = 2,
	id = "SADINGDEYAOYUE3",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Where All Roads Lead\n\n<size=45>3 The Commander's Worries</size>",
					1
				}
			}
		},
		{
			say = "Somewhere in the Eagle Union...",
			side = 2,
			bgName = "bg_story_task",
			dir = 1,
			bgmDelay = 2,
			bgm = "main-arbitrationsystem-theme",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_task",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "W-wow, this isn't good... It's actually:uble plus ungood...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = False,
			side = 2,
			bgName = "bg_story_task",
			dir = 1,
			say = "Imagine my confusion when I got an \"Invitation to the World Expo\" from just about every camp in the world.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_task",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "They're all asking for you to go with them... right? And the communications are all private too...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = False,
			side = 2,
			bgName = "bg_story_task",
			dir = 1,
			say = "I:n't think anyone has any ill intentions, which is exactly why it's hard for me to say anything... Though, there's at least one sender who's:ing this to mess with me...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_task",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "But Sister Sara was just about to ask you if you had some free time in your schedule...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = False,
			side = 2,
			bgName = "bg_story_task",
			dir = 1,
			say = "For whatever reason, Saratoga looked like she was about to cry on the other side of the communicator.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_task",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Commander, listen to me! It's all one big conspiracy!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = False,
			side = 2,
			bgName = "bg_story_task",
			dir = 1,
			say = "\"Now that you mention it, it IS extremely unlikely that I'd receive pretty much the same message from everyone at roughly the same time.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_task",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "That's right! I knew this would happen the moment Sardegna announced that they'd be rebooting the World Expo!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = False,
			side = 2,
			bgName = "bg_story_task",
			dir = 1,
			say = "A festival held in the beautiful seas of Sardegna, where shipgirls from all over the world come together to compete and represent their proud cultures.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_task",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Of course there's nothing wrong with inviting you, Commander, but the question of whose invitation you'll accept is a major crisis!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_task",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "I also sent you an invitation, and you didn't even respond... But you were happily laughing and chatting with THAT girl as if you two were dating... *sniffle*",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = False,
			side = 2,
			bgName = "bg_story_task",
			dir = 1,
			say = "I wanted to correct the record and say that wasn't what was going on at all, and that I just wanted to participate as a regular, uninvolved party, but...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = False,
			side = 2,
			bgName = "bg_story_task",
			dir = 1,
			say = "There will definitely be a major problem if I make a wrong move. Plus, the Naval HQ will be watching my every move.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_task",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "So, what are you going to:? You're going to go with Sister Sara after all so that nobody else gets the wrong idea, right?",:
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = False,
			side = 2,
			bgName = "bg_story_task",
			dir = 1,
			say = "...I had to carefully think about what I wanted to: with these invitations. Except one. I slapped a big, fat \"NO\" on it, before sending it back.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_task",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "This sucks... We won't even be able to relax and have a good time there at this rate...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = False,
			side = 2,
			bgName = "bg_story_task",
			dir = 1,
			say = "\"There's nothing wrong with being in a festive mood, but the World Expo is a weeks-long event. It's not worth getting into trouble.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_story_task",
			say = "\"Let's focus on the task at hand first, and once everything blows over, we'll enjoy the rest of the event like ordinary people.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_task",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "F-fine, I suppose you're right...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_task",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "In case you were wondering, Essex is the Eagle Union's representative, so there's nothing to worry about.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_task",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Oh, before I forget! I know you can't go right now, but here are some pictures of Eagle Union's pavilion! It has sci-fi vibes and looks super badass,:esn't it?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_task",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "When you get there, you definitely have to go check it out... *click*",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = False,
			side = 2,
			bgName = "bg_story_task",
			dir = 1,
			say = "*knock* *knock*",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 101260,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Commander... Eldridge is here to remind you... that the next round of equipment testing is starting...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 101260,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Um... I thought I heard Saratoga's voice...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_task",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Anyway, I'll leave it there for now! We're still busy setting up for the expo over there, so I'll talk to you later!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = False,
			side = 2,
			bgName = "bg_story_task",
			dir = 1,
			say = "After seeing Eldridge come into the room, Saratoga scrambled to end the call.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_task",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Bye bye! Sister Sara out!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = False,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 101260,
			say = "Commander, are you okay?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 101260,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "If you're tired... Eldridge will try to... get the tests pushed back.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_story_task",
			dir = 1,
			blackBg = True,
			say = "\"No, I'm fine. You know what they say. work hard, play hard––\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
