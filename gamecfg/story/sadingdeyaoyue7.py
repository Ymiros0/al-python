return {
	fadeOut = 1.5,
	mode = 2,
	id = "SADINGDEYAOYUE7",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Where All Roads Lead\n\n<size=45>7 The Great Peng Takes Flight</size>",
					1
				}
			}
		},
		{
			say = "Somewhere in the Dragon Empery...",
			side = 2,
			bgName = "star_level_bg_144",
			dir = 1,
			bgm = "china-party",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 501010,
			side = 2,
			bgName = "star_level_bg_144",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Good evening, Miss Yat Sen.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_144",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 599010,
			say = "Yat Sen cooking dinner for us? Has the karma of the cosmos turned upside:wn or something?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_144",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 506010,
			say = "Oh my, if it isn't An Shan and Harbin.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_144",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 599010,
			say = "Chen Hai's here too? Well, something big must be happening if we're all here in one place, heh...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 502010,
			side = 2,
			bgName = "star_level_bg_144",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Please be seated, you two. Let's have a little chat over dinner.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_144",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 599010,
			say = "Woah... What the heck did you cook for us today? Is this... a vulture?",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_144",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 506010,
			say = "It's the Great Peng.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_144",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 599010,
			say = "It looks pretty exquisite, but the portions are a bit...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 502010,
			side = 2,
			bgName = "star_level_bg_144",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "What did I say, Chen Hai? You know how Harbin is.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_144",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 506010,
			say = "Yes, yes, I should've known better. I'll go bring out the rest of the portion.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_144",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 506010,
			say = "This should be enough to eat to your heart's content, yes?",
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_144",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 599010,
			say = "Haha! That it is! The best food is whatever fills your stomach up!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 502010,
			side = 2,
			bgName = "star_level_bg_144",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Alright:. Everyone, have you heard about Sardegna's World Expo?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 501010,
			side = 2,
			bgName = "star_level_bg_144",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Sorry, but I've been a bit out of the loop... All the training recently has taken a lot out of me.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_144",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 599010,
			say = "Yeah, I heard about it. Like, five seconds ago.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_144",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 599010,
			say = "So what's this expo thing, and are going?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 502010,
			side = 2,
			bgName = "star_level_bg_144",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "What: you all think? We've been invited, and the Commander has also been contacted regarding this matter.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 502010,
			side = 2,
			bgName = "star_level_bg_144",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "So I: wonder if this is the right opportunity for us.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_144",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 599010,
			say = "Right opportunity? What are you... Wait,: you mean–",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 502010,
			side = 2,
			bgName = "star_level_bg_144",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Yes, we're about to make a big announcement, right?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 502010,
			side = 2,
			bgName = "star_level_bg_144",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "\"Upon the wind the Great Peng soars into the sky, in a single day ninety-thousand meters high.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 502010,
			side = 2,
			bgName = "star_level_bg_144",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "In other words, the Dragon Empery shall step onto the world stage as a major player.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 502010,
			side = 2,
			bgName = "star_level_bg_144",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "As such, let us enliven the World Expo together and make it the prelude to our great announcement.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_144",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 599010,
			say = "I like the sound of that! So, have you decided on who's going? Is there anything I can:?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_144",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 599010,
			say = "I, for one, would love to see Europa in person!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 502010,
			side = 2,
			bgName = "star_level_bg_144",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Hehe. Sorry, Harbin, but you'll have to wait for your turn.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_144",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 599010,
			say = "I guess that's True... Sucks, but I'll just have to deal with it! Ahaha!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 501010,
			side = 2,
			bgName = "star_level_bg_144",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Then, who will be our representative?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 502010,
			side = 2,
			bgName = "star_level_bg_144",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "That has already been decided.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 502010,
			side = 2,
			bgName = "star_level_bg_144",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "The fact that we've been relatively isolated from the other camps up until this point has been both a blessing and a curse.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 502010,
			side = 2,
			bgName = "star_level_bg_144",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Naturally, the others have little knowledge of the cards we hold in our hands.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 502010,
			side = 2,
			bgName = "star_level_bg_144",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "But we also know very little about them.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 502010,
			side = 2,
			bgName = "star_level_bg_144",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Therefore, our top priority at this expo should be to learn as much as we can about the other camps, and bring that knowledge back.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 502010,
			side = 2,
			bgName = "star_level_bg_144",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Chen Hai, will you make the journey to Sardegna?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 502010,
			side = 2,
			bgName = "star_level_bg_144",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Take Hai Tien and Hai Chi with you, and broaden your horizons for the sake of the Dragon Empery. Then––",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_144",
			dir = 1,
			blackBg = True,
			actor = 506010,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Display the allure of the Dragon Empery to the world, yes? I will try not to disappoint you. Hehehe~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
