return {
	fadeOut = 1.5,
	mode = 2,
	id = "SANLI02",
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			actor = 303120,
			nameColor = "#a9f548",
			side = 2,
			actorName = "{namecode.67}",
			dir = -1,
			say = "Dear me, after all that talk about the Sirens possessing the power of Poseidon, this is all they amount to...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 303110,
			side = 0,
			nameColor = "#a9f548",
			actorName = "{namecode.66}",
			dir = 1,
			say = "A fitting end for these soulless husks...",
			paintingFadeOut = {
				time = 0.5,
				side = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 303110,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.66}",
			dir = 1,
			say = "The unrighteous shall not stand before my blade!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307050,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.95}",
			dir = 1,
			say = "Phew... Thank goodness these were just mass-produced Pawns... I'd hate to come across one of their leaders.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301050,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.6}",
			dir = 1,
			say = "Attention, detecting a large target to our front... Signal identified as... Tester-α.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307050,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.95}",
			dir = 1,
			say = "……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.96}",
			dir = 1,
			say = "Yeah, uh, sis... might be best if you'd keep quiet...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900011,
			nameColor = "#ff0000",
			side = 1,
			actorName = "Tester-α",
			dir = 1,
			say = "Well, well, it seems we have guests... I was expecting to see the Azur Lane, though.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.96}",
			dir = 1,
			say = "Are you the \"Creator\" Akagi spoke of?!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900011,
			nameColor = "#ff0000",
			side = 1,
			actorName = "Tester-α",
			dir = 1,
			say = "Ahaha, I am no Creator. My kin and I are simply here to offer humanity, as well as your kind, a choice.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900011,
			nameColor = "#ff0000",
			side = 1,
			actorName = "Tester-α",
			dir = 1,
			say = "Either join us and sacrifice the present in the search for evolution, preventing a future of certain:om...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900011,
			nameColor = "#ff0000",
			side = 1,
			actorName = "Tester-α",
			dir = 1,
			say = "Or reject change and proceed:wn the path of annihilation, like you have before.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.96}",
			dir = 1,
			say = "Sorry, we're not interested in cooperating with the likes of you. Go back to whatever abyss you crawled out of!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900011,
			nameColor = "#ff0000",
			side = 1,
			actorName = "Tester-α",
			dir = 1,
			say = "We learned much about you from your time in the Sanctuary. Indeed, we have already gathered a lot of fascinating data.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900011,
			nameColor = "#ff0000",
			side = 1,
			actorName = "Tester-α",
			dir = 1,
			say = "But unfortunately for you, your current selves have yet to exceed our Simulations' output.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900011,
			nameColor = "#ff0000",
			side = 1,
			actorName = "Tester-α",
			dir = 1,
			say = "Your cuboid souls were shaped by thousands upon thousands of simulations. That is the core of your existence...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900011,
			nameColor = "#ff0000",
			side = 1,
			actorName = "Tester-α",
			dir = 1,
			say = "Your steadfast \"soul\" brims with confidence, driving you to reject what you perceive as \"evil.\" Yet you lack the capacity to understand your True nature...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.96}",
			dir = 1,
			say = "All you: is spout a bunch of nonsense... If it's a fight you're looking for,: just say it! When I'm:ne with you, maybe you'll learn to stay home next time!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900011,
			nameColor = "#ff0000",
			side = 1,
			actorName = "Tester-α",
			dir = 1,
			say = "Ahaha... Well, we:n't have guests here every day, so I suppose it's time we conduct another experiment.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900011,
			nameColor = "#ff0000",
			side = 1,
			actorName = "Tester-α",
			dir = 1,
			say = "Come! Show me what you're worth!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
