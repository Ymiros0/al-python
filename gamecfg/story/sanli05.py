return {
	fadeOut = 1.5,
	mode = 2,
	id = "SANLI05",
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			actor = 900011,
			nameColor = "#ff0000",
			side = 2,
			actorName = "Tester-α",
			dir = 1,
			say = "*WARNING. DAMAGE LEVELS CRITICAL. CONTROL SYSTEMS DESTROYED.*",
			soundeffect = "event./ui/alarm",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900011,
			nameColor = "#ff0000",
			side = 2,
			actorName = "Tester-α",
			dir = 1,
			say = "What...?! The Simulations said nothing of this!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900011,
			nameColor = "#ff0000",
			side = 2,
			actorName = "Tester-α",
			dir = 1,
			say = "How could an ancient, outdated battleship: this to me?!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305110,
			nameColor = "#a9f548",
			side = 2,
			actorName = "{namecode.82}",
			dir = 1,
			say = "A cannon that lands its every shot fights on even ground against a hundred cannons that only land one-hundredths of theirs.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305110,
			nameColor = "#a9f548",
			side = 2,
			actorName = "{namecode.82}",
			dir = 1,
			say = "The difference in firepower between us is not the decisive factor!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900011,
			nameColor = "#ff0000",
			side = 2,
			actorName = "Tester-α",
			dir = 1,
			say = "Fascinating... A miscalculation has reared itself. I'll forgive you for now and take my leave.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900011,
			nameColor = "#ff0000",
			side = 2,
			actorName = "Tester-α",
			dir = 1,
			say = "I've prepared an escape route for you.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900011,
			nameColor = "#ff0000",
			side = 2,
			actorName = "Tester-α",
			dir = 1,
			say = "If you hope to change history,: go ahead and try.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900011,
			nameColor = "#ff0000",
			side = 2,
			actorName = "Tester-α",
			dir = 1,
			say = "However, there is one thing you should remember. we are always watching you... Ahaha!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			nameColor = "#a9f548",
			side = 2,
			actorName = "{namecode.96}",
			dir = 1,
			say = "Wait! Get back here!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307050,
			side = 1,
			nameColor = "#a9f548",
			actorName = "{namecode.95}",
			dir = 1,
			say = "No, Zuikaku! There's no point in pursuing her!",
			paintingFadeOut = {
				time = 0.5,
				side = 0
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900011,
			nameColor = "#ff0000",
			side = 2,
			actorName = "Tester-α",
			dir = 1,
			say = "Hahaha! You would: well to listen to her, unless you're that eager to throw away the life I just spared.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900011,
			nameColor = "#ff0000",
			side = 2,
			actorName = "Tester-α",
			dir = 1,
			say = "Oh, and next time, make a reservation before coming here, will you? Bye-bye, little ones~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
