return {
	fadeOut = 1.5,
	mode = 2,
	id = "SANLI07",
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			actor = 301170,
			nameColor = "#a9f548",
			side = 2,
			actorName = "{namecode.19}",
			dir = 1,
			say = "L-listen, everyone! A person with an incredible air of grandeur has just shown up in the harbor!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301130,
			side = 1,
			nameColor = "#a9f548",
			actorName = "{namecode.15}",
			dir = 1,
			say = "An air of grandeur...? Wh-what:es that mean?",
			paintingFadeOut = {
				time = 0.5,
				side = 0
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301170,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.19}",
			dir = 1,
			say = "It means... Well, it basically means they're a grandiose person! She's a veteran from the war!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301130,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.15}",
			dir = 1,
			say = "Oooh! I:n't really get the hype, but I know what grandiose means!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301150,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.17}",
			dir = 1,
			say = "Old news! Ayanami told me about that ages ago!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301150,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.17}",
			dir = 1,
			say = "I heard all the cruisers and battleships are already on their way to meet her, but... is she seriously that big of a deal?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301170,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.19}",
			dir = 1,
			say = "Of course she is! She was the Combined Fleet's flagship during the first war! She's amazing, and I want to take a photo with her!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301180,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.20}",
			dir = 1,
			say = "Ehehe... This smells like a good business opportunity...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301140,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.16}",
			dir = 1,
			say = "Is she the reason why the canteen closed early?! That's animal cruelty! I'm starving here!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301110,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.13}",
			dir = 1,
			say = "Hmm, I see! But I thought most who fought in the war went off somewhere, never to be seen again...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301120,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.14}",
			dir = 1,
			say = "Don't you think it's strange that she's come here all of a sudden...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301130,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.15}",
			dir = 1,
			say = "Zzzzzzz...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301170,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.19}",
			dir = 1,
			say = "Well, if you're not coming with me, so be it! I'll head to the harbor on my own!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			say = "Port - Docking Area",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			nameColor = "#a9f548",
			side = 2,
			actorName = "{namecode.82}",
			dir = 1,
			say = "Ohh! What magnificence!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305110,
			nameColor = "#a9f548",
			side = 2,
			actorName = "{namecode.82}",
			dir = 1,
			say = "Located in a strategic position, equipped with every facility one could desire, and stockpiled with supplies and items of all manner...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305110,
			nameColor = "#a9f548",
			side = 2,
			actorName = "{namecode.82}",
			dir = 1,
			say = "Mhm. It has everything a naval port should have.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307050,
			side = 1,
			nameColor = "#a9f548",
			actorName = "{namecode.95}",
			dir = 1,
			say = "As a naval port, specifically?",
			paintingFadeOut = {
				time = 0.5,
				side = 0
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305110,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.82}",
			dir = 1,
			say = "Indeed. Back in my day, I made many visits to naval and civilian ports alike, you see... That aside, I must say, there are many things unfamiliar to me in this new age...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305110,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.82}",
			dir = 1,
			say = "Especially these soaring... what did you call them, \"ear-playings\"?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307050,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.95}",
			dir = 1,
			say = "You mean... airplanes, I believe?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307050,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.95}",
			dir = 1,
			say = "Yes, compared to your time in service, airplanes are much more pivotal to controlling the flow of combat than cannons. Like in the battle the other day...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305110,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.82}",
			dir = 1,
			say = "Hah, you:n't need to be so demure. I may be behind on the times, but there's no need to worry so much about me! Here, cheer up!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305110,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.82}",
			dir = 1,
			say = "I'll admit, I could feel my age in our battle with that \"Siren\" individual.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305110,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.82}",
			dir = 1,
			say = "But the thrill of battle hasn't changed after all this time.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305110,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.82}",
			dir = 1,
			say = "The chills from your enemy's murderous intent, the smell of gunpowder, the heat of a salvo, the shriek of clashing metal...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305110,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.82}",
			dir = 1,
			say = "All these things have stayed just the same.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.96}",
			dir = -1,
			say = "You really live up to your name as a veteran... I deeply admire you, Mikasa!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305110,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.82}",
			dir = 1,
			say = "N-now, now... There's no need for excessive praise. I, and all the others of my time, merely did what was necessary of us.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305110,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.82}",
			dir = 1,
			say = "If anything, it is you young ones who deserve praise. Your valiant effort during that battle was truly astounding.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.96}",
			dir = -1,
			say = "Um, I have a question, if you:n't mind! I'd like to know how I can become as strong as you, Mikasa!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305110,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.82}",
			dir = 1,
			say = "You're quite the straightforward one... Very well. There is one critical thing that you are missing, that has nothing to: with your armaments.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305110,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.82}",
			dir = 1,
			say = "Something indispensable to ultimately achieving victory.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.96}",
			dir = -1,
			say = "\"Ultimate victory\"...! You know of a secret, special training method? If it isn't about our equipment,: I assume it has to be something like-",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305110,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.82}",
			dir = 1,
			say = "Nay. You may be ill-disposed to understand it, but, if you ask an old-timer battleship like me...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305110,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.82}",
			dir = 1,
			say = "What truly determines who emerges victorious in a battle is not equipment, tactics, or logistics.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305110,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.82}",
			dir = 1,
			say = "The thing that it ultimately comes:wn to... is the strength of your convictions.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
