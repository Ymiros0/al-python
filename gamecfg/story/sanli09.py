return {
	fadeOut = 1.5,
	mode = 2,
	id = "SANLI09",
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			actor = 307060,
			nameColor = "#a9f548",
			side = 2,
			actorName = "{namecode.96}",
			dir = 1,
			say = "Mikasa, I wanted to thank you for yesterday. Admittedly, there were a few things you mentioned I didn't quite understand...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305110,
			side = 1,
			nameColor = "#a9f548",
			actorName = "{namecode.82}",
			dir = -1,
			say = "In that case, why:n't we have a friendly duel?",
			paintingFadeOut = {
				time = 0.5,
				side = 0
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.96}",
			dir = 1,
			say = "A duel... with you?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305110,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.82}",
			dir = -1,
			say = "With whom else? Rigorous training builds character. You might even learn a thing or two.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.96}",
			dir = 1,
			say = "Okay! I'll try not to hold myself back! Say, the exercise area is vacant right now, so how about we go there right now?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305110,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.82}",
			dir = -1,
			say = "We could just as well: it here... but very well. Let's head over:. Oh, Zuikaku, before I forget...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305110,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.82}",
			dir = -1,
			say = "(She said dancing isn't her forte, but she didn't step on my toes at any point during the dance...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			say = "A short while later...",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.82}",
			dir = -1,
			say = "Come on, Zuikaku! Where'd your vigor from the battle with the Siren go?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305110,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.82}",
			dir = -1,
			say = "Your attacks are all too stilted! With but one glance, I can predict where you're aiming!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.96}",
			dir = 1,
			say = "So I really:n't stand a chance against you after all...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305110,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.82}",
			dir = -1,
			say = "What hogwash! You aren't fighting me earnestly!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.96}",
			dir = 1,
			say = "... There's nothing wrong with my equipment, so why... Why: I struggle against you?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305110,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.82}",
			dir = -1,
			say = "Your wit still fails you, I see. Compared to when you fight the Sirens or Enterprise, there is a decisive difference in how you approach a battle with me.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.96}",
			dir = 1,
			say = "...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305110,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.82}",
			dir = -1,
			say = "You are afraid to injure me, thus all your attacks are muddled by hesitation.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305110,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.82}",
			dir = -1,
			say = "Some simply overthink war, and: there's the other side of the coin - you. Once on the battlefield, focus only on the battle!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.96}",
			dir = 1,
			say = "That's easier said than:ne...!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305110,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.82}",
			dir = -1,
			say = "I understand your anxiety. However, if you cannot even choose your own path,: nobody else will be able to help you.",:
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305110,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.82}",
			dir = -1,
			say = "It is your decision if you wish to stay in my shadow. But if you pledge to \"wager everything,\": commit to seeing it through!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.96}",
			dir = 1,
			say = "Commit to it... I understand now. Mikasa, would you please have another duel with me someday?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305110,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.82}",
			dir = -1,
			say = "Absolutely. My guidance shall serve as a stepping stone.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305110,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.82}",
			dir = -1,
			say = "But,: not rely on me as a crutch. Afterwards, you must move forward through your own strength.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = -1,
			side = 1,
			blackBg = True,
			say = "……",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			actorName = "{namecode.82}",
			side = 1,
			nameColor = "#a9f548",
			dir = -1,
			say = "Phew... here we are.",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307050,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.95}",
			dir = -1,
			say = "What is this flag for...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305110,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.82}",
			dir = -1,
			say = "This flag once led me to victory.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305110,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.82}",
			dir = -1,
			say = "And with it, you too will surely find your own path to victory.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305110,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.82}",
			dir = -1,
			say = "Dishearten the enemy with your dignity, bolster your shortcomings with your courage, and seize victory in the very end with your conviction and willpower.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305110,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.82}",
			dir = -1,
			say = "Miracles have never existed; not in my time, and certainly not now. Miraculous victories, however,: exist. And now, I leave my flag, as well as my conviction, in your hands!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
