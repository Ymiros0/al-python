return {
	fadeOut = 1.5,
	mode = 2,
	id = "SHANCHENG01",
	once = True,
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			mode = 1,
			sequence = {
				{
					"Winter Offensive\n\n<size=45>Chapter 1 - A Kotatsu in the Winter</size>",
					1
				}
			}
		},
		{
			actor = 305020,
			nameColor = "#a9f548",
			side = 2,
			actorName = "{namecode.79}",
			dir = 1,
			say = "Phew... there's nothing quite like a warm kotatsu during the winter...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305020,
			nameColor = "#a9f548",
			side = 2,
			actorName = "{namecode.79}",
			dir = 1,
			say = "Ahh... I:n't want to ever get out...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305010,
			side = 0,
			nameColor = "#a9f548",
			actorName = "{namecode.78}",
			dir = 1,
			say = "My, my, is that how you should be acting?",
			paintingFadeOut = {
				time = 0.5,
				side = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305020,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.79}",
			dir = 1,
			say = "Ehh? But Fusou, you look pretty cozy too...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305010,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.78}",
			dir = 1,
			say = "Umm... *cough*",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305020,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.79}",
			dir = 1,
			say = "It's not like I have a choice... it's way too cold this year...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305010,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.78}",
			dir = 1,
			say = "We're going to get all chubby at this rate. Even if it's cold, we're going to need to get some exercise.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305020,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.79}",
			dir = 1,
			say = "Ehh? In this weather? What kind of stuff is there to:?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305010,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.78}",
			dir = 1,
			say = "Hmm... how about cold-water freediving? We'd also be able to show some guts in the process. Heehee~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305020,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.79}",
			dir = 1,
			say = "Jeez, Fusou,:n't even make jokes about that... just thinking about it is making me cold...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
