return {
	fadeOut = 1.5,
	mode = 2,
	id = "SHANCHENG06",
	once = True,
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			mode = 1,
			sequence = {
				{
					"Winter Offensive\n\n<size=45>Chapter 6 - No Guts, No Glory!</size>",
					1
				}
			}
		},
		{
			actor = 305040,
			nameColor = "#a9f548",
			side = 2,
			actorName = "{namecode.77}",
			dir = 1,
			say = "H-h-h-h-h-how's that?! Don't underestimate the S-Sakura Fleet!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305030,
			nameColor = "#a9f548",
			side = 2,
			actorName = "{namecode.76}",
			dir = 1,
			say = "S-See? A-A-A-All you n-n-need is g-guts and w-winter:esn't stand a ch-chance!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 303112,
			nameColor = "#a9f548",
			side = 2,
			actorName = "{namecode.66}",
			dir = 1,
			say = "Nooooooo! M-Miss Saratoga! N-N-Next time, I won't lose to you again!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 107031,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "Ah... Achoo!! Wh-why did I agree to participate in this meet in this first place...? It's practically hazing!!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 0,
			side = 2,
			nameColor = "#a9f548",
			say = "……",
			dir = 1,
			withoutPainting = True,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305020,
			nameColor = "#a9f548",
			side = 2,
			actorName = "{namecode.79}",
			dir = 1,
			say = "……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305020,
			nameColor = "#a9f548",
			side = 2,
			actorName = "{namecode.79}",
			dir = 1,
			say = "You seem... pretty cold...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305020,
			nameColor = "#a9f548",
			side = 2,
			actorName = "{namecode.79}",
			dir = 1,
			say = "M-Maybe we should find some other kind of training...? This:esn't seem very healthy. Don't you agree, Milord?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 0,
			side = 0,
			nameColor = "#a9f548",
			say = "*nods*",
			dir = 1,
			withoutPainting = True,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
