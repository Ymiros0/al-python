return {
	fadeOut = 1.5,
	mode = 2,
	id = "SHENDUHUIYIN1",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			say = "Northern Parliament Base - Location Unknown",
			side = 2,
			bgName = "bg_guild_blue_n",
			dir = 1,
			bgmDelay = 1,
			bgm = "bgm-cccp3",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 900218,
			say = "...Our intentions are as stated in our message to the Eagle Union command.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 900218,
			say = "The Northern Parliament will commence a retaliatory operation against the Iron Blood for their senseless attacks on our relief supplies.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 900218,
			say = "We have requested the Commander's assistance in this campaign.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 900218,
			say = "This operation has the approval of the highest echelons of our leadership, and holds the highest degree of priority for the Northern Parliament.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 900218,
			say = "There is no rescinding or halting it now. We will continue on this path, no matter what the Eagle Union's reply may be.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 900218,
			say = "Let it be known, we are mobilizing with one goal in mind. putting a stop to this war.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_guild_blue_n",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "What?! You can't be serious! You can't go forward with something this big all willy-Nonely!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_guild_blue_n",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "We've worked hard enough just to get the situation to where it is right now! This isn't something you can just decide without consulting the rest of us first!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 107030,
			say = "(Did they find out about the Iron Blood's experiments? Is that what brought this on? Either way, this is NOT a good time to be settling scores...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_guild_blue_n",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Please, let's put this on the backburner for now. There are still Sirens active in the Arctic areas, right? That should be our next priority for now.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 502010,
			say = "Saratoga, I have to disagree. The Crimson Axis has attacked Northern Parliament supply vessels under the pretense of a \"Reenactment.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 502010,
			say = "While none of us wish to be mired in pointless infighting, if push comes to shove, we must stand our ground.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 502010,
			say = "Soyuz's plan follows this principle. It is also worth pointing out that the Commander has not shown any opposition to this plan either.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 502010,
			say = "And I suspect, we will soon find out about what they truly have in mind. Let us wait and see how things unfold, Saratoga.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_guild_blue_n",
			paintingNoise = True,
			dir = 1,
			actor = 205050,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "I concur. It seems inconceivable that Soyuz would just accidentally \"leak\" her plans to the Naval HQ.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205050,
			say = "Know that we would like to see the Iron Blood be put in their place. Yet for the sake of maintaining even this delicate status quo, the situation should be left to the Commander.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_guild_blue_n",
			paintingNoise = True,
			dir = 1,
			actor = 205050,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Still, the Crimson Axis's movements bring us no peace of mind. Mass-produced ships have been seen entering and exiting Toulon, and the Vichya Dominion also seems poised to move.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_guild_blue_n",
			paintingNoise = True,
			dir = 1,
			actor = 205050,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Though Sardegna and the Sakura Empire have been silent, who knows how long that will last? What else will be:ne under the guise of a \"Reenactment?\"",:
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205050,
			say = "Therefore, the question that the Royal Navy has for you is. How far are you willing to go?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 900218,
			say = "The fact that we're requesting the Commander's aid should assure you that we seek a measured response.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 900218,
			say = "Moreover, I have a general understanding of the Commander's response as well as the scope of the Iron Blood's movements.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_guild_blue_n",
			paintingNoise = True,
			dir = 1,
			actor = 205050,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "I see. The last thing we would want is for humanity to devolve into bickering while the Siren threat still looms large.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_guild_blue_n",
			paintingNoise = True,
			dir = 1,
			actor = 205050,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "However, I am not impressed by your uNoneateral announcement.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 900218,
			say = "All I can say is, we all have our own circumstances.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900218,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I hope we can still count on the Royal Navy's assistance in supplying the Northern Parliament's war efforts, lest we lose our initiative.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_guild_blue_n",
			paintingNoise = True,
			dir = 1,
			actor = 205050,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Very well. We will maintain the flow of supplies as before so that the Northern Parliament can continue to push back against the Sirens.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_guild_blue_n",
			paintingNoise = True,
			dir = 1,
			actor = 205050,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Surely, that is what the Commander would want as well.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_guild_blue_n",
			paintingNoise = True,
			dir = 1,
			actor = 205050,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "However, should a large-scale conflict break out, know in advance that the Royal Navy will not directly participate.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_guild_blue_n",
			paintingNoise = True,
			dir = 1,
			actor = 205050,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "At least not in the opening act of the engagement. I'm sure that Iris Libre shares my view on this?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 805010,
			say = "Indeed. Iris Libre cannot afford to be dragged into a conflict of this nature.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 805010,
			say = "I echo the Royal Navy's sentiment in asking that the Northern Parliament's operation not interfere with the other factions.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 805010,
			say = "Especially in matters concerning the Vi– Ahem... In any case, we shall handle our own internal affairs.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 900218,
			say = "Rest assured. This is the reason why I requested the Commander's assistance.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 900218,
			say = "We are aware of the situation with the Vichya, Cardinal. You have my word that we will not interfere.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 900218,
			say = "And finally, we promise to not use any of the Royal Navy's aid for this operation, focusing it solely towards our efforts against the Sirens.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_guild_blue_n",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "(Bleh, she sounds way too sure about all of this. Talk about too good to be True...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_guild_blue_n",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "So I have a question! What kind of fighting force:es the Northern Parliament plan to send out on this operation?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 107030,
			say = "As far as I know, the bulk of your forces has been concentrated in the North fighting off the Sirens, which is why you've rarely participated in the Reenactments.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_guild_blue_n",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Besides, I dunno if anyone else feels like this is the elephant in the room, but how:es the Northern Parliament plan to go against the Iron Blood's technology? To my understanding, most of your fleet is rather... old-fashioned.",:
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_guild_blue_n",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "(Surely Soyuz also knows about the Iron Blood's blueprint ships... If they were to deploy those ships against them, what chance would the Northern Parliament stand...?)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_guild_blue_n",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "(Wait... that excessive confidence... Does the Northern Parliament also have blueprint ships of their own?)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900218,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Thanks to the Commander's aid, we were able to capture the Sirens' iceberg stronghold, allowing us to free up a large amount of our forces.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 900218,
			say = "There is a lot less pressure on our defensive lines compared to before. We will hold as long as we begin the operation promptly and finish up as quickly as possible.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900218,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "While blueprint ships are certainly a force to be reckoned with, they are limited by their cost and difficulty of development. We will be able to gauge the impact they'll be able to have on the battlefield.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 900218,
			say = "With the intelligence we have, I can assure you that this battle will not turn into a Reenactment.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 900218,
			say = "I cannot disclose anything else about the nature of our intelligence, but I hope this is enough to address Saratoga's concerns.",:
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 900218,
			say = "The Eagle Union should have firsthand knowledge of our Polar Fleet's capabilities. We will be able to go blow-for-blow with the Iron Blood.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 900218,
			say = "Finally, the Commander's participation should mitigate any unpredictable scenarios.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_guild_blue_n",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "What if the Commander declines, or is asked to help the Iron Blood?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900218,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Should the Commander be absent... we already have contingencies in place.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_guild_blue_n",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "I'm not quite sure what to think still... I find it hard to believe that the Naval HQ would just nod and go along with a plan this ambitious.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_guild_blue_n",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "(Yat Sen might know more than she's letting on, but green-lighting an operation like this:esn't seem right, even if the Commander knows about it...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 107030,
			say = "I hope you're not getting the Commander into any weirdness...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 900218,
			say = "You are right to have some reservations. We won't deny the possibility that the Commander might be exposed to danger during this operation.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900218,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "But, the same goes for any time the Commander joins an operation with the Eagle Union.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_guild_blue_n",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "(Ugh... I:n't have a comeback for that...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 900218,
			say = "Having that said, we will respect the Commander's decision, whatever it might be.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_guild_blue_n",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Well, I guess it wouldn't make sense for us to make decisions for the Commander in advance...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 900218,
			say = "I'm glad we could reach an understanding, Saratoga. Will there be any further questions?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_guild_blue_n",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Oh, right! The Eagle Union isn't prohibited from joining this operation, right? Would it be acceptable if we sent a support fleet?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900218,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "We:n't have any objections to that, but might I ask why?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_guild_blue_n",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Hmm... Just as an insurance option, in case the Commander's life is threatened in any way.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_guild_blue_n",
			paintingNoise = True,
			dir = 1,
			actor = 205050,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Apologies for the interruption, but I believe the Northern Parliament would be able to accomplish their goals more easily with the assistance of another fleet.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205050,
			say = "Saratoga's considerations about the Commander's safety are shared by every person here, and the presence of an Eagle Union fleet would go a long way in ensuring that no Reenactment occurs.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_guild_blue_n",
			paintingNoise = True,
			dir = 1,
			actor = 205050,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Preventing a Reenactment would kill two birds with one stone, eliminating the biggest source of risk to the Commander's safety.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 107030,
			say = "Does that mean that the Royal Navy supports having the Commander participate in this operation?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 205050,
			say = "I wouldn't say \"support,\" since nothing we say here matters if the Commander declines or Soyuz declines your proposal.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_guild_blue_n",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "That's True... Now that I think about it, why isn't Queen Elizabeth here?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_guild_blue_n",
			paintingNoise = True,
			dir = 1,
			actor = 205050,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Her Majesty has her own way of:ing things. Do you have some objection to me being here in her stead?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_guild_blue_n",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Ah... Not at all! I wasn't trying to imply anything like that!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 107030,
			say = "(Woah... Did I make things awkward all of a sudden?)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_guild_blue_n",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Well, in any case, let's bring your proposal to the Commander and: assess the situation from there.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 107030,
			say = "We've all heard what Soyuz has to say except for the Commander, and it should be possible to get the others in the Eagle Union on board.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_guild_blue_n",
			paintingNoise = True,
			dir = 1,
			actor = 107030,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "By the way, if everything pans out, can we have permission to use the Northern Parliament's ports and have safe passage through your waters?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 900218,
			say = "Of course. Anything for the success of the operation.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 900218,
			say = "But, everything hinges on the Commander's response. Let us continue our discussions:.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 900218,
			say = "Hopefully I kept a good-enough poker face...",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 900218,
			say = "With the Mikoyan's discovery, we must recover Kronshtadt and the other members of the Polar Expedition as quickly as possible...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 900218,
			say = "If we can deceive the prying eyes of the Naval HQ and uncover the Sirens' secrets before they can...: the entire course of this war will change.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_guild_blue_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 900218,
			say = "The Commander will ultimately have to answer to someone after this operation... and that is an inevitability.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			mode = 1,
			stopbgm = True,
			sequence = {
				{
					"Shortly before the appearance of the Sirens and the outbreak of the first great war...",
					1
				},
				{
					"A meteorite fell into territory controlled by the Northern Parliament.",
					2
				},
				{
					"An expedition team was dispatched to investigate the phenomenon",
					3
				},
				{
					"but mysteriously vanished without a trace - equipment, fleet, and all.",
					4
				},
				{
					"Unknownst to that expedition team, what they had uncovered",
					5
				},
				{
					"was one piece of the truth behind the Sirens.",
					7
				}
			}
		},
		{
			mode = 1,
			stopbgm = True,
			blackBg = True,
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			effects = {
				{
					active = True,
					name = "SHENDUHUIYIN"
				}
			},
			sequence = {
				{
					"",
					1
				}
			}
		}
	}
}
