return {
	fadeOut = 1.5,
	mode = 2,
	id = "SHENDUHUIYIN13",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			actorName = "???",
			side = 2,
			bgName = "bg_story_nepu2",
			dir = 1,
			bgmDelay = 1,
			bgm = "theme-dailyfuture",
			nameColor = "#ffff4d",
			hidePaintObj = True,
			say = "Hey, you there! That area's off-limits!",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_story_nepu2",
			say = "The Northern Parliament girls were exploring the foreign island when suddenly they were stopped by a stranger.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 701100,
			say = "Back off!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 718010,
			say = "Don't shoot, Kiev! She's a civilian!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 701100,
			say = "...Sorry. Instinct took over.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 707010,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Gosh, I'm terribly sorry! I hope we didn't scare you.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#ffff4d",
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			actorName = "???",
			say = "Nah, it's all good. My bad for calling out to you out of nowhere.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_story_nepu2",
			say = "The stranger emerged from a roadside thicket, revealing herself. She was dressed much differently to them.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#ffff4d",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "???",
			say = "Plus, I'm not a civilian. I:n't scare that easily",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Arkhangelsk",
			say = "Interesting uniform... Do you mind telling us your name and where we are?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#ffff4d",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "My name's Bon Homme Richard, and this place is Samos Island.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Arkhangelsk",
			say = "Curious pronunciation. Are you from Iris Libre or the Vichya Dominion?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#ffff4d",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "I get that a lot, but nope, I was born in the Eagle Union and work for the Federation.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Arkhangelsk",
			say = "Oh, I see. And you said Samos just now? So we're in the Aegean Sea?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#ffff4d",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "Not exactly. We're in the seas of Bermuda.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#ffff4d",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "Sounds to me like you've lost your bearings. Did you sneak past the filter to play in the sun like me?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Arkhangelsk",
			say = "That's one way of putting it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#ffff4d",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "Yeah, I getcha. Sneaking around's pretty fun.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Arkhangelsk",
			say = "You're... not going to ask us to introduce ourselves?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#ffff4d",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "I mean, your uniforms make it pretty obvious you're from the Northern Parliament.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#ffff4d",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "Based on how disoriented you sound, I'll assume you suddenly found yourselves here with no clue how it happened.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#ffff4d",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "In other words, same as what happened to me!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#ffff4d",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "And I've got no reason to report you unless you: something totally unjustified. So just find a beach to lie:wn on and take it easy!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Arkhangelsk",
			say = "\"No clue how it happened?\" Excuse me?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Kronshtadt",
			say = "(Psst. Let me take it from here.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Kronshtadt",
			say = "Let's rewind a bit. You said, \"That area's off-limits\" earlier. Why?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Kronshtadt",
			say = "Is there more to this place than a tropical resort?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#ffff4d",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "THIS place is a tropical resort, yeah, but keep going that-a-way and you'll arrive at the Samos Marine Research Institute.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#ffff4d",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "You:n't wanna go there. They had an accident a while ago which released ungodly beasties into the water. So I've heard, anyway.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#ffff4d",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "I:n't know what really went:wn, but my advice is to steer clear of that place.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Kronshtadt",
			say = "Why hasn't anyone put up warning signs? One oblivious tourist is all it takes to cause a PR nightmare.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#ffff4d",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "That's... a good question! Paradoxically though, tourism to this place has skyrocketed 'cause of that accident.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Kronshtadt",
			say = "All people hoping to snap a picture of an urban legend, I presume?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#ffff4d",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "Not all of them, but a large majority, yeah.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#ffff4d",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "Anyway, there's no issue as long as you:n't go there. Stick to this area and you'll be fine.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Kronshtadt",
			say = "Thanks for the warning. One more question...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Kronshtadt",
			say = "You also said we \"snuck past the filter,\" just like you, yes?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#ffff4d",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "Yup, it's just that I got here earlier than you. Speaking of which, I've actually gotta go soon.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#ffff4d",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "Professor's got me on a strict schedule, giving me the ol' \"it's only temporary\" consolation, but it's hard getting used to.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#ffff4d",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "Eh, forget it. Don't wanna ruin the wonderful weather with my grumbling!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#ffff4d",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "Before I go though, wanna take a group picture with me?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#ffff4d",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "Only natural to want to capture the moment in time you went to a beautiful place and made new friends, right?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Arkhangelsk",
			say = "(A group picture...? What's the deal with this girl? How: we proceed?)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Kronshtadt",
			say = "(She'll distrust us if we refuse. Just go with it.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Soobrazitelny",
			say = "(Marvelous! I've been waiting for a chance to show off my genius mechanic look! ...Come on, smile for the camera, Kiev!)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Kiev",
			say = "(Like this? Hope this is fine...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#ffff4d",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "Don't blink! 3, 2, 1!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#ffff4d",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "Say \"cheese!\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Volga",
			say = "Cheese!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "Bon Homme Richard",
			side = 2,
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Niiice! And by the way, I won't be sharing this pic with anyone. It's just a little keepsake.",
			soundeffect = "event./ui/kuaimen",
			flashN = {
				color = {
					1,
					1,
					1,
					1
				},
				alpha = {
					{
						0,
						1,
						0.5,
						0
					},
					{
						1,
						0,
						0.5,
						0.2
					}
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#ffff4d",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "Man, I'm running short on time... What are you girls gonna:? Stay and have another look around?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actorName = "Kronshtadt",
			say = "Most likely. There's still more to see.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#ffff4d",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "All right, have a good one~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#ffff4d",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "Just stay away from the exercise area, or you're going to have a bad time.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#ffff4d",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "If anyone asks, just say you got lost or suffered rigging malfunctions.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Kronshtadt",
			say = "Huh? What did you just say?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#ffff4d",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "Nothing! Just talking to myself! You guys would be nervous too if someone started asking you personal questions.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#ffff4d",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "Now it's time for me to go. Peace out~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_story_nepu2",
			say = "The girls bade farewell to Bon Homme Richard, who went back the way she came and disappeared into the wilderness.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 705060,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I clearly heard that girl say something about a rigging... Kronshtadt,: you think she's...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 718010,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "A shipgirl, yes. From the Eagle Union, at that.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 718010,
			say = "...I shouldn't have let her take that picture. It could be used against us somehow...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 718010,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "How sloppy of me, damnit...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 707010,
			say = "W-well... Hindsight is 20/20, so:n't be too hard on yourself. All we can: is hope she really won't report us.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 701090,
			say = "We haven't:ne anything totally unjustified in this place, so we'll have to take her word for it now that she's left!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 701100,
			say = "Wait... She said she's FROM the Eagle Union but works for the Federation... What was that about?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 705060,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Who knows. More importantly, what should we investigate next? The marine institute, or that exercise area she mentioned?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 718010,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Good question. Both are potential leads, and we'll likely want to visit each anyway.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 718010,
			say = "What: you think, Soobrazitelny?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 701090,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "My vote goes to the exercise area! That whole thing about monsters in the water sounds like a fabricated story to bring in tourists!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 701090,
			say = "If it wasn't made up, they would at the very least put fences to keep people out! There's not even a warning sign to be seen!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 701090,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "So, I say we go to the exercise area where we can fully leverage our abilities!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_nepu2",
			dir = 1,
			blackBg = True,
			actor = 718010,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "You have a point... Riggings were built for use on the open sea, not inside a building on land. Exercise area it is.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
