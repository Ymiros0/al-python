return {
	fadeOut = 1.5,
	mode = 2,
	id = "SHENDUHUIYIN18",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			say = "Over on the island, the girls followed the tourist road until they came across a place with a thicket.",
			side = 2,
			bgName = "bg_story_nepu2",
			dir = 1,
			bgmDelay = 1,
			bgm = "theme-dailyfuture",
			flashin = {
				delay = 0.5,
				dur = 0.5,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 718010,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "If memory serves me right, it's just up ahead...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 718010,
			say = "Now she should pop out after we turn this right corner...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 705060,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Who? How: you know someone else is here?",:
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 718010,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "(Crap! Spoke my thoughts out loud!)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 705060,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "You're scaring Volga – she says something's felt wrong about you since shortly after we got here.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 718010,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Really now? What makes her think that?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 705060,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I dunno. Kiev and Soobrazitelny haven't said anything, and I didn't notice it either until Volga pointed it out.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 705060,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "But, Volga's intuition tends to be really good when it comes to people's emotions. That's why I wanted to check on you.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 705060,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "The fact that you're so calm and decisive in this strange situation... I'm not sure whether it's totally in character or totally out of character for you.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 718010,
			say = "...I'm not sure what to make of that.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 705060,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "See, our intelligence personnel are trained to handle unknown situations, but that's not you, right?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 718010,
			say = "That:esn't sound like a compliment to me.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 705060,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "In other words, I'm the one who should be saying, I'm not sure what to make of your behavior.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 705060,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "You've been acting weird from the moment you sat:wn in that chair.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 705060,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Don't you think you can open up to us? Your friends are getting worried.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 718010,
			say = "I... I'm sorry, but I still haven't sorted things through in my own head yet.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 718010,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Just give me a bit more time...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 705060,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "All right. I'm not going to push you for answers, but we're in this together.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			actorName = "???",
			say = "Hey, you there! That area's off-limits!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_story_nepu2",
			say = "The conversation between Kronshtadt and Arkhangelsk was suddenly interrupted by a young girl's voice.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 718010,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "(Right on schedule.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 701100,
			say = "Back off!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 718010,
			say = "Stand:wn, Kiev. I'll handle this.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 701100,
			say = "...You sure that's a good idea?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 718010,
			say = "Just: as I say. Now... Bon Homme Richard, I presume? I hope we didn't frighten you.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			actorName = "？？？",
			say = "Nah, it's all good. My bad for calling out to you out of nowhere.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			actorName = "？？？",
			say = "You know my name, huh? This is the last place I expected to run into one of my fans!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_deepecho_5",
			say = "The stranger emerged from a roadside thicket, revealing herself. She was dressed much differently to them.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Kronshtadt",
			say = "What are you be:ing all the way out here? Is there something going on in that exercise area up ahead? Oh, and: you mind if I just call you Richard?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "Woah, that's a lot of questions out of the blue... Say, are you girls spies from the Northern Parliament?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Soobrazitelny",
			say = "No, we're not! At least I'm no spy - there's no room for other stuff in the brain of a genius mechanic!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Kiev",
			say = ".........",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "Haha, I was just cracking a joke. Nobody would hire a bunch of oddballs like you to be spies!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "Sorry about that. It's just, those questions you asked me earlier came off a bit suspicious, you know?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "Normally when you meet someone, you:n't just go right into \"what are you:ing here,\" right?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "But since you're interested, why:n't we play a little game and have you guess what I'm:ing here~♪",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Kronshtadt",
			say = "Hmm... How about, you snuck past the filter to have some fun in the sun? How:es that sound?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "Woow, you really got it right on the first try!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "Bingo, bingo! I snuck away and came to visit this island!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "But, I can't stay out too long or people will get antsy.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "As for this exercise area... It's funny to me that you even know what it is.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "You know what's really odd? That the Federation and the Revolutionary Front – both at odds with each other – are conducting huge exercises here. Same place, same time.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "Oh, but I'm sure there are a bunch of other strong ships here! Are you going to participate as well?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Kronshtadt",
			say = "I'm afraid not. We're just here as tourists.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Kronshtadt",
			say = "(This is new... She mentioned the Federation before, but what is this \"Revolutionary Front\" organization?)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "Before I go though, wanna take a group picture with me?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "Only natural to want to capture the moment in time you went to a beautiful place and made new friends, right?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Arkhangelsk",
			say = "(A group picture...? What's the deal with this girl? How: we proceed?)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Kronshtadt",
			say = "I:n't mind. Sounds like a good idea to me.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Soobrazitelny",
			say = "(Marvelous! I've been waiting for a chance to show off my genius mechanic look! ...Come on, smile for the camera, Kiev!)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Kiev",
			say = "(Like this? Hope this is fine...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "Don't blink! 3, 2, 1!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "Say \"cheese\"!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Volga",
			say = "Cheese!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "Bon Homme Richard",
			side = 2,
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#BDBDBD",
			say = "Man, I'm running short on time... What are you girls gonna:? Stay and have another look around?",
			soundeffect = "event./ui/kuaimen",
			flashN = {
				color = {
					1,
					1,
					1,
					1
				},
				alpha = {
					{
						0,
						1,
						0.5,
						0
					},
					{
						1,
						0,
						0.5,
						0.2
					}
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Kronshtadt",
			say = "Most likely. There's still more to see.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "Fair enough. Don't you think it's boring just chilling on this island though?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Kronshtadt",
			say = "...Hm?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "You're still curious about that exercise area, aren't you? Ehehe. If you want to see for yourself what True \"power\" looks like––",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "Why:n't you head:wn there yourselves? Hehe~♪",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Kronshtadt",
			say = "(What the hell is going on?! She's acting like a completely different person than before...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Kronshtadt",
			say = "Are you sure you're fine with that? Your \"Federation\" isn't going to find people like us suspicious?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "Why? There's nothing wrong with girls who want \"power.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_5",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bon Homme Richard",
			say = "Now it's time for me to go. Peace out~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_story_nepu2",
			say = "The girls bade farewell to Bon Homme Richard, who went back the way she came and disappeared into the wilderness.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 705060,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "What a weird girl. Someone you know, Kronshtadt?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 705060,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "There are too many things to keep track of that I've never heard of before. What is this \"Federation,\" and what's the deal with this exercise area?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 718010,
			say = "(But what I'm more curious about... is why none of you seem to know anything.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 718010,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Sorry, but I've been totally lost since the moment I got here.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 705060,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Sigh... You really are a True professional, Kronshtadt.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 705060,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "So, what: we check out next? Want to head:wn to the exercise area like that girl said?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 718010,
			say = "The way she was talking about power in quotes was giving me the chills.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 701090,
			say = "But did you see the smartphone she had on her? It looked insanely advanced!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 701090,
			say = "Mechanical marvels are the only power that this genius mechanic needs! Let's stay away from that exercise area.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 701090,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Or so I'd like to say, but if they really: have some ridiculous power, we should at least find out what it is.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 701100,
			say = "I also want to see it for myself...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 707010,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "What are those two \"at odds with each other\" organizations she mentioned? I'm not sure they'd appreciate outsiders intruding into their exercise...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 718010,
			say = "We'll have to move forward really carefully and avoid stepping on any toes:. Arkhangelsk, Volga, can we count on you two for the reconnaissance again?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 705060,
			side = 2,
			bgName = "bg_story_nepu2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Sure thing. Ready whenever you are.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_nepu2",
			dir = 1,
			blackBg = True,
			actor = 718010,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Let's make good use of our time. Get going, everyone!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
