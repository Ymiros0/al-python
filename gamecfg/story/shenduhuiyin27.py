return {
	fadeOut = 1.5,
	mode = 2,
	id = "SHENDUHUIYIN27",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			say = "An alien scenery spread out before Kronshtadt's eyes.",
			side = 2,
			bgName = "bg_deepecho_9",
			dir = 1,
			bgmDelay = 1,
			bgm = "main-ashes-theme",
			flashin = {
				delay = 1,
				dur = 1,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 718010,
			say = "...So this is the climax of the show.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_deepecho_9",
			say = "Flames danced atop water. Twisted metallic headstones adorned the surface. Unworldly weather raged above the exercise area from before.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_deepecho_9",
			say = "An abandoned sector of the sea, ravaged by an unfathomable storm that consumed everything in its path.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_deepecho_9",
			say = "The grand fleet that had once:tted the horizon was now nothing more than a graveyard, a solemn witness to the calamity that had passed over it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_deepecho_9",
			say = "Flames, smoke, and blackened waters. Even the very air choked the senses in this apocalyptic hellscape.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_deepecho_9",
			say = "Only one thing continued to dance in the gala of death and destruction.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 718010,
			say = "It seems... alive.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 718010,
			side = 2,
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "No, it always was...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_deepecho_9",
			say = "Flames and storms raging all around her, Kronshtadt took aim with her main guns as she approached the black tornado.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_deepecho_9",
			say = "Kronshtadt knew she had to confront it. No matter how many tries it took or how much time passed.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 718010,
			say = "If I defeat you,: the operation... My mission... can finally end...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_deepecho_9",
			say = "She turned her eyes towards the figure at the heart of the maelstrom – Bon Homme Richard.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_deepecho_9",
			say = "Kronshtadt could not see the girl. But she could feel her presence.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_deepecho_9",
			say = "As if answering her, a laughter began to ring out in her head, both innocent and maniacal.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			actorName = "???",
			say = "Defeat me? Why would you want to: that?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 718010,
			say = "I must complete my mission.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			actorName = "???",
			say = "Why? Is that mission really that important to you?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 718010,
			say = "I will defeat you, put an end to this dream, and return to the Northern Parliament with my comrades.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			actorName = "???",
			say = "Comrades? No, no, Kronshtadt. Don't you remember how you hunted them:wn yourself?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			actorName = "???",
			say = "There is no camaraderie in this world. Only temporary alliances out of convenience.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			actorName = "???",
			say = "Your Northern Parliament is no different. They only support you when they need something from you.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			actorName = "???",
			say = "Friendship. Factions. Loyalty. You:n't need those things. You:n't need any of those things.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 718010,
			side = 2,
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "Shut your crazed, lying mouth!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 718010,
			say = "I will defeat you, solve the mystery of this meteorite, and bring my comrades back...!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			actorName = "???",
			say = "Hahah! Is that so? I see, I see.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			actorName = "???",
			say = "Even if you manage to defeat me, all that proves is that you have great power.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			actorName = "???",
			say = "But power is fearsome. That applies to countries, to factions, to organizations, and to people.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			actorName = "???",
			say = "People seek power for themselves, but when they get it, all that power:es is breed fear.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			actorName = "???",
			say = "Isn't that ironic? Isn't that hilarious?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			actorName = "???",
			say = "Face it, Kronshtadt. Even if you defeat me, no glory awaits you. Your \"comrades\" won't ever come back.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			actorName = "???",
			say = "You are all alone. Forlorn. Shunned. Denounced. Destroyed.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 718010,
			say = "No, you're wrong! You know nothing about my comrades or the Northern Parliament!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			actorName = "???",
			say = "You are the one who knows nothing.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			actorName = "???",
			say = "Everything is a lie, a convenient excuse to use you... to use the KAN-SEN.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			actorName = "???",
			say = "Yes... Kindness, support, respect. It's all a lie.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			actorName = "???",
			say = "The glories afforded to you are cheap tokens to buy your loyalty, good for nothing but manipulating you.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			actorName = "???",
			say = "The moment they realize they are unable to control you...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			actorName = "???",
			say = "Is the moment they will come for you to destroy you. After all, what is an unusable weapon but a heap of scrap metal?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			actorName = "???",
			say = "If you still won't admit it,: show me that power.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "???",
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			blackBg = True,
			nameColor = "#BDBDBD",
			say = "Defeat me! Destroy me! ERASE MY EXISTENCE!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
