return {
	fadeOut = 1.5,
	mode = 2,
	id = "SHENDUHUIYIN28",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			soundeffect = "event./battle/boom2",
			side = 2,
			bgName = "bg_deepecho_9",
			say = "Kronshtadt pumped shell after shell into the black tornado, resuming the symphony of destruction with a new movement.",
			dir = 1,
			bgmDelay = 1,
			bgm = "main-ashes-theme",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			flashN = {
				color = {
					1,
					1,
					1,
					1
				},
				alpha = {
					{
						0,
						1,
						0.2,
						0
					},
					{
						1,
						0,
						0.2,
						0.2
					},
					{
						0,
						1,
						0.2,
						0.4
					},
					{
						1,
						0,
						0.2,
						0.6
					}
				}
			},
			dialogShake = {
				speed = 0.09,
				x = 8.5,
				number = 2
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 718010,
			say = "\"Die! Die! DIE!\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_deepecho_9",
			say = "She had lost all control of her emotions, becoming a puppet to her pure lust for destruction, her existence reduced to unload her entire arsenal at her target.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_deepecho_9",
			say = "The tornado, meanwhile, simply took it all. It did not retaliate or even defend itself.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 718010,
			say = "(How laughable... Am I really trying to kill a storm?)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 718010,
			say = "(It's a force of nature, not a living being. I'm making a fool of myself...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 718010,
			side = 2,
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "(But... Pointless as it may be...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_deepecho_9",
			say = "Her assault did not let up.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			actorName = "???",
			say = "Is that the best you can:, lady? You've not left as much as a dent on me!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_deepecho_9",
			dir = 1,
			soundeffect = "event./battle/boom2",
			actor = 718010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "\"Shut up! You've not seen the start of it!\"",
			flashN = {
				color = {
					1,
					1,
					1,
					1
				},
				alpha = {
					{
						0,
						1,
						0.2,
						0
					},
					{
						1,
						0,
						0.2,
						0.2
					},
					{
						0,
						1,
						0.2,
						0.4
					},
					{
						1,
						0,
						0.2,
						0.6
					}
				}
			},
			dialogShake = {
				speed = 0.09,
				x = 8.5,
				number = 2
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_deepecho_9",
			say = "Her identity was being overwritten. Her consciousness began to wane.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_deepecho_9",
			say = "Soon destruction would be the only thing she could think of. Soon she would be no more than a living weapon.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_deepecho_9",
			say = "Her comrades, her allegiance, her duty, her mission – everything she held dear was being reduced to forgotten memories.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_deepecho_9",
			say = "Never before had something felt so reprehensibly wrong. And yet she couldn't stop firing. Her power grew and grew without limit.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_deepecho_9",
			dir = 1,
			soundeffect = "event./battle/boom2",
			actor = 718010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "\"AHAHAHAHAHAHAH!\"",
			flashN = {
				color = {
					1,
					1,
					1,
					1
				},
				alpha = {
					{
						0,
						1,
						0.2,
						0
					},
					{
						1,
						0,
						0.2,
						0.2
					},
					{
						0,
						1,
						0.2,
						0.4
					},
					{
						1,
						0,
						0.2,
						0.6
					}
				}
			},
			dialogShake = {
				speed = 0.09,
				x = 8.5,
				number = 2
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 718010,
			say = "\"Prepare to meet your end! Drop dead already! DIEEEEE!\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_deepecho_9",
			say = "Richard was right there, standing right in front of her.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_deepecho_9",
			say = "With a smile like a fallen angel, she pressed her little head against the barrel of Kronshtadt's cannon.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			actorName = "???",
			say = "You've almost got me, lady.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#BDBDBD",
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			actorName = "???",
			say = "A little more should: it~♪",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_deepecho_9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 718010,
			say = "No... No! This isn't like me! I would never...!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_deepecho_9",
			say = "Despite her rejection, she could not stop firing. The world turned pitch black, and she realized: that she was on the brink of METAmorphosis.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_deepecho_9",
			say = "But:... a ray of light shone through to Kronshtadt.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			mode = 1,
			blackBg = True,
			bgName = "star_level_bg_1104",
			flashin = {
				delay = 1,
				dur = 1,
				alpha = {
					1,
					0
				}
			}
		}
	}
}
