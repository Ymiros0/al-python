return {
	fadeOut = 1.5,
	mode = 2,
	id = "SHENDUHUIYIN29",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			side = 2,
			bgName = "star_level_bg_1104",
			dir = 1,
			bgmDelay = 1,
			bgm = "story-2",
			actor = 718010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "(..................)",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_1104",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 718010,
			say = "(Huh...?)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 718010,
			side = 2,
			bgName = "star_level_bg_1104",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "(What... happened to me...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_1104",
			say = "The black tornado was no longer there. Nor were Richard, the wrecks, or the exercise area.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_1104",
			say = "Kronshtadt instead found herself in a strange, concrete hall.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_1104",
			say = "Columns of lamps illuminated her surroundings with a faint light.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_1104",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 718010,
			say = "(Maybe I've finally arrived at the gates of heaven...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_1104",
			say = "Kronshtadt wandered out of the hall without any sense of purpose, trudging into a corridor. Suddenly, a plain wooden:or caught her attention.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 718010,
			side = 2,
			bgName = "star_level_bg_1104",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "(Huh, I wonder what's in here...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_story_task",
			dir = 1,
			say = "The moment her fingertips brushed against the:orknob, the white concrete hall transformed into a dimly-lit room.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_story_task",
			dir = 1,
			say = "Books and scraps of paper littered the floor, and opposite her in this horribly cluttered room sat a woman at her desk, writing something.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_story_task",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900308,
			actorName = "Female Researcher",
			hidePaintObj = True,
			say = "I:n't understand... How did this happen?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_story_task",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900308,
			actorName = "Female Researcher",
			hidePaintObj = True,
			say = "Her Wisdom Cube... Why did that happen...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_story_task",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900308,
			actorName = "Female Researcher",
			hidePaintObj = True,
			say = "I've never seen anything like it before...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_story_task",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900308,
			actorName = "Female Researcher",
			hidePaintObj = True,
			say = "I:n't understand... I just:n't understand...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_story_task",
			dir = 1,
			say = "BANG! She swung her fist without thinking, knocking over a stack of books.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_story_task",
			oldPhoto = True,
			dir = 1,
			actor = 718010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "(Watch out!)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_story_task",
			dir = 1,
			say = "Kronshtadt instinctively rushed over to shield the woman from the falling books, but they slipped through her body and crashed all over the desk and the floor.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 718010,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			oldPhoto = True,
			say = "(What...?!)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_story_task",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900308,
			actorName = "Female Researcher",
			hidePaintObj = True,
			say = "Damnit... That hurt...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_story_task",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900308,
			actorName = "Female Researcher",
			hidePaintObj = True,
			say = "If only I could see the test data... But I:n't have any clearance anymore...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_story_task",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900308,
			actorName = "Female Researcher",
			hidePaintObj = True,
			say = "I'll have to count on Dr. Aoste:... and focus on:ing what I can: right here!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_story_task",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900308,
			actorName = "Female Researcher",
			hidePaintObj = True,
			say = "Don't give up! If anyone can crack this problem, it's you, Anzeel!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_story_task",
			say = "Kronshtadt tried to reach out to the woman, but the moment she did so, the scenery around her warped and whisked her back to the previous hallway.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 718010,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "(Was that a researcher? What a slovenly lifestyle... Not that I have any right to judge others though...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 718010,
			say = "(I:n't know who she is or what she's:ing, but I hope she succeeds.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_story_task",
			say = "Continuing:wn the corridor, she came across another:or.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 718010,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "(What's behind this one? Another vision?)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_story_task",
			dir = 1,
			say = "This room was lit even more dimly than the last. Kronshtadt could just make out the silhouettes of an adult man and a young girl.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900323,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			oldPhoto = True,
			say = "Magister, your calculation has been completed.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_story_task",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900307,
			actorName = "Male Researcher",
			hidePaintObj = True,
			say = "It's finally:ne?! What are the results?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900323,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			oldPhoto = True,
			say = "The results indicate that the proposed safety lock mechanism is able to satisfy project specifications.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_story_task",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900307,
			actorName = "Male Researcher",
			hidePaintObj = True,
			say = "I see... This is great news. We've finally:ne it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900323,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			oldPhoto = True,
			say = "I will consider the task complete,:. However, I could perform further calculations if you would like a more stable alternative.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_story_task",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900307,
			actorName = "Male Researcher",
			hidePaintObj = True,
			say = "No need. What we've got now strikes the perfect balance between stability and energy output ratio.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_story_task",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900307,
			actorName = "Male Researcher",
			hidePaintObj = True,
			say = "Stability comes at the cost of output – limiting it too much would be to put the cart before the horse.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900323,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			oldPhoto = True,
			say = "Do you not have faith in Antiochus as a contingency plan?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_story_task",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900307,
			actorName = "Male Researcher",
			hidePaintObj = True,
			say = "That's not what I'm saying. I'm just biased towards those girls, perhaps because Anzeel sees so much in them.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_story_task",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900307,
			actorName = "Male Researcher",
			hidePaintObj = True,
			say = "Her incessant gushing has piqued my curiosity. Now I, too, want to observe this \"Awakening\" process that unlocks their latent potential.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900323,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			oldPhoto = True,
			say = "What of the Bon Homme Richard incident? Do you believe it to be an example of an Awakening?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_story_task",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900307,
			actorName = "Male Researcher",
			hidePaintObj = True,
			say = "Don't be ridiculous. That was the opposite of an Awakening – it was more akin to the girls reverting to their original selves.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_story_task",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900307,
			actorName = "Male Researcher",
			hidePaintObj = True,
			say = "That is to say, their Wisdom Cubes underwent the process of degenerating into Origin Cubes. Knowing Anzeel, she's brainstorming a snappy term for it as we speak.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_story_task",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900307,
			actorName = "Male Researcher",
			hidePaintObj = True,
			say = "And if she is, she really should be working on something more important.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_story_task",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900307,
			actorName = "Male Researcher",
			hidePaintObj = True,
			say = "You know how it goes. When Anzeel slacks, I have to watch her back.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_story_task",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900307,
			actorName = "Male Researcher",
			hidePaintObj = True,
			say = "Oh, that reminds me. It's about time I had a talk with our assistant.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_1104",
			say = "When the conversation in the dimly-lit room wrapped up, Kronshtadt found herself back in the white corridor once more.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 718010,
			side = 2,
			bgName = "star_level_bg_1104",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "(Another researcher? Huh... Who was that \"assistant\" he mentioned?)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_1104",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 718010,
			say = "(Surely not the little girl? She looked young enough to be his daughter. Far too young to be a lab assistant.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 718010,
			side = 2,
			bgName = "star_level_bg_1104",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "(Beyond that, I didn't get a word of what they were talking about. Sigh... If only Soobrazitelny were here. She could make sense of it.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_1104",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 718010,
			say = "(Wait... If only who were here? Who's Soobrazitelny?)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 718010,
			side = 2,
			bgName = "star_level_bg_1104",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "(I've never heard that name before, and yet... It feels familiar, for some reason.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_1104",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 718010,
			say = "(Soobrazitelny... Soobra...zitelny... Oh!)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_1104",
			say = "Repeating the name buried in the back of her mind caused an unexpected third:or to appear right before her.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_cccpv2_9",
			dir = 1,
			say = "The vision on the other side seemed to be set in some sort of assembly hall, or a theater.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_cccpv2_9",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900308,
			actorName = "Female Researcher",
			hidePaintObj = True,
			say = "Should we leave some sort of message here?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_cccpv2_9",
			oldPhoto = True,
			dir = 1,
			actorName = "■■■",
			say = "This is the final stage of the safety lock. If they got this far, they should already have Awakened... or they're on the brink of:ing the opposite.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_cccpv2_9",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900308,
			actorName = "Female Researcher",
			hidePaintObj = True,
			say = "I mean, why not write something at least? It'll be like a fun little Easter egg.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_cccpv2_9",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900308,
			actorName = "Female Researcher",
			hidePaintObj = True,
			say = "They might read it and gain the strength they need to overcome a great tragedy.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_cccpv2_9",
			oldPhoto = True,
			dir = 1,
			actorName = "■■■",
			say = "Frankly, that notion is almost too silly to entertain. Nevertheless... I'll put my faith in that genius instinct of yours, Anzeel.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_cccpv2_9",
			oldPhoto = True,
			dir = 1,
			actorName = "■■■",
			say = "You should be the first to leave a message though, not me.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_cccpv2_9",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900308,
			actorName = "Female Researcher",
			hidePaintObj = True,
			say = "Oof... Dumping the toughest task on me, huh?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_cccpv2_9",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900308,
			actorName = "Female Researcher",
			hidePaintObj = True,
			say = "\"Heya, little shipgirl! Good job finding this message! Hope you have a great day!\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_cccpv2_9",
			oldPhoto = True,
			dir = 1,
			actorName = "■■■",
			say = "I hate to be a critic, but maybe you could... tone it:wn a little?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_cccpv2_9",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900308,
			actorName = "Female Researcher",
			hidePaintObj = True,
			say = "Yeah, I guess you're right... Okay, how about this.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_cccpv2_9",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900308,
			actorName = "Female Researcher",
			hidePaintObj = True,
			say = "\"Hang in there, child of the Creator.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_cccpv2_9",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900308,
			actorName = "Female Researcher",
			hidePaintObj = True,
			say = "\"Your very existence is a miracle born from the hopes and hearts of countless people.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_cccpv2_9",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900308,
			actorName = "Female Researcher",
			hidePaintObj = True,
			say = "\"Face the darkness, but:n't let it consume you! Mama's cheering for you!\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_cccpv2_9",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900308,
			actorName = "Female Researcher",
			hidePaintObj = True,
			say = "Better?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_cccpv2_9",
			oldPhoto = True,
			dir = 1,
			actorName = "■■■",
			say = "Serious, yet light-hearted at the same time... I'm not sure what kind of message that'll send.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_cccpv2_9",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900308,
			actorName = "Female Researcher",
			hidePaintObj = True,
			say = "Look, I'm a researcher, not a writer! This is the best I can:!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_cccpv2_9",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900308,
			actorName = "Female Researcher",
			hidePaintObj = True,
			say = "Now it's your turn to write something!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_cccpv2_9",
			oldPhoto = True,
			dir = 1,
			actorName = "■■■",
			say = "I'm not sure I could come up with something in the spur of the moment... Can I sleep on it first?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_cccpv2_9",
			nameColor = "#a9f548",
			dir = 1,
			actor = 900308,
			actorName = "Female Researcher",
			hidePaintObj = True,
			say = "Nuh-uh, you're not getting off that easy! You're:ing it right now!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_cccpv2_9",
			oldPhoto = True,
			dir = 1,
			actorName = "■■■",
			say = "Fine... Let's see:––",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			options = {
				{
					content = "\"For those you love, and those who love you,:n't give up.\"",
					flag = 1
				},
				{
					content = "\"Chin up and keep fighting. Everyone back home needs you.\"",
					flag = 2
				},
				{
					content = "\"I too would like to meet you, perhaps in another time or place.\"",
					flag = 3
				},
				{
					content = "\"If you make it back to port, drinks are on me.\"",
					flag = 4
				},
				{
					content = "\"ERROR CODE 96421. PLEASE RESTART THE APPLICATION.\"",
					flag = 5
				},
				{
					content = "Write nothing",
					flag = 6
				}
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_cccpv2_9",
			dir = 1,
			optionFlag = 1,
			actor = 718010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "Don't give up...? What have I given up on?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_cccpv2_9",
			oldPhoto = True,
			dir = 1,
			optionFlag = 2,
			actor = 718010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "Home...? I still have a home to return to?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			oldPhoto = True,
			bgName = "bg_cccpv2_9",
			dir = 1,
			optionFlag = 3,
			actor = 718010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "Meet me? Excuse me? Is this a pick-up line?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_cccpv2_9",
			oldPhoto = True,
			dir = 1,
			optionFlag = 4,
			actor = 718010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "Famous last words to tell a comrade on the battlefield...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_cccpv2_9",
			oldPhoto = True,
			dir = 1,
			optionFlag = 5,
			actor = 718010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "How: I restart it? And what application even is this?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_cccpv2_9",
			oldPhoto = True,
			dir = 1,
			optionFlag = 6,
			actor = 718010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "Sigh... Out of all the things you could say, you chose nothing?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_cccpv2_9",
			oldPhoto = True,
			dir = 1,
			actor = 718010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "These people are... interesting, even if I still have no clue what anyone's talking about.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 718010,
			side = 2,
			bgName = "bg_cccpv2_9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			oldPhoto = True,
			say = "I recognize one of them. She's the female researcher from before, I think.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_cccpv2_9",
			oldPhoto = True,
			dir = 1,
			actor = 718010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "And the other person... Nope,:esn't ring a bell. Must be someone important, though.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_cccpv2_9",
			oldPhoto = True,
			dir = 1,
			actor = 718010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "Mentioned something about a \"genius instinct,\" though... That reminds me of how Soobrazitelny always goes around calling herself a genius.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_cccpv2_9",
			oldPhoto = True,
			dir = 1,
			actor = 718010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "And: there's Kiev, Volga, and of course, Arkhangelsk...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_cccpv2_9",
			oldPhoto = True,
			dir = 1,
			actor = 718010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "My... My comrades...!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 718010,
			side = 2,
			bgName = "bg_cccpv2_9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			oldPhoto = True,
			say = "Yes! I remember now! I remember all of it!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_cccpv2_9",
			oldPhoto = True,
			dir = 1,
			actor = 718010,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "I would never forget about my comrades! And they would never forget about me!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_cccpv2_9",
			oldPhoto = True,
			dir = 1,
			actor = 718010,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "What am I:ing here... I've got my duty to fulfill, and the Northern Parliament to return home to!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_cccpv2_9",
			oldPhoto = True,
			dir = 1,
			actor = 718010,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "I know you can hear me! Get me out of here and put me back where I belong!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_cccpv2_9",
			oldPhoto = True,
			dir = 1,
			actor = 718010,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Take me back home, to where I belong!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_1100",
			say = "FATAL ERROR.",
			flashout = {
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_1100",
			say = "ORIGINATION PROGRESS. 99.7%",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_1100",
			say = "ORIGINATION PROGRESS. 83.5%",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_1100",
			say = "ORIGINATION PROGRESS. 65.9%",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_1100",
			say = "ORIGINATION PROGRESS. 41.6%",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_1100",
			say = "ORIGINATION PROGRESS. 10.2%",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_1100",
			say = "ORIGINATION PROGRESS. 0.0%",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			bgm = "battle-boss-deepecho",
			side = 2,
			bgName = "bg_deepecho_11",
			dir = 1,
			say = "Kronshtadt's fractured senses pieced themselves back together, her consciousness clawing its way out of the void.",
			flashin = {
				delay = 1,
				dur = 1,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_deepecho_11",
			say = "Next came her humanity,: her emotions, flooding back into her being.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_11",
			hidePaintObj = True,
			dir = 1,
			actorName = "Kronshtadt",
			say = "*cough* *cough*! Wh-which loop is this?!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_11",
			hidePaintObj = True,
			dir = 1,
			actorName = "Kronshtadt",
			say = "Enough is enough, please... I think I've endured enough punishment already...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_11",
			hidePaintObj = True,
			dir = 1,
			actorName = "Arkhangelsk",
			say = "Kronshtadt! You came back to us!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_11",
			hidePaintObj = True,
			dir = 1,
			actorName = "Arkhangelsk",
			say = "You wouldn't believe how long you slept. Frankly, I considered waking you with brute force more than a few times.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_11",
			hidePaintObj = True,
			dir = 1,
			actorName = "Kronshtadt",
			say = "That explains why my whole body hurts... Violence is never the answer, Arkhangelsk.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_11",
			hidePaintObj = True,
			dir = 1,
			actorName = "Arkhangelsk",
			say = "I only said I considered it. Maybe I'll give you a friendly whupping after we've returned to reality, though.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_11",
			hidePaintObj = True,
			dir = 1,
			actorName = "Kronshtadt",
			say = "Heheh. Hit me with your best shot.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_11",
			hidePaintObj = True,
			dir = 1,
			actorName = "Arkhangelsk",
			say = "So, what now? Everyone's gathered in one place –: you have a plan?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_11",
			hidePaintObj = True,
			dir = 1,
			actorName = "Kronshtadt",
			say = "Yes. I've gathered all the intel we'll need. All that's left is to finish this just the way I like it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_11",
			hidePaintObj = True,
			dir = 1,
			actorName = "Volga",
			say = "Please pace yourself, okay? We'll gladly take part of the burden off your back.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_11",
			hidePaintObj = True,
			dir = 1,
			actorName = "Soobrazitelny",
			say = "Exactly! You have a genius mechanic at your disposal – use her!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_11",
			hidePaintObj = True,
			dir = 1,
			actorName = "Kiev",
			say = "All we have to: is beat this big thing up, right?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_11",
			hidePaintObj = True,
			dir = 1,
			actorName = "Kronshtadt",
			say = "No, we're not:ing that. You can't beat a force of nature.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_11",
			hidePaintObj = True,
			dir = 1,
			actorName = "Soobrazitelny",
			say = "Then what? If we:n't strike now while it's sitting around, it'll overwhelm us with its waves again!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_deepecho_11",
			say = "The black tornado was still spinning, but its intimidating presence and air of madness were gone. Now it was no more than a destructive storm.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_11",
			hidePaintObj = True,
			dir = 1,
			actorName = "Kronshtadt",
			say = "(It's lost its menacing edge... You've shown me everything you had to show, is that it?)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_11",
			hidePaintObj = True,
			dir = 1,
			actorName = "Kronshtadt",
			say = "(Or is this just the aftermath of it all, and it's our job to take care of it?)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_11",
			hidePaintObj = True,
			dir = 1,
			actorName = "Kronshtadt",
			say = "(If so, it's a humiliating reminder of how stupid I was to think I could fight a tornado with cannon fire.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_11",
			hidePaintObj = True,
			dir = 1,
			actorName = "Kronshtadt",
			say = "It:esn't matter. I will finish this mission no matter what and go home with my comrades and the data we gathered!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_11",
			hidePaintObj = True,
			dir = 1,
			actorName = "Kronshtadt",
			say = "And if anything or anyone tries to stand in our way, we will fight until it is no more!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_deepecho_11",
			hidePaintObj = True,
			dir = 1,
			actorName = "Kronshtadt",
			say = "All ships! Aim for the tornado's center! Hit it with everything you've got!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			say = "\"Prototype submarine Suliko approaching maximum depth.\"",
			side = 2,
			bgName = "bg_deepecho_12",
			dir = 1,
			bgm = "bgm-cccp2",
			flashout = {
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_deepecho_12",
			say = "\"Impact in 3... 2... 1...\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_deepecho_12",
			say = "\"......\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_deepecho_12",
			say = "The Suliko hit the seafloor, sending out shockwaves that rippled through the meteorite and the surrounding environment.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_deepecho_12",
			say = "Seemingly in response, the Sirens in the area adopted more aggressive behavioral protocols.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_deepecho_12",
			say = "The raised threat level posed a significant challenge for search parties in the area. This, coupled with the confidential nature of the mission, put a temporary stop to major rescue efforts.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_deepecho_12",
			say = "But, at no point did any shipgirl from the Northern Parliament forget that their comrades were out here, somewhere.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_deepecho_12",
			say = "Their search continued for any lead that might lead them to the girls' whereabouts. They waited, restlessly, for a chance to rescue their missing comrades.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_deepecho_12",
			dir = 1,
			blackBg = True,
			say = "Beneath the endless ice, an echo continued to emanate from the depths, an abyssal refrain waiting to be heard.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
