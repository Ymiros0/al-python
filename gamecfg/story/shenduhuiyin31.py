return {
	fadeOut = 1.5,
	mode = 2,
	id = "SHENDUHUIYIN31",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			say = "Back at the Northern Parliament",
			side = 2,
			bgName = "bg_deepecho_13",
			dir = 1,
			bgmDelay = 1,
			bgm = "bgm-cccp",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_deepecho_13",
			say = "Sovetsky Soyuz reunited with Kronshtadt at a grand parade celebrating the return of the long-lost heroes.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_deepecho_13",
			say = "The fact that they were able to return home safely brought relief to the hearts of many, but just as importantly– the information they gathered while connecting with the meteorite would surely pave a new path for the Northern Parliament.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Kronshtadt",
			bgName = "bg_deepecho_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "Some things never change, huh? It's good to be back home.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_deepecho_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "Indeed. This is the place where we belong, and it will always await us with open arms.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_deepecho_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "How was your trip back?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Kronshtadt",
			bgName = "bg_deepecho_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "I guess I got the first-class treatment? In any case, I slept through most of it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Kronshtadt",
			bgName = "bg_deepecho_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "Somehow, our resident genius mechanic still had the energy to go back to the research base.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Kronshtadt",
			bgName = "bg_deepecho_13",
			nameColor = "#a9f548",
			dir = 1,
			say = ".........",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Kronshtadt",
			bgName = "bg_deepecho_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "Honestly, I never thought I'd be able to see my home again.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_deepecho_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "Yes, it was a miracle for us as well to find that you were still alive. We've never stopped thinking about your disappearance since that time–",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Kronshtadt",
			bgName = "bg_deepecho_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "So, time didn't seem to be flowing normally inside the chamber?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_deepecho_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "Correct. Apparently you stayed asleep inside the chamber the entire time until you were found. But nothing else had changed.",:
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_deepecho_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "It was as if time stood still.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Kronshtadt",
			bgName = "bg_deepecho_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "...Let's set that aside for now. No matter how deep the submarine:ve, there's no way that time itself would freeze over inside the chamber.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Kronshtadt",
			bgName = "bg_deepecho_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "But, if that had anything to: with the Siren technology we were using, it becomes clear why they are so keenly interested in the meteorite.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_deepecho_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "...So what: you plan on:ing next? Do you want some time off, or: you plan to resume your training?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Kronshtadt",
			bgName = "bg_deepecho_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "I am always prepared to give more for the Northern Parliament, Soyuz.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_deepecho_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "We appreciate your service, comrade.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_deepecho_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "Welcome home, Kronshtadt.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Kronshtadt",
			bgName = "bg_deepecho_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "*sigh*... Hah, it's good to be back, my friend.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Kronshtadt",
			bgName = "bg_deepecho_13",
			nameColor = "#a9f548",
			dir = 1,
			blackBg = True,
			say = "...Sorry to have kept you waiting!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
