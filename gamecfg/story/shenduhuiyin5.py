return {
	fadeOut = 1.5,
	mode = 2,
	id = "SHENDUHUIYIN5",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			say = "Polar Region - Northern Parliament Research Base",
			side = 2,
			bgName = "bg_deepecho_1",
			dir = 1,
			bgmDelay = 1,
			bgm = "bgm-cccp3",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_deepecho_1",
			say = "The girls sailed onward, using the moored icebreaker as a landmark. Not long after, an enormous structure reared its peak over the horizon.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_deepecho_1",
			say = "Its hypermodern design was reminiscent of a majestic chapel, or perhaps a rocket launch pad. It towered silently over the ice field surrounding it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 707010,
			side = 2,
			bgName = "bg_deepecho_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "It seems safe to assume that's the research base.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 701100,
			side = 2,
			bgName = "bg_deepecho_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Doesn't look like something built by humanity... It feels threatening...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_deepecho_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 718010,
			say = "No surprise you get that impression, considering who DID build it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_deepecho_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 701100,
			say = "Huh...? What: you mean by that?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_deepecho_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 718010,
			say = "Simply put, this used to be a Siren facility. It was abandoned and in ruins when we found it, so we decided to rebuild it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_deepecho_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 701090,
			say = "We did what? That sounds way too risky!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_deepecho_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 718010,
			say = "It was worth the risk. This place is vital to our operations out here.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 718010,
			side = 2,
			bgName = "bg_deepecho_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "We:n't have the resources to build a facility from scratch, but we need a base to function as a stopping point in order to dive:wn.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_deepecho_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 718010,
			say = "The Sirens gave us free real estate here; it would be foolish not to take full advantage of it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_deepecho_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 701100,
			say = "Do I have to go inside? I'd rather stay out here...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 701090,
			side = 2,
			bgName = "bg_deepecho_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Why? What are you afraid of?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_deepecho_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 701100,
			say = "I'm not afraid. It just feels like it's dangerous to go near it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_deepecho_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 718010,
			say = "That's what it means to be afraid, Kiev. Tell you what – everyone who's going in, assemble on the deck.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_deepecho_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 718010,
			say = "Kiev and Soobrazitelny, you'll stay here and guard the sub. You never know when Sirens might appear.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_deepecho_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 718010,
			say = "And Volga, you're on babysitting duty. You can all patrol the sector if you want, but:n't get lost. This place is bigger than I expected.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_deepecho_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 701100,
			say = "Okay. We should be fine if we use the icebreaker as a landmark.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_deepecho_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 718010,
			say = "Good. I'd be astonished if you managed to lose track of an ordinary vessel in an area littered with Siren structures.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_deepecho_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 701100,
			say = "Then we have a plan. I'll start scouting the waters around the submarine after we've dropped you off.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_deepecho_1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 701090,
			say = "I can see the base's pier! Initiating:cking sequence!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_deepecho_1",
			dir = 1,
			blackBg = True,
			actor = 718010,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Perfect. Time to get cracking, team!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
