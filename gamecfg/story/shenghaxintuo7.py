return {
	id = "SHENGHAXINTUO7",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"The Secret to Curing Exhaustion?!\n\n<size=45>7 Is the Secret the Commander?</size>",
					1
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_173",
			hidePaintObj = True,
			say = "Our last stop of the day is an aquarium.",
			bgm = "story-richang-8",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_173",
			hidePaintObj = True,
			say = "There, I let San Jacinto take the lead again.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_173",
			hidePaintObj = True,
			say = "The aquarium at night has a different charm compared to daytime. At first, things proceed without incident...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_173",
			hidePaintObj = True,
			say = "Until we reach the:lphin show, that is.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_173",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "I didn't think they'd splash water at us...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_173",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107300,
			say = "SS Commander has been sunk~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_173",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107300,
			say = "Here's a towel. Dry up nice and quick, okay?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107300,
			side = 2,
			bgName = "star_level_bg_173",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...Oh. A little towel isn't enough, is it?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_173",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107300,
			say = "That's too bad. Let's get those clothes off of you, Commander~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_173",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "R-right here? Now?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_173",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107300,
			say = "Yes, of course. You:n't have to worry. I brought you a change of clothes just in case~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_173",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Huh? Where were you keeping that towel and those clothes, exactly...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_173",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107300,
			say = "Must you interrogate everything? A girl's gotta have her secrets♪",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_173",
			hidePaintObj = True,
			say = "In the end, I change into the clothes she brought for me.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_173",
			hidePaintObj = True,
			say = "Though it's a little distracting when she squeals from excitement the whole time...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_173",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(She:es seem like she's more energetic now...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_173",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(I guess the date was a success,:?)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_173",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107300,
			say = "Heehee. I'm just so happy to have had you all to myself today.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107300,
			side = 2,
			bgName = "star_level_bg_173",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "But I think there's one last thing I need to get rid of the rest of my fatigue.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_173",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107300,
			say = "Commander,: you remember when you promised to give me a reward?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_173",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "I sure:. Did you decide on something?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_173",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107300,
			say = "Of course. Close your eyes first.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_173",
			hidePaintObj = True,
			say = "I close my eyes as directed. My other senses become slightly more acute.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_173",
			hidePaintObj = True,
			say = "Warm breath caresses my cheek.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_173",
			hidePaintObj = True,
			say = "A soft kiss touches my lips.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_173",
			hidePaintObj = True,
			say = "A damp, gentle, but mischievous little kiss – a real one that suits her.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_173",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107300,
			say = "You can open your eyes now~ Heehee, all I ever need to cure my exhaustion... is you, Commander.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_173",
			hidePaintObj = True,
			say = "She stands on her tiptoes and gently lays her head on my shoulder, breathing just a little heavily.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_173",
			hidePaintObj = True,
			say = "When I look over, I see red in the corner of her eyes. Tears seem ready to flow.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_173",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107300,
			say = "Aquariums are a staple for dates, yes, but making them the last stop?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_173",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107300,
			say = "I think you need to understand women's hearts a little better, Commander.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_173",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107300,
			say = "Thank you for leading the way today. Now you can leave it to me.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_173",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107300,
			say = "After all, our little promise from before still hasn't been fulfilled.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_173",
			hidePaintObj = True,
			say = "With a sly grin, San Jacinto takes my hand and guides me to the aquarium exit.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
