return {
	fadeOut = 1.5,
	mode = 2,
	id = "SHENGYONGQU34",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			bgm = "battle-boss-italy",
			side = 2,
			bgName = "bg_italy_cg6",
			dir = 1,
			say = "Mediterranean Sea - Malta",
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900237,
			side = 2,
			bgName = "bg_italy_cg6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Withered grass beneath the cloudy skies... the smell of gasoline... the sound of propellers and engines...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_italy_cg6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 900237,
			say = "What an awful place for a tea party... But, what else is one to: in front of the Royal Navy's forward base?",:
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_italy_cg6",
			actorName = "？？？",
			dir = 1,
			say = "Sister? Le Terrible, is that you over there?",
			dialogShake = {
				speed = 0.09,
				x = 10,
				number = 2
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900237,
			side = 2,
			bgName = "bg_italy_cg6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Sure is. Oh my, if it isn't my cute and elegant little sister. How have you been?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 801010,
			side = 2,
			bgName = "bg_italy_cg6",
			nameColor = "#a9f548",
			dir = 1,
			say = "How long has it been? I haven't seen you since Mers-el-Kébir...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 801010,
			side = 2,
			bgName = "bg_italy_cg6",
			nameColor = "#a9f548",
			dir = 1,
			say = "Wait, did you come here instead of heading to Casablanca?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_italy_cg6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 900237,
			say = "Yup. Had to: some soul-searching after all, as well as some research.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_italy_cg6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 900237,
			say = "But you know... I still have some reservations about fighting with the Royal Navy.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 801010,
			side = 2,
			bgName = "bg_italy_cg6",
			nameColor = "#a9f548",
			dir = 1,
			say = "You're as honest as ever... It's True that given what happened to Dunkerque, everyone is still a bit on edge... especially Richelieu...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900237,
			side = 2,
			bgName = "bg_italy_cg6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "It'll be fine, my wise sister. You:n't have to say anything that this fool hasn't already heard.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 801010,
			side = 2,
			bgName = "bg_italy_cg6",
			nameColor = "#a9f548",
			dir = 1,
			say = "Le Terrible...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_italy_cg6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 900237,
			say = "Algérie asked me to tell Richelieu about Jean Bart's recovery.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_italy_cg6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 900237,
			say = "Has she left the Vichya Dominion yet?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900237,
			side = 2,
			bgName = "bg_italy_cg6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "It's all a bit rushed, but if the elder sister isn't allowed to make her move, the younger one won't take the next step either.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 801010,
			side = 2,
			bgName = "bg_italy_cg6",
			nameColor = "#a9f548",
			dir = 1,
			say = "Huh? What: you mean by that?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900237,
			side = 2,
			bgName = "bg_italy_cg6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Well, tell you what. Let's wait until Richelieu comes back before we talk about work.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_italy_cg6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 900237,
			say = "More importantly...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_italy_cg6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 900237,
			say = "I'm back, Le Triomphant, my lovely little sister~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 801010,
			side = 2,
			bgName = "bg_italy_cg6",
			nameColor = "#a9f548",
			dir = 1,
			say = "Wait... Le Terrible...",
			dialogShake = {
				speed = 0.09,
				x = 10,
				number = 2
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_italy_cg6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 900237,
			say = "Ohh? Should the Knight-Princess of Iris Libre really be crying so readily? Here, have some tea...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900237,
			side = 2,
			bgName = "bg_italy_cg6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "......",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_italy_cg6",
			dir = 1,
			blackBg = True,
			actor = 900237,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Ah, this tea's a bit too hot, isn't it...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
