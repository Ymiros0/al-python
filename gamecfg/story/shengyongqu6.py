return {
	id = "SHENGYONGQU6",
	mode = 2,
	once = True,
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			dir = 1,
			side = 2,
			blackBg = True,
			say = "The Basilica Isle",
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			blackBg = True,
			say = "While the combined forces of the Royal Navy and Iris Libre were engaging the Sirens...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#ffff4d",
			bgName = "bg_qiongding_3",
			actorName = "Vauquelin",
			dir = 1,
			bgmDelay = 0.5,
			say = "The Iris-Royal Navy forces have entered the waters around the Basilica and are now engaging the Sirens there!",
			bgm = "bgm-cccp2",
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "La Galissonnière",
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "They're already here?! Ahaha, now this is what I've been looking forward to!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Algérie",
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "If the Cardinal is personally leading this operation, their morale is probably quite high~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Algérie",
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "...Anyway, they sure are devoting a considerable amount of effort into taking this Basilica.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "La Galissonnière",
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "I'm guessing the party's over if we lose this place to them, right?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Vauquelin",
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "Super over. They should just libre-rate their inner party animals and join us instead. Get it? Libre-rate? Okay, never mind...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Vauquelin",
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "...I just:n't understand them at all. Why'd they come all the way out here, so far from the European continent?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Vauquelin",
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "This Basilica might be important for the Iris Libre, but I can't believe they'd go so far as to try to steal it from us.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "La Galissonnière",
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "Just what is their leadership thinking?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "La Galissonnière",
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "Algérie, is there anything unusual about this Basilica?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Algérie",
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "Not that I know. Having that said, there is something that bothers me... We were ordered to \"protect the Basilica at any cost.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "La Galissonnière",
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "So, even Algérie:esn't know. Y'know, I actually looked into it a little.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Algérie",
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "Oh, is that so?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#ffff4d",
			bgName = "bg_qiongding_3",
			actorName = "Vauquelin",
			dir = 1,
			say = "Ooh, I wanna hear about it!",
			dialogShake = {
				speed = 0.09,
				x = 10,
				number = 2
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "La Galissonnière",
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "It's not like I learned anything though. Not even my friend on the \"other\" side knew about it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Vauquelin",
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "Aww...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "La Galissonnière",
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "Well, maybe they're just trying to settle an old score? Algérie, should we intercept them?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Vauquelin",
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "It'd be nice if the Sirens could push them back without us~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "La Galissonnière",
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "Nuh-uh, no way that's happening. I could take those scrapheaps on all by myself; there's no way they'd be able to stop a joint fleet led by the Cardinal.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "La Galissonnière",
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "The best we can hope for is that they'll buy us some time. Not sure how we keep them from approaching the Basilica, but this is the best we got.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Algérie",
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "We have no way of controlling those mass-produced Sirens manufactured by the Iron Blood, huh?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "La Galissonnière",
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "Well, it's none of my concern. I wonder what the leadership of the Vichya and Iron Blood are up to right now...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_qiongding_3",
			say = "La Galissonnière looked over into the distance, where she saw a figure on the horizon.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "La Galissonnière",
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "The Vichya Dominion's secret weapon, prepared especially for this operation... is that her over there?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "La Galissonnière",
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "I've never heard of a fourth Richelieu-class ship before...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "La Galissonnière",
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "She's been there motionless ever since she got here. She has following Algérie's orders, but in this emotionless way, like she's a robot or something...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "La Galissonnière",
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "Is she really on our side...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Algérie",
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "We can't ask for too much when we:n't even have the mass-production ships at our disposal.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Algérie",
			bgName = "bg_qiongding_3",
			nameColor = "#ffff4d",
			dir = 1,
			say = "Now, allow me to explain our strategy...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
