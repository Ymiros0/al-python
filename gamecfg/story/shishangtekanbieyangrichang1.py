return {
	fadeOut = 1.5,
	mode = 2,
	id = "SHISHANGTEKANBIEYANGRICHANG1",
	placeholder = {
		"playername"
	},
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Port Fashion Collection! A Glimpse Into Another Life\n\n<size=45>1 The Lover's Heart Flutters On Duty</size>",
					1
				}
			}
		},
		{
			live2d = "login",
			side = 2,
			bgName = "star_level_bg_147",
			dir = 1,
			bgm = "story-richang-7",
			actor = 207053,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Phew...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			live2d = "login",
			side = 2,
			bgName = "star_level_bg_147",
			withoutActorName = True,
			hideRecordIco = True,
			actor = 207053,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "In an empty classroom after school, a certain well-bred young lady stretches on her tip-toes,:ing her best to clean the blackboard.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			live2d = True,
			side = 2,
			bgName = "star_level_bg_147",
			withoutActorName = True,
			hideRecordIco = True,
			actor = 207053,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Based on how serious she looks, I:n't think she's noticed my presence yet.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			live2d = True,
			side = 2,
			bgName = "star_level_bg_147",
			withoutActorName = True,
			hideRecordIco = True,
			actor = 207053,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "While I consider helping her, she stretches even more to reach the top of the blackboard. By this point, her entire upper body is pressed against it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			live2d = True,
			side = 2,
			bgName = "star_level_bg_147",
			withoutActorName = True,
			hideRecordIco = True,
			actor = 207053,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Chalk dust falls off of it... and straight into her eyes.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 207053,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = "touch",
			say = "Eep!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			live2d = True,
			side = 2,
			bgName = "star_level_bg_147",
			withoutActorName = True,
			hideRecordIco = True,
			actor = 207053,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Blinded by chalk dust, she steps back and trips over.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "Be careful!",
					flag = 1
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_147",
			live2d = True,
			hideRecordIco = True,
			actor = 207053,
			actorName = "{playername}",
			hidePaintObj = True,
			say = "*cough* *cough*...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			actor = 207053,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = "main2",
			say = "Professor?! Are you okay?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 207053,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "N-no, I can't possibly be that heavy... Surely you're fine...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 207053,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = "main3",
			say = "W-wait, that's not what's wrong...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 207053,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Anyway, thank you, Professor!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_147",
			live2d = True,
			hideRecordIco = True,
			actor = 207053,
			actorName = "{playername}",
			hidePaintObj = True,
			say = "(I know we're being filmed, but her calling me Professor still throws me off...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_147",
			live2d = True,
			hideRecordIco = True,
			actor = 207053,
			actorName = "{playername}",
			hidePaintObj = True,
			say = "I'm fine. Here, let me erase the chalk at the top. Are your eyes okay, Formidable?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 207053,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = "mission_complete",
			say = "Yes... They sting a little, but I'll be fine.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_147",
			live2d = True,
			hideRecordIco = True,
			actor = 207053,
			actorName = "{playername}",
			hidePaintObj = True,
			say = "I'll try blowing it out of your eyes. Stay still for me, okay?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 207053,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = "touch",
			say = "Huh? Okay... Thank you, Professor.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			live2d = True,
			side = 2,
			bgName = "star_level_bg_147",
			withoutActorName = True,
			hideRecordIco = True,
			actor = 207053,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Despite clearly blushing all the way to her ears, Formidable:es her best to lean in and face me.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 207053,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = "main2",
			say = "U-umm... Will this:?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			live2d = True,
			side = 2,
			bgName = "star_level_bg_147",
			withoutActorName = True,
			hideRecordIco = True,
			actor = 207053,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "She leans in close enough that our lips are mere inches from each other.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			live2d = True,
			side = 2,
			bgName = "star_level_bg_147",
			withoutActorName = True,
			hideRecordIco = True,
			actor = 207053,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Her sweet perfume and teary eyes have a bewitching power to them, coming ever so close to stealing my heart–",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_147",
			live2d = True,
			hideRecordIco = True,
			actor = 207053,
			actorName = "{playername}",
			hidePaintObj = True,
			say = "Hmm... It's a little hard to blow on your eyes at this angle...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 207053,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = "main1",
			say = "Geez, Comman–ahem! Professor!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 207053,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Okay... A little lower. All you need is for me to angle my head a little lower, right?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			live2d = True,
			side = 2,
			bgName = "star_level_bg_147",
			withoutActorName = True,
			hideRecordIco = True,
			actor = 207053,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "She flips her long hair, evidently dissatisfied, and slowly looks:wn.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			live2d = True,
			side = 2,
			bgName = "star_level_bg_147",
			withoutActorName = True,
			hideRecordIco = True,
			actor = 207053,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "But for some reason, she squats:wn low enough until her head is level with my waist.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 207053,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = "main2",
			say = "Professor? You won't tell me I'm too low this time, will you?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			live2d = True,
			side = 2,
			bgName = "star_level_bg_147",
			withoutActorName = True,
			hideRecordIco = True,
			actor = 207053,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Formidable looks up at me. There's a mischievous glint in her eyes and a grin on her face.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "A little, yeah...",
					flag = 1
				},
				{
					content = "No, I think it's perfect.",
					flag = 2
				}
			}
		},
		{
			live2d = "mission_complete",
			side = 2,
			bgName = "star_level_bg_147",
			dir = 1,
			optionFlag = 1,
			actor = 207053,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "But I'm ever so tired, Professor. Why:n't you bend over and find the right height yourself?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			live2d = True,
			side = 2,
			bgName = "star_level_bg_147",
			dir = 1,
			optionFlag = 1,
			actor = 207053,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Heheh, you did want to blow the chalk dust out of my eyes, didn't you?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_147",
			live2d = True,
			optionFlag = 1,
			hideRecordIco = True,
			actor = 207053,
			actorName = "{playername}",
			hidePaintObj = True,
			say = "Yeah, but it looks to me like you already cried it out...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			live2d = "touch",
			side = 2,
			bgName = "star_level_bg_147",
			dir = 1,
			optionFlag = 1,
			actor = 207053,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Aww. Must you be so observant now, of all times?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			live2d = True,
			side = 2,
			bgName = "star_level_bg_147",
			dir = 1,
			optionFlag = 2,
			actor = 207053,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Oh? Really, now?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			live2d = "main2",
			side = 2,
			bgName = "star_level_bg_147",
			dir = 1,
			optionFlag = 2,
			actor = 207053,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "But my head and your mouth are just so much further from each other now.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			live2d = True,
			side = 2,
			bgName = "star_level_bg_147",
			dir = 1,
			optionFlag = 2,
			actor = 207053,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Why would you say this is perfect, I wonder?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			live2d = True,
			side = 2,
			bgName = "star_level_bg_147",
			dir = 1,
			optionFlag = 2,
			actor = 207053,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "It's okay. I already cried out the chalk dust, so I:n't need your help anymore, Professor.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			live2d = True,
			side = 2,
			bgName = "star_level_bg_147",
			withoutActorName = True,
			hideRecordIco = True,
			actor = 207053,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "With that, Formidable stands back up and twirls gracefully in front of me.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			live2d = True,
			side = 2,
			bgName = "star_level_bg_147",
			withoutActorName = True,
			hideRecordIco = True,
			actor = 207053,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "*BEEP! BEEP!*",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			live2d = True,
			side = 2,
			bgName = "star_level_bg_147",
			withoutActorName = True,
			hideRecordIco = True,
			actor = 207053,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Just:, the recording equipment beeps, signaling that it's stopped.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_147",
			live2d = True,
			hideRecordIco = True,
			actor = 207053,
			actorName = "{playername}",
			hidePaintObj = True,
			say = "Phew... Okay. Does that mean the recording is:ne?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 207053,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = "mission",
			say = "Not at all... Ahem. The recording may not yet be complete yet, Commander.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 207053,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Because a certain someone failed to follow the script.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 207053,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Heehee, I think we'll have to redo the scene at this rate.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 207053,
			side = 2,
			bgName = "star_level_bg_147",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = "main3",
			say = "It's still early, so I expect you to give it another try now... my dear Professor~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
