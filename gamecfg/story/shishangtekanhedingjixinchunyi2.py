return {
	id = "SHISHANGTEKANHEDINGJIXINCHUNYI2",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Port Fashion Collection. Spring Festival I\n\n<size=45>2 Ascendant Dragon's Spring Feast</size>",
					1
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			say = "It is the eve of the New Year's banquet, and a lone girl is sitting:wn, looking anxious.",
			bgm = "main-chunjie3",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501051,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = "mail",
			say = "Mumble, mumble... Grumble...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501051,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Dang it all! I'm so worried!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Oh...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "Are you okay?",
					flag = 1
				},
				{
					content = "Are you all:ne in the kitchen?",
					flag = 2
				}
			}
		},
		{
			live2d = "touch",
			side = 2,
			bgName = "star_level_bg_165",
			dir = 1,
			optionFlag = 1,
			actor = 501051,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "No, I'm not okay!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			live2d = True,
			side = 2,
			bgName = "star_level_bg_165",
			dir = 1,
			optionFlag = 2,
			actor = 501051,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "That's the thing! I couldn't even get in!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501051,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = "main3",
			say = "Earlier this morning, everyone chased me out of the kitchen and forbade me from entering. They said they were preparing a surprise for me...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501051,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "But... Aaaggghhh! I'm worried out of my mind!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501051,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "There's all these dishes to cook, and I:n't know if Ning Hai and Ping Hai can: it!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501051,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = "mission_complete",
			say = "What if something goes wrong and they mess it up?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501051,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "There's also the fact that we all have different tastes and seasoning preferences, so in the worst case scenario...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			say = "Lung Wu's face turns more and more pale as she imagines everything that could possibly go wrong.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501051,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = "headtouch",
			say = "No! I need to go check on them!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(Hang on... Lung Wu must be exhausted after all the preparatory work she's:ne the past few days.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(I also remember Chen Hai asking me to not let her into the kitchen. \"She has to rest,\" she said.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(That being the case...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "(Look around.)",
					flag = 1
				}
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(There are appetizers plated up on the table. Drunken crab, baozis, and more...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(I:n't know who thinks of baozis as appetizers, but as a thing to distract Lung Wu with, they'll: the trick!)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "You know what they say – \"Don't be scared of freshly baked potatoes.\" Obviously, these aren't potatoes, but why:n't you try one anyway?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501051,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = "expedition",
			say = "Huh? Potatoes? What on Earth are you talking about?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501051,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "I:n't get what baozis have to: with anything... but if you say so, I'll try one...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501051,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = "main1",
			say = "Om, nom... Mmm!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			effects = {
				{
					active = True,
					name = "咬着包子惊讶的表情"
				}
			}
		},
		{
			actor = 501051,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "The texture of this:ugh! The seasoning in the bean paste! I... I hate to admit it, but Ping and Ning sure cook a good baozi!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Oh yeah? Now try the drunken crab.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501051,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = "main1",
			say = "Om, nom... Mmm!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501051,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "It's sweet and juicy... Did they use the Shaoxing wine I took out of the cellar? This has just the perfect amount of soy sauce, too!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(If a delicacy like drunken crab can't help her forget her worries,: not much will.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(Hm? Come to think of it, someone told me to never, ever let Lung Wu into the kitchen when she's drunk...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(Chen Hai was so insistent about it that I can't remember everything she said...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(It's fine. The banquet is almost ready, so there won't be any reason for Lung Wu to go into the kitchen before long.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			say = "I end up so distracted by the delicious drunken crab that... when I come to, I realize that Lung Wu has disappeared.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			options = {
				{
					content = "(Go find her NOW!)",
					flag = 1
				},
				{
					content = "(Don't worry about it.)",
					flag = 2
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			optionFlag = 1,
			say = "When I get up to search, Lung Wu is already on her way back to the table.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			optionFlag = 2,
			say = "They're already serving the food, so even if she went to the kitchen, there's surely nothing left for her to flub.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			optionFlag = 2,
			say = "While I rationalize my mistake, Lung Wu is already on her way back to the table.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			say = "Before long, the final dish of the banquet – spring rolls – is served.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			say = "That marks the completion of my mission to keep Lung Wu in check.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501051,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = "mission_complete",
			say = "Commander, spring rolls are a must during the New Year's festivities! Give this one a try.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			say = "When I put it in my mouth, chew, and taste the spring roll... spiciness beyond words assaults my senses!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "Wh-what is this?!",
					flag = 1
				},
				{
					content = "Water... I need water!",
					flag = 2
				},
				{
					content = "Delicious! Seconds, please!",
					flag = 3
				}
			}
		},
		{
			actor = 501051,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = "expedition",
			say = "Heh heh heh... Spicy, huh? That's the good stuff!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501051,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Those are my special recipe – flaming hot spring rolls! I snuck into the kitchen to make them!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = 501060,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			dir = 1,
			actorName = "Hu Pen",
			nameColor = "#A9F548FF",
			say = "Flaming hot?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = 501060,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			dir = 1,
			actorName = "Hu Pen",
			nameColor = "#A9F548FF",
			say = "Ack! Eating all that drunken crab turned her into a drunken Lung Wu...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = 501060,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			dir = 1,
			actorName = "Hu Pen",
			nameColor = "#A9F548FF",
			say = "Come on, Commander! When she's drunk, she can't cook anything but spicy food!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "How could this be...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "My perfect strategy, defeated at the hands of drunken crab!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
