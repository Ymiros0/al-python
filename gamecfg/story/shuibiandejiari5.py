return {
	fadeOut = 1.5,
	mode = 2,
	id = "SHUIBIANDEJIARI5",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Beachside Vacation Vignettes\n\n<size=45>Rabbit on Watch?!</size>",
					1
				}
			}
		},
		{
			side = 2,
			bgName = "star_level_bg_133",
			dir = 1,
			bgmDelay = 2,
			bgm = "story-richang-3",
			actor = 403092,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "*stares bunNoney*",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403092,
			side = 2,
			bgName = "star_level_bg_133",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Halt! Come over here, Commander~!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403092,
			side = 2,
			bgName = "star_level_bg_133",
			live2d = True,
			withoutActorName = True,
			nameColor = "#A9F548FF",
			dir = 1,
			say = "I'm just about to jump in the pool when Prinz Heinrich orders me to stop.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403092,
			side = 2,
			bgName = "star_level_bg_133",
			live2d = "touch2",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I hope you weren't seriously considering going into the pool just now!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403092,
			side = 2,
			bgName = "star_level_bg_133",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Because you totally neglected to: your stretches! Don't think I didn't notice!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403092,
			side = 2,
			bgName = "star_level_bg_133",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Going into water unprepared is dangerous, you know. You could get a leg cramp!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403092,
			side = 2,
			bgName = "star_level_bg_133",
			live2d = "mission",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I have:ne lifeguard training, and I: know how to perform CPR, but that's no excuse not to take safety precautions.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403092,
			side = 2,
			bgName = "star_level_bg_133",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "You're not dipping one toe in water until I've seen you: your stretches. If you:n't know how to, I'll show you!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403092,
			side = 2,
			bgName = "star_level_bg_133",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "C'mon, stretch those muscles! No half-assing on my watch!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403092,
			side = 2,
			bgName = "star_level_bg_133",
			live2d = True,
			withoutActorName = True,
			nameColor = "#A9F548FF",
			dir = 1,
			say = "A couple minutes later, I finish my warmup stretches under her watchful eye.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 403092,
			side = 2,
			bgName = "star_level_bg_133",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Alright, you're clear. Enjoy yourself in the pool!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "\"Aren't you going to swim with me?\"",
					flag = 1
				}
			}
		},
		{
			actor = 403092,
			side = 2,
			bgName = "star_level_bg_133",
			live2d = "main3",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I will, later! I've got to finish my shift first.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403092,
			side = 2,
			bgName = "star_level_bg_133",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It might look like I'm just sitting around, but trust me, I'm keeping my eyes on ten different things at once.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403092,
			side = 2,
			bgName = "star_level_bg_133",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Like if everyone's:ne their stretches, if someone looks like they need rescuing, or if any prankster submarines are up to no good.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403092,
			side = 2,
			bgName = "star_level_bg_133",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "So sadly, I've gotta watch from the sidelines until my shift is over!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403092,
			side = 2,
			bgName = "star_level_bg_133",
			live2d = "touch",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "But hey, you're gonna stick around for a while, right?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403092,
			side = 2,
			bgName = "star_level_bg_133",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It's not too much trouble to wait for me until my replacement comes in, is it?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403092,
			side = 2,
			bgName = "star_level_bg_133",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...Awesome! Don't go back on your word later, or you'll have a very upset bunny to deal with~!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
