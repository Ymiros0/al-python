return {
	fadeOut = 1.5,
	mode = 2,
	id = "SHUIBIANDEJIARI6",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Beachside Vacation Vignettes\n\n<size=45>Water Gun Samurai</size>",
					1
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_106",
			say = "I expected a beach resort full of laughter in the air. Instead, what I got...",
			bgmDelay = 2,
			bgm = "story-richang-6",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302216,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Found you!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302216,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			withoutActorName = True,
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Was a faceful from Noshiro's water gun!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302216,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = "mission_complete",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Looks like I win again.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302216,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "What,: you think I'm taking this too seriously?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302216,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Heheh... I distinctly remember YOU telling me to have fun to my heart's content today.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302216,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Which is exactly what I'm:ing. Or would you like to rescind your order?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302216,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = "expedition",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Whew. Water gun fights really are great fun. Invigorating, too. But what: you think, Commander?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			options = {
				{
					content = "\"I had a great time as well.\"",
					flag = 1
				},
				{
					content = "\"I'm left feeling a bit tired.\"",
					flag = 2
				}
			}
		},
		{
			actor = 302216,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			optionFlag = 1,
			nameColor = "#A9F548FF",
			say = "Good to hear. I want this to be relaxing for both of us.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302216,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			optionFlag = 2,
			nameColor = "#A9F548FF",
			say = "Tired? Aww... Maybe I should've gone a little easier on you after all.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302216,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = "main3",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "What should we: now? May I suggest a casual stroll in the sand?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302216,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "The weather's not too hot, the sand is warm, and the winds are nice. Perfect conditions for a walk.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302216,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "We:n't have to go anywhere in particular, either. Walking for walking's sake rejuvenates the body and soul.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302216,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "And I'd certainly like to get a little closer to you...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302216,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Ahem. I:n't hear any objections. Shall we get going?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302216,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			withoutActorName = True,
			nameColor = "#A9F548FF",
			dir = 1,
			say = "She starts walking in one direction, but quickly comes to a stop and turns around.。",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302216,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = "main2",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "On second thought, let's go the other way. It looks crowded over there, and I'd rather avoid running into Sakawa or Agano. They'd no:ubt tease you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302216,
			side = 2,
			bgName = "star_level_bg_106",
			live2d = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Also, umm... Can we hold hands?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
