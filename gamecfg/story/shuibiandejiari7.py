return {
	fadeOut = 1.5,
	mode = 2,
	id = "SHUIBIANDEJIARI7",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Beachside Vacation Vignettes\n\n<size=45>Bright-Sky Mermaid</size>",
					1
				}
			}
		},
		{
			side = 2,
			bgName = "star_level_bg_106",
			spine = True,
			dir = 1,
			bgmDelay = 2,
			bgm = "story-richang-6",
			actor = 107101,
			nameColor = "#A9F548FF",
			say = "How:es this pose look, Commander?",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107101,
			side = 2,
			bgName = "star_level_bg_106",
			spine = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...I look nervous to you?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107101,
			side = 2,
			bgName = "star_level_bg_106",
			spine = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I:n't feel nervous, I just:n't know how to pose in a bikini... Give me a minute to compose myself and I'll figure it out.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107101,
			side = 2,
			bgName = "star_level_bg_106",
			spine = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...It looks good on me? Thank you. Hornet picked it for me.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107101,
			side = 2,
			bgName = "star_level_bg_106",
			spine = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "She said I should be forward with you on our beach trip, and that an \"equally forward swimsuit\" would help with that.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107101,
			side = 2,
			bgName = "star_level_bg_106",
			spine = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "When I first saw it, I thought it was far too revealing for my tastes.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107101,
			side = 2,
			bgName = "star_level_bg_106",
			spine = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "But: I went to the beach and saw others wearing even more revealing swimwear. So I reconsidered. Maybe it wasn't so wrong after all, right?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107101,
			side = 2,
			bgName = "star_level_bg_106",
			spine = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Besides, I can live with wearing something a more risqué when there's no one but us around.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107101,
			side = 2,
			bgName = "star_level_bg_106",
			spine = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Whew... The wind feels lovely, but it is a bit strong...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107101,
			side = 2,
			bgName = "star_level_bg_106",
			spine = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...Hm? What's the matter, Commander?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "\"It's just that your bikini... well...\"",
					flag = 1
				}
			}
		},
		{
			actor = 107101,
			side = 2,
			bgName = "star_level_bg_106",
			spine = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "What about it? I– Oh my goodness! The strap came loose!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107101,
			side = 2,
			bgName = "star_level_bg_106",
			spine = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "My deepest apologies, Commander! I had no idea... Hold on a second!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107101,
			side = 2,
			bgName = "star_level_bg_106",
			spine = True,
			withoutActorName = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "She quickly fixes her wardrobe malfunction, but the awkwardness of it stays in the air for some time.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107101,
			side = 2,
			bgName = "star_level_bg_106",
			spine = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "This backdrop you chose is very tasteful, Commander. Also, is this pose any good?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 107101,
			side = 2,
			bgName = "star_level_bg_106",
			spine = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...Oh, forgive me, you must be exhausted. You poor thing.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107101,
			side = 2,
			bgName = "star_level_bg_106",
			spine = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I know you've been busy socializing with others all day and indulging their requests.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107101,
			side = 2,
			bgName = "star_level_bg_106",
			spine = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "You need a well-earned break. If you'd like, you may rest your head on my lap.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107101,
			side = 2,
			bgName = "star_level_bg_106",
			spine = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "No need to be shy. I've let Hammann, Enterprise, and even Hornet: the same before. It's not weird to me.~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107101,
			side = 2,
			bgName = "star_level_bg_106",
			spine = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Come now. This is the least I can offer to the person I'd give my life to protect.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107101,
			side = 2,
			bgName = "star_level_bg_106",
			spine = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Relax, close your eyes, and listen to the tranquil sounds of the wind and the sea with me~.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
