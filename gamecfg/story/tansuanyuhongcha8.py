return {
	fadeOut = 1.5,
	mode = 2,
	id = "TANSUANYUHONGCHA8",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Tonic and Tea\n\n<size=45>8. A Word of Thanks</size>",
					1
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_145",
			dir = 1,
			bgmDelay = 2,
			bgm = "bsm-7",
			actor = 405012,
			nameColor = "#a9f548",
			hidePaintObj = True,
			say = "Welcome, Commander.",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_145",
			say = "I'm once again greeted by Bismarck at the café's entrance.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 405012,
			say = "This way, please.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 405012,
			say = "Take your time, we'll be with you soon.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_145",
			say = "Bismarck leads me to my table. We pass through a corridor, passing not a single other customer on the way.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405031,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Just so you know, we're not open for another half an hour.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_145",
			say = "\"Oh, really? Sorry, I must've misremembered when you open.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 405012,
			say = "It's fine. Everything is ready for us to start early today anyway.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 405012,
			say = "This presents us a rare opportunity, in fact. Care for a drink, Commander?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_145",
			say = "She places a foaming drink on the table.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 401465,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Ah. Latte art, but with beer.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405031,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Mhm. The principle's the same with any drink that foams.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_coffeetea_1",
			hidePaintObj = True,
			dir = 1,
			actorName = "Ulrich von Hutten",
			say = "That said, you have to be intimately familiar with Iron Blood beer to pull it off. You'd better appreciate the skill it takes.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_coffeetea_1",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bismarck",
			say = "It certainly:es. It takes an expert hand, like Ulrich's, and specialized equipment to make a drink like this. It also tastes as good as it looks.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_coffeetea_1",
			say = "I examine the drink from above. There is a word written in the foam – \"Danke.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			nameColor = "#a9f548",
			bgName = "bg_coffeetea_1",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bismarck",
			say = "Don't be shy now. Though it may be a work of art, it's still a drink. Drinks are meant to be drunk.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_coffeetea_1",
			say = "I take one final appreciatory look at the lovingly crafted message,::wn the glass' contents all in one go.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_coffeetea_1",
			hidePaintObj = True,
			dir = 1,
			actorName = "Ulrich von Hutten",
			say = "Heh. If the Commander's look is any indication, the drink is a success.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			nameColor = "#a9f548",
			bgName = "bg_coffeetea_1",
			hidePaintObj = True,
			dir = 1,
			actorName = "Z46",
			say = "Indeed. And its success entails our glorious victory.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_coffeetea_1",
			say = "\"This drink is incredible. The nutty, full flavor complements the kick of the carbonation. I feel like I just got supercharged with energy.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			nameColor = "#a9f548",
			bgName = "bg_coffeetea_1",
			hidePaintObj = True,
			dir = 1,
			actorName = "Bismarck",
			say = "Ah yes, that's the secret ingredient you tasted – almonds.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403030,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Ulrich, Z46, come to the kitchen. We have a small problem on our hands.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405012,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "A \"problem\"? I'll go and have a look.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405031,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "No need to, we'll take care of it. Go make sure we're ready to open instead.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_145",
			say = "At Eugen's behest, Ulrich and Z46 make for the kitchen to resolve the problem.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 405012,
			say = "I hope they're not up to anything strange again.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405012,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Oh, by the way, I personally taste-test all the drinks we make for you. I always worry if you'll like them, though.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 405012,
			say = "After Eugen first suggested I take on the role of store manager... frankly, I struggled for a long while.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 405012,
			say = "But I'd already agreed to it. And once I agree to something, I commit wholeheartedly.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_145",
			say = "\"It's a good thing to push yourself sometimes. Just:n't: overdo it, okay?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_145",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 405012,
			say = "Don't worry, I won't. Having overcome that initial hurdle, I'm actually having a good time now.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "star_level_bg_145",
			dir = 1,
			blackBg = True,
			say = "Bismarck and I have a pleasant chat, killing a bit of time before formal opening hours.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
