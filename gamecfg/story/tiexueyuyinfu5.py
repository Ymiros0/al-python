return {
	fadeOut = 1.5,
	mode = 2,
	id = "TIEXUEYUYINFU5",
	once = True,
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Third Movement\n\n<size=45>Great Longing</size>",
					1
				}
			}
		},
		{
			side = 0,
			actorName = "Bismarck",
			bgName = "bg_bsm_5",
			nameColor = "#ffde38",
			dir = 1,
			say = "... I have no reason to listen to a single word from you Sirens.",
			useBg2 = True,
			bgm = "bsm-2",
			bgAspectMode = AspectMode.FitInParent,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Observer",
			bgName = "bg_bsm_5",
			nameColor = "#ff5c5c",
			dir = 1,
			useBg2 = True,
			say = "Ahahaha~ Be silent and listen carefully, you insignificant being.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 0,
			actorName = "Bismarck",
			bgName = "bg_bsm_5",
			nameColor = "#ffde38",
			dir = 1,
			useBg2 = True,
			say = "How dare you...!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Observer",
			bgName = "bg_bsm_5",
			nameColor = "#ff5c5c",
			dir = 1,
			useBg2 = True,
			say = "If this continues, Iron Blood will cease to exist in this world, too.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Observer",
			bgName = "bg_bsm_5",
			nameColor = "#ff5c5c",
			dir = 1,
			useBg2 = True,
			say = "Your enemies will hurt those you wish to protect, steal your riches, and make your oceans their own.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Observer",
			bgName = "bg_bsm_5",
			nameColor = "#ff5c5c",
			dir = 1,
			useBg2 = True,
			say = "I suppose one would call it a humiliating defeat, no?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Observer",
			bgName = "bg_bsm_5",
			nameColor = "#ff5c5c",
			dir = 1,
			useBg2 = True,
			say = "None of your comrades will be spared of the bitter taste of this defeat. Especially not your dear sister Tirpitz... Ahaha...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Observer",
			bgName = "bg_bsm_5",
			nameColor = "#ff5c5c",
			dir = 1,
			useBg2 = True,
			say = "You:n't want to suffer another Article 231,: you?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			nameColor = "#ffde38",
			side = 0,
			bgName = "bg_bsm_5",
			actorName = "Bismarck",
			dir = 1,
			useBg2 = True,
			say = "Silence!",
			dialogShake = {
				speed = 0.09,
				x = 8.5,
				number = 2
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 0,
			actorName = "Bismarck",
			bgName = "bg_bsm_5",
			nameColor = "#ffde38",
			dir = 1,
			useBg2 = True,
			say = "The Iron Blood is not so brittle that it would crumble from a few isolated defeats!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 0,
			actorName = "Bismarck",
			bgName = "bg_bsm_5",
			nameColor = "#ffde38",
			dir = 1,
			useBg2 = True,
			say = "We will seize victory and eradicate you once more, you despicable enemies of humankind!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Observer",
			bgName = "bg_bsm_5",
			nameColor = "#ff5c5c",
			dir = 1,
			useBg2 = True,
			say = "Ahaha... Let me tell you something I once heard from a human...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Observer",
			bgName = "bg_bsm_5",
			nameColor = "#ff5c5c",
			dir = 1,
			useBg2 = True,
			say = "\"Humankind, much like a tree, wishes to grow to reach the clouds; the light; the Heavens. As its branches grow, its roots:, too. Deeper and deeper into the ground; into darkness; towards Hell.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Observer",
			bgName = "bg_bsm_5",
			nameColor = "#ff5c5c",
			dir = 1,
			useBg2 = True,
			say = "And is it not the light that you desire?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "Bismarck",
			side = 0,
			bgName = "bg_bsm_5",
			nameColor = "#ffde38",
			dir = 1,
			say = "I have no more to say to you... Main guns, feuer!",
			soundeffect = "event./battle/boom2",
			flashN = {
				color = {
					1,
					1,
					1
				},
				alpha = {
					{
						0,
						1,
						0.2
					},
					{
						1,
						0,
						0.2,
						0.2
					},
					{
						0,
						1,
						0.2,
						0.4
					},
					{
						1,
						0,
						0.2,
						0.6
					}
				}
			},
			dialogShake = {
				speed = 0.09,
				x = 8.5,
				number = 2
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Observer",
			bgName = "bg_bsm_5",
			nameColor = "#ff5c5c",
			dir = 1,
			useBg2 = True,
			say = "With this little power, your wish hasn't the slightest chance of being granted.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Observer",
			bgName = "bg_bsm_5",
			nameColor = "#ff5c5c",
			dir = 1,
			say = "Think about it. Just what: YOU have that WE want?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Observer",
			bgName = "bg_bsm_5",
			nameColor = "#ff5c5c",
			dir = 1,
			useBg2 = True,
			say = "Your inferior technology? Your nonexistent resources? Your waters, which we've already claimed for ourselves?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Observer",
			bgName = "bg_bsm_5",
			nameColor = "#ff5c5c",
			dir = 1,
			useBg2 = True,
			say = "You have nothing, not a single thing worthy of our interest.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 0,
			actorName = "Bismarck",
			bgName = "bg_bsm_5",
			nameColor = "#ffde38",
			dir = 1,
			useBg2 = True,
			say = "…………",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Observer",
			bgName = "bg_bsm_5",
			nameColor = "#ff5c5c",
			dir = 1,
			useBg2 = True,
			say = "Just the Royal Navy alone is many times powerful than you. If they so wanted, they could keep your trade routes blockaded in perpetuity...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Observer",
			bgName = "bg_bsm_5",
			nameColor = "#ff5c5c",
			dir = 1,
			useBg2 = True,
			say = "If you'd realize that it's they, not us, who are threatening your existence,: you'd be able to understand the situation.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Observer",
			bgName = "bg_bsm_5",
			nameColor = "#ff5c5c",
			dir = 1,
			useBg2 = True,
			say = "You may not believe it, but unlike those hypocrites who only pay lip service to the idea of liberty and justice, we will actually hold you in high regards.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Observer",
			bgName = "bg_bsm_5",
			nameColor = "#ff5c5c",
			dir = 1,
			useBg2 = True,
			say = "Which is why I gave you the key to your future. But how you use it is up to you...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Observer",
			bgName = "bg_bsm_5",
			nameColor = "#ff5c5c",
			dir = 1,
			useBg2 = True,
			say = "Whether this forbidden power leads to a bright new future for you and your comrades or a tragic repeat of the sorrowful demise you've known before, all depends on you.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Observer",
			bgName = "bg_bsm_5",
			nameColor = "#ff5c5c",
			dir = 1,
			useBg2 = True,
			say = "... I assure you, we will meet again. Ahahaha...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 0,
			actorName = "Bismarck",
			bgName = "bg_bsm_5",
			nameColor = "#ffde38",
			dir = 1,
			useBg2 = True,
			say = "... Throughout the whole ordeal, I was trembling.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 0,
			actorName = "Bismarck",
			bgName = "bg_bsm_5",
			nameColor = "#ffde38",
			dir = 1,
			useBg2 = True,
			say = "Such power, vast beyond measure. There was never any room for discussion, let alone debate... I had no choice but to accept the power.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 0,
			actorName = "Bismarck",
			bgName = "bg_bsm_5",
			nameColor = "#ffde38",
			dir = 1,
			useBg2 = True,
			say = "Chancellor... if you were in my place, what would you:...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 207040,
			side = 2,
			nameColor = "#a9f548",
			say = "Ahah, I've discovered Bismarck! May I scramble my planes?",
			dir = 1,
			bgmDelay = 2,
			bgm = "bsm-1",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205050,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "Certainly. Permission granted. Show her the might of our aircraft carriers.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 207040,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "Understood! This is Victorious, commencing plane launch! God save the Queen!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
