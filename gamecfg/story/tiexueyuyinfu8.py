return {
	fadeOut = 1.5,
	mode = 2,
	id = "TIEXUEYUYINFU8",
	once = True,
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Fourth Movement\n\n<size=45>Passion, Joy, and Suffering</size>",
					1
				}
			}
		},
		{
			expression = 3,
			side = 2,
			blackBg = True,
			dir = 1,
			bgmDelay = 2,
			bgm = "bsm-5",
			actor = 408040,
			nameColor = "#ffde38",
			say = "Captain Bismarck! Captain Bismarck, please wait!",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			effects = {
				{
					delay = 2,
					name = "memoryFog",
					active = True
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405010,
			side = 1,
			nameColor = "#ffde38",
			dir = 1,
			blackBg = True,
			say = "Ah, the little submarine. Good morning to you.",
			paintingFadeOut = {
				time = 0.5,
				side = 0
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 0,
			actor = 408040,
			nameColor = "#ffde38",
			dir = 1,
			blackBg = True,
			say = "Hey, um! Your performance yesterday was absolutely fantastic, and I will never ever in my whole life forget it!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 2,
			side = 1,
			actor = 405010,
			nameColor = "#ffde38",
			dir = 1,
			blackBg = True,
			say = "…………",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 0,
			actor = 408040,
			nameColor = "#ffde38",
			dir = 1,
			blackBg = True,
			say = "A-also! Captain Bismarck, I was hoping I could ask you for a small little favor!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 405010,
			side = 1,
			nameColor = "#ffde38",
			dir = 1,
			blackBg = True,
			say = "What would that be?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 408040,
			side = 0,
			nameColor = "#ffde38",
			dir = 1,
			blackBg = True,
			say = "T-thank you for hearing me out! You see, I just entered service today! I was hoping you could, uh...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 0,
			actor = 408040,
			nameColor = "#ffde38",
			dir = 1,
			blackBg = True,
			say = "Like in one of my fairy tales! \"Starting today, U-556 shall gallop across the seas, much like the honorable knight Sir Parzival did!\" ... H-how:es that sound?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 1,
			actor = 405010,
			nameColor = "#ffde38",
			dir = 1,
			blackBg = True,
			say = "Congratulations. Do your utmost in servitude to Iron Blood.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 408040,
			side = 0,
			nameColor = "#ffde38",
			dir = 1,
			blackBg = True,
			say = "No, no, I meant, like... Um, I know this is reeeeaaally improper of me to ask, and I'm sorry for even considering being so rude as to: it anyway, but...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 0,
			actor = 408040,
			nameColor = "#ffde38",
			dir = 1,
			blackBg = True,
			say = "I was hoping you could give me your blessings! If so, I'd be able to push myself to my absolute limits in my missions on the front!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 3,
			side = 1,
			actor = 405010,
			nameColor = "#ffde38",
			dir = 1,
			blackBg = True,
			say = "……………",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 408040,
			side = 0,
			nameColor = "#ffde38",
			dir = 1,
			blackBg = True,
			say = "I-I know! I'll show you my special trick!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 0,
			actor = 408040,
			nameColor = "#ffde38",
			dir = 1,
			blackBg = True,
			say = "Look! It's a water handstand!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "move",
					y = -2000,
					delay = 0.5,
					dur = 1,
					x = -1000
				}
			}
		},
		{
			dir = 1,
			side = 2,
			blackBg = True,
			say = "U-556 did a backflip and plunged deep below the water.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			actor = 405010,
			nameColor = "#ffde38",
			dir = 1,
			blackBg = True,
			say = "... Pffft...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 15,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			dir = 1,
			side = 2,
			blackBg = True,
			say = "The strange yet humorous trick was able to bring a smile even to Bismarck's lips.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 408040,
			side = 2,
			nameColor = "#ffde38",
			dir = 1,
			blackBg = True,
			say = "Captain... Bismarck?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			actor = 405010,
			nameColor = "#ffde38",
			dir = 1,
			blackBg = True,
			say = "Pardon, I'm just a little awestruck by how different you are to everyone else I've met.",:
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			actor = 408040,
			nameColor = "#ffde38",
			dir = 1,
			blackBg = True,
			say = "I've heard from some people that you're really reserved and hard to approach, Captain Bismarck...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			actor = 408040,
			nameColor = "#ffde38",
			dir = 1,
			blackBg = True,
			say = "But that's not the case at all... You:n't have trouble talking even to a submarine like me! In fact, you're actually really friendly!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405010,
			side = 2,
			nameColor = "#ffde38",
			dir = 1,
			blackBg = True,
			say = "(... Is that... how people view me? That's... rather disheartening.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			blackBg = True,
			say = "The thought of it made Bismarck's smile disappear. She turned to U-556, whose head was poking out of the water, to say her goodbyes.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			actor = 405010,
			nameColor = "#ffde38",
			dir = 1,
			blackBg = True,
			say = "I've heard your wish. Now:, if you'll excuse me.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			blackBg = True,
			say = "Bismarck departed, leaving U-556 all to herself.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			blackBg = True,
			say = "Then, on the following day...",
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 408030,
			side = 2,
			nameColor = "#ffde38",
			dir = 1,
			blackBg = True,
			say = "556, what's the matter? ... You:n't look very happy.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			actor = 408040,
			nameColor = "#ffde38",
			dir = 1,
			blackBg = True,
			say = "Well...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			actor = 408040,
			nameColor = "#ffde38",
			dir = 1,
			blackBg = True,
			say = "Captain Bismarck promised she'd come back to give her blessings, but she's still not back yet...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 408030,
			side = 2,
			nameColor = "#ffde38",
			dir = 1,
			blackBg = True,
			say = "Captain Bismarck is probably very busy... I:ubt she'd remember a promise like that...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			actor = 408040,
			nameColor = "#ffde38",
			dir = 1,
			blackBg = True,
			say = "Yeah... you're right. I'll have to prove myself in battle, that way Captain Bismarck will one day bestow me with the Knight's Cross with Oak Leaves...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			actor = 408040,
			nameColor = "#ffde38",
			dir = 1,
			blackBg = True,
			say = "That's it! The Knight's Cross with Oak Leaves! That's only fitting for me, Parzival of The Seas!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 408040,
			side = 2,
			nameColor = "#ffde38",
			dir = 1,
			blackBg = True,
			say = "I've formed a plan, and I'm gonna execute it! Submarine U-556, setting sa...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			stopbgm = True,
			dir = 1,
			blackBg = True,
			say = "U-556 noticed there were Manjuu gathering just within her sight. Then, a familiar face approached her...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			actor = 405010,
			nameColor = "#ffde38",
			dir = 1,
			blackBg = True,
			say = "One of our newest Iron Blood comrades will soon head into her first battle.",
			bgm = "bsm-7",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405010,
			side = 2,
			nameColor = "#ffde38",
			dir = 1,
			blackBg = True,
			say = "Though she may be small, her talents and reliability are second to none.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			actor = 405010,
			nameColor = "#ffde38",
			dir = 1,
			blackBg = True,
			say = "Now I, Bismarck, as the representative of the Iron Blood fleet, shall offer her my blessings!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405010,
			side = 2,
			nameColor = "#ffde38",
			dir = 1,
			blackBg = True,
			say = "Submarine U-556! Your success shall echo to the ends of the earth, and your reputation shall invoke fear within our enemies! May the God of war be with you!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405010,
			side = 2,
			nameColor = "#ffde38",
			dir = 1,
			blackBg = True,
			say = "Fanfare band, begin!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_bsm_1",
			dir = 1,
			useBg2 = True,
			say = "The band made up of Manjuu began playing a song which wishes success to a soldier departing to the front.",
			effects = {
				{
					active = True,
					name = "juqing02"
				},
				{
					active = False,
					name = "memoryFog"
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_bsm_1",
			dir = 1,
			useBg2 = True,
			say = "The largest battleship in all of the world was offering her blessings to a small submarine.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "U-556",
			bgName = "bg_bsm_1",
			nameColor = "#ffde38",
			dir = 1,
			useBg2 = True,
			say = "Captain Bismarck...!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_bsm_1",
			dir = 1,
			useBg2 = True,
			say = "Standing before Bismarck, the woman she adored, U-556 was determined to say one thing.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "U-556",
			bgName = "bg_bsm_1",
			nameColor = "#ffde38",
			dir = 1,
			useBg2 = True,
			say = "Captain Bismarck, i-if you'll please forgive me, I have one more favor to ask...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Bismarck",
			bgName = "bg_bsm_1",
			nameColor = "#ffde38",
			dir = 1,
			useBg2 = True,
			say = "My, you're a greedy child. What favor: you ask of me?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "U-556",
			bgName = "bg_bsm_1",
			nameColor = "#ffde38",
			dir = 1,
			useBg2 = True,
			say = "Please... allow me to call you \"Lord\"!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Bismarck",
			bgName = "bg_bsm_1",
			nameColor = "#ffde38",
			dir = 1,
			useBg2 = True,
			say = "... Huh?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_bsm_1",
			dir = 1,
			useBg2 = True,
			say = "Not even Bismarck could help being confused by this unexpected request.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "U-556",
			bgName = "bg_bsm_1",
			nameColor = "#ffde38",
			dir = 1,
			useBg2 = True,
			say = "It's to express my gratitude and respect for you! ... Um, may I please call you \"Lord Bismarck\"?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Bismarck",
			bgName = "bg_bsm_1",
			nameColor = "#ffde38",
			dir = 1,
			useBg2 = True,
			say = "Well... I suppose why not.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "U-556",
			bgName = "bg_bsm_1",
			nameColor = "#ffde38",
			dir = 1,
			useBg2 = True,
			say = "L-Lord Bismarck! I'm incredibly grateful!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Bismarck",
			bgName = "bg_bsm_1",
			nameColor = "#ffde38",
			dir = 1,
			useBg2 = True,
			say = "(... Dear me...) ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_bsm_1",
			dir = 1,
			useBg2 = True,
			say = "(Nobody has ever called me something that flattering before... a-am I blushing?)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "U-556",
			bgName = "bg_bsm_1",
			nameColor = "#ffde38",
			dir = 1,
			useBg2 = True,
			say = "Ah, also, I was hoping I could give you this before I depart, Lord Bismarck!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_bsm_1",
			dir = 1,
			useBg2 = True,
			say = "U-556 took out a slip of paper and handed it to Bismarck.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Bismarck",
			bgName = "bg_bsm_1",
			nameColor = "#ffde38",
			dir = 1,
			useBg2 = True,
			say = "What's this...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "U-556",
			bgName = "bg_bsm_1",
			nameColor = "#ffde38",
			dir = 1,
			useBg2 = True,
			say = "It's a certificate of sponsorship which I wrote yesterday!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "U-556",
			bgName = "bg_bsm_1",
			nameColor = "#ffde38",
			dir = 1,
			useBg2 = True,
			say = "I hereby pledge always to protect Lord Bismarck, no matter when and no matter where!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_bsm_1",
			dir = 1,
			useBg2 = True,
			say = "Bismarck's lips gently parted, after which she turned to U-556 and nodded.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "U-556",
			bgName = "bg_bsm_1",
			nameColor = "#ffde38",
			dir = 1,
			useBg2 = True,
			say = "Ah, sorry, I forgot! I have to sortie now! ... Lord Bismarck, I must take my leave!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "U-556",
			bgName = "bg_bsm_1",
			nameColor = "#ffde38",
			dir = 1,
			useBg2 = True,
			say = "We'll have to talk more next time! ... U-556, weighing anchor!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_bsm_1",
			dir = 1,
			useBg2 = True,
			say = "U-556 got on her rigging and joined U-557 as they disembarked from the port.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Bismarck",
			bgName = "bg_bsm_1",
			nameColor = "#ffde38",
			dir = 1,
			useBg2 = True,
			say = "So she has a little sister... Isn't that nice...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Bismarck",
			bgName = "bg_bsm_1",
			nameColor = "#ffde38",
			dir = 1,
			useBg2 = True,
			say = "…………",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Bismarck",
			bgName = "bg_bsm_1",
			nameColor = "#ffde38",
			dir = 1,
			useBg2 = True,
			say = "I wonder if Tirpitz is okay...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			mode = 1,
			effects = {
				{
					active = False,
					name = "juqing02"
				}
			},
			sequence = {
				{
					"<size=60>I, U-556, hereby declare before Neptune, Lord over oceans, seas, lakes, rivers, brooks, ponds, and rivulets,</size>",
					1
				},
				{
					"<size=60>that I will provide any desired assistance to my ally and Lord, battleship Bismarck,</size>",
					4
				},
				{
					"<size=60>at any place on the water, under water, on land, or in the air.</size>",
					6
				},
				{
					"<size=60>Signed. U-556, Parzival of The Seas</size>",
					8
				},
				{
					"<size=60>At the bottom, U-556 had made a drawing of herself towing her Lord Bismarck back to port.</size>",
					10
				}
			}
		},
		{
			bgm = "bsm-1",
			side = 2,
			seDelay = 2,
			dir = 1,
			bgmDelay = 2,
			soundeffect = "event./battle/boom2",
			actor = 204010,
			nameColor = "#a9f548",
			say = "I am Renown, battlecruiser of the Royal Navy's Force H! I am here because of your ambush on our ships! Cease your meaningless resistance at once!",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			flashN = {
				color = {
					1,
					1,
					1
				},
				alpha = {
					{
						0,
						1,
						0.2
					},
					{
						1,
						0,
						0.2,
						0.2
					},
					{
						0,
						1,
						0.2,
						0.4
					},
					{
						1,
						0,
						0.2,
						0.6
					}
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			nameColor = "#ffde38",
			actor = 405010,
			dir = -1,
			say = "Gah...! These new recruits of theirs...!",
			soundeffect = "event./battle/boom2",
			flashN = {
				color = {
					1,
					1,
					1
				},
				alpha = {
					{
						0,
						1,
						0.2
					},
					{
						1,
						0,
						0.2,
						0.2
					},
					{
						0,
						1,
						0.2,
						0.4
					},
					{
						1,
						0,
						0.2,
						0.6
					}
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 204010,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "The Royal Family's most capable ships are heading this way en masse! I will not let you flee!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405010,
			side = 2,
			expression = 3,
			nameColor = "#ffde38",
			dir = -1,
			say = "... It seems there's one thing you've forgotten.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405010,
			side = 2,
			nameColor = "#ffde38",
			dir = -1,
			say = "A confrontation may slow me:wn, but I have the advantage in this situation!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			nameColor = "#ffde38",
			side = 2,
			actor = 405010,
			dir = -1,
			say = "I'm afraid I'll have to deal with you! Engines, full speed ahead!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 207020,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "She's resisting, just as I figured she would!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 207020,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "Let's: precisely what King George V said and slow her:wn!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 207020,
			nameColor = "#a9f548",
			side = 2,
			soundeffect = "event./battle/plane",
			dir = 1,
			say = "Swordfishes, launch! Capture Bismarck!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
