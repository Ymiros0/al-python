return {
	fadeOut = 1.5,
	mode = 2,
	id = "WANSHENGJIE1",
	once = True,
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			mode = 1,
			sequence = {
				{
					"Witches' Eve I\n\n<size=45> 「Witches, Everlasting Innocence」</size>",
					1
				}
			}
		},
		{
			actor = 306050,
			nameColor = "#a9f548",
			side = 2,
			actorName = "{namecode.87}",
			dir = 1,
			say = "“Gift of an Everlasting Innocent Doll”? A present for me?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 306050,
			side = 2,
			actorName = "{namecode.87}",
			nameColor = "#a9f548",
			dir = 1,
			blackBg = True,
			withoutPainting = True,
			say = "(Opening...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 306051,
			actorName = "{namecode.87}",
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "Heehee... I really am beautiful!",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 306051,
			nameColor = "#a9f548",
			side = 2,
			actorName = "{namecode.87}",
			dir = 1,
			say = "But who’s this from? There's no name on the package...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 306051,
			actorName = "{namecode.87}",
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "Pumpkin congee... are we having pumpkin again for lunch? Yesterday was pumpkin stew, and pumpkin pie the day before... I'm so:ne with pumpkins!",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301050,
			side = 0,
			nameColor = "#a9f548",
			actorName = "{namecode.6}",
			dir = 1,
			say = "So am I, talk about a nutritional imbalance…",
			paintingFadeOut = {
				time = 0.5,
				side = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 306051,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.87}",
			dir = 1,
			say = "That's not the issue! Oh, by the way, {namecode.6}, I just received a strange present.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301050,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.6}",
			dir = 1,
			say = "A present—— ?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 306051,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.87}",
			dir = 1,
			say = "Heehee~ Doesn't this look good on me?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301050,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.6}",
			dir = 1,
			say = "It really suits {namecode.87}... What did your sisters say? Did they praise you?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 306051,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.87}",
			dir = 1,
			say = "Lately, they:n't really hang out with me... and keep saying that, \"{namecode.87} should find friends similar in age.\" Seriously?! They are almost the same age as me! Well, let's head out this afternoon!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301050,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.6}",
			dir = 1,
			say = "……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 306051,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.87}",
			dir = 1,
			say = "I saw {namecode.96} storing up her food supplies a few days ago. I thought she was planning to cook tempura for me, but it turned out to be pumpkin again!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301050,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.6}",
			dir = 1,
			say = "{namecode.96} won't miss any opportunity of having tempura, unless...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 306051,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.87}",
			dir = 1,
			say = "Speaking of {namecode.96}, recently she always looks at me in a strange way. Pumpkin... {namecode.96}... huh, let's dig into this!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301050,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.6}",
			dir = 1,
			say = "Do I have to go with you?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 306051,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.87}",
			dir = 1,
			say = "Of course! Stop losing yourself in combat and training, you deserve a break.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307050,
			actorName = "{namecode.95}",
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "What a pity, I’m not sure if I’ll go… you girls have fun~",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 312010,
			actorName = "{namecode.98}",
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "Pumpkins are delicious, nyaa~ and wasting food is a serious crime, nyaa~ {namecode.87} eat a little more, nyaa~",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			actorName = "{namecode.96}",
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "Tempura? No, there’s no more left! I wanted to store food for the coming winter!",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 306051,
			actorName = "{namecode.87}",
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "I still feel like… the atmosphere is very tense.",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 306051,
			nameColor = "#a9f548",
			side = 2,
			actorName = "{namecode.87}",
			dir = 1,
			say = "Although the most telling information is always hidden in unexpected places, {namecode.6}, follow me!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			actorName = "？？？",
			nameColor = "#a9f548",
			dir = 1,
			blackBg = True,
			withoutPainting = True,
			say = "How come my sister and I can’t attend this Halloween… Say! When the time comes...",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
