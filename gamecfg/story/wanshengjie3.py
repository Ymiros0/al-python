return {
	fadeOut = 1.5,
	mode = 2,
	id = "WANSHENGJIE3",
	once = True,
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			mode = 1,
			sequence = {
				{
					"Witches' Eve III\n\n<size=45>「Late Night Pranks」</size>",
					1
				}
			}
		},
		{
			actor = 301050,
			nameColor = "#a9f548",
			side = 2,
			actorName = "{namecode.6}",
			dir = 1,
			say = "The pumpkin lights have all been prepared and placed all over the school courtyard.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 306051,
			side = 1,
			nameColor = "#a9f548",
			actorName = "{namecode.87}",
			dir = 1,
			say = "Wow! It’s becoming more and more like a Halloween atmosphere!",
			paintingFadeOut = {
				time = 0.5,
				side = 0
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307050,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.95}",
			dir = -1,
			say = "Thanks to you, everyone must be highly anticipating Halloween now, huh~ I’ll also join your “Trick or treat”!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 306051,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.87}",
			dir = 1,
			say = "Hehehe, what a pity but you’re already an adult!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307050,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.95}",
			dir = -1,
			say = "Well... you’re right. Did you know? Lately, I hear that the port area has ghosts lurking at night.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301050,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.6}",
			dir = 1,
			say = "Ghosts?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307050,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.95}",
			dir = -1,
			say = "Well, all those floating pumpkin lanterns shimmering late at night looks a lot like “ghostly” pumpkin heads.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 306051,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.87}",
			dir = 1,
			say = "Ahhhh—— How come I haven’t seen any of those “ghosts”, I’m missing out on all the fun!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301170,
			actorName = "{namecode.19}",
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "{namecode.87}, lately a strange thing is happening.",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 306051,
			side = 1,
			nameColor = "#a9f548",
			actorName = "{namecode.87}",
			dir = 1,
			say = "A strange thing?!",
			paintingFadeOut = {
				time = 0.5,
				side = 0
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301170,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.19}",
			dir = 1,
			say = "So… the other day when I returned to my:rm, I found a package sitting in front of the:or. Inside there was—— this.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301050,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.6}",
			dir = 1,
			say = "This is——",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 306051,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.87}",
			dir = 1,
			say = "So pretty—— I want one too...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301171,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.19}",
			dir = -1,
			say = "So cute~ But there’s no name on the package, and my sister only saw a white figure placing it at the:or. I want to thank that mystery figure in person. I heard {namecode.87} also received a similar package, is there any meaning behind all of this?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 306051,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.87}",
			dir = 1,
			say = "I didn’t see anything...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301050,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.6}",
			dir = 1,
			say = "A white figure...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 306051,
			actorName = "{namecode.87}",
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "At last, it’s Halloween night! Hehe~ Do you like my costume? (Twirls)",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301051,
			side = 0,
			nameColor = "#a9f548",
			actorName = "{namecode.6}",
			dir = -1,
			say = "Actually—— I’ve also received a “gift”.",
			paintingFadeOut = {
				time = 0.5,
				side = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 306051,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.87}",
			dir = 1,
			say = "Huh, you got one as well? Did you find any clues to these mystery packages?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301051,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.6}",
			dir = -1,
			say = "Yes, I saw something—— those packages were sent by ghosts.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 306051,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.87}",
			dir = 1,
			say = "Haha~ {namecode.6} is making a joke. Strange, {namecode.19} still hasn’t arrived yet?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301051,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.6}",
			dir = -1,
			say = "Let’s go see if she’s in her:rm.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			actorName = "？？？",
			nameColor = "#a9f548",
			dir = 1,
			blackBg = True,
			withoutPainting = True,
			say = "I didn’t expect sewing clothes to be this troublesome...",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
