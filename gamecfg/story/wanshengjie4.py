return {
	fadeOut = 1.5,
	mode = 2,
	id = "WANSHENGJIE4",
	once = True,
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			mode = 1,
			sequence = {
				{
					"Witches' Eve IV\n\n<size=45>「The Missing Pumpkin Pie」</size>",
					1
				}
			}
		},
		{
			actor = 301171,
			nameColor = "#a9f548",
			side = 2,
			actorName = "{namecode.19}",
			dir = -1,
			say = "Sorry, I’ve got an emergency—— Has anyone seen my pumpkin pie?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 306051,
			side = 1,
			nameColor = "#a9f548",
			actorName = "{namecode.87}",
			dir = 1,
			say = "No I haven’t… what’s wrong?",
			paintingFadeOut = {
				time = 0.5,
				side = 0
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301180,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.20}",
			dir = 1,
			say = "{namecode.19}’s baked pumpkin pie has disappeared...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301171,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.19}",
			dir = -1,
			say = "Argh… I’ve worked tirelessly for three weeks in order to make the tastiest pumpkin pies for my sisters, and now I’ve got nothing to show for it!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301180,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.20}",
			dir = 1,
			say = "When the pies were baking, {namecode.19} had gone to change her outfit. And I was blanking out at the front:or——:, the pies were gone...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301171,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.19}",
			dir = -1,
			say = "After I changed my clothes, all those pies went missing——",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301180,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.20}",
			dir = 1,
			say = "Not entirely “all”, I tried one of the pies before they went missing, it was very delicious.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301171,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.19}",
			dir = -1,
			say = "Really? You better not be lying to your sisters!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 306051,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.87}",
			dir = 1,
			say = "When they went missing, was it just you two in the room?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 102091,
			side = 0,
			nameColor = "#a9f548",
			dir = -1,
			say = "Hehe, actually I was also there at the time.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 306051,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.87}",
			dir = 1,
			say = "Cleveland?!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 102091,
			side = 0,
			nameColor = "#a9f548",
			dir = -1,
			say = "This costume was given to me by Helena. I’m dressed as a reaper harvesting your “souls”. What: you think? Hehehe~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 102091,
			side = 0,
			nameColor = "#a9f548",
			dir = -1,
			say = "I happened to pass by {namecode.19}’s:rm, during that time—— no one came in or out of the entrance.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301171,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.19}",
			dir = 1,
			say = "You “passed by” for so long?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 102091,
			side = 0,
			nameColor = "#a9f548",
			dir = -1,
			say = "Even though “time is money”, I: tend to wander around from time to time.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301171,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.19}",
			dir = 1,
			say = "—— (Stares)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301051,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.6}",
			dir = 1,
			say = "It couldn’t have been Cleveland, she can’t hide so many pies by herself.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 102091,
			side = 0,
			nameColor = "#a9f548",
			dir = -1,
			say = "Is there a problem?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301180,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.20}",
			dir = 1,
			say = "A knight out at sea shouldn’t waste her precious time “wandering around” other people’s:rms.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301180,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.20}",
			dir = 1,
			say = "We’ve fully prepared for Halloween, well, your absence will not affect the result, Shiranui...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301171,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.19}",
			dir = -1,
			say = "Idiot {namecode.20}, what: you mean “the result is not affected”! All those sisters were highly anticipating my pumpkin pies. Drat, I’ll start baking another batch right now...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301180,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.20}",
			dir = 1,
			say = "Sunset is fast upon us, there’s not enough time. Everyone has already tried your pies before...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301171,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.19}",
			dir = -1,
			say = "But the missing ones are perfect this time!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301180,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.20}",
			dir = 1,
			say = "Halloween is not only about baking pies. Sister, they’re all waiting for you.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301171,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.19}",
			dir = -1,
			say = "I guess I’ll count this as bad luck… darn, I’ll definitely remember this!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301180,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.20}",
			dir = 1,
			say = "{namecode.87}, {namecode.6}, Cleveland and that idiot sister of mine… I hope you all have fun tonight…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 102091,
			side = 0,
			nameColor = "#a9f548",
			dir = -1,
			say = "Hehe, you too, you’ve worked hard!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 306051,
			actorName = "{namecode.87}",
			side = 2,
			nameColor = "#a9f548",
			dir = -1,
			say = "Cleveland, how come Helena did not come with you?",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 102091,
			nameColor = "#a9f548",
			side = 1,
			dir = 1,
			say = "Those Fletcher class sisters are already out asking for candy—— Even if their main target is not me, I’ve tried my best to avoid them...",
			paintingFadeOut = {
				time = 0.5,
				side = 0
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301051,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.6}",
			dir = 1,
			say = "So {namecode.19} has been resting for awhile at her:rm?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 102091,
			side = 1,
			nameColor = "#a9f548",
			dir = 1,
			say = "Hehehe...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			actorName = "？？？",
			nameColor = "#a9f548",
			dir = 1,
			blackBg = True,
			withoutPainting = True,
			say = "Excellent, we’re ready for Halloween! Yes… I:n’t think we missed anything.",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
