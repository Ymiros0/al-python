return {
	fadeOut = 1.5,
	mode = 2,
	id = "WEICENGHUNHE29",
	once = True,
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_banama_6",
			bgm = "hunhe-battle",
			say = "Canal Stronghold? - Out at sea?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_banama_6",
			say = "As the mist began to recede, Intrepid could be seen fiercely fighting back wave after wave of Siren forces.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_banama_6",
			say = "Although the enemies had increased their firepower, the carriers which had been suppressed by the heavy mist once again took control of the skies.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Bunker Hill",
			bgName = "bg_banama_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "What an absurd number of enemies. Have the Sirens already seen through our plans?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Essex",
			bgName = "bg_banama_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "It's possible. A full-scale assault launched from three dimensions, using a thick mist for cover...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Essex",
			bgName = "bg_banama_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "It's all to conceal the position of their flagship.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Intrepid",
			bgName = "bg_banama_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "Our advantages will only shrink at this rate... What should we:?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Essex",
			bgName = "bg_banama_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "There's nothing else to:. It's: or die at this point.",:
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Essex",
			bgName = "bg_banama_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "Come on, chin up. Intrepid means \"without fear,\" right?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Intrepid",
			bgName = "bg_banama_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "You're right...!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Essex",
			bgName = "bg_banama_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "Now that Enterprise isn't here, we have to: our best in her stead... No, we'll have to go above and beyond and take on her part.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Bunker Hill",
			bgName = "bg_banama_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "Not just for her sake, but also for Baltimore's, and for everyone counting on us in the Canal Stronghold.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Bunker Hill",
			bgName = "bg_banama_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "I'm glad you care about everyone, but this is your opportunity to break free from your past.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Intrepid",
			bgName = "bg_banama_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "Essex... Bunker Hill...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Bunker Hill",
			bgName = "bg_banama_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "Ah, but:n't go charging forward blindly. Enterprise would never rush in first before assessing the situation.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Intrepid",
			bgName = "bg_banama_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "As if I'd: something like that!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Intrepid",
			bgName = "bg_banama_6",
			nameColor = "#a9f548",
			dir = 1,
			say = "(Though I said all that, what should I: next...?)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			actor = 107110,
			nameColor = "#a9f548",
			dir = 1,
			bgmDelay = 1,
			say = "Oh, in case anyone hasn't noticed, the mist has gradually been thinning out.",
			bgm = "hunhe-eagle",
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			},
			effects = {
				{
					active = True,
					name = "miwu_01",
					center = True
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 107170,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "Of course. That's the only reason we've been able to get our planes off the ground.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			nameColor = "#a9f548",
			side = 2,
			actor = 107110,
			dir = 1,
			say = "Bunker Hill, were you able to get a good look at the mist density from the air? Pretty good view up there!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 107170,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "...I did, but why are you talking about that now...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			nameColor = "#a9f548",
			side = 2,
			actor = 107090,
			dir = 1,
			say = "If the mist was generated from a single point, the density should not change significantly even if the mist is thinning out.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			nameColor = "#a9f548",
			side = 2,
			actor = 107110,
			dir = 1,
			say = "That's right! Basically, this will let us know where to search!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 107090,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "Good going, Intrepid! Finally, we have a lead on where to head towards!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			nameColor = "#a9f548",
			side = 2,
			actor = 107110,
			dir = 1,
			say = "All right. Next up is figuring out how to approach the enemy flagship.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 107170,
			nameColor = "#a9f548",
			side = 0,
			hideOther = True,
			dir = 1,
			actorName = "Bunker Hill&Essex",
			say = "We:n't have time to worry about that, so let's get going!",
			subActors = {
				{
					actor = 107090,
					pos = {
						x = 1185
					}
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			actor = 107110,
			nameColor = "#a9f548",
			dir = 1,
			say = "Ehhhhh?!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 3,
			nameColor = "#a9f548",
			side = 2,
			actor = 107110,
			dir = 1,
			say = "...Haha, good grief, I guess that's just part of what makes us the Essex class.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			actor = 107110,
			nameColor = "#a9f548",
			dir = 1,
			say = "All right, my recon planes! Take us to the enemy core flagship!",
			soundeffect = "event./battle/plane",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
