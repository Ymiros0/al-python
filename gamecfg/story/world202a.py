return {
	id = "WORLD202A",
	mode = 2,
	once = True,
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			dir = 1,
			side = 2,
			bgm = "story-french1",
			say = "NA Ocean NE Sector - Royal Navy Operation Zone",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 203030,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "All right! Another wave of enemies wiped out!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 203030,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "Would you look at that, Contestant Kent smashes through the first Siren barrier with ease!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 202110,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "Just as rumoured, the enemies are sparser than usual. We've barely encountered any resistance even out this far.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			nameColor = "#a9f548",
			side = 2,
			actor = 205060,
			dir = 1,
			say = "Edinburgh, Kent, is everything going well on your end?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = False,
			side = 2,
			actor = 203030,
			nameColor = "#a9f548",
			dir = 1,
			say = "Piece of cake! If the Sirens are gonna roll over just like this, we'll be back home in no time!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			paintingNoise = True,
			nameColor = "#a9f548",
			side = 2,
			actor = 205060,
			dir = 1,
			say = "Things are going better than I expected. Hm? Where are Norfolk and Suffolk?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = False,
			nameColor = "#a9f548",
			side = 2,
			actor = 202110,
			dir = 1,
			say = "They're currently chasing:wn the routed Siren forces. Their formations are falling apart with even a mere touch......",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			nameColor = "#a9f548",
			side = 2,
			actor = 205060,
			dir = 1,
			say = "Mm, that’s what I'm hearing from the other teams as well. Apparently, the Sirens’ chain of command:esn't seem to be working properly.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			nameColor = "#a9f548",
			side = 2,
			actor = 205060,
			dir = 1,
			say = "Hopefully, this is giving the Eagle Union some extra breathing room as well.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			nameColor = "#a9f548",
			side = 2,
			actor = 205060,
			dir = 1,
			say = "Having that said... It’s unusual for Suffolk to take the initiative in chasing:wn the enemy.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = False,
			nameColor = "#a9f548",
			side = 2,
			actor = 202110,
			dir = 1,
			say = "“She muttered something like, “the faster we wipe out the enemy, the sooner I can go back to watching the seagulls...”...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = True,
			nameColor = "#a9f548",
			side = 2,
			actor = 205060,
			dir = 1,
			say = "......",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = False,
			side = 2,
			actor = 202110,
			nameColor = "#a9f548",
			dir = 1,
			say = "W-well, whatever her reasons might be... I'm just glad to see Suffolk giving it her best.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			paintingNoise = True,
			nameColor = "#a9f548",
			side = 2,
			actor = 205060,
			dir = 1,
			say = "Heh, indeed. Keep up the good work as well, Edinburgh.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = False,
			nameColor = "#a9f548",
			side = 2,
			actor = 202110,
			dir = 1,
			say = "Got that, Kent? We can't afford to fall behind either!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
