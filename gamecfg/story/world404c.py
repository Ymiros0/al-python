return {
	id = "WORLD404C",
	mode = 2,
	once = True,
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			actor = 307010,
			side = 2,
			nameColor = "#a9f548",
			say = "The smell of gunpowder in the air... I’ve been here before... Mid...way...?",
			dir = 1,
			bgm = "battle-boss-italy",
			effects = {
				{
					active = True,
					name = "miwu_01",
					center = True
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "Yes... Yes... It is! I remember, this is Operation AF! That Grey Ghost is here!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 307010,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "Damnit... Had it not been for you, the Eagle Union would have been powerless to stop us in the Pacific... It’s all your fault, Grey Ghost...!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "Because of you... I, Akagi... was ignobly defeated...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "Soryuu... Hiryuu... Kaga... Even the memories of my sister...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "It’s your fault... You took everything from me...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "I won’t forgive you... I’ll never forgive you... Grey Ghost! Damn you...!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 900296,
			actorShadow = True,
			nameColor = "#ff5c5c",
			side = 2,
			dir = 1,
			actorName = "Enterprise?",
			say = "You still remember me. I’m honored.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900297,
			actorShadow = True,
			nameColor = "#ff5c5c",
			side = 2,
			dir = 1,
			actorName = "Hornet?",
			say = "That was a damn exciting battle! Your leadership was top notch!~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900295,
			actorShadow = True,
			nameColor = "#ff5c5c",
			side = 2,
			dir = 1,
			actorName = "Yorktown?",
			say = "Indeed. You still want to take your revenge on us, right? Then come and fight us, forevermore.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900296,
			actorShadow = True,
			nameColor = "#ff5c5c",
			side = 2,
			dir = 1,
			actorName = "Enterprise?",
			say = "Even after you returned, you seemed to blame everything on the Five Minutes of Fate. But given five days, five years, five eternities... What could you possibly:?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900297,
			actorShadow = True,
			nameColor = "#ff5c5c",
			side = 2,
			dir = 1,
			actorName = "Hornet?",
			say = "You cannot escape fate... the fate engraved upon your hull. Fate is not so fragile, unlike that little amulet your sister gave you~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "Shut up! Shut up! You’ve all come before me to die... And I’ve been waiting to send you all to hell! Ahahahaha!!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 900296,
			actorShadow = True,
			nameColor = "#ff5c5c",
			side = 2,
			dir = 1,
			actorName = "Enterprise?",
			say = "Come, Akagi... This time, nobody will keep you from your revenge. Not the Sirens. Not the Sakura Empire. Not the Eagle Union. And not Amagi.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "I’ll run you through for underestimating me, Grey Ghost! My entire life, I’ve lived for this moment... The chance to sink you myself!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "The strong devour the weak... The fit survive... Such is the nature of... this...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			nameColor = "#a9f548",
			side = 2,
			dir = 1,
			say = "—————That voice... Shoukaku... Zuikaku...?",
			effects = {
				{
					active = True,
					name = "speed"
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
