return {
	id = "XIAFEI5",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Date with an Angel\n\n<size=45>5 The Angel and the Seagull</size>",
					1
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_700",
			hidePaintObj = True,
			say = "We continue to walk hand-in-hand, and before long, we arrive on the pier.",
			bgm = "story-richang-8",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_700",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 907010,
			say = "Let's head:wn and feel the sea breeze.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_700",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Sure.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_700",
			hidePaintObj = True,
			say = "A flock of seagulls takes flight from the surface, splashing water in all directions.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_700",
			hidePaintObj = True,
			say = "Upon seeing this, Joffre immediately spreads the wings on her back to shield me from the incoming seaspray.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_700",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "What an angel...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_700",
			hidePaintObj = True,
			say = "I couldn't help but gasp in admiration upon seeing the girl's wings spread beneath the bright sunshine.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_700",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 907010,
			say = "Indeed, that is my title.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 907010,
			side = 2,
			bgName = "star_level_bg_700",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "By the way, Commander, what: you think a canonical angel looks like anyway?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_700",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Umm... Someone with wings, dressed in a white gown of some sort, with an angel on their head?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_700",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Well, the way I see things, I:n't think there's much difference between you and a \"real\" angel.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_700",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 907010,
			say = "Hmm? May I ask how you came to that conclusion?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_700",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "I guess, uh, the \"Angel of the Iris\" is an embodiment of purity and justice, one who offers holy words to those in distress.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_700",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Isn't that exactly what you are?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 907010,
			side = 2,
			bgName = "star_level_bg_700",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "But, I...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_700",
			hidePaintObj = True,
			say = "I press my finger against Joffre's lips, trying to get her to let me finish.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_700",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "I know what you're about to say, but...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_700",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "You judge sin for the sake of all that is righteous and good in this world.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_700",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "You fight hard for the sake of a better tomorrow.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_700",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "That, in my eyes, is what truly makes you an angel.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_700",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "And that's the part of you that I respect and love.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_700",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 907010,
			say = "...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_700",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 907010,
			say = "Commander, what you just said... sounded a bit like a confession of love.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_700",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 907010,
			say = "But... I'm still glad I got to hear you say those words. Heehee.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_700",
			hidePaintObj = True,
			say = "With a gentle smile on her face, the angel approached me and pressed her head against my shoulder.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_700",
			hidePaintObj = True,
			say = "The flock of seagulls that flew away upon our arrival has now returned, nestling up against each other in pairs.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_700",
			hidePaintObj = True,
			say = "...a bit like me and Joffre in this moment.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
