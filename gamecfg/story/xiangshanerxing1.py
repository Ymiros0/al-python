return {
	id = "XIANGSHANERXING1",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			asideType = 3,
			blackbg = True,
			bgmDelay = 2,
			sequence = {
				{
					"Iris Orthodoxy - Holy Capital",
					1
				},
				{
					"Faction Conference Hall",
					2
				},
				{
					"One day before meeting",
					3
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_guild_blue",
			dir = 1,
			bgm = "story-italy",
			actor = 601050,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Alright! Chairs and desks are all in place!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_guild_blue",
			hidePaintObj = True,
			side = 2,
			actor = 601060,
			say = "Nice! You've even arranged all the furnishings so neatly.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 601050,
			say = "Yeah! I used a ruler to determine where to put them!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 601060,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Oh! No wonder it looks like they've been placed with great care!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 601050,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Got to live up to our reputation for great hospitality, after all!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 601050,
			say = "Wait, why are we chatting? We still have work to:, Cap'n!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 601050,
			say = "There's going to be a dinner party in the banquet hall after the meeting!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 601050,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Those tables aren't going to set themselves!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 601040,
			say = "Yes. In fact, the banquet hall is our top priority. We can't cut any corners with it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 601040,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Especially not with the ingredients. Time was running short, so Trieste went out to buy them herself.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 601040,
			say = "I hope she's able to able to find everything...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_guild_blue",
			hidePaintObj = True,
			side = 2,
			actor = 601060,
			say = "You know, that reminds me of a thing I've been wondering about...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_guild_blue",
			hidePaintObj = True,
			side = 2,
			actor = 601060,
			say = "This conference was our idea and we're the ones organizing it, so why are we hosting it in the Orthodoxy?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_guild_blue",
			hidePaintObj = True,
			side = 2,
			actor = 601060,
			say = "We wouldn't have to worry about getting the right ingredients if we were:ing this back home.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 601050,
			say = "Yeah, that IS weird! Maestrale, you're smart – what: you think?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 601040,
			say = "Umm... It's for diplomatic reasons, maybe? I'm not sure either, really.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = 605010,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actorName = "Vittorio Veneto",
			say = "Heehee. There are two good reasons why we're:ing it here rather than home, actually.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_guild_blue",
			portrait = 605010,
			dir = 1,
			actorName = "Vittorio Veneto",
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "The first is that the Orthodoxy lent us this locale. The second is that they're covering half of our expenditures, but still letting us have the honor of organizing it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = 605010,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actorName = "Vittorio Veneto",
			say = "All in all, this is quite a fine arrangement for us.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 601040,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "That explains it...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_guild_blue",
			hidePaintObj = True,
			side = 2,
			actor = 601060,
			say = "Veneto! You're back!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 605010,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Hello. I just finished revising our program, so I came here to check on you all.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 605010,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "You've:ne a great job so far. Everything looks neat and tidy and the decorations are very Sardegnian.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Communicator",
			bgName = "bg_guild_blue",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "BEEP BEEP!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_guild_blue",
			paintingNoise = True,
			dir = 1,
			actor = 605020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Everything's good here, Veneto. How is it on your end?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 605010,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I just received a message from Trieste. She said she was out buying groceries and got help from some Iris girls.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 605010,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Sounds to me like the dinner party tomorrow will go fine.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 605010,
			say = "Oh, and they're:ne arranging the tables and chairs around the venue.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 605010,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I can say with confidence that our guests will be able to treated extraordinarily well in Sardegnian style.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 8,
			side = 2,
			bgName = "bg_guild_blue",
			paintingNoise = True,
			dir = 1,
			actor = 605020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Excellent. I feared we'd have to start sending ingredients by plane. Glad that worst case scenario didn't come True.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_guild_blue",
			paintingNoise = True,
			dir = 1,
			actor = 605020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "What about your conference opening speech for tomorrow? Have you written it yet?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 605010,
			say = "Oh... My opening speech...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_guild_blue",
			dir = 1,
			fontsize = 20,
			actor = 605010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Right... I'm delivering a speech tomorrow...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 605010,
			say = "Umm, it still has a few details I need to fill in...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 605020,
			say = "Oh boy... That:esn't sound very reassuring.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 8,
			side = 2,
			bgName = "bg_guild_blue",
			paintingNoise = True,
			dir = 1,
			actor = 605020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "I'll come over. I won't be long!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
