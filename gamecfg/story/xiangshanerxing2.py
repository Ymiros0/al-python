return {
	id = "XIANGSHANERXING2",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_endingsong_14",
			hidePaintObj = True,
			say = "Somewhere, in an unknown location...",
			bgm = "bsm-2",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_endingsong_14",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "I knew it. There weren't just three Arbiters that came to this branch.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_endingsong_14",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Empress III... You're good at hiding.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_endingsong_14",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "In the end, my patience won out, though.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_endingsong_14",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Now that you've involved yourself in this branch, you can't slip back into the darkness anymore.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_endingsong_14",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "You have access to far more resources than I:, yes, but that's not something I can't compensate for.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_endingsong_14",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "My plan can finally continue now, thanks to you.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_14",
			paintingNoise = True,
			dir = 1,
			actor = 307010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "...Observer?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_endingsong_14",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "We meet again. How are you, Akagi? Have you been sleeping and eating well?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_14",
			paintingNoise = True,
			dir = 1,
			actor = 307010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "...What: you want? It's the middle of the night. If you just want to chat, I'm going to hang up.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_endingsong_14",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Hang up? That's no way to treat the person you want help from.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_endingsong_14",
			paintingNoise = True,
			dir = 1,
			actor = 307010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Help? I thought you were dead. I haven't heard from you for the longest time.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_endingsong_14",
			paintingNoise = True,
			dir = 1,
			actor = 307010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Besides, we're not friends. We're just acquaintances who exploit one another.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_endingsong_14",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "I know it's been a long while. I couldn't: anything about that.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_endingsong_14",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "But:n't talk to me about \"exploiting one another.\" I never needed you in particular. Anyone who could open a Singularity would've been fine by me.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_endingsong_14",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "You, on the other hand? You DID need me. Without access to my resources, who could've granted your wish?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_14",
			paintingNoise = True,
			dir = 1,
			actor = 307010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "......",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_endingsong_14",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Don't give me that look. I'm not here to waste your time. Have a look at this.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_endingsong_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 307010,
			say = "...What is this? Documents?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_endingsong_14",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "In your language, I guess you'd call it a... formation diagram.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_endingsong_14",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "You want to amplify the Watatsumi's power using the Sacred Sakura system I constructed,:n't you? This is the system's operation manual.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_endingsong_14",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "But perhaps you've changed your mind and you're going to pin your hopes that toy you and Musashi made.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_endingsong_14",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "You won't get anywhere with that thing. This, on the other hand, is tailor-made for your needs.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_endingsong_14",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "It's all yours. Go on and take it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_14",
			paintingNoise = True,
			dir = 1,
			actor = 307010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "...That I will.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_endingsong_14",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Incidentally, have you named your operation?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_endingsong_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 307010,
			say = "I've not even thought about it yet. What:es it matter?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_endingsong_14",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Well, it's going to need one, so how about \"Orochi\"?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_endingsong_14",
			paintingNoise = True,
			dir = 1,
			actor = 307010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "\"Orochi\"? Is this another one of your sick schemes?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_endingsong_14",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "No, not at all. It's just a name I came across in my database.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_endingsong_14",
			paintingNoise = True,
			dir = 1,
			actor = 307010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Orochi... The legendary serpent...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_14",
			paintingNoise = True,
			dir = 1,
			actor = 307010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Heehee... You know what? I'll go with that name.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_endingsong_14",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Great. Take the Origin Cube out of your toy and stick it into the core system. You know which tree that is, right?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_endingsong_14",
			paintingNoise = True,
			dir = 1,
			actor = 307010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Yes. I'm not stupid.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_endingsong_14",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 307010,
			say = "I'll... BZZZ... set up... BZZZ... and when it's... BZZZ...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_endingsong_14",
			paintingNoise = True,
			dir = 1,
			actor = 307010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "BZZZ... set off... BZZZ...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Communicator",
			bgName = "bg_endingsong_14",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "BZZZZZZ...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_endingsong_14",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Akagi?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_endingsong_14",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Hello? Akagi?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Communicator",
			bgName = "bg_endingsong_14",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "......",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_endingsong_14",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Strange...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_endingsong_14",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Is the signal being jammed?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_endingsong_14",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Isn't she using the communicator I gave her?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_endingsong_14",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Someone is jamming MY equipment in MY test site?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900232,
			side = 2,
			bgName = "bg_endingsong_14",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "No... THAT CANNOT BE!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		}
	}
}
