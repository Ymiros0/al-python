return {
	id = "XIANGSHANERXING6",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			asideType = 3,
			blackbg = True,
			bgmDelay = 2,
			sequence = {
				{
					"Iris Orthodoxy - Holy Capital",
					1
				},
				{
					"Northern Parliament Embassy",
					2
				},
				{
					"Three days after the conference",
					3
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_603",
			paintingNoise = True,
			bgm = "battle-deepecho",
			actor = 900218,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Pamiat', Avrora, thank you for your work at the conference.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			actor = 900218,
			nameColor = "#A9F548FF",
			paintingNoise = True,
			say = "I've heard about the results, and they're what I expected. Don't feel anxious about them.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 702020,
			say = "Meh, I had a feeling the talks wouldn't go anywhere.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 702010,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "They want to reform the Azur Lane through diplomacy... While it's a nice idea, it's just too sudden.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 702010,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "We can't set aside our ideological differences just like that, even with the Commander orchestrating things.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 702010,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Then there's Naval HQ's role in the matter...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 702010,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Even supposing all factions agree to a temporary pact, there's still no guarantee that True unity will bloom from it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 702010,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It will just be a repeat of the past, with us fragmenting once again.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 702010,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "If anything, we should thoroughly destroy the current order and–",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900218,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			paintingNoise = True,
			say = "Avrora, be careful what you say. Though we are on the Tribunal's encrypted channel, we: not know if someone is listening in.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 702010,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Oh...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 702020,
			say = "Ahem...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 702020,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "You know, it's really nice how the Orthodoxy paid part of this AND provided a venue!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			actor = 900218,
			nameColor = "#A9F548FF",
			paintingNoise = True,
			say = "It's convenient for us, yes, but \"nice\" isn't the word I'd use.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			actor = 900218,
			nameColor = "#A9F548FF",
			paintingNoise = True,
			say = "Since the Vichya Dominion and Iris Libre have reunited, the question of the Iris Orthodoxy's return to the Azur Lane has become front-and-center.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900218,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			paintingNoise = True,
			say = "If they're left on their own, they're likely to become prime targets for exploitation.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			actor = 900218,
			nameColor = "#A9F548FF",
			paintingNoise = True,
			say = "The Cardinal is surely aware of this, and so she \"nicely\" sprung into action to petition Comrade Commander for help.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			actor = 900218,
			nameColor = "#A9F548FF",
			paintingNoise = True,
			say = "Therefore, by establishing this conference, they'll at least be able to protect their homeland.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 702020,
			say = "What about Sardegna,:? They:n't really expect one conference to resolve everything,: they?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			actor = 900218,
			nameColor = "#A9F548FF",
			paintingNoise = True,
			say = "...Knowing them, maybe they:.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			actor = 900218,
			nameColor = "#A9F548FF",
			paintingNoise = True,
			say = "Perhaps they are also trying to regain the honor that the near-superimposition event cost them.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900218,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			paintingNoise = True,
			say = "I think we've spoken enough for now. Pamiat', Avrora, keep up your work at the conference.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			actor = 900218,
			nameColor = "#A9F548FF",
			paintingNoise = True,
			say = "Though I:n't expect anything to come out of this, we should let them have their moment of glory.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 702010,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Of course. Leave it to us.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			actor = 900218,
			nameColor = "#A9F548FF",
			paintingNoise = True,
			say = "Before I go, there's one thing I want you to bear in mind.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900218,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			paintingNoise = True,
			say = "Kronshtadt has informed me that the frequency of the term \"Antarctica\" has risen drastically in the Tribunal's transmissions.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			actor = 900218,
			nameColor = "#A9F548FF",
			paintingNoise = True,
			say = "One possibility is that one of the Orthodoxy's Basilicas lie there, but another is that Iron Blood has found something and told the Tribunal about it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			actor = 900218,
			nameColor = "#A9F548FF",
			paintingNoise = True,
			say = "I have already tasked a team to investigate further. In the meantime, I ask the two of you to keep an eye on the other factions at the conference.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 702010,
			side = 2,
			bgName = "star_level_bg_603",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Antarctica... Understood. We'll look into it as well.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
