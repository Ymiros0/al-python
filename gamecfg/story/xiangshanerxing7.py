return {
	id = "XIANGSHANERXING7",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_guild_red_n",
			hidePaintObj = True,
			say = "Pacific Ocean - Iron Blood Floating Fortress",
			bgm = "bsm-4",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_guild_red_n",
			paintingNoise = True,
			dir = 1,
			actor = 405050,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Antarctica again, you say?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_guild_red_n",
			hidePaintObj = True,
			side = 2,
			actor = 407030,
			say = "Yes. We've picked up Observer's trail.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 407030,
			side = 2,
			bgName = "bg_guild_red_n",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Our investigation led us to conclude her mainframe is somewhere in the South Atlantic, the Central Pacific, the Southern Ocean, or the Mediterranean.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_guild_red_n",
			hidePaintObj = True,
			side = 2,
			actor = 407030,
			say = "Having said that, it's been long since she last showed herself.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 407030,
			side = 2,
			bgName = "bg_guild_red_n",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "She has almost never directly engaged us in combat before, so our intel on her is severely lacking.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_guild_red_n",
			hidePaintObj = True,
			side = 2,
			actor = 407030,
			say = "But, judging by her disposition, she's most likely hiding in an unexplored location that connects to another ocean.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 407030,
			side = 2,
			bgName = "bg_guild_red_n",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "If we narrow our scope:wn using those constraints, we're left with the Southern Ocean.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			nameColor = "#A9F548FF",
			bgName = "bg_guild_red_n",
			hidePaintObj = True,
			side = 2,
			actor = 407030,
			say = "Also, if I may ask – why did you say, Antarctica \"again\"?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_guild_red_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 405050,
			say = "I've come across some odd intel about it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_guild_red_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 405050,
			say = "The Eagle Union and the Northern Parliament have been talking about Naval HQ:ing something there. The Tribunal's intel network picked up on it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 407030,
			side = 2,
			bgName = "bg_guild_red_n",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Are they searching for Observer's mainframe? No, wait, Naval HQ wouldn't be:ing that...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_guild_red_n",
			paintingNoise = True,
			dir = 1,
			actor = 405050,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Exactly. We can rule that out.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_guild_red_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 405050,
			say = "I asked some people stationed nearby to investigate, and not long after, an unusual thawing phenomenon was observed in an Antarctic mountain range.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_guild_red_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 405050,
			say = "It seems they've discovered \"extraordinary crystals.\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_guild_red_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 405050,
			say = "When they analyzed the sample they collected, they found out that these crystals seem to resonate with riggings.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_guild_red_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 405050,
			say = "They say it's capable of drastically improving a rigging's power output and combat performance.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_guild_red_n",
			paintingNoise = True,
			dir = 1,
			actor = 405050,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "After that... I lost contact with them.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			nameColor = "#A9F548FF",
			bgName = "bg_guild_red_n",
			hidePaintObj = True,
			side = 2,
			actor = 407030,
			say = "Wait, they stopped responding to you?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_guild_red_n",
			hidePaintObj = True,
			side = 2,
			actor = 407030,
			say = "I could see why the Eagle Union and the Northern Parliament would also want to get their hands on the crystals, but I can't see them attacking us for it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			nameColor = "#A9F548FF",
			bgName = "bg_guild_red_n",
			hidePaintObj = True,
			side = 2,
			actor = 407030,
			say = "There are no Reenactments going on in that region either, for that matter.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_guild_red_n",
			paintingNoise = True,
			dir = 1,
			actor = 405050,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "I agree. It:esn't make sense to sour our relationship at the same time as the Commander is calling for all factions to reform the Azur Lane.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_guild_red_n",
			paintingNoise = True,
			dir = 1,
			actor = 405050,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "That leaves only Naval HQ... or the Sirens. We should probably approach this as a conspiracy orchestrated by Observer.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_guild_red_n",
			hidePaintObj = True,
			side = 2,
			actor = 407030,
			say = "It can't be a coincidence. Everything lines up too well. I certainly wouldn't put this past the Sirens.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 407030,
			side = 2,
			bgName = "bg_guild_red_n",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "If this really is a trap to protect her mainframe,: our fleet is not the only one at risk. The Eagle Union's and the Northern Parliament's are, too.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_guild_red_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 405050,
			say = "Precisely. I want you to form a fleet and head to our Antarctic research base at once.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_guild_red_n",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 405050,
			say = "You'll have to improvise according to the situation.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_guild_red_n",
			hidePaintObj = True,
			side = 2,
			actor = 407030,
			say = "Got it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
