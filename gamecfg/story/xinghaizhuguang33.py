return {
	id = "XINGHAIZHUGUANG33",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			mode = 1,
			stopbgm = True,
			asideType = 3,
			blackBg = True,
			bgmDelay = 2,
			sequence = {
				{
					"Iris Orthodoxy - Holy Capital",
					1
				},
				{
					"Azur Lane Temporary Command Center",
					2
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			say = "I check my inbox and see a mail from the Sea of Stars. It's an accident report.",
			bgm = "theme-ucnf-image",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "\"Accident\" could mean anything... I hope it was nothing too bad.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "...During a data salvage operation, three teams were transported to an unknown simulation, where we encountered an organization called \"the Council.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "That emblem... I'm pretty sure I saw it in the Arcana Sanctum with Belorussiya.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "So that facility must have been one of theirs.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "...Their leader went by the codename \"Silver Fox\" and knew who Dr. Aoste was.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Interesting. I wonder if both Anzeel and Aoste were fellow members of this \"Council.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Since they clearly knew me when we spoke, I might also have been a member.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "...A lone Antiochus specimen named \"War Protocol Fortress.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "That's a new model of Siren – or should I say, Antiochus.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Wisdom Cube limiters, a way to control METAmorphosis...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "......",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Nope, no pieces are falling into place in my head.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "I suppose new information won't always trigger flashbacks like I've had before.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Come to think of it, the flashbacks have always happened when I'm in a Mirror Sea or in one of the Sea of Stars' simulations...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "They were even more frequent when I was hooked up to the Reality Lens...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Up next, the question the Devil raised... \"Where are the Sea of Stars' simulations?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "It is odd, and to the best of my knowledge, the Galactic Core didn't malfunction.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "The existence of this \"Council\" and their mysterious fleet... might be the key to everything.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Ideally, I'd like to speak with them face to face.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "If I ever:, I'm going to ask them how that beacon came to be infected with abnormal data.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Finally, there's the black tornado, or what they called a \"corrosion phenomenon.\" The Northern Parliament and Royal Navy have both reported similar sightings.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "...That beacon cannot be allowed to be restored. We'll halt the remaining simulations.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "It's time to shift our focus to developing Type II riggings. We've gathered more than enough testing data to: it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
