return {
	fadeOut = 1.5,
	mode = 2,
	id = "XINNIAN1",
	once = True,
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			mode = 1,
			sequence = {
				{
					"Lunar New Year Celebration\n\n<size=45>Chapter 1 - Preparations for the Wonderful New Year!</size>"
				}
			}
		},
		{
			actor = 303120,
			nameColor = "#a9f548",
			side = 2,
			actorName = "{namecode.67}",
			dir = 1,
			say = "Phew... I'm finally back...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 312010,
			side = 1,
			nameColor = "#a9f548",
			actorName = "{namecode.98}",
			dir = 1,
			say = "Welcome back, nya! You've got a big catch today, nya...",
			paintingFadeOut = {
				time = 0.5,
				side = 0
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 303120,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.67}",
			dir = 1,
			say = "Big catch, huh? If memory serves me properly, we were supposed to: some New Year's shopping... Right, Akashi?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 312010,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.98}",
			dir = 1,
			say = "Nya! ... I stumbled upon some nice things and, uh, well... No harm, no foul, right? Consider it a \"business opportunity,\" nya!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 312010,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.98}",
			dir = 1,
			say = "And besides, I'm sure Commander and the others will appreciate this stuff, nya!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 303120,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.67}",
			dir = 1,
			say = "Hey Akashi, where: you think you're running off to? Put those parts:wn and come help out.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 312010,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.98}",
			dir = 1,
			say = "Nya... Akashi wanted to go sleep in Commander's lap, nya~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307030,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.93}",
			dir = -1,
			say = "Hm, Atago and Akashi, you're back? How did shopping go?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 312010,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.98}",
			dir = 1,
			say = "Nya! It's Souryuu, nya!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 303120,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.67}",
			dir = 1,
			say = "Indeed. We're:ne with the New Year's shopping, but a certain cat decided to buy a lot of stuff that wasn't on the list. Hehe...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307030,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.93}",
			dir = 1,
			say = "Great. After you're:ne organizing, we need to get ready for Spring Cleaning. We'll have to call everyone to help.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 303120,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.67}",
			dir = 1,
			say = "Souryuu, you sure are something else. As expected of Commander's right hand.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307030,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.93}",
			dir = 1,
			say = "I-I just: what needs to be:ne, that is all...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 312010,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.98}",
			dir = 1,
			say = "Akashi is Commander's other right hand, nya!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 303120,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.67}",
			dir = 1,
			say = "Two right hands? Is Commander a Stand user...? Well, Akashi, if you're going to be a \"right hand,\" you'll have to act like one and help me move all this stuff~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 312010,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.98}",
			dir = 1,
			say = "Fine, nya...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307030,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.93}",
			dir = 1,
			say = "Ah, shall I lend a hand as well?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
