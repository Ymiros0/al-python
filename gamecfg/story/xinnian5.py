return {
	fadeOut = 1.5,
	mode = 2,
	id = "XINNIAN5",
	once = True,
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			mode = 1,
			sequence = {
				{
					"Lunar New Year Celebration\n\n<size=45>Chapter 5 - New Year's Cards</size>"
				}
			}
		},
		{
			actor = 301330,
			nameColor = "#a9f548",
			side = 2,
			actorName = "{namecode.34}",
			dir = 1,
			say = "Big sister Souryuu, what are you:ing?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307030,
			side = 1,
			nameColor = "#a9f548",
			actorName = "{namecode.93}",
			dir = 1,
			say = "Ah, Kisaragi. I'm writing New Year's cards for everyone.",
			paintingFadeOut = {
				time = 0.5,
				side = 0
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301330,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.34}",
			dir = 1,
			say = "New Year's cards?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307030,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.93}",
			dir = 1,
			say = "They're for sending regards to your friends. You can also write a bit about what's going on in life, that kind of stuff.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301330,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.34}",
			dir = 1,
			say = "Can I... try writing one?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307030,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.93}",
			dir = 1,
			say = "Of course. Who would you like to send it to?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301330,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.34}",
			dir = 1,
			say = "Um... uh... Commander?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301330,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.34}",
			dir = 1,
			say = "I... want to wish Commander a happy New Year...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307030,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.93}",
			dir = 1,
			say = "Oh? That's a good idea. I'm sure Commander would be very happy to get a card from you.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301140,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.16}",
			dir = 1,
			say = "Eh? Why would Commander be happy?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307030,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.93}",
			dir = 1,
			say = "Well... Hey! Yuudachi, no running in the hallway! ... Anyway, children who send cards to adults might just receive a nice reward.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301140,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.16}",
			dir = 1,
			say = "Say what? Then, I wanna write one too!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307030,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.93}",
			dir = 1,
			say = "Do you really? Then, sit:wn right here. Kisaragi, you too. All right, take a postcard and a pen.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301330,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.34}",
			dir = 1,
			say = "Okay! This looks fun!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301140,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.16}",
			dir = 1,
			say = "Souryuu, why is there a picture of a pig on this card?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307030,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.93}",
			dir = 1,
			say = "These cards are usually printed with the corresponding zodiac animal. It's now the Year of the Pig. Is something the matter, Yuudachi?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301140,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.16}",
			dir = 1,
			say = "Just looking at it is making me hungry...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307030,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.93}",
			dir = 1,
			say = "Haha... In short, there are no rules on what you can write on your card, especially for children. Just writing what's on your mind is good enough.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301140,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.16}",
			dir = 1,
			say = "In that case, I'll write... \"Commander, I wanna eat lots of meat this year as well.\" ... And:ne!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307030,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.93}",
			dir = 1,
			say = "... ... At least write \"Happy New Year,\" okay?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301140,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.16}",
			dir = 1,
			say = "Ohh! ... But uh, how: I spell that again?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301330,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.34}",
			dir = 1,
			say = "Um... I also...:n't know how...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307030,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.93}",
			dir = 1,
			say = "I suppose we'll have to start from the basics:. Here, I'll write it out first,: you can try for yourself.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 303030,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.55}",
			dir = 1,
			say = "Hrm? Souryuu, Yuudachi, and Kisaragi? What are you three:ing here?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307030,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.93}",
			dir = 1,
			say = "I'm teaching them how to write a New Year's card to send to Commander.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 303030,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.55}",
			dir = 1,
			say = "Eh? Um... I see... In that case, I'll be slowly making my way toward the egress~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			say = "One hour later...",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307030,
			nameColor = "#a9f548",
			side = 2,
			actorName = "{namecode.93}",
			dir = 1,
			say = "Aoba, is this your:ing?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 303030,
			side = 0,
			nameColor = "#a9f548",
			actorName = "{namecode.55}",
			dir = 1,
			say = "What could you be talking about?",
			paintingFadeOut = {
				time = 0.5,
				side = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307030,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.93}",
			dir = 1,
			say = "Who else would call all the destroyers to come write cards?",:
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 303030,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.55}",
			dir = 1,
			say = "Oh come on, I merely asked everyone if they had anything they'd like to thank the commander for...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301320,
			nameColor = "#a9f548",
			side = 2,
			actorName = "{namecode.33}",
			dir = 1,
			say = "Ms. Souryuu, I drew a flower, see?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301330,
			nameColor = "#a9f548",
			side = 2,
			actorName = "{namecode.34}",
			dir = 1,
			say = "Souryuu... Um, teacher... Did I: it right?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301140,
			nameColor = "#a9f548",
			side = 2,
			actorName = "{namecode.16}",
			dir = 1,
			say = "Ah! This card has a porkchop on it! I wanna use this card!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301300,
			nameColor = "#a9f548",
			side = 2,
			actorName = "{namecode.124}",
			dir = 1,
			say = "Oh ho ho~ I: believe I shall write some most \"stimulating\" things~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301560,
			nameColor = "#a9f548",
			side = 2,
			actorName = "{namecode.125}",
			dir = 1,
			say = "Uuu.... I made a mistake again...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301170,
			nameColor = "#a9f548",
			side = 2,
			actorName = "{namecode.19}",
			dir = 1,
			say = "Souryuu, what: you think of my elegant writing?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 303030,
			nameColor = "#a9f548",
			side = 0,
			hideOther = True,
			dir = 1,
			actorName = "{namecode.55}",
			say = "Eheh, it's decided:! The \"Super Popularity Contest - Ms. Souryuu's Special Classroom, End of Year Edition,\" will begin tomorrow!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307030,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.93}",
			dir = 1,
			say = "Why is Kamikaze even here... Ah jeez, I suppose this isn't so bad...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307030,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.93}",
			dir = 1,
			say = "All right, if you have any questions, please come up one at a time. First, I'll write \"Happy New Year\" on the board, so make sure to watch me carefully!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
