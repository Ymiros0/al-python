return {
	fadeOut = 1.5,
	mode = 2,
	id = "XINNIAN6",
	once = True,
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			mode = 1,
			sequence = {
				{
					"Lunar New Year Celebration\n\n<size=45>Chapter 6 - New Year's Banquet?</size>"
				}
			}
		},
		{
			actor = 307031,
			nameColor = "#a9f548",
			side = 2,
			actorName = "{namecode.93}",
			dir = 1,
			say = "Phew... All right, the New Year Soba is coming out nicely...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301331,
			side = 0,
			nameColor = "#a9f548",
			actorName = "{namecode.34}",
			dir = 1,
			say = "It smells really good...",
			paintingFadeOut = {
				time = 0.5,
				side = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307031,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.93}",
			dir = 1,
			say = "Ah, watch out, Yuudachi! Don't fall into the pot!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301141,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.16}",
			dir = 1,
			say = "This one seems yummy too! Gimme seconds!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307031,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.93}",
			dir = 1,
			say = "Calm:wn, there's more than enough left.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 303122,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.67}",
			dir = 1,
			say = "Souryuu, you really are quite talented~ Oh my, is Kaga here too?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307022,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.92}",
			dir = 1,
			say = "Being able to enjoy a moment of peace every now and: isn't so bad.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 303122,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.67}",
			dir = 1,
			say = "That's not something I hear from you every day, Kaga. Oh, by the way, where's Akagi?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307022,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.92}",
			dir = 1,
			say = "She spouted something like \"I need to pamper Commander, who is always so hard working,\" and: ran off again.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 303122,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.67}",
			dir = 1,
			say = "Oh my... Looks like Commander will have no shortage of troubles this New Year as well...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307022,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.92}",
			dir = 1,
			say = "I suppose this is \"the weight of love.\" Well, since it's the New Year, we might as well let Akagi cut loose a bit.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 303122,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.67}",
			dir = 1,
			say = "Speaking of \"cutting loose,\" I think these two have gone a bit overboard.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403032,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "Oh, sakura's liquor... is truly ambrosia... *hic*",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 101172,
			nameColor = "#a9f548",
			side = 0,
			dir = 1,
			say = "Laffey... wants another glass too...",
			paintingFadeOut = {
				time = 0.5,
				side = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307031,
			nameColor = "#a9f548",
			side = 2,
			actorName = "{namecode.93}",
			dir = 1,
			say = "I'll turn a blind eye on Eugen... But why is that girl from the Eagle Union here?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 312011,
			side = 1,
			nameColor = "#a9f548",
			actorName = "{namecode.98}",
			dir = 1,
			say = "I ran into her at the store, nya. She seemed quite interested in our New Year's customs, so I brought her along, nya.",
			paintingFadeOut = {
				time = 0.5,
				side = 0
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307031,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.93}",
			dir = 1,
			say = "Ah, what a waste. She's wearing such a cute outfit... but give her some drinks and she turns into a puddle of mush.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307022,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.92}",
			dir = 1,
			say = "A New Year's drinking party, huh? Haha... I haven't gotten fired up in a while.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 303122,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.67}",
			dir = 1,
			say = "Oh my, I can already see what happens next...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301331,
			nameColor = "#a9f548",
			side = 1,
			actorName = "{namecode.34}",
			dir = 1,
			say = "Li... quor? Is that edible...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 312011,
			nameColor = "#a9f548",
			side = 0,
			actorName = "{namecode.98}",
			dir = 1,
			say = "That would be the name of a miserable end for adults who have cast away too much. It's not something a good kid like you needs to know about, nya.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
