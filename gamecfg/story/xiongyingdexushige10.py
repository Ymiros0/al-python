return {
	fadeOut = 1.5,
	mode = 2,
	id = "XIONGYINGDEXUSHIGE10",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			stopbgm = True,
			say = "Over in the security control room, Veneto was staring at the footage from the surveillance cameras and pondering the situation.",
			bgm = "story-roma-outside",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603050,
			say = "I'm afraid I have bad news, Lady Veneto.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603050,
			say = "We reviewed the drone and camera footage from all around the venue, but were unable to locate Lady Roma.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 605010,
			say = "But nobody has seen her leave the Colosseum, correct? That means she must still be somewhere inside.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 602020,
			say = "Inside the Colosseum? Well, that was the first place we looked...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 602020,
			say = "She didn't show up on any of our cameras. That can only mean one of two things – either Roma isn't in the Colosseum, or the cameras aren't working properly.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 605010,
			say = "Not working properly... Perhaps that is what Shinano meant by \"touched by a malicious presence.\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 605010,
			say = "Supposing they've been broken—or worse yet, hacked into—we're not going to find Roma just by looking through security footage.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 605010,
			say = "That leaves us only one option. to temporarily put the Colosseum on lockdown.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 602020,
			say = "Lockdown?! And put the World Expo on hold?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 605010,
			say = "No, only the Colosseum. It will act as a preventative measure until we've completed our investigation.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603050,
			say = "Understood. Then I shall evacuate the spectators within the Colosseum... under the pretense of a mechanical failure.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 605010,
			say = "...Wait, let me think things through.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 605010,
			say = "(Marco Polo proposed this expo as the best chance we'll get to spread Sardegna's splendor to the world...)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 605010,
			say = "(The other factions will take to wild speculation if we put our pavilion under lockdown, and our public image will plummet as a result...)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 605010,
			say = "(On the other hand, we cannot resolve the situation without closing the Colosseum to investigate. At the same time, abandoning one of our own to save face is an unforgivably disgraceful notion.)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 605010,
			say = "(Furthermore, there's no guarantee that Roma is still inside the Colosseum. What if we close the pavilion and still come up empty-handed? Then...)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 605010,
			say = "(Argh! Why: you hesitate, Veneto?! Things will only get worse if you keep waiting around instead of taking action now!)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 605010,
			say = "(That's right, we can't assume that only the Colosseum has been compromised. More guests may go missing from their pavilions while our focus is solely on the Colosseum...)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 605010,
			say = "(Littorio... If you were here, what would you:?)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 602020,
			say = "Veneto hasn't said a thing for a while now. Any ideas why?",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603050,
			say = "I can't say for sure... But the decision to close the Colosseum is not an easy one.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603050,
			say = "The revival of the World Expo was entirely at Sardegna's suggestion. In other words, our reputation is on the line for the entire world to see.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603050,
			say = "Should an incident break out on the first day, the other factions would surely call question to our competence, regardless of the circumstances...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 602020,
			say = "So what: we::? Bury our heads in the sand and pretend everything's fine?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603050,
			say = "Well, um... I believe ensuring the safety of our guests is our top priority.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603050,
			say = "That said, I think we should only close the Colosseum if it's our last resort.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603050,
			say = "There must be other options. Like if there's a chance the security system has been tampered with, we could ask its designer or a security expert to have a look.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 605010,
			say = "Yes! That's it, Bolzano!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603050,
			say = "P-pardon? Did I disrupt your train of thought...?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 605010,
			say = "Quite the opposite – what you just said is a great idea. If anyone can tell us if the security system has been compromised, it's someone with intimate knowledge of it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 605010,
			say = "I should've thought of it sooner, myself... Say, why:n't we bring this up with Torricelli?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			actor = 608010,
			nameColor = "#A9F548FF",
			paintingNoise = True,
			say = "Oh, I see... That's not good at all... Hehehehehe...",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			actor = 608010,
			nameColor = "#A9F548FF",
			paintingNoise = True,
			say = "There's only two things that could override the cameras – a full-on hack, or the properties of a Mirror Sea... But I only: maintenance on the system, so take that with a grain of salt...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			actor = 608010,
			nameColor = "#A9F548FF",
			paintingNoise = True,
			say = "I:ubt it's a Mirror Sea... It'd need to be tiny to only affect the Colosseum... In the case of a hack, I can't tell you much from where we are now...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 605010,
			say = "Is there no way we can confirm one thing or the other?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			actor = 608010,
			nameColor = "#A9F548FF",
			paintingNoise = True,
			say = "I mean, there IS, but it's probably too complicated for you all...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			actor = 608010,
			nameColor = "#A9F548FF",
			paintingNoise = True,
			say = "I could head over there and: it if you want, though... Hehe, hehehehe...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603040,
			say = "May I have a word? Lady Veneto, the drones suddenly were issued a new patrol route. Did you: something in the control room?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 605010,
			say = "A new patrol route? No, we haven't:ne anything.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 602020,
			say = "Hey, look! The drones are moving en-masse! Their cameras are all over the place!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603050,
			say = "Someone must have seized control of them! How could this have happened?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 605010,
			say = "Is it possible to shut:wn the system at a moment's notice?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			actor = 608010,
			nameColor = "#A9F548FF",
			paintingNoise = True,
			say = "Hehe, hehehe... You'd have to ask da Vinci...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 605010,
			say = "I see... She built the foundation of the security system, didn't she?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			actor = 608010,
			nameColor = "#A9F548FF",
			paintingNoise = True,
			say = "Sure did. Even designed the control system for the drones... If anyone could shut it:wn, it's her...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			actor = 608010,
			nameColor = "#A9F548FF",
			paintingNoise = True,
			say = "Hehehehe... By the way, she also slipped a backdoor into the security system, which she might be using right now to take the investigation into her own hands...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			actor = 608010,
			nameColor = "#A9F548FF",
			paintingNoise = True,
			say = "And if this is the Sirens':ing instead, asking her for advice is the quickest way to a solution... Hehehe...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			actor = 608010,
			nameColor = "#A9F548FF",
			paintingNoise = True,
			say = "Anyway, we should probably find her as soon as possible... She'd have a way better idea of what to: than me...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			paintingNoise = True,
			blackBg = True,
			actor = 608010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Oh, and:n't let her know I told you about the backdoor, alright? Hehe, hehehe...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
