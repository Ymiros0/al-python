return {
	fadeOut = 1.5,
	mode = 2,
	id = "XIONGYINGDEXUSHIGE12",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			stopbgm = True,
			say = "Sakura Empire Pavilion - Woods",
			bgm = "story-roma-outside",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 608020,
			say = "Heheh! I've updated all the drones' patrol routes and scan parameters! This is what I call Hyper-Focused Super-Searcher Mode!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 608020,
			say = "Now we just wait for them to find the abnormality and send in a status report!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Photo op! Here we see Leonardo da Vinci, the to-be savior of Sardegna, the guiding star of the patrol team, taking matters into her own hands!",
			soundeffect = "event./ui/kuaimen",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601090,
			say = "You:n't think they'll use that photo as proof that she hacked the security system?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "It'll be fine! They'll hail her as a hero when they learn she did it to resolve this whole ordeal!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 608020,
			say = "Aw, dangit! Hide, everyone! Trieste is coming towards us, and fast!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Whaaat?! She's on to us already?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Let's head into the woods! She'll brand us as some band of conspirators if she catches us now!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601090,
			say = "*sigh*... You should honestly rip that photo apart now while you have the chance.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Maybe later! But right now, we've gotta split!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "*pant pant*... Did we lose her?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 608020,
			say = "No idea. The drone cameras can't see through these thick woods.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 608020,
			say = "But realistically, there's no way she'd chase us THIS far.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actorName = "???",
			nameColor = "#A9F548FF",
			bgName = "bg_roma_2",
			hidePaintObj = True,
			side = 2,
			say = "Wrong. I'd pursue you to the ends of the earth after what you've:ne.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Is that... It's her! H-h-how the heck did you manage to find us?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603040,
			say = "Using the drone that went in the same direction as me. You were using it to keep an eye on me, weren't you?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 608020,
			say = "Oops... I was so focused on running that I forgot to change that thing's flight route...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601090,
			say = "We're sorry, Trieste... But believe me, we weren't trying to cause any trouble.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603040,
			say = "I know you weren't. I came looking for you by orders from Veneto.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603040,
			say = "You're going to help us resolve this situation.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Wait, Veneto knows we took over the system?! Then she should've just asked us for help directly!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Because like, we spent all this time running and hiding, and for what?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603040,
			say = "For what? You of all people should know, Alfredo.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603040,
			say = "Oh, and we also know you trailed Bolzano and Garibaldi. You set yourselves up to be caught sooner or later.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603040,
			say = "Now that we're on the same page, would you return command of the drones to the control room already? The other factions will start getting suspicious soon.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603040,
			say = "Be quick,: follow me to the Colosseum. Veneto is waiting for you there.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			blackBg = True,
			actor = 608020,
			say = "Roger-roger!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
