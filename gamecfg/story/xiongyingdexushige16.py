return {
	fadeOut = 1.5,
	mode = 2,
	id = "XIONGYINGDEXUSHIGE16",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_6",
			nameColor = "#A9F548FF",
			bgm = "story-roma-outside",
			actor = 601080,
			stopbgm = True,
			hidePaintObj = True,
			say = "What'cha writing in your notepad, Ema? Mind if I have a look?",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601090,
			say = "Well, um... Fine, it's not like you'll take no for an answer anyway...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Hehe, appreciate it! ...Umm, what's all this?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Oh, I see now! These cute little symbols form a map!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "The speech bubbles mark the spot where we had a chat with those girls...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Crosses denote places we've investigated... And exclamation marks denote points of interest. Am I on the money?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601090,
			say = "Yeah. Which means this next hall is the only place left to look, save for the underground warehouse.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601090,
			say = "So far we haven't found anything unusual...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 608020,
			say = "That's a good sign. It means that there aren't any abnormalities on the ground level of the Colosseum.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 608020,
			say = "That, in turn, means it can stay open for business while we investigate the underground areas with peace of mind.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 608020,
			say = "We'll want to be thorough, because we: know Roma went:wn there to: a check of the exhibition items stored there.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 608020,
			say = "It would make sense if she went there right after the opening ceremony, which is when she went missing.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "You're right it would, Teach! But that raises the question... How: we get there in the first place?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 608020,
			say = "Good question. It's the hardest place to get inside in the whole Colosseum, since it's where the items not on display are stored.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 608020,
			say = "The entrance is behind a hidden:or, and finding it is not going to be... Wait, what?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_roma_6",
			hidePaintObj = True,
			side = 2,
			say = "At the end of a corridor between two halls, the party found the passage to the underground warehouse. The:or had been left open for anyone to see.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Oh, silly Roma~ She forgot to close the:or behind her!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 608020,
			say = "She would NOT forget! And even if she did, the:or would sound an alarm when left open for too long!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 608020,
			say = "The fact there's no alarm means the security system is probably offline:wn there. I have a bad feeling about this...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 101490,
			say = "Hey, fellow detectives! Are you hunting the green ghost too?",
			effects = {
				{
					active = True,
					name = "speed"
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			actorPosition = {
				x = 2000,
				y = 0
			},
			action = {
				{
					type = "move",
					y = 0,
					delay = 0.2,
					dur = 0.5,
					x = -2000
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "What green ghost? And who are you?",
			effects = {
				{
					active = False,
					name = "speed"
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 101490,
			say = "I'm Bristol, Allen M. Sumner-class destroyer from the Eagle Union! I'm investigating rumors about a ghost that's appeared at the World Expo!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 101490,
			say = "Strange that it's nowhere to be seen... Data 857 said it came from here...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 101490,
			say = "Oh well, I'll go on ahead! Good luck to you all, and take care not to lose your sanity!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 101490,
			say = "Just you wait, green ghost! I'm coming for you!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "move",
					y = 0,
					delay = 0.5,
					dur = 1,
					x = -2000
				}
			}
		},
		{
			expression = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_roma_6",
			hidePaintObj = True,
			side = 2,
			say = "Holding a lantern in one hand and a strange robot in the other, the self-proclaimed Eagle Union detective departed for the exit, going where the Sardegnian team had already investigated before.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "She didn't see the entrance to the warehouse, did she? Also, why the heck is someone besides us snooping around here?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Carrying a lantern in broad daylight, at that! Is that super suspicious or what?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 608020,
			say = "Ditto for that little robot of hers! Very, very suspicious!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Let's not kid ourselves. We both know you just want to take it apart to study!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 608020,
			say = "Oof... Anyway! She's probably a random lost guest. Seeing as she headed for the exit, let's just leave her be.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601090,
			say = "What was that green ghost she mentioned, though?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Oh yeah, that's another one for the suspicious list!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "A green ghost... Hang on,:esn't that kind of sound like the emerald eagle...?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "That makes two oddities spotted around the Colosseum, huh...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Hey, that's it! Two eyewitnesses, two sightings of green things, both around the Colosseum! It's all connected!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601090,
			say = "You're right. It's not like two separate eyewitnesses would lie about that.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 608020,
			say = "That Bristol girl said it \"came from here\" too...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_roma_6",
			hidePaintObj = True,
			side = 2,
			say = "Just:, da Vinci and Alfredo looked at each other. Their eyes lit up as they were both hit by similar realizations.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 0,
			bgName = "bg_roma_6",
			actor = 601080,
			dir = 1,
			nameColor = "#A9F548FF",
			hideOther = True,
			actorName = "Alfredo Oriani & Leonardo da Vinci",
			hidePaintObj = True,
			say = "- Hot scoop, here I coooome! - Increased budget, here I coooome!",
			effects = {
				{
					active = True,
					name = "speed"
				}
			},
			subActors = {
				{
					actor = 608020,
					pos = {
						x = 1185
					}
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601090,
			say = "W-wait, come back here...",
			effects = {
				{
					active = False,
					name = "speed"
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_roma_6",
			hidePaintObj = True,
			side = 2,
			say = "The person on the other end of the communicator in Emanuele's hand let out a massive sigh.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			blackBg = True,
			actor = 601090,
			say = "F-forgive me, Veneto... I'll t-try to catch up to them...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
