return {
	fadeOut = 1.5,
	mode = 2,
	id = "XIONGYINGDEXUSHIGE18",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_1100",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			stopbgm = True,
			say = "Alfredo, da Vinci, and Emanuele continued:wn the staircase to the underground warehouse.",
			bgm = "xinnong-3",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_1100",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Nice one, Teach! Bringing a lantern was great foresight on your part!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_1100",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 608020,
			say = "Heheh! It's far from the only thing I brought! Check out my Dark Place Lighter-Upper – perfect for times like these!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_1100",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601090,
			say = "It's handy to have, considering you nearly tripped over yourselves in the darkness right after we closed the:or...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_1100",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "You still taking notes there, Ema?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_1100",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601090,
			say = "Mhm. I'm logging every leg of our journey. Da Recco's going to want to read it when I get back.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_1100",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601090,
			say = "Plus this is an easy way to track our progress and review the map.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_1100",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601090,
			say = "...Oh. We can't go past this point.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_1100",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Huh? What: you mean we can't?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_1100",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601090,
			say = "Uh, because the Colosseum's boundary lies just up ahead? There shouldn't be anywhere for us TO go.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_1100",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Huh? Well, if my eyes:n't deceive me, this corridor:esn't seem to end any time soon...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_1100",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601090,
			say = "How? That:esn't make sense...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_1100",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601090,
			say = "We've been moving in a straight line for so long we should be in the wall, based on the blueprints. And yet there's no end in sight, never mind any sign of the storage...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_1100",
			hidePaintObj = True,
			side = 2,
			say = "BZZZT... BZZZZZT...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_1100",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "The light's fading... Teach, I think your lamp's broken!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_1100",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 608020,
			say = "Nonsense! My inventions: not break! It CAN'T break, not now when–",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_1100",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601090,
			say = "Eek! It went out completely!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_1100",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 608020,
			say = "AAAAAAAHHHHHH!",
			effects = {
				{
					active = True,
					name = "speed"
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.1,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_1100",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 608020,
			say = "No, calm:wn, me... Darn you, stupid lamp! I'll fix you good!",
			effects = {
				{
					active = False,
					name = "speed"
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 1
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_1100",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Something's not right here... I think this scoop has gone paranormal...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 0,
					delay = 0,
					dur = 0.2,
					x = 30,
					number = 2
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_1100",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "We're in way over our heads! Let's turn back!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_1100",
			hidePaintObj = True,
			side = 2,
			say = "Alfredo looked back at the way the party came, but there was only endless darkness as far as the eye could see.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_1100",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Eep! H-hey, I think we should call Veneto right now...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_1100",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 608020,
			say = "Already tried that. No response. Something's obviously blocking our signal.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_1100",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 608020,
			say = "My guess is that this is the exact point Roma went missing after she went underground.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_1100",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Teach... It's almost scary how quickly you regained your composure.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_1100",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 608020,
			say = "You need to calm:wn yourself. Panicking won't help us.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_1100",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 608020,
			say = "Think about this logically. We suddenly lost signal while walking forward, and the way we came from just disappeared.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_1100",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 608020,
			say = "I'll be honest, I:n't believe in ghost stories. What we're dealing with here is probably a micro-sized Mirror Sea, like Torricelli speculated.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_1100",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			blackBg = True,
			actor = 608020,
			say = "And we know how to deal with those. Locate the central control unit and destroy it, or look for another way out. There's our plan! Let's pull ourselves together and go get 'em, girls!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
