return {
	fadeOut = 1.5,
	mode = 2,
	id = "XIONGYINGDEXUSHIGE22",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			stopbgm = True,
			say = "Somewhere in the Expo Venue...",
			bgm = "story-roma-outside",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 301290,
			say = "They've finally let us in, Suruga!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 301290,
			say = "Phew~ It's so nice to get to stretch my legs a bit!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 301290,
			say = "Since Shinano said we're free to enjoy ourselves, I'm heading straight for the exhibit to have a look ar– Ouch!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 305140,
			say = "We're here to gather intel, not mess around.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 305140,
			say = "Shinano can't stay long at the expo, remember? That's why she tasked us with keeping tabs on all the other factions.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 305140,
			say = "With good reason, too. The Sardegnians have been on edge for a while now. Shinano was right in saying something strange is going on.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 305140,
			say = "So let's start with a tour around the pavilions. Keep your eyes and ears open, got it?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 301290,
			say = "Got it! Forgive me for getting overexcited for a moment. I'll head that-a-way and have a look, if that's okay!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 301290,
			say = "Onwards to the next exhibit area! Full speed ahe– Owow! Hey! Was it really necessary to bonk me twice?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "move",
					y = 0,
					delay = 0.2,
					dur = 0.5,
					x = -2000
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 305140,
			say = "Don't run off yet, scatterbrain! We need a plan first!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 301290,
			say = "Oh! Off-topic, but look! That guest seems rather anxious!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 301290,
			say = "Almost like she's lost, wouldn't you say? We should help her!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 305140,
			say = "(Who? That Iron Blood girl?)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 305140,
			say = "(Yikes, she looks like a bomb waiting to go off... Better steer clear of her, lest she vents her anger on us.)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 305140,
			say = "(Intel-gathering is supposed to be:ne on the low. Talking to her and getting roped into something is the LAST thing I need!)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 305140,
			say = "Ahem! Yes, she looks like she could use some directions, but how are we supposed to help?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 305140,
			say = "We:n't know our way around here either. There's not much we can: for her.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 301290,
			say = "No, I suppose not...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 305140,
			say = "So unfortunate! Now let's find a quiet place and form a plan.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_roma_6",
			hidePaintObj = True,
			side = 2,
			say = "Suruga grabbed Shimakaze's hand and started scurrying off elsewhere. However...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			actorName = "???",
			nameColor = "#A9F548FF",
			say = "Hold it, you two! Don't ignore me!",
			effects = {
				{
					active = True,
					name = "speed"
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 305140,
			say = "(Dammit, dammit, dammit! Okay, need to come up with an excuse to get her off our case! Think, Suruga, think!)",
			effects = {
				{
					active = False,
					name = "speed"
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 301290,
			say = "S-Suruga? What's the matter?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_roma_6",
			hidePaintObj = True,
			side = 2,
			say = "Suruga froze in place as she racked her brain.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actorName = "???",
			say = "You're Sardegnian, aren't you?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 305140,
			say = "(...Sardegnian?)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_roma_6",
			hidePaintObj = True,
			side = 2,
			say = "Suruga looked over her shoulder and spotted two Sardegnian shipgirls who had appeared seemingly from nowhere.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603050,
			say = "Yes. I'm Bolzano, and this is Giuseppe Garibaldi. Would you kindly tell us your name?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 499070,
			say = "Prinz Rupprecht. You'd be wise to remember it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 499070,
			say = "Listen here. There are weird things going on around here.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603050,
			say = "Weird things? I'm afraid I:n't know what you mean. Everything is in order to the best of my knowledge.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 499070,
			say = "So I hope. But: explain why you're off to the Colosseum in such a rush.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603050,
			say = "Well, um... We're members of staff, see? We just finished our patrols and are heading back to the employee area.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 602020,
			say = "Aww, are you lost, little girl? We can show you back to the Iron Blood pavilion if you want.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 499070,
			say = "I'm perfectly fine right here, thank you very much! And who are you calling a little girl?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 499070,
			say = "Besides, aren't you in a hurry? You've got better things to: than play babysitter.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 499070,
			say = "Since you won't tell me what's going on, I'll just find out myself! Hmph!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 602020,
			say = "...You hear that? What a piece of work.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603050,
			say = "She's right, however. We have to focus on our own task over helping others.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603050,
			say = "People have realized something is wrong. First the Sakura Empire, now the Iron Blood as well.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 602020,
			say = "Seems so... Come on, we'd better hurry.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 602020,
			say = "This incident is too important to leave up to a group of kids like da Vinci's team.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 301290,
			say = "Are you alright, Suruga?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 305140,
			say = "Phew... Thank god she wasn't talking about us.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 301290,
			say = "Yup. I think she thought it'd be better to ask the Sardegnian staff instead.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 305140,
			say = "Have they left?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 301290,
			say = "I believe so. They must be headed for the Colosseum.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 305140,
			say = "(The place Shinano outright said is dangerous...)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 301290,
			say = "Well:, Suruga, where: we go? You said something about forming a plan, right?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 305140,
			say = "(Considering where we are now and where those two went...)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 305140,
			say = "To the Eagle Union's area, for a start.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			blackBg = True,
			actor = 305140,
			say = "(Everything will be fine as long as we stay far away from Sardegna's area! Shimakaze won't get the chance to rope us into someone else's business!)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
