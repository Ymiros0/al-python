return {
	fadeOut = 1.5,
	mode = 2,
	id = "XIONGYINGDEXUSHIGE24",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			stopbgm = True,
			say = "Bolzano and Garibaldi made it to the Colosseum where they found the:or to the underground warehouse.",
			bgm = "story-roma-outside",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603050,
			say = "Here it is. The place where we last recorded the signal from da Vinci's team.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 602020,
			say = "Mhm. No sign of Emanuele or any of the rest ever since:.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603050,
			say = "It's like they vanished off the face of the earth, just like Roma. I can only pray no harm has come to them...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 602020,
			say = "Chill out, Bolzano. If we make a huge fuss, it'll reflect poorly upon the expo. You already know that, right?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603050,
			say = "...You're right. With any luck, we'll succeed in resolving this incident.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603050,
			say = "If we:n't, we'll have no choice but to mobilize Veneto's rearguard, and: the cat will really be out of the bag.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Colosseum - Underground Warehouse",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603050,
			say = "Do you read me, da Vinci? Come in, da Vinci.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 602020,
			say = "Still nothing, huh?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603050,
			say = "No. No response from her, Alfredo, or Emanuele.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 602020,
			say = "Yet we can still contact Veneto. Wonder what's the deal with that.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 602020,
			say = "What if there's an underground Mirror Sea near here? Wouldn't be the most far-fetched thing in the world.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 602020,
			say = "Well, I guess it'd be more like a Mirror Space in that case...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603050,
			say = "Hold up, Garibaldi! Isn't this the storage area?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603050,
			say = "I'm not familiar with these artifacts. Could you have a look?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 602020,
			say = "Give me a sec.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 40,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 602020,
			say = "Looks like... the Royal Navy's exhibition items, as well as some of the stuff our own higher-ups brought in.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 602020,
			say = "And look, there are signs that someone was moving stuff around in here not too long ago...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 602020,
			say = "Hang on a minute! Where's the...?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			actor = 602020,
			nameColor = "#A9F548FF",
			blackFg = 0.2,
			say = "A storage case is missing! Where the hell has it gone?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			actor = 603050,
			nameColor = "#A9F548FF",
			blackFg = 0.4,
			say = "Based on what we know, I'd speculate that someone stole it after disabling the surveillance cameras.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			actor = 602020,
			nameColor = "#A9F548FF",
			blackFg = 0.6,
			say = "Well, presumably, but how? The Colosseum's main and service entrances are maximum security.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			actor = 602020,
			nameColor = "#A9F548FF",
			blackFg = 0.8,
			say = "I'm starting to think this incident is way bigger than we first imagined...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			actor = 603050,
			nameColor = "#A9F548FF",
			blackFg = 1,
			say = "Check if any other cases are missing. I'll radio Veneto in the meanwhile.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			actor = 603050,
			nameColor = "#A9F548FF",
			blackFg = 1,
			say = "Veneto, this is Bolzano. We're inside the... Veneto?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			actor = 603050,
			nameColor = "#A9F548FF",
			blackFg = 1,
			say = "...My communicator isn't working!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			blackFg = 1,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actorName = "Communicator",
			say = "BZZZZZZT...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			actor = 602020,
			nameColor = "#A9F548FF",
			blackFg = 1,
			say = "Crap, now even we've lost radio contact!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			actor = 603050,
			nameColor = "#A9F548FF",
			blackFg = 1,
			say = "I'm afraid so... Looks like we're in the same pickle as everyone else now...",:
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			actor = 602020,
			nameColor = "#A9F548FF",
			blackFg = 1,
			say = "Fine, I'm up for a challenge. Though I can't say for sure if whoever is behind all this knows that we're here.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			actor = 602020,
			nameColor = "#A9F548FF",
			blackFg = 1,
			say = "We can't get to the bottom of this without getting our hands dirty anyway.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			actor = 602020,
			nameColor = "#A9F548FF",
			blackFg = 1,
			say = "We have a lot to investigate, like that \"emerald eagle\" Emanuele mentioned.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_roma_6",
			hidePaintObj = True,
			actor = 602020,
			nameColor = "#A9F548FF",
			blackFg = 1,
			say = "Finally, we need to figure out who's trying to wreck our celebration of peace, and what they want with Roma...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_6",
			blackFg = 1,
			blackBg = True,
			actor = 602020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "In any case, we can't afford to stand around! Let's go, Bolzano!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
