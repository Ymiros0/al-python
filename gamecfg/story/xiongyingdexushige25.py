return {
	fadeOut = 1.5,
	mode = 2,
	id = "XIONGYINGDEXUSHIGE25",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_7",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			stopbgm = True,
			say = "Garibaldi and Bolzano emerged from the long, dark corridor beneath the Colosseum and found themselves in a vast seascape.",
			bgm = "story-roma-inside",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_7",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 602020,
			say = "Crazy how they built all this beneath the Colosseum. Jokes aside, what is this place?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 40,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_7",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 602020,
			say = "There's no way in hell this could've gone unnoticed during construction. It must've appeared very recently. What: you think, Bolzano?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_7",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603050,
			say = "It's quite clearly a Mirror Sea. That explains why our communicators stopped working.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_roma_7",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 602020,
			say = "Well, yeah. I mean the place itself though. What: you make of it?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_7",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603050,
			say = "Let's see... This place looks pretty run-down, as if it's been abandoned for quite some time. There's something disconcerting about the architecture though...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_7",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 602020,
			say = "Yeah, it almost looks like... someone decided to build an identical copy of OUR Colosseum.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_roma_7",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 602020,
			say = "The way everything's all fallen apart almost gives it a sense of foreboding...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_7",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603050,
			say = "Do you believe the Sirens designed it based on our World Expo?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_7",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 602020,
			say = "Who knows? But based on the other factions' experiences with the Sirens, I'd say pretty much anything is possible.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_7",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 602020,
			say = "So, now that we're here, what should we: next?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_7",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603050,
			say = "Locate and regroup with da Vinci's team, I'd say. We'll need all the backup we can get if we're to last long enough to find Roma.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_7",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 602020,
			say = "Sounds like a plan to me... Whoa, would you look at that aurora.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 40,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_7",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603050,
			say = "Quite the sight, isn't it? But something about it feels wrong to me...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_roma_7",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 602020,
			say = "You're right... It's actually a massive flock of emerald eagles! I thought there was just one!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_7",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 602020,
			say = "Seems they're interested in us. How about we catch one and keep it as a pet?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 40,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_roma_7",
			hidePaintObj = True,
			side = 2,
			say = "Garibaldi waved at the swarm of eagles soaring above, and that's when the \"aurora\" took on a new behavior.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_roma_7",
			hidePaintObj = True,
			side = 2,
			say = "The green-glowing Siren creatures formed a coordinated swarm and began descending.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_roma_7",
			hidePaintObj = True,
			side = 2,
			say = "The passive birds had become hunters, and the two shipgirls, their prey.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_7",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603050,
			say = "Garibaldi! Look out!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_7",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 603050,
			say = "Prepare all anti-air guns! Shoot them:wn before they can reach us!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_7",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			blackBg = True,
			actor = 602020,
			say = "Huh? Okay! On it!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
