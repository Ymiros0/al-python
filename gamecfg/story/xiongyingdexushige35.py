return {
	fadeOut = 1.5,
	mode = 2,
	id = "XIONGYINGDEXUSHIGE35",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_13",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			stopbgm = True,
			say = "World Expo Venue",
			bgm = "story-roma-outside",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403030,
			nameColor = "#A9F548FF",
			bgName = "bg_roma_13",
			hidePaintObj = True,
			side = 2,
			say = "Hmm~♪ Hmhm~♪",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_13",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 401020,
			say = "I'm glad to see you in high spirits, but umm, allow me to offer you a few words of caution.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_13",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 401020,
			say = "Don't you think you should be keeping Rupprecht and Odin on a tighter leash?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_13",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 401020,
			say = "Are you not worried that they'll get into trouble?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403030,
			nameColor = "#A9F548FF",
			bgName = "bg_roma_13",
			hidePaintObj = True,
			side = 2,
			say = "What trouble? The World Expo is a celebration of peace. There's no way they'd get into a fight.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_13",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 401020,
			say = "That's not what I'm talking about. You know, blueprint ships, out in the open...?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403030,
			nameColor = "#A9F548FF",
			bgName = "bg_roma_13",
			hidePaintObj = True,
			side = 2,
			say = "Is there a problem with that? Hehe, if you ask me, it's totally fine. People want to take a closer look at our blueprint ships, so why not let them?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403030,
			nameColor = "#A9F548FF",
			bgName = "bg_roma_13",
			hidePaintObj = True,
			side = 2,
			say = "If they want to know about our girls so badly, they can just ask~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403030,
			nameColor = "#A9F548FF",
			bgName = "bg_roma_13",
			hidePaintObj = True,
			side = 2,
			say = "Besides, even if I tried to take a more...:mineering approach, Odin's a grouchy loner and Rupprecht:esn't like taking orders. So what's the point?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403030,
			nameColor = "#A9F548FF",
			bgName = "bg_roma_13",
			hidePaintObj = True,
			side = 2,
			say = "That's why it's better to let the chips fall where they may. We're all happier that way~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403030,
			nameColor = "#A9F548FF",
			bgName = "bg_roma_13",
			hidePaintObj = True,
			side = 2,
			say = "But look at you, worrying about other people again. You really are a sweetheart, Thiele.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_13",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 401020,
			say = "I'm not worried about you in the slightest. I just get the feeling that I'm being dragged into another annoying mess again.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403030,
			nameColor = "#A9F548FF",
			bgName = "bg_roma_13",
			hidePaintObj = True,
			side = 2,
			say = "Oh my, look who just came out from the Dragon Empery's pavilion... The one and only Kronshtadt.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_13",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 401020,
			say = "Yeah, what about her? ...What are you trying to:?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403030,
			nameColor = "#A9F548FF",
			bgName = "bg_roma_13",
			hidePaintObj = True,
			side = 2,
			say = "We made a promise to go out for drinks the next time we met. And now is that time~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403030,
			nameColor = "#A9F548FF",
			bgName = "bg_roma_13",
			hidePaintObj = True,
			side = 2,
			say = "Heehee. I wonder what kinds of stories she'll tell me this time.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_13",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 401020,
			say = "Eugen, you'd better not: anything too reckless––",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403030,
			side = 2,
			bgName = "bg_roma_13",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			blackBg = True,
			say = "Hey, Kronshtadt! Heeeey!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
