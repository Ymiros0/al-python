return {
	fadeOut = 1.5,
	mode = 2,
	id = "XIONGYINGDEXUSHIGE36",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_13",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			stopbgm = True,
			say = "World Expo Venue - Vichya Pavilion",
			bgm = "story-roma-outside",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_13",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Joffre opened up a memo on top of the table in the break room that detailed each faction's movements since the expo's start.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_13",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 903010,
			say = "Iron Blood, the Sakura Empire, the Eagle Union, and Iris Libre...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_13",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 903010,
			say = "Sardegna and the Northern Parliament are in close touch with each other, and the Dragon Empery is rising...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_roma_13",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 903010,
			say = "As for me, I just hope more people will stop by our pavilion.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_13",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 907010,
			say = "There are benefits to having fewer guests. Such as having to: less work.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_13",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 907010,
			say = "Our mission is to look after this pavilion and take care of the guests. Your musings are superfluous, Foch.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_13",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 907010,
			say = "Even if we know exactly what each faction is:ing... It won't: us any good.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_13",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 903010,
			say = "That may be the case. But unlike Dunkerque and Algérie, I'm not one to sit around on my hands.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_13",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "*knock* *knock*",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_13",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 903010,
			say = "L'Indomptable, is someone at the:or?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_13",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 901130,
			say = "Yes. It looks like Avrora from the Northern Parliament would like to speak to the Vichya Dominion's representative.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_13",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 903010,
			say = "Avrora, huh? Joffre, what's the plan?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_13",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 907010,
			say = "Nothing in particular. Let's extend a warm greeting – to our fellow shipgirl.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_13",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 903010,
			say = "You're right. As representatives of the Vichya Dominion, we should show hospitality to our guests. L'Indomptable, could you tell Avrora to wait a minute? I'll be there shortly.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_13",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			blackBg = True,
			actor = 901130,
			say = "Understood!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
