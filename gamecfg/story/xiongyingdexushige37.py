return {
	fadeOut = 1.5,
	mode = 2,
	id = "XIONGYINGDEXUSHIGE37",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			expression = 1,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			stopbgm = True,
			say = "World Expo Venue - Colosseum?",
			bgm = "story-italy",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 605050,
			say = "Marco Polo? Are you in here?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 605050,
			say = "The expo YOU proposed is going on right above you, and you're not going to go take a look?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 605050,
			say = "...I guess not.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_midgard_9",
			hidePaintObj = True,
			side = 2,
			say = "The anxious Conte di Cavour opened the:or to Marco Polo's room.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 605050,
			say = "I should've known she wouldn't be here. Does that mean she already succeeded?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 605050,
			say = "I really:n't get her. If she did succeed, shouldn't she be here lounging in a wine bath with a goofy grin on her face?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_midgard_9",
			hidePaintObj = True,
			side = 2,
			say = "She searched around the room some more, but Marco Polo was indeed nowhere to be found.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 605050,
			say = "Maybe she already went up to check out the expo? She never was one to sit still, just like Cesare.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 605050,
			say = "Well, that's none of my concern.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 605050,
			say = "Guess I'll drop my report off on her desk and be on my merry way...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_midgard_9",
			hidePaintObj = True,
			side = 2,
			say = "She placed the carefully-bound bundle of papers on Marco Polo's desk before turning to leave.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_midgard_9",
			hidePaintObj = True,
			side = 2,
			say = "That is when she realized she wasn't alone. Another shipgirl stood in the corner of the room.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9703010,
			nameColor = "#ffa500",
			bgName = "bg_midgard_9",
			hidePaintObj = True,
			actorName = "???",
			side = 2,
			say = "......",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9703010,
			nameColor = "#ffa500",
			bgName = "bg_midgard_9",
			hidePaintObj = True,
			actorName = "???",
			side = 2,
			say = "...So that those harrowing visions will never haunt me again...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9703010,
			nameColor = "#ffa500",
			bgName = "bg_midgard_9",
			hidePaintObj = True,
			actorName = "???",
			side = 2,
			say = "...Your will shall be:ne...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 605050,
			say = "Is that really you, Trento?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 605050,
			say = "I must say, you feel very different from the person I know...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			blackBg = True,
			actor = 605050,
			say = "Hmm... I'll just pretend I saw nothing.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
