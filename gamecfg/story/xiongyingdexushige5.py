return {
	fadeOut = 1.5,
	mode = 2,
	id = "XIONGYINGDEXUSHIGE5",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			stopbgm = True,
			say = "World Expo Venue",
			bgm = "story-roma-outside",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502080,
			say = "Woah, so many people! So many decorations! It's just like celebrating our Spring Festival back home!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "Indeed. I can't believe how many different cultures there are here.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502080,
			say = "Sis, what are those flying things over there?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 10,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "Let's see... The large one is a dirigible, the smaller one is a sightseeing hot-air balloon, and the smallest one is...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502080,
			say = "A flying boat?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "Yes, the vessel can float because the gas pumped into it is lighter than air. It's the same principle as the balloon you bought earlier.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "As once was said in a poem, \"When you wish to return home, take to the skies with a balloon lighter than air.\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502080,
			say = "Hmm, that makes sense. What about that building over there?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "That's the main pavilion built by Sardegna. I heard they named it \"The Colosseum.\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "It's based on a building of the same name constructed by an ancient civilization predating Sardegna.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "\"To the West lies a great sea, a thousand li beyond the clouds. A kingdom to which a:zen others bend their knee, four hundred cities united and proud.\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "...I wonder what it was like to live in such a place.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502080,
			say = "You really know your stuff, sis!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 10,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "It's not that big of a deal. It's just that I've been reading a lot of books in preparation for this trip.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "After all, we were tasked with learning as much as we can from the others, so I need to be knowledgeable enough to recognize what I'm looking at. Hehe~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502080,
			say = "What about that strange thing the girl over there is pointing at us? Is that how they make cameras over here?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 9,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "Oh my, that seems to be a camera with a special high-performance lens... Is she trying to take a picture of us?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "I hope she's not some kind of... voyeur. Hai Chi, let's go somewhere else.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502080,
			say = "Alright... Wait, is she running after us?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 9,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "Hurry, let's shake her off!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502080,
			say = "Right behind you, sis!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "*pant* *pant*... I can't feel my legs anymore...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Me neither... *pant*... Why did you start running anyway?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "Isn't that the natural thing to: when some suspicious person is chasing after you with a camera?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "A suspicious person? Where? Tell me where they are!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "Um... Right here. In front of my eyes.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "I:n't see any... Wait a second, who are you calling suspicious?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "So, why are you trying to take a picture of us without asking?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 608020,
			say = "Just stop talking, Alfredo! You've caused another huge misunderstanding!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "S-sorry... I'm Alfredo Oriani, from the Sardegnian fleet. And this is my friend, Leonardo da Vinci.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "We're patrolling the venue right now, but I've never seen clothes like yours before and just couldn't pass up the photo op! I just wanted to say hello!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 10,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "You're one of the organizer's ships:, huh...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "I just wanted to approach the two of you to introduce myself, but: you broke out into a sprint!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "I apologize as well:. I suppose I jumped to conclusions too quickly.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 608020,
			say = "Well, all's well that ends well. Besides, this only happened because SOMEONE can't keep her camera to herself.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Ehehe, my bad, my bad~ In any case, are the two of you from the Dragon Empery?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 10,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "Yes, my name is Hai Tien, and this is my sister, Hai Chi.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "We are here as representatives of the Dragon Empery.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "Actually, we wanted to know how to get to the Colosseum...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Really? I'll show you the quickest way to get there. Sorry again for what happened earlier!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "Oh, there's no need for you to trouble yourself with that. We have a map, and:n't mind taking a more scenic route to see the sights and sounds. After all, \"when in Rome,: as the Romans:.\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "If you:n't mind the inconvenience, there is one thing I'd like to ask you though...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Sure, fire away! I'll answer to the best of my ability!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 10,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "What are those things floating in the sky with the little white propellers?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Those are probably our security drones.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "Drones? At first glance, those things look so flimsy that it's a miracle they're staying in the air... How exactly is that possible?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "I can't answer technical questions like that...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "But I can refer you to the person who designed them – Leonardo da Vinci here!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 608020,
			say = "Heh heh, what you're looking at is my original flying device – the Pneumatic Spinner-Arounder Helidrone!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 608020,
			say = "Though these things may look like flimsy pieces of wood and paper,:n't be fooled! They've been fitted with the latest, cutting-edge technology by yours truly!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 608020,
			say = "They're lightweight, versatile, maneuverable, and best of all... this latest model can even carry people!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 608020,
			say = "Hehe, want to give it a try?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "I... I appreciate the offer, but I think I'll pass.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502080,
			say = "You bet I:!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "...Hai Chi?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Da Vinci pulled a controller out from nowhere and slapped at it a few times before waving at the sky, and a drone descended right after.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_roma_2",
			hidePaintObj = True,
			side = 2,
			say = "Without even waiting for the drone's altitude to stabilize, Hai Chi jumped up acrobatically and plopped:wn on the disc-shaped \"seat.\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_roma_2",
			hidePaintObj = True,
			side = 2,
			say = "The drone soon carried her higher and higher, the anxiety on Hai Tien's face following suit.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_4",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actorName = "Hai Chi",
			say = "Woooo, the wind suddenly picked up! It feels great up here–!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 9,
			side = 1,
			bgName = "bg_roma_4",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actorName = "Hai Tien",
			say = "So, um, your... Pneumatic Spinner-Arounder Helidrone... is it going to be okay...?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 1,
			bgName = "bg_roma_4",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actorName = "Leonardo da Vinci",
			say = "Of course! All of my inventions are a-hundred-percent safe and secure! As long as you:n't suddenly deploy your rigging, there's no chance of falling!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_4",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actorName = "Hai Chi",
			say = "Look over there, sis! Those two girls seem to be rushing over here!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_4",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actorName = "Hai Chi",
			say = "They're from Sardegna's fleet, right? Their uniforms are so cool... I wonder what they're up to?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 1,
			bgName = "bg_roma_4",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actorName = "Leonardo da Vinci",
			say = "Hrmm... You're right! It's Bolzano and Garibaldi...?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 1,
			bgName = "bg_roma_4",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actorName = "Leonardo da Vinci",
			say = "I thought those two were in charge of security around the Colosseum... Now what are they:ing over here?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_4",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actorName = "Alfredo Oriani",
			say = "They seem to be secretively whispering something into a communicator too... *gasp*!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_roma_4",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actorName = "Alfredo Oriani",
			say = "Come with us, Teach! Let's go check out the Dragon Empery area first!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "What's wrong? You two look awfully serious all of a sudden...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "There's nothing to worry about! It may not seem like it, but I'm keeping a careful eye on the venue! If there's a scoop to be had... Err, I mean, if any trouble breaks out, I'll be the first to sound the alarm!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502070,
			say = "Oh, um, I see... In any case, we'll be fine over here, so:n't let us keep you from your investigation.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 502080,
			say = "Hup! Give me a holler when you swing by our pavilion! I'll show you around!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "I'll take you up on that:!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			blackBg = True,
			actor = 601080,
			say = "Da Vinci, let's go see what Bolzano and Garibaldi are up to!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
