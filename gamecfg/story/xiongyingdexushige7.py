return {
	fadeOut = 1.5,
	mode = 2,
	id = "XIONGYINGDEXUSHIGE7",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			stopbgm = True,
			say = "Somewhere in the Expo Venue...",
			bgm = "story-roma-outside",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601090,
			say = "What's wrong, Alfredo? Why the sudden call?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601090,
			say = "And why are you hiding behind the other side of the wall? Where's da Vinci?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Hush! Teach is currently monitoring things from the air, so we have to stay on the:wn-low and focus on staying hidden!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			actor = 608020,
			nameColor = "#A9F548FF",
			paintingNoise = True,
			say = "Is Ema here? Good, I'll reconfirm the targets' location:–",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			actor = 608020,
			nameColor = "#A9F548FF",
			paintingNoise = True,
			say = "Mark B and Mark G are currently heading towards the Colosseum.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			actor = 608020,
			nameColor = "#A9F548FF",
			paintingNoise = True,
			say = "They seem to be checking something while moving, and they've been busy on the communicators this whole time.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Understood! Something big is happening, Teach, just like I said!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Continue offering air support. Ema and I are moving out now!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			actor = 608020,
			nameColor = "#A9F548FF",
			paintingNoise = True,
			say = "You got it!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601090,
			say = "...I thought you were patrolling the venue. Why in the world are you tailing Bolzano now?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Excuse me, I'm not TAILING her, I'm conducting an undercover investigation!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Don't you think something's fishy, Ema? We're in charge of patrolling the venues, so why would the two people in charge of keeping an eye on the Colosseum suddenly leave their posts?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Plus, they've continuously been in touch with someone this whole time while moving about!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601090,
			say = "Or... They got someone else to watch the underground warehouse and went on patrol themselves because some people here, and I won't name names, are so unreliable.",:
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Wow, mean!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 0,
					delay = 0,
					dur = 0.4,
					x = 30,
					number = 2
				}
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			actor = 608020,
			nameColor = "#A9F548FF",
			paintingNoise = True,
			say = "Yeah, what she said! That was mean!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601090,
			say = "...In any case, I want you, Alfredo, to call off your \"undercover\" investigation, and I want you, da Vinci, to get:wn from there already. At this rate, WE'RE going to start looking like the suspicious ones sneaking around...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601090,
			say = "If you're worried about Bolzano being out here, you should probably just ask her upfront. I guarantee you, she'll happily fill you in on whatever she's:ing.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Heh heh... And that's why you're an amateur! You'll never be able to land a huge scoop with such naive methods!",
			effects = {
				{
					active = True,
					name = "speed"
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601090,
			say = "...Sure:.",
			effects = {
				{
					active = False,
					name = "speed"
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			actor = 608020,
			nameColor = "#A9F548FF",
			paintingNoise = True,
			say = "This is da Vinci! The situation has changed! Bolzano and Garibaldi are no longer heading to the Colosseum!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Way ahead of you! I saw them walk into a small room that says \"Do Not Enter\" near the Colosseum! What kind of place: you think it is, da Vinci?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			actor = 608020,
			nameColor = "#A9F548FF",
			paintingNoise = True,
			say = "It's the control room for the venue's security system.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Oh...? In other words, there must be something wrong for them to need to check the security system?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Hrmmmmmmmmm... Yes – this is, without a:ubt, a huge scoop in the making!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Is there any way to find out what exactly they're looking for?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			actor = 608020,
			nameColor = "#A9F548FF",
			paintingNoise = True,
			say = "Hah~♪ Who the hell: you think I am? I practically designed the base framework for the security system here!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			actor = 608020,
			nameColor = "#A9F548FF",
			paintingNoise = True,
			say = "Naturally, I left a little backdoor specifically for times like this!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "Woah, I knew I could count on you! The excitement's making my hairs stand on end!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			actor = 608020,
			nameColor = "#A9F548FF",
			paintingNoise = True,
			say = "Well, the process is actually quite complicated, so we'll need some peace and quiet as well as a bit of time.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601090,
			say = "Hmm... Then how about the forest next to the Sakura Empire pavilion?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 601080,
			say = "That's a great idea! I'm so glad you're on board, Ema~! The investigation team gets another member!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_roma_2",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			blackBg = True,
			actor = 601080,
			say = "Let's get ready to go, Teach!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
