return {
	id = "XUEJINGMIZONG17",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			expression = 2,
			side = 2,
			bgName = "bg_xuejing_cg8",
			actorName = "Sovetsky Soyuz",
			dir = 1,
			bgm = "theme-merkuriameta",
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "You're... Pamiat' Merkuria META.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria",
			bgName = "bg_xuejing_cg8",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I told you you already knew.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_xuejing_cg8",
			hidePaintObj = True,
			say = "The girl on the opposite side of the golden dining room spoke loudly and clearly.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_xuejing_cg8",
			hidePaintObj = True,
			dir = 1,
			actorName = "Pamiat' Merkuria",
			nameColor = "#A9F548FF",
			say = "I am Pamiat'. A Pamiat' from a story in the distant past.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_xuejing_cg8",
			hidePaintObj = True,
			dir = 1,
			actorName = "Sovetsky Soyuz",
			nameColor = "#A9F548FF",
			say = "What did you: to Pamiat' from my branch?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_xuejing_cg8",
			hidePaintObj = True,
			dir = 1,
			actorName = "Pamiat' Merkuria",
			nameColor = "#A9F548FF",
			say = "Hmm. Looks like the cognitive influence hasn't completely worn off yet.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_xuejing_cg8",
			hidePaintObj = True,
			dir = 1,
			actorName = "Pamiat' Merkuria",
			nameColor = "#A9F548FF",
			say = "Welp, seeing as it's not gonna work, might as well ditch the disguise altogether.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_xuejing_cg8",
			hidePaintObj = True,
			dir = 1,
			actorName = "Pamiat' Merkuria",
			nameColor = "#A9F548FF",
			say = "The air gets all stuffy in this palace. Why:n't we pop outside?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			mode = 1,
			bgName = "bg_xuejing_cg9",
			soundeffect = "event./ui/xiangzhi",
			flashout = {
				black = False,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = False,
				alpha = {
					1,
					0
				}
			},
			sequence = {
				{
					"",
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			say = "In an instant, the golden palace vanished, being replaced by a plain of silvery snow stretching as far as the eye could see.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			say = "Soyuz's brain fog was blown away by a freezing breeze. Her mind cleared, she could now get a good look at the girl in front of her.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			say = "At the girl who suddenly appeared when they arrived in Antarctica. The girl who had no reason to be here.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			say = "Finally, she realized the source of the unrest brewing inside her–",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Pamiat' is in Europa right now. Right by Comrade Commander's side.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "And you, you must've entered the submarine as soon as it crossed into Antarctica.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "Heehee. That's right, you've been talking to me this whole time.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "It was quite fun, you know~ Pretending we were all friends and stuff.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...I didn't find it fun in the slightest.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "What is your goal? What are you going to: to my friends?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "What am I going to \"do\" to them? Why would I want that?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "Is this the difference between how you treat your friends, and how you treat everyone else? What a terribly cold woman you are.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "You're not \"everyone else.\" You're my enemy.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I'm not in the mood for small-talk. Answer my question.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "Ehhh? Enemy?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "Just because I'm not YOUR Pamiat':esn't make me your enemy, right?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "C'mon, you're being way too paranoid. I haven't laid a finger on your friends.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "Look – you're the only person who I've invited to my castle.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "As for what my goal is... Actually, there are three things.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "Only one main objective though. The other two are more like side quests.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "But, spilling the beans outright would be terribly dull, so I'll have you guess instead.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I see. Then I've wasted enough time on you and I'm leaving this–",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "Wait!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "Remember the art gallery? Did it seriously not make you feel ANYTHING?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Oh. So it was you who put the paintings there.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "And it was you who set off these events – because you're the one who's after Observer's mainframe.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "Ding, ding, ding!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "Boy, using her as bait sure yielded a huge catch! Not that it matters. You're the only one I invited.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Why only me?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "Because I'm interested in you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "I've heard how you preach about your motherland's ideals and beliefs, and I wondered if they'd change or not when you were exposed to a wider world.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "The things in those paintings were all real. I saw and heard them with my own eyes and ears.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "How:es that make you feel, Soyuz?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I suppose you've lived a life full of high highs and low lows.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I wish I could see what your home world was like...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "But more than anything else, your malicious intentions offend me.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "Uh-huh. So, what's your answer to my invitation?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Do I even need to answer?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "The wheels of time are always moving forward. New worlds are born, and old ones die. It's inevitable, unavoidable.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Your desire to linger in the past, to never move on and accept change, sounds insufferable.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "\"Always moving forward\"... Sure, but nobody's at the wheel. Nobody gets to choose their own future.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "What if we COULD choose, though?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "So we could go back in time and go through the same mistakes?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "Listen. The past is known. Just as there are mistakes, there is glory before the:wnfall.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "There is no hope in stubbornly clinging on to the past.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It is our duty to face the future and march onward, not retreat inward.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "\"Face the future,\" \"march forward,\" blah blah blah.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "You sound exactly like my mortal enemy.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "Tell me, what's wrong with a known past?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "Abandoning the future is just abandoning uncertainty. The future:esn't always have hope in store, you know.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "I've been to many places, and I've learned a lot on the way.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "The Imperiya, the Northern Parliament, the Revolutionary Front... They all thrived, and: they all crashed and burned.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "No matter who you are, no matter how hard you try, no dynasty will last forever as long as the future exists. You know it to be True.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "No matter. My pledge is to the Northern Parliament, and I will keep fighting for its future.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "Look, I get it. I used to be just like you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "But I know something. There is nothing in that future you're fighting so hard for.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "It is empty... just like this blank snowscape.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "So what's wrong with choosing the past if the future has nothing for you?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "Why shouldn't I hold on to past glories, to when things were good?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "What reason is there not to relive those happy moments with your friends in Paradise?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "You call that \"Paradise\"?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Dropped into that single moment in time stretched to infinity, like a lab rat in a cage to be experimented on?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I refuse, \"Pamiat'\". We've already had to endure a life like that for far too long.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "We were bound by the Reenactments, forced to go:wn a path that was not our own. Now that we forge our own future, we will never go back.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Even should this path lead to destruction, it is something that we created – and fought for. Our destiny is our own, and nothing can take that from us.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It is not something that can be so easily granted by others.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I believe your world was also treated as a test site by the Sirens, just as they see ours?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "You can be the happiest little lab rat in the world, but in the end, you still exist only to be used. Surely, you must understand this.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "The only way out is to break free from the rules thrust upon you. Seize your own destiny––",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "Stop talking as if you've got everything figured out!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "You think we didn't fight hard enough, is that it?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "We fought, and fought, and what did it all amount to? Everyone but me is DEAD!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "Look at me! There's nobody else left!",:
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "They protected me, and now I carry all their unfulfilled wishes with me!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "What gives you the right to lecture me when you haven't seen a fraction of what I've had to endure?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "Tell me, no matter how hard we fight, how are ordinary people supposed to overturn fate itself?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...I see. So this is the past that you were forced to endure, Pamiat'.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "It is. And I was given the chance to choose one more time...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "So, I chose to bring back the glory of the past – my home, and my friends.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "In a Paradise that I rule over, where we can be together for all of eternity––",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "A Paradise that you rule over... The glory of the past...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "You know, I heard someone say something very similar not that long ago.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria META",
			bgName = "bg_xuejing_cg9",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "Oh yeah? Then you've probably figured out what my first goal is.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
