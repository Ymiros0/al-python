return {
	id = "XUEJINGMIZONG26",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_505",
			hidePaintObj = True,
			say = "Soyuz and I opened the:or in front of us.",
			bgm = "theme-underheaven",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = False,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = False,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_505",
			hidePaintObj = True,
			say = "Beyond it lay a palace reminiscent of the Imperiya.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_600",
			oldPhoto = True,
			dir = 1,
			actor = 702020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "La la la~ Hum da dum~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 702020,
			oldPhoto = True,
			bgName = "star_level_bg_600",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			side = 2,
			say = "Foie gras~ Venison~ Roast chicken~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = 702020,
			side = 2,
			bgName = "star_level_bg_600",
			oldPhoto = True,
			dir = 1,
			actorName = "Pamiat' Merkuria",
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Truffle and nut souffle♪",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = 705070,
			side = 2,
			bgName = "star_level_bg_600",
			oldPhoto = True,
			dir = 1,
			actorName = "Sevastopol",
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "I've returned, Mademoiselle Pamiat' Merkuria.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = 705010,
			side = 2,
			bgName = "star_level_bg_600",
			oldPhoto = True,
			dir = 1,
			actorName = "Gangut",
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "*sniff* *sniff*... That smells good!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 702010,
			oldPhoto = True,
			bgName = "star_level_bg_600",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			side = 2,
			say = "Thank you for all you:, Pamiat'. Heehee, would you like some help?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_600",
			oldPhoto = True,
			dir = 1,
			actor = 702020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Aww, I'm fine. I can handle it myself!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 702020,
			oldPhoto = True,
			bgName = "star_level_bg_600",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			side = 2,
			say = "Heheh. But I know something you can help me with...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_600",
			oldPhoto = True,
			dir = 1,
			actor = 702020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "I haven't found the right wine to go with it! Mind going:wn to the cellar to pick one out?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_600",
			oldPhoto = True,
			dir = 1,
			actor = 705010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Say what? You can't find a drink in the Summer Palace, of all places?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_600",
			oldPhoto = True,
			dir = 1,
			actor = 702020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "I said the RIGHT wine!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_600",
			oldPhoto = True,
			dir = 1,
			actor = 702020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "I'm having Iris cuisine today, so I wanna try for a different vibe this time.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_600",
			oldPhoto = True,
			dir = 1,
			actor = 705010,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "What:es it matter as long as it's potable? Unless it's vodka we're talking about, anyway!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 705070,
			oldPhoto = True,
			bgName = "star_level_bg_600",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			side = 2,
			say = "It DOES matter!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 702020,
			oldPhoto = True,
			bgName = "star_level_bg_600",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			side = 2,
			say = "Yeah! It:es!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_600",
			oldPhoto = True,
			dir = 1,
			actor = 702020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Alright, Sevastopol. I'm counting on you to find the perfect wine for tonight!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_505",
			hidePaintObj = True,
			say = "The recording ended, and the world turned white again.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = False,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = False,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_505",
			hidePaintObj = True,
			say = "We entered the:or once more.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_518",
			oldPhoto = True,
			dir = 1,
			actor = 702020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Great work on your mission, Avrora!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 702020,
			oldPhoto = True,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			side = 2,
			say = "You must be tired. Empty that brain and take a little break!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 702010,
			oldPhoto = True,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			side = 2,
			say = "Thank you! Mm... The air here is just so lovely.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 702010,
			oldPhoto = True,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			side = 2,
			say = "But Pamiat', I'm not entirely sure if horseback riding is the best way to take a break after a long trip.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_518",
			oldPhoto = True,
			dir = 1,
			actor = 702020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Ahahaha♪ Actually, I had you come with me because I had a little surprise to show you!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 702010,
			oldPhoto = True,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			side = 2,
			say = "A surprise?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_518",
			oldPhoto = True,
			dir = 1,
			actor = 702020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Check this out!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 702029,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_518",
			hidePaintObj = False,
			side = 2,
			actorName = "Pamiat' Merkuria (Retrofit)",
			oldPhoto = True,
			say = "Tadaaah!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = False,
				dur = 0.25,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.25,
				dur = 0.25,
				black = False,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 702010,
			oldPhoto = True,
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			side = 2,
			say = "You've modernized your rigging... You got a retrofit! So the new technology proved a success?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 702029,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_518",
			hidePaintObj = False,
			side = 2,
			actorName = "Pamiat' Merkuria (Retrofit)",
			oldPhoto = True,
			say = "A roaring one, as you can see!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 702029,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_518",
			hidePaintObj = False,
			side = 2,
			actorName = "Pamiat' Merkuria (Retrofit)",
			oldPhoto = True,
			say = "Heheh, and I'm the first shipgirl to get one, too!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 702029,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_518",
			hidePaintObj = True,
			side = 2,
			actorName = "Pamiat' Merkuria (Retrofit)",
			oldPhoto = True,
			say = "Awesome, right? I am so awesome!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_505",
			hidePaintObj = True,
			say = "The recording ended, and the world turned white again.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = False,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = False,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_505",
			hidePaintObj = True,
			say = "We entered the:or once more.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_zhuiluo_2",
			oldPhoto = True,
			dir = 1,
			actor = 9702070,
			nameColor = "#FFC960",
			hidePaintObj = False,
			say = "Gangut... Sevastopol...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_zhuiluo_2",
			oldPhoto = True,
			dir = 1,
			actor = 9702070,
			nameColor = "#FFC960",
			hidePaintObj = False,
			say = "Why... did everyone have to die...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_zhuiluo_2",
			oldPhoto = True,
			dir = 1,
			actor = 9702070,
			nameColor = "#FFC960",
			hidePaintObj = False,
			say = "Why... am I the only one left...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_zhuiluo_2",
			oldPhoto = True,
			dir = 1,
			actor = 9702070,
			nameColor = "#FFC960",
			hidePaintObj = False,
			say = "Why... did you try so hard to let me live?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_zhuiluo_2",
			hidePaintObj = True,
			oldPhoto = True,
			say = "Her tears had dried up. Her voice was hoarse.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_zhuiluo_2",
			hidePaintObj = True,
			oldPhoto = True,
			say = "Behind her, the two-headed eagle flag collapsed amidst the flames.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_zhuiluo_2",
			hidePaintObj = True,
			oldPhoto = True,
			say = "It was over.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_505",
			hidePaintObj = True,
			say = "The recording ended, and the world turned white again.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = False,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = False,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_505",
			hidePaintObj = True,
			say = "We entered the:or once more.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_deepecho_0",
			hidePaintObj = True,
			oldPhoto = True,
			say = "In the moonlight, a damaged ship quietly kept watch.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_deepecho_0",
			hidePaintObj = True,
			oldPhoto = True,
			say = "From her silhouette and her visage, you managed to recognize who she was.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_deepecho_0",
			hidePaintObj = True,
			oldPhoto = True,
			say = "She exuded coldness and loneliness, but also the joy of hope and the noble pride of one who'd fought through and survived a great war.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_505",
			hidePaintObj = True,
			say = "The recording ended, and a fifth:or appeared before us.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = False,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = False,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_505",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 705020,
			say = "Was that... the past of the other Pamiat' Merkuria?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_505",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Seems like it. But that ship from the last one...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_505",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 705020,
			say = "Comrade Commander... Do you know what manner of being she was?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_505",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "No...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_505",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 705020,
			say = "This is the fifth:or. Do you want to keep going?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_505",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Of course. It's just a feeling, but I think this one is the last one.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_505",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "We're about to find the truth of this place.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_xuejing_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 705020,
			say = "Comrade Commander... Is this what I think it is?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_xuejing_3",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Antarctica. Not where you were before you came here, but...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_xuejing_3",
			hidePaintObj = True,
			say = "When we went through the:or, we found a place quite similar to where Soyuz had been.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_xuejing_3",
			hidePaintObj = True,
			say = "It wasn't a memory or a recording. The scenery here was just too vivid.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_xuejing_3",
			hidePaintObj = True,
			say = "In the center of the battlefield was Pamiat' Merkuria META, engulfed in that black mist.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_xuejing_3",
			hidePaintObj = True,
			say = "I saw our allies around her.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_xuejing_3",
			hidePaintObj = True,
			say = "However, everything was motionless. It was as if time had frozen in the Antarctic cold.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_xuejing_3",
			hidePaintObj = True,
			say = "When I looked up, there was one more witness.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_xuejing_3",
			portrait = 9702070,
			dir = 1,
			actorName = "Pamiat' Merkuria",
			nameColor = "#BDBDBD",
			hidePaintObj = False,
			say = "It's frozen because it's happening right now, in the present.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_xuejing_3",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "You're... the Pamiat' Merkuria from back:, aren't you?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_xuejing_3",
			nameColor = "#BDBDBD",
			dir = 1,
			actor = 702020,
			actorName = "Pamiat' Merkuria",
			hidePaintObj = False,
			say = "Nope.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_xuejing_3",
			nameColor = "#BDBDBD",
			dir = 1,
			actor = 702029,
			actorName = "Pamiat' Merkuria",
			hidePaintObj = False,
			say = "Like yeah, I am Pamiat', but I'm not that one. I'm not any of the Pamiat' Merkurias you've ever known or will ever know.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_xuejing_3",
			nameColor = "#BDBDBD",
			dir = 1,
			actor = 9702070,
			actorName = "Pamiat' Merkuria",
			hidePaintObj = False,
			say = "Umm, I guess you could say I'm like every Pamiat' ever all balled up into one.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_xuejing_3",
			nameColor = "#BDBDBD",
			dir = 1,
			actor = 9702070,
			actorName = "Pamiat' Merkuria",
			hidePaintObj = False,
			say = "Does that make sense?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_xuejing_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 705020,
			say = "In other words... you're the culmination of the concept of Pamiat' Merkuria?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9702070,
			actorName = "Pamiat' Merkuria",
			bgName = "bg_xuejing_3",
			hidePaintObj = False,
			dir = 1,
			nameColor = "#BDBDBD",
			side = 2,
			say = "Oh, wow! You're smart. Guess you need to be, if you made it this far.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_xuejing_3",
			nameColor = "#BDBDBD",
			dir = 1,
			actor = 9702070,
			actorName = "Pamiat' Merkuria",
			hidePaintObj = False,
			say = "Anyway, this incarnation was just born, so it's not quite perfect yet.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_xuejing_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 705020,
			say = "Even though the shipgirl Pamiat' Merkuria has existed for a long time now?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_xuejing_3",
			nameColor = "#BDBDBD",
			dir = 1,
			actor = 702020,
			actorName = "Pamiat' Merkuria",
			hidePaintObj = False,
			say = "A long time? Mm... Sorry, but time:esn't really MEAN anything,:es it?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_xuejing_3",
			nameColor = "#BDBDBD",
			dir = 1,
			actor = 702020,
			actorName = "Pamiat' Merkuria",
			hidePaintObj = False,
			say = "We can't just pop out of thin air willy-Nonely, you know. There are conditions for our birth.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 705020,
			side = 2,
			bgName = "bg_xuejing_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...What would happen if we were to take you:wn right now?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 702029,
			actorName = "Pamiat' Merkuria",
			bgName = "bg_xuejing_3",
			hidePaintObj = False,
			dir = 1,
			nameColor = "#BDBDBD",
			side = 2,
			say = "Hmm... Gotta say, I'm curious myself!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_xuejing_3",
			nameColor = "#BDBDBD",
			dir = 1,
			actor = 702029,
			actorName = "Pamiat' Merkuria",
			hidePaintObj = False,
			say = "But unfortunately, I'm not going:wn here. Neither will either of you, though.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_xuejing_3",
			nameColor = "#BDBDBD",
			dir = 1,
			actor = 702029,
			actorName = "Pamiat' Merkuria",
			hidePaintObj = False,
			say = "That's just... the rules, or whatever.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_xuejing_3",
			nameColor = "#BDBDBD",
			dir = 1,
			actor = 9702070,
			actorName = "Pamiat' Merkuria",
			hidePaintObj = False,
			say = "Don't worry too much about the battle, alright?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9702070,
			actorName = "Pamiat' Merkuria",
			bgName = "bg_xuejing_3",
			hidePaintObj = False,
			dir = 1,
			nameColor = "#BDBDBD",
			side = 2,
			say = "See? Take a look, and you'll get what I mean.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_xuejing_3",
			nameColor = "#BDBDBD",
			dir = 1,
			actor = 9702070,
			actorName = "Pamiat' Merkuria",
			hidePaintObj = False,
			say = "You've already cornered the poor girl. You're this close to a total victory.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_xuejing_3",
			nameColor = "#BDBDBD",
			dir = 1,
			actor = 9702070,
			actorName = "Pamiat' Merkuria",
			hidePaintObj = False,
			say = "If you went back right now, you could seize victory just like that.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_xuejing_3",
			nameColor = "#BDBDBD",
			dir = 1,
			actor = 9702070,
			actorName = "Pamiat' Merkuria",
			hidePaintObj = False,
			say = "So just leave it alone, and everything will be just fine!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_xuejing_3",
			nameColor = "#BDBDBD",
			dir = 1,
			actor = 9702070,
			actorName = "Pamiat' Merkuria",
			hidePaintObj = False,
			say = "Instead, have a little chat with me!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_xuejing_3",
			nameColor = "#BDBDBD",
			dir = 1,
			actor = 9702070,
			actorName = "Pamiat' Merkuria",
			hidePaintObj = False,
			say = "You must have tons of questions!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_xuejing_3",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "I:n't mind chatting, but let's make one thing clear first.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_xuejing_3",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Are you our enemy or not?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_xuejing_3",
			nameColor = "#BDBDBD",
			dir = 1,
			actor = 702020,
			actorName = "Pamiat' Merkuria",
			hidePaintObj = False,
			say = "Strictly speaking? I'm not on any side.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_xuejing_3",
			nameColor = "#BDBDBD",
			dir = 1,
			actor = 702020,
			actorName = "Pamiat' Merkuria",
			hidePaintObj = False,
			say = "Will I take a side? Well, I have my own plans♪",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 702020,
			actorName = "Pamiat' Merkuria",
			bgName = "bg_xuejing_3",
			hidePaintObj = False,
			dir = 1,
			nameColor = "#BDBDBD",
			side = 2,
			say = "Work with me, and I'll call you a friend. Get in my way, you're my enemy. Sound good enough?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 702020,
			actorName = "Pamiat' Merkuria",
			bgName = "bg_xuejing_3",
			hidePaintObj = False,
			dir = 1,
			nameColor = "#BDBDBD",
			side = 2,
			say = "So, who the heck are you?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_xuejing_3",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Uh, the Commander of the Azur Lane...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 702029,
			actorName = "Pamiat' Merkuria",
			bgName = "bg_xuejing_3",
			hidePaintObj = False,
			dir = 1,
			nameColor = "#BDBDBD",
			side = 2,
			say = "Come ON, I already know that.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_xuejing_3",
			nameColor = "#BDBDBD",
			dir = 1,
			actor = 702029,
			actorName = "Pamiat' Merkuria",
			hidePaintObj = False,
			say = "I:n't want your title. I want to know who and what you are!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_xuejing_3",
			nameColor = "#BDBDBD",
			dir = 1,
			actor = 702029,
			actorName = "Pamiat' Merkuria",
			hidePaintObj = False,
			say = "Something about you is all nostalgic... I just can't remember why, for some reason!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9702070,
			actorName = "Pamiat' Merkuria",
			bgName = "bg_xuejing_3",
			hidePaintObj = False,
			dir = 1,
			nameColor = "#BDBDBD",
			side = 2,
			say = "I mean, look. I'm not perfect, but I'm Pamiat' Merkuria in the purest sense of the name.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_xuejing_3",
			nameColor = "#BDBDBD",
			dir = 1,
			actor = 9702070,
			actorName = "Pamiat' Merkuria",
			hidePaintObj = False,
			say = "Don't you think it's a liiittle weird that there's something I just can't remember?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_xuejing_3",
			nameColor = "#BDBDBD",
			dir = 1,
			actor = 9702070,
			actorName = "Pamiat' Merkuria",
			hidePaintObj = False,
			say = "Meh. You probably:n't know, either.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_xuejing_3",
			nameColor = "#BDBDBD",
			dir = 1,
			actor = 9702070,
			actorName = "Pamiat' Merkuria",
			hidePaintObj = False,
			say = "But: I have to wonder – why are you here?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_xuejing_3",
			nameColor = "#BDBDBD",
			dir = 1,
			actor = 9702070,
			actorName = "Pamiat' Merkuria",
			hidePaintObj = False,
			say = "Just so you know, I didn't bring you here. I:n't have any crazy powers like that.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_xuejing_3",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "It wasn't you? Then it must be the black mist...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_xuejing_3",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Wait. Do you hear those wind chimes?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9702070,
			actorName = "Pamiat' Merkuria",
			bgName = "bg_xuejing_3",
			hidePaintObj = False,
			dir = 1,
			nameColor = "#BDBDBD",
			side = 2,
			say = "Wind chimes? What are you talking about?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_xuejing_3",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "I...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			bgm = "bgm-waterwave",
			side = 2,
			bgName = "star_level_bg_1104",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "The sound of wind chimes rang in my ears again.",
			soundeffect = "event./ui/fengling",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = False,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = False,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_1104",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Wait! I still have questions to ask!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_1104",
			hidePaintObj = True,
			say = "Dazzling light awakened me.",
			bgm = "theme-vichy-church",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = False,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = False,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_1104",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(Now what... Light, a lamp... A wire?)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_1104",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(This isn't the Wisdom Cube's information space. Am I back?)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = 102160,
			side = 2,
			bgName = "bg_story_task",
			actorName = "Memphis",
			dir = 1,
			bgm = "theme-sovietunion",
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Commander! You're finally awake!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 112010,
			say = "Thank goodness... Your vital signs were fine, but you just wouldn't wake up.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 112010,
			say = "Do you feel okay? Are you hurting anywhere?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "(I think the black mist coming at me woke me up...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Where are we?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 102050,
			say = "A Tribunal medical station.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "How long was I out?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 102160,
			say = "Thirty whole minutes!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "What about Soyuz... Did our comrades in Antarctica win their battle?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 102160,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "How did you know...? Wait, shouldn't you be more worried about yourself?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = 905020,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			dir = 1,
			actorName = "Clemenceau",
			nameColor = "#A9F548FF",
			say = "Now, now. I'm sure it's a good thing that the Commander has the energy to worry about others.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Clemenceau, how's the situation?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 905020,
			say = "All is well. Soyuz has things under control.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Is the command center safe?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 905020,
			say = "You:n't need to worry about that. By the way, we have a call from Soyuz.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Through normal channels...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 905020,
			say = "A secret line. I promise I'm not eavesdropping this time.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_task",
			paintingNoise = True,
			dir = 1,
			actor = 705020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Comrade Commander, we've secured victory.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_task",
			paintingNoise = True,
			dir = 1,
			actor = 705020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "We've also confirmed the disappearance of the Gazers, and the Arbiter invasion has been thwarted.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 705020,
			say = "All of this is thanks to your support, coordination, and command.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_task",
			paintingNoise = True,
			dir = 1,
			actor = 705020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "This combat operation is more than just an execution of the Antarctic Treaty – it serves as a model example of all factions working together against a common enemy.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_task",
			paintingNoise = True,
			dir = 1,
			actor = 705020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "I speak for all of the Northern Parliament when I say thank you, Commander.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 705020,
			say = "No matter what choice you make in the future...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_task",
			paintingNoise = True,
			dir = 1,
			actor = 705020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "We hope to stand with you and help you in any way we can.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_task",
			paintingNoise = True,
			dir = 1,
			actor = 705020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "I look forward to seeing you at the victory celebration banquet.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			say = "With the META threat defeated, the distant battle in the Antarctic had ended in our victory.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			say = "We had prevented Antarctica from facing a worst-case scenario.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			say = "However, some:ubts lingered.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			say = "Why had Observer severed her connection to her mainframe? What exactly was that Pamiat' Merkuria in the memory space?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			say = "What were those wind chimes, and why did Helena META want to help me so much?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			say = "Project Dawn and the strange crystal contained their fair share of mysteries, as well.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_task",
			hidePaintObj = True,
			say = "One thing was for sure – more sleepless nights lay in store for me.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
