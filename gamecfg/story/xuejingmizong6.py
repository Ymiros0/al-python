return {
	id = "XUEJINGMIZONG6",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			asideType = 3,
			bgmDelay = 2,
			blackBg = True,
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			},
			sequence = {
				{
					"Antarctica",
					1
				},
				{
					"Ice Field",
					2
				},
				{
					"Sometime later",
					3
				}
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_544",
			dir = 1,
			bgm = "theme-antarctica",
			actor = 705080,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Soyuz, got a minute? I wanted to ask you something.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_544",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 705080,
			say = "I know Iron Blood gave us a hand with our own base, and that we should help those in need...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 705080,
			side = 2,
			bgName = "star_level_bg_544",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "But nobody said you yourself need to go, nor that you had to go at once.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_544",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 705080,
			say = "They've built a fortress out there, like the Eagle Union. I think they're able to defend themselves.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_544",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 705020,
			say = "You're right.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_544",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 705020,
			say = "Iron Blood:es not need our assistance. That was clear from the moment they sent us advice instead of a request for help.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 705020,
			side = 2,
			bgName = "star_level_bg_544",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Having said that, I see this as a good opportunity to: some reconnaissance.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_544",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 705020,
			say = "Now that Bismarck has returned, what strategies:es Iron Blood employ? What is their organization like out in this vast desert of snow?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_544",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 705020,
			say = "Are their projects from the Friedrich era still ongoing? Has their stance on cooperating with us changed?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 705020,
			side = 2,
			bgName = "star_level_bg_544",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "These are just some of the things I'd like to know. Things you can't just ask through regularly scheduled messages.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_544",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 705020,
			say = "Thus, I want to be proactive and emphasize upfront that cooperation is in both our best interests.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 705020,
			side = 2,
			bgName = "star_level_bg_544",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "They won't have any reason to refuse help since, naturally, this follows Comrade Commander's principles.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_544",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 705020,
			say = "And: there's the mysterious enemy that attacked. I must look into that sooner or later anyway.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_544",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 702020,
			say = "Yeah, exactly!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_544",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 701120,
			say = "Basically, it's gettin' a lot of shit:ne at once. Efficient! However you cut it, there's no loss in Soyuz going.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_544",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 701110,
			say = "Hey! Look! There's a bunch of buildings over there! I think that's Iron Blood's base!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				}
			}
		},
		{
			bgName = "bg_xuejing_cg3",
			mode = 1,
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			sequence = {
				{
					"",
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_xuejing_cg3",
			hidePaintObj = True,
			say = "The girls cast their gazes to where Ognevoy pointed. There was a cluster of Iron Blood buildings, rising from the ice field like concrete shells.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Poltava",
			bgName = "bg_xuejing_cg3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It certainly looks like a military complex. Compare that to our little research station.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Svirepy",
			bgName = "bg_xuejing_cg3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Ain't it kinda overkill, though?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			side = 2,
			actorName = "Svirepy",
			bgName = "bg_xuejing_cg3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Like, ya:n't need a sturdy fortress in Antarctica. Siren attacks are so rare out here.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It seems unlikely that it's JUST a research station or a safety measure in case of a Siren invasion.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "(Something about this is fishy. Perhaps Friedrich is still in our branch...)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria",
			bgName = "bg_xuejing_cg3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Heehee. Something happened in Antarctica, Soyuz.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria",
			bgName = "bg_xuejing_cg3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Do you wanna know what?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Huh? What are you talking–",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			bgName = "bg_xuejing_cg4",
			mode = 1,
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			sequence = {
				{
					"",
					0
				}
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...about? What is this?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Pamiat'?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_xuejing_cg4",
			hidePaintObj = True,
			say = "Soyuz was standing completely still, yet the scenery around her changed in an instant.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_xuejing_cg4",
			hidePaintObj = True,
			say = "Pamiat' and the rest of her comrades were gone. In their place, a city made of steel towered in the distance.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_xuejing_cg4",
			hidePaintObj = True,
			say = "Its scale was almost unbelievable. It wasn't so much that the ice field surrounded the city, but rather that the city surrounded the ice field.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_xuejing_cg4",
			hidePaintObj = True,
			say = "Its border stretched from horizon to horizon, and it didn't stop there. Soyuz looked:wn and realized she was standing not on snow and rock, but rather on metallic pavement.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_xuejing_cg4",
			hidePaintObj = True,
			say = "She was on the edge of a city that was still growing, consuming the silver landscape around it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "(This entire expanse... has become a fortress.)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "(No... Could this be the original form of the place where we're standing now?)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_xuejing_cg4",
			hidePaintObj = True,
			say = "Soyuz could not help but be astonished by this sudden thought.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "(That's impossible... This cannot be real...)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_zhuiluo_2",
			hidePaintObj = True,
			say = "The scenery changed once more, allowing her no time to digest the thought.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = False,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = False,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_zhuiluo_2",
			hidePaintObj = True,
			say = "This time, the ground was ablaze and the city was crumbling.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_zhuiluo_2",
			hidePaintObj = True,
			say = "In the sky, a shimmering aurora. The daunting yet splendorous city had collapsed in the blink of an eye.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_zhuiluo_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 705020,
			say = "(What is this? Antarctica's essence? Its truth? Its... future?)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_zhuiluo_2",
			hidePaintObj = True,
			say = "Yet again, thoughts formed from the blank space in the back of her mind.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9702070,
			side = 2,
			bgName = "star_level_bg_1104",
			hidePaintObj = True,
			actorShadow = True,
			actorName = "Pamiat' Merkuria?",
			nameColor = "#A9F548FF",
			say = "Yes. This is my story. One of the possible futures for this continent.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = False,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = False,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_xuejing_cg3",
			hidePaintObj = True,
			dir = 1,
			actorName = "Sovetsky Soyuz",
			nameColor = "#A9F548FF",
			say = "Phew...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = False,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = False,
				alpha = {
					1,
					0
				}
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "(Was I just hallucinating? Everything is back to normal, looking as it should...)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Pamiat' Merkuria",
			bgName = "bg_xuejing_cg3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Hum-de-dum♪",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_xuejing_cg3",
			hidePaintObj = True,
			dir = 1,
			actorName = "Sovetsky Soyuz",
			nameColor = "#A9F548FF",
			say = "Pamiat', what exactly did you mean by \"your story\"?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_xuejing_cg3",
			hidePaintObj = True,
			dir = 1,
			actorName = "Pamiat' Merkuria",
			nameColor = "#A9F548FF",
			say = "My \"story\"? What are you talking about?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_xuejing_cg3",
			hidePaintObj = True,
			dir = 1,
			actorName = "Pamiat' Merkuria",
			nameColor = "#A9F548FF",
			say = "Me and Ognevoy were just having a chat. You must've misheard something I said.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Misheard? But...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_xuejing_cg3",
			hidePaintObj = True,
			dir = 1,
			actorName = "Poltava",
			nameColor = "#A9F548FF",
			say = "Are you alright, Soyuz? Are you hearing things?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_xuejing_cg3",
			hidePaintObj = True,
			dir = 1,
			actorName = "Sovetsky Soyuz",
			nameColor = "#A9F548FF",
			say = "Not just hearing things. I saw something, too.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_xuejing_cg3",
			hidePaintObj = True,
			dir = 1,
			actorName = "Poltava",
			nameColor = "#A9F548FF",
			say = "Audiovisual hallucinations, eh... You did touch that crystal earlier. Maybe that has something to: with it?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_xuejing_cg3",
			hidePaintObj = True,
			dir = 1,
			actorName = "Pamiat' Merkuria",
			nameColor = "#A9F548FF",
			say = "Could be! Her very rigging did get restructured. That takes a huge toll on your body and mind.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_xuejing_cg3",
			hidePaintObj = True,
			dir = 1,
			actorName = "Sovetsky Soyuz",
			nameColor = "#A9F548FF",
			say = "Yes... You're probably right.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_xuejing_cg3",
			hidePaintObj = True,
			dir = 1,
			actorName = "Ognevoy",
			nameColor = "#A9F548FF",
			say = "Are you sure you're okay? Should we stop and take a break?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Sovetsky Soyuz",
			bgName = "bg_xuejing_cg3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I'm fine, I promise. The base isn't far now, so let's get out of the cold.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
