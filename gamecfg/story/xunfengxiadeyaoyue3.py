return {
	fadeOut = 1.5,
	mode = 2,
	id = "XUNFENGXIADEYAOYUE3",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Invitation of the Wind\n\n<size=45>3 Diving Under Blue Skies</size>",
					1
				}
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			bgm = "story-richang-7",
			say = "The light above grows brighter, informing us that we'll reach the surface soon.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			say = "I gesture to my partner next to me and swim up to the surface.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = "mission_complete",
			say = "Phew...! We're back on the water's surface!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = "mission",
			say = "Commander, you look pale...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "I get it. If you're not used to diving, it's easy to get scared.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "I panicked a little when I started out, too.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = "main1",
			say = "You're okay now, Commander. Look at the skies above, and see that you're safe.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			withoutActorName = True,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "\"Actually, Enterprise, I'm not worried about myself. Look:wn.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			withoutActorName = True,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Her diving suit has a big hole in the thigh.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = "touch2",
			say = "Oh, this?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			withoutActorName = True,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "\"I heard you yelp during our dive, so I decided to have us exit early.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = "headtouch",
			say = "Heh, yeah, it was a surprise when I hit that rock.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "I felt better after I checked the damage, though.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "We weren't that deep, either. Just tearing my suit a little is no biggie.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "And see? I'm not even wounded.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = "headtouch",
			say = "Don't believe me? Check it out for yourself.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			withoutActorName = True,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Enterprise swims closer to me.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = "touch2",
			say = "See, I'm just fine.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = "mission",
			say = "I appreciate your concern, but it's not like I'm acting tough, or anything.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			withoutActorName = True,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Fortunately, Enterprise isn't hurt. All the scrape did was tear her wetsuit.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			withoutActorName = True,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Suddenly, I realize that she's gotten really close to me.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			withoutActorName = True,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "It's normal to be this close mid-dive, but she:esn't care even when we're on the water's surface.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = "main2",
			say = "Those colorful reefs, all the little sea creatures...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "It was just like a busy city, wasn't it?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "We think we know the sea well, because we're always fighting on it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Carrier-based craft might let us see beyond the horizon...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "But there's a whole other world hidden:wn there.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = "main1",
			say = "Is this how our submarines feel when they fight?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			withoutActorName = True,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "\"It's a shame that only they get to see certain sights. You need special gear to descend that far.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			withoutActorName = True,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "\"Even mass-produced submarines wouldn't let us take a leisurely look around.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "...Yeah, you're right.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Sitting in a windowless submarine, deep underwater...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "I'll pass. I'm built for fighting on the surface.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = "expedition",
			say = "Oh, sorry. I just can't stop talking about fighting.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Here I'd promised that we'd relax together... So much for that, huh?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Phew... Feel better now, Commander?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "That's enough of a break. Let's start our next dive, alright?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			withoutActorName = True,
			nameColor = "#A9F548FF",
			live2d = "touch2",
			say = "\"I have plenty of stamina and oxygen left, but what about your wetsuit?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = "mission",
			say = "...Now that you mention it, this is kind of a problem.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Diving in a damaged wetsuit is against safety regulations.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Let's go back to the coast,:.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = "headtouch",
			say = "It:esn't matter if we're on the surface or underwater. Me, I can relax as long as you're here.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = "main2",
			say = "What say you, Commander?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			withoutActorName = True,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "\"I feel the same, of course.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Heheh. Let's find another way to enjoy the rest of our time together.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			live2d = "touch",
			say = "The sights underwater may be pretty, but they:n't beat the world above.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 137060,
			side = 2,
			bgName = "star_level_bg_106",
			hidePaintObj = True,
			withoutActorName = True,
			nameColor = "#A9F548FF",
			live2d = True,
			say = "Enterprise's smile is as clear as the blue sky. She offers me her hand—and I hold on tight.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
