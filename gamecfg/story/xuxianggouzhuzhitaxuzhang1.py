return {
	fadeOut = 1.5,
	mode = 2,
	id = "XUXIANGGOUZHUZHITAXUZHANG1",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			bgm = "story-italy",
			side = 2,
			bgName = "bg_map_maliyana",
			dir = 1,
			bgmDelay = 1,
			say = "Somewhere At Sea",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 603040,
			side = 2,
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "We're about half a day away from the Port. Oil reserves are over half, ammunition is plentiful, but our supply of food is...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 601070,
			say = "Yikes... We're almost there, and you're still counting beans?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 601070,
			say = "A quartermaster you may be, but isn't this a bit too much dedication to your job?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 603040,
			say = "Nothing is more important than keeping tabs on our supplies. Victory or defeat during battle hinges upon it – and so:es your lunch today.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 601070,
			say = "Hwuh?! We:n't even have enough supplies for lunch?!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 603040,
			say = "Heh heh... Don't worry. Because I've been rationing so diligently, we'll have enough food left over for three more days.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 601070,
			say = "...How: we have more days' worth of food NOW than before we set sail?!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 603040,
			say = "...Bean counting is quite miraculous, is it not?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 607020,
			say = "...Spaghetti.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 601070,
			side = 2,
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Huh? Oh, Impero, did you want spaghetti for lunch?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 601070,
			say = "Captain Pompey the Great happens to agree with you, so let us have spaghetti!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 607020,
			say = "Carrier-based spaghetti. We'll shoot the spaghetti out from my planes...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 601070,
			say = ".........Huh?!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 601070,
			say = "What the heck is that! Nobody asked for this high-level playing with your food!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 601070,
			say = "And why are you sticking spaghetti into your planes?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 607020,
			say = "The planes help keep the spaghetti warm.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 607020,
			say = "Because I still need to sleep and bathe... Spaghetti fired from my planes can be ready to eat at any time...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 603040,
			side = 2,
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Impero came up with something incredible again...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 601070,
			say = "More like incredibly stupid... Well, I must admit that the ambition behind the idea has moved Captain Pompey the Great.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 603040,
			say = "Huh...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 607020,
			say = "Uhh......",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 603040,
			side = 2,
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "What is it this time? Flying pizza?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 607020,
			say = "...I've lost track of my planes.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 601070,
			say = "What?! Could it be a Siren attack? What was the last bit of data you got from them before you lost their signals?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 607020,
			say = "A strange city and tower... That's the last thing I got from them.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 603040,
			side = 2,
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Sounds a bit like a Mirror Sea,:esn't it...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 603040,
			say = "A Mirror Sea appearing only half a day away from the Port is definitely noteworthy.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 603040,
			side = 2,
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Let's request backup from the Port first,: turn around to scout out the situation.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 601070,
			say = "Well said! However, Captain Pompey the Great and her valiant comrades will make quick work of any Sirens even before reinforcements show up!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 607020,
			say = "How dare they shoot:wn Impero's planes... How dare they shoot:wn my SPAGHETTI! We must make them pay!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_map_maliyana",
			say = "Port - Communication Room",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 601020,
			say = "Incoming communication! Seems like Pompeo and the others will arrive back at the Port in time!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_map_maliyana",
			say = "Carabiniere played the contents of the message, but was completely unprepared for what she heard.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 601020,
			say = "A s-support request?!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 601020,
			side = 2,
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Ehh?! If you need backup, you should've contacted the Commander directly!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 601020,
			say = "This is how you: it, right... Okay, sent the info over.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 601020,
			say = "...Alright, now to wait for the Commander's response. Hopefully Pompeo and the others aren't in any real trouble.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			actorName = "Communicator",
			say = "BEEP––––",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 601020,
			side = 2,
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Confirming receipt, huh. Guess the secretary today is on top of it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			actorName = "Communicator",
			say = "BEEP BEEP––––",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			actorName = "Communicator",
			say = "BEEP BEEP BEEP––––",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_map_maliyana",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 601020,
			say = "Huh? Wh-why am I getting multiple confirmation replies...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_map_maliyana",
			dir = 1,
			blackBg = True,
			say = "Carabiniere suddenly shuddered when she thought of a potential possibility––",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
