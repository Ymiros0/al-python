return {
	fadeOut = 1.5,
	mode = 2,
	id = "XUXIANGGOUZHUZHITAXUZHANG7",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			say = "Abnormal Sea Area - Near the Center",
			side = 2,
			bgName = "bg_story_tower",
			dir = 1,
			bgmDelay = 1,
			bgm = "theme-arbitrator-tower",
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_tower",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 107070,
			say = "Wait, did you girls also get the message sent from the port?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403030,
			side = 2,
			bgName = "bg_story_tower",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Sure did. You should've seen my sister's face change colors as she dragged me here at breakneck speed~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403010,
			side = 2,
			bgName = "bg_story_tower",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Huuh? My face did no such thing!!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 403030,
			side = 2,
			bgName = "bg_story_tower",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Look, you're:ing it again~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_story_tower",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Alright, we've all seen enough of this comedic duo––",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_story_tower",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Having that said, I'm surprised we all showed up without coordinating our efforts, and even covered different areas on our way here.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_story_tower",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Usually, the Commander provides detailed instructions when arranging these reinforcement operations.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_tower",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 107070,
			say = "Instead, this time...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_tower",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 107070,
			say = "We got a broadcast from the port rather than direct orders from the Commander.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403030,
			side = 2,
			bgName = "bg_story_tower",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Doesn't seem quite right,:es it? Shouldn't the Commander know that we're on vacation?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403010,
			side = 2,
			bgName = "bg_story_tower",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Huh? Do you even know what you're saying, Eugen?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403030,
			side = 2,
			bgName = "bg_story_tower",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "What'd you think about the enemies we encountered along the way?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403010,
			side = 2,
			bgName = "bg_story_tower",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Weak! Total trash not even worth mentioning!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403010,
			side = 2,
			bgName = "bg_story_tower",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Wait... I think I see what you're getting at...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403010,
			side = 2,
			bgName = "bg_story_tower",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "For enemies this weak, it shouldn't be necessary to call all of us to one place...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403030,
			side = 2,
			bgName = "bg_story_tower",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Do you remember who was on duty in the communications room before we left?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403010,
			side = 2,
			bgName = "bg_story_tower",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Let's see... Carabiniere from Sardegna?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_story_tower",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Carabiniere must've received a backup request from Impero...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_story_tower",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "And she forwarded the message to all of us in a hurry, and that's the reason we're all here now?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403030,
			side = 2,
			bgName = "bg_story_tower",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "That sounds about right~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403010,
			side = 2,
			bgName = "bg_story_tower",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "...You've got to be kidding me. Is this some kind of joke?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 403030,
			side = 2,
			bgName = "bg_story_tower",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Now, now, getting angry is bad for your skin, sis. Since we're here already, why not have a look around?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403030,
			side = 2,
			bgName = "bg_story_tower",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I'm sure we're all equally interested in that strange-looking tower in the middle of this place.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403010,
			side = 2,
			bgName = "bg_story_tower",
			hidePaintObj = True,
			dir = 1,
			blackBg = True,
			nameColor = "#a9f548",
			say = "...Let's get moving:! When we're:ne here, I'm going to give those Sardegna girls a good lecturing!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
