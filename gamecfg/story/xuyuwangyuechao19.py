return {
	fadeOut = 1.5,
	mode = 2,
	id = "XUYUWANGYUECHAO19",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			side = 2,
			stopbgm = True,
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "In the dark, cold seal, monsters made of black mist shrieked and roared ceaselessly.",
			bgm = "battle-unknown-approaching",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			say = "They jumped between the ethereal sea and floating rocks, launched out of the ground by the uprooting of the Prime Sakura, searching for the targets of their ire.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = 303190,
			side = 2,
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actorName = "The Great Sage",
			say = "............",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			say = "After the signal flare from before, she'd used the seal's divine light to find and rescue the trio.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			say = "The light had repelled the black mist and brought them to this safe haven for the time being.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = 303190,
			side = 2,
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actorName = "The Great Sage",
			say = "I did warn them not to try to walk around, so I'm sure they will be safe for now.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = 303190,
			side = 2,
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actorName = "The Great Sage",
			say = "Next...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			say = "She had a general idea of their remaining friends' whereabouts, but there was no way to narrow it:wn and help them.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = 303190,
			side = 2,
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actorName = "The Great Sage",
			say = "That sound... Is that aircraft?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			say = "A single carrier-based aircraft, hardly useful in the black mist's:main, quickly lost control and crash-landed.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = 303190,
			side = 2,
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actorName = "The Great Sage",
			say = "Which direction...? Aha, that way.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			say = "She stepped into the ethereal waters and used the sanctuary's power to rush to the shipgirls' aid.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900402,
			side = 2,
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "I-I've lost another carrier-based craft!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 4,
			nameColor = "#A9F548FF",
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			side = 2,
			actor = 900402,
			say = "Urk... I:n't have any more recon craft to send out...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 305130,
			say = "This is rough... It's dangerous, but I've got no choice but to act.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 305130,
			say = "But first... Let's deal with what's right in front of us!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900402,
			side = 2,
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "S-someone's coming this way!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 305130,
			say = "I'll give 'em the trouncing of a lifetime! Which way, Zuihou?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			side = 2,
			actor = 900402,
			say = "Behind you! Six o' clock!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 305130,
			say = "Behind me?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			say = "Noticing a presence in an unexpected direction, Owari used her rigging to whip around in a hurry.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_yunxian_3",
			dir = 1,
			soundeffect = "event./battle/boom2",
			actor = 305130,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Fiiiire!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashN = {
				color = {
					1,
					1,
					1,
					1
				},
				alpha = {
					{
						0,
						1,
						0.2,
						0
					},
					{
						1,
						0,
						0.2,
						0.2
					},
					{
						0,
						1,
						0.2,
						0.4
					},
					{
						1,
						0,
						0.2,
						0.6
					}
				}
			},
			dialogShake = {
				speed = 0.09,
				x = 8.5,
				number = 2
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			say = "*PING*",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			say = "Her shells were deflected by an invisible barrier, covering the surroundings in smoke.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "???",
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "*cough* *cough*...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305130,
			side = 2,
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...A shipgirl?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "???",
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Heehee. Bit of a rude welcome, I have to say.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 303190,
			nameColor = "#A9F548FF",
			bgName = "bg_yunxian_3",
			side = 2,
			withoutActorName = True,
			say = "She floated above the water without rigging, but there was no:ubt she was a shipgirl just like Owari and Zuihou.",
			hidePaintEquip = True,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900402,
			nameColor = "#A9F548FF",
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			side = 2,
			say = "Sorry, sorry!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305130,
			side = 2,
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "(I think I saw this person in that illusion... Great Sage, or something?)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			say = "As she approached the Prime Sakura, Owari's memory of the illusion became more vivid.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305130,
			actorName = "Owari",
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			side = 2,
			say = "Great Sage...?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			actor = 303190,
			actorName = "The Great Sage",
			nameColor = "#A9F548FF",
			say = "That is me. I've taken your friends to a safe place, and now, I'll have you two come with me as well.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			actor = 303190,
			actorName = "The Great Sage",
			nameColor = "#A9F548FF",
			say = "It's the last pure land remaining in this seal. With my power, I will protect you all.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			say = "Once she finished talking, light shone through the sky and enveloped Owari and Zuihou.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			say = "However, the pillar of light flickered multiple times before disappearing.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900402,
			nameColor = "#A9F548FF",
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			side = 2,
			say = "...Was that a light signal, or...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			actor = 303190,
			actorName = "The Great Sage",
			nameColor = "#A9F548FF",
			say = "Unfortunately, straying too far away makes it consume far too much power... My remaining strength isn't enough.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 303190,
			nameColor = "#A9F548FF",
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			side = 2,
			actorName = "The Great Sage",
			say = "We have no choice. I'll just have to guide you myself.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_yunxian_3",
			hidePaintObj = True,
			actor = 303190,
			actorName = "The Great Sage",
			nameColor = "#A9F548FF",
			say = "Come with me.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
