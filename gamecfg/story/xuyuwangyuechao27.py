return {
	fadeOut = 1.5,
	mode = 2,
	id = "XUYUWANGYUECHAO27",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			bgName = "bg_yunxian_cg2",
			bgm = "story-unzen-heart",
			sequence = {
				{
					"",
					0
				}
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			side = 2,
			actorName = "Shimanto",
			bgName = "bg_yunxian_cg2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Behold– the might of the Dragon God!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_cg2",
			hidePaintObj = True,
			say = "The concept of the seal had weakened, and Unzen now confronted the black mist with Shimanto's support.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_cg2",
			hidePaintObj = True,
			say = "Shimanto protected herself with her shikigami dragon, supporting Unzen from afar as she sailed through the void sea.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_cg2",
			hidePaintObj = True,
			say = "Despite the paper-like appearance of her rigging, Shimanto was easily able to repel the attacks from the black mist.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Shimanto",
			bgName = "bg_yunxian_cg2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Unzen, now!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_cg2",
			factiontag = "Heart of Watatsumi",
			dir = 1,
			soundeffect = "event./battle/boom2",
			actorName = "Unzen",
			hidePaintObj = True,
			say = "Now, may this place be cleansed by my flames!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashN = {
				color = {
					1,
					1,
					1,
					1
				},
				alpha = {
					{
						0,
						1,
						0.2,
						0
					},
					{
						1,
						0,
						0.2,
						0.2
					},
					{
						0,
						1,
						0.2,
						0.4
					},
					{
						1,
						0,
						0.2,
						0.6
					}
				}
			},
			dialogShake = {
				speed = 0.09,
				x = 8.5,
				number = 2
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_cg2",
			hidePaintObj = True,
			say = "The explosive impacts of her artillery fire caused the shadows to reel back.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Shimanto",
			bgName = "bg_yunxian_cg2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "The corruption is gathering... It's going to unleash its full might!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_cg2",
			hidePaintObj = True,
			say = "The serpentine heads of the beast created by the creeping shadows continued to grow longer and longer.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_cg2",
			hidePaintObj = True,
			say = "The leviathan's coils continued to grow and expand, casting a profound darkness across all who turned who would turn their heads upward at it to wallow in their utter insignificance.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Shimanto",
			bgName = "bg_yunxian_cg2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "No effect. In that case...!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_cg2",
			hidePaintObj = True,
			say = "The Dragon God struck again, but her attacks were not even enough to cause a ripple in the infinite darkness.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_cg2",
			hidePaintObj = True,
			say = "The next moment, another of the leviathan's heads erupted from the water's surface, bearing its unfathomable form:wn upon the shipgirls there.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_cg2",
			hidePaintObj = True,
			say = "The beast seemed inevitable.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Shimanto",
			bgName = "bg_yunxian_cg2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Kh...?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_cg2",
			hidePaintObj = True,
			say = "The paper dragon placed itself in the path of the mist's onslaught, but shattered the moment it came into contact with its yawning maw.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_cg2",
			hidePaintObj = True,
			say = "All seemed to be futile–",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "???",
			bgName = "bg_yunxian_5",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "......",
			bgm = "story-unzen-heart",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			side = 2,
			actorName = "???",
			bgName = "bg_yunxian_5",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Impurity be purged!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_5",
			hidePaintObj = True,
			say = "In a flash, only a single blade of cold steel held the threshold before the abyss.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 399010,
			side = 2,
			bgName = "bg_yunxian_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Kh...?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 399010,
			side = 2,
			bgName = "bg_yunxian_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Back off, you two!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_5",
			hidePaintObj = True,
			say = "Using the massive beast's momentum, Ibuki parried the leviathan's head aside and sent it hurtling into a nearby island.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_yunxian_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 399060,
			say = "Who...?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_5",
			hidePaintObj = True,
			say = "Just as quickly as she had appeared to save the day, Ibuki disappeared beneath the eruption of debris, leaving the other two to scramble for cover.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_5",
			hidePaintObj = True,
			say = "Unzen and Shimanto looked at each other with tacit agreement. They had to find a way to mount a counterattack, and fast.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_yunxian_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 399060,
			say = "Lady Nagato:esn't have much time left... Unzen,: you have a plan?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_5",
			hidePaintObj = True,
			say = "When asked, Unzen thought for a moment before releasing her rigging.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_5",
			hidePaintObj = True,
			say = "That part of her being became butterflies of light that disappeared into her left eye.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 303190,
			side = 2,
			bgName = "bg_yunxian_5",
			factiontag = "Heart of Watatsumi",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Shimanto, I have a request.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 399060,
			side = 2,
			bgName = "bg_yunxian_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Sure. I'll: whatever it takes!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 303190,
			side = 2,
			bgName = "bg_yunxian_5",
			factiontag = "Heart of Watatsumi",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "If we can break the \"core\" of the black mist, we'll be able to fend it off long enough to restore the seal.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 303190,
			side = 2,
			bgName = "bg_yunxian_5",
			factiontag = "Heart of Watatsumi",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "But that involves jumping into the depths of that darkness. We'll only have one chance.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 303190,
			side = 2,
			bgName = "bg_yunxian_5",
			factiontag = "Heart of Watatsumi",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "I will need to call upon the Prime Sakura to make that moment count–",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_yunxian_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 399060,
			say = "Understood. Before that though, I'll: my part–",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 399060,
			side = 2,
			bgName = "bg_yunxian_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...and clear a way to the core!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
