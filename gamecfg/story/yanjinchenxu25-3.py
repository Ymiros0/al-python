return {
	id = "YANJINCHENXU25-3",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			expression = 2,
			side = 2,
			nameColor = "#A9F548FF",
			actor = 205140,
			dir = 1,
			stopbgm = True,
			say = "What?! It's repairing itself using armour plates torn off from its mass-produced ships!",
			bgm = "battle-whaling-normal",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			actor = 202330,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Expending as many troops as necessary to win... Fitting for an embodiment of war.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			actor = 202330,
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It's very bad news for us, however. We cannot hope to win a war of attrition.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			nameColor = "#A9F548FF",
			side = 2,
			actor = 201360,
			dir = 1,
			say = "Even if we:, we won't have any bloody ammo left to beat the last Horseman!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			nameColor = "#A9F548FF",
			side = 2,
			actor = 206020,
			dir = 1,
			say = "It:esn't matter! We need to defeat War before we can even start thinking about defeating Conquest!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			nameColor = "#A9F548FF",
			side = 2,
			actor = 207070,
			dir = 1,
			say = "No. In fact, now is the time we SHOULD be thinking about Conquest!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 207070,
			side = 2,
			nameColor = "#A9F548FF",
			dir = 1,
			say = "War as a concept is not a goal; it serves a higher purpose. That purpose is conquest.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			nameColor = "#A9F548FF",
			side = 2,
			actor = 207070,
			dir = 1,
			say = "As long as the Horseman of Conquest is incapacitated, nothing is currently expressing the concept of conquest in this area.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			nameColor = "#A9F548FF",
			side = 2,
			actor = 207070,
			dir = 1,
			say = "But what if I become a conduit for the concept of Conquest?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			nameColor = "#A9F548FF",
			side = 2,
			actor = 207070,
			dir = 1,
			say = "In other words, if I can successfully \"conquer\" these mass-produced vessels, I can diminish the concept of War.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			nameColor = "#A9F548FF",
			side = 2,
			actor = 207070,
			dir = 1,
			say = "What: you all think? I believe La Galissonnière had the same idea of using Conquest to weaken War.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			paintingNoise = True,
			actor = 900353,
			dir = 1,
			nameColor = "#ffa500",
			say = "...Uh, I wasn't thinking of anything philosophical; I was just gonna make them fight.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900353,
			side = 2,
			nameColor = "#ffa500",
			paintingNoise = True,
			dir = 1,
			say = "Your idea is certainly gutsy, I'll give you that. And, I:n't see anything wrong with it at a glance, as long as you're fine with the risks.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			nameColor = "#A9F548FF",
			side = 2,
			actor = 805010,
			dir = 1,
			say = "I'm against it. Channeling one of the Mirror Sea's concepts is far too dangerous. I can't let you shoulder such a burden for the mistakes we made.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 805010,
			side = 2,
			nameColor = "#A9F548FF",
			dir = 1,
			say = "If it's our only choice, I shall be the one to: it rather than you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			nameColor = "#A9F548FF",
			side = 2,
			actor = 207070,
			dir = 1,
			say = "Sorry, Cardinal, but I think your strong faith inhibits you. You can't truly become a vessel for Conquest. It has to be:ne by a nonbeliever like me.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			nameColor = "#A9F548FF",
			side = 2,
			actor = 207070,
			dir = 1,
			say = "Don't worry, I'll be fine. I have Her Majesty's protection.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			nameColor = "#A9F548FF",
			side = 2,
			actor = 207070,
			dir = 1,
			say = "If the worst thing happens and I start to lose control, just save me like you did the last time.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			nameColor = "#A9F548FF",
			side = 2,
			actor = 805010,
			dir = 1,
			say = "But...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			nameColor = "#A9F548FF",
			side = 2,
			actor = 207070,
			dir = 1,
			say = "Hero, give me your bow!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			say = "Implacable took the bow – the symbol of Conquest – and drew the string, training her sights on the Horseman of War.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			nameColor = "#A9F548FF",
			side = 2,
			actor = 207070,
			dir = 1,
			say = "\"I watched as the Lamb opened the first of the seven seals, and there before me was a white horse.\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			nameColor = "#ffa500",
			side = 2,
			actor = 207070,
			dir = 1,
			say = "\"Its rider held a bow, and he was given a crown, and he rode out as a conqueror bent on conquest.\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			nameColor = "#ffa500",
			side = 2,
			actor = 207070,
			dir = 1,
			say = "\"With this bow, he would smite those who opposed him, until none were left!\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
