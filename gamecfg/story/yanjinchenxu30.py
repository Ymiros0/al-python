return {
	fadeOut = 1.5,
	mode = 2,
	id = "YANJINCHENXU30",
	once = True,
	fadeType = 2,
	scripts = {
		{
			side = 2,
			nameColor = "#ffa500",
			bgName = "bg_burningsea_1",
			stopbgm = True,
			say = "Unknown location",
			bgmDelay = 1,
			bgm = "battle-whaling-normal",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#ffa500",
			side = 2,
			bgName = "bg_burningsea_1",
			say = "KABOOM–––!!",
			soundeffect = "event./battle/boom2",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashN = {
				color = {
					1,
					1,
					1,
					1
				},
				alpha = {
					{
						0,
						1,
						0.2,
						0
					},
					{
						1,
						0,
						0.2,
						0.2
					},
					{
						0,
						1,
						0.2,
						0.4
					},
					{
						1,
						0,
						0.2,
						0.6
					}
				}
			},
			dialogShake = {
				speed = 0.09,
				x = 8.5,
				number = 2
			}
		},
		{
			actor = 9702030,
			side = 2,
			bgName = "bg_burningsea_1",
			nameColor = "#ffa500",
			dir = 1,
			say = "Sorry to keep you waiting. How's the situation?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_burningsea_1",
			actor = 9701010,
			dir = 1,
			nameColor = "#ffa500",
			say = "I was a step too late. The \"whale\" got away, and it's only the small fry left now.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_burningsea_1",
			actor = 9702030,
			dir = 1,
			nameColor = "#ffa500",
			say = "Unfortunate. Did you manage to retrieve anything else?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_burningsea_1",
			actor = 9701010,
			dir = 1,
			nameColor = "#ffa500",
			say = "Yes, I have everything here. Want to take a look?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_burningsea_1",
			actor = 9702030,
			dir = 1,
			nameColor = "#ffa500",
			say = "...I knew it. The \"whale's\" seemingly random movements: have a purpose after all.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_burningsea_1",
			actor = 9701010,
			dir = 1,
			nameColor = "#ffa500",
			say = "Which makes me even more confused as to what they're trying to:. Can the Magister's plan still proceed in this state?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9701010,
			side = 2,
			bgName = "bg_burningsea_1",
			nameColor = "#ffa500",
			dir = 1,
			say = "Or, did the Magister also account for all this?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_burningsea_1",
			actor = 9702030,
			dir = 1,
			nameColor = "#ffa500",
			say = "Why are you asking me?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_burningsea_1",
			actor = 9701010,
			dir = 1,
			nameColor = "#ffa500",
			say = "Well, you have a pretty good track record and whatnot...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_burningsea_1",
			actor = 9702030,
			dir = 1,
			nameColor = "#ffa500",
			say = "Isn't there someone who'd be more suitable to answer that than me? For example, a certain \"Miss D.\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_burningsea_1",
			actor = 9701010,
			dir = 1,
			nameColor = "#ffa500",
			say = "I've already tried. She won't tell us anything unless we catch the \"whale.\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9702030,
			side = 2,
			bgName = "bg_burningsea_1",
			nameColor = "#ffa500",
			dir = 1,
			say = "I see. I'll leave you to it:.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_burningsea_1",
			actor = 9701010,
			dir = 1,
			nameColor = "#ffa500",
			say = "Mmhmm.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_burningsea_1",
			actor = 9702030,
			dir = 1,
			nameColor = "#ffa500",
			say = "But, let's head back first. Her Majesty is waiting for us.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			bgName = "bg_burningsea_1",
			mode = 1,
			spacing = 30,
			stopbgm = True,
			asideType = 4,
			bgmDelay = 0.5,
			bgm = "theme-shallowoftheworld",
			rectOffset = {
				100,
				100,
				400,
				400
			},
			effects = {
				{
					active = True,
					name = "yuanzhou_juqing02"
				}
			},
			sequence = {
				{
					"\"All beings in creation have both beginning and end.\"",
					1
				},
				{
					"\"Ashes to ashes, dust to dust.\"",
					2
				},
				{
					"\"But, I shall continue to live.\"",
					3
				},
				{
					"\"For:ing so is the only way to achieve my purpose.\"",
					4
				},
				{
					"\"No matter how desperate the odds, it must be:ne.\"",
					5
				},
				{
					"\"––The Truth must be found.\"",
					6
				},
				{
					"\"For that pursuit, for that hunt, one must become as a beast.\"",
					7
				},
				{
					"\"If this is a price you are willing to pay,\"",
					9
				},
				{
					"\"Then I may accept your allegiance.\"",
					10
				},
				{
					"\"Otherwise... Hmm, let's see...\"",
					11
				},
				{
					"\"Yes, offer unto me your prayers.\"",
					12
				}
			}
		},
		{
			blackBg = True,
			mode = 1,
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			effects = {
				{
					active = True,
					name = "yanjinchenxu"
				}
			},
			sequence = {
				{
					"",
					1
				}
			}
		}
	}
}
