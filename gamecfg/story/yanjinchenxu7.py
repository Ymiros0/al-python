return {
	fadeOut = 1.5,
	mode = 2,
	id = "YANJINCHENXU7",
	once = True,
	fadeType = 2,
	scripts = {
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_task",
			nameColor = "#A9F548FF",
			dir = 1,
			bgmDelay = 2,
			bgm = "theme-vichy-church",
			actor = 201360,
			stopbgm = True,
			say = "*pant*... *pant*...",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_task",
			actor = 201360,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Is there no end to these damn Pawns?! I'm working up a sweat here!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_task",
			actor = 201360,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Not that a bunch of mooks could ever dream of beating me, but still!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_story_task",
			actor = 202330,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "While I wouldn't quite say we're losing, our weapons will wear out sooner or later if we continue fighting in these close quarters.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_task",
			actor = 207070,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Mhm. Moreover, I need more room to launch my planes. Even that armoury would suffice.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_task",
			actor = 207070,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Do you think we could lead the pawns in that direction?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_task",
			actor = 202330,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Certainly. If need be, we could also break through the wall there to retreat into the water.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_story_task",
			actor = 202330,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Our riggings are designed for naval combat, not ground combat, after all.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_story_task",
			actor = 207070,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Good point. Every enemy we take out is one fewer for Argus and Royal Oak to deal with.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 207070,
			side = 2,
			bgName = "bg_story_task",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Although, we wouldn't want to draw more enemy fire than we already:.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			effects = {
				{
					active = True,
					name = "speed"
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_task",
			actor = 207070,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "At any rate, let's go! To the hall we came from!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_task",
			say = "Basilica Blanc - Indoors",
			effects = {
				{
					active = False,
					name = "speed"
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_story_task",
			actor = 201360,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Move it, losers! Hero coming through!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_task",
			say = "KABOOOM!",
			soundeffect = "event./battle/boom2",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashN = {
				color = {
					1,
					1,
					1,
					1
				},
				alpha = {
					{
						0,
						1,
						0.2,
						0
					},
					{
						1,
						0,
						0.2,
						0.2
					},
					{
						0,
						1,
						0.2,
						0.4
					},
					{
						1,
						0,
						0.2,
						0.6
					}
				}
			},
			dialogShake = {
				speed = 0.09,
				x = 8.5,
				number = 2
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_story_task",
			actor = 207070,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Nice work. You have a knack for punching holes through walls and hordes alike.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_task",
			actor = 201360,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Well, yeah! It comes with the title!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_task",
			actor = 207070,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Of course. Does anyone have a backup plan if the Pawns:n't pursue us?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_story_task",
			actor = 202330,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Isn't that for you to consider, Lady Implacable? You are our leader, aren't you?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_task",
			actor = 202330,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Although, we may not need one as they seem to be following us. Keep advancing and I shall cover your back.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_task",
			actor = 207070,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Got it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			stopbgm = True,
			bgName = "bg_firedust_2",
			nameColor = "#A9F548FF",
			say = "Basilica Blanc - Coastline",
			bgmDelay = 1,
			bgm = "theme-richelieu",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_firedust_2",
			say = "KABOOOM!",
			soundeffect = "event./battle/boom2",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashN = {
				color = {
					1,
					1,
					1,
					1
				},
				alpha = {
					{
						0,
						1,
						0.2,
						0
					},
					{
						1,
						0,
						0.2,
						0.2
					},
					{
						0,
						1,
						0.2,
						0.4
					},
					{
						1,
						0,
						0.2,
						0.6
					}
				}
			},
			dialogShake = {
				speed = 0.09,
				x = 8.5,
				number = 2
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_firedust_2",
			actor = 201360,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Bosh! There's our path to the ocean~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_firedust_2",
			actor = 207070,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Thanks, Hero. Now we can fight unrestrained.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_firedust_2",
			actor = 207070,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "No pity, no mercy. Let us show these mindless Pawns our True strength!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_firedust_2",
			say = "KABOOOM!",
			soundeffect = "event./battle/boom2",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashN = {
				color = {
					1,
					1,
					1,
					1
				},
				alpha = {
					{
						0,
						1,
						0.2,
						0
					},
					{
						1,
						0,
						0.2,
						0.2
					},
					{
						0,
						1,
						0.2,
						0.4
					},
					{
						1,
						0,
						0.2,
						0.6
					}
				}
			},
			dialogShake = {
				speed = 0.09,
				x = 8.5,
				number = 2
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_firedust_2",
			actor = 805010,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Well said. Waste neither pity nor mercy on them, for they will show you none.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "bg_firedust_2",
			actor = 207070,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Cardinal?! Is that you?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_firedust_2",
			paintingNoise = True,
			dir = 1,
			actor = 805010,
			nameColor = "#A9F548FF",
			say = "Yes. It's a pleasure to see you again, Implacable.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 207070,
			side = 2,
			bgName = "bg_firedust_2",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "You're here sooner than Her Majesty predicted! Thank goodness for that.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_firedust_2",
			paintingNoise = True,
			dir = 1,
			actor = 206020,
			nameColor = "#A9F548FF",
			say = "She, well... also rescued us. Cardinal Richelieu was the first to respond to our SOS.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_firedust_2",
			paintingNoise = True,
			dir = 1,
			actor = 206020,
			nameColor = "#A9F548FF",
			say = "If she hadn't come along, Royal Oak and I likely wouldn't be here right now.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_firedust_2",
			paintingNoise = True,
			dir = 1,
			actor = 205140,
			nameColor = "#A9F548FF",
			say = "Forgive me, Implacable... I've put the Royal Knights to shame.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_firedust_2",
			actor = 207070,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It's alright. I'm just glad you're both in one piece.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_firedust_2",
			actor = 207070,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Cardinal, if I may ask, what brought you to this remote island in the first place?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_firedust_2",
			paintingNoise = True,
			dir = 1,
			actor = 805010,
			nameColor = "#A9F548FF",
			say = "I shall explain later. For now, we must focus on defeating the enemy in front of us.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_firedust_2",
			paintingNoise = True,
			dir = 1,
			actor = 805010,
			nameColor = "#A9F548FF",
			say = "Let us smite these wicked:ppelgangers who dare violate the sanctity of the Iris's territory!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
