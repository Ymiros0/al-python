return {
	fadeOut = 1.5,
	mode = 2,
	id = "YINGGELAHAN4",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Searching for the Light\n\n<size=45>4 The Surprise</size>",
					1
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_148",
			say = "A few days later – Ingraham's makeshift workshop",
			bgmDelay = 2,
			bgm = "story-richang-1",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101480,
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "That's one job taken care of. Now onto the next thing...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 101480,
			say = "Umm... What:es it mean to me?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			effects = {
				{
					active = True,
					name = "memoryFog"
				}
			}
		},
		{
			actor = 101480,
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I'm not sure. I just want to get things fixed up ASAP so I can move on to fixing the next thing.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_task_2",
			say = "\"What: you think about the way people react after you finish repairing things for them?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101480,
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Well, I normally notify people as soon as I finish the repairs.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 101480,
			say = "As for their reactions... That's not something I usually pay attention to.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101480,
			side = 2,
			bgName = "star_level_bg_148",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...Maybe I'll deliver it myself this time and see how she reacts?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			effects = {
				{
					active = False,
					name = "memoryFog"
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_148",
			say = "The reassembled shield is casually lying on the workbench, along with a few other completed objects.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_148",
			say = "Ingraham scoops them up quietly, and turns to leave the workshop.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			bgm = "story-richang-1",
			side = 2,
			bgName = "star_level_bg_109",
			nameColor = "#A9F548FF",
			say = "*knock* *knock*",
			bgmDelay = 2,
			soundeffect = "event./ui/knockdoor1",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 102260,
			side = 2,
			bgName = "star_level_bg_109",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Coming~!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_109",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 102260,
			say = "Hm? Ingraham, is that you? Wait a sec, is that...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101480,
			side = 2,
			bgName = "star_level_bg_109",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Mhm. Sorry it took so long. I've finished repairing your shield.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_109",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 101480,
			say = "Want me to help you put it on?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_109",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 102260,
			say = "If you:n't mind, yes please!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_109",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 101480,
			say = "You just need to buckle it on like this, and:... Try waving your arm around a bit to see if it wobbles.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 102260,
			side = 2,
			bgName = "star_level_bg_109",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It feels perfect; no problems at all! Thank you so much!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_109",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 101480,
			say = "Piece of cake. Oh, and there's one more thing I need to tell you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101480,
			side = 2,
			bgName = "star_level_bg_109",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Do you feel that little bump right by your little finger? Go ahead and press it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_109",
			say = "ZAM!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_109",
			say = "The shield suddenly:ubles in size, accompanied by the slick sound of metal rubbing together.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_109",
			say = "It all happens in a flash, before Reno can process what even happened.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101480,
			side = 2,
			bgName = "star_level_bg_109",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...That seems good enough.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_109",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 101480,
			say = "By the way, because the original shield used metals that were too unwieldy, I remade the frame using carbon fibers.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_109",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 101480,
			say = "The metallic sound is just a sound effect. If you find it too jarring, you can hit this switch to disable the speaker.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_109",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 101480,
			say = "In addition to the automatic expansion function I added, you can also retract it using this switch by your index finger.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101480,
			side = 2,
			bgName = "star_level_bg_109",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Hold it:wn for three seconds, and it'll retract automatically.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_109",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 101480,
			say = "Reno...? Are you listening?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_109",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 101480,
			say = "Reno?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_109",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 102260,
			say = "You made the automatic transformation... a real thing?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			},
			effects = {
				{
					active = True,
					name = "speed"
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_109",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 102260,
			say = "Sorry, sorry, this is just... too much for me to take in all at once!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			},
			effects = {
				{
					active = False,
					name = "speed"
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_109",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 102260,
			say = "Is that seriously the same toy shield I gave you earlier? You didn't jump into some futuristic dimension to buy a real one for me, did you?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_109",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 101480,
			say = "...I didn't: anything except add a few simple modifications. I'm glad you like it though.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_109",
			say = "Unfortunately, Reno:esn't seem to be paying attention to any of Ingraham's words.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_109",
			say = "The \"vibranium shield\" has her mesmerized as though she's holding a magic treasure, unable to put it:wn for even a second.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_109",
			say = "Ingraham simply watches her from nearby, noticing a strange glimmer shining in Reno's eyes.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_109",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 101480,
			say = "(So this is how people react after getting their stuff back...)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_109",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 101480,
			say = "(Is this what the Commander wanted me to see?)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101480,
			side = 2,
			bgName = "star_level_bg_109",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "......",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_109",
			say = "A strong drive inexplicably arises within Ingraham's heart.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_109",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 101480,
			say = "(Since I already have the equipment I need, I guess I should knock out everything on the backlog.)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_109",
			say = "After waving goodbye to Reno, Ingraham trots back to her makeshift workshop.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_109",
			say = "The lights in the workshop: not go out until late, late into the night.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
