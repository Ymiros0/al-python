return {
	fadeOut = 1.5,
	mode = 2,
	id = "YINGHUA4",
	once = True,
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"",
					0.2
				}
			},
			signDate = {
				"The night of the war's outbreak - coordinates. 27.328699，-159.870474",
				0.2,
				{
					600,
					-480
				}
			}
		},
		{
			actor = 305050,
			side = 2,
			dir = 1,
			blackBg = True,
			say = "Are you saying that, no matter what, it's inevitable?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			dir = 1,
			blackBg = True,
			say = "It is all for the future of the Sakura Empire. All you need: is give us the order, and we shall: it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305050,
			side = 2,
			dir = 1,
			blackBg = True,
			say = "But... If a war breaks out, many will die... Both on their side, and ours...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			dir = 1,
			blackBg = True,
			say = "The force known as change requires tribute in the form of souls. And it's thanks to change that the world can move forward. I'm sure you know this fact very well.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307010,
			side = 2,
			dir = 1,
			blackBg = True,
			say = "Please,: not worry. The enemy still has no idea that our troops are coming. I can guarantee the attack will be successful.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305050,
			side = 2,
			dir = 1,
			blackBg = True,
			say = "I see...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307020,
			side = 2,
			dir = 1,
			blackBg = True,
			say = "Give us the order, Nagato. The elite of the 1st Carrier Division are always ready to sortie.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			say = "She has seen into the eyes of many. She has seen the anticipation in the eyes of faithful believers, and she has seen the gleam that signifies hope for the future.",
			blackBg = True,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			say = "However, that is not what she's seeing in these people's eyes. Their eyes are filled not with hope, but the desire to receive her authorization to start a war.",
			blackBg = True,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			say = "She may hesitate, but ultimately she has no choice. The people in charge have already decided what course the future will take.",
			blackBg = True,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			say = "She knew that her objections would be futile. She clenched her fist, took a deep breath, and made a declaration in the dignified voice she had now grown accustomed to using.",
			blackBg = True,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305050,
			side = 2,
			dir = 1,
			blackBg = True,
			say = "I am Nagato. Protector of the Sakura Empire and flagship of her Combined Fleet.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305050,
			side = 2,
			dir = 1,
			blackBg = True,
			say = "I hereby issue the following order to all personnel of the Sakura Empire's Combined Fleet.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 305050,
			side = 2,
			dir = 1,
			blackBg = True,
			say = "Commence the attack... on Azur Lane!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
