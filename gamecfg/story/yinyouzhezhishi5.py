return {
	id = "YINYOUZHEZHISHI5",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_158",
			factiontag = "???",
			dir = 1,
			actorName = "Bard",
			portrait = 401466,
			bgm = "ryza-az-theme",
			nameColor = "#FEF15E",
			hidePaintObj = True,
			say = "♪ The winter has passed, the spring has come, and life begins new ♪",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 9,
			side = 2,
			bgName = "star_level_bg_158",
			factiontag = "???",
			dir = 1,
			portrait = 401466,
			actorName = "Bard",
			nameColor = "#FEF15E",
			hidePaintObj = True,
			say = "♪ From the gods get we our blessings, and this lovely world:es too ♪",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_158",
			hidePaintObj = True,
			say = "A village sits on a grass plain. A bard's singing can be heard from:wn the road.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_158",
			factiontag = "???",
			dir = 1,
			actorName = "Bard",
			actor = 401466,
			nameColor = "#FEF15E",
			hidePaintObj = True,
			say = "♪ The seasons oh they come and go, the time has gone so fast ♪",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_158",
			factiontag = "???",
			dir = 1,
			actorName = "Bard",
			actor = 401466,
			nameColor = "#FEF15E",
			hidePaintObj = True,
			say = "♪ The glory days have passed us, so many years elapsed ♪",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_158",
			factiontag = "???",
			dir = 1,
			actorName = "Bard",
			actor = 401466,
			nameColor = "#FEF15E",
			hidePaintObj = True,
			say = "♪ Though darkness once was vanquished, its return is overdue ♪",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_158",
			factiontag = "???",
			dir = 1,
			actorName = "Bard",
			actor = 401466,
			nameColor = "#FEF15E",
			hidePaintObj = True,
			say = "♪ And while it creeps upon us, adventures start anew ♪",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_158",
			factiontag = "???",
			dir = 1,
			actorName = "Bard",
			actor = 401466,
			nameColor = "#FEF15E",
			hidePaintObj = True,
			say = "♪ Today a hero rises, fight evil they shall: ♪",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_158",
			factiontag = "???",
			dir = 1,
			actorName = "Bard",
			actor = 401466,
			nameColor = "#FEF15E",
			hidePaintObj = True,
			say = "♪ The world now needs a champion, perhaps... it's... you ♪",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
