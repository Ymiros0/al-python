return {
	fadeOut = 1.5,
	mode = 2,
	id = "YIWAIDEGUITU1",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Letters and Travels\n\n<size=45>1 Opening Ceremony</size>",
					1
				}
			}
		},
		{
			say = "Scapa Flow - Ceremony Venue",
			side = 2,
			bgName = "bg_endingsong_3",
			dir = 1,
			bgmDelay = 2,
			bgm = "theme-partydress",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 203040,
			say = "Whew... Everything is finally in place. It took a lot of work, but I did it!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 202270,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Great work. Although I'm afraid this chair is in the wrong place, and the banner behind the podium is a little misaligned.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 14,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 203040,
			say = "Don't sweat the small stuff! Nobody will notice!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 202270,
			say = "I'll have someone else take care of it. You can head to the dessert room and start on that next.",:
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 11,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 203040,
			say = "Awww... Just when I thought I could catch a break...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 202080,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "There you are, Suffolk.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 202080,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Why are you in here and not outside, welcoming the guests as Belfast asked you to?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 203040,
			say = "Oh right, that's what I was supposed to:! I'll be right there!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 202270,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Oh, my apologies. I found Suffolk staring at the clouds outside, so I decided to ask her to set up things around here.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 202080,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "That explains that,:. Speaking of which, are you finished in here?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 202270,
			say = "Not yet – it's quite a lot bigger than I anticipated. But worry not, I'll finish on schedule!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 202080,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "That's what I like to hear. If you'll excuse me, I have to get back to my station.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 202080,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "We have some very important guests tonight. We cannot slack on security.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			say = "Scapa Flow - Pier",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 204010,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Incredible. This place has already recovered so well after all the damage the Sirens did in that battle...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 204020,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "You can thank the manjuu construction corps for that. Those little guys are rather capable.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 204010,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Yes, but we should be ashamed that a critical part of our naval infrastructure has come under attack so many times!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 204010,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "We must defend it to the last woman henceforth and never let such a thing happen again.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 204020,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Yes, and we shall: just that.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 204020,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "For now, just settle:wn. Scapa Flow is back in business, and that's what we're here to celebrate.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 204020,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Brighten up, Renown. Hood's going to be in attendance, and you wouldn't want to disgrace the Royal Knights by facing her with a frown, would you?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 204010,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "You're right. I must conduct myself properly on this occasion for Her Majesty's and Lady Hood's sake!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 204020,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "There you go! That's more like the Renown I know.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 204010,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "All right, enough frivolities. Look sharp and let's head for the ceremony venue.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 204010,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "...Incidentally, where even is it?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 204020,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "No idea. Belfast said someone would come get us, but I:n't see anyone here.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 8,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 203040,
			say = "P-pardon my tardiness! ",
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			},
			actorPosition = {
				x = -1000,
				y = 0
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "move",
					y = 0,
					delay = 1.2,
					dur = 1,
					x = 1000
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 203040,
			say = "I'll show you to the venue. Follow me, please.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			say = "Scapa Flow - Ceremony Venue",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			say = "A cannon salute and fireworks were set off at the open-air venue to commemorate the occasion.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			say = "Friedrich's plan had succeeded, resulting in the elimination of a key Siren. However, Scapa Flow sustained significant damage in the battle.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			say = "The Royal Navy and Eagle Union attributed this damage to the Sirens in their formal report to Naval HQ.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			say = "And so, this ceremony serves two purposes – to keep up appearances, and to celebrate this great leap forward.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 213040,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Gee, granny Hood's awfully late.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			say = "As the curtain rose on the opening ceremony, Hood – the guest of honor – was nowhere to be seen.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 204020,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Wonder if something came up that she has to tend to...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 204010,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Lady Hood is punctual and a lady of her word. She wouldn't miss this event for the world.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 204020,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "She's not one to keep Naval HQ representatives waiting either, but she IS late...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 204020,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I can't imagine she's engaging Sirens either. They'd have to be mad to attack when practically our whole navy is here.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 204020,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Maybe the Royal Maids know what's up?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 204010,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "They aren't acting out of the ordinary, so I:n't believe so... I'll have a look around the area. You stay here, Repulse.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			say = "Just as Renown rose from her seat to: some sleuthing, an alarm went off.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			bgm = "airRaidAlarm",
			side = 2,
			bgName = "bg_endingsong_3",
			dir = 1,
			say = "BWEEEEE! BWEEEEE!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 204020,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "What's that?! The air raid siren?!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 204010,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "How?! They couldn't possibly have slipped through our anti-air patrols!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_3",
			say = "Panic broke out across the venue as the warning alarm kept blaring.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 202120,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Dear guests, please calm:wn. We assure you everything is fine.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 202120,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "This venue is under the protection of the Royal Navy's finest shipgirls. Any Siren aircraft headed this way will be shot:wn before they reach the harbour.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 202120,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "However, as a precautionary measure, please follow our staff to the on-site bunker.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 204010,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "I:n't understand... The alarm shouldn't sound if our fleets can eliminate the planes well before they get near us...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 204010,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Belfast, me and Repulse shall sortie as well. Kindly tell us where the enemy is!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 202120,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Of course. Our patrols are currently spread out over a wide perimeter beyond the bay.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 202120,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Please join up with King George's support fleet. They shall be setting sail to assist the patrols shortly.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 202120,
			side = 2,
			bgName = "bg_endingsong_3",
			hidePaintObj = True,
			dir = 1,
			blackBg = True,
			nameColor = "#a9f548",
			say = "As for the rest of the ceremony... I'm afraid to say it needs to be cancelled.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
