return {
	fadeOut = 1.5,
	mode = 2,
	id = "YONGYEHUANGUANG2",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			side = 2,
			nameColor = "#a9f548",
			stopbgm = True,
			dir = 1,
			blackBg = True,
			say = "Upon receiving the relief supplies from the Eagle Union, the Royal Navy quickly assembled their transport and escort fleets.",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			blackBg = True,
			say = "Their mission was to pass through the Barents Sea's polar night - in other words, the extremely disadvantageous task of running a convoy through prolonged darkness. However, the Northern Parliament's request had to be prioritized.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			blackBg = True,
			say = "Fortunately, the winds were favorable when the fleet departed from the Royal Islands.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 0,
			actorName = "Belfast",
			bgName = "bg_hms_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "It's about time to depart. Hermione, have you finished your preparations?",
			bgm = "story-richang",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Hermione",
			bgName = "bg_hms_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "Yes. The transport and escort fleets, as well as the supporting main fleet and vanguard, have all been assembled. We can set sail once the supplies are loaded.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 0,
			actorName = "Belfast",
			bgName = "bg_hms_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "Very well. As representatives of the Royal Maids in this operation, please fulfill the duties entrusted to you by Her Majesty.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_hms_1",
			actorName = "Edinburgh & Sheffield",
			dir = 1,
			say = "Understood!",
			dialogShake = {
				speed = 0.09,
				x = 10,
				number = 2
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Hermione",
			bgName = "bg_hms_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 0,
			actorName = "Belfast",
			bgName = "bg_hms_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "Hermione, what's the matter?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Hermione",
			bgName = "bg_hms_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "Hwuh...?! Oh, my apologies... I was just a bit nervous...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 0,
			actorName = "Belfast",
			bgName = "bg_hms_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "Oh my, is this your first major mission in quite some time? I have no:ubts about your abilities, Hermione. If anything, I hope you'll be able to look after my sister.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			nameColor = "#a9f548",
			bgName = "bg_hms_1",
			actorName = "Edinburgh",
			dir = 1,
			say = "Gosh, Bel, what: you mean by that? I'm perfectly capable of looking after myself!",
			dialogShake = {
				speed = 0.09,
				x = 10,
				number = 2
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 0,
			actorName = "Belfast",
			bgName = "bg_hms_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "Hehe~ I know. In any case, let's go over the mission objectives one last time.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Hermione",
			bgName = "bg_hms_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "We are to form a fleet to transport relief supplies to the Northern Parliament. We will: retrieve the commander and come back to the Royal Islands, before transferring—",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Hermione",
			bgName = "bg_hms_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "If the Iron Blood tries to intercept us, the supporting fleet and the main fleet will join up to eliminate the threat.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Sheffield",
			bgName = "bg_hms_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "(And finally, our secret mission to collect information about the Northern Parliament, and about the \"Reenactments.\")",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Edinburgh",
			bgName = "bg_hms_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "See, I remembered all of that! I've even reviewed the reports about the Northern Parliament's uhh... ahem, yes, the Iceberg Stronghold!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Hermione",
			bgName = "bg_hms_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "Well:ne, Miss Edinburgh.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Edinburgh",
			bgName = "bg_hms_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "Yes, yes, praise me more! I'm Bel's older sister after all, so you can count on me~!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 0,
			actorName = "Belfast",
			bgName = "bg_hms_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "All right. Hermione, Sheffield, Edinburg, I look forward to the good news.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Hermione",
			bgName = "bg_hms_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "Understood! I'll: my best!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Edinburgh",
			bgName = "bg_hms_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "Damn, why can't Bel just praise me...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Sheffield",
			bgName = "bg_hms_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "(If Her Majesty bothered to come all the way out to port,: she might be...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Sheffield",
			bgName = "bg_hms_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "Sorry, Belfast, there's still something that's bothering me... Please take care of Her Majesty.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 0,
			actorName = "Belfast",
			bgName = "bg_hms_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "I will, Sheffy. Make sure you also stay safe.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Sheffield",
			bgName = "bg_hms_1",
			nameColor = "#a9f548",
			dir = 1,
			blackBg = True,
			say = "...Glory to the Queen.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
