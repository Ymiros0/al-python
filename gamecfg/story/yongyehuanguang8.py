return {
	id = "YONGYEHUANGUANG8",
	mode = 2,
	once = True,
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			nameColor = "#ff5c5c",
			side = 2,
			bgName = "bg_hms_3",
			say = "Unknown Location",
			dir = 1,
			bgm = "bsm-5",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			actorName = "Admiral Hipper",
			bgName = "bg_hms_3",
			nameColor = "#ff5c5c",
			dir = 1,
			say = "Eugen, you sure took your sweet time. The hell were you:ing?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Prinz Eugen",
			bgName = "bg_hms_3",
			nameColor = "#ff5c5c",
			dir = 1,
			say = "Calm:wn, Hipper. Your cute face will get all wrinkly if you pout so much~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 1,
			nameColor = "#ff5c5c",
			bgName = "bg_hms_3",
			actorName = "Admiral Hipper",
			dir = 1,
			say = "I can't calm:wn because you're the one in charge of the operation.",
			dialogShake = {
				speed = 0.09,
				x = 10,
				number = 2
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Prinz Eugen",
			bgName = "bg_hms_3",
			nameColor = "#ff5c5c",
			dir = 1,
			say = "Is that so? Isn't our mighty Tirpitz the flagship this time?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Prinz Eugen",
			bgName = "bg_hms_3",
			nameColor = "#ff5c5c",
			dir = 1,
			say = "Anyway, to put on the finishing touches...♪",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Deutschland",
			bgName = "bg_hms_3",
			nameColor = "#ff5c5c",
			dir = 1,
			say = "Do I get to sortie yet? You've got some nerve to keep me waiting like this!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			side = 2,
			actorName = "Gneisenau",
			bgName = "bg_hms_3",
			nameColor = "#ff5c5c",
			dir = 1,
			say = "No matter how airtight they think their plan is, we'll stab it full of holes.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Scharnhorst",
			bgName = "bg_hms_3",
			nameColor = "#ff5c5c",
			dir = 1,
			say = "Ahaha! Let's show those pompous bastards an explosively good time!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Admiral Graf Spee",
			bgName = "bg_hms_3",
			nameColor = "#ff5c5c",
			dir = 1,
			say = "(...I've always been alone... But now, with everyone...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Tirpitz",
			bgName = "bg_hms_3",
			nameColor = "#ff5c5c",
			dir = 1,
			say = "(I know how you feel, Spee. Cherish this moment, for you are needed for the Iron Blood to achieve victory.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Prinz Eugen",
			bgName = "bg_hms_3",
			nameColor = "#ff5c5c",
			dir = 1,
			say = "Now, mercurial light of the North... How shall we bend you to achieve our victory?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Prinz Eugen",
			bgName = "bg_hms_3",
			nameColor = "#ff5c5c",
			dir = 1,
			say = "If you wish to ensnare the core fleet of the Iron Blood, you may not like what you find in your net.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Prinz Eugen",
			bgName = "bg_hms_3",
			nameColor = "#ff5c5c",
			dir = 1,
			say = "Now, show me how you dance, my little queen and your loyal followers~♪",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "Prinz Eugen",
			bgName = "bg_hms_3",
			nameColor = "#ff5c5c",
			dir = 1,
			say = "Victory to the Iron Blood!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
