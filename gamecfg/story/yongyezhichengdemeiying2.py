return {
	fadeOut = 1.5,
	mode = 2,
	id = "YONGYEZHICHENGDEMEIYING2",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"Tales From the Count's Castle\n\n<size=45>2 The First Trial</size>",
					1
				}
			}
		},
		{
			say = "I entered a big hall, after which the:ors shut behind me with a creak.",
			side = 2,
			bgName = "star_level_bg_156",
			dir = 1,
			bgmDelay = 2,
			bgm = "theme-highseasfleet-reborn",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			dir = 1,
			actorName = "???",
			say = "Heeheehee...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_156",
			say = "A girly giggle echoed forth from the darkness.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403111,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "That smell about you... You're not one of us.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_156",
			say = "Rows of candles along the walls lit up one after another, illuminating the staircase up to the second floor, where pallid girls sat upon grand thrones.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 402071,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Welcome to our abode, my lost, darling human.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 402071,
			say = "\"Most would reel in fear at the sight of Emden, but not you. Eager to become my next meal, hmm?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_156",
			say = "They had a menacing aura about them, like that of a video game final boss. I'd walked into the wrong neighborhood...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_156",
			say = "Sensing I'd messed up, I dialed a number on my communicator. No response.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403111,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "What an interesting plaything you have. Whatever its intended purpose, it will not help you in here.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_156",
			say = "It seemed they were as committed to their roles as I was committed to seeing the escape room to its conclusion.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 402071,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "\"Emden isn't terribly hungry, but you'll: as a means to relieve boredom.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 402071,
			say = "You're in luck, poor little human. Emden would like to play a little game. What say you?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_156",
			say = "I walked up to Emden's throne, expecting something like a chessboard... It was a stone tablet set up for a game of tic-tac-toe.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 402071,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "This is all we've had, and will have, to kill time for an eternity.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			actor = 402071,
			say = "Emden has never won a single game, nor ever lost a single game.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 402071,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Breaking this curse and resolving Emden's eternal grievance may grant you the key to escaping this place.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_156",
			say = "\"You're saying you'll let me go if I manage to beat you? All right.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_156",
			say = "I knew, however, that normal tic-tac-toe always ends in a draw. There had to be some trick I was allowed to use.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 403111,
			side = 2,
			bgName = "star_level_bg_156",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a9f548",
			say = "Are you ready to begin, human?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "star_level_bg_156",
			dir = 1,
			blackBg = True,
			say = "If this was the only way out, so be it. I sat:wn and began the match.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
