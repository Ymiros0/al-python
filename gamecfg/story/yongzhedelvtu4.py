return {
	id = "YONGZHEDELVTU4",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgm = "story-richang-2",
			say = "Great Dungeon - Top Floor",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Javelin",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Holy cow! We've found the Four Beasts' Dungeon!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Javelin",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "All we have to: is make it to the exit and we'll have a straight path to the Demon King's lair!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "{namecode.408}",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "While that sounds great, this place is a maze... Finding the exit will take forever.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Laffey",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Ugh... I'm already tired as heck...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "{namecode.408}",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Don't get lazy so soon! We haven't even fought one battle yet!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Ayanami",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "I've figured out the dungeon's layout.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Ayanami",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "This wall just ahead has a weak spot in its structure. Yes, I can: this.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Javelin",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Wait,: what? What are you going to:?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 331055,
			side = 2,
			nameColor = "#A9F548FF",
			dir = 1,
			say = "One's body is one's most powerful weapon. You wouldn't understand.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			nameColor = "#A9F548FF",
			side = 2,
			actor = 331055,
			dir = 1,
			say = "I'm going to use my Ultimate Ability. Dynamic Entry Kick!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "System",
			nameColor = "#A9F548FF",
			say = "AYANAMI performed DYNAMIC KICK! SURPRISINGLY STURDY WALL took 999 DMG! SURPRISINGLY STURDY WALL was destroyed!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				dur = 0.15,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.2,
				dur = 0.2,
				alpha = {
					1,
					0
				}
			}
		},
		{
			side = 2,
			actorName = "System",
			nameColor = "#A9F548FF",
			say = "A path to the dungeon's center appeared!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			nameColor = "#A9F548FF",
			side = 2,
			actor = 331055,
			dir = 1,
			say = "Heheh... Yes, this is much faster. Now let's go!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
