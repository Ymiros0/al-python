return {
	fadeOut = 1.5,
	mode = 2,
	id = "YUANHUIDIANZUOZHAN10",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			stopbgm = True,
			say = "The experiment continued smoothly with the girls all resting on some days and fighting on others.",
			bgm = "bsm-7",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			say = "While Brünhilde had lost the first battle, she mustered additional Pawns for the following rounds, slowly but surely turning the tides in her favor.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			say = "Even so, a clear victor had yet to emerge. Neither side had achieved the experiment's objective of conquering the other's base.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			say = "Until that happened, nobody was allowed to leave the Mirror Sea.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			say = "Mirror Sea - Iron Blood Encampment",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 5,
			say = "Darn it! We were in such a good position, but she turned the tables on us just like that!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 2,
			say = "She somehow knew we weren't prepared for two carriers out of nowhere. They completely ruined our formation!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 4,
			say = "What a load of bull! Like Graf Zeppelin wasn't tough enough, she even had a Pawn of one of your sisters!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 1,
			say = "Seydlitz? Yeah, it felt weird seeing a Pawn of her... To make things worse, there are still more carriers Brünhilde can field if she wants!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 2,
			say = "How the heck are we gonna deal with those?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 4,
			say = "CAN we even deal with 'em? The way I see it, this experiment is rigged against us!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 1,
			say = "I get what you mean. Not only:es she have more Pawns, those Pawns are all carriers and cruisers.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 2,
			say = "And what: we get? Just a bunch of submarines and destroyers.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 5,
			say = "Hey! Don't talk crap about destroyers! I'm one, and I'm the best there is!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 1,
			say = "I'm not badmouthing them at all, but it's objective reality that they're no match against a whole fleet of capital ships.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 2,
			say = "Remember, this is Pawn-on-Pawn combat. Our own capital ships:n't stand a chance if they're outnumbered.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			say = "You have a point...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 2,
			say = "On the bright side, a lot of her Pawns:n't have ASW capabilities. We could use our subs to chip away at her fleet.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 1,
			say = "Ah, wolfpack tactics! That might just work!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 2,
			say = "Putting strategy talk on hold for a sec, there's one thing that's been bothering me...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 1,
			say = "The radio silence. Have you heard anything from Bismarck?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 2,
			say = "Nope, still nothing.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 2,
			say = "She must be busy with her own operation,:. She said she'd give us advice if we needed, but... I guess we only have ourselves to rely on.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 2,
			say = "You:n't think she might've...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 2,
			say = "Nuh-uh, no way! She's the flagship of our fleet, the strongest of us all!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 1,
			say = "The Royal Navy's even grumbled over how unsinkable she is, remember? That battle is nothing she can't handle.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 2,
			say = "If anything has gone wrong, it has to be our on-site equipment.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 4,
			say = "Yeah, I guess... Ugh, I'm so fed up with being stuck here. I wanna go hoooome!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 2,
			say = "I: too, but we can't walk out on this experiment. Bismarck trusted us to finish it!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 1,
			say = "And:n't let your homesickness get the better of you, or we'll end up losing!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 5,
			say = "I'm not gonna walk out or give away the win! I know Iron Blood's future hinges on this thing!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 2,
			say = "Good! If it helps, I think our final match is tomorrow.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			blackBg = True,
			actor = 403020,
			say = "While we: have a Bismarck Pawn, we'll still have to go all-in and win as fast as we can.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
