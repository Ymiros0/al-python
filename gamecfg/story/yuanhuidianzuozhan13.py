return {
	fadeOut = 1.5,
	mode = 2,
	id = "YUANHUIDIANZUOZHAN13",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			stopbgm = True,
			say = "The experiment was nearing its final stage.",
			bgm = "bsm-7",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			say = "Blücher's fleet of capital ships tried to push through with brute force only to be stopped and swiftly defeated by a single battleship Pawn.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			say = "What followed was another victory for Brünhilde. All areas were now under her control, save for one – the base's last line of defense.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			say = "Blücher and Z16 were facing defeat. If Brünhilde conquered their base, that would spell the end of the experiment.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			say = "Mirror Sea - Iron Blood Encampment",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 5,
			say = "She beat us AGAIN! How could this even happen...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 2,
			say = "It's not hard to explain, really. We lost our firepower advantage as soon as that battleship Pawn showed up.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 4,
			say = "Well, what: we: now?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 2,
			say = "There's not much we CAN:. It's game over for us once she takes the next area.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 5,
			say = "Grrr... What a load of bull!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 2,
			say = "This game is rigged! It's unfair! Our Pawns have nothing on hers!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 4,
			say = "Hell, we've hardly even gained any new Pawns since the start while hers keep getting stronger! And we still haven't heard a darn thing from Bismarck!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 2,
			say = "That's all part of the operation. Well, probably.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 2,
			say = "In the end, it:esn't matter whether we win, or Brünhilde wins. Both outcomes benefit Iron Blood.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 1,
			say = "And hey, we did everything we could, from strategizing to commanding! If we lose, we lose knowing we did our best!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 4,
			say = "That's not much consolation...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 2,
			say = "Look, I'm fed up with this place too! There's barely anything to: to kill time!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 1,
			say = "Like, what are we supposed to: for fun? Talk to the Pawns? They:n't say a thing back!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 4,
			say = "What: you mean? Thiele and I talk all the time.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 1,
			say = "...You what? Are you joking? What: you talk about?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			say = "Just stuff. I talked about food just yesterday and she listened to it all!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			say = "What?! This is huge if True! I've gotta note it:wn in my report!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 1,
			say = "I need details! What exactly did she say?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 4,
			say = "Is it that important? Hang on, let me try to remember...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			side = 2,
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Hey, Thiele! Just had breakfast – probably the best I've ever had!",
			flashout = {
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				alpha = {
					1,
					0
				}
			},
			effects = {
				{
					active = True,
					name = "memoryFog"
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			actorName = "Z2?",
			say = "\"Wow, did you? I'm listenin'. What else you got?\"",:
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			say = "Yesterday's lunch was off the hook too! I never even knew Sardegna had so many kinds of pizza!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			actorName = "Z2?",
			say = "\"Wow, did you? I'm listenin'. What else you got?\"",:
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 7,
			say = "And it's not just the pizza, they've got a million different kinds of spaghetti too!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			actorName = "Z2?",
			say = "\"Wow, did you? I'm listenin'. What else you got?\"",:
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 3,
			say = "Wonder what we're havin' for dinner! Bet it'll be great too!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 403020,
			say = "...This is just a hunch, but I think that conversation only took place in your head.",
			effects = {
				{
					active = False,
					name = "memoryFog"
				}
			},
			flashout = {
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 5,
			say = "What the hell! Why: you say that?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 1,
			say = "First of all, she would never talk the same way you:. Second, why:es she sound like a broken record. *sigh*... I'm not gonna bother writing any of that:wn.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 4,
			say = "Why not? What's the harm?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 1,
			say = "I dunno, someone reading it and thinking you've lost your marbles?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 5,
			say = "Now let's hit the hay. We're gonna need all the energy we can get for tomorrow's experiment.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 1,
			say = "Now that I think about it, we used to have experiments every three days, but recently we've been fighting every day. Do you think something's wrong with the central terminal?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 2,
			say = "Maybe it's just as fed up as we are.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 2,
			say = "Wouldn't that be funny? Heheh☆",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 1,
			say = "Oh well. Next battle's probably gonna be our last, but that's no reason not to give it our best shot!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			blackBg = True,
			say = "Damn right. I'm not going:wn without a fight!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
