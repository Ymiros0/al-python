return {
	fadeOut = 1.5,
	mode = 2,
	id = "YUANHUIDIANZUOZHAN2",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			stopbgm = True,
			say = "Mirror Seas. Pocket dimension created by the Sirens.",
			bgm = "bsm-7",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			say = "These artificial dimensions are almost completely cut off from the outside world. On the inside, they have their own environment, structure, and even physical laws.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			say = "Indeed, a Mirror Sea can exist within a place that appears completely empty from the outside. That there would be a huge island inside with a large array of facilities is not beyond the realm of imagination.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			say = "Mirror Sea - Iron Blood Encampment",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			say = "Woah, so this is our Mirror Sea research facility...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 1,
			say = "It looks like another gloomy Siren construct from the outside, but once you walk inside, it's actually rather comfy!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			say = "No kidding! With how spacious and fancy this place is, it seems more like a luxury resort than a base of operations.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 7,
			say = "Is this where we're goin' to stay until the end of our experiment?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 4,
			say = "Whew~!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			say = "Blücher plopped herself:wn on a sofa inside the rest area and opened up the mission briefs.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 5,
			say = "That's right. We're gonna be here for a long, long while, so we might as well make ourselves at home.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 1,
			say = "Hmm, let's take a look around. This is the rest area... And here's the fleet... Um, is this a crafting room and a workshop?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 6,
			say = "This must be where the Sirens make their Pawns! It's my first time seeing something like this! How 'bout you, Blücher?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 1,
			say = "Well yeah. It's not like the Sirens normally let us wander around their facilities.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 2,
			say = "And if I'm seeing this right... This Mirror Sea can produce upgraded Pawns with enhanced combat capabilities and more autonomy...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			say = "I guess they've been modified for the purpose of our experiment, just like our own riggings.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 1,
			say = "Remember to be careful though. If you charge in recklessly against these kinds of enemies, you might end up injured, experiment or not.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 403020,
			say = "Heheh☆ Pretty spooky stuff, huh?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			say = "While Blücher was running through operational briefs, Z16 was investigating the empty canteen next:or.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			say = "Would you look at that, Blücher, the pantries are totally stocked with goodies!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			say = "They've got all sorts of food here! It's not just Iron Blood stuff, they've got Iris, Royal, and Sardegnian cuisine as well!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 2,
			say = "I see, I see... Each sea area is divided into smaller sectors. If the flagship of a sector is incapacitated,: that sector will be considered \"captured.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 1,
			say = "Once enough sectors have been captured,: it will be possible to attack the enemy's base of operations...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			actor = 401160,
			say = "Blücher, are you listening?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 2,
			say = "What was that again?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 2,
			say = "I just checked out the canteen! We've got lots of good stuff here!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			say = "Aren't you hungry after we spent such a long time sailing? Let's chow:wn before we get to work!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 3,
			say = "It's not like we can start the operation before startin' these facilities up anyway.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 3,
			say = "Hmm, you: have a point... So, what are you gonna have?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 7,
			say = "Schweinhaxe, Sauerkraut, and Kartoffelbrei!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 4,
			say = "Get me some Schweinhaxe as well:. And some wurst and grilled veggies to go with that.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 2,
			say = "Now let's see... Until one of the operational bases has been captured, we won't be able to leave the Mirror Sea, nor will anything be able to come in from outside...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 1,
			say = "...What the heck is this?!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 4,
			say = "Gah! What are you yelling about all of a sudden?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			say = "That was a close one... I almost dropped the food you asked me to bring.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 2,
			say = "Appreciate it. Uhh... The operation will begin at 3 P.M. on the first day...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 5,
			say = "Huh?! We're already going to have to fight on the first day?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 4,
			say = "What time is it anyway...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 2,
			say = "It's already 11.30, and I haven't even inspected the facilities yet!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 6,
			say = "Uh-oh...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			blackBg = True,
			actor = 403020,
			say = "Let's finish eating quickly and inspect the conditions of the facilities and the Pawns!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
