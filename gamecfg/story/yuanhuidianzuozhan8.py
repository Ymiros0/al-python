return {
	fadeOut = 1.5,
	mode = 2,
	id = "YUANHUIDIANZUOZHAN8",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			stopbgm = True,
			say = "Mirror Sea - Iron Blood Encampment",
			bgm = "bsm-7",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			say = "Tonight we're having – drumroll please – tadah! A full course of Iris cuisine!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			say = "Holy crap, that's a lot of food! And it all looks delish!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 1,
			say = "Mhm! It's not every day we get to pig out like this!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 2,
			say = "There are drinks too, if you want something. Just go pour it yourself.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			say = "Sweet! Maybe I'll have a soda or something...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 3,
			say = "Speaking of food, what: Pawns eat? Do they even eat at all?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 4,
			say = "I was thinking, wouldn't it be surreal if they just all walked in and started queuing up?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 2,
			say = "I can't imagine them needing to eat. I mean, they're basically just Siren-created robots, right? I:ubt they'd need food.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 1,
			say = "The survey notes say that when Pawns returns to the workshop, the first thing they: is upload their combat data to a separate facility.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 2,
			say = "Then they go to... I forget what it's called, but some place where they compare their data amongst each other to reflect on their performance.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 1,
			say = "After that, I guess they just head into storage and take a nap.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 5,
			say = "That's when they recharge, regroup with the other Pawns for the next day, and get ready for the next experiment. Seems simple enough.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 8,
			say = "Interesting... I'm surprised you know all this stuff.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			say = "Before we got here, I just assumed we were both equally clueless.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 1,
			say = "You'd have known too if you maybe read the briefs. It's all in there.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 2,
			say = "Think about the situation we're in here. Nobody's allowed in or out of the Mirror Sea until a victor's been determined.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 2,
			say = "Meaning, we're stuck here until the experiment's over. Isn't that really scary?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 1,
			say = "Learning everything you can is super important if you want to leave as soon as possible and reunite with our friends.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 4,
			say = "Oof. You're absolutely right... I just assumed this would be like any regular exercise...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401160,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 5,
			say = "Maaan, realizin' that we're stuck here is making my appetite go away...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 2,
			say = "Hey, it's not all bad. Our opponent is one of our own and she's not a Pawn, meaning that we can communicate with her in case something goes wrong.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403020,
			nameColor = "#A9F548FF",
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			side = 2,
			expression = 1,
			say = "But more importantly, as long as we keep winning, we'll be out of here in no time!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "bg_yuanhuidian",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			blackBg = True,
			actor = 401160,
			say = "Yeah, you're right! You know what, I think I'll have a burger as well!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
