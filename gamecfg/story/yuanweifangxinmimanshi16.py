return {
	fadeOut = 1.5,
	mode = 2,
	id = "YUANWEIFANGXINMIMANSHI16",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"The Orthodoxy's Banquet\n\n<size=45>A Toast to Our Host</size>",
					1
				}
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_115",
			hidePaintObj = True,
			stopbgm = True,
			say = "After finishing my stroll around the palace with Elizabeth, I return to the banquet hall, where I find someone waiting for me.",
			bgm = "story-musicanniversary-gorgeous",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_115",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 202083,
			say = "Master, I've been asked to forward a message to you from Her Majesty. I urge you to read it at once.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_115",
			hidePaintObj = True,
			say = "She hands me an ornate card, embellished with royal iconography.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_115",
			hidePaintObj = True,
			say = "In short, it tells me to come to a certain numbered room.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_115",
			hidePaintObj = True,
			say = "By the time I finish reading and look up, Sheffield is gone. I infer this to mean I should come alone.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			say = "Beyond the numbered:or is a big room, furnished in typical Royal Navy fashion.",
			soundeffect = "event./ui/dg-zoxiazhuanpan",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			say = "Its lavishness rivals that of Elizabeth's own royal chamber. It's almost hard to believe a room like this exists within this palace.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			say = "At the center of the room is a dining table. On the other end of it, Elizabeth sits by herself.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 202123,
			say = "Welcome, Master. Her Majesty is waiting for you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "Took you long enough, servant!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				},
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			say = "\"Sorry, I:n't know this palace so I got lost for a while. That aside, very luxurious place you have here.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "You mean this VIP room?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "Of course it is. Elites such as me deserve elite treatment. I'm sure they provided Bismarck with something equally opulent.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			say = "\"I take it you're:ne experiencing normalcy,:?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "I've enjoyed it enough for a while.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			say = "Belfast gestures at me,: shows me to my seat at the table.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "Take a seat.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "Reckon you still have room in your stomach?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			say = "\"I might. Did the Royal Maids prepare the whole feast you have here?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "No, only the tea, courtesy of Bel. Everything else is from the Orthodoxy's kitchen.",:
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "It's the same as what they're serving in the banquet hall. If you're not hungry, you may pass.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			say = "\"It might be the same food, but the experience is different. It surely affects the taste when you eat in a queen's opulent room, per her invitation.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "Well, well! You can be charming from time to time.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			say = "My dinner with Elizabeth is pleasant.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			say = "The mood is quiet, relaxing, and feels like it's another world compared to the hustle and bustle of the banquet hall.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			say = "Leaning back to admire my surroundings, I notice something strange – while my cup contains tea, Elizabeth's seems to contain wine.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "Wondering why you:n't get any wine,: you? Because you need to stay sober until the main event.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "As for why I'M drinking wine... I just wanted something different.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "Let's not forget that the Orthodoxy is famous for its wine.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "Furthermore, Richelieu put a great deal of thought into her selection.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "She hand-picked different vintage brands to match each guest's preferences, you see. And she did a marvellous job.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			say = "She takes a sip from her cup of wine and savors it with all the elegance befitting a queen.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "As I've said many times already, the Orthodoxy really did a remarkable job with this banquet.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "Everything is spotless, from the general gist to the fine details.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "But what: you think, servant? Have you been enjoying yourself?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			say = "\"Absolutely. I've not once felt anything could've been better.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "Good to hear. We're in agreement.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			say = "Right:, I hear a ring on my communicator. It's an invitation.",
			soundeffect = "event./ui/deng1",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "Must be from Richelieu. Seems you never have a moment to rest, eh?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "Go. She's waiting for you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "Don't worry about me. I've had you to myself enough for one day. Plus, I'm a little tipsy, so I should stay and rest.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 202123,
			say = "If you would, Master, please follow me.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			say = "Belfast leads me to the:or. As I leave, I overhear Elizabeth whispering something to herself.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "The Orthodoxy can have this one victory.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "It's their banquet, after all. It'd be rude of us to steal their thunder.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_107",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "Enjoy the celebration now, servant.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			effects = {
				{
					active = True,
					name = "speed"
				}
			}
		}
	}
}
