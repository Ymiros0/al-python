return {
	fadeOut = 1.5,
	mode = 2,
	id = "YUANWEIFANGXINMIMANSHI2",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"The Orthodoxy's Banquet\n\n<size=45>Enterprise's Speech</size>",
					1
				}
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			stopbgm = True,
			say = "As much as I want to check out the venue, I feel that I should have a look around the palace.",
			bgm = "story-musicanniversary-gorgeous",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			say = "Two people are having a chat just:wn the hall...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107065,
			say = "That's everything I need to know? Noted.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107065,
			say = "I'm surprised that you came to tell me that. I was expecting Richelieu.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 905020,
			say = "What can I say? Even I – a known behind-the-scenes schemer – need to take center stage now and again.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 905020,
			say = "Now, I trust you're willing to deliver the Eagle Union's speech, yes?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107065,
			say = "Yeah. I'm up for it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 905020,
			say = "Great. Then if you'll excuse me... Oh, if it isn't the Commander.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107065,
			say = "Didn't see you there, Commander.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			say = "\"Good evening, you two. I hope I didn't interrupt your conversation.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 905020,
			say = "Not at all. Your timing couldn't have been better, in fact. We just finished our little talk.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 905020,
			say = "Seeing as there's still some time before the banquet officially starts, I'll go on ahead and find some drinks. You two have a good one, now.~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107065,
			say = "You haven't seen Essex or Massachusetts around, have you? They said they were on the way.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			say = "As far as I can tell by Enterprise's expression, she:esn't seem particularly worried about them.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			say = "\"Can't say I have, no. Do you reckon they've gotten lost?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107065,
			say = "Maybe. I helped Essex with her dress,: sometime later she just disappeared on me.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107065,
			say = "Same with Massachusetts. She put on her dress, I turned around for a minute, and she was gone.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107065,
			say = "They're obviously SOMEWHERE in the palace, so I:n't think they're in danger or anything. It's just...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107065,
			say = "Ah, forget it. I'm sure they're just:ing something to pass the time.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107065,
			say = "Anyway... Could I have a minute of your time, Commander?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107065,
			say = "It's nothing important. I just wanted to ask you something.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			say = "\"Sure, not a problem. What's on your mind?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107065,
			say = "It feels weird to just come forth and ask it outright, but... what: you think about my dress?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			say = "I expected something else, given how she beat around the bush for a bit. I'm relieved this is all it was.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "\"It looks great on you.\"",
					flag = 1
				},
				{
					content = "\"You completely own the look.\"",
					flag = 3
				},
				{
					content = "\"It's breathtakingly beautiful.\"",
					flag = 2
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107065,
			say = "You really think so?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107065,
			say = "It took a while to put on, and I rarely ever wear dresses, so I wasn't entirely sure.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107065,
			say = "Hearing you say it:es look good is a big relief.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			say = "\"Glad to give you some peace of mind. On another subject, what were you and Clemenceau talking about earlier?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107065,
			say = "Oh, that. We were discussing a speech for the banquet. It's a big day, after all, so it feels appropriate.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107065,
			say = "She asked me to deliver it on behalf of the Eagle Union.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			say = "\"I see. Do you need any help with that?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107065,
			say = "Well, the script itself is already finished, so I'm good on that front.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107065,
			say = "But I'd appreciate it if you could read it through, and maybe offer some feedback too.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			say = "\"I could: that, no problem. First, though, we should find a place with fewer inquisitive people.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107065,
			say = "You're right. Someplace quiet would be ideal. Let's go.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			say = "And so, Enterprise and I take off, leaving the hall behind.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
