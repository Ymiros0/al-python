return {
	fadeOut = 1.5,
	mode = 2,
	id = "YUANWEIFANGXINMIMANSHI8",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"The Orthodoxy's Banquet\n\n<size=45>A Minute with Her Majesty</size>",
					1
				}
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			stopbgm = True,
			say = "As much as I want to check out the venue, I feel that I should have a look around the palace.",
			bgm = "story-musicanniversary-gorgeous",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 202201,
			say = "Your Majesty! Look out!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			say = "A decorative vase placed dangerously to the edge of a desk threatens to fall over as Queen Elizabeth approaches. Sirius jumps in front of her and shields her with her own body.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 202123,
			say = "Please be calm, Sirius. The inside of the palace is safe.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "Mrmf! Mmgh!!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				},
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 202201,
			say = "Oh, how clumsy of me! Please forgive me, Your Majesty!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "*gasp*! What were you thinking? You nearly choked me with those huge knockers of yours!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				},
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "Grrrr... Hmph!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 202201,
			say = "My sincerest apologies. I shouldn't have held you so forcefully. Please give this naughty maid her due punishment!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			say = "\"What's going on here?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "Why, if it isn't my servant.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "Nothing is \"going on\" – except for Sirius being a klutz.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "No matter. Your timing is perfect, servant. Come with me on a little stroll.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "\"Gladly!\"",
					flag = 1
				},
				{
					content = "\"Well, I've got an important thing to:...\"",
					flag = 2
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_502",
			dir = 1,
			optionFlag = 1,
			actor = 205011,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Good. Right this way.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_502",
			dir = 1,
			optionFlag = 2,
			actor = 205011,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Nonsense! What could possibly be more important than accompanying me, the queen?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			optionFlag = 2,
			say = "Darn. Doesn't sound like I can't wriggle out of this one...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 202123,
			say = "In the meantime, I shall prepare a beverage for after you finish your stroll.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "Thanks, Bel.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "Sirius, you go with her. She'll need your help.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 202201,
			say = "But Your Majesty, you need an escort...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "I'll be fine. The venue is safe, plus I have Sheffy watching over me from the shadows.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 202201,
			say = "Well, if you're certain...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 202201,
			say = "Shall I also prepare a light meal for you and my honourable Master while you're away?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 202201,
			say = "There's quite some time before dinner, and I'd hate for you to be hungry until–",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "No need! There are hors d'oeuvres in the banquet hall. Moreover, the Iris Orthodoxy should be allowed to display its cooking knowledge today. The Royal Maids needn't: anything.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			say = "Elizabeth and I wander around the palace, admiring the sights and architecture.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "Hmm... The Orthodoxy has:ne a respectable job with this banquet.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "Although it's still only about 80% as good as the banquets we host.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			say = "\"That's a specific estimate. You must have a keen, critical eye for these things.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "I:. Let's imagine that we were hosting this banquet...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 205011,
			say = "First of all, we wouldn't hang a painting on that wall. All the elements of the interior need to be in harmony, and...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			say = "With an excited tone, she points out a myriad of things they'd: differently. She seems to quite enjoy the act of critiquing.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			say = "Then, all of a sudden, she falls silent and deep in thought. I wonder why?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "(Observe her closely.)",
					flag = 1
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			optionFlag = 1,
			say = "\"You seem a bit restless. Did you dismiss Sirius and Belfast because you wanted to tell me something in private?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_502",
			dir = 1,
			optionFlag = 1,
			actor = 205011,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "What? No. I have nothing to tell you that I wouldn't if they were here.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_502",
			dir = 1,
			optionFlag = 1,
			actor = 205011,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Where did that come from? Are you pining for Sirius' cooking?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			optionFlag = 1,
			say = "\"She has improved – she can make chocolate and simple meals now – but no, that's not why I asked.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_502",
			dir = 1,
			optionFlag = 1,
			actor = 205011,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Ugh... Alright, if you really must know...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_502",
			dir = 1,
			optionFlag = 1,
			actor = 205011,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Look around you. People have been coming and going since I arrived.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_502",
			dir = 1,
			optionFlag = 1,
			actor = 205011,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "And I've scarcely seen anyone with their subordinates in tow.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_502",
			dir = 1,
			optionFlag = 1,
			actor = 205011,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Richelieu:esn't have any assistants accompanying her, and neither: you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_502",
			dir = 1,
			optionFlag = 1,
			actor = 205011,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "That's what's normal at this banquet, and I simply wanted to experience this normalcy for myself.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_502",
			dir = 1,
			optionFlag = 1,
			actor = 205011,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Even if only for a little bit.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_502",
			dir = 1,
			optionFlag = 1,
			actor = 205011,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Just an ordinary shipgirl, not a queen, taking a stroll through this giant palace.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_502",
			dir = 1,
			optionFlag = 1,
			actor = 205011,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "And with you for company.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			optionFlag = 1,
			say = "\"Is it 'normal' for me to follow you around?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_502",
			dir = 1,
			optionFlag = 1,
			actor = 205011,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "If I SAY it's normal,: it is!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_502",
			dir = 1,
			optionFlag = 1,
			actor = 205011,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Or what,: you think I:n't mean it?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_502",
			dir = 1,
			optionFlag = 1,
			actor = 205011,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "I'm confiding in you here, servant. I demand you take me seriously!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_502",
			dir = 1,
			optionFlag = 1,
			actor = 205011,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Otherwise, I can always get mad like a normal person and discipline you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "\"Please forgive me, Your Majesty!\"",
					flag = 1
				},
				{
					content = "\"By:ing... what?\"",
					flag = 2
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_502",
			dir = 1,
			optionFlag = 1,
			actor = 205011,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "If you truly want my forgiveness, earn it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				},
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_502",
			dir = 1,
			optionFlag = 1,
			actor = 205011,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Lucky for you, there's still some time before Bel comes back with that drink.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_502",
			dir = 1,
			optionFlag = 1,
			actor = 205011,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Until she:es, think of something to appease me!~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_502",
			dir = 1,
			optionFlag = 2,
			actor = 205011,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "What? Umm... By... By bemeaning you, or inflicting some light punishment...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_502",
			dir = 1,
			optionFlag = 2,
			actor = 205011,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "I've got it! Something like what Ajax or Scylla would:.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			optionFlag = 2,
			say = "Elizabeth's disturbing idea makes me stifle a laugh.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_502",
			hidePaintObj = True,
			optionFlag = 2,
			say = "\"To that, I say – it's not normal for a shipgirl to punish her Commander, is it?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_502",
			dir = 1,
			optionFlag = 2,
			actor = 205011,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "It's not? But I thought...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_502",
			dir = 1,
			optionFlag = 2,
			actor = 205011,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "No matter! If you:n't want to suffer, straighten out and look sharp!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			effects = {
				{
					active = True,
					name = "speed"
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_502",
			dir = 1,
			optionFlag = 2,
			actor = 205011,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Lucky for you, there's still some time before Bel comes back with that drink.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			effects = {
				{
					active = False,
					name = "speed"
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_502",
			dir = 1,
			optionFlag = 2,
			actor = 205011,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Until she:es, think of something to appease me!~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
