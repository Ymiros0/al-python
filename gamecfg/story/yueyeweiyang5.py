return {
	fadeOut = 1.5,
	mode = 2,
	id = "YUEYEWEIYANG5",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_509",
			hidePaintObj = True,
			stopbgm = True,
			say = "Memory - Sakura Empire Long ago?",
			bgm = "story-tiancheng",
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_509",
			hidePaintObj = True,
			say = "A fresh breeze reminiscent of early morning blew through the trees, whipping up robust pink petals and scattering them about.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_509",
			hidePaintObj = True,
			say = "The sloped road seemed to go on forever, lined by trees shedding their petals.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301650,
			side = 2,
			bgName = "star_level_bg_509",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Oh, Yuudachi? Good morning~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301140,
			side = 2,
			bgName = "star_level_bg_509",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Wooow, the sakura are in full bloom~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301140,
			side = 2,
			bgName = "star_level_bg_509",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Hm? Why are you here, Michishio? Aren't you supposed to be cleaning the courtyard at this time of day?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301140,
			side = 2,
			bgName = "star_level_bg_509",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Are you slacking off?! Do I smell a slacker?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301650,
			side = 2,
			bgName = "star_level_bg_509",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Geez, Yuudachi, I finished cleaning hours ago~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301650,
			side = 2,
			bgName = "star_level_bg_509",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Lady Mutsu has a very important guest today, so she sent us all home early~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301140,
			side = 2,
			bgName = "star_level_bg_509",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Is that the same as Lady Nagato's guest?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301650,
			side = 2,
			bgName = "star_level_bg_509",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Maybe... Isn't it unusual, though?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301650,
			side = 2,
			bgName = "star_level_bg_509",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "She didn't give us any more details than that, though, so there's no point trying to guess.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301650,
			side = 2,
			bgName = "star_level_bg_509",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Heheh, by the way, I made some more sakura mochi yesterday~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301140,
			side = 2,
			bgName = "star_level_bg_509",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "You mean I can have some?! Your sakura mochi is the best! Let's find a place to chow:wn!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.1,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 301650,
			side = 2,
			bgName = "star_level_bg_509",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "You're so impatient! When you want to eat good food, you can't just sit anywhere willy-Nonely. You need good scenery!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301650,
			side = 2,
			bgName = "star_level_bg_509",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...Hmm, but where would that be...?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301140,
			side = 2,
			bgName = "star_level_bg_509",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Lady Nagato's courtyard?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301650,
			side = 2,
			bgName = "star_level_bg_509",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Absolutely not! Geez~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301140,
			side = 2,
			bgName = "star_level_bg_509",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Then let's sit in the grass over there! We can look at the sakura trees while we eat. Yeah, that's perfect!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301650,
			side = 2,
			bgName = "star_level_bg_509",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "You just go find a place now, Yuudachi. I'll go get the cake box.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301650,
			side = 2,
			bgName = "star_level_bg_509",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...Oopsie!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_509",
			hidePaintObj = True,
			say = "Not looking where she was going, Michishio ran into somebody and fell. However, her victim quickly caught her.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_509",
			hidePaintObj = True,
			say = "The person she had run into – a tall woman wearing a white robe – gently brushed the flower petals off of her head.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_509",
			hidePaintObj = True,
			say = "Michishio looked up at her apologetically and bowed.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301650,
			side = 2,
			bgName = "star_level_bg_509",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I'm sorry! I should've watched where I was going!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.1,
					x = 0,
					number = 2
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_509",
			hidePaintObj = True,
			say = ".........",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_509",
			hidePaintObj = True,
			say = "But the tall woman had already disappeared, before the flower petals she'd brushed away could fall to the ground.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301140,
			side = 2,
			bgName = "star_level_bg_509",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Michishio! I found a good spot!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_509",
			hidePaintObj = True,
			say = "When Michishio turned around, she saw Yuudachi sitting in the grass and waving at her.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301650,
			side = 2,
			bgName = "star_level_bg_509",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "R-right! I'll go get that cake box~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301650,
			side = 2,
			bgName = "star_level_bg_509",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "(That lady... was kind of weird...)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
