return {
	fadeOut = 1.5,
	mode = 2,
	id = "YUYEJINGHUN28",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			expression = 10,
			side = 2,
			bgName = "star_level_bg_159",
			factiontag = "Bookworm GM",
			dir = 1,
			bgm = "theme-ijndailymeeting",
			actor = 502070,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Congratulations, Commander.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_159",
			factiontag = "Impromptu Replacement GM",
			dir = 1,
			actor = 401020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Yes. Who would have thought you'd actually see every last ending?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9600010,
			side = 2,
			bgName = "star_level_bg_159",
			factiontag = "As Castle Maid",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "You wouldn't believe how hard it was to play my character while watching you fail your dice rolls!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 9,
			side = 2,
			bgName = "star_level_bg_159",
			factiontag = "As Lord's 2nd Daughter",
			dir = 1,
			actor = 9600050,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Yup. Acting was such a pain...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9600030,
			side = 2,
			bgName = "star_level_bg_159",
			factiontag = "As Lord's Daughter",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Agreed~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_159",
			factiontag = "Script Writer & Technician",
			dir = 1,
			actor = 403030,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "On the other hand, the Commander had fun, and so did everyone in the control room. All the time spent scripting was worth it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301160,
			side = 2,
			bgName = "star_level_bg_159",
			factiontag = "LCK-Maxed GM",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "I told you, you should've let Yukikaze the Great: all the dice rolls!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 401020,
			side = 2,
			bgName = "star_level_bg_159",
			factiontag = "Impromptu Replacement GM",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "That would utterly ruin the game's balance, I'm afraid.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_159",
			factiontag = "As Investigator",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Just wondering... In that one ending, who was the woman in the wedding dress at the banquet? Don't tell me it was...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_159",
			factiontag = "(Self-Proclaimed) Great Detective GM",
			dir = 1,
			actor = 101490,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "She was Lord Manjuu's wife, of course!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_159",
			factiontag = "As Investigator",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "What about the charm from the Miskatonic University? Why did Whydah sneak those out?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_159",
			factiontag = "As Investigator",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Did she know what was going on back home from the very start?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_159",
			factiontag = "Serious Protégé GM",
			dir = 1,
			actor = 107090,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "It's possible. Maybe the maid wasn't affected by the cult's influence because the second daughter had those items.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107090,
			side = 2,
			bgName = "star_level_bg_159",
			factiontag = "Serious Protégé GM",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "I'm sure there are other minor contrivances, but I think they're best left as mysteries.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_159",
			factiontag = "Serious Protégé GM",
			dir = 1,
			actor = 107090,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Explaining every little thing would just take away the fun of speculating.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_159",
			factiontag = "Serious GM",
			dir = 1,
			actor = 107060,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "You can analyze the story later. What we really wanna hear now is your overall impression, while it's fresh.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "Darkness, mystery, horror... It's everything I expected!",
					flag = 1
				},
				{
					content = "It was scary as hell!",
					flag = 2
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_159",
			factiontag = "(Self-Proclaimed) Great Detective GM",
			dir = 1,
			actor = 101490,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "That's what Halloween is all about!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9600040,
			side = 2,
			bgName = "star_level_bg_159",
			factiontag = "As Phantom of the Ancient God",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "And giving people the creeps – that's what we ghost ships are all about!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_159",
			factiontag = "Serious GM",
			dir = 1,
			actor = 107060,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Good to hear. Then again, there's a lot of squeamish people at the port.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107060,
			side = 2,
			bgName = "star_level_bg_159",
			factiontag = "Serious GM",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "There's probably room for improvement, and accessibility.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "You should add a faint-of-heart mode.",
					flag = 1
				},
				{
					content = "Let's include teamplay as an option.",
					flag = 2
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_159",
			factiontag = "Serious GM",
			dir = 1,
			actor = 107060,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Huh... Good idea. I think we can: that.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_159",
			factiontag = "As Lord's 2nd Daughter",
			dir = 1,
			actor = 9600050,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "I:n't care what the story is as long as my character gets to chill in her room, surrounded by crisps and cola.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_159",
			factiontag = "As Lord's 2nd Daughter",
			dir = 1,
			actor = 9600050,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "So... please:n't change that.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			stopbgm = True,
			mode = 1,
			bgm = "story-oldcastle-carnival",
			sequence = {
				{
					"A Bump in the Rainy Night",
					1
				}
			}
		},
		{
			mode = 1,
			stopbgm = True,
			blackBg = True,
			effects = {
				{
					active = True,
					name = "yuyejinghun"
				}
			},
			sequence = {
				{
					"",
					1
				}
			}
		}
	}
}
