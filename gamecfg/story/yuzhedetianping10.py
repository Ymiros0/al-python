return {
	fadeOut = 1.5,
	mode = 2,
	id = "YUZHEDETIANPING10",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			asideType = 3,
			sequence = {
				{
					"Royal Navy Rally Point Iberian Peninsula - Strait Fortress",
					1
				},
				{
					"After the Gazer appearance",
					2
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_underheaven_0",
			hidePaintObj = True,
			bgm = "story-clemenceau-judgement",
			say = "The blaring of anti-raid sirens through the air–",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 203070,
			side = 2,
			bgName = "bg_underheaven_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Unidentified vessels spotted! You have entered our:main... Er, the Royal Navy alert perimeter! Identify yourselves!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 203070,
			side = 2,
			bgName = "bg_underheaven_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Repeating! Unidentified vessels, you've...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 203070,
			side = 2,
			bgName = "bg_underheaven_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Miss Warspite, they're not responding.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 203070,
			side = 2,
			bgName = "bg_underheaven_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "They're just approaching silently, not even attacking... Should I keep yelling at them?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 205020,
			side = 2,
			bgName = "bg_underheaven_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Please:. Rather than being the first to attack, we should patiently work to understand the situation.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 203070,
			side = 2,
			bgName = "bg_underheaven_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Roger!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 203010,
			side = 2,
			bgName = "bg_underheaven_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Miss Warspite, we've received word that giant, anomalous machines have appeared in nearby waters and along the coast!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 203010,
			side = 2,
			bgName = "bg_underheaven_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It seems they can move through the sea of their own volition... What should we: about them?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 205020,
			side = 2,
			bgName = "bg_underheaven_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I can see them even from here.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 205020,
			side = 2,
			bgName = "bg_underheaven_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Do we know anything about them?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 203010,
			side = 2,
			bgName = "bg_underheaven_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "They:n't look like Siren weapons, and we think they aren't made from any other faction's technology... In short, no...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 205020,
			side = 2,
			bgName = "bg_underheaven_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Understood. Are we able to communicate with other bases?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 203010,
			side = 2,
			bgName = "bg_underheaven_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "We're:ing our best to restore the connections now.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 203010,
			side = 2,
			bgName = "bg_underheaven_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "We made contact with the Malta base for a moment and learned that they were in a similar situation, but we quickly lost the connection again.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 203010,
			side = 2,
			bgName = "bg_underheaven_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Unfortunately, we still haven't restored contact with the World Expo venue or Royal mainland.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 203010,
			side = 2,
			bgName = "bg_underheaven_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "According to wide-area recon, the seas between the Mediterranean and the NA Sea are covered in fog.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 203010,
			side = 2,
			bgName = "bg_underheaven_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It extends from surface to sky like a wall, making it impossible to measure its height.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 203010,
			side = 2,
			bgName = "bg_underheaven_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "We've sent mass-produced submarines, too, but...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 203010,
			side = 2,
			bgName = "bg_underheaven_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "We lost control of recon ships and submarines alike the moment they entered the foggy area. They're lost at sea.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 203010,
			side = 2,
			bgName = "bg_underheaven_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It's possible that we may be trapped in the Mediterranean.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 203040,
			side = 2,
			bgName = "bg_underheaven_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Oh, no... It's like the clouds combined and turned into a big, scary monster.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 203010,
			side = 2,
			bgName = "bg_underheaven_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "The same thought occurred to me. This is an extremely abnormal situation, so we should be cautious.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 205020,
			side = 2,
			bgName = "bg_underheaven_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "So we should stay calm no matter what, huh?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 205020,
			side = 2,
			bgName = "bg_underheaven_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "The unusual substance emitted by those machines continues to spread. At this rate, it's only a matter of time...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 205020,
			side = 2,
			bgName = "bg_underheaven_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "York, have you warned that mass-produced fleet that we will attack them if they:n't identify themselves?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 203070,
			side = 2,
			bgName = "bg_underheaven_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Yes! I did, but they haven't responded whatsoever!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 205020,
			side = 2,
			bgName = "bg_underheaven_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Then they are the enemy.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 205029,
			side = 2,
			bgName = "bg_underheaven_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "We have no more need for warnings. Her Majesty told us to protect this area, and that is exactly what we will:.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 205029,
			side = 2,
			bgName = "bg_underheaven_0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "All ships, obliterate the enemy fleet! You have the fortress's defenses to support you!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
