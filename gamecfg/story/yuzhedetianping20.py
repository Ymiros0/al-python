return {
	fadeOut = 1.5,
	mode = 2,
	id = "YUZHEDETIANPING20",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			nameColor = "#A9F548FF",
			stopbgm = True,
			bgName = "bg_underheaven_3",
			hidePaintObj = True,
			side = 2,
			bgm = "theme-clemenceau",
			say = "Silence swept across the peak of Mount Vesuvius.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_underheaven_3",
			hidePaintObj = True,
			say = "This was one place where the white substance – the Divine Vestige – had not made its mark yet.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_underheaven_3",
			hidePaintObj = True,
			say = "Battles were being fought across the Mediterranean Sea, but the sounds of war could not reach this tall peak.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Marco Polo",
			bgName = "bg_underheaven_cg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Oh, Clemenceau! Where are you now?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			side = 2,
			actorName = "Clemenceau",
			bgName = "bg_underheaven_cg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "I'm on the way to the safehouse you told me about earlier.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Clemenceau",
			bgName = "bg_underheaven_cg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "The Tribunal's mass-produced ships have arrived, so it's time for me to set off to the Expo.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_underheaven_cg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actorName = "Marco Polo",
			say = "Magnificent! Finally, some good news.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_underheaven_cg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actorName = "Marco Polo",
			say = "It's ridiculous how much pushback we've faced when God's descent is so close to completion... Thank goodness I have you to rely on.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_underheaven_cg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actorName = "Marco Polo",
			say = "Before your final battle, I have something nice to tell you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_underheaven_cg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actorName = "Marco Polo",
			say = "I chose the safehouse you'll be staying in, especially for you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_underheaven_cg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actorName = "Marco Polo",
			say = "It's separated from the Senate's secret facilities by only one block.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_underheaven_cg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actorName = "Marco Polo",
			say = "Also, you'll find Flandre waiting for you there.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_underheaven_cg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actorName = "Marco Polo",
			say = "You heard that right – Flandre, the blueprint ship. The ace up our sleeve, better than anyone else at manifesting religious concepts.",:
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_underheaven_cg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			actorName = "Clemenceau",
			say = "Really? I would've never guessed she was so close by.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_underheaven_cg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actorName = "Marco Polo",
			say = "Hahaha! I reckoned she'd be of use to you, so I had her redeployed.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_underheaven_cg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actorName = "Marco Polo",
			say = "Besides, she belongs to the Holy See, right? It only makes sense that you'd command her, not me.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_underheaven_cg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actorName = "Marco Polo",
			say = "Now go. Rendezvous with Flandre, take the Tribunal's fleet with you, and head for the World Expo!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_underheaven_cg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actorName = "Marco Polo",
			say = "I'll send Trento META to assist you as well.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_underheaven_cg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actorName = "Marco Polo",
			say = "Make those nonbelievers see the light of God!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Clemenceau",
			bgName = "bg_underheaven_cg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "I will, Marco Polo.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Clemenceau",
			bgName = "bg_underheaven_cg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "Don't worry about me. Focus only on completing the ritual to...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Clemenceau",
			bgName = "bg_underheaven_cg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ff5c5c",
			say = "Oh. She hung up.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			stopbgm = True,
			side = 2,
			bgName = "bg_underheaven_3",
			dir = 1,
			bgm = "theme-designshipVI",
			actor = 905020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "So you're going to show your full hand this early. Reckless as always, Marco Polo.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_underheaven_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 905020,
			say = "And I hoped you'd stay in the game for at least a while longer...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 905020,
			side = 2,
			bgName = "bg_underheaven_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "A shame. It truly is a shame.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_underheaven_3",
			hidePaintObj = True,
			say = "Clemenceau lightly shook her head in disappointment,: extended a hand up to her holograms and tapped through a menu.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_underheaven_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 901040,
			say = "Kersaint speaking. I've arrived at the coordinates. What am I supposed to: now?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_underheaven_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 905020,
			say = "I'll tell you. Listen closely. Blueprint ship Flandre will come there to meet you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 905020,
			side = 2,
			bgName = "bg_underheaven_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "She did briefly serve Marco Polo, but I assure you, she's on our side. Whatever you:,:n't attack her.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_underheaven_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 905020,
			say = "Once she has arrived, call everyone to regroup with you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 905020,
			side = 2,
			bgName = "bg_underheaven_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Then, you will go to the new set of coordinates I just sent. Be ready for a battle.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 905020,
			side = 2,
			bgName = "bg_underheaven_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Your enemy is a META – Trento.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_underheaven_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 905020,
			say = "She'll think you're on her side, so try to catch her off guard.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 905020,
			side = 2,
			bgName = "bg_underheaven_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Be quick and incapacitate her with one blow.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 905020,
			side = 2,
			bgName = "bg_underheaven_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "You must not sink her, and you must not let her get away. Just incapacitate her.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 905020,
			side = 2,
			bgName = "bg_underheaven_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Once that's:ne, call me back, and remain on standby until your next orders.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 905020,
			side = 2,
			bgName = "bg_underheaven_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...Did you get all that, or was it too shocking for you? Shall I start over?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_underheaven_3",
			paintingNoise = True,
			dir = 1,
			actor = 901040,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "N-no need, I heard you loud and clear! I've committed it all to memory!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_underheaven_3",
			paintingNoise = True,
			dir = 1,
			actor = 901040,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "By the will of the Holy Iris.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 905020,
			side = 2,
			bgName = "bg_underheaven_3",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Yes. By the will of the Holy Iris.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_underheaven_3",
			hidePaintObj = True,
			say = "Clemenceau terminated the call,: closed all her holograms.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_underheaven_3",
			hidePaintObj = True,
			say = "She grasped her staff firmly, held it high...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_underheaven_3",
			hidePaintObj = True,
			say = "And at that exact moment, Mount Vesuvius erupted.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			bgName = "bg_underheaven_cg5",
			mode = 1,
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			},
			sequence = {
				{
					"",
					0
				}
			}
		},
		{
			side = 2,
			actorName = "Clemenceau",
			bgName = "bg_underheaven_cg5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			say = "The old Tribunal's crowning achievement among conceptual weapons – Blanc, the symbol of Conquest.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Clemenceau",
			bgName = "bg_underheaven_cg5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			say = "Smoke blotting out the sun, flames consuming everything; finally, a horseman to signal the end days.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Clemenceau",
			bgName = "bg_underheaven_cg5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			say = "It really sets the scene for the climactic finale. Heehee.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Clemenceau",
			bgName = "bg_underheaven_cg5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			say = "Now, I'd better leave while I'm ahead of the clock.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Clemenceau",
			bgName = "bg_underheaven_cg5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			say = "Although I've told Marco Polo countless lies, there's one thing I haven't lied about.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Clemenceau",
			bgName = "bg_underheaven_cg5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			say = "That roads covered by the Divine Vestige are truly, truly awful to walk on.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
