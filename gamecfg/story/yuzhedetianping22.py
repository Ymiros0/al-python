return {
	fadeOut = 1.5,
	mode = 2,
	id = "YUZHEDETIANPING22",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			side = 2,
			stopbgm = True,
			bgName = "bg_story_sainthelena1",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "A massive fleet was gathering in the waters outside of Saint Helena's port.",
			bgm = "theme-richelieu",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_sainthelena1",
			hidePaintObj = True,
			say = "Before Richelieu's eyes, countless Iris Libre flags fluttered in the wind.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_sainthelena1",
			hidePaintObj = True,
			say = "But among them was a flag that had not been flown for what felt like an eternity – the Iris Orthodoxy's insignia, representing both Iris Libre and the Vichya Dominion.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_sainthelena1",
			hidePaintObj = True,
			say = "All who saw it couldn't help but wonder – how long has it been? How long have they waited for this moment?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_sainthelena1",
			hidePaintObj = True,
			say = "Richelieu's eyes turned from the flags raised all around her, towards the sea fog hanging low in the distance.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_sainthelena1",
			hidePaintObj = True,
			say = "What had once been a source of mystery and:ubt was now a:orway towards her goals.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_sainthelena1",
			hidePaintObj = True,
			say = "But unlike the sunny, placid seas around her that shimmered with the colors of her homeland, what awaited on the other side of the:orway was the scent of gunpowder and burning debris.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_sainthelena1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 805010,
			say = "(Le Terrible, this is what you were searching for at the Tribunal stronghold...)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_sainthelena1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 805010,
			say = "(But why:es the Tribunal use Siren technology to this extent?)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_sainthelena1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 805010,
			say = "(Clemenceau, what role: you play in all of this...?)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 902020,
			side = 2,
			bgName = "bg_story_sainthelena1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Cardinal, it's almost time.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_sainthelena1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 805010,
			say = "Yes. At long last...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_sainthelena1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 805010,
			say = ".........",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_sainthelena1",
			hidePaintObj = True,
			say = "Richelieu took a deep breath, stepped forward in front of the podium, and felt the weight of her fingertip pressing:wn into the tiny button on the microphone.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_sainthelena1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 805010,
			say = "My friends of the Iris Orthodoxy, listen well–",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_sainthelena1",
			hidePaintObj = True,
			say = "The entire audience fell silent as everyone's attention shifted to the Cardinal's words.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_sainthelena1",
			hidePaintObj = True,
			say = "Richelieu looked out across the people gathered there. She saw her friends – those who she had fought alongside before, and those who she would fight alongside in the battle to come.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_sainthelena1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 805010,
			say = "My friends, you have seen what lies on the other side of that:or–",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 805010,
			side = 2,
			bgName = "bg_story_sainthelena1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Our blue seas, violated by the enemy.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_sainthelena1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 805010,
			say = "Our land, rivers, and skies engulfed in a nightmare from which there is no waking up.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_sainthelena1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 805010,
			say = "As we speak, the anomalies that attacked the World Expo encroach on our world.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_sainthelena1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 805010,
			say = "The flames of war threaten to consume us once again.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 805010,
			side = 2,
			bgName = "bg_story_sainthelena1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "But: not forget – there are still those who would also give anything to snuff out those flames.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_sainthelena1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 805010,
			say = "Yes, our compatriots in the Vichya Dominion are fighting as we speak.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_sainthelena1",
			hidePaintObj = True,
			say = "Richelieu gestured to the:orway in front of them. The scenery on the other side became clearer and more vivid as the sea fog prepared to teleport them.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_sainthelena1",
			hidePaintObj = True,
			say = "Cannons firing desperately, swarms of bizarre enemies, giant machines raining from the sky, a sea choked with unnatural, grasping clouds.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_sainthelena1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 805010,
			say = "Do not forget – both Iris Libre and the Vichya Dominion used to exist as one.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 805010,
			side = 2,
			bgName = "bg_story_sainthelena1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "The Iris Orthodoxy has always been one.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_sainthelena1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 805010,
			say = "However, the misfortunes of the past divided us, misleading us into turning our blades on our own.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 805010,
			side = 2,
			bgName = "bg_story_sainthelena1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "We never asked for those scars that now cover us and our homeland.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_sainthelena1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 805010,
			say = "None of us chose the unreasonable reality we now face today.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_sainthelena1",
			hidePaintObj = True,
			say = "After a pause, she surveyed her compatriots again.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_sainthelena1",
			hidePaintObj = True,
			say = "Then, she reached for a flag standing on the podium—one that she did not normally bear.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_qiongding_7",
			hidePaintObj = True,
			dir = 1,
			actorName = "Richelieu",
			nameColor = "#A9F548FF",
			say = "Despite that, many of you have followed me across the Royal Islands, through the flames of war.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			side = 2,
			actorName = "Richelieu",
			bgName = "bg_qiongding_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Others joined us along the way on this long march toward justice.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_qiongding_7",
			hidePaintObj = True,
			dir = 1,
			actorName = "Richelieu",
			nameColor = "#A9F548FF",
			say = "I, Cardinal Richelieu, firmly believe that we are all driven by the same belief, regardless of how long we have been on this journey.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Richelieu",
			bgName = "bg_qiongding_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "There is no:ubt in my mind that we are all gathered here for a unified purpose–",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_qiongding_7",
			hidePaintObj = True,
			dir = 1,
			actorName = "Richelieu",
			nameColor = "#A9F548FF",
			say = "To see the Iris Orthodoxy become whole once more!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_qiongding_7",
			hidePaintObj = True,
			dir = 1,
			actorName = "Richelieu",
			nameColor = "#A9F548FF",
			say = "We will not turn a blind eye while our home is ravaged!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_qiongding_7",
			hidePaintObj = True,
			dir = 1,
			actorName = "Richelieu",
			nameColor = "#A9F548FF",
			say = "We will not turn a blind eye to the suffering of our friends!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_qiongding_7",
			hidePaintObj = True,
			dir = 1,
			actorName = "Richelieu",
			nameColor = "#A9F548FF",
			say = "We proud warriors will not bend. Not to Sirens, not to the Reenactments, and certainly not to the fear and uncertainty.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_qiongding_7",
			hidePaintObj = True,
			dir = 1,
			actorName = "Richelieu",
			nameColor = "#A9F548FF",
			say = "Even if what lies beyond this:or is a calamity that threatens all of civilization, there is but one thing for us to:.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_qiongding_7",
			hidePaintObj = True,
			dir = 1,
			actorName = "Richelieu",
			nameColor = "#A9F548FF",
			say = "We will defy it, and we will fight in the name of the Iris Orthodoxy to the very end!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_qiongding_7",
			hidePaintObj = True,
			dir = 1,
			actorName = "Richelieu",
			nameColor = "#A9F548FF",
			say = "I, Cardinal Richelieu,: solemnly swear upon this sacred banner...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Richelieu",
			bgName = "bg_qiongding_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I will fight until I breathe my last.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_qiongding_7",
			hidePaintObj = True,
			dir = 1,
			actorName = "Richelieu",
			nameColor = "#A9F548FF",
			say = "I will fight until the banner of the Iris Orthodoxy flies free over our homeland, when no enemy shall ever threaten us again!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Richelieu",
			bgName = "bg_qiongding_7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Come, my compatriots of the Iris Orthodoxy. The time is upon us.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_qiongding_7",
			hidePaintObj = True,
			dir = 1,
			actorName = "Richelieu",
			nameColor = "#A9F548FF",
			say = "Weigh anchor! We set sail for our motherland!",
			effects = {
				{
					active = True,
					name = "speed"
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_qiongding_7",
			hidePaintObj = True,
			say = "Their vessels rumbled like thunderous applause.",
			effects = {
				{
					active = False,
					name = "speed"
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_qiongding_7",
			hidePaintObj = True,
			say = "Thus, the glorious march of a unified Iris Orthodoxy had begun.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			stopbgm = True,
			mode = 1,
			asideType = 3,
			sequence = {
				{
					"Vichya Dominion Territory",
					1
				},
				{
					"Toulon Harbor",
					2
				}
			},
			flashout = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_underheaven_5",
			hidePaintObj = True,
			bgm = "story-clemenceau-judgement",
			say = "Fierce fighting had been raging on for quite some time.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_underheaven_5",
			hidePaintObj = True,
			say = "Burning wreckages now littered the surface of the water.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_underheaven_5",
			hidePaintObj = True,
			say = "Though the defenders had managed to buy a moment of peace, there was no:ubt in their minds that they would not be able to hold off the next onslaught.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_underheaven_5",
			hidePaintObj = True,
			say = "After destroying the final enemy in her line of sight, Jean Bart kneeled, exhausted, and looked at her injured allies.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_underheaven_5",
			hidePaintObj = True,
			say = "What drove them was no longer pride, but an indignation of not wanting it to end like this.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 905010,
			side = 2,
			bgName = "bg_underheaven_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Not again, damn it...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 905010,
			side = 2,
			bgName = "bg_underheaven_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Am I just losing control of my emotions...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 905010,
			side = 2,
			bgName = "bg_underheaven_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "...Or did I make the wrong decision?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 905010,
			side = 2,
			bgName = "bg_underheaven_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Did I lead my friends to destruction again?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_underheaven_5",
			paintingNoise = True,
			dir = 1,
			actor = 807020,
			nameColor = "#ffff4d",
			hidePaintObj = True,
			say = "Jean Bart, the next wave is coming!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_underheaven_5",
			paintingNoise = True,
			dir = 1,
			actor = 807020,
			nameColor = "#ffff4d",
			hidePaintObj = True,
			say = "It's not just mass-produced ships this time! They have more forces in the skies above and waters below...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_underheaven_5",
			paintingNoise = True,
			dir = 1,
			actor = 807020,
			nameColor = "#ffff4d",
			hidePaintObj = True,
			say = "What should we:...?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 905010,
			side = 2,
			bgName = "bg_underheaven_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "(Good question... I damn sure wish I knew the answer.)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 905010,
			side = 2,
			bgName = "bg_underheaven_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "(First Richelieu, now Clemenceau. Why must we suffer betrayal after betrayal?)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_underheaven_5",
			hidePaintObj = True,
			say = "Jean Bart numbly pushed away the call from her communicator and sat:wn in the water.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			stopbgm = True,
			mode = 1,
			bgName = "bg_underheaven_cg7",
			bgm = "story-french2",
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			},
			sequence = {
				{
					"",
					0
				}
			}
		},
		{
			side = 0,
			actorName = "Richelieu",
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Don't give up just yet, Jean Bart.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			actorName = "Jean Bart",
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Sister...?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			actorName = "Jean Bart",
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Tch... What, is that just my life flashing before my eyes? Or is it the stupid black sun making me hallucinate?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			actorName = "Jean Bart",
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "What a cruel joke. So this is who God sends to take me away.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			actorName = "Richelieu",
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Jean Bart, as leaders, there are always those who look towards us for hope.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			actorName = "Richelieu",
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "If even we give up, we shut the:or on those who deserve a brighter tomorrow.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			actorName = "Richelieu",
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Please fight until the last light has faded.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			actorName = "Jean Bart",
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Quoting the Commander, huh? God sure has a funny sense of humor, using my sister's face to say those words...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			say = "BOOOOM!",
			soundeffect = "event./battle/boom2",
			flashN = {
				color = {
					1,
					1,
					1,
					1
				},
				alpha = {
					{
						0,
						1,
						0.2,
						0
					},
					{
						1,
						0,
						0.2,
						0.2
					},
					{
						0,
						1,
						0.2,
						0.4
					},
					{
						1,
						0,
						0.2,
						0.6
					}
				}
			},
			dialogShake = {
				speed = 0.09,
				x = 8.5,
				number = 2
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			say = "The booming of artillery heralded the return of war to the quiet sea.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			say = "The anomalous enemies born from Divine Vestige appeared on the horizon once more.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			say = "However, their attacks were not directed toward Jean Bart's fleet.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actorName = "Vauquelin",
			say = "Jean Bart, reinforcements! Reinforcements have finally come!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actorName = "Vauquelin",
			say = "Look over there! It's the Iris Libre fleet, along with the Tribunal's fighting angel!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actorName = "Vauquelin",
			say = "The enemy is being routed! We're saved, Jean Bart!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actorName = "Dunkerque",
			say = "Phew... I'd just made peace with the fact that I was about to get sunk. Talk about good timing...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actorName = "Dunkerque",
			say = "But, if both the Tribunal and Iris Libre are here,:es that mean... that the conflict with Clemenceau was just an elaborate ruse?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			say = "Jean Bart could not make sense of the words she was hearing.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			say = "Her emotions had been roiling for so long. What did it all mean? Could she afford to have hope again?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			actorName = "Jean Bart",
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Iris Libre... and the Tribunal, together...? What's going on...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			actorName = "Jean Bart",
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "And when I heard Richelieu's voice before... Was that...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			actorName = "Richelieu",
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Sorry I'm late, Jean Bart.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			actorName = "Richelieu",
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "No matter how desperate the situation is, we must not give up on hope.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			actorName = "Richelieu",
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Otherwise, we will not be able to seize that brief glimmer of light when it finally arrives.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			actorName = "Jean Bart",
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "Cardinal Richelieu... Why are you here?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			actorName = "Jean Bart",
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "You didn't come just to laugh at me, did you?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			actorName = "Richelieu",
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "No, I'm here to save someone dear to me.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			actorName = "Richelieu",
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "A certain shipgirl who calls me her sister, in fact.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			actorName = "Jean Bart",
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = ".........",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			actorName = "Richelieu",
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Though, it wouldn't hurt to call me that more often.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			actorName = "Richelieu",
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "For example... During a touching reunion? Such as this one?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			actorName = "Richelieu",
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Repeat after me, now. \"Sister.\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#ffff4d",
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			dir = 1,
			actorName = "Jean Bart",
			say = "Why, you...!",
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			actorName = "Richelieu",
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Sorry, I got a bit carried away there.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			actorName = "Richelieu",
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I'm sure there's a lot you'd like to get off your chest right now, but the enemy is still out there.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			actorName = "Richelieu",
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I'd love to catch up with you later, but let's get right to business first–",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			actorName = "Richelieu",
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Le Terrible told me about the situation at the World Expo venue. We're going to support the Commander next.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			actorName = "Richelieu",
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "But, I can only speak for Iris Libre. The Vichya Dominion will need to make its own choice.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			actorName = "Richelieu",
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Jean Bart, will you join me?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			actorName = "Jean Bart",
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "...I:n't speak for the Vichya Dominion anymore.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			actorName = "Jean Bart",
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			say = "We've been excommunicated by Clemenceau. We're just exiles now.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			actorName = "Richelieu",
			bgName = "bg_underheaven_cg7",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "That:esn't change my offer.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_underheaven_5",
			hidePaintObj = True,
			say = "Jean Bart stood up, took a deep breath, and flipped the switch on her communicator.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 905010,
			side = 2,
			bgName = "bg_underheaven_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "To all those shipgirls who no longer have a place to call home – I am battleship Jean Bart, formerly of the Vichya Dominion, speaking!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 905010,
			side = 2,
			bgName = "bg_underheaven_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I will now join Cardinal Richelieu's fleet, and aid her in fighting for a unified Iris Orthodoxy.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 905010,
			side = 2,
			bgName = "bg_underheaven_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I'm no longer in any position to order any of you around, but...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 905010,
			side = 2,
			bgName = "bg_underheaven_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I hope you'll decide to join us in driving those bastards out of our homeland!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_underheaven_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 807020,
			say = "Jean Bart...!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "bg_underheaven_5",
			paintingNoise = True,
			dir = 1,
			actor = 901030,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "I'd be glad to!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_underheaven_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 904010,
			say = "Of course. Banished or not, we'll follow you to the ends of the earth.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 902010,
			side = 2,
			bgName = "bg_underheaven_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Haha, you stubborn old grouch! I'm just glad you and Algérie finally decided to open up!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 905010,
			side = 2,
			bgName = "bg_underheaven_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...Yeah, it's been too damn long.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 905010,
			side = 2,
			bgName = "bg_underheaven_5",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It's time... to put the past behind us.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			bgName = "bg_underheaven_cg8",
			mode = 1,
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			},
			sequence = {
				{
					"",
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_underheaven_cg8",
			hidePaintObj = True,
			say = "Jean Bart discarded the shattered banner of the Vichya Dominion, and got:wn on one knee in front of Richelieu.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_underheaven_cg8",
			hidePaintObj = True,
			say = "Richelieu: lay her own Iris flag atop Jean Bart's shoulder.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			actorName = "Richelieu",
			bgName = "bg_underheaven_cg8",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "As Cardinal of the Iris Orthodoxy, I solemnly declare...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			actorName = "Richelieu",
			bgName = "bg_underheaven_cg8",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Jean Bart, your unjustly-deprived status as a Templar Knight and your position within the Iris Orthodoxy's fleet are hereby restored to you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			actorName = "Richelieu",
			bgName = "bg_underheaven_cg8",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "The same goes for all the other shipgirls here formerly under the command of the Vichya Dominion.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			actorName = "Richelieu",
			bgName = "bg_underheaven_cg8",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Please fight alongside us, Jean Bart.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			actorName = "Jean Bart",
			bgName = "bg_underheaven_cg8",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = ".........",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			actorName = "Jean Bart",
			bgName = "bg_underheaven_cg8",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "By the will of the Holy Iris...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			actorName = "Jean Bart",
			bgName = "bg_underheaven_cg8",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I swear my allegiance to you, Cardinal Richelieu. Sister.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			actorName = "Richelieu",
			bgName = "bg_underheaven_cg8",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...I've finally come back to you, my sister.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			actorName = "Richelieu",
			bgName = "bg_underheaven_cg8",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Sorry it took me so long.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_underheaven_cg8",
			hidePaintObj = True,
			say = "Cheers erupted over the ocean.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_underheaven_cg8",
			hidePaintObj = True,
			say = "And, without anyone's notice, Gascogne silently slipped away after bearing witness to it all.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
