return {
	fadeOut = 1.5,
	mode = 2,
	id = "YUZHEDETIANPING28",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			mode = 1,
			stopbgm = True,
			asideType = 3,
			blackBg = True,
			sequence = {
				{
					"East Mediterranean Sea - Suez Canal",
					1
				},
				{
					"Meanwhile, with Carabiniere...",
					2
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_underheaven_8",
			dir = 1,
			bgm = "theme-thehierophantV",
			actor = 605020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Carabiniere, are you alright?! Where are you?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_underheaven_8",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 601020,
			say = "Right here! I'm fine! I've just plunged into the affected region with the Dragon Empery fleet!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 601020,
			side = 2,
			bgName = "bg_underheaven_8",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Are you:ing okay, Lady Littorio?! Something awful has happened... not just at the World Expo, but across the whole Mediterranean!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_underheaven_8",
			paintingNoise = True,
			dir = 1,
			actor = 605020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "I'm just fine. Let's be brief here.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_underheaven_8",
			paintingNoise = True,
			dir = 1,
			actor = 605020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "First. what: you mean when you say you've just plunged into the affected region?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 601020,
			side = 2,
			bgName = "bg_underheaven_8",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Right, so, I was in the middle of a tea party with the Dragon Empery representatives by the canal.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 601020,
			side = 2,
			bgName = "bg_underheaven_8",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "That was when Chen Hai noticed that something was wrong. She asked me to contact the Sardegnian mainland...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_underheaven_8",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 601020,
			say = "I couldn't get through, though, so I asked the Dragon Empery reps to return to the venue with me!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 601020,
			side = 2,
			bgName = "bg_underheaven_8",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "And now, umm... We've just entered a sea region covered in a weird substance.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_underheaven_8",
			paintingNoise = True,
			dir = 1,
			actor = 605020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "I see... No wonder you couldn't get through to them. You were outside of the anomaly.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_underheaven_8",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 605020,
			say = "The Dragon Empery shipgirls are with you, right? Carabiniere, connect me to Chen Hai.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_underheaven_8",
			paintingNoise = True,
			dir = 1,
			actor = 605020,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Commander, we have the Dragon Empery fleet.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 506010,
			side = 2,
			bgName = "bg_underheaven_8",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I've been informed of the situation. You've clearly been hard at work...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_underheaven_8",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 506010,
			say = "Our forces here are enough. Leave the anchor near the canal to us.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 506010,
			side = 2,
			bgName = "bg_underheaven_8",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "One more thing. I hear the Skybound Wall in Alexandria was activated.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_underheaven_8",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 506010,
			say = "We have no idea what's going on there. The sandstorm still hasn't stopped...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_underheaven_8",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 506010,
			say = "You:n't know, either? Then we'll just have to leave it aside and hope for the best for now.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_underheaven_8",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 506010,
			say = "We'll: what we can to help investigate. If we learn anything, I'll call you right away.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_underheaven_8",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 506010,
			say = "I pray for your good fortune. Goodbye.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
