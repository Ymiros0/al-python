return {
	fadeOut = 1.5,
	mode = 2,
	id = "YUZHEDETIANPING37-3",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			actor = 900357,
			nameColor = "#BDBDBD",
			bgName = "star_level_bg_504",
			hidePaintObj = True,
			stopbgm = True,
			side = 2,
			say = "▅■cease▆▅",
			bgm = "story-thehierophantV",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_504",
			hidePaintObj = True,
			say = "All of a sudden, I heard a girl's emotionless, mechanical voice inside of my head.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "star_level_bg_504",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 207030,
			say = "What on earth was that?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 602010,
			side = 2,
			bgName = "star_level_bg_504",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			paintingNoise = True,
			say = "I feel like... I'm slowing:wn. My rigging's grind to a halt!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 718010,
			side = 2,
			bgName = "star_level_bg_504",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Mine, too! Commander, we're all affected by whatever this is!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_504",
			paintingNoise = True,
			dir = 1,
			actor = 9707010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "Same here. I can't... move... Urgh...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_504",
			paintingNoise = True,
			dir = 1,
			actor = 9707010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "Argh! Ark Royal! My head!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_504",
			paintingNoise = True,
			dir = 1,
			actor = 9707020,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "Damnit! The energy in my hull's going completely haywire!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_504",
			paintingNoise = True,
			dir = 1,
			actor = 9707020,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "Memphis? Are you hanging in there?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_504",
			paintingNoise = True,
			dir = 1,
			actor = 9702020,
			nameColor = "#a020f0",
			hidePaintObj = True,
			say = "Barely... Somehow, yeah...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_504",
			hidePaintObj = True,
			say = "With but a single otherworldly word, the entire battlefield was gripped by an icy, death-like chill.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_504",
			actor = 307080,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Did you hear that voice too? What manner of enchantment is this?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_504",
			hidePaintObj = True,
			say = "\"Yes. I can still move like normal. Whatever it is, it:esn't look like it affected me.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_504",
			hidePaintObj = True,
			say = "\"You:n't look like it affected you much either, for that matter, Shinano.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_504",
			hidePaintObj = True,
			say = "\"Could it be that my command vessel is immune to this phenomenon?\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 905020,
			side = 2,
			bgName = "star_level_bg_504",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "No, it's not. I'm affected, albeit it not to the same degree as the rest.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_504",
			actor = 307080,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Most puzzling... So the effect varies from shipgirl to shipgirl...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "star_level_bg_504",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 499070,
			say = "I'm basically fine. My arms and legs just feel a little heavy.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "star_level_bg_504",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 499040,
			say = "Likewise, I am still fit for battle. I feel hardly affected at all.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "star_level_bg_504",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 107090,
			say = "Same with the mass-produced ships... It's:ne practically nothing to them. They can still fight!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_504",
			hidePaintObj = True,
			say = "(So the blueprint ships and the mass-produced ships aren't affected much, while the METAs are gravely affected.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_504",
			hidePaintObj = True,
			say = "(Among everyone on the command ship, only Shinano hasn't been affected at all.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_504",
			paintingNoise = True,
			dir = 1,
			actor = 9702020,
			nameColor = "#a020f0",
			hidePaintObj = True,
			say = "It:esn't seem like my hull's being corrupted, but...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_504",
			paintingNoise = True,
			dir = 1,
			actor = 9702020,
			nameColor = "#a020f0",
			hidePaintObj = True,
			say = "I've felt this pain before... It feels like the exact same thing...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_504",
			paintingNoise = True,
			dir = 1,
			actor = 9707010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "Urgh... I'm... all but certain... that it's the same thing...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_504",
			paintingNoise = True,
			dir = 1,
			actor = 9707010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "Commander... Is it too late to retreat now...?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "star_level_bg_504",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a020f0",
			actor = 9702020,
			say = "Retreat? You're just gonna run away, Hiryuu?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_504",
			paintingNoise = True,
			dir = 1,
			actor = 9707010,
			nameColor = "#ffa500",
			hidePaintObj = True,
			say = "I'm not talking about me... We all... need to get out of here... before it's too late!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_504",
			paintingNoise = True,
			dir = 1,
			actor = 9702020,
			nameColor = "#a020f0",
			hidePaintObj = True,
			say = "Haha.. Sorry...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_504",
			paintingNoise = True,
			dir = 1,
			actor = 9702020,
			nameColor = "#a020f0",
			hidePaintObj = True,
			say = "But if we run away with the Commander... what will happen to this experimental field...?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 905020,
			side = 2,
			bgName = "star_level_bg_504",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Whatever you're thinking, we cannot have you take the Commander from us.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_504",
			actor = 905020,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "We're lucky a certain someone here has barely been affected...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_504",
			actor = 905020,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Gascogne! Force a direct connection into cognitive space!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_firedust_5",
			hidePaintObj = True,
			stopbgm = True,
			dialogueBgAlpha = 0.6,
			say = "A black sun, quietly burning in the dusky sky.",
			bgm = "theme-vichy-church",
			effects = {
				{
					active = True,
					name = "yuanzhou_juqing01",
					interlayer = 1001
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 0.25,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.25,
				dur = 0.25,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_firedust_5",
			dialogueBgAlpha = 0.6,
			dir = 1,
			actor = 307080,
			nameColor = "#A9F548FF",
			say = "The black sun...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_firedust_5",
			dialogueBgAlpha = 0.6,
			dir = 1,
			actor = 307080,
			nameColor = "#A9F548FF",
			say = "This is that other sacrament Marco Polo received from God...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 905020,
			side = 2,
			dialogueBgAlpha = 0.6,
			bgName = "bg_firedust_5",
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Correct. The problem with it is that's nigh-impossible to control.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_firedust_5",
			dialogueBgAlpha = 0.6,
			dir = 1,
			actor = 905020,
			nameColor = "#A9F548FF",
			say = "How is everyone? Can you move freely?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_firedust_5",
			dialogueBgAlpha = 0.6,
			dir = 1,
			actor = 301290,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Yes! Thank you kindly, Clemenceau!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_firedust_5",
			dialogueBgAlpha = 0.6,
			dir = 1,
			actor = 9702020,
			nameColor = "#a020f0",
			hidePaintObj = True,
			say = "Forcibly connecting minds to a separate conceptual space to protect against an anti-conceptual weapon in reality. That must've taken a huge amount of energy...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_firedust_5",
			dialogueBgAlpha = 0.6,
			dir = 1,
			actor = 9702020,
			nameColor = "#a020f0",
			hidePaintObj = True,
			say = "Depending on how you use it, it can even affect your opponent's mind...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_firedust_5",
			dialogueBgAlpha = 0.6,
			dir = 1,
			actor = 9702020,
			nameColor = "#a020f0",
			hidePaintObj = True,
			say = "(This is Arbiter-level technology. Can we even be sure this space:esn't connect to somewhere else?)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 605010,
			side = 2,
			bgName = "bg_firedust_5",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			dialogueBgAlpha = 0.6,
			say = "So if understand correctly... the black sun is actually located in a cognitive plane of existence...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 605010,
			side = 2,
			bgName = "bg_firedust_5",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			dialogueBgAlpha = 0.6,
			say = "How are we supposed to get back from here?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900357,
			side = 2,
			bgName = "bg_firedust_5",
			hidePaintObj = True,
			nameColor = "#BDBDBD",
			dialogueBgAlpha = 0.6,
			say = "▅■one▆▅",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_504",
			hidePaintObj = True,
			stopbgm = True,
			say = "For the first time since the battle began, the Hierophant moved her body.",
			bgm = "battle-thehierophantV",
			effects = {
				{
					active = False,
					name = "yuanzhou_juqing01"
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 0.25,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.25,
				dur = 0.25,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_504",
			hidePaintObj = True,
			say = "She raised the wand in her hands high above her,: swung it:wn...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_504",
			hidePaintObj = True,
			say = "...Instantly shattering the vision of the black sun like a glass pane.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_504",
			hidePaintObj = True,
			say = "Just like that, she was in control of the world once more.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_504",
			actor = 905020,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "The symbol of judgment was destroyed THAT easily?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_504",
			actor = 905020,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Did we overload the connection? Or is this Arbiter simply that much stronger than it?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_504",
			paintingNoise = True,
			dir = 1,
			actor = 9702020,
			nameColor = "#a020f0",
			hidePaintObj = True,
			say = "Neither. That wand... The ability to recognize and control concepts...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "star_level_bg_504",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a020f0",
			actor = 9702020,
			say = "This isn't just one of the Hierophant's vessels! She's here, in person!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_504",
			paintingNoise = True,
			dir = 1,
			actor = 9702020,
			nameColor = "#a020f0",
			hidePaintObj = True,
			say = "Be extremely careful! You: NOT want to be hit by attacks from that wand!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "star_level_bg_504",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a020f0",
			actor = 9702020,
			say = "It'll either turn you to dust, or if you're really unlucky, the very concept of your existence will be wiped from reality!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "star_level_bg_504",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 9707010,
			say = "Good to know... Too bad I'm basically stuck in place!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_504",
			paintingNoise = True,
			dir = 1,
			actor = 9702020,
			nameColor = "#a020f0",
			hidePaintObj = True,
			say = "At least those attacks are time-consuming and short-ranged... Commander, rush her with the mass-produced ships and organize a fleet out of the blueprint ships!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "star_level_bg_504",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#a020f0",
			actor = 9702020,
			say = "The Hierophant's grip on everyone is letting up! Keep her busy while we get back on our feet!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
