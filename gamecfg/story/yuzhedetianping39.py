return {
	fadeOut = 1.5,
	mode = 2,
	id = "YUZHEDETIANPING39",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			side = 2,
			stopbgm = True,
			bgName = "star_level_bg_506",
			hidePaintObj = True,
			nameColor = "#A9F548FF",
			say = "Clemenceau gazed sadly at the salvaged throne before her.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_506",
			hidePaintObj = True,
			say = "She could not stop the memories of their first meeting from rushing through her head.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			mode = 1,
			stopbgm = True,
			asideType = 3,
			blackBg = True,
			sequence = {
				{
					"Some time in the past...",
					1
				},
				{
					"Unknown location in Iron Blood",
					2
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			stopbgm = True,
			bgName = "star_level_bg_506",
			hidePaintObj = True,
			side = 2,
			bgm = "theme-clemenceau",
			say = "On one side of the room was the schemer, representing the dark side of the shining Orthodoxy. On the other side was a jester, charming yet ambitious.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_506",
			hidePaintObj = True,
			say = "The two could hardly have less in common, save for one thing – they had both been bored to death by the preceding meeting.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 699010,
			side = 2,
			bgName = "star_level_bg_506",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "So you're Clemenceau, the Vichya's representative?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_506",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 905020,
			say = "That I am. And I already know your name well, Marco Polo.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_506",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 699010,
			say = "The Iron Blood folks and your lackeys are gone. Why are you still here?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_506",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 905020,
			say = "Hmm~ I like jotting:wn notes while the memory is still fresh in my mind.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 699010,
			side = 2,
			bgName = "star_level_bg_506",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Magnificent. You play the part of a bureaucrat perfectly.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 699010,
			side = 2,
			bgName = "star_level_bg_506",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "But, you didn't speak at all during the meeting. Do you not like dealing with Iron Blood?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_506",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 905020,
			say = "I wouldn't say that. The Vichya and Iron Blood are close friends, after all.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_506",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 699010,
			say = "Cut it out already. There's nobody else here!",:
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_506",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 699010,
			say = "Stop putting on airs already. I'm only speaking to you because I see your potential!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 699010,
			side = 2,
			bgName = "star_level_bg_506",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Look, the two of us are kindred spirits. Everyone else? They're a whole bunch of fools who:n't know a thing.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 699010,
			side = 2,
			bgName = "star_level_bg_506",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "If you:n't like their proposals,: why:n't you lend me your ear?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_506",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 699010,
			say = "Hahaha! Magnificent! I can tell, the two of us are going to accomplish great things together!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
