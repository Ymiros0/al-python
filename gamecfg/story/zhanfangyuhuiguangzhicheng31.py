return {
	id = "ZHANFANGYUHUIGUANGZHICHENG31",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			nameColor = "#5CE6FF",
			side = 2,
			bgName = "bg_camelot_4",
			hidePaintObj = True,
			say = "There was total darkness, broken up only by the faint light of an instrument panel.",
			bgm = "battle-thedevilxv-control",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#5CE6FF",
			side = 2,
			bgName = "bg_camelot_4",
			hidePaintObj = True,
			say = "The place was abuzz with electrical noise and the sound of running machines – it was a control room of some sort.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900335,
			side = 2,
			bgName = "bg_camelot_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FF9B93",
			say = "Gee, it's kind of dark.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#5CE6FF",
			side = 2,
			bgName = "bg_camelot_4",
			hidePaintObj = True,
			say = "A girl sat amid the chaotic crisscross of:zens of wires, preoccupied with what was right in front of her.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900335,
			side = 2,
			bgName = "bg_camelot_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FF9B93",
			say = "If I force-boot it... it'll push the power system to its limit.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900335,
			side = 2,
			bgName = "bg_camelot_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FF9B93",
			say = "Have to focus on hacking the security instead.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900335,
			side = 2,
			bgName = "bg_camelot_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FF9B93",
			say = "Then the first thing I'll: is turn the lights back on.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900335,
			side = 2,
			bgName = "bg_camelot_4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FF9B93",
			say = "Ugh... This place is too damn dark.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_camelot_4",
			actor = 9702030,
			dir = 1,
			nameColor = "#FFC960",
			say = "......",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_camelot_4",
			actor = 9702030,
			dir = 1,
			nameColor = "#FFC960",
			say = "Indeed. It IS too dark.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
