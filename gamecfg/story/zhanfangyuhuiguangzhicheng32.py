return {
	id = "ZHANFANGYUHUIGUANGZHICHENG32",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			nameColor = "#5CE6FF",
			side = 2,
			bgName = "bg_zhuguang_6",
			hidePaintObj = True,
			say = "Test Site Beta - Outer Defensive Perimeter",
			bgm = "theme-richard",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#5CE6FF",
			side = 2,
			bgName = "bg_zhuguang_6",
			hidePaintObj = True,
			say = "Two Arbiters, the Lovers and the Empress, had erected a defensive line to ward off the invasion of the X.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#5CE6FF",
			side = 2,
			bgName = "bg_zhuguang_6",
			hidePaintObj = True,
			say = "This clash between the unstoppable and the immovable was still being waged.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_zhuguang_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#BDBDBD",
			say = "You people make me laugh.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_zhuguang_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#BDBDBD",
			say = "Come on, can't you call it quits already?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_zhuguang_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#BDBDBD",
			say = "I'm only gonna pay a small visit. You're being dramatic about this.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_zhuguang_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#BDBDBD",
			say = "Look at yourselves. You've constructed an airtight defensive line, and what good has it:ne?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_zhuguang_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#BDBDBD",
			say = "Test site beta is already a giant mess, and I haven't even gotten in yet.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_zhuguang_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#BDBDBD",
			say = "Like, what's the point in keeping me out?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900320,
			side = 2,
			bgName = "bg_zhuguang_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#5CE6FF",
			say = "Your logic is flawed.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900320,
			side = 2,
			bgName = "bg_zhuguang_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#5CE6FF",
			say = "Allowing you inside would only make a bad situation even worse.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900320,
			side = 2,
			bgName = "bg_zhuguang_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#5CE6FF",
			say = "I told you before – your battlefield is HERE.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900320,
			side = 2,
			bgName = "bg_zhuguang_6",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#5CE6FF",
			say = "You're patient, no? Then show us how patient you can be.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#5CE6FF",
			side = 2,
			bgName = "star_level_bg_539",
			hidePaintObj = True,
			say = "Somewhere, in the Pacific...",
			bgm = "theme-warmwinter-daily",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_539",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			actor = 9702070,
			say = "Ahh! Sunshine, sand, and a tropical drink!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_539",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			actor = 9702070,
			say = "I needed this vacation.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_539",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			actor = 9702070,
			say = "If they all wanna busy themselves, they can: it without me.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9702070,
			side = 2,
			bgName = "star_level_bg_539",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			say = "I'll just be here, chilling.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "PA System",
			bgName = "star_level_bg_539",
			hidePaintObj = True,
			nameColor = "#5CE6FF",
			say = "*sirens blaring*",
			bgm = "airraidalarm",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_539",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			actor = 9702070,
			say = "What?! An air raid alarm?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_539",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			actor = 9702070,
			say = "But I haven't even:ne anything!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_539",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			actor = 9702070,
			say = "Are Sirens invading or something?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_539",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			actor = 9702070,
			say = "I thought this was the Eagle Union's own backyard?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_539",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			actor = 9702070,
			say = "What's going on?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_539",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#FFC960",
			actor = 9702070,
			say = "...Oh.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
