return {
	id = "ZHANFANGYUHUIGUANGZHICHENG34",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			bgmDelay = 2,
			mode = 1,
			asideType = 3,
			stopbgm = True,
			blackBg = True,
			bgm = "story-startravel",
			flashout = {
				black = True,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = True,
				alpha = {
					1,
					0
				}
			},
			sequence = {
				{
					"Orthodoxy Holy Capital - Conference Venue",
					1
				},
				{
					"Commander's Lounge",
					2
				},
				{
					"Following the operation's conclusion",
					3
				}
			}
		},
		{
			nameColor = "#5CE6FF",
			side = 2,
			bgName = "star_level_bg_542",
			hidePaintObj = True,
			say = "I gather up my resolve and open the:or to my lounge.",
			bgm = "story-richang-sooth",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#5CE6FF",
			side = 2,
			bgName = "star_level_bg_542",
			hidePaintObj = True,
			say = "The inside looks practically the same as I left it. There's no one here to welcome me back, either.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_542",
			hidePaintObj = True,
			nameColor = "#5CE6FF",
			say = "(Phew. Elizabeth and Richelieu really did cover for me. No one caught on.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_542",
			hidePaintObj = True,
			nameColor = "#5CE6FF",
			say = "(There's a lot, and I mean A LOT to think about for this operation. I've kept some notes, but they're rough.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_542",
			hidePaintObj = True,
			nameColor = "#5CE6FF",
			say = "(I'd better write:wn the details while they're still fresh in my memory.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "Computer",
			bgName = "star_level_bg_542",
			hidePaintObj = True,
			nameColor = "#5CE6FF",
			say = "BEEP BEEP!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_542",
			hidePaintObj = True,
			nameColor = "#5CE6FF",
			say = "Hm? A message as soon as I get back?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_542",
			hidePaintObj = True,
			nameColor = "#5CE6FF",
			say = "(Huh... It's from the Eagle Union.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_542",
			hidePaintObj = True,
			nameColor = "#5CE6FF",
			say = "\"There has been an as-of-yet unexplained incident in the Pacific.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_542",
			hidePaintObj = True,
			nameColor = "#5CE6FF",
			say = "\"During a 48-hour period, a large number of naval bases received large quantities of data which all contradict one another.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_542",
			hidePaintObj = True,
			nameColor = "#5CE6FF",
			say = "\"Additionally, contact with PH was briefly lost, determined later to be due to communication line issues.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_542",
			hidePaintObj = True,
			nameColor = "#5CE6FF",
			say = "\"Commander, you are to remain in the Orthodoxy for your own safety. Proceed with the conference as normal.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_542",
			hidePaintObj = True,
			nameColor = "#5CE6FF",
			say = "\"Enterprise is in charge of monitoring the Pacific situation as it develops. She will contact you with any eventual updates.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_542",
			hidePaintObj = True,
			nameColor = "#5CE6FF",
			say = "I'm so confused by... whatever this is.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_542",
			hidePaintObj = True,
			nameColor = "#5CE6FF",
			say = "What: they mean by \"unexplained incident\"?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_542",
			hidePaintObj = True,
			nameColor = "#5CE6FF",
			say = "(I:n't recognize the sender either. Could be Naval HQ, could be someone I know.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_542",
			hidePaintObj = True,
			nameColor = "#5CE6FF",
			say = "(Although, given how scarce the details are, it's probably Naval HQ. Something happened and they can't explain it.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_542",
			hidePaintObj = True,
			nameColor = "#5CE6FF",
			say = "I'll just let this simmer for a bit: call Enterprise and Saratoga.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_542",
			hidePaintObj = True,
			nameColor = "#5CE6FF",
			say = "(I wonder if this happened specifically in the Pacific...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_542",
			hidePaintObj = True,
			nameColor = "#5CE6FF",
			say = "(It's adjacent to many factions. The Eagle Union, Northern Parliament, Sakura Empire, even the Dragon Empery.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_542",
			hidePaintObj = True,
			nameColor = "#5CE6FF",
			say = "(The Royal Navy, Iron Blood, and Iris Orthodoxy all have bases there, too.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_542",
			hidePaintObj = True,
			nameColor = "#5CE6FF",
			say = "(At any rate, I'll make some calls and check if anyone knows something.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_542",
			hidePaintObj = True,
			nameColor = "#5CE6FF",
			say = "(The lack of details about this \"incident\" gives me some major premonitions, though...)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_542",
			hidePaintObj = True,
			nameColor = "#5CE6FF",
			say = "(I really hope my fears prove unfounded.)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
