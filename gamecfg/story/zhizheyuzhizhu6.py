return {
	fadeOut = 1.5,
	mode = 2,
	id = "ZHIZHEYUZHIZHU6",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = True,
			mode = 1,
			sequence = {
				{
					"The Weaver's Omen\n\n<size=45>6 Late-Night Emergency</size>",
					1
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_fuxiangxian_2",
			say = "Floating Fortress A1 - Sakura Empire Guest Area",
			bgmDelay = 2,
			bgm = "theme-shallowoftheworld",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 9704010,
			say = "Finally. I was honestly afraid you weren't going to open the:or.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "You were gonna break the:or:wn at that rate! Who WOULDN'T open it?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			actor = 9704010,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "Everyone else, it seems. I've tried every other:or already.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "You're joking, right? What: you mean you've tried every other:or?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 9704010,
			say = "That despite my banging, nobody answered, so I had to force my way in. Everyone I've found is unconscious.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "So you're saying... we're the only people awake right now?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 9704010,
			say = "Indeed. Some unexplained phenomenon has left everyone from Iron Blood in a comatose state.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9704010,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "Presumably, you are unaffected since you're from the Sakura Empire. In my case...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Because you're from a different branch, right?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "This is crazy... Iron Blood has full control over this Mirror Sea, and the Sirens couldn't possibly have launched this kind of sneak attack.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 9704010,
			say = "But it is not out of the realm of possibility for the Arbiters.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 9704010,
			say = "Or, consider the possibility that there's an even more dangerous entity out there that is neither Arbiter nor shipgirl...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9704010,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "In any case, that's the situation at hand. I should've been able to derive it when I saw Ulrich's condition earlier...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 9704010,
			say = "She's been suffering from persistent nightmares and even visual hallucinations over the last few days.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 9704010,
			say = "While I initially bought into her explanation that they were related to stress or her environment...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 9704010,
			say = "I'm almost certain that isn't the case.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 9704010,
			say = "This feels more like... sets of eyes, fixated on Ulrich.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Uhh... Could you run that by me again, but more simply?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 9704010,
			say = "It's hard to understand unless you've fought against the Arbiters the way we have. In any case, someone's targeting Ulrich, and everyone else just happens to be collateral.",:
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 9704010,
			say = "And, my META intuition has a pretty good idea of who's behind it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Probability is one thing, 'cause you've fought them before, but what: you mean by \"META intuition\"?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 9704010,
			say = "I can't explain it. It's instinct – a gut feeling that something from beyond this branch is here.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 9704010,
			say = "At least you can take solace in the fact that it's not after you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 9704010,
			say = "Turn around, close your eyes, and nothing bad will happen to you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9704010,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "Now, if you'll excuse me, I'm heading to bed.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...Bed?! Are you insane?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			},
			effects = {
				{
					active = True,
					name = "speed"
				}
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Your friends are under attack, and you're gonna: nothing?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			effects = {
				{
					active = False,
					name = "speed"
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 9704010,
			say = "Friends? I'm afraid you're mistaken.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9704010,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "The \"Iron Blood\" here is nothing more than a fickle illusion. My only connection to this place... is Bismarck.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Liar! If you:n't care about them, why'd you even bother trying to find out what's going on?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Why'd you come knocking on my:or with your full set of rigging out?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "These people matter to you and you want to save them. That's a fact!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 9704010,
			say = "......",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Fine, if you won't: it for them,:: it for me. Help me resolve this situation– please.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 9704010,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			say = "...Very well.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "*sigh*... Just say so from the very beginning:...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "So, where: we start? Do we head to Ulrich's room and–",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_fuxiangxian_2",
			say = "BEEP– BEEP– BEEP–",
			soundeffect = "event./ui/alarm",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Huh? What's happening?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			actor = 307060,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Did something just get turned on? Is the Flying Fortress starting to move?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 9704010,
			say = "I believe that noise came from the Tower of Midgard...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_fuxiangxian_2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffa500",
			actor = 9704010,
			say = "Someone has activated the teleporter!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
