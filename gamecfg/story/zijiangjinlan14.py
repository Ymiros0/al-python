return {
	fadeOut = 1.5,
	mode = 2,
	id = "ZIJIANGJINLAN14",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg3",
			hidePaintObj = True,
			stopbgm = True,
			say = "Sanctuary Pagoda - Reception Room",
			bgm = "theme-musashi-inside",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg3",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "Nagato, I wasn't expecting you to enter the battle so soon.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg3",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "To think that a bit of testing the waters would trigger such a strong rebuke... Perhaps I underestimated that girl's resolve.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg3",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.82}",
			say = "Maybe not. That might've just been what Nagato wanted the whole time - to stand on the front lines.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg3",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.91}",
			say = "She would put herself in harm's way just to prevent unnecessary conflict...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg3",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.91}",
			say = "Because of that antiquated ego, she fails to see the bigger picture. Her fixation on \"protecting\" what is dear to her may well cause her to lose everything. How foolish.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg3",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.91}",
			say = "There is no future to be had:wn that road, with or without fighting.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg3",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "Regardless, by sacrificing her advantage as a \"player\" in this match, her forces now vastly eclipse ours in combat effectiveness.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg3",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.91}",
			say = "So, she insists on standing in the way... Mikasa,: you have what it takes to keep up with her?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg3",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.82}",
			say = "Keep up? Hah, I:n't have the energy or determination that girl has. The most I can: is to fill in the gaps behind her as she forges a new path forward.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg3",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.82}",
			say = "Now that the balance of power has been broken, shouldn't we go back to securing as many bases as we can instead of fighting each other?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg3",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.91}",
			say = "You may be right in saying that Nagato is forging a new path forward.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg3",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.91}",
			say = "But:es it not fall upon us to determine if that path leads to the light... or to the abyss?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg3",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.91}",
			say = "As someone responsible for shouldering the Sakura Empire's future, there are many other things to consider than your own narrow sense of justice. One cannot make judgments based on speculation and assumptions alone.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg3",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.82}",
			say = "I cannot deny what you have:ne for us. But at the same time, it is too dangerous to work with the Sirens.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg3",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.82}",
			say = "The Commander informed me about the Iron Blood incident not too long ago. Unlike in our case, the Commander was involved with their plan to a far greater degree.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg3",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.82}",
			say = "Plus, both Bismarck and Friedrich shared the same vision despite their diverging paths... That is why they were able to complete their lengthy project in the end.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg3",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.82}",
			say = "If Nagato and I could understand the path that you've chosen, perhaps we might've also been able to work together towards a common goal.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg3",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.82}",
			say = "But, your heart is just as opaque as the sea fog concealing this board.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg3",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.91}",
			say = "...In order to outwit the Sirens, you must first learn the taste of the poison they serve.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg3",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.82}",
			say = "Akagi, you...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg3",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.91}",
			say = "After the success of the Iron Blood rebellion, the Sirens have become much more vigilant.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg3",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.91}",
			say = "Now that Nagato has made her intentions clear, I will crush her and demonstrate the superiority of my vision.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg3",
			hidePaintObj = True,
			say = "With Akagi's declaration, a dazzling light enveloped both her and the board, just as it had with Nagato previously.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			say = "When the light faded, only two people remained around the board.",
			flashout = {
				black = False,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = False,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "She also wants to prove herself, huh... Just like Nagato, she too can only: things in her own way.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900330,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "The balance of power has again been restored with Akagi's entry into the battle.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900330,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "She will most likely join up with Miyuki to cut you off from Nagato's forces.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900330,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Now the question is, will she try to gobble up your bases, or will she launch an attack on the central tower?",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "She's not here anymore, so it'll be harder to read her intentions.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It would appear that the youngsters are too impatient to keep our little game of chess going. Hahaha!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900330,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Hehe. I'm sure you were no less hot-blooded yourself during that war. Isn't that right, Great Veteran?",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "But that was a completely different era, Musashi. A long, long time ago, wasn't it?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Even if I tried to run off like them, my body wouldn't move. After all, there are people leaning on me now.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...Musashi, be honest with me. This exercise area is actually a Mirror Sea, isn't it?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900330,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "What makes you say that?",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Mapping one's mental imagery into existence, the sudden disappearance of Nagato and Akagi... Those aren't things that should be possible through the Sakura Islands' rituals.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900330,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I wonder,: you really know everything there is to know about our rituals?",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "...Probably not.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "But, that's why I want to know. Pardon me if it seemed like I was accusing you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900330,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "No, there's no need for that. Your instincts were correct - this is indeed a Mirror Sea, entrusted to Akagi by the Sirens.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900330,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Originally, it was a Siren experimental site that none of us could use. But after combining it with spirit barriers, I was able to turn it into the:main you now see.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I'm not sure if what you've:ne is amazing or borderline heretical...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900330,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I'll take that as high praise from the Great Veteran. I: wonder though... Has this place been to your liking?",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I:n't think it matters whether or not I \"like\" it. The reality is, I'm stunned by the number of things here that seem to defy conventional wisdom.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "And even more impressively, you're able to control these things. Perhaps the stories of the Iron Blood singularity aren't as far-fetched as I initially believed.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900330,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I've also heard of a Sculpting Room that generates storms to keep everyone who:es not possess the \"Key\" away.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900330,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "In any case, the actual Mirror Sea itself is sealed away, but I did not disclose this to you or Nagato knowing the suspicions both of you have towards Siren technology.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "There was once a period of time when I was paranoid of Akagi's every move. At least now, I have fewer:ubts about her intentions.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "By the way, how: you command your allies after entering the battlefield? To my understanding, our communications are not affected the way they would be in a Mirror Sea?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "We're able to give instructions directly to the entire fleet from here, and losing that ability would be a huge disadvantage.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900330,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Don't worry. You won't be able to immediately grasp everyone's positions at once, but you will still be able to issue commands to them as before.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I see, so there aren't as many disadvantages as I imagined. So, for our allies inside, there's no difference in how they're getting their orders?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900330,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Correct.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900330,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "There is a key difference though. when the retainer is unable to hear the player's voice, she has no choice but to obey the order. But if she is able to voice her opinions, she may be able to influence those orders.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900330,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "You will also be able to directly observe the condition of your forces, and that may cause you to make different decisions than if you treated them as pieces on a board.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900330,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I wonder if Nagato and Akagi were aware of this before they made their decisions.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I'd imagine so. They chose to fight alongside their friends rather than sit out here enjoying tea with us.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900330,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Hehe. Can't blame them.",
			hidePaintEquip = True,
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Hahaha. Well:, as I told Akagi, I will remain here a bit longer.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Show me what kind of moves the three of you will make.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
