return {
	fadeOut = 1.5,
	mode = 2,
	id = "ZIJIANGJINLAN18",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			stopbgm = True,
			say = "Sanctuary Pagoda - Reception Room",
			bgm = "theme-musashi-inside",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "I see you've finally caught a small glimpse into this game's True purpose, Nagato.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "It's just as you said, Mikasa - she is prepared to: what it takes in order to secure her \"bloodless\" victory...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "...Even if she knows it is a waste of the immense power bestowed upon her. Did you predict this as well?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.82}",
			say = "Hah, I'm not a mind-reader, nor: I claim to know everything about Nagato. But what I: know is, now that she has chosen a path, it is my duty to follow it through.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.82}",
			say = "If Nagato:es decide to fight, I will gladly take up arms by her side. Both you and Akagi were aware of this before the game even began, right?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "In my eyes, you seem more concerned with keeping Akagi in check than facilitating Nagato's victory.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.82}",
			say = "Possibly? It sometimes seems like you know me better than I know myself.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "Compassion for the weak may be a virtue, but on the battlefield, it may just as well be your undoing.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "I can easily understand your loyalties and what you are aiming for... But why are you keeping that child from facing Akagi?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "Your actions: not pave the path to victory. Instead of showing me a vision of the future, you still seem to be shackled by your mistrust.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "Now, your fate is no longer in your own hands.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.82}",
			say = "Mistrust? No, it is a measured anticipation more than anything else. Everything depends on what Akagi decides to: - and I have accepted this.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "Once you've been put on the back foot, you will continue to remain at a disadvantage. Can someone who has given up the initiative keep her eyes on the future when she cannot even keep up with the present?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.82}",
			say = "Initiative is not everything. We will find out soon enough if Akagi has the discipline to stay, or if she ends up going bust in her haste.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "Heh. Right you are, but what will you: if she:esn't end up going bust?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.82}",
			say = "I can tell where you'd prefer to hedge your bets. I won't take it personally if we end up standing on opposite sides.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "Surely you jest, Great Veteran~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.82}",
			say = "At this point, I still have time on my side. You're not scrambling to make any moves either, are you?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			stopbgm = True,
			say = "True. Unlike Nagato, you are not so easy to provoke.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "But surely this isn't what you're interested in. You've had questions for me this whole time, haven't you?",
			bgm = "theme-musashi-outside",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.82}",
			say = "Too many things have been on my mind. Things that:n't make any sense to me.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.82}",
			say = "For example, why did you send Shinano to Sardegna, and Zuikaku to Iron Blood?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.82}",
			say = "Neither of these things were ordered by the higher-ups, and these decisions were made on extremely short notice.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "Are you suggesting that I'm deliberately keeping them away from the Sakura Empire?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.82}",
			say = "I can't think of any other explanation.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "Shinano is a good representative for our goodwill delegation to Sardegna, and Zuikaku already has some standing with Iron Blood following her visit to their floating fortress.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "Do you disagree with my choice of delegates?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.82}",
			say = "Then, what about the Watatsumi? I heard it had been returned to us, but before anyone realized it, it had gone missing again.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.82}",
			say = "None of us get to arbitrarily decide what to: with the Sakura Empire's relics. Where is it?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "Due to the attack on the Diadem of Light, the security protocols regarding our sacred relics have been called into question, so I had it moved to a safer location.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.82}",
			say = "...Do you know anything about the Watatsumi's origins? The more I look into it, the more I:ubt it's actually an ancient relic passed:wn from generation to generation.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.82}",
			say = "I get the same impressions of the Sanctuary. I can't help but feel that Sirens are involved somehow.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "That's news to me. I:n't know any more about these things than you:, unfortunately.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "Perhaps, I know even less. After all, Akagi's dealings with the Sirens are her responsibility alone. I merely give her the authorization she needs.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.82}",
			say = "Akagi's dealings with Siren technology wouldn't have been possible without your backing.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "If you already know this much,: what need is there to hear it from my mouth?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.82}",
			say = "We cannot allow Siren technology... No, the Sirens themselves, to go unchecked! Do you not have enough love and loyalty for the Sakura Empire to realize that?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "I have nothing but love and loyalty. However...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "The Sakura Empire owes the power it currently enjoys to Siren technology. Even without the Naval HQ's Reenactments, we have already become addicted to their poison.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "News of this should have reached your ears while you were in seclusion.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.82}",
			say = "I: not believe for a second that this momentary peace means the Sirens have retreated. Ever since the Iron Blood's rebellion, they've been far too quiet.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "Then, would you say the Sirens are watching us, biding their time?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "Akagi has led the Sakura Empire thus far not because of her relationship to Amagi, nor because she's actively trying to obtain Siren technology.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "It is because she has chosen the path that you, I, and the entire Sakura Empire believes in.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "What you see is an Akagi ensnared by the corrupting influence of the Sirens, just as how the Black Cube ensnared Bismarck.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "But without that power, the Sakura Empire would have no future.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.82}",
			say = "No future? You're not making any sense...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "Mikasa, you know what Reenactments are, right? As long as they continue, we will all be condemned to annihilation, just like Bismarck was.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.82}",
			say = "We cannot allow that to happen. I would gladly give my life to resist.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "Akagi said the exact same thing. You are focusing only on her madness and have lost sight of her True nature.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "If Akagi's path is wrong,: it is necessary to see the reflections of her mind and heart which paved that path.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "Akagi is not as simple-minded as you, and Zuikaku, seem to believe.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.82}",
			say = "Are you saying... that Akagi also seeks to destroy our Reenactment, just like the Iron Blood did with theirs?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "You've seen the Commander's report too, haven't you? The Sirens deployed a weapon that causes the very foundation of our hulls to tremble.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "Those scenes they showed us were not illusions. They are the basis of the Sirens' Natural Calculation System... In other words, their \"reality.\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "Shinano has seen the same dream over and over again. From that single \"reality,\" we are destroyed again and again in countless futures.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "A black tornado that seeks to devour the ocean. Horrendous beasts that defy the power of the KAN-SEN and crush our destined future... There is no escape from them.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "As long as we cannot break the recorded Reenactment that leads to their \"reality,\" we will be nothing more than ragdolls dancing in the Sirens' palms.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.82}",
			say = "All those sacrifices we made to push them back during the last war... Does none of that matter?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "What matters is... None of us can accept that reality lying:wn. Not Akagi, and not the Commander.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "The Naval HQ's... No, rather, the Sirens' Reenactments... They may be too numerous to count.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "Even if we are able to escape the flames of war, the power of those aberrant beasts will still bear:wn upon us–",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "Akagi, Kaga, Yamato, myself... All of the Sakura Empire. All the shipgirls in this world. Everything will be devoured... until even the dreamer herself meets the same fate.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "...Do you understand now, Mikasa?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.82}",
			say = "That... is not a future we can accept.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "What Akagi seeks to: is to render all Reenactments null and void, so that they are nothing more than leaves on the tree of possibility.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "As principled and compassionate as you are, your distrust towards Akagi is understandable.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "However, principles mean nothing if they cannot be defended and enforced. If you believe your path to be correct,: you must prove it to Akagi - with your strength.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "Unlike Akagi and Nagato, you are not weak. But even:, you could not bear the burden of the entire Sakura Empire atop your shoulders.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.82}",
			say = "Musashi...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "This exercise was designed to be a clash of ideals, not rhetoric.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "But in the end, isn't it too boring waxing philosophical around a chessboard?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "Make your move, Mikasa. There are those who believe in you, and in the victory they believe you will lead them to.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg4",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "Don't keep them waiting too long, Great Veteran.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
