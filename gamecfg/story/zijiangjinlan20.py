return {
	fadeOut = 1.5,
	mode = 2,
	id = "ZIJIANGJINLAN20",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_wuzang_bg11",
			stopbgm = True,
			say = "Exercise Area - Turn 6 (Musashi's Action)",
			bgm = "musashi-1",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_wuzang_bg11",
			actor = 302230,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "No hard feelings, Haguro, but the boss lady wants you outta here. Are you ready?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "bg_wuzang_bg11",
			actor = 303100,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Ugh... I knew this was gonna turn into a pain in the ass sooner or later...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_wuzang_bg11",
			actor = 303100,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I've been milling around over here for a while now, so why'd I become so important all of a sudden?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_wuzang_bg11",
			actor = 302230,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Jintsuu's pretty much:ne for, right? So we've gotta even things up a bit. You're actually not even Musashi's main target - that would be poor little Harutsuki.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_wuzang_bg11",
			actor = 303100,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Alright, guess we're:ing this... Come at me,:.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_wuzang_bg11",
			actor = 303100,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "But just a word of warning - be careful where you aim your attacks. Otherwise, you might end up taking Miyuki out.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_wuzang_bg11",
			actor = 302230,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Huh? I thought we were gonna have a fair fight! Why are you dragging Miyuki into this?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_wuzang_bg11",
			actor = 303100,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "So, what's the plan? I'm good at fighting with my back to the wall, so:n't blame me if Miyuki gets caught in the crossfire.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302230,
			side = 2,
			bgName = "bg_wuzang_bg11",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Why you...! Ugh, I can't just turn around and leave though! I have orders from Musashi!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg11",
			actor = 302230,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Miyuki,:n't let Haguro bully you! Let's team up and take her out this turn!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301040,
			side = 2,
			bgName = "bg_wuzang_bg11",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "O-okay! I'll: my best!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg11",
			actor = 303100,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Tch, guess my bluff didn't work out the way I wanted it to... Well, that sucks.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			bgName = "bg_wuzang_cg5",
			mode = 1,
			flashout = {
				black = True,
				dur = 0.1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.1,
				dur = 0.1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			sequence = {
				{
					"",
					0
				}
			}
		},
		{
			side = 2,
			stopbgm = True,
			bgName = "bg_wuzang_cg5",
			nameColor = "#A9F548FF",
			bgm = "battle-inthememory",
			say = "After her attempt to use the possibility of friendly fire to deter Sakawa failed, Haguro braced herself for a lopsided two-versus-one, when suddenly–",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "{namecode.82}",
			bgName = "bg_wuzang_cg5",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Stop right there!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_wuzang_cg5",
			say = "A voice boomed from the sky, and the three shipgirls about to clash all stopped in their tracks.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			actorName = "{namecode.226}",
			bgName = "bg_wuzang_cg5",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Huuh? What's happening to my rigging? It feels a bit wonky...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			actorName = "{namecode.65}",
			bgName = "bg_wuzang_cg5",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "I was able to move perfectly fine until a moment ago... Is this someone's special command at work?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			actorName = "{namecode.5}",
			bgName = "bg_wuzang_cg5",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "I, I can't move my rigging either... Um,:es it have to: with this ring of light that just appeared around us?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			actorName = "{namecode.65}",
			bgName = "bg_wuzang_cg5",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "That must be it... Is that you, Mikasa?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 0,
			actorName = "{namecode.82}",
			bgName = "bg_wuzang_cg5",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Hahaha, you guessed it!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_wuzang_cg5",
			say = "As the three turned towards the source of the voice, Mikasa's figure slowly appeared from the distance.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg11",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Sorry if I sound like a broken record, but Nagato:esn't want us fighting, and I:n't either.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_wuzang_bg11",
			actor = 303100,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Phew, that wouldn't have ended well for me... Was that Nagato's:ing just now?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_wuzang_bg11",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "No, that was my command. I also share the same vision as her - one in which we no longer fight amongst ourselves.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg11",
			actor = 301040,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Does that mean... You're overriding Musashi's orders?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg11",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "I guess so. Aren't you glad you:n't have to fight anymore?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_wuzang_bg11",
			actor = 301040,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Y-yeah... I was just capturing this base on Akagi's orders...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_wuzang_bg11",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "What's wrong, Sakawa? Are you going to insist on fighting?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_wuzang_bg11",
			actor = 302230,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Ahaha, no, no! I was just, uh... getting a sense of deja vu, that's all!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg11",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I:n't have anything against us sparring during a normal exercise, but it makes no sense to fight each other when the central tower is still standing.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302230,
			side = 2,
			bgName = "bg_wuzang_bg11",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Haha, I:n't mind, really. As long as this game has exciting twists and turns, I'm perfectly content to sit back and watch things unfold!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_wuzang_bg11",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I'm glad you understand. However, the same can't be said for the mindless mass-produced ships headed this way.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_wuzang_bg11",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "But, they seem to be firing indiscriminately at everything in their path... Does Musashi not care about the safety of her peers, or...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_wuzang_bg11",
			say = "As Musashi's command dissipated, the mass-produced ships also began to fall silent again. However, Jintsuu, who had already been heavily damaged, sustained enough damage to render her incapacitated.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg11",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "These were unfortunate circumstances, Jintsuu.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_wuzang_bg11",
			actor = 301040,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I'm so sorry, Jintsuu... I should've turned back to help you...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302130,
			side = 2,
			bgName = "bg_wuzang_bg11",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Don't worry about it, it's just a simulated exercise anyway. I'm not actually hurt, just... a little numb?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302130,
			side = 2,
			bgName = "bg_wuzang_bg11",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "I have to say, the technology here is pretty lifelike. But I'm sure I'll be fine as soon as I leave the exercise area.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg11",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "You:n't necessarily have to leave just because you've been determined to be incapacitated, right?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302130,
			side = 2,
			bgName = "bg_wuzang_bg11",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "That's alright. I'd just get in the way of everyone else if I lingered around without a particularly strong motive in mind.",:
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302130,
			side = 2,
			bgName = "bg_wuzang_bg11",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Honestly, Sendai probably would've been better-suited for this exercise than me. I guess I'll see you folks later.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
