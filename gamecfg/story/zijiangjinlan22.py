return {
	fadeOut = 1.5,
	mode = 2,
	id = "ZIJIANGJINLAN22",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_wuzang_bg11",
			stopbgm = True,
			say = "Exercise Area - Turn 7 (Nagato's Action)",
			bgm = "musashi-1",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301490,
			side = 2,
			bgName = "bg_wuzang_bg11",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Lady Nagato, why did Mikasa leave that base before clearing out all the enemies? Is this some sort of miscalculation?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg11",
			actor = 305050,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "No, she most likely did that on purpose.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_wuzang_bg11",
			say = "After defeating the rest of the enemies occupying the base node, Nagato looked into the distance towards Mikasa and opened a communication channel.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305050,
			side = 2,
			bgName = "bg_wuzang_bg11",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Did you leave that base for me on purpose, Mikasa?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_wuzang_bg11",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "That's right.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg11",
			actor = 305050,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Th-thank you...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305050,
			side = 2,
			bgName = "bg_wuzang_bg11",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "You protected Harutsuki until you yourself joined the battlefield... I, I cannot thank you enough.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_wuzang_bg11",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Don't worry about it. The tea party up there was making me sleepy, so I needed a change in scenery.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg11",
			paintingNoise = True,
			dir = 1,
			actor = 305110,
			nameColor = "#A9F548FF",
			say = "Since we:n't get an opportunity to experience a place like this every day, I figured it'd be nice to take advantage of the front-row seats.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_wuzang_bg11",
			actor = 305050,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Regardless, I must thank you again...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg11",
			paintingNoise = True,
			dir = 1,
			actor = 305110,
			nameColor = "#A9F548FF",
			say = "What's wrong? You sound like you're about to cry.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_wuzang_bg11",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Remember what Musashi said? This:main reflects your mental imagery and your convictions. If you:n't keep your will strong, you'll be put on the back foot.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg11",
			actor = 305050,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I just need to believe in my ideals... and capture this base without hesitation.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_wuzang_bg11",
			say = "Nagato ended the communication, repeating Mikasa's words in her head again and again, a slight sigh escaping from her lips.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg11",
			actor = 305050,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Alright... Kawakaze, Shoukaku, let's move on to the next base. We can't wait for Mikasa to serve everything to us on a silver platter.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305050,
			side = 2,
			bgName = "bg_wuzang_bg11",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Haruna, Hiei, the two of you should make preparations to attack the central base.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg11",
			actor = 305050,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Harutsuki, the two of us will go check on Sakawa and Miyuki. If you have any emergency repair supplies left, we'll give those to them.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301570,
			side = 2,
			bgName = "bg_wuzang_bg11",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Understood!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305050,
			side = 2,
			bgName = "bg_wuzang_bg11",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Focus on healing anyone who needs it. We: not make foes amongst ourselves in the Sakura Empire.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301570,
			side = 2,
			bgName = "bg_wuzang_bg11",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Are these the rest of your emergency repair supplies? Thank you, Lady Nagato! I'll be sure to put them to good use!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_wuzang_bg11",
			say = "Harutsuki waved her gohei around and offered an earnest prayer. Miraculously, some of the signs of damage on Miyuki's and Sakawa's riggings seemed to recede.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_wuzang_bg11",
			actor = 301040,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Woah... What a warm sensation... Th-thank you very much...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302230,
			side = 2,
			bgName = "bg_wuzang_bg11",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "I can feel Harutsuki's prayers healing me up! Ahh, even after everything I've:ne, you're still so kind to me, Lady Nagato!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg11",
			actor = 305050,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Sakawa, I know why Musashi sent you over and over. But no matter how many times she tries, she will not shake my resolve.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg11",
			actor = 302230,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Look, I really appreciate you giving me the benefit of the:ubt... But if Musashi orders it, I'm going to have to attack you again.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305050,
			side = 2,
			bgName = "bg_wuzang_bg11",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "We'll cross that bridge when we get there. But nothing changes the fact that you're also a member of the Sakura Empire and one of our friends.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_wuzang_bg11",
			actor = 302230,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Ahh, you're so pure-hearted it hurts to look at you! Buuut, you'd better be careful not to let others take advantage of that~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302230,
			side = 2,
			bgName = "bg_wuzang_bg11",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Musashi was right about one thing - this place definitely HAS been quite the experience. Too bad my sisters aren't here to witness it as well.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_wuzang_bg11",
			paintingNoise = True,
			dir = 1,
			actor = 305110,
			nameColor = "#A9F548FF",
			say = "Speaking of your sisters... I haven't seen Yura in a while. Do you know where she is?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg11",
			actor = 302230,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Hmm... I heard Musashi gave her some top-secret mission not too long ago, but I obviously:n't know the details beyond that...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302230,
			side = 2,
			bgName = "bg_wuzang_bg11",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Why, did you need her for something?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg11",
			paintingNoise = True,
			dir = 1,
			actor = 305110,
			nameColor = "#A9F548FF",
			say = "No, I was just curious.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg11",
			actor = 305050,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Phew... That concludes my turn.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305050,
			side = 2,
			bgName = "bg_wuzang_bg11",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "I can only hope Musashi comes to understand my intentions.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
