return {
	fadeOut = 1.5,
	mode = 2,
	id = "ZIJIANGJINLAN23",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_wuzang_bg12",
			stopbgm = True,
			say = "Exercise Area - Turn 7 (Musashi's Action)",
			bgm = "musashi-1",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_wuzang_bg12",
			say = "When Musashi's turn rolled around, Sakawa, who was still with Nagato's fleet, suddenly found herself surrounded by purple butterflies.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 303100,
			side = 2,
			bgName = "bg_wuzang_bg12",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "What is she planning to: this time...?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302230,
			side = 2,
			bgName = "bg_wuzang_bg12",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "I suppose I'll be heading towards the tower now. These butterflies seem to be patching me up and making me stronger.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_wuzang_bg12",
			actor = 302230,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Lady Nagato, I think you've gotten your point across! Hopefully we won't have to fight anymore!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302230,
			side = 2,
			bgName = "bg_wuzang_bg12",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "I was just wondering why we had to jump through so many hoops in the first place. I thought there'd be a lot more competition over the big 10-point base in the middle!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_wuzang_bg12",
			say = "Sakawa looked towards the imposing tower in the middle of the exercise area. Akagi had tried to scout it out earlier, but waves of Siren enemies made any movement around the area difficult.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_wuzang_bg12",
			say = "One after another, ships from the four fleets began to arrive around the tower.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301840,
			side = 2,
			bgName = "bg_wuzang_bg12",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "That ominous red aura was spooky enough from a distance, but up close, the thing looks like it might even be alive!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_wuzang_bg12",
			actor = 306090,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Ugh, please:n't say that. The way the light is flickering makes it seem like it's breathing...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg12",
			paintingNoise = True,
			dir = 1,
			actor = 301880,
			nameColor = "#A9F548FF",
			say = "The other four facilities here were some of the Sakura Empire's most sacred sites... So why would this Siren facility be sitting in the middle of this whole place?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302230,
			side = 2,
			bgName = "bg_wuzang_bg12",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Do you remember hearing from Akagi and the Commander that they also had to destroy some kind of tower during the Iron Blood rebellion?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg12",
			actor = 302230,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Do you think this tower represents some higher form of Siren?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_wuzang_bg12",
			actor = 306090,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "How much \"higher\" can it be if it lost the ability to move around? Chitose, let's go smash that thing up, just the two of us!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 306080,
			side = 2,
			bgName = "bg_wuzang_bg12",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "O-okay...! Um, also, I've decided on a name for our combo attack... F-Featherstorm!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_wuzang_bg12",
			actor = 302230,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Is everyone showing off the special techniques they've developed here? I guess I have no choice but to join in on the fun myself––",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_wuzang_bg12",
			actor = 302230,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Ahem! ...I am the piercer of the blue waves, inheritor of the Rakshasa's blade! True Demon Slash!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_wuzang_bg12",
			say = "Sakawa and Wakatsuki both unleashed a blade of light from their riggings, the rippling bolts of energy expanding to several times their original size as they hurtled towards the tower.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_wuzang_bg12",
			say = "Chitose and Chiyoda's combination attack also landed a direct hit on the base of the tower, a plume of flame and smoke quickly rising in the aftermath.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_wuzang_bg12",
			say = "But in a heartbeat, the tower released a nigh-intangible shockwave back at the shipgirls.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 304040,
			side = 2,
			bgName = "bg_wuzang_bg12",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "It absorbed our attacks, just like before... Except this time, the shockwaves seem to be stronger corresponding to the strength of the attacks.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 304010,
			side = 2,
			bgName = "bg_wuzang_bg12",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "That's not much of a \"counterattack\" though - it's barely enough to rock the waters. That can't be all there is to this tower.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302080,
			side = 2,
			bgName = "bg_wuzang_bg12",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Wait, there's something different about the tower now. Do you see any differences in the pattern of light coming from it?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 304040,
			side = 2,
			bgName = "bg_wuzang_bg12",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "You're right, it's not the same as before. Do you think it's reacting to the attacks we just threw at it?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg12",
			actor = 301790,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It's too early to say... But if it's like the tower the Commander encountered during that battle, there might be something nasty sleeping inside.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 304010,
			side = 2,
			bgName = "bg_wuzang_bg12",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Those weak shockwaves won't even leave a scratch on us!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_wuzang_bg12",
			say = "However, the moment Musashi's fleet stopped attacking, a blinding flash radiated out from the tip of the tower, tearing violently through the entire:main.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_wuzang_bg12",
			actor = 303100,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Wh-what in the world was that?! The tower's counterattacking for real now?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_wuzang_bg12",
			actor = 304010,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Damnit, I spoke too soon... Watch out, everyone! Something's totally different about that tower!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_wuzang_bg12",
			say = "When the shipgirls gathered their senses from the previous shockwave, they quickly noticed that Kirishima, Suzutsuki, Chiyoda, and Yamashiro were no longer present. Instead, they had been replaced by Sirens.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = False,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = False,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 900287,
			side = 2,
			bgName = "bg_wuzang_bg12",
			nameColor = "#ff5c5c",
			say = ".........",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg12",
			actor = 304010,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Kirishima... got taken out in one blow?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_wuzang_bg12",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I guess the first pulse was just a warning shot... This tower seems to throw back all the energy we pump into it, several times over...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_wuzang_bg12",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Did the Commander also have to fight against an enemy like this, or...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg12",
			actor = 301790,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Three of Musashi's retainers were taken out in a single blow... They aren't even present in the:main anymore.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_wuzang_bg12",
			actor = 301790,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "If Musashi designed this place, shouldn't she have been aware of exactly what she's up against? Why did she waste her own troops like this:?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg12",
			actor = 303100,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "G-Great Veteran, look over there at that Siren...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_wuzang_bg12",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "The Arbiter that appeared in the NA Ocean...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305050,
			side = 2,
			bgName = "bg_wuzang_bg12",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "It must be a vessel...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307050,
			side = 2,
			bgName = "bg_wuzang_bg12",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "I see. I've heard all about the Commander's exploits in the NA Ocean, but it seems these towers are guarded by Arbiters...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_wuzang_bg12",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Is this the surprise you prepared for us, Musashi?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_wuzang_bg12",
			actor = 307010,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I'll admit, I'm impressed. How did you manage to procure data on an Arbiter and even manufacture a vessel for it?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_wuzang_bg12",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Shoukaku, you remember this Siren too,:n't you? We encountered it briefly in the heart of the NA Ocean.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_wuzang_bg12",
			actor = 307010,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I won't forgive it for what it did... Making such a vile imitation of Amagi...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg12",
			actor = 307010,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Hahaha... Hahahahaha... Even if you're nothing more than a vessel, I'll make you pay!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_wuzang_bg12",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "If this is what I was invited here to:,: I will entertain you, Musashi!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
