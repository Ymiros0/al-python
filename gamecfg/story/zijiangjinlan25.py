return {
	fadeOut = 1.5,
	mode = 2,
	id = "ZIJIANGJINLAN25",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_wuzang_bg12",
			stopbgm = True,
			say = "Exercise Area - Turn 8 (Akagi's Action)",
			bgm = "musashi-2",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_wuzang_bg12",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Woaaaah... I could've sworn I was going to sink! My head's still spinning...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_wuzang_bg12",
			paintingNoise = True,
			dir = 1,
			actor = 301040,
			nameColor = "#A9F548FF",
			say = "Y-Yamashiro... Are you still able to stand...?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307020,
			side = 2,
			bgName = "bg_wuzang_bg12",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "What incredible tenacity. Is that the strength of your determination to overcome bad luck?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_wuzang_bg12",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Ehehe... I'm not sure what happened, but, I kept thinking about how I didn't want to let everyone:wn! And:, it didn't hurt anymore!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_wuzang_bg12",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Wait, huh? Are we missing some people? Where did Chiyoda and Suzutsuki go? And is Kirishima gone too?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_wuzang_bg12",
			actor = 307010,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "That tower almost took all four of you:wn in a single blow. I always had a feeling you couldn't just march right up and capture it, but I never expected something like this.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_wuzang_bg12",
			actor = 307010,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Musashi aside, I can see Nagato and Mikasa preparing to attack this tower as well.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_wuzang_bg12",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Do they think we can still work together? Or are they trying to seize victory away from me at the last moment? Not that it matters in the end.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 306070,
			side = 2,
			bgName = "bg_wuzang_bg12",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Akagi, it's your turn again.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg12",
			actor = 306070,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "After the tower's last attack, the turbulence in these waters has made it harder for certain ships to operate.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_wuzang_bg12",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "I see... Meanwhile, battleships have greater stability under these conditions, giving them a significant advantage.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_wuzang_bg12",
			actor = 307010,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "But we're hardly old-fashioned ships prone to being battered around by the waves. This won't affect how we proceed.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_wuzang_bg12",
			actor = 307020,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Akagi, should we continue attacking the tower?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg12",
			actor = 307010,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "No, we need to regroup and investigate the tower more thoroughly. It's clear that it cannot be captured through conventional means.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_wuzang_bg12",
			actor = 307010,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Everyone, get back into formation first. Miyuki, how's the situation over there?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_wuzang_bg12",
			paintingNoise = True,
			dir = 1,
			actor = 301040,
			nameColor = "#A9F548FF",
			say = "I, I'm seeing increased activity from mass-produced ships! Should I continue observing them and alert you if they're headed your way?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg12",
			actor = 307010,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Very well. You'll have safety in numbers around Mikasa's forces. I'll be in touch with you later, Miyuki.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_wuzang_bg12",
			actor = 301040,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Understood!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_wuzang_bg12",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Um... Is there anything I can:?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_wuzang_bg12",
			actor = 307010,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "You:n't look like you're in any condition to fight, so you might as well head back and try to capture some of the perimeter bases we skipped over, and see if you can find any emergency repairs.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_wuzang_bg12",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "O-okay, thank you very much! But wow, that attack from the tower was something else!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_wuzang_bg12",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "Ryuuhou, go with Yamashiro and cover her until she can patch herself up.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_wuzang_bg12",
			actor = 306070,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Yes ma'am! I haven't seen much action for a while, so it'll be a nice change of pace to go back and capture some bases.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
