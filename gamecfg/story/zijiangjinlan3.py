return {
	fadeOut = 1.5,
	mode = 2,
	id = "ZIJIANGJINLAN3",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_bg1",
			hidePaintObj = True,
			stopbgm = True,
			say = "Exercise Area - Turn 1 (Akagi's Action)",
			bgm = "musashi-1",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_bg1",
			hidePaintObj = True,
			say = "In the distance, black clouds churned unpredictably around imposing mechanical castles, a strange and unsettling scenery worthy of a Siren test site.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_bg1",
			hidePaintObj = True,
			say = "Even more bizarrely, beams of various colors of light literally \"cut\" the exercise area up into distinct grid-like sectors.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_bg1",
			hidePaintObj = True,
			say = "Each different sector had a point value that would ultimately determine victory or defeat in this exercise.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 306070,
			side = 2,
			bgName = "bg_wuzang_bg1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "What is this place...? I've never seen anything like it in any of the Mirror Seas I've encountered...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307020,
			side = 2,
			bgName = "bg_wuzang_bg1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "If I remember correctly, this was a \"gift\" that Akagi gave to Musashi. But how did she manage to turn it into something like this...?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_wuzang_bg1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 306070,
			say = "Wait, look over there... Is that the Diadem of Light? It looks completely different, but I'd never mix up that distinctive silhouette!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_wuzang_bg1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Wow, you're right! This place seems to have been built by drawing on the motifs from some of the Sakura Empire's most sacred sites!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_wuzang_bg1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Look, that place looks like our shrine... and there's also Akagi's estate over there!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_wuzang_bg1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "And is that the Sacred Sakura? ––Oh wow, even the Dragon Palace we found not too long ago is here!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_wuzang_bg1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 306070,
			say = "The great sakura tree of the Guardian Fox... I've never been there myself, but I heard Lady Nagato was once sealed within that place.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_wuzang_bg1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 306070,
			say = "That's before Zuikaku and the Reborn Combined Fleet reawakened her, anyway.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_wuzang_bg1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Yeah! I heard all about it!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_wuzang_bg1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Hmm... Now that I think about it, I've been feeling a little strange ever since coming here. I think my combat performance might be getting tweaked or something?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301040,
			side = 2,
			bgName = "bg_wuzang_bg1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Mmhmm... I think we were assigned the role of \"subordinates,\" so our specs might have been adjusted for fairness...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301040,
			side = 2,
			bgName = "bg_wuzang_bg1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Otherwise, capital ships would have an overwhelming advantage, and carriers would be able to wreak havoc from afar...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301040,
			say = "That's why I think... Our equipment and specifications aren't actually reflective of our actual performance, but are special conditions imposed in this exercise.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301040,
			say = "B-but, because of that, maybe I can finally be useful for a change...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_wuzang_bg1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I see! So that's why I can equip planes now! Heh heh, I never get to: this stuff normally so I'll have to make every moment count~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_wuzang_bg1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Thanks for the explanation, Miyuki!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "bg_wuzang_bg1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301040,
			say = "D-don't mention it, as long as I can be of use to someone...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_wuzang_bg1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "You're:ing great! I'm not sure how much of a difference I'll be able to make either, but let's both: our best, okay?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302130,
			side = 2,
			bgName = "bg_wuzang_bg1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Each and every one of us has her own role to play. In this exercise, we will not be testing each others' might, but rather making our thoughts known to each other in a clash of wits.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302130,
			side = 2,
			bgName = "bg_wuzang_bg1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "As such, we are not only responsible for how we fight, but also if and when we fight.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302130,
			side = 2,
			bgName = "bg_wuzang_bg1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "If we are to clash, our motivations and rationale may prove to be more important than the equipment we field and the tactics we employ in combat.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_wuzang_bg1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Motivations, huh? Alright! So basically, if we just try our best and think about what kind of orders we'd be given,: everything should be fine, right?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_wuzang_bg1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I've always wanted to feel what it's like to have a more active role on the battlefield, so I'm really excited to test these planes out!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_wuzang_bg1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Oh, but, since we're supposed to use our heads and not our weapons, I'll try my best not to land us into a sticky situation!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302130,
			side = 2,
			bgName = "bg_wuzang_bg1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "It's good to keep that in mind, but being overly cautious can be disadvantageous as well.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_wuzang_bg1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Eh?! How: I know what to::...?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 302130,
			side = 2,
			bgName = "bg_wuzang_bg1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Uhh, I wouldn't worry about it too much. Just trust your gut instinct.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_bg1",
			hidePaintObj = True,
			say = "Suddenly, a symbol appeared in the sky bearing the image of a lightning bolt.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305020,
			side = 2,
			bgName = "bg_wuzang_bg1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Woah, what in the world is that?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_wuzang_bg1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301040,
			say = "That... must be Akagi's special command. In other words, she's empowering us to carry out her orders...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307020,
			side = 2,
			bgName = "bg_wuzang_bg1",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Correct. \"Make haste and capture the nearest sector.\" We're moving out, everyone.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
