return {
	fadeOut = 1.5,
	mode = 2,
	id = "ZIJIANGJINLAN30",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg6",
			hidePaintObj = True,
			stopbgm = True,
			say = "Sanctuary Pagoda - Reception Room",
			bgm = "theme-shinanometa",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg6",
			hidePaintObj = True,
			say = "Lightning flashed, thunder roared. A storm had begun raging outside the pagoda.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg6",
			hidePaintObj = True,
			say = "Musashi continued to sit alone in the dark room, lit only by the holographic board to which her eyes were fixed.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg6",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "Until now, there was nothing I could: for you. But now, I shall grant your wish.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg6",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "Yes, Akagi... Your audience still awaits.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg6",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "Your tenacity, your willpower. The path of thorns you've chosen to tread that earns you nothing but scorn and distrust.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg6",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "To unleash that passion bordering on madness, you need to sever all your ties.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg6",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "For your weak self to finally become strong... I shall give you the final push you need.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg6",
			hidePaintObj = True,
			say = "An order was given. Rings of light appeared beneath Chitose and her mass-produced ships.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg6",
			hidePaintObj = True,
			say = "Wave after wave of planes began to pour out from them, the humming of engines filling the sky as the metallic flock descended upon the still-injured Hatakaze.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg6",
			hidePaintObj = True,
			say = "Musashi knew that Mikasa lacked the ability to ward off a large-scale aerial assault.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg6",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "Join forces all you'd like. But alliances that fail to produce results are worthless, nothing more than vacuous words on a page.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg6",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "In order for the Sakura Empire to move forward, our convictions must all be subject to a trial by fire, where every benefit and every flaw is brought before the light.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg6",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "Are those ideals strong enough to bear the burden of our future, or are they nothing more than self-serving whims?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg6",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "I may not agree with Akagi's ideals in their entirety, but I believe she:es deserve the opportunity to fight for them.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg6",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "Similarly, Nagato must get the opportunity to show that her will to protect is not pacifism or isolationism, but rather a way of seizing victory.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg6",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "The question of our future has never been mine to decide, nor is it even a matter of who holds on the most points on this board.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 1,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_cg6",
			hidePaintObj = True,
			dir = 1,
			actorName = "{namecode.84}",
			say = "Now show me, my friends. Let your hearts sing, and let your wills be heard.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
