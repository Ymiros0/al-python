return {
	fadeOut = 1.5,
	mode = 2,
	id = "ZIJIANGJINLAN37",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_wuzang_bg16",
			stopbgm = True,
			say = "Exercise Area - Central Sector",
			bgm = "theme-shinanometa",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 8,
			side = 2,
			bgName = "bg_wuzang_bg16",
			actor = 305100,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "What are the results of your observation?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			nameColor = "#a020f0",
			bgName = "bg_wuzang_bg16",
			side = 2,
			actor = 900331,
			say = "Watatsumi's output was stable.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			nameColor = "#a020f0",
			bgName = "bg_wuzang_bg16",
			side = 2,
			actor = 900331,
			say = "It was able to successfully reconstruct the Origin Cube, changing its structure and shape in accordance to Akagi's \"mental image.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			nameColor = "#a020f0",
			bgName = "bg_wuzang_bg16",
			side = 2,
			actor = 900331,
			say = "Though there are some restrictions, Akagi is able to safely draw energy from this cube.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 8,
			side = 2,
			bgName = "bg_wuzang_bg16",
			actor = 305100,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "That's a relief... Assuming that IS the end of the story.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			nameColor = "#a020f0",
			bgName = "bg_wuzang_bg16",
			side = 2,
			actor = 900331,
			say = "This isn't anything new. We've already conducted experiments like this before.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "bg_wuzang_bg16",
			actor = 305100,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "But this is an Origin Cube we're talking about... How will this \"power\" reshape the Cube that serves as the basis for Akagi's foundation?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			nameColor = "#a020f0",
			bgName = "bg_wuzang_bg16",
			side = 2,
			actor = 900331,
			say = "In theory, the Origin Cube eats away at the user's hull, eventually erasing them from existence. But, the speed at which this occurs differs from person to person.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			nameColor = "#a020f0",
			bgName = "bg_wuzang_bg16",
			side = 2,
			actor = 900331,
			say = "Of course, this method is much more preferable than directly tapping into the Origin Cube and disappearing like a flash in the pan. We might even be able to halt the METAmorphosis process if we cut off the flow of energy early enough.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			nameColor = "#a020f0",
			bgName = "bg_wuzang_bg16",
			side = 2,
			actor = 900331,
			say = "I must say though, Musashi, you seem to have an abundance of concern for Akagi.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 8,
			side = 2,
			bgName = "bg_wuzang_bg16",
			actor = 305100,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "In other words, this isn't a cure. It's just a stopgap measure that drags out the time you have before you lose yourself.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			nameColor = "#a020f0",
			bgName = "bg_wuzang_bg16",
			side = 2,
			actor = 900331,
			say = "Well, this was just a byproduct of our research. If we can remake our ultimate weapon using Watatsumi as its core, that would solve all of our problems.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 8,
			side = 2,
			bgName = "bg_wuzang_bg16",
			actor = 305100,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Were you a sacrifice to that research as well?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			nameColor = "#a020f0",
			bgName = "bg_wuzang_bg16",
			side = 2,
			actor = 900331,
			say = "You can figure that out yourself, can't you? ...*cough* *cough*",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "bg_wuzang_bg16",
			actor = 305100,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "The way you are now... Does that not speak volumes about the risks of using this technology?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			nameColor = "#a020f0",
			bgName = "bg_wuzang_bg16",
			side = 2,
			actor = 900331,
			say = "You misunderstand. This technology was incredibly robust by the time I utilized it. As for why my body is in this condition... that stems from a tragedy I'd rather not talk about.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			nameColor = "#a020f0",
			bgName = "bg_wuzang_bg16",
			side = 2,
			actor = 900331,
			say = "Besides, if my own METAmorphosis was progressing poorly, the two of us wouldn't be talking right now.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 8,
			side = 2,
			bgName = "bg_wuzang_bg16",
			actor = 305100,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "An \"ultimate weapon\" using Watatsumi, huh...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "bg_wuzang_bg16",
			actor = 305100,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "If you had something so powerful, how did you still lose to \"them?\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			nameColor = "#a020f0",
			bgName = "bg_wuzang_bg16",
			side = 2,
			actor = 900331,
			say = "Because we never got to use that weapon. Someone was trying to stop us from:ing so at all costs.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			nameColor = "#a020f0",
			bgName = "bg_wuzang_bg16",
			side = 2,
			actor = 900331,
			say = "...And unfortunately, they managed to succeed in the end.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			nameColor = "#a020f0",
			bgName = "bg_wuzang_bg16",
			side = 2,
			actor = 900331,
			say = "But let's not talk about that any more. The only thing that matters is that we:n't repeat the mistakes of the past.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 8,
			side = 2,
			bgName = "bg_wuzang_bg16",
			actor = 305100,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "So you've pinned your hopes on us? Or rather, on this branch? I feel like I've heard those reassuring words before, but that makes me even less inclined to trust you.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "bg_wuzang_bg16",
			actor = 305100,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "What: you really want from us, and from Akagi?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			nameColor = "#a020f0",
			bgName = "bg_wuzang_bg16",
			side = 2,
			actor = 900331,
			say = "My time is already short. I only wish to rid myself of a few lingering regrets before I fall into the abyss.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			nameColor = "#a020f0",
			bgName = "bg_wuzang_bg16",
			side = 2,
			actor = 900331,
			say = "A journey of a thousand miles begins with a single step, after all. You've taken the first step towards saving your world - but:n't take too long, lest you also run out of time.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
