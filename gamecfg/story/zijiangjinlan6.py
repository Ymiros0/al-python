return {
	fadeOut = 1.5,
	mode = 2,
	id = "ZIJIANGJINLAN6",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			stopbgm = True,
			say = "Exercise Area - Turn 1 (Musashi's Action)",
			bgm = "musashi-1",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 306090,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "So, we've got our orders from Musashi. Doesn't look like she'll be using a special command this turn either, yeah?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301840,
			say = "Waiting for our opportunity to strike is a pretty sensible decision, isn't it?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301840,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Wakatsuki,: you wanna lead the advance on the nearest base?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301880,
			say = "You've got it! Just leave it to me!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			say = "After receiving her order, Wakatsuki stepped forward from the formation of mass-produced ships, leading them towards the base on the adjacent diamond.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 302230,
			say = "Musashi has chosen a very different strategy from the other three players...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301840,
			say = "Huh? How can you tell from just the opening move?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 302230,
			say = "Well, I obviously:n't know what her endgame is, but she seems to want to capture the Dragon Palace as quickly as possible. Nobody else is positioning themselves to: that yet, right?",:
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301840,
			say = "I see. Once you capture a base, it can't be taken over by somebody else, so it makes sense to target the highest-value areas first!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301840,
			say = "The smaller bases are only worth one point each, but capturing a sacred site is worth five whole points! That'll be a huge lead!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301840,
			say = "It's still too early for there to be any clashes between the fleets, but once that happens, it'll be harder to plan ahead.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301840,
			say = "If we can capture as many points as possible before that happens, we'll have a lot more options to choose from even if the situation gets pretty chaotic!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 302230,
			say = "Right. And if we exhaust all our strength from fighting each other, it'll be harder to take:wn the Sirens guarding each base.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301840,
			say = "Well said! As long as things go smoothly, we'll also reach the central tower before anybody else! Lady Musashi's wisdom is not to be underestimated!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 302230,
			say = "S-see, I already knew all of that! That's why Musashi has chosen a strategy the others haven't grasped!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301840,
			say = "So, now that everyone has taken their first turn, who: you think is winning?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 302230,
			say = "I dunno, this is the first time any of us are playing a game like this, no?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301840,
			say = "It just occurred to me, our victory's practically guaranteed once we take the central tower, right? What's stopping everyone from just rushing straight there?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 302230,
			say = "Well, none of us would be able to get 15 points without butting heads.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301840,
			say = "Yeah. We need to be especially wary of Akagi as well. Chitose, Chiyoda, what: you two think? Akagi wouldn't just hand Musashi victory on a silver platter, would she?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 306080,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "I, I have no idea... Akagi can be kind-hearted at times and terrifying at others, as if she has two sides to her...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 306090,
			say = "Haha, there's no way shmucks like us could ever figure out what Akagi is thinking~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 302230,
			say = "That's True, but:n't you think Musashi gives off pretty similar vibes?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301840,
			say = "I can see that... Hey,: you remember what Musashi told us right before this exercise began?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900330,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Sakawa, Wakatsuki, Suzutsuki, Chitose, and Chiyoda. I trust the five of you to act as my retainers for this exercise.",
			flashout = {
				dur = 0.2,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.2,
				dur = 0.3,
				alpha = {
					1,
					0
				}
			},
			effects = {
				{
					active = True,
					name = "memoryFog"
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900330,
			side = 2,
			bgName = "bg_wuzang_bg0",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "All you need to: is to follow my orders, like pieces on a Go board.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 900330,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Simply keep your eyes in front of you. Relinquish your thoughts, and enjoy the experience about to unfold - no matter what happens.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 302230,
			say = "\"Enjoy the experience about to unfold,\" huh? The way she put it makes it seem like we're watching a movie or something.",
			flashout = {
				dur = 0.2,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.2,
				dur = 0.3,
				alpha = {
					1,
					0
				}
			},
			effects = {
				{
					active = False,
					name = "memoryFog"
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 302230,
			say = "I've participated in a lot of drills and exercises before, but this is like a completely different game with a whole other set of rules...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 302230,
			say = "It's like we stepped into an entirely new world when we entered Musashi's:main.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 306090,
			say = "Y'know what rubbed me the wrong way? How Musashi said \"no matter what happens.\" It's almost as if she's foreshadowing that something IS going to happen.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 302230,
			say = "Hmm, that:es sound a little questionable. But what's the worst that could happen? It's not like any of us could actually get hurt inside this exercise area.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 306090,
			say = "Yeah, but we're not actually playing Go here. We actually have to fight, and getting hit by blanks still hurts!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 302230,
			say = "Meh. It wouldn't be a naval exercise without some stakes.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 302230,
			say = "The one thing I can say for certain is, if anything is actually happening behind the scenes, that's for the players to decide. We're just the pieces on the board, so there's nothing we can: anyway.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 302230,
			say = "Now that I think about it, when's the last time we've seen Nagato, Mikasa, Akagi, and Musashi in one room? Quite literally the stars aligning, right?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 302230,
			say = "We've even got replicas of some of the Sakura Empire's most sacred sites here, and the design of the surrounding sectors seem to reflect the architecture of the nearest sacred site.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 302230,
			say = "There really is a lot of great scenery to soak in, if not for the dark and ominous clouds and whatnot. Who knows, maybe Musashi is really impressed with the way this place turned out and actually is just telling us to enjoy ourselves?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301840,
			say = "No matter what happens, huh... There must be some surprise at the end!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301840,
			say = "(I'm not sure if I'm overthinking this, but... when she says for us to \"enjoy the experience,\":es that mean it:esn't matter if we win or lose? What:es Musashi even stand to gain from all this?)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301840,
			say = "(I was told that this place reflects the mental state of the participants... Does that mean we'll be able to find clues as to what Musashi is thinking...?)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = True,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 301880,
			say = "Um, hello? Excuse me? I've arrived at the designated location!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301840,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Oh, Wakatsuki! Do you have visuals on the base's defender?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_wuzang_bg4",
			paintingNoise = True,
			dir = 1,
			actor = 301880,
			nameColor = "#A9F548FF",
			hidePaintObj = True,
			say = "Y-yes, I can see the target clearly! Umm, it appears to be a Tester vessel!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 302230,
			say = "A Tester model? Those aren't usually known for their combat capabilities, but I wonder what specs this one has... Let's go find out!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_wuzang_bg4",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 302230,
			say = "It's been fun chatting and all, but we've got a job to: here~!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
