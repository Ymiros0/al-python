return {
	fadeOut = 1.5,
	mode = 2,
	id = "ZOUXIANGYUANWEIZHIGE10",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_story_italy",
			hidePaintObj = True,
			stopbgm = True,
			bgmDelay = 1,
			say = "Saint Helena Island - Inside the Tribunal Facility",
			bgm = "theme-vichy-church",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			say = "After an eventful night, the ships gathered in the Tribunal facility decided on their next course of action.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 902020,
			say = "We should prioritize locating and recapturing the Crown of the Holy See.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 902020,
			say = "Even if it is symbolic in nature, we mustn't let it fall into the wrong hands.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "As I said before, it has little intrinsic value on its own, but I: share Marseillaise's sentiments.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "Without the Crown in hand, you might find a number of things more difficult to accomplish, Cardinal Richelieu.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "More importantly, we cannot ignore the new enemy that attacked us and snatched it away.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 805010,
			say = "Indeed. The reasons you've provided are sound, and I also see the importance in recapturing it.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 805010,
			say = "However, I am more worried about the Sirens and this new unknown enemy than the Crown itself.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 805010,
			say = "How did they manage to locate the secret facility housing it, and what were they planning to: with it?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 805010,
			say = "Finally, what is the relationship between the Arbiters, Infection, and the mysterious enemy? We must find the answers to these questions as soon as possible.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 902010,
			say = "Say,: you mind if I head back to the Tribunal for now?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 902010,
			say = "I mean, nobody knows what happened here yesterday except us, right?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 902010,
			say = "I can just walk in like nothing happened and be a totally badass spy,: use the Tribunal's extensive intel network to search for leads.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 903020,
			say = "Risky, but a potentially rewarding option. What: you think, Richelieu?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 805010,
			say = "That's True. I might be able to get a glimpse into the workings of the Naval HQ, as well as how things are faring back in the Iris Orthodoxy homeland...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 902010,
			say = "We're all on the same page:? In that case, I'm gonna get ready to head back! Man, being a:uble agent is gonna be sooooo much fun!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "Oh, right. Before you go, let me give you a bit more information about this secret stronghold.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "The Crown wasn't the only thing special about Saint Helena Island.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "There are a number of important Tribunal assets stored here, and I expect them to come into play sooner rather than later...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "Who knows? You might even find something that you can use to your benefit.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "But, there's a lot of ground to cover, so you may find it in your best interest to remain here a while longer.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 902010,
			say = "Ehh? Don't we have a whole bunch of people here already? Do you absolutely need me to stay as well?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 902010,
			say = "Wouldn't it be better for us to spread out and get as much info as we can?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "Sure, if you're fine with the possibility of getting your cover blown without anyone to bail you out.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 902010,
			say = "...Hmm, you: have a point.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				},
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "I'm not saying you have to stay here forever. Just know that if we can fully secure this facility for ourselves, we'll be able to use it as our base of operations.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "In the meantime, I think we've all earned ourselves some time to rest and recuperate.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 805010,
			say = "In other words, let's not get too ahead of ourselves right now.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 805010,
			say = "I'm not opposed to you returning to the Tribunal, but let's follow Le Terrible's suggestions first.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "Good. So, can we all agree on that plan?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "Ah, yes. One last thing. La Galissonnière,: you still have that \"medicine\" from the Tribunal?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "Cardinal Richelieu, the nightmares you've been seeing may be related to one of the basilica's mechanisms.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 805010,
			say = "Wait, there's medication for my nightmares...?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "It's just speculation on my part, but the nightmares began since you were exposed to the basilicas in Kerguelen, right?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "A number of the Tribunal's people sent to work in and around the facilities reported a number of strange symptoms, so they started issuing these to alleviate some of the side effects.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 902010,
			say = "Gimme a sec. I think I happen to have some on me...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			say = "La Galissonnière dug around and retrieved a tightly-sealed phial with liquid inside of it.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 902010,
			say = "Huh, I never really connected the:ts before, but this much should: the trick. Here, give it a try.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "Make that two. Leave one here for me as well.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 902010,
			say = "Huh...?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				},
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "Something wrong?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			say = "La Galissonnièr pondered for a moment, but decided not to question it. She fetched a second phial, and placed it on the table next to Le Terrible.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "Thank you very much.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_italy",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "Now onto the next topic of interest, Cardinal Richelieu––",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			stopbgm = True,
			mode = 1,
			bgmDelay = 1,
			bgm = "theme-clemenceau",
			sequence = {
				{
					"Record of a past broadcast–",
					2.5
				},
				{
					">>I have asked the enemy to cease all acts of aggression.",
					3
				},
				{
					">>It brings me no pleasure to make this decision, but there is no other choice.",
					3
				},
				{
					">>Some will feel numbness in their hearts when they hear these words.",
					3.5
				},
				{
					">>Perhaps it is relief, perhaps it is paralysis caused by the harshness of reality.",
					4
				},
				{
					">>Some may see this act as despicable, but it will be worth it in the end.",
					4.5
				}
			}
		},
		{
			mode = 1,
			sequence = {
				{
					">>Some will be outraged, and some will grieve.",
					2.5
				},
				{
					">>Some may feel wrath rise up within them, and direct it at the person who made this decision.",
					3
				},
				{
					">>Let that anger become fertile soil for resistance to take root, and take the first step towards victory.",
					3.5
				},
				{
					">>The Iris Orthodoxy's battle is over.",
					4
				},
				{
					">>But, my battle has only just begun–",
					4.5
				}
			}
		},
		{
			mode = 1,
			blackBg = True,
			effects = {
				{
					active = True,
					name = "zouxiangyuanweizhige"
				}
			},
			sequence = {
				{
					"",
					1
				}
			}
		}
	}
}
