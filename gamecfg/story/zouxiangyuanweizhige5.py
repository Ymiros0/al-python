return {
	fadeOut = 1.5,
	mode = 2,
	id = "ZOUXIANGYUANWEIZHIGE5",
	once = True,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			stopbgm = True,
			bgmDelay = 1,
			say = "Present day - Saint Helena Island",
			bgm = "level-french1",
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			say = "KABOOM––!!",
			soundeffect = "event./battle/boom2",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashN = {
				color = {
					1,
					1,
					1,
					1
				},
				alpha = {
					{
						0,
						1,
						0.2,
						0
					},
					{
						1,
						0,
						0.2,
						0.2
					},
					{
						0,
						1,
						0.2,
						0.4
					},
					{
						1,
						0,
						0.2,
						0.6
					}
				}
			},
			dialogShake = {
				speed = 0.09,
				x = 8.5,
				number = 2
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			say = "With a sudden explosion, two new figures burst onto the battlefield.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 902010,
			say = "Ahahaha! That's more like it!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				},
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 902010,
			say = "This is the kind of entertainment I've been craving!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 903020,
			say = "Now, now, La Galissonnière. You know I haven't given the order to attack, right?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				},
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 902010,
			say = "Ahaha~♪ Whoops, guess I got a bit carried away.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 805010,
			say = "Algérie? And La Galissonnière?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 902010,
			say = "Uhh, huh?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 902010,
			say = "Aw, shit... Looks like our cover just got blown. Should we go over and talk to her?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				},
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 903020,
			say = ".........*sigh*.........",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 805010,
			say = "I haven't seen the two of you since the Skybound Wall incident.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 805010,
			say = "I'm glad to see that you're both:ing well.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 903020,
			say = "Likewise, Cardinal Richelieu. It's been quite a while since that battle.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 903020,
			say = "And you seem to be:ing just fine yourself.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 902010,
			say = "What about me? I'm still as kick-ass as ever and, uhh...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 902010,
			say = "Umm...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 903020,
			say = "......",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "What's wrong, La Galissonnière? It's rare to see you at a loss for words.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 902010,
			say = "C'mon, give me some credit here... Even I know how to read a room, sometimes!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			effects = {
				{
					active = True,
					name = "speed"
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 902010,
			say = "More importantly... You... You're the real Le Terrible, right? I'm not just seeing things, right?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			effects = {
				{
					active = False,
					name = "speed"
				}
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "Real? Why wouldn't I be...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 903020,
			say = "That really is you, isn't it? Where have you been this entire time?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			effects = {
				{
					active = True,
					name = "speed"
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 903020,
			say = "When you suddenly disappeared... All this time, we could only assume the worst...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			effects = {
				{
					active = False,
					name = "speed"
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 903020,
			say = "The awful rumors were everywhere. They were saying you had already sunk...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "...Huh?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				},
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "I have been pretty busy, but my whereabouts haven't been leaked after all this time?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "You really had no idea where I was this entire time?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 902010,
			say = "Of course not! You never reached out to any of us!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			effects = {
				{
					active = True,
					name = "speed"
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "Well, yeah. The Tribunal gave me a secret mission.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			effects = {
				{
					active = False,
					name = "speed"
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 902010,
			say = "That's my point exactly! I'm also a part of the Tribunal, so isn't it worse that you didn't contact your colleagues at all?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "The Tribunal isn't a sports team, you know. Isn't it standard procedure to keep our work secret from one another?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "Just like how I have no idea why the two of you are here right now.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "So, let's hear it. What business: you have with us?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 902010,
			say = "Uhhh...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "Let's not worry about it:. More importantly, you can see the state Saint Helena Island is in right now.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "Which brings me to my proposal. Let's not worry about internal politics right now, and join up to defeat the Sirens.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 902010,
			say = "...What: you think, Algérie?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 903020,
			say = "Le Terrible brought the Cardinal here for a reason, so she should be fully aware of whatever secrets are hidden here.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 903020,
			say = "It:esn't: us any good to keep our cards too close to our chest at this point, right?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 903020,
			say = "So here's the deal. We were sent by the Tribunal for two purposes. to go to Saint Helena Island to facilitate the retrieval of the Crown of the Holy See, and to escort Marseillaise back home.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "I knew it... The same thing we were told.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "(What are you up to, Clemenceau? Why give the same task to both Iris Libre and the Vichya Dominion?)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 903020,
			say = "I see. In that case... We have no choice but to fight.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "Perhaps not, Lady Algérie.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "We both have the same objectives, no? To retrieve the Crown, and rescue Marseillaise.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 903020,
			say = "Get to the point.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "Hear me out – I'm not joking here. I actually just managed to locate Marseillaise.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "...Hmm? But, why would she be there...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				},
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "Béarn, hurry and send recon planes to the coordinates I just gave you!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "Pay attention to the cliff areas in particular! We should be able to find Marseillaise there!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_sainthelena2",
			paintingNoise = True,
			dir = 1,
			actor = 807010,
			nameColor = "#92fc63",
			hidePaintObj = True,
			say = "The cliffs...?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "I:n't know why she'd leave the secret stronghold and locate herself on such a conspicuous cliffside.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 805010,
			say = "Wait, those coordinates... That's the area the Sirens seem to be avoiding on purpose, no?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 805010,
			say = "Béarn, please be careful. We:n't know what dangers we might encounter there.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_sainthelena2",
			paintingNoise = True,
			dir = 1,
			actor = 807010,
			nameColor = "#92fc63",
			hidePaintObj = True,
			say = "Understood. I'll be in touch shortly, Cardinal.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_story_sainthelena2",
			paintingNoise = True,
			dir = 1,
			actor = 807010,
			nameColor = "#92fc63",
			hidePaintObj = True,
			say = "Cardinal Richelieu, my recon planes have detected an anomalous situation.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = True,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = True,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_sainthelena2",
			paintingNoise = True,
			dir = 1,
			actor = 807010,
			nameColor = "#92fc63",
			hidePaintObj = True,
			say = "It seems that your premonitions were right on the mark.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_sainthelena2",
			paintingNoise = True,
			dir = 1,
			actor = 807010,
			nameColor = "#92fc63",
			hidePaintObj = True,
			say = "The area indicated by the coordinates is steeped in a black-and-red haze, similar to the Infection encountered on Kerguelen.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_story_sainthelena2",
			paintingNoise = True,
			dir = 1,
			actor = 807010,
			nameColor = "#92fc63",
			hidePaintObj = True,
			say = "There also seems to be an enemy force inside the haze– Wait, it can't be...",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_sainthelena2",
			paintingNoise = True,
			dir = 1,
			actor = 807010,
			nameColor = "#92fc63",
			hidePaintObj = True,
			say = "There are unidentified enemies approaching from land, air, and sea! And Marseillaise seems to already be locked in combat with them!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 805010,
			say = "Unidentified? Are they not Sirens?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_story_sainthelena2",
			paintingNoise = True,
			dir = 1,
			actor = 807010,
			nameColor = "#92fc63",
			hidePaintObj = True,
			say = "That's correct... I can't identify their shapes at all through my planes' cameras... I've never seen anything like it before.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "(An unidentified enemy... appearing alongside a strange haze...)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "(Is that why Marseillaise decided to leave the stronghold?)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "(In any case, if these enemies make their way into the mountains, we'll completely lose control over the situation.)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 805010,
			say = "Le Terrible,: you know anything about these unidentified enemies?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "My apologies, but I've never seen or heard of them before.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "But it seems possible that the Sirens didn't deploy their troops here to avoid a confrontation with these... things.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "What we: know, however, is that Marseillaise is in danger. Algérie, La Galissonnière, let's work out the details later. If we:n't act now, we'll all fail our missions.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "Richelieu, please bring the main fleet along and follow me.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			effects = {
				{
					active = True,
					name = "speed"
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 903020,
			say = "...I see. I suppose rescuing Marseillaise is our top priority for now.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			effects = {
				{
					active = False,
					name = "speed"
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "As long as you understand. Let's get going.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#ffff4d",
			actor = 903020,
			say = "Very well. La Galissonnière, come with me.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_sainthelena2",
			hidePaintObj = True,
			dir = 1,
			nameColor = "#92fc63",
			actor = 805010,
			say = "Remember, we're fighting against an unknown foe. Don't take any unnecessary risks, and bring Marseillaise back safely!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			effects = {
				{
					active = True,
					name = "speed"
				}
			}
		}
	}
}
