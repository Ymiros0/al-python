...
class ALJsonAPI:
	...
	def replace_namecode(self, inputstring: str, client: Client) -> str:
		...
		for namecode_string, namecode_num in results:
			...
			censoredstring = inputstring.replace(namecode_string, code.code)
			inputstring = inputstring.replace(namecode_string, code.name)
		return inputstring, censoredstring

	def find_namecode(self, inputstring: str, client: Client) -> str:
		"""
		Replaces a name with the respective code for *client*.
		"""
		name_code = self.get_sharecfgmodule('name_code')
		results = name_code.all_client(client)
		for i in results:
			# find the entry with the same name for *client*
			if i.name == inputstring:
				censoredstring = i.code
				break
		else:
			return
