...
def var_0_0.OnRegister(arg_3_0):
	onButton(arg_3_0, arg_3_0.stampBtn, function()
		getProxy(TaskProxy).dealMingshiTouchFlag(7), SFX_PANEL)
	onButton(arg_3_0, arg_3_0.shareBtn, function()
		pg.ShareMgr.GetInstance().Share(pg.ShareMgr.TypeBackyard, pg.ShareMgr.PANEL_TYPE_PINK), SFX_PANEL)
	onButton(arg_3_0, arg_3_0.granaryBtn, function()
		arg_3_0.emit(CourtYardMediator.GO_GRANARY), SFX_PANEL)
	onButton(arg_3_0, arg_3_0.shopBtn, function()
		arg_3_0.emit(CourtYardMediator.GO_SHOP), SFX_PANEL)
	onButton(arg_3_0, arg_3_0.decorateBtn, function()
		arg_3_0.emit(CourtYardMediator.OPEN_DECORATION), SFX_PANEL)
	onButton(arg_3_0, arg_3_0.templateBtn, function()
		if LOCK_BACKYARD_TEMPLATE:
			return

		arg_3_0.emit(CourtYardMediator.GO_THEME_TEMPLATE), SFX_PANEL)
	onButton(arg_3_0, arg_3_0.trainBtn, function()
		if arg_3_0.contextData.floor == 1:
			arg_3_0.emit(CourtYardMediator.SEL_TRAIN_SHIP)
		elif arg_3_0.contextData.floor == 2:
			arg_3_0.emit(CourtYardMediator.SEL_REST_SHIP), SFX_PANEL)
...
def var_0_0.OnFlush(arg_12_0, arg_12_1):
	...
	if bit.band(arg_12_1, BackYardConst.DORM_UPDATE_TYPE_UPDATEFOOD) > 0 or bit.band(arg_12_1, BackYardConst.DORM_UPDATE_TYPE_USEFOOD) > 0 or bit.band(arg_12_1, BackYardConst.DORM_UPDATE_TYPE_EXTENDFOOD) > 0:
		arg_12_0.CalcStockLeftTime()

	if bit.band(arg_12_1, BackYardConst.DORM_UPDATE_TYPE_SHIP) > 0:
		arg_12_0.CalcStockLeftTime()
		arg_12_0.UpdateTrainBtn()

	if bit.band(arg_12_1, BackYardConst.DORM_UPDATE_TYPE_LEVEL) > 0:
		SetActive(arg_12_0.templateBtn, not LOCK_BACKYARD_TEMPLATE)

		if not LOCK_BACKYARD_TEMPLATE:
			arg_12_0.PlayBackYardThemeTemplate()
			SetActive(arg_12_0.templateBtn, var_12_0.IsMaxLevel() and arg_12_0.IsInner())

	if bit.band(arg_12_1, BackYardConst.DORM_UPDATE_TYPE_UPDATEFOOD) > 0 or bit.band(arg_12_1, BackYardConst.DORM_UPDATE_TYPE_LEVEL) > 0 or bit.band(arg_12_1, BackYardConst.DORM_UPDATE_TYPE_USEFOOD) > 0 or bit.band(arg_12_1, BackYardConst.DORM_UPDATE_TYPE_EXTENDFOOD) > 0:
...
def var_0_0.PlayBackYardThemeTemplate(arg_13_0):
	if getProxy(DormProxy).getRawData().IsMaxLevel() and not pg.NewStoryMgr.GetInstance().GetPlayedFlag(90021):
...
def var_0_0.UpdateTrainBtn(arg_14_0):
	if arg_14_0.contextData.floor == 1:
	...
	elif arg_14_0.contextData.floor == 2:
...
def var_0_0.UpdateShopTip(arg_15_0):
	setActive(arg_15_0.shopTip, getProxy(SettingsProxy).IsTipNewTheme() or getProxy(SettingsProxy).IsTipNewGemFurniture())

def var_0_0.OnRemoveLayer(arg_16_0, arg_16_1):
	if arg_16_1 == NewBackYardShopMediator:
		arg_16_0.UpdateShopTip()

def var_0_0.CalcStockLeftTime(arg_17_0):
	...
	if var_17_0.GetStateShipCnt(Ship.STATE_TRAIN) <= 0 or var_17_0.food <= 0:
		return
	...
	arg_17_0.timer = Timer.New(function()
		...
		if var_18_0 <= 0:
			arg_17_0.RemoveTimer(), 1, -1)

	arg_17_0.timer.Start()
	arg_17_0.timer.func()

def var_0_0.RemoveTimer(arg_19_0):
...
def var_0_0.UpdateFloor(arg_21_0, arg_21_1):
