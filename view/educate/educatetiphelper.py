...
def var_0_0.GetSystemUnlockTips():
	...
	for iter_1_0, iter_1_1 in pairs(var_0_0.system_tip_list):
		if not (PlayerPrefs.GetInt(var_1_0 .. var_0_0.system_save_key .. iter_1_0, 0) == 1) and EducateHelper.IsSystemUnlock(iter_1_0):
			table.insert(var_1_1, iter_1_0)

	return var_1_1

def var_0_0.SaveSystemUnlockTip(arg_2_0):
	local var_2_0 = getProxy(PlayerProxy).getRawData().id

	PlayerPrefs.SetInt(var_2_0 .. var_0_0.system_save_key .. arg_2_0, 1)
	PlayerPrefs.Save()

def var_0_0.ClearSystemUnlockTips():
	local var_3_0 = getProxy(PlayerProxy).getRawData().id

	for iter_3_0, iter_3_1 in pairs(var_0_0.system_tip_list):
		local var_3_1 = var_3_0 .. var_0_0.system_save_key .. iter_3_0

		if PlayerPrefs.HasKey(var_3_1):
...
for iter_0_0, iter_0_1 in ipairs(pg.child_site.all):
	if pg.child_site[iter_0_1].type == 1:
		table.insert(var_0_0.needTipSiteIds, iter_0_1)

def var_0_0.GetSiteUnlockTipIds():
	if not getProxy(EducateProxy).IsFirstGame():
		return {}
	...
	for iter_4_0, iter_4_1 in ipairs(var_0_0.needTipSiteIds):
		if not (PlayerPrefs.GetInt(var_4_0 .. var_0_0.site_save_key .. iter_4_1, 0) == 1) and EducateHelper.IsSiteUnlock(iter_4_1, True):
			table.insert(var_4_1, iter_4_1)
			var_0_0.SetNewTip(var_0_0.NEW_SITE, iter_4_1)

	return var_4_1

def var_0_0.SaveSiteUnlockTipId(arg_5_0):
	local var_5_0 = getProxy(PlayerProxy).getRawData().id

	PlayerPrefs.SetInt(var_5_0 .. var_0_0.site_save_key .. arg_5_0, 1)
	PlayerPrefs.Save()

def var_0_0.ClearSiteUnlockTipIds():
	local var_6_0 = getProxy(PlayerProxy).getRawData().id

	for iter_6_0, iter_6_1 in ipairs(pg.child_site.all):
		local var_6_1 = var_6_0 .. var_0_0.site_save_key .. iter_6_1

		if PlayerPrefs.HasKey(var_6_1):
...
for iter_0_2, iter_0_3 in ipairs(pg.child_plan.all):
	if #pg.child_plan[iter_0_3].pre > 0:
		table.insert(var_0_0.needTipPlanIds, iter_0_3)

def var_0_0.GetPlanUnlockTipIds():
	local var_7_0 = getProxy(PlayerProxy).getRawData().id
	local var_7_1 = {}
	local var_7_2 = getProxy(EducateProxy).GetPlanProxy()

	for iter_7_0, iter_7_1 in ipairs(var_0_0.needTipPlanIds):
		if not (PlayerPrefs.GetInt(var_7_0 .. var_0_0.plan_save_key .. iter_7_1, 0) == 1):
			local var_7_3 = pg.child_plan[iter_7_1].pre

			if var_7_2.GetHistoryCntById(var_7_3[1]) >= var_7_3[2]:
				table.insert(var_7_1, iter_7_1)

	return var_7_1

def var_0_0.SavePlanUnlockTipId(arg_8_0):
	local var_8_0 = getProxy(PlayerProxy).getRawData().id

	PlayerPrefs.SetInt(var_8_0 .. var_0_0.plan_save_key .. arg_8_0, 1)
	PlayerPrefs.Save()

def var_0_0.ClearPlanUnlockTipIds():
	local var_9_0 = getProxy(PlayerProxy).getRawData().id

	for iter_9_0, iter_9_1 in ipairs(var_0_0.needTipPlanIds):
		local var_9_1 = var_9_0 .. var_0_0.plan_save_key .. iter_9_1

		if PlayerPrefs.HasKey(var_9_1):
			PlayerPrefs.DeleteKey(var_9_1)
			PlayerPrefs.Save()

def var_0_0.ClearAllRecord():
...
def var_0_0.SetNewTip(arg_11_0, arg_11_1):
	...
	if PlayerPrefs.GetInt(var_11_0 .. var_11_2, 0) == 1:
		return

	PlayerPrefs.SetInt(var_11_0 .. var_11_2, 1)
	PlayerPrefs.Save()

def var_0_0.IsShowNewTip(arg_12_0, arg_12_1):
...
def var_0_0.ClearNewTip(arg_13_0, arg_13_1):
	...
	if PlayerPrefs.HasKey(var_13_3):
