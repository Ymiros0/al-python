...
def var_0_0.onTechnologyNotify():
	local var_7_0 = getProxy(TechnologyProxy).getPlanningTechnologys()

	return #var_7_0 > 0 and var_7_0[#var_7_0].isCompleted()

def var_0_0.onBlueprintNotify():
	local var_8_0 = getProxy(TechnologyProxy)

	if PlayerPrefs.GetString("technology_day_mark", "") != pg.TimeMgr.GetInstance().CurrentSTimeDesc("%Y/%m/%d", True) and var_8_0.CheckPursuingCostTip():
		return True
	...
	if not var_8_2:
		...
		if var_8_2.getState() == ShipBluePrint.STATE_DEV_FINISHED:
			return True
...
def var_0_0.handleNotification(arg_12_0, arg_12_1):
	...
	if var_12_0 == PlayerProxy.UPDATED:
